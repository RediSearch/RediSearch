...
def handle_redisearch(pr: PullRequest, versions: list[dict], version_text: str,
    ...
    return [
        Lookup(rs, "RediSearch/version-branch -> exact RediSearch vX.Y.Z",
               find_release_exact(versions, rs)),
        Lookup(oss["name"] if oss else f"Open Source {X}.{Y}",
               "RediSearch/version-branch -> unreleased Open Source X.Y", oss),
    ]
