...
- **DocIdMeta reclamation in memory mode (MOD-17109 review).** Disk mode GCs
  ...
  - *Logical de-index* (surviving key stops matching: FILTER change, rename out
    of prefix, REPLACE) — `DocIdMeta_DeleteWithOpenKey` now resets the KeyMeta
    value (`SetKeyMeta(key, 0)`) and frees the per-key dict when its last entry
    is removed, so a de-indexed-but-surviving key retains no DocIdMeta dict.
    `SetKeyMeta` does not free the previous value and Redis skips the free
    callback once `meta == reset_value(0)`, so the order is reset-then-free. The
    current Redis module API does not remove/shrink an existing KeyMeta slot; it
