...
FT.SEARCH {index} {query} [NOCONTENT] [VERBATIM] [NOSTOPWORDS] [WITHSCORES] [WITHPAYLOADS] [WITHSORTKEYS]
  ...
  [MULTISORTBY {nargs} {property} [ASC|DESC] ... [MAX {num}]]
  [LIMIT offset num]
```
...
- **MULTISORTBY {nargs} {property} {ASC|DESC} [MAX {num}]**: Sort the pipeline up until the point of MULTISORTBY,
  ...
  `MULTISORTBY 4 @foo ASC @bar DESC`.

    `MAX` is used to optimized sorting, by sorting only for the n-largest elements. Although it is not connected to `LIMIT`, you usually need just `MULTISORTBY … MAX` for common queries.
