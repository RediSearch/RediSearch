...
Use the same title format as the manual `/pr-backport` skill in
`.skills/pr-backport/SKILL.md`: `[<TARGET>] <original PR title>`. Original
RediSearch PR titles are conventionally `[MOD-XXXX] <summary>` (see
CLAUDE.md), so the rendered backport title is e.g. `[8.6] [MOD-15720]
<summary>` — the version, the ticket, and the summary in that order, with
no further rewriting required from you.
...
benchmark, CI, release, and routing labels that usually don't apply on a
release-branch backport, and bringing them in is noise. The two
`auto-backport*` labels above are the only labels you should touch.
...
## Notify the original PR

After printing the summary to stdout (above), **post the same summary as a
comment on the original PR** so the author and reviewers can see the backport
results without digging through Actions logs. The plain-text block above is for
the workflow log; the PR comment should be GitHub-flavored markdown with one row
per target. Render `clean` and `conflicts(N)` rows as links to the opened
backport PR; render `skipped` rows as plain text with the reason.

```bash
gh pr comment "${PR}" --body-file <(cat <<'EOF'
🤖 Auto-backport summary

| Target | Status | Backport PR |
|---|---|---|
| `8.8` | clean | #9999 |
| `8.6-rse` | conflicts(2) | #10000 — see Conflict Log in the PR body |
| `8.6` | conflicts(2) | #10001 — see Conflict Log in the PR body |
| `8.2` | skipped | Non-mechanical conflict in `src/rdb.c` — manual backport required |

(Summary generated by `.github/codex/prompts/pr-backport-auto.md`.)
EOF
)
```

`${PR}` is the **original** PR number from the context JSON (`.pr`), not any
backport PR you opened. Post exactly one comment per run, covering every
target you processed. If a target was preempted by a pre-flight check
(`already open`, `already merged`, `no such branch`), include it in the
table with the relevant `skipped`-style row so the reviewer sees the full
state in one place.
