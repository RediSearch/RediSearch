...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
    if [[ ! -d ${BOOST_DIR} ]]; then
        ...
        _dry_line "BOOST_URL=\"${BOOST_URL}\""
        _dry_line "BOOST_TARBALL=\"${_bd}/${BOOST_NAME}.tar.gz\""
        _dry_line 'for attempt in 1 2 3 4 5; do'
        _dry_line '    wget -T 60 "$BOOST_URL" -O "$BOOST_TARBALL" && break'
        _dry_line '    if [[ "$attempt" == 5 ]]; then'
        _dry_line '        echo "Boost download failed after $attempt attempts" >&2'
        _dry_line '        exit 1'
        _dry_line '    fi'
        _dry_line '    echo "Boost download failed (attempt $attempt), retrying in 10s..." >&2'
        _dry_line '    sleep 10'
        _dry_line 'done'
        _dry_line "tar -xzf \"\$BOOST_TARBALL\" -C \"${_bd}\""
        _dry_line "mv \"${_bd}/boost-${VERSION}\" \"${_bd}/${BOOST_DIR}\""
        _dry_line "rm \"\$BOOST_TARBALL\""
    fi
    return 0 2>/dev/null || exit 0
fi
