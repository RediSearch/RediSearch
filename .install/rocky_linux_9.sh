...
$MODE dnf install -y gcc-toolset-13-gcc gcc-toolset-13-gcc-c++ make wget git \
    openssl openssl-devel which rsync unzip clang curl clang-devel llvm-libs --nobest --skip-broken --allowerasing
...
# Ensure libclang is available for bindgen
$MODE dnf install -y llvm-libs 2>/dev/null || true
