...
<div class="wrap">
  ...
  <div class="card">
    <h2>C — serial overhead vs WINDOW</h2>
    <p class="sub" id="c-sub"></p>
    <div class="legend" id="legend-c"></div>
    <div class="panels" id="chart-c"></div>
  </div>
...
(function () {
  ...
  if (c6 && c0) {
    ...
    tiles.appendChild(tile("C per fused doc", fmt(1000 * c6.c / (wBig + wBig / 2), 1) + " µs",
      "≈ stable across sizes and windows — the merger's unit cost"));
    tiles.appendChild(tile("C vs one branch", fmt(100 * c6.c / c6.maxBr, 0) + "%",
      `at WINDOW=${wBig} the fusion stage costs ~one entire subquery`));
  }
...
function renderC() {
  document.getElementById("c-sub").textContent =
    `fields=none (pinned). C = hybrid mean − ${state.workers === 0 ? "(search + vsim)" : "max(search, vsim)"} at workers=${state.workers}. ` +
    `Left: absolute ms. Right: normalized per fused doc (WINDOW + K) — flat ⇒ the merger's cost is per-document.`;
  legend("legend-c", SIZES.map((s, i) => ({ name: sizeName(s) + " docs", v: SIZE_V[i] })), true);
  const box = document.getElementById("chart-c"); box.textContent = "";
  const mk = norm => SIZES.map((s, i) => ({
    name: sizeName(s), color: css(SIZE_V[i]),
    values: WINDOWS.map(w => {
      const c = cOf(s, w, state.workers, "none");
      if (!c) return null;
      return norm ? 1000 * c.c / (w + w / 2) : c.c;
    }),
  }));
  let p = panelBox(box, "C (ms)", "");
  p.appendChild(linePanel(mk(false), WINDOWS.map(String), { xname: "WINDOW", unit: " ms" }));
  p = panelBox(box, "C per fused doc (µs)", "");
  p.appendChild(linePanel(mk(true), WINDOWS.map(String), { xname: "WINDOW", unit: " µs" }));
}
...
function render() { syncW(); syncF(); renderC(); renderMain(); renderGain(); }
render();
