...
<div class="wrap">
  ...
  <div class="card">
    <h2 id="merger-title">Merger overhead C</h2>
    <p class="sub" id="c-sub"></p>
    <div class="legend" id="legend-c"></div>
    <div class="panels" id="chart-c-top"></div>
    <p class="sub" id="c-sub2" style="margin-top:14px"></p>
    <div class="panels" id="chart-c-matches"></div>
  </div>

  <div class="card">
    <h2 id="tail-title">Tail latency vs selectivity</h2>
    <p class="sub" id="tail-sub"></p>
    <div class="legend" id="legend-t"></div>
    <div class="panels" id="chart-tail"></div>
  </div>
  ...
  <div class="card">
    <h2>Parallelism gain — p50(workers=0) / p50(workers=6)</h2>
    <p class="sub">Uses both workers settings (the Workers filter does not apply here; Fields does). Hairlines at 1× and 2×: mirrors are single queries (≈1×); hybrids' ceiling with equal overlapping branches is 2×, Amdahl-capped by C.</p>
    <div class="panels" id="chart-gain"></div>
  </div>
...
const state = { workers: WORKERS[0], fields: FIELDS[0] };
...
function cOf(size, window, workers, fields) {
  ...
  const branch = workers === 0 ? se.mean_ms + vs.mean_ms : Math.max(se.mean_ms, vs.mean_ms);
  return { c: hy.mean_ms - branch, hy: hy.mean_ms, branch,
           maxBr: Math.max(se.mean_ms, vs.mean_ms), fused: window + (row1({size,window,workers,fields,contender:"hybrid_linear"})||{}).k };
}
...
(function () {
  ...
  const c6 = cOf(big, wBig, 6, "none"), c0 = cOf(big, wBig, 0, "none");
  if (c6 && c0) {
    tiles.appendChild(tile(`C at WINDOW=${wBig}`, fmt(c6.c, 1) + " ms",
      `serial fusion overhead (${sizeName(big)}-doc corpus, workers=6, fields=none); w0 derivation agrees: ${fmt(c0.c, 1)} ms`));
    tiles.appendChild(tile("C per fused doc", fmt(1000 * c6.c / (wBig + wBig / 2), 1) + " µs",
      `C / (WINDOW+K) at WINDOW=${wBig}`));
...
const syncW = segControl(document.getElementById("f-workers"),
  WORKERS.map(w => ({ name: String(w), value: String(w) })), () => state.workers, v => state.workers = Number(v));
const syncF = segControl(document.getElementById("f-fields"),
  FIELDS.map(f => ({ name: f, value: f })), () => state.fields, v => state.fields = v);
...
function renderMerger() {
  document.getElementById("merger-title").textContent =
    `Merger overhead C — workers=${state.workers}, fields=none (pinned)`;
  document.getElementById("c-sub").textContent =
    `C = what FT.HYBRID pays on top of its branches: hybrid mean − ` +
    `${state.workers === 0 ? "(search + vsim), branches run sequentially at workers=0" : "max(search, vsim), branches overlap at workers=6"}. ` +
    `Checked here: (left) how C varies with WINDOW; (right) C relative to the slowest branch (hairline = 100%).`;
  legend("legend-c", SIZES.map((s, i) => ({ name: sizeName(s) + " docs", v: SIZE_V[i] })), true);

  const top = document.getElementById("chart-c-top"); top.textContent = "";
  const series = SIZES.map((s, i) => ({
    name: sizeName(s), color: css(SIZE_V[i]),
    values: WINDOWS.map(w => { const c = cOf(s, w, state.workers, "none"); return c ? c.c : null; }),
  }));
  let p = panelBox(top, "C by WINDOW (ms) — one line per corpus size", "");
  p.appendChild(linePanel(series, WINDOWS.map(String), { xname: "WINDOW", unit: " ms" }));

  const groups = WINDOWS.map(w => ({
    label: "W=" + w,
    cols: SIZES.map((s, i) => {
      const c = cOf(s, w, state.workers, "none");
      ...
      return { value: pct, color: css(SIZE_V[i]),
        tooltip: [{ text: fmt(pct, 0) + "% of the slowest branch" },
                  ...
                  { text: `C ${fmt(c.c)} ms vs slowest branch ${fmt(c.maxBr)} ms · W=${w}` }] };
    }),
  }));
  p = panelBox(top, "C as % of max(search, vsim)", "");
  p.appendChild(columnPanel(groups, { refLines: [100] }));

  document.getElementById("c-sub2").textContent =
    "Below: the same C at a FIXED window while the text query's |matches| varies with the corpus — " +
    "checks whether C depends on selectivity or only on the fused-set size (WINDOW+K).";
  const bot = document.getElementById("chart-c-matches"); bot.textContent = "";
  WINDOWS.forEach(w => {
    const pts = SIZES.map((s, i) => ({ s, i, m: matchesOf(s, w), c: cOf(s, w, state.workers, "none") }))
      .filter(x => x.m != null && x.c != null)
      .sort((a, b) => a.m - b.m);
    const pb = panelBox(bot, `WINDOW=${w} — C (ms) vs |matches|`, "");
    pb.appendChild(linePanel(
      [{ name: "C", color: css("--s-hyblin"), values: pts.map(x => x.c.c) }],
      pts.map(x => fmt(x.m, 0)),
      { xname: "|matches|", unit: " ms" }));
  });
}

function renderTail() {
  document.getElementById("tail-title").textContent =
    `Tail inflation (p99 / p50) at fixed WINDOW — workers=${state.workers}, fields=${state.fields}`;
  document.getElementById("tail-sub").textContent =
    "Each panel holds WINDOW fixed while |matches| varies with the corpus — checks whether " +
    "selectivity affects FT.HYBRID's tail inflation at a fixed window. SEARCH branch shown as the " +
    "no-merger reference.";
  legend("legend-t", [CONTENDERS[0], CONTENDERS[2]]);
  const box = document.getElementById("chart-tail"); box.textContent = "";
  WINDOWS.forEach(w => {
    const p = panelBox(box, `WINDOW=${w}`, "");
    const groups = SIZES.map(size => ({
      label: sizeName(size) + " (" + fmt(matchesOf(size, w) || 0, 0) + ")",
      cols: ["hybrid_linear", "search_branch"].map(key => {
        const c = CONTENDERS.find(x => x.key === key);
        const r = row1({ contender: key, size, window: w, workers: state.workers, fields: state.fields });
        if (!r) return { value: 0, color: css(c.v), tooltip: [{ text: "no data" }] };
        const ratio = r.p99_ms / r.p50_ms;
        return { value: ratio, color: css(c.v),
          tooltip: [{ text: `p99/p50 = ${fmt(ratio, 2)}` }, { text: c.name, color: css(c.v) },
                    { text: `p50 ${fmt(r.p50_ms)} · p99 ${fmt(r.p99_ms)} ms · ${sizeName(size)} docs · W=${w}` }] };
      }),
    }));
    p.appendChild(columnPanel(groups, { refLines: [1] }));
  });
}

function renderMain() {
  document.getElementById("t2").textContent =
    `p50 latency by contender — workers=${state.workers}, fields=${state.fields}`;
  ...
  WINDOWS.forEach(w => {
    ...
    const groups = SIZES.map(size => ({
      ...
      cols: CONTENDERS.map(c => {
        const r = row1({ contender: c.key, size, window: w, workers: state.workers, fields: state.fields });
        if (!r) return { value: 0, color: css(c.v), tooltip: [{ text: "no data" }] };
        return { value: r.p50_ms, color: css(c.v),
          ...
          tooltip: [{ text: `${fmt(r.p50_ms)} ms p50` }, { text: c.name, color: css(c.v) },
                    { text: `${sizeName(size)} docs · W=${w} · workers=${state.workers} · fields=${state.fields}` },
...
function renderGain() {
  const box = document.getElementById("chart-gain"); box.textContent = "";
  WINDOWS.forEach(w => {
    const p = panelBox(box, `WINDOW=${w}`, "");
    const groups = SIZES.map(size => ({
      label: sizeName(size) + " docs",
      cols: CONTENDERS.map(c => {
        const a = row1({ contender: c.key, size, window: w, workers: 0, fields: state.fields });
        const b = row1({ contender: c.key, size, window: w, workers: 6, fields: state.fields });
        if (!a || !b) return { value: 0, color: css(c.v), tooltip: [{ text: "no data" }] };
        const gain = a.p50_ms / b.p50_ms;
        return { value: gain, color: css(c.v),
          dlabel: c.key === "hybrid_linear" ? "×" + fmt(gain, 2) : null,
          tooltip: [{ text: `×${fmt(gain, 2)} faster with workers` }, { text: c.name, color: css(c.v) },
                    { text: `${sizeName(size)} docs · W=${w} · fields=${state.fields}` }] };
      }),
    }));
    p.appendChild(columnPanel(groups, { refLines: [1, 2] }));
  });
}
...
function render() { syncW(); syncF(); renderMerger(); renderMain(); renderTail(); renderGain(); }
render();
