...
D's query is in its spec, not in a CSV, so nothing there is inferred. Two properties of it
are load-bearing and were checked against the pipeline builder:

- `LOAD *` leaves `nkeys == 0`, which is exactly the condition
  `rploaderNew_setLoadOpts` uses to set `load_all = true`, and `processLoadStep` always
  creates a loader when `PLN_F_LOAD_ALL` is set. So the `load_all` branch of
  `rpLoader_loadDocument` — the injection site — is always reached.
- `LOAD *` appears **before** `SORTBY … MAX 1000`, and `buildPipeline` walks plan steps in
  list order, so the loader is upstream of the sorter and runs on every matching row rather
  than on the surviving 1000. This is a property of the query text, not of the engine: a
  spec written `SORTBY … LOAD *` would invert it and gut the fixture.
- Both loader flavours route through the patched function: `RPLoader_New` picks
  `RPSafeLoader_New` under `QEXEC_F_RUN_IN_BACKGROUND` and `RPPlainLoader_New` otherwise, and
  `rpSafeLoader_Load` calls the same `rpLoader_loadDocument`. `oss-standalone` uses the plain
  loader; the fixture would still work on `oss-standalone-threads-6`.
...
result is discarded. LLVM removes exactly that. A fixture that compiled down to the baseline
would report "flat", and we would conclude the detector missed nothing when in fact there
was nothing to detect.

**Anchor the inputs, not the output.** The first cut of these fixtures put a
`std::hint::black_box` on the *result* of each redundant pass, which is the obvious move and
is not sufficient. Measured on the exact pattern (`rustc -O`, one `#[inline(always)]` decode
helper, counting multiply instructions in the emitted assembly):

| shape | multiplies |
|---|---|
| one pass | 7 |
| three passes, no anchor | 7 |
| three passes, `black_box` on the output | **7** |
| three passes, `black_box` on the read position | 21 |
| two passes, `black_box` on the read position | 14 |

Anchoring the output keeps the *stores* — the assembly shows one block of arithmetic
followed by repeated `str` — but LLVM still folds the arithmetic and loads into a single
copy, because the passes have provably identical inputs. Making each pass start from a
`black_box`'d read position removes that proof and restores the full multiplier. So every
Rust injection here anchors an input.

The C injections need no anchor: their inner calls (`RLookupRow_Get`, `RSValue_Hash`,
`RLookup_LoadDocumentAll`) are all Rust `extern "C"` symbols, opaque to the translation unit
and to any C-side LTO, so they cannot be folded — the `static volatile uint64_t` sink is
belt and braces. Likewise `aggregate-groupby-small`'s `rm_malloc`/`rm_free` pair is an
indirect call through `RedisModule_Alloc`.

This is unashamedly not code anyone would merge, which is the right trade — these branches
exist to carry a cost, not to look plausible in review.

### Tier `obvious` — multiply the dominant per-document work
...
- **B** `inverted_index/src/codec/numeric.rs` — in the shared `decode`, decode each numeric
  record 3x, keeping the last. Sits on the decode, not on `value_in_range`: the predicate is
  two float comparisons against an already-decoded value, a small enough slice of a numeric
  scan that tripling it would not land in the `obvious` band. Same file as B-small, which
  does the same thing twice instead of three times — the pair differs only in the multiplier,
  which is what the two tiers are for.
- **C** `src/aggregate/group_by.c` — in `invokeGroupReducers`, do the `RLookupRow_Get` +
  `RSValue_Hash` key-building pass **12x** per row; reducers still invoked once, so behaviour
  is unchanged (tripling the reducers would make COLLECT collect each value three times).
  Twelve rather than three because, unlike the other three cases, the slice being multiplied
  is a minority of the per-row cost here — C's `LOAD 5` loader upstream dominates — so a 3x
  on it would land in the `small` band instead. Twelve is a judgement call, not a measured
  figure.
