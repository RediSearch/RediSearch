...
static RSDocumentMetadata *DocTable_GetOwn(const DocTable *t, t_docId docId) {
    ...
    // While we iterate over the chain, we have locked the index spec (R/W), so we either a writer
    // alone or multiple readers. In any case, we can safely iterate over the chain without a lock
    // and increment the ref count of the document metadata when we find it.
    ...
    for (uint32_t i = 0; i < t->cap; ++i) {
        ...
        DLLIST2_FOREACH(it, &t->buckets[i].lroot) {
            ...
            if (dmd->flags & Document_HasPayload) {
                if (hasPayload(dmd->flags)) {
                    // save an extra space for the null terminator to make the payload null
                    // terminated on
...
void DocIdMap_Free(DocIdMap *m) { TrieMap_Free(m->tm, rm_free); }
