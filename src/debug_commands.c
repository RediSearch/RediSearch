...
DEBUG_COMMAND(DumpTagIndex) {
  ...
  TrieMapIterator *iter = TrieMap_Iterate(tagIndex->values, "", 0, TM_ITER_MODE_DEFAULT);
...
DEBUG_COMMAND(DumpSuffix) {
  ...
  if (argc == 3) { // suffix trie of global text field
    ...
    TrieMapIterator *it = TrieMap_Iterate(idx->suffix, "", 0, TM_ITER_MODE_DEFAULT);
    char *str;
    tm_len_t len;
    void *value;
...
DEBUG_COMMAND(InfoTagIndex) {
  ...
  TrieMapIterator *iter = TrieMap_Iterate(idx->values, "", 0, TM_ITER_MODE_DEFAULT);
  char *tag;
  tm_len_t len;
  InvertedIndex *iv;
