...
/* As `ReplyReaderResultsIDs`, for a tag's posting list, which the Rust tag index
 * hands out as an opaque `TagIndexValue` rather than an `InvertedIndex`. */
static void ReplyTagValueResultsIDs(TagIndexValue *tiv, RSIndexResult *res, RedisModuleCtx *ctx) {
  size_t resultSize = 0;
  TagIndexValueIter *it = Rust_TagIndexValue_Iter(tiv);

  RedisModule_ReplyWithArray(ctx, REDISMODULE_POSTPONED_ARRAY_LEN);
  while (Rust_TagIndexValueIter_Next(it, res)) {
    RedisModule_ReplyWithLongLong(ctx, res->docId);
    ++resultSize;
  }
  RedisModule_ReplySetArrayLength(ctx, resultSize);

  Rust_TagIndexValueIter_Free(it);
  IndexResult_Free(res);
}
...
DEBUG_COMMAND(DumpTagIndex) {
  ...
  // Debug dump not supported for disk-mode tag indexes (their values trie holds
  // presence sentinels; the postings live on disk)
  ...
  while (Rust_TagIndex_ValueIterator_Next(iter, &tag, &len, &tiv)) {
    ...
    res->freq = 1;
    res->fieldMask = RS_FIELDMASK_ALL;
    ReplyTagValueResultsIDs(tiv, res, sctx->redisCtx);
    ++resultSize;
  }
...
DEBUG_COMMAND(DumpSuffix) {
  ...
  if (argc == 3) { // suffix trie of global text field
    ...
    char *str;
    tm_len_t len;
    // The key is borrowed from the suffix index, so it is replied by length
    // rather than NUL-terminated in place.
    while (Rust_TagIndex_ValueIterator_NextKey(it, &str, &len)) {
...
static void seekTagIterator(ValueIterator *it, size_t offset) {
  ...
  for (size_t n = 0; n < offset; n++) {
    if (!Rust_TagIndex_ValueIterator_NextKey(it, &tag, &len)) {
      break;
    }
  }
}
...
DEBUG_COMMAND(InfoTagIndex) {
  ...
  while (nvalues++ < limit && Rust_TagIndex_ValueIterator_Next(iter, &tag, &len, &tiv)) {
    size_t nsubelem = 8;
    if (!options.dumpIdEntries) {
      nsubelem -= 2;
    }
    ...
    RedisModule_ReplyWithLongLong(ctx, Rust_TagIndexValue_NumDocs(tiv));

    RedisModule_ReplyWithLiteral(ctx, "num_blocks");
    RedisModule_ReplyWithLongLong(ctx, Rust_TagIndexValue_NumBlocks(tiv));
