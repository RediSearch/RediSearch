...
static atomic_uint_fast64_t g_backgroundReplySerializationCount = 0;
...
void BackgroundReplySerializationDebug_RecordCurrentThread(void) {
  if (MainThread_GetBlockedQueries() == NULL) {
    atomic_fetch_add_explicit(&g_backgroundReplySerializationCount, 1, memory_order_relaxed);
  }
}

uint64_t BackgroundReplySerializationDebug_GetCount(void) {
  return atomic_load_explicit(&g_backgroundReplySerializationCount, memory_order_relaxed);
}
#endif
...
/**
 * FT.DEBUG QUERY_CONTROLLER GET_BACKGROUND_REPLY_SERIALIZATION_COUNT
 */
DEBUG_COMMAND(getBackgroundReplySerializationCount) {
  if (!debugCommandsEnabled(ctx)) {
    return RedisModule_ReplyWithError(ctx, NODEBUG_ERR);
  }
  if (argc != 2) {
  ...
  return RedisModule_ReplyWithLongLong(ctx,
                                       (long long)BackgroundReplySerializationDebug_GetCount());
}
...
DEBUG_COMMAND(queryController) {
  ...
  if (!strcmp("GET_BACKGROUND_REPLY_SERIALIZATION_COUNT", op)) {
    return getBackgroundReplySerializationCount(ctx, argv + 1, argc - 1);
  }
