...
#define END_POSTPONED_LEN_ARRAY(array_name) RedisModule_ReplySetArrayLength(ctx, len_##array_name)
...
static size_t InvertedIndexSummaryHeader(RedisModuleCtx *ctx, InvertedIndex *invidx) {
    ...
    if (invidx->flags & Index_StoreNumeric) {
        REPLY_WITH_DOUBLE("blocks_efficiency (numEntries/numberOfBlocks)",
                          InvertedIndexGetEfficiency(invidx), invIdxBulkLen);
    }
...
DEBUG_COMMAND(DumpNumericIndex) {
    ...
    while ((currNode = NumericRangeTreeIterator_Next(iter))) {
        ...
        if (range) {
            ...
            IndexReader *reader =
                NewNumericReader(NULL, range->entries, NULL, range->minVal, range->maxVal, true);
            ReplyReaderResults(reader, sctx->redisCtx);
...
DEBUG_COMMAND(DumpPrefixTrie) {
    ...
    REPLY_WITH_LONG_LONG("prefixes_count", prefixes_map->cardinality,
                         ARRAY_LEN_VAR(prefixesMapDump));
...
InvertedIndexStats InvertedIndex_DebugReply(RedisModuleCtx *ctx, InvertedIndex *idx) {
    ...
    REPLY_WITH_DOUBLE("blocks_efficiency (numEntries/size)", indexStats.blocks_efficiency,
...
InvertedIndexStats NumericRange_DebugReply(RedisModuleCtx *ctx, NumericRange *r) {
    ...
    if (r) {
        ...
        REPLY_WITH_DOUBLE("invertedIndexSize [bytes]", r->invertedIndexSize,
...
InvertedIndexStats NumericRangeNode_DebugReply(RedisModuleCtx *ctx, NumericRangeNode *n) {
    ...
    if (n) {
        if (n->range) {
            ...
            invIdxStats.blocks_efficiency +=
                NumericRange_DebugReply(ctx, n->range).blocks_efficiency;
            len += 2;
        } else {
            ...
            invIdxStats.blocks_efficiency +=
                NumericRangeNode_DebugReply(ctx, n->left).blocks_efficiency;
            len += 2;

            RedisModule_ReplyWithStringBuffer(ctx, "right", strlen("right"));
            invIdxStats.blocks_efficiency +=
                NumericRangeNode_DebugReply(ctx, n->right).blocks_efficiency;
            len += 2;
        }
    }
...
void NumericRangeTree_DebugReply(RedisModuleCtx *ctx, NumericRangeTree *rt) {
    ...
    REPLY_WITH_DOUBLE("Average memory efficiency (numEntries/size)/numRanges",
                      (invIndexStats.blocks_efficiency) / rt->numRanges, ARRAY_LEN_VAR(tree_stats));
...
DEBUG_COMMAND(GCForceInvoke) {
    ...
    RedisModuleBlockedClient *bc =
        RedisModule_BlockClient(ctx, GCForceInvokeReply, GCForceInvokeReplyTimeout, NULL, timeout);
...
DEBUG_COMMAND(GCStopFutureRuns) {
    ...
    // mark as stopped. This will prevent the GC from scheduling itself again if it was already
    // running.
    sp->gc->timerID = 0;
...
// This command blocks the client and adds a job to the end of the GC queue, that will later unblock
// it.
...
DEBUG_COMMAND(ttlPause) {
    ...
    RedisModule_Assert(RedisModule_StopTimer(RSDummyContext, sp->timerId, (void **)&timer_ref) ==
                       REDISMODULE_OK);
...
static void replyDocFlags(const char *name, const RSDocumentMetadata *dmd,
...
static void replySortVector(const char *name, const RSDocumentMetadata *dmd, RedisSearchCtx *sctx,
                            RedisModule_Reply *reply) {
...
DEBUG_COMMAND(DocInfo) {
    ...
    RedisModule_ReplyKV_LongLong(reply, "refcount",
                                 dmd->ref_count -
                                     1); // TODO: should include the refcount of the command call?
    if (dmd->sortVector) {
...
void replyDumpHNSW(RedisModuleCtx *ctx, VecSimIndex *index, t_docId doc_id) {
    ...
    VecSimDebugCommandCode res =
        VecSimDebug_GetElementNeighborsInHNSWGraph(index, doc_id, &neighbours_data);
    ...
    if (!strcasecmp(op, "pause")) {
        if (workersThreadPool_pause() != REDISMODULE_OK) {
            return RedisModule_ReplyWithError(ctx,
                                              "Operation failed: workers thread pool doesn't exists"
                                              " or is not running");
        }
        ...
        if (workerThreadPool_isPaused()) {
            return RedisModule_ReplyWithError(
                ctx, "Operation failed: workers thread pool is not running");
        }
        workersThreadPool_Drain(RSDummyContext, 0);
        // After we drained the thread pool and there are no more jobs in the queue, we wait until
        // all threads are idle, so we can be sure that all jobs were executed.
        ...
        REPLY_WITH_LONG_LONG("totalJobsDone", stats.total_jobs_done,
                             ARRAY_LEN_VAR(num_stats_fields));
        REPLY_WITH_LONG_LONG("totalPendingJobs", stats.total_pending_jobs,
                             ARRAY_LEN_VAR(num_stats_fields));
        REPLY_WITH_LONG_LONG("highPriorityPendingJobs", stats.high_priority_pending_jobs,
                             ARRAY_LEN_VAR(num_stats_fields));
        REPLY_WITH_LONG_LONG("lowPriorityPendingJobs", stats.low_priority_pending_jobs,
                             ARRAY_LEN_VAR(num_stats_fields));
        REPLY_WITH_LONG_LONG("numThreadsAlive", stats.num_threads_alive,
                             ARRAY_LEN_VAR(num_stats_fields));
...
DebugCommandType commands[] = {
    {"DUMP_INVIDX", DumpInvertedIndex}, // Print all the inverted index entries.
    {"DUMP_NUMIDX",
     DumpNumericIndex}, // Print all the headers (optional) + entries of the numeric tree.
    {"DUMP_NUMIDXTREE",
     DumpNumericIndexTree}, // Print tree general info, all leaves + nodes + stats
    ...
    {"INVIDX_SUMMARY",
     InvertedIndexSummary}, // Print info about an inverted index and each of its blocks.
