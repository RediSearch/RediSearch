...
static void normalizeSettings(const ReturnedField *srcField, const ReturnedField *defaults,
    ...
    if ((defaults->mode & SummarizeMode_Synopsis) &&
        (srcField->mode & SummarizeMode_Synopsis) == 0) {
...
static char *trimField(const ReturnedField *fieldInfo, const char *docStr, size_t *docLen,
    ...
    size_t headLen = estWordSize * fieldInfo->summarizeSettings.contextLen *
                     fieldInfo->summarizeSettings.numFrags;
...
static void hlpFree(ResultProcessor *p) { rm_free(p); }
