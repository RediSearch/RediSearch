...
int IR_Read(void *ctx, IndexHit *e) {
    ...
    if (rc == INDEXREAD_OK) {
        // LG_DEBUG("docId %d Flags 0x%x, field mask 0x%x, intersection: %x", e->docId, e->flags,
...
void writeIndexHeader(IndexWriter *w) {
    ...
    LG_DEBUG("Writing index header. offest %d , lastId %d, ndocs %d, will seek to %zd", h.size,
             h.lastId, w->ndocs, offset);
...
inline t_docId UI_LastDocId(void *ctx) { return ((UnionContext *)ctx)->minDocId; }
...
int UI_SkipTo(void *ctx, u_int32_t docId, IndexHit *hit) {
    ...
    for (int i = 0; i < ui->num; i++) {
        ...
        if ((rc = ui->its[i]->SkipTo(ui->its[i]->ctx, docId, &ui->currentHits[i])) ==
            INDEXREAD_EOF) {
            continue;
        }
    ...
    do {
        ...
        if (nh == ic->num) {
            ...
            if (ic->its[0]->Read(ic->its[0]->ctx, &ic->currentHits[0]) == INDEXREAD_EOF) {
                ...
                // but advancing docId makes sure we'll read the first iterator again in the next
                // round
                ic->lastDocId++;
            } else {
