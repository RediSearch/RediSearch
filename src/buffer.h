...
static inline size_t BufferReader_Offset(const BufferReader *br) { return br->pos; }

static inline size_t BufferReader_Remaining(const BufferReader *br) {
    return br->buf->cap - br->pos;
}

static inline size_t Buffer_Offset(const Buffer *ctx) { return ctx->offset; }

#define BufferReader_AtEnd(br) ((br)->pos >= (br)->buf->offset)

static inline size_t Buffer_Capacity(const Buffer *ctx) { return ctx->cap; }

static inline int Buffer_AtEnd(const Buffer *ctx) { return ctx->offset >= ctx->cap; }
...
static inline size_t Buffer_WriteU8(BufferWriter *bw, uint8_t u) { return Buffer_Write(bw, &u, 1); }
...
static inline size_t BufferWriter_Offset(BufferWriter *b) { return b->pos - b->buf->data; }

static inline char *BufferWriter_PtrAt(BufferWriter *b, size_t pos) { return b->buf->data + pos; }
