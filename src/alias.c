...
void IndexAlias_InitGlobal(void) { AliasTable_g = AliasTable_New(); }
...
static int AliasTable_Add(AliasTable *table, const char *alias, StrongRef spec_ref, int options,
                          QueryError *error) {
    ...
    // Dictionary holds a pointer tho the spec manager. Its the same reference owned by the specs
    // dictionary.
    e->v.val = spec_ref.rm;
...
StrongRef IndexAlias_Get(const char *alias) { return AliasTable_Get(AliasTable_g, alias); }
