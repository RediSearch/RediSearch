...
static int parseCommon(ArgsCursor *ac, FieldList *fields, int isHighlight) {
    ...
    while (!AC_IsAtEnd(ac)) {
        if (isHighlight && AC_AdvanceIfMatch(ac, "TAGS")) {
            ...
            if (AC_GetString(ac, (const char **)&defOpts.summarizeSettings.separator, NULL, 0) !=
                AC_OK) {
                rc = REDISMODULE_ERR;
                goto done;
            }
...
int ParseSummarize(ArgsCursor *ac, FieldList *fields) { return parseCommon(ac, fields, 0); }

int ParseHighlight(ArgsCursor *ac, FieldList *fields) { return parseCommon(ac, fields, 1); }
