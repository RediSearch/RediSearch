...
DEBUG_COMMAND(InvertedIndexSummary) {
  ...
  InvertedIndex *invidx = IDX_LoadTerm(sctx->spec, invIdxName, len, REDISMODULE_READ);
  if (!invidx) {
...
DEBUG_COMMAND(NumericIndexSummary) {
  ...
  GET_SEARCH_CTX(argv[0])
  NumericRangeTree *rt =
      IDX_LoadRangeFieldname(sctx->spec, RedisModule_StringPtrLen(argv[1], NULL), REDISMODULE_READ);
  if (!rt) {
...
DEBUG_COMMAND(DumpTagIndex) {
  ...
  GET_SEARCH_CTX(argv[0])
  TagIndex *tagIndex =
      IDX_LoadTagsFieldname(sctx->spec, RedisModule_StringPtrLen(argv[1], NULL), REDISMODULE_READ);
  if (!tagIndex) {
