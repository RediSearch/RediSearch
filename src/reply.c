...
bool RedisModule_HasMap(RedisModule_Reply *reply) { return _ReplyMap(reply->ctx); }
...
int RedisModule_Reply_Array(RedisModule_Reply *reply) {
    RS_LOG_ASSERT(!RedisModule_Reply_LocalIsKey(reply),
                  "reply: should not write an array as a key");
...
int RedisModule_ReplyKV_StringBuffer(RedisModule_Reply *reply, const char *key, const char *val,
                                     size_t len) {
    RedisModule_ReplyWithSimpleString(reply->ctx, key);
    RedisModule_ReplyWithStringBuffer(reply->ctx, val, len);
    json_add(reply, false, "\"%s\"", key);
...
int RedisModule_ReplyKV_String(RedisModule_Reply *reply, const char *key,
                               const RedisModuleString *val) {
    RedisModule_ReplyWithSimpleString(reply->ctx, key);
    json_add(reply, false, "\"%s\"", key);
    RedisModule_ReplyWithString(reply->ctx, (RedisModuleString *)val);
    _RedisModule_Reply_Next(reply);
