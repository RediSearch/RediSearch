...
RSSortingVector *SortingVector_RdbLoad(RedisModuleIO *rdb, int encver) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        case RSValue_String: {
            size_t len;
            // strings include an extra character for null terminator. we set it to zero just in
            // case
...
void SortingTable_Free(RSSortingTable *t) { rm_free(t); }

int RSSortingTable_Add(RSSortingTable **tbl, const char *name, RSValueType t) {
    if ((*tbl)->len == RS_SORTABLES_MAX)
        return -1;
