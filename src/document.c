        ...
        if (!f->indexAs) {
            ...
            if ((f->indexAs & fs->types) != f->indexAs) {
                QueryError_SetErrorFmt(&aCtx->status, QUERY_EUNSUPPTYPE,
                                       "Tried to index field %s as type not specified in schema",
                                       fs->name);
                return -1;
            }
        }
...
RSAddDocumentCtx *NewAddDocumentCtx(IndexSpec *sp, Document *doc, QueryError *status) {
    ...
    if (!actxPool_g) {
        mempool_options mopts = {
            .initialCap = 16, .alloc = allocDocumentContext, .free = freeDocumentContext};
    ...
    if (sp->smap) {
        // we get a read only copy of the synonym map for accessing in the index thread without
        // worrying about thready safe issues
...
void AddDocumentCtx_Finish(RSAddDocumentCtx *aCtx) { doReplyFinish(aCtx, aCtx->sctx->redisCtx); }
...
#define FIELD_HANDLER(name)                                                                        \
    static int name(RSAddDocumentCtx *aCtx, RedisSearchCtx *sctx, DocumentField *field,            \
...
FIELD_PREPROCESSOR(fulltextPreprocessor) {
    ...
    if (FieldSpec_IsIndexable(fs)) {
        ...
        if (aCtx->byteOffsets) {
            curOffsetField =
                RSByteOffsets_AddField(aCtx->byteOffsets, fs->ftId, aCtx->totalTokens + 1);
        ...
        for (size_t i = 0; i < valueCount; ++i) {
            ...
            ForwardIndexTokenizerCtx_Init(&tokCtx, aCtx->fwIdx, c, curOffsetWriter, fs->ftId,
                                          fs->ftWeight);
...
FIELD_BULK_INDEXER(geometryIndexer) {
    ...
    if (!rt) {
        ...
        if (!rt) {
            QueryError_SetError(status, QUERY_EGENERIC,
                                "Could not open geoshape index for indexing");
            return -1;
        }
    }
    ...
    if (!fdata->isMulti) {
        if (!api->addGeomStr(rt, fdata->format, fdata->str, fdata->strlen, aCtx->doc->docId,
                             &errMsg)) {
...
FIELD_PREPROCESSOR(vectorPreprocessor) {
    ...
    if (fdata->vecLen != fs->vectorOpts.expBlobSize) {
        ...
        QueryError_SetErrorFmt(status, QUERY_EBADATTR,
                               "Could not add vector with blob size %zu (expected size %zu)",
                               fdata->vecLen, fs->vectorOpts.expBlobSize);
        return -1;
    }
    aCtx->fwIdx->maxFreq++;
    return 0;
...
FIELD_PREPROCESSOR(geoPreprocessor) {
    ...
    if (str_count == 1) {
        ...
        if (geohash == INVALID_GEOHASH) {
            QueryError_SetErrorFmt(status, QUERY_EINVAL, "Invalid geo coordinates: %f, %f", lon,
                                   lat);
            return REDISMODULE_ERR;
        }
        fdata->numeric = geohash;
...
static PreprocessorFunc preprocessorMap[] = {
    // nl break
    [IXFLDPOS_FULLTEXT] = fulltextPreprocessor, [IXFLDPOS_NUMERIC] = numericPreprocessor,
    [IXFLDPOS_GEO] = geoPreprocessor,           [IXFLDPOS_TAG] = tagPreprocessor,
    [IXFLDPOS_VECTOR] = vectorPreprocessor,     [IXFLDPOS_GEOMETRY] = geometryPreprocessor,
};
    ...
    for (size_t i = 0; i < doc->numFields; i++) {
        ...
        for (size_t ii = 0; ii < INDEXFLD_NUM_TYPES; ++ii) {
            ...
            if (pp(aCtx, sctx, ff, fs, fdata, &aCtx->status) != 0) {
                IndexError_AddError(&aCtx->spec->stats.indexError,
                                    QueryError_GetError(&aCtx->status), doc->docKey);
                IndexError_AddError(&aCtx->spec->fields[fs->index].indexError,
                                    QueryError_GetError(&aCtx->status), doc->docKey);
...
int Document_EvalExpression(RedisSearchCtx *sctx, RedisModuleString *key, const char *expr,
    ...
    lookup_s.options |= RLOOKUP_OPT_ALL_LOADED; // Setting this option will cause creating keys of
                                                // non-sortable fields possible
    ...
    ExprEval evaluator = {
        .err = status, .lookup = &lookup_s, .res = NULL, .srcrow = &row, .root = e};
...
#define BAIL(s)                                                                                    \
    ...
    if (aCtx->stateFlags & ACTX_F_SORTABLES) {
        ...
        for (int i = 0; i < doc->numFields; i++) {
            ...
            RS_LOG_ASSERT((fs->options & FieldSpec_Dynamic) == 0,
                          "Dynamic field cannot use PARTIAL");

            switch (fs->types) {
            case INDEXFLD_T_FULLTEXT:
            case INDEXFLD_T_TAG:
            case INDEXFLD_T_GEO:
                RSSortingVector_Put(md->sortVector, idx,
                                    (void *)RedisModule_StringPtrLen(f->text, NULL),
