...
impl<'index, E: ExpirationChecker> ScoreSource for VectorScoreSource<'index, E> {
        ...
        Self: 'r,
    {
        ...
        // Upserting rather than pushing: the child reuses one storage slot across
        // yields, so an entry from a previous yield may already exist for our key.
        // Any metrics the child's own subtree carries under other keys are untouched.
