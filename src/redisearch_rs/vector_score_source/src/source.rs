...
use ffi::{
    QueryIterator, RLookupKeyHandle, RS_VecSimCheckTimeout, TimeoutCtx, VecSearchMode, VecSimIndex,
...
pub struct VectorScoreSource<'index, E: ExpirationChecker = FieldExpirationChecker> {
    ...
    /// `HybridIterator_GetChild`. Set by the C side via an accessor; null
    /// until then.
...
impl<'index, E: ExpirationChecker + Clone> ScoreSource for VectorScoreSource<'index, E> {
    type Batch = VecSimScoreBatchCursor<E>;
    fn all_results_unfiltered_batch(&mut self) -> Result<Option<Self::Batch>, RQEIteratorError> {
        ...
        Ok(reply
            ...
            .map(|results| VecSimScoreBatchCursor::new(results, self.expiration.clone())))
    }
    ...
    fn adhoc_check_timeout(&mut self) -> bool {
        // SAFETY: `timeout_ctx` is heap-allocated and lives as long as `self`,
        // so the pointer is valid for this call. `RS_VecSimCheckTimeout` only
        // reads the deadline and bumps the periodic counter.
