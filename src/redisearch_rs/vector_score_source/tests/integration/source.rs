...
unsafe fn make_source(
    ...
    // SAFETY: the caller guarantees `index` outlives the source; `timeout_ctx`
    // is null and the index is a RAM (non-disk) HNSW index.
    unsafe {
...
#[test]
fn unfiltered_returns_top_k_nearest_by_id() {
    let n = 100;
    let k = 10;
    ...
    // The unfiltered path streams VecSim's reply ordered BY_ID, so the k
    // nearest neighbours (ids 91..=100) come out in ascending id order.
    let ids = collect_ids(&mut it);
    assert_eq!(ids, (91..=100).collect::<Vec<_>>());
    assert!(it.at_eof());
...
fn unfiltered_results_are_metric_kind_with_exact_distance() {
    ...
    // Unfiltered streams BY_ID, so ids 91..=100 come out ascending. Each result
    // is a Metric carrying the exact squared-L2 distance DIM*(n-id)^2.
    let mut expected_id = 91 as t_docId;
    ...
    assert_eq!(expected_id, n as t_docId + 1);
...
fn first_result_after_rewind_is_best() {
    ...
    assert!(
        dist.abs() < 1e-3,
        "best result distance should be ~0, got {dist}"
    );
