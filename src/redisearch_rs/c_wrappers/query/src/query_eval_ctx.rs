...
impl QueryEvalContext {
    ...
    ///    The nested `sctx.spec.terms` pointer — the index's primary terms
    ///    dictionary — must be valid and non-null: every path that creates an
    ///    [`IndexSpec`](ffi::IndexSpec) installs a terms dictionary,
    ///    unconditionally and regardless of whether the schema has any text
    ///    field, before the spec can answer a query. Loading a legacy spec is
    ///    the one path that leaves the field null for a while, and it fills it
    ///    in — or releases the half-built spec — before handing the spec out,
    ///    so no queryable spec has a null dictionary.
    ...
    /// The index's primary terms dictionary, the one every term lookup and
    /// pattern expansion walks.
    ///
    /// The dictionary is always there, per invariant (2) of [`new`](Self::new), which
    ...
    /// borrow this hands out is not one the type system can see: the dictionary
    /// is owned by the spec rather than by the context, so binding it to `&self`
    /// would say it is borrowed *from the context* — and then a caller
