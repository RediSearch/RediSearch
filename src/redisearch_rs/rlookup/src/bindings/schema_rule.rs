...
mod test {
    ...
    fn fields_and_indices() {
        ...
        schema_rule.filter_fields = rm_array([c"aaa", c"bbb"].map(|cstr| cstr.as_ptr().cast_mut()));
        schema_rule.filter_fields_index = rm_array([10, 20]);
    ...
    fn rm_array<const N: usize, T: Copy>(fields: [T; N]) -> *mut T {
        let arr = unsafe {
