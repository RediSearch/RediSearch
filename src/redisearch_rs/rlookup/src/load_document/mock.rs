...
unsafe extern "C" fn RedisModule_ScanKey(
    ...
    let ctx: &crate::load_document::tests::LoadDocumentTestContext =
        unsafe { &*(ctx.as_ptr() as *const crate::load_document::tests::LoadDocumentTestContext) };
    ...
    for (k, v) in ctx.access_key_values().iter() {
        ...
        let value = unsafe { RedisModule_CreateString(ctx_arg, cstring.as_ptr(), vlen) };
        ...
        // we drop the cstring after freeing the redis string:
        drop(cstring);
    }
