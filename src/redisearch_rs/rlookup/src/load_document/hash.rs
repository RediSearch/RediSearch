...
impl DocumentFormat for HashDocumentFormat {
    ...
    fn load_all(
        ...
        scan_cursor.for_each(|_key, field, value| {
            ...
            // per row, no behaviour change. The probe's *argument* is the thing that
            // has to be opaque: with identical arguments the two lookups are one
            // common subexpression and the optimiser keeps only one of them.
            std::hint::black_box(rlookup.find_key_by_name(std::hint::black_box(field_cstr)).is_some());
