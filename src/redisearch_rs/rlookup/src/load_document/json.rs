...
fn json_iter_to_value(
    ...
    if api_version < ffi::APIVERSION_RETURN_MULTI_CMP_FIRST {
        ...
        if len > 0 {
            ...
            let serialized = unsafe { iter.serialize(ctx.ctx.cast())? };
...
fn json_val_to_value_expanded(ctx: redis_module::Context, json: JsonValueRef) -> RSValueFFI {
    match json.get_type() {
        JsonType::Object => {
            ...
            if len > 0 {
                ...
                let iter = unsafe { json.key_values(ctx.ctx.cast()).unwrap() };
...
fn json_val_to_value(ctx: redis_module::Context, json: JsonValueRef<'_>) -> RSValueFFI {
    ...
    match json.get_type() {
        ...
        JsonType::Object | JsonType::Array => {
            ...
            let v = unsafe { json.serialize(ctx.ctx.cast()).unwrap() };
