...
fn is_value_available(rlookup: &RLookupKey, dst_row: &RLookupRow<'_, RSValueFFI>) -> bool {
...
pub fn load_all_keys(
    ...
    match dmd.type_() {
        DocumentType::Hash => hash::load_all_keys(rlookup, dst_row, ctx, dmd, force_string),
        DocumentType::Json => json::load_all_keys(rlookup, dst_row, ctx, dmd, api_version),
    ...
    'a: 'k,
{
    ...
    let key_name =
        unsafe { RedisString::from_raw_parts(NonNull::new(ctx.ctx), dmd.keyPtr, key_name_len) };
...
pub fn load_specific_keys_from_rlookup(
    ...
    load_specific_keys(dst_row, dmd, keys_to_load)
}
