...
/// Row data for a lookup key. This abstracts the question of if the data comes from a [RSSortingVector]
...
impl<T: RSValueTrait> RLookupRow<T> {
    ...
    /// Readonly access to [`RLookupRow::sorting_vector`]
    pub fn sorting_vector(&self) -> &RSSortingVector<T> {
        &self.sorting_vector
    }

    /// Mutable access to the [`RLookupRow::sorting_vector`]
    ...
    pub fn write_key(&mut self, key: &RLookupKey, val: T) {
        ...
        if self.dyn_values.len() <= idx as usize {
            self.dyn_values.resize((idx + 1) as usize, None);
        }
