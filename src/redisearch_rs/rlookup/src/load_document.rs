...
use crate::lookup::KeyList;
use crate::row::RLookupRow;
use ffi::REDISMODULE_OK;
use sorting_vector::RSSortingVector;

use ffi::RSDocumentMetadata_s;
use ffi::RedisModuleKey;
use ffi::redis_api;
use ffi::redis_api::*;
use value::RSValueFFI;

type RedisModuleStringPtr = *mut ffi::RedisModuleString;

/// Variables that are global in the C API and required for loading documents.
///
/// This is a temporary solution until we have a proper Rust port of the C API.
pub struct GlobalCVariables {
    ...
    server_version: u32,
    // todo: Decide on how to handle the Redis Javascript API
    //redis_json_api: Option<Box<dyn RedisJavascriptModule>>,
}
...
pub enum LoadDocumentError {
    /// An error from the Redis Module or Json API that indicates a failure to create or use a resource.
    #[error(transparent)]
    Resource(#[from] RedisResourceError),
...
impl From<WithError<LoadDocumentError>> for LoadDocumentError {
    fn from(error: WithError<LoadDocumentError>) -> Self {
        match error {
            WithError::Resource(err) => LoadDocumentError::Resource(err),
            WithError::Closure(err) => err, // err is already LoadDocumentError
        }
    }
}
...
pub fn load_document<'a>(
    ...
    let rc = if check {
        if options_safe.dmd.type_() == DocumentType::Hash as u32 {
            ...
            // todo: Ensure that KeyList can be casted to RLookup
            let it = it as *mut KeyList as *mut ffi::RLookup;
            let dst = dst as *mut RLookupRow<RSValueFFI> as *mut ffi::RLookupRow;
            let options = options as *const RLookupLoadOptions as *mut ffi::RLookupLoadOptions;
...
struct HgetallPrivateData<'a, 'b> {
    ...
    options: &'b LoadDocumentOptions<'b>,
}
...
fn h_get_all<'a>(
    ...
    let krstr = redis_api::RedisString::new((options.ctx, key_ptr, sds_len))?;

    let feature = ffi::RM_SCAN_KEY_API_FIX;
    let feature_supported = feature <= options.server_version;

    if feature_supported && !options.is_crdt {
        // 1. Open the key
        // A bit flag with four parts has been used that was not exported by bindgen:
        const T1: u32 = ffi::REDISMODULE_READ;
        const T2: u32 = ffi::REDISMODULE_OPEN_KEY_NOEFFECTS;
        const T3: u32 = ffi::REDISMODULE_OPEN_KEY_NOEXPIRE;
        const T4: u32 = ffi::REDISMODULE_OPEN_KEY_ACCESS_EXPIRED;
        const DOCUMENT_OPEN_KEY_QUERY_FLAGS: u32 = T1 | T2 | T3 | T4;

        let key = RedisKey::new((options.ctx, *krstr, DOCUMENT_OPEN_KEY_QUERY_FLAGS as i32))?;
        // Check if the key is a hash
        if redis_api::key_type(*key) != ffi::REDISMODULE_KEYTYPE_HASH as i32 {
        ...
        RedisScanCursor::use_with((), |scan_cursor| -> Result<(), LoadDocumentError> {
            let mut priv_data = HgetallPrivateData { it, dst, options };
            // 3. Loop: Scan the key and move cursor forward
            loop {
                let res = redis_api::scan_key(
                    *key,
                    scan_cursor,
                    Some(h_get_all_scan_callback),
                    &mut priv_data as *mut _ as *mut c_void,
                );
                if res == 0 {
        ...
        RedisCallReply_Hgetall::use_with(
            (options.ctx, c"HGETALL".as_ptr(), c"s".as_ptr(), *krstr),
            |reply| {
                if reply.is_null() {
                    return Err(LoadDocumentError::FallbackAPINotAvailable);
                }

                // Check if the reply is an array
                if redis_api::call_reply_type(reply) != ffi::REDISMODULE_REPLY_ARRAY as i32 {
                    let str_err = "kstr.to_string()".to_string();
                    return Err(LoadDocumentError::KeyDoesNotExist(str_err));
                }

                // Get the length of the array
                let len = redis_api::call_reply_length(reply);
                if len == 0 {
                    let str_err = "kstr.to_string()".to_string();
                    return Err(LoadDocumentError::KeyDoesNotExist(str_err));
                }
                ...
                for i in (0..len).step_by(2) {
                    let k = redis_api::call_reply_array_element(reply, i);
                    let value = redis_api::call_reply_array_element(reply, i + 1);

                    let mut klen: libc::size_t = 0;
                    let kstr = redis_api::call_reply_string_ptr(k, &mut klen as *mut _);

                    // Safety: The kstr is expected to be a valid C string obtained from Redis Module API.
                    let kstr = unsafe { CStr::from_ptr(kstr) };
                    ...
                    process_rlookup_key_value(kstr, &mut priv_data, |ctype| {
                        ...
                        let ptr = unsafe { ffi::replyElemToValue(value, ctype) };
...
unsafe extern "C" fn h_get_all_scan_callback(
    ...
    field: RedisModuleStringPtr,
    value: RedisModuleStringPtr,
    ...
    let mut field_str_len: libc::size_t = 0;
    let field_cstr = redis_api::string_ptr_len(field, &mut field_str_len as *mut _);

    // Safety: The field_cstr is expected to be a valid C string obtained from Redis Module API.
    let field_cstr = unsafe { CStr::from_ptr(field_cstr) };
...
fn process_rlookup_key_value<F>(
    key_str: &CStr,
    priv_data: &mut HgetallPrivateData,
    rsvalue_create: F,
) where
    F: FnOnce(u32) -> RSValueFFI,
{
    // todo: use right type after merge of `RLookup_FindKey`
    let cursor = priv_data.it.find_by_name(key_str);
    ...
    let (_, rlk) = if let Some(cursor) = &cursor {
        ...
        // see https://github.com/RediSearch/RediSearch/pull/6437
        let _rlk = todo!("RLookup_GetKeyLoadEx");
        (None, _rlk)
    };
