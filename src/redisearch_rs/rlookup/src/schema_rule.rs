...
impl SchemaRuleWrapper {
    ...
    pub fn is_special_key(&self, key: &RLookupKey) -> bool {
        ...
        let key_is_lang_field = self.lang_field().is_some_and(|lf| lf == key.name());
        let key_is_score_field = self.score_field().is_some_and(|sf| sf == key.name());
        let key_is_payload_field = self.payload_field().is_some_and(|p| p == key.name());
