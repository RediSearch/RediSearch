...
pub extern "C" fn Term_Free(_t: *mut ffi::RSQueryTerm) {
    // Deep copying a term test will call this function
}
...
#[test]
fn deep_copy_an_aggregate_index_result() {
    ...
    ir_copy.get_mut(0).unwrap().as_numeric_mut().unwrap().0 = 1.0;
    assert_eq!(
        ir.get(0).unwrap().as_numeric().unwrap().0,
        5.0,
...
#[test]
fn deep_copy_a_numeric_index_result() {
    ...
    ir_copy.as_numeric_mut().unwrap().0 = 1.0;

    assert_eq!(
        ir.as_numeric().unwrap().0,
        8.0,
        "cloned value should not have changed"
    );
}
...
#[test]
fn deep_copy_a_virtual_index_result() {
    let ir = RSIndexResult::virt().doc_id(8).field_mask(4).weight(2.0);
    let ir_copy = ir.deep_copy();
...
#[test]
fn deep_copy_a_term_index_result() {
