...
pub use index_result::{
    RSAggregateResult, RSAggregateResultIter, RSIndexResult, RSOffsetVectorRef, RSResultData,
...
impl<'index, D: Decoder> IndexReader<'index, D> {
    ...
    pub fn new(blocks: &'index Vec<IndexBlock>, decoder: D) -> IndexReader<'index, D> {
        debug_assert!(
            !blocks.is_empty(),
    ...
    pub fn next_record<'a>(&'a mut self) -> std::io::Result<Option<RSIndexResult<'index>>> {
