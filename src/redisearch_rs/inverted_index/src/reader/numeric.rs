...
pub struct NumericFilter {
    ...
    /// Re-derive the `FieldSpec` from this at evaluation time - a raw pointer
    /// captured when the filter was built could dangle across a concurrent
    /// `FT.ALTER` under `WORKERS>0` (MOD-18361).
