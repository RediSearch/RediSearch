    ...
    for IndexReaderCore<'index, E>
{
    ...
    fn needs_revalidation(&self) -> bool {
        ...
        // Reading `as_ptr()` and `len()` off the cached slice inspects the fat pointer's own
        // fields; it does not touch the bytes behind it.
        let cached = *self.current_buffer.get_ref();
        let live = block.buffer.as_slice();
        !std::ptr::eq(cached.as_ptr(), live.as_ptr()) || cached.len() != live.len()
    }
