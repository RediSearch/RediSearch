...
impl<'index> RSIndexResult<'index> {
    ...
    /// Resets the metadata attached to this [`RSIndexResult`].
    ///
    /// This [`RSIndexResult`] is assumed to be [`Virtual`] with no
    ...
    pub fn reset_virt_metadata(&mut self) -> &mut Self {
        ...
        if !self.metrics.is_null() {
            // SAFETY: we know `self` still exists because this function
            // is called by the owner of the object. We also know the C type is
