...
impl Decoder for Dummy {
    fn decode<R: std::io::Read>(
        ...
        Ok(DecoderResult::Record(RSIndexResult::virt(doc_id)))
    }
}
...
fn reading_records() {
    ...
    assert_eq!(record, RSIndexResult::virt(10));

    let record = ir
        .next()
        .expect("to be able to read from the buffer")
        .expect("to get a record");
    assert_eq!(record, RSIndexResult::virt(11));

    let record = ir
        .next()
        .expect("to be able to read from the buffer")
        .expect("to get a record");
    assert_eq!(record, RSIndexResult::virt(100));
...
fn reading_over_empty_blocks() {
    ...
    let record = ir
        ...
        .expect("to be able to read from the buffer")
        .expect("to get a record");
    assert_eq!(record, RSIndexResult::virt(30));
