...
impl Decoder for Full {
    ...
    fn decode<'index>(
        ...
        // The rewind position is opaque so the real decode below cannot be CSE'd with the
        // peek above. A `black_box` on the result alone leaves one copy of the arithmetic.
        cursor.set_position(std::hint::black_box(peek_from));
