...
// ============================================================================
// BlockScoreContext tests
// ============================================================================

#[test]
fn block_score_context_default_values() {
    let ctx = BlockScoreContext::default();
    assert_eq!(ctx.idf, 1.0);
    assert_eq!(ctx.avg_doc_len, 100.0);
    assert_eq!(ctx.bm25_k1, 1.2);
    assert_eq!(ctx.bm25_b, 0.75);
}

// ============================================================================
// TfIdfBlockScorer tests
// ============================================================================
...
fn tfidf_returns_zero_for_empty_block() {
    ...
    let score = TfIdfBlockScorer::block_max_score(&block, &ctx);
    assert_eq!(score, 0.0);
}
...
fn tfidf_returns_max_for_block_without_metadata() {
    ...
    // test_with_scoring_metadata(0, ...) creates a block with max_freq=0 and num_entries=1
    let block = IndexBlock::test_with_scoring_metadata(0, 100, 1.0);
    let ctx = BlockScoreContext::default();

    // max_freq = 0 with num_entries > 0 means no metadata
    let score = TfIdfBlockScorer::block_max_score(&block, &ctx);
...
// ============================================================================
// Bm25BlockScorer tests
// ============================================================================

#[test]
fn bm25_returns_zero_for_empty_block() {
    // An empty block has valid metadata but no documents
    let block = IndexBlock::new(1);
    let ctx = BlockScoreContext::default();

    let score = Bm25BlockScorer::block_max_score(&block, &ctx);
    assert_eq!(score, 0.0);
}
...
fn bm25_shorter_docs_get_higher_scores() {
    ...
    let short_score = Bm25BlockScorer::block_max_score(&short_doc_block, &ctx);
    let long_score = Bm25BlockScorer::block_max_score(&long_doc_block, &ctx);
...
fn docscore_returns_zero_for_empty_block() {
    ...
    let score = DocScoreBlockScorer::block_max_score(&block, &ctx);
    assert_eq!(score, 0.0);
}
...
fn docscore_ignores_freq_and_doc_len() {
    ...
    let score1 = DocScoreBlockScorer::block_max_score(&block1, &ctx);
    let score2 = DocScoreBlockScorer::block_max_score(&block2, &ctx);
