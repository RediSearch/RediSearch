...
pub enum RSAggregateResult<'index> {
    Borrowed(RSAggregateResultRef<'index>),
...
impl<'index> RSAggregateResult<'index> {
    ...
    pub fn push_borrowed<'child_index>(&mut self, child: &'child_index RSIndexResult<'child_index>)
    where
        'child_index: 'index,
    {
        match self {
...
impl<'index> RSIndexResult<'index> {
    ...
    pub fn push<'child_index, 'child_children>(
        &mut self,
        child: &'child_index RSIndexResult<'child_index>,
    ) where
        'child_index: 'index,
    {
        match &mut self.data {
