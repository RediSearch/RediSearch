    ...
    pub const fn as_bytes(&self) -> &'a [u8] {
        match self.data {
            ...
            Some(data) => {
                ...
                // `Ptr<Active<'a>, u8>`, it points to memory valid for
...
/// `Option<Ptr<R, u8>>` field collapses to a bare pointer thanks to
/// `NonNull`'s niche.
#[repr(C)]
...
impl RSOffsetVector {
    ...
    pub const fn as_slice<'a>(&'a self) -> RSOffsetSlice<'a> {
        let data = if self.data.is_null() {
            ...
            // borrow), so the resulting `Ptr<Active<'a>, u8>` is valid
            // for `'a`.
            Some(unsafe { Ptr::from_non_null(nn) })
        };
        RSOffsetSlice {
            data,
            len: self.len,
