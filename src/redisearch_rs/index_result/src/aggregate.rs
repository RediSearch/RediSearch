...
impl<'query> RawAggregateResult<'query, Suspended> {
    /// The address of every entry this aggregate borrows, in order.
    ///
    /// These are exactly the addresses
    /// [`rederive_borrowed`](Self::rederive_borrowed) matches a child against,
    /// exposed so that a caller holding the live children can settle the
    /// question that method's per-child count cannot: whether *every* entry has
    /// a live child behind it. Compare each one against
    /// [`NonNull::from_ref`]`(child).cast()` for the children still in play; an
    /// entry that matches none of them cannot be re-derived, and the aggregate
    /// has to be cleared rather than re-narrowed.
    ///
    /// The addresses are erased to `NonNull<()>` deliberately: an entry is only
    /// ever compared here, never read, and a stale one designates memory that
    /// may already have been freed.
    ///
    /// An [`Owned`](Self::Owned) aggregate borrows nothing and yields nothing.
    pub fn borrowed_addresses(&self) -> impl ExactSizeIterator<Item = NonNull<()>> + '_ {
        let records: &[SharedPtr<Suspended, RawIndexResult<'query, Suspended>>] = match self {
            Self::Borrowed { records, .. } => records,
            Self::Owned { .. } => &[],
        };
        records.iter().map(|entry| entry.as_non_null().cast())
    }
    ...
    /// no live child. The count returned here says how many entries *this* child
    /// answered for, which is not on its own enough to conclude that every entry
    /// was answered for: the same child passed twice counts twice, and a total
    /// that reaches [`num_borrowed`](Self::num_borrowed) that way says nothing
    /// about the entries it skipped. Decide with
    /// [`borrowed_addresses`](Self::borrowed_addresses) instead — every entry's
    /// address must appear among the live children — and clear the aggregate
    /// when it does not. Nothing here dereferences an entry — they are compared
    /// and overwritten by address — so a stale entry is harmless until the cast,
    /// which is exactly why the check belongs on this side of it.
    #[must_use = "a count that is not checked against `borrowed_addresses` cannot \
                  tell a fully re-derived aggregate from one that must be cleared"]
