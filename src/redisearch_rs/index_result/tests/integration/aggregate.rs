...
/// Why the shortfall check is per entry and not a running total: the counts are
/// per *child*, so a child offered twice is counted twice, and a total that
/// reaches [`num_borrowed`] that way says nothing about the entry it skipped.
/// [`borrowed_addresses`] settles the question the counts cannot.
///
/// [`num_borrowed`]: index_result::RawAggregateResult::num_borrowed
/// [`borrowed_addresses`]: index_result::RawAggregateResult::borrowed_addresses
#[test]
fn a_child_offered_twice_cannot_cover_for_an_unanswered_entry() {
    let survivor = Box::new(RSIndexResult::build_numeric(1.0).doc_id(7).build());
    let dropped = Box::new(RSIndexResult::build_numeric(2.0).doc_id(7).build());

    let mut parent = RSIndexResult::build_union(2).build();
    push_borrowed(&mut parent, &survivor, MetricsVec::new());
    push_borrowed(&mut parent, &dropped, MetricsVec::new());
    ...
    // The defeat: two entries, one live child, and a total that clears the bar
    // anyway. Only the second entry was ever rewritten — it is the first that
    // has no child left, and it would have ridden the cast into freed memory.
    let counted = aggregate.rederive_borrowed(&survivor) + aggregate.rederive_borrowed(&survivor);
    assert_eq!(counted, aggregate.num_borrowed(), "the total is defeated");

    let live: NonNull<()> = NonNull::from_ref(&*survivor).cast();
    assert!(
        !aggregate.borrowed_addresses().all(|entry| entry == live),
        "asking the question per entry is not: one of the two answers to no live child",
    );
}
