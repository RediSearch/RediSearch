 ...
 *
 * Rather than reimplementing a stripped-down loop on the VecSim API, this shim
 * constructs and runs the actual hybrid top-k iterator, so the Rust-vs-C
 * comparison is apples-to-apples: both sides maintain a real top-k heap over the
 * same sorted-id child filter.
 *
 ...
 */
size_t bench_c_hybrid(VecSimIndex *index, const void *query_vec, size_t dim, size_t k,
                      t_docId *ids, size_t child_count, int force_adhoc) {
    ...
    if (it == NULL || QueryError_HasError(&err)) {
        if (it) it->Free(it);
        else if (child) child->Free(child);
