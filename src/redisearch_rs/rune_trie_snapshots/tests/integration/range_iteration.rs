...
use ffi::{
    NewTrie, Trie, Trie_IterateRange, Trie_Size, TrieSortMode_Trie_Sort_Lex, TrieType_Free, rune,
};
use libc::c_int;
...
fn run_range_query(
    ...
    let min_buf = min.map(encode_runes);
    let max_buf = max.map(encode_runes);
