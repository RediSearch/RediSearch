...
fn populated_tree(count: u64, start: f64, step: f64) -> NumericRangeTree {
...
fn test_debug_summary_populated() {
    let tree = populated_tree(50, 0.0, 1.0);
    let ctx = mock_ctx();
    let reply = capture_single_reply(|| unsafe { debug::debug_summary(ctx, &tree) });
    insta::assert_debug_snapshot!(reply, @r#"
    [
...
fn test_debug_dump_index_no_headers() {
    let tree = populated_tree(10, 1.0, 1.0);
    let ctx = mock_ctx();
...
fn test_debug_dump_tree_with_children() {
    ...
    let tree = populated_tree(100, 0.0, 1.0);
    let ctx = mock_ctx();
    let reply = capture_single_reply(|| unsafe { debug::debug_dump_tree(ctx, &tree, false) });
    with_redactions(|| insta::assert_debug_snapshot!(reply));
}
