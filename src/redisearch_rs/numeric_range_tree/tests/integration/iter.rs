...
use numeric_range_tree::{DepthFirstNumericRangeTreeIterator, NumericRangeTree};
use rstest::rstest;
...
fn test_from_node_single_leaf() {
    let tree = NumericRangeTree::new(false);
    let mut iter = DepthFirstNumericRangeTreeIterator::from_node(&tree, tree.root_index());
...
fn test_from_node_with_children() {
    ...
    let iter = DepthFirstNumericRangeTreeIterator::from_node(&tree, tree.root_index());
