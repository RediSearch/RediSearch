...
mod proptests {
    ...
    use crate::helpers::{assert_tree_invariants, gc_all_leaves};
    ...
    proptest::proptest! {
        ...
        fn prop_tree_invariants_after_operations(
            ...
            fn check_balance(tree: &NumericRangeTree, node_idx: numeric_range_tree::NodeIndex) {
                let node = tree.node(node_idx);
                if let Some((left_idx, right_idx)) = node.child_indices() {
                    let left_depth = tree.node(left_idx).max_depth();
                    let right_depth = tree.node(right_idx).max_depth();
                    let imbalance = left_depth.abs_diff(right_depth);
                    assert!(
                        imbalance <= 3,
                        "depth imbalance ({imbalance}) exceeds threshold at node"
                    );
                    check_balance(tree, left_idx);
                    check_balance(tree, right_idx);
                }
            }
            check_balance(&tree, tree.root_index());
        }
        ...
        fn prop_gc_then_trim_preserves_surviving_docs(
            ...
            gc_all_leaves(&mut tree, &|doc_id| doc_id > delete_threshold);
