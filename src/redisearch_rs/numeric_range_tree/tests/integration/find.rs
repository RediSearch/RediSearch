/*
 * Copyright (c) 2006-Present, Redis Ltd.
 * All rights reserved.
 *
 * Licensed under your choice of the Redis Source Available License 2.0
 * (RSALv2); or (b) the Server Side Public License v1 (SSPLv1); or (c) the
 * GNU Affero General Public License v3 (AGPLv3).
*/

//! Tests for `NumericRangeTree::find` — range query functionality.

use inverted_index::NumericFilter;
use numeric_range_tree::NumericRangeTree;
use rstest::rstest;

/// Build a filter with the given min/max bounds. All other fields use defaults
/// (ascending=true, offset=0, limit=0, inclusive bounds).
fn make_filter(min: f64, max: f64) -> NumericFilter {
    NumericFilter {
        min,
        max,
        ..Default::default()
    }
}

/// Build a filter with full control over ascending, offset and limit.
fn make_filter_full(
    min: f64,
    max: f64,
    ascending: bool,
    offset: usize,
    limit: usize,
) -> NumericFilter {
    NumericFilter {
        min,
        max,
        ascending,
        offset,
        limit,
        ..Default::default()
    }
}

/// Build a large tree with 50k entries whose values cycle through 1..=5000.
/// This mirrors the C `testRangeTree` setup.
fn build_large_tree(compress_floats: bool) -> NumericRangeTree {
    let mut tree = NumericRangeTree::new(compress_floats);
    for i in 1..=50_000u64 {
        let value = ((i - 1) % 5000 + 1) as f64;
        tree.add(i, value, false, 0);
    }
    tree
}

#[rstest]
#[case(false)]
#[case(true)]
fn test_find_large_tree(#[case] compress_floats: bool) {
    let tree = build_large_tree(compress_floats);
    assert_eq!(tree.num_entries(), 50_000);

    let test_ranges: &[(f64, f64)] = &[
        (0.0, 100.0),
        (10.0, 1000.0),
        (2500.0, 3500.0),
        (0.0, 5000.0),
        (4999.0, 4999.0),
        (0.0, 0.0),
    ];

    for &(min, max) in test_ranges {
        let filter = make_filter(min, max);
        let ranges = tree.find(&filter);

        // Every returned range must overlap the query bounds.
        for range in &ranges {
            assert!(
                range.overlaps(min, max),
                "range [{}, {}] does not overlap query [{min}, {max}]",
                range.min_val(),
                range.max_val(),
            );
        }
    }
}

#[rstest]
#[case(false)]
#[case(true)]
fn test_find_ascending_vs_descending(#[case] compress_floats: bool) {
    let tree = build_large_tree(compress_floats);

    let filter_asc = make_filter_full(500.0, 1500.0, true, 0, 0);
    let filter_desc = make_filter_full(500.0, 1500.0, false, 0, 0);

    let ranges_asc = tree.find(&filter_asc);
    let ranges_desc = tree.find(&filter_desc);

    // Both should return the same set of ranges (possibly in different order).
    assert_eq!(
        ranges_asc.len(),
        ranges_desc.len(),
        "ascending and descending find should return the same number of ranges"
    );

    // Collect (min_val, max_val) pairs, sort, and compare.
    let mut asc_vals: Vec<(u64, u64)> = ranges_asc
        .iter()
        .map(|r| (r.min_val().to_bits(), r.max_val().to_bits()))
        .collect();
    let mut desc_vals: Vec<(u64, u64)> = ranges_desc
        .iter()
        .map(|r| (r.min_val().to_bits(), r.max_val().to_bits()))
        .collect();
    asc_vals.sort();
    desc_vals.sort();
    assert_eq!(asc_vals, desc_vals);
}

#[test]
fn test_find_full_range() {
    let tree = build_large_tree(false);

    let filter = make_filter(f64::NEG_INFINITY, f64::INFINITY);
    let ranges = tree.find(&filter);

    // At minimum one range must be returned.
    assert!(
        !ranges.is_empty(),
        "full-range find must return at least one range"
    );

    // Every leaf range should be reachable. We can verify the total doc count
    // across all returned ranges is at least as large as num_entries (ranges
    // may overlap due to internal-node retention).
    let total_docs: u64 = ranges.iter().map(|r| r.num_docs() as u64).sum();
    assert!(
        total_docs >= tree.num_entries() as u64,
        "total docs across ranges ({total_docs}) should be >= num_entries ({})",
        tree.num_entries()
    );
}

#[test]
fn test_find_no_overlap() {
    let tree = build_large_tree(false);

    // All values are in 1..=5000, so querying below 0 should find nothing.
    let filter = make_filter(-1000.0, -1.0);
    let ranges = tree.find(&filter);
    assert!(
        ranges.is_empty(),
        "expected no ranges for query outside stored values, got {}",
        ranges.len()
    );
}

#[test]
fn test_find_single_point() {
    let tree = build_large_tree(false);

    let filter = make_filter(42.0, 42.0);
    let ranges = tree.find(&filter);

    // At least one range must overlap the point.
    assert!(
        !ranges.is_empty(),
        "single-point query must find at least one range"
    );

    for range in &ranges {
        assert!(
            range.overlaps(42.0, 42.0),
            "range [{}, {}] does not contain 42.0",
            range.min_val(),
            range.max_val(),
        );
    }
}

#[test]
fn test_find_with_offset_and_limit() {
    let tree = build_large_tree(false);

    // First get all ranges without offset/limit.
    let filter_all = make_filter(0.0, 5000.0);
    let all_ranges = tree.find(&filter_all);

    // Now use a limit.
    let filter_limited = make_filter_full(0.0, 5000.0, true, 0, 10);
    let limited_ranges = tree.find(&filter_limited);

    // Limited should return fewer or equal ranges.
    assert!(
        limited_ranges.len() <= all_ranges.len(),
        "limited find ({}) should return <= all ranges ({})",
        limited_ranges.len(),
        all_ranges.len()
    );

    // With offset, we should also get fewer or equal ranges.
    let filter_offset = make_filter_full(0.0, 5000.0, true, 100, 10);
    let offset_ranges = tree.find(&filter_offset);
    assert!(
        offset_ranges.len() <= all_ranges.len(),
        "offset find ({}) should return <= all ranges ({})",
        offset_ranges.len(),
        all_ranges.len()
    );
}

#[test]
fn test_find_on_empty_tree() {
    let tree = NumericRangeTree::new(false);

    let filter = make_filter(0.0, 100.0);
    let ranges = tree.find(&filter);

    // Empty tree has a root leaf with inverted bounds (INFINITY, NEG_INFINITY),
    // so the range should NOT overlap any finite query.
    // However the tree implementation may still return the root range
    // depending on how overlap is checked with inverted bounds.
    // We just verify no panic occurs and results are reasonable.
    for range in &ranges {
        // Any returned range should have 0 docs on an empty tree.
        assert_eq!(range.num_docs(), 0, "empty tree should have no docs");
    }
}
