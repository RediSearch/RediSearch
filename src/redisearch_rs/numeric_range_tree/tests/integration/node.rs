...
#[test]
fn test_is_leaf() {
    let mut node = NumericRangeNode::new_leaf(false);
    assert!(node.is_leaf());

    // Adding a left child makes it not a leaf
    node.set_left(Some(Box::new(NumericRangeNode::new_leaf(false))));
    assert!(!node.is_leaf());

    // Even with only one child
    node.set_left(None);
    node.set_right(Some(Box::new(NumericRangeNode::new_leaf(false))));
    assert!(!node.is_leaf());
}
...
fn test_take_left() {
    let mut node = NumericRangeNode::new_leaf(false);
    let mut child = NumericRangeNode::new_leaf(false);
