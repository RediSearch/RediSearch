...
impl NumericIndex {
    ...
    pub fn block_first_id(&self, block_idx: usize) -> Option<ffi::t_docId> {
        match self {
...
impl NumericRange {
    ...
    pub fn reset_cardinality_after_gc(
        &mut self,
        ...
        let values: Vec<f64> = {
            ...
            match &self.entries {
                NumericIndex::Uncompressed(idx) => {
                    ...
                    inverted_index::IndexReader::skip_to(&mut reader, start_id);
                    while inverted_index::IndexReader::next_record(&mut reader, &mut result)
                        .unwrap_or(false)
                    {
