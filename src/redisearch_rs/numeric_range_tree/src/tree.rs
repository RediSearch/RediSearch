...
impl NumericRangeTree {
    ...
    pub fn add(
        ...
        debug!(
            ...
            num_records = rv.num_records,
            "NumericRangeTree::add completed"
        );
    ...
    pub const fn subtract_num_entries(&mut self, count: usize) {
        self.num_entries = self.num_entries.saturating_sub(count);
    }

    /// Update the inverted indexes size by a delta (can be negative).
    pub const fn update_inverted_indexes_size(&mut self, delta: isize) {
        if delta < 0 {
            self.inverted_indexes_size =
    ...
    pub const fn increment_empty_leaves(&mut self) {
    ...
    pub fn find<'a>(&'a self, filter: &NumericFilter) -> Vec<&'a NumericRange> {
        ...
        debug!(
            ...
            total, "NumericRangeTree::find completed"
        );
    ...
    fn recursive_find_ranges<'a>(
        ...
        if filter.limit > 0 && *total >= filter.offset + filter.limit {
            debug!("Limit reached, returning early");
            return;
        }
        ...
        if let Some(range) = node.range() {
            let range_min = range.min_val();
            let range_max = range.max_val();
            ...
            if contained {
                if filter.offset == 0 {
                    *total += num_docs as usize;
                    ranges.push(range);
                    debug!(num_docs, total = *total, "Added contained range (offset=0)");
                } else {
                    *total += num_docs as usize;
                    if *total > filter.offset {
                        ...
                        debug!(
                            num_docs,
                            total = *total,
                            "Added contained range with offset"
                        );
                    }
                }
                return;
            ...
            debug!(is_leaf = node.is_leaf(), "Node has no range");
        }

        // For non-leaf nodes, descend into children
        if !node.is_leaf() {
            debug!(split_value = node.split_value(), "Descending into children");
            if filter.ascending {
