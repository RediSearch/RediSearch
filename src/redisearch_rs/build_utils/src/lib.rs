...
/// Run cbindgen using for the calling crate. Will read from the provided config file and output to the
...
pub fn run_cbinden(
    ...
    let config = cbindgen::Config::from_file(config_path).expect("Failed to find cbindgen config");
