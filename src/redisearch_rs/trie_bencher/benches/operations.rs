...
//! that can be found in the `data` folder. Besides the files generated by the `pretty_print` function,
//! in that folder you can find the cached corpus files downloaded from the internet.
//!
...
//! 3. For complicated data sources open the pretty print, e.g. `redis_wiki1k_titles_bench.txt` for [CorpusType::RedisBench1kWiki] and research the depth, etc.
