...
//! The [`TagIndex`] itself: its two storage modes, the [`Tag`] values it accepts and the
//! helpers that turn a wildcard pattern into the trie lookups used to expand it.
//!
//! See the crate-level documentation for what a tag index is and how it is queried.

use std::ffi::{CStr, c_char};
use std::ptr::NonNull;
use std::time::Instant;

use crate::{SuffixData, TagSuffixIndex, TagUniqueId, suffix};
use ffi::{
...
pub struct InMemoryMode {
    ...
    pub(crate) values: TrieMap<Box<InvertedIndex<DocIdsOnly>>>,
}
