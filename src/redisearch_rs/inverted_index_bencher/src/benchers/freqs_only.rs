...
pub struct FreqsOnlyBencher {
    test_values: Vec<FreqOnlyTestValue>,
}

#[derive(Debug)]
struct FreqOnlyTestValue {
    freq: u32,
    delta: u64,
    encoded: Vec<u8>,
}

impl FreqsOnlyBencher {
    ...
    pub fn new() -> Self {
        ...
        let test_values = freq_values
            ...
            .map(|(freq, delta)| {
                ...
                let _grew_size =
                    FreqsOnly::encode(&mut buffer, Delta::new(delta as usize), &record).unwrap();
            ...
            b.iter_batched(
                ...
                |mut buffer| {
                    for test in &self.test_values {
                        ...
                        let grew_size = FreqsOnly::encode(
                            &mut buffer,
                            Delta::new(test.delta as usize),
                            &record,
                        )
                        .unwrap();
    ...
    fn rust_decode<M: Measurement>(&self, group: &mut BenchmarkGroup<'_, M>) {
        group.bench_function("Rust", |b| {
            for test in &self.test_values {
                b.iter(|| {
                    let result = FreqsOnly.decode(test.encoded.as_slice(), 100);
