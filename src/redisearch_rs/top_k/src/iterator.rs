...
impl<'index, S: ScoreSource + 'index, C: RQEIterator<'index> + 'index> TopKIterator<'index, S, C> {
    ...
    fn advance_from_results(
        ...
        // Select the entry to yield. The filtered path leaves the child parked on
        // the chosen doc and breaks out with its score, so the record reference is
        // re-acquired below: a reference returned from inside the loop would
        // conflict with the next iteration's re-borrow of `self.child`.
        let score = loop {
            ...
            let Some(ScoredResult { doc_id, score }) = entry else {
                self.at_eof = true;
                return Ok(None);
            };

            let Some(child) = &mut self.child else {
                // Unfiltered: build a fresh result from the source.
                let result = self.source.build_result(doc_id, score);
                if self.source.is_expired(&result) {
                    continue;
                }
            ...
            // Filtered: yield the child's record so BM25 inputs survive. Results
            // come out in score order, so the child is rewound before each seek.
            child.rewind();
            let cur = match child.skip_to(doc_id)? {
                Some(SkipToOutcome::Found(r)) => r,
                Some(SkipToOutcome::NotFound(_)) | None => {
                    // Doc was matched during collection but the child can't find it
                    // now — its state changed between collection and yield. Treat as
                    // EOF rather than panicking; surface via at_eof.
                    self.at_eof = true;
                    return Ok(None);
                }
            };
            if self.source.is_expired(cur) {
                continue;
            }
            self.last_doc_id = doc_id;
            break score;
        ...
        let cur = self
            ...
            .expect("child is parked on the doc it just skipped to");
