...
impl<'index, S: ScoreSource + 'index, C: RQEIterator<'index> + 'index> TopKIterator<'index, S, C> {
    ...
    fn collect_adhoc(&mut self) -> Result<(), RQEIteratorError> {
        ...
        if scope.0.should_rerank() && !self.heap.is_empty() {
            ...
            // carrying each entry's captured record along. The count never
            // exceeded k, so every entry is retained.
            for (entry, scored) in entries.into_iter().zip(scored) {
                self.heap
                    .push_with_record(scored.doc_id, scored.score, entry.record);
            }
        }
    ...
    fn advance_from_results(
        ...
        loop {
            ...
            let expired = self.source.is_expired(doc_id);
