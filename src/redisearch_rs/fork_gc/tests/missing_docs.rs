...
fn add_entry(dict: &mut OwnedDict, field_name: &[u8], ii: *mut std::ffi::c_void) {
    ...
    let hs = unsafe {
        ffi::NewHiddenString(field_name.as_ptr().cast(), field_name.len(), false)
    };
...
fn empty_dict_writes_only_terminator() {
    ...
    assert!(matches!(Frame::decode(&mut cursor).unwrap(), Frame::Terminator));
}
...
fn entry_with_deleted_docs_writes_delta_frame() {
    ...
    ii.add_record(&RSIndexResult::build_virt().doc_id(1).build()).unwrap();
    ii.add_record(&RSIndexResult::build_virt().doc_id(2).build()).unwrap();
...
fn multiple_entries_write_multiple_delta_frames() {
    let make_ii = |doc_id| {
        let mut ii = InvertedIndex::<DocIdsOnly>::new(IndexFlags_Index_DocIdsOnly);
        ii.add_record(&RSIndexResult::build_virt().doc_id(doc_id).build()).unwrap();
    ...
    assert_eq!(
        field_names_seen,
        [b"field_a".to_vec(), b"field_b".to_vec()]
    );
...
fn roundtrip_protocol_is_decodable() {
    let mut ii = InvertedIndex::<DocIdsOnly>::new(IndexFlags_Index_DocIdsOnly);
    ii.add_record(&RSIndexResult::build_virt().doc_id(5).build()).unwrap();
    let ii = Box::new(OpaqueInvertedIndex::DocIdsOnly(ii));
    let ii_ptr = Box::into_raw(ii);
...
fn delta_frame_serialisation_roundtrips_via_msgpack() {
    let mut ii = InvertedIndex::<DocIdsOnly>::new(IndexFlags_Index_DocIdsOnly);
    ii.add_record(&RSIndexResult::build_virt().doc_id(3).build()).unwrap();
    let ii = Box::new(OpaqueInvertedIndex::DocIdsOnly(ii));
    let ii_ptr = Box::into_raw(ii);
