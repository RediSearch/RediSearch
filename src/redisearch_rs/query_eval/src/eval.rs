...
pub fn eval_node<'index>(
    ...
    match node.as_enum() {
        ...
        QueryNode::Ids { keys, doc_ids } => Some(eval_ids(keys, doc_ids)),
...
///
/// * `keys` — SDS key strings from the query node, kept only for the length
///   invariant against `doc_ids`.
...
fn eval_ids<'index>(keys: &[ffi::sds], doc_ids: Option<&[DocId]>) -> Evaluated<'index> {
    debug_assert!(
        doc_ids.is_some(),
        "QN_IDS nodes must carry pre-resolved doc_ids"
    );
    // `doc_ids` must line up positionally with `keys` (they describe the same
    // id-filter node).
    debug_assert!(
    ...
    // Ids are resolved on the main thread during query construction; drop the
    // misses (unresolved keys encoded as 0). The unwrap fallback only keeps
    // release builds from panicking if an invalid caller violates the invariant.
