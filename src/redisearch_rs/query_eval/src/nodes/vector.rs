...
fn bind_metric_request(
    ...
    let wiring = match iterator_type {
        IteratorType::Hybrid => KeyWiring::Hybrid,
        IteratorType::MetricSortedById
        | IteratorType::MetricSortedByScore
        | IteratorType::MetricLazySortedById
        | IteratorType::MetricLazySortedByScore => KeyWiring::Metric,
        _ => return,
    };
    ...
    let own_key = match wiring {
        // SAFETY: the discriminant says this is a hybrid iterator, and we hold
        // it exclusively.
        //
        // The cast is C's view of the key — its header — widened back to the
        // whole key the header is the first field of.
        KeyWiring::Hybrid => unsafe { ffi::HybridIterator_GetOwnKeyRef(it.as_ptr()) }.cast(),
        // SAFETY: the discriminant says this is one of the metric iterators,
        // built by a metric constructor, and we hold it exclusively; the borrow
        // it is typed with is discarded rather than relied upon, as above.
        KeyWiring::Metric => unsafe { metric::own_key_ref(it) },
    };

    // SAFETY: `id` was reserved by `add_metric_request` on this same context;
    // `own_key` points into the iterator, which outlives the handle (it clears
    // the handle's validity flag when freed).
    let handle = unsafe { ctx.bind_metric_request_key(id, own_key) };

    match wiring {
        // SAFETY: as above, and `handle` is a valid handle that lives as long
        // as the AST. The cast is the same handle as C declares it.
        KeyWiring::Hybrid => unsafe {
            ffi::HybridIterator_SetKeyHandle(it.as_ptr(), handle.cast());
        },
        // SAFETY: as above.
        KeyWiring::Metric => unsafe { metric::set_key_handle(it, handle) },
    }
}

/// Which pair of accessors reaches an iterator's key wiring.
///
/// Chosen once from the iterator's type, so a family that grows an own-key slot
/// has to be handled at both the read and the write, rather than falling through
/// to the wrong accessor at one of them.
enum KeyWiring {
    /// A hybrid iterator, whose accessors are still in C.
    Hybrid,
    /// One of the metric iterators.
    Metric,
}
