...
// Compare Rust tag_strtolower against the FFI `tag_strtolower`.
// The FFI function uses `std::alloc::alloc`/`dealloc` for reallocation,
// so the input buffer must be allocated via the global allocator.
#[cfg(not(miri))]
mod ffi_comparison {
    ...
    unsafe extern "C" {
        #[link_name = "tag_strtolower"]
        fn ffi_tag_strtolower(
            pstr: *mut *mut std::os::raw::c_char,
            len: *mut usize,
            case_sensitive: std::ffi::c_int,
        );
    }

    /// Call `tag_strtolower` via FFI and return the resulting string.
    fn c_tag_strtolower(s: &str, case_sensitive: bool) -> String {
        // Allocate with the global allocator: the FFI function uses
        // `std::alloc::dealloc` to free the old buffer when reallocating.
        let buf_len = s.len() + 1; // +1 for null terminator
        // SAFETY: buf_len > 0, alignment 1 is always valid.
        let layout = unsafe { Layout::from_size_align_unchecked(buf_len, 1) };
        // SAFETY: layout has non-zero size.
        let buf = unsafe { alloc(layout) };
        assert!(!buf.is_null(), "tag_ffi_comparison: allocation failed");
        ...
        let mut len = s.len();
        // SAFETY: `ptr` is a valid, null-terminated buffer allocated via
        // the global allocator. `tag_strtolower` may dealloc and replace it.
        unsafe {
            ffi_tag_strtolower(&mut ptr, &mut len, if case_sensitive { 1 } else { 0 });
        }

        // SAFETY: `ptr` (possibly reallocated) is valid for `len` bytes.
        let result = unsafe { std::slice::from_raw_parts(ptr.cast::<u8>(), len) }.to_vec();
        // SAFETY: `ptr` was allocated via `std::alloc::alloc` (either the
        // original or a replacement allocated by the FFI function).
        unsafe {
            dealloc(ptr.cast(), Layout::from_size_align_unchecked(len + 1, 1));
        }

        String::from_utf8(result).expect("tag_strtolower FFI result must be valid UTF-8")
    }
