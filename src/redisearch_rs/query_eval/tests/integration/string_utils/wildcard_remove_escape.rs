...
use query_eval::string_utils::wildcard_remove_escape;

// Cases mirror tests/ctests/test_wildcard.c:test_removeEscape,
// covering beginning / mid / end positions.

#[test]
fn no_escapes() {
    let result = wildcard_remove_escape("foo");
...
#[test]
fn empty() {
    let result = wildcard_remove_escape("");
    assert!(matches!(result, Cow::Borrowed(_)));
    assert_eq!(result, "");
}
...
fn escape_at_beginning() {
    assert_eq!(wildcard_remove_escape("\\foo"), "foo");
}
...
#[test]
fn escape_mid() {
    assert_eq!(wildcard_remove_escape("f\\oo"), "foo");
}

#[test]
fn double_escape_mid() {
    assert_eq!(wildcard_remove_escape("f\\\\oo"), "f\\oo");
}
...
fn consecutive_escapes() {
    assert_eq!(wildcard_remove_escape("\\a\\b\\c"), "abc");
}

#[test]
fn only_backslashes() {
    // \\ → second \, then trailing \ is dropped
    assert_eq!(wildcard_remove_escape("\\\\\\"), "\\");
}

// ── wildcard_remove_escape vs ffi::Wildcard_RemoveEscape ────────
// Verify that the Rust port matches the C implementation for every
// test vector.
...
mod ffi_comparison {
    ...
    /// Call the C `Wildcard_RemoveEscape` via FFI and return the resulting bytes.
    fn c_wildcard_remove_escape(input: &[u8]) -> Vec<u8> {
        let mut buf = input.to_vec();
        buf.push(0); // null terminator expected by C
    ...
    fn assert_matches_c(input: &str) {
        ...
        let rust_result = wildcard_remove_escape(input);
        assert_eq!(
    ...
    /// Generate ASCII strings biased toward backslashes.
