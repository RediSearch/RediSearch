...
impl<'index> SearchResult<'index> {
    ...
    pub const fn row_data(&self) -> &rlookup::RLookupRow<'index, RSValueFFI> {
        &self._row_data
    }

    /// Returns a mutable reference to the [`RLookupRow`][ffi::RLookupRow] of this search result.
    pub const fn row_data_mut(&mut self) -> &mut rlookup::RLookupRow<'index, RSValueFFI> {
        &mut self._row_data
    }
