...
/// (the default).
///
/// This exercises the `IteratorType_UNION_ITERATOR` branch in `CRQEIterator::sort_weight`.
/// The `prioritize=true` arm (which returns `ui.num as f64`) is not tested here because
/// it would require mutating the global `RSGlobalConfig`, which is unsafe to do in a
/// multi-threaded test binary.
///
/// FIXME: also test `prioritize=true` → `ui.num as f64`. Mutating `RSGlobalConfig` is
/// not safe in a shared test binary without serializing against all other tests that
/// may read the flag.
///
