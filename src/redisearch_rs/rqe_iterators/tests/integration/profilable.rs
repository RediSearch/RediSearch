...
fn not_wraps_child_and_self() {
    ...
    let not = Not::new(child, 5, 1.0, None::<TimeoutContextClock>);
...
fn not_empty_child() {
    let child = Mock::new([]);
    let not = Not::new(child, 3, 1.0, None::<TimeoutContextClock>);
    let mut profiled = Profile::new(not);
...
fn not_skip_to() {
    ...
    let not = Not::new(child, 10, 1.0, None::<TimeoutContextClock>);
    let mut profiled = Profile::new(not);
