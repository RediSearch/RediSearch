...
fn not_with_child() {
    let mut replier = init();
    let child = rqe_iterators::Empty;
    let iter = rqe_iterators::not::Not::new(child, 100, 1.0, std::time::Duration::ZERO, true);
    let reply = print(&mut replier, &iter, false);
    insta::assert_debug_snapshot!(reply);
}
...
fn not_optimized_with_child() {
    ...
    let child = rqe_iterators::Empty;
    let iter = rqe_iterators::not_optimized::NotOptimized::new(wcii, child, 100, 1.0, None);
    let reply = print(&mut replier, &iter, false);
    insta::assert_debug_snapshot!(reply);
}
