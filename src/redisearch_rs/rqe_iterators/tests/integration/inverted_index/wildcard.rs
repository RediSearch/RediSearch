...
use inverted_index::{RSIndexResult, doc_ids_only::DocIdsOnly, opaque::OpaqueEncoding};
...
mod not_miri {
    ...
    impl WildcardRevalidateTest {
        ...
        fn create_iterator(&self) -> Wildcard<'_, DocIdsOnly> {
            let ii = DocIdsOnly::extract_ii_from(self.test.context.wildcard_inverted_index())
                .expect("expected DocumentIdOnly variant");
    ...
    fn wildcard_revalidate_after_document_deleted() {
        let test = WildcardRevalidateTest::new(10);
        let mut it = test.create_iterator();
        let ii = DocIdsOnly::extract_ii_from_mut(test.test.context.wildcard_inverted_index())
            .expect("expected DocumentIdOnly variant");
