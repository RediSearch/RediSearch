...
mod not_miri {
    ...
    fn tag_revalidate_after_index_disappears() {
        ...
        // Simulate the tag's inverted index being garbage collected and then
        // recreated: drop the tag's posting list and register a fresh, empty one.
        // The reader still holds a (now dangling) raw pointer to the old list, but
        // `should_abort` only compares pointers via `points_to_ii`
        // (`std::ptr::eq`), never dereferencing it.
        ...
        let tag_index = unsafe { tag_index.as_mut() };
        tag_index.delete_tag_value(b"test_tag");
        tag_index
            .open_index(b"test_tag", true)
            .expect("a fresh, empty posting list is registered");
    ...
    mod via_resume {
        ...
        fn tag_revalidate_after_index_disappears() {
            ...
            // Simulate the tag's inverted index being garbage collected and then
            // recreated: drop the tag's posting list and register a fresh, empty
            // one. The index owns its posting lists, so `delete_tag_value` frees
            // the old one and the test has nothing left to clean up.
            let mut tag_index = test.test.context.tag_index();
            // SAFETY: `tag_index` points to the `TagIndex` owned by `context`,
            // which outlives this call, and the iterator is suspended here.
            let tag_index = unsafe { tag_index.as_mut() };
            tag_index.delete_tag_value(b"test_tag");
            tag_index
                .open_index(b"test_tag", true)
        ...
        fn tag_revalidate_after_document_deleted() {
            ...
            let mut tag_index = test.test.context.tag_index();
            // SAFETY: `tag_index` points to the `TagIndex` owned by `context`,
            // which outlives this borrow; the helper below only reads the iterator
            // between revalidations, per the revalidation protocol.
            let ii = unsafe { tag_index.as_mut() }
                .find_value_mut(b"test_tag")
                .expect("test_tag should be indexed");
        ...
        fn tag_revalidate_after_triemap_entry_removed() {
            ...
            // Simulate the garbage collector removing every document for the tag,
            // which drops the tag from the values trie. The reader still holds a
            // (now dangling) raw pointer to the freed list, but `should_abort` sees
            // the tag is missing and returns true without dereferencing it.
            let mut tag_index = test.test.context.tag_index();
            // SAFETY: `tag_index` points to the `TagIndex` owned by `context`,
            // which outlives this call, and the iterator is suspended here.
            unsafe { tag_index.as_mut() }.delete_tag_value(b"test_tag");
