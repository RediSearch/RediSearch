...
use inverted_index::{RSIndexResult, RSOffsetSlice, doc_ids_only::DocIdsOnly};
...
impl TagBaseTest {
    fn expected_record(doc_id: t_docId) -> RSIndexResult<'static> {
        RSIndexResult::with_term(None, RSOffsetSlice::empty(), doc_id, RS_FIELDMASK_ALL, 1)
            .weight(0.0)
    }
...
mod not_miri {
    ...
    fn tag_revalidate_after_index_disappears() {
        ...
        // is a valid TrieMap.
        let trie = unsafe { &mut *tag_index.as_ref().values.cast::<triemap_ffi::TrieMap>() };
