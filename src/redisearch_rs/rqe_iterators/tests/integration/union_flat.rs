...
mod via_resume {
    ...
    /// usable rather than merely well-addressed. A full union is the interesting
    /// shape for this: its aggregate carries one entry per child sitting on the
    /// document, so the count varies with the data rather than being fixed at
    /// one per child.
    #[test]
    ...
    /// slot over every survivor where the read path picks by scan order over the
    /// active region, and any child on the
    ...
    fn quick_resume_republishes_a_single_backer() {
        ...
        let mut union = UnionQuickFlat::new(boxed_children(child0, child1, child2));
        assert_eq!(union.read().unwrap().unwrap().doc_id, 10);
    ...
    ///
    /// A conforming child cannot get there. `advance_and_find_min` leaves no
    /// active child behind the union, and exhaustion is terminal across the cycle
    /// ([`at_eof`](RQEIterator::at_eof)), so one dropped from the active set stays
    /// dropped. The mock breaks that on purpose, and its lever is
    /// [`MockData::set_force_read_none`]: a `read` that answers `None` while the
    /// child is neither exhausted nor out of documents, leaving it parked on a
    /// live, lagging position. (The legacy twin uses
    ...
    fn resume_asserts_when_a_lagging_child_lands_behind_a_full_union() {
        ...
        // so the union parks it in the dead tail with a live, lagging position.
    ...
    /// no test can observe).
    ///
    /// child1 is spent and parked in the dead tail before the suspend, so a resume
    /// that walked only the active region would leave it suspended, and the rewind
    ...
    fn resume_then_rewind_revives_the_exhausted_children_too() {
        ...
        assert_eq!(
            (0..4)
                ...
                .collect::<Vec<_>>(),
            [10, 20, 30, 40],
            "reading past 20 parks child1 in the dead tail",
        );
        ...
        assert_eq!(
            drain(&mut union),
            [10, 20, 30, 40, 60, 70],
            "the rewind must revive every child, including the one parked as spent",
        );
    }
    ...
    /// The settle's `min_doc_id < original_last_doc_id` recovery, reached the only
    /// way a conforming union can reach it: `skip_to_quick` returns on the first
    /// exact match, stranding a live sibling behind the union. That makes the
    ...
    fn resume_aborts_when_the_result_is_no_longer_a_union() {
        ...
        for substitute in [
            ...
            let mut union = Box::new(UnionFullFlat::new(vec![
                TypeErasedRQEIterator::new(Box::new(child0)),
                TypeErasedRQEIterator::new(Box::new(child1)),
            ]));
