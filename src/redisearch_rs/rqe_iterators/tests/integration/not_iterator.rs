...
fn read_skips_child_docs() {
    ...
    let mut it = Not::new(IdListSorted::new(child_ids), 10, 1.0);
...
fn read_with_empty_child_behaves_like_wildcard() {
    ...
    let mut it = Not::new(IdListSorted::new(vec![]), 5, 1.0);
...
fn read_with_child_covering_full_range_yields_no_docs() {
    let mut it = Not::new(IdListSorted::new(vec![1, 2, 3, 4, 5]), 5, 1.0);
...
fn skip_to_honours_child_membership() {
    let mut it = Not::new(IdListSorted::new(vec![2, 4, 7]), 10, 1.0);
...
fn skip_to_child_doc_at_max_docid_returns_none() {
    ...
    let mut it = Not::new(IdListSorted::new(vec![2, 5, 10]), 10, 1.0);
...
fn skip_to_child_ahead_returns_found() {
    let mut it = Not::new(IdListSorted::new(vec![5, 10]), 15, 1.0);
...
fn skip_to_child_at_eof_returns_found() {
    let mut it = Not::new(IdListSorted::new(vec![1, 2]), 10, 1.0);
