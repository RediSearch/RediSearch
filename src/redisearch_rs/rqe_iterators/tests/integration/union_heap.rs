...
//!
//! These tests mirror the comprehensive tests in `union_flat.rs` but target
//! the heap-based union iterator. They use [`Mock`] iterators which provide
//! observability features like `read_count()` and `revalidate_count()` that
//! allow us to verify not just correctness but also efficiency optimizations.
...
fn heap_union_cases(#[case] num_children: usize, #[case] base_result_set: &[u64]) {}

/// Type alias for the union variant used in these tests.
type Union<I> = UnionFullHeap<'static, I>;
...
fn skip_to_edge_cases() {
    // Quick mode - child already at target doc_id
    {
        ...
        let mut quick_iter = UnionQuickHeap::new(children);
    ...
    // Full mode - child at EOF before skip_to
    {
        ...
        let mut full_iter = UnionFullHeap::new(children);
...
fn mode_quick_variant_produces_same_doc_ids() {
    ...
    let mut full_iter = UnionFullHeap::new(full_children);
    ...
    let mut quick_iter = UnionQuickHeap::new(quick_children);
