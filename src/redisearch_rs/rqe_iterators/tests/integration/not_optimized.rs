...
fn read_test(wc_ids: Vec<t_docId>, child_ids: Vec<t_docId>, max_doc_id: t_docId) {
    ...
    let mut it = NotOptimized::new(wcii, child, max_doc_id, 1.0, None::<TimeoutContextClock>);
...
fn read_continuous_child_empty_wc() {
    ...
    let mut it = NotOptimized::new(wcii, child, 10, 1.0, None::<TimeoutContextClock>);
    assert!(it.read().unwrap().is_none());
    assert!(it.at_eof());
}
...
fn skip_to_found_in_wc_not_in_child() {
    ...
    let mut it = NotOptimized::new(wcii, child, 15, 1.0, None::<TimeoutContextClock>);
...
fn skip_to_not_in_wc_returns_not_found() {
    ...
    let mut it = NotOptimized::new(wcii, child, 25, 1.0, None::<TimeoutContextClock>);
...
fn num_estimated_returns_wcii_estimate() {
    ...
    let wcii = wc_helper.create_wildcard();
    let child = MockVec::new(vec![2, 4]);
    let it = NotOptimized::new(wcii, child, 10, 1.0, None::<TimeoutContextClock>);
...
mod revalidate {
    ...
    fn create_not_optimized(
        ...
        let it = NotOptimized::new(wcii, child, 100, 1.0, None::<TimeoutContextClock>);
        (it, child_data)
    }
    ...
    fn revalidate_wc_moved_to_eof() {
        ...
        let mut child_data = child.data();
        child_data.set_revalidate_result(MockRevalidateResult::Ok);
        let mut it = NotOptimized::new(wcii, child, 200, 1.0, None::<TimeoutContextClock>);
