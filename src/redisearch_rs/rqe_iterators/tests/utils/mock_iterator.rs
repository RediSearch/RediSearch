...
impl MockData {
    ...
    pub fn set_error_at_done(
        &mut self,
        maybe_err: Option<rqe_iterators::RQEIteratorError>,
    ) -> &mut Self {
...
struct MockDataInternal {
    ...
    read_count: usize,
    error_at_done: Option<rqe_iterators::RQEIteratorError>,
}
...
impl<'index, const N: usize> RQEIterator<'index> for MockIterator<'index, N> {
    ...
    fn read(
        ...
        data.read_count += 1;
        if self.at_eof() {
            return if let Some(err) = data.error_at_done.take() {
                Err(err)
            } else {
                Ok(None)
            };
