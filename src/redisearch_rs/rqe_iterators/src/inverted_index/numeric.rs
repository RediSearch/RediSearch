...
impl<'query, Rf: Ref, R, E> RawNumeric<'query, Rf, R, E> {
    ...
    E: ExpirationChecker + 'static,
{
        ...
        'query: 'a,
    {
        ...
        // suspended. `guard` witnesses the read lock the refresh requires.
        let outcome = self.it.refresh_pointers(guard);
...
pub enum NumericIteratorVariantSuspended<'query> {
    ...
    Unfiltered(RawNumeric<'query, Suspended, NumericIndexReaderSuspended, FieldExpirationChecker>),
