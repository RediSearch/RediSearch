...
/// yet) or when no sub-range matches the filter. Shared by the FFI numeric-filter
/// entry point and the Rust query evaluator so both build the identical iterator
...
pub unsafe fn build_numeric_filter_iterator(
    ...
    unsafe {
        crate::union_opaque::build_union(
            children,
            true,
            min_union_iter_heap,
            node_type,
