...
use crate::{
    IteratorType, RQEIterator, RQEIteratorBoxed, RQEIteratorError, RQESuspendedIterator, SkipToOutcome, SkipToOutcomeRaw,
...
pub struct RawInvIndIterator<Rf: Ref, R, E = NoOpChecker> {
    ...
    skip_to_impl:
        fn(&mut Self, t_docId) -> Result<Option<SkipToOutcomeRaw<'_, Rf>>, RQEIteratorError>,
}
...
impl<Rf: Ref, R, E> RawInvIndIterator<Rf, R, E> {
    ...
    pub(crate) const fn last_doc_id_field(&self) -> t_docId {
        self.last_doc_id
    }
