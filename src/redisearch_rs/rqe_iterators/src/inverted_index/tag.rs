...
pub struct Tag<'index, E: DecodedBy, C = crate::expiration_checker::NoOpChecker> {
    ...
    C: ExpirationChecker,
{
    ...
    /// in the [`TagIndex`]'s [`TrieMap`](triemap_ffi::TrieMap).
    ///
    ...
    /// 5. `tag_index.values` must be a valid non-null [`TrieMap`](triemap_ffi::TrieMap) pointer.
        ...
        debug_assert!(
            {
                ...
                if !tag_idx.values.is_null() {
                    ...
                    let trie = unsafe { &*tag_idx.values.cast::<triemap_ffi::TrieMap>() };
