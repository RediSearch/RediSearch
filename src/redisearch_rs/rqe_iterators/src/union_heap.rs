    ...
    S: RQESuspendedIterator<'query>,
{
        ...
        'query: 'a,
    {
        ...
        // re-derived. All `kept` of them are offered, including the ones the heap
        // left out as exhausted, because an entry can point at any child the
        ...
        // move a survivor onto the very slot the dropped child vacated, so a
        // stale entry can match the wrong child (see its `# What this cannot
        // see`). The entries go instead. Nothing is lost by that — a drop sets
        // `any_change`, and every `any_change` exit below either rebuilds the
        // result or ends the union.
        let rederived = {
            ...
            if kept < len {
                ...
                // `S::Resumed<'a>` — same size and alignment as the `S` the
                // buffer was allocated for, statically enforced by
                // `resume_child_slot_in_place` — and `raw` is exclusively owned,
                // so the slice is unaliased.
                let children =
