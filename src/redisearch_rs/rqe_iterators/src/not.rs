        ...
        'query: 'a,
    {
        ...
        match unsafe { resume_child_slot_in_place(child_slot, guard) } {
            ...
            Err(e) => {
                ...
                // the box normally (frees `result` + the allocation). The empty
                // arm carries no `S` payload, so writing it typed as the resumed
                // slot and dropping it through the suspended type read the same
                // bytes: a `#[repr(C)]` discriminant and an `Empty` leaf,
                ...
                // SAFETY: every field of `raw` is initialised again; reclaim the
                // allocation `Box::into_raw` released above and drop it.
