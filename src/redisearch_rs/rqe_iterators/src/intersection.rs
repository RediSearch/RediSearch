...
use crate::{
    ...
    boxed::{
        RederiveOutcome, ResumeSlotOutcome, assert_layout_compatible, rederive_aggregate_entries,
    ...
    S: RQESuspendedIterator<'query>,
{
        ...
        'query: 'a,
    {
        ...
        // cast. An intersection never relocates a child, so an entry's address
        // still identifies the child it was taken from and re-derivation is
        ...
        if !any_child_moved || active.is_eof {
            // Unless it does not. A cleared aggregate keeps the position but no
            // longer says which children back it, and this is the one path that
            // hands the result straight back — there is no re-seek below it to
            // rebuild what was dropped. An intersection that cannot account for
            // a contributing child could not have agreed on the document anyway,
            // so it aborts rather than publish a consensus it can no longer
            // show. (An intersection already at EOF publishes no `current`, so a
            // cleared aggregate has no observer there and `revalidate`'s `Ok` is
            // still the faithful answer.)
            if rederived == RederiveOutcome::Cleared && !active.is_eof {
                return Ok(ResumeOutcome::Aborted);
            }
        ...
        // `is_eof` suppresses `current`, so a cleared aggregate stays unobserved.
        if moved_to_eof {
            active.is_eof = true;
        ...
        // which is also what makes a `Cleared` harmless here — and an `Err`
