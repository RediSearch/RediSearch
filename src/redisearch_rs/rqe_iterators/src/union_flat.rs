...
use crate::{
    ...
    boxed::{
        RederiveOutcome, ResumeSlotOutcome, clear_aggregate_entries, rederive_aggregate_entries,
        resume_child_slot_in_place,
    },
};
    ...
    S: RQESuspendedIterator<'query>,
{
        ...
        'query: 'a,
    {
        ...
        //
        // With nothing dropped no survivor moved, so an entry's address still
        // identifies the child it was taken from and the entries can be
        // re-derived. All `kept` of them are offered, including any parked past
        // `num_active`, because an entry can point at any child the aggregate was
        // built from.
        //
        // A drop is the other case, and it is the one
        // `rederive_aggregate_entries` cannot be asked about: the compaction can
        // move a survivor onto the very slot the dropped child vacated, so a stale
        // entry can match the wrong child (see its `# What this cannot see`). The
        // entries go instead. Nothing is lost by that — a drop sets `any_change`,
        // and every `any_change` exit below either rebuilds the result or ends the
        // union.
        let rederived = {
            ...
            if kept < len {
                clear_aggregate_entries(result)
            } else {
                ...
                let children = unsafe {
                    std::slice::from_raw_parts_mut(
                        base.cast::<IndexedChild<S::Resumed<'a>>>(),
                        kept,
                    )
                };
        ...
        if !any_change {
            ...
            // faithful answer.)
            if rederived == RederiveOutcome::Cleared {
                return Ok(ResumeOutcome::Aborted);
            }
            return Ok(ResumeOutcome::Ok(active));
        }
