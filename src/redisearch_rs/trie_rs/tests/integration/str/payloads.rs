...
//! Assert `StrTrieMap` reproduces the C trie's payload roundtrip and
...
//!
//! - `StrTrieMap::<V>::insert(&mut self, key, value) -> Option<V>` so
...
fn insert_with_payload(
    trie: &mut StrTrieMap<LoggingPayload>,
    term: &str,
    label: &str,
) -> i32 {
