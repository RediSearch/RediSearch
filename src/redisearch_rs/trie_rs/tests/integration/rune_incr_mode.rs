...
//! Assert `RuneTrieMap` reproduces the C trie's `ADD_INCR` insert-mode
...
//!
//! `RuneTrieMap` is value-agnostic — score, numDocs and payload bytes are
//! NOT stored on the node. Instead, this test (and others in the crate)
//! defines a local `TermEntry` and instantiates `RuneTrieMap<TermEntry>`.
...
//! `RuneTrieMap<TermEntry>` (or a newtype around it); the mode-aware
