...
//!
//! Semantics pinned by the shared snapshots under
//! `rune_trie_snapshots/tests/integration/snapshots/lex_iterate_*`:
//!
...
/// holding only positions where `val <= max_edits`.
...
impl SparseAutomaton {
    /// Initial DP row: `[(0, 0), (1, 1), ..., (max, max)]`.
    ...
    /// fallback edge taken by any input char outside the query alphabet.
...
/// One DFA node. `state` is kept only to drive the build-time cache
/// lookup (linear scan — faster than a hash for the tiny queries we get
/// in practice).
struct DfaNode {
    distance: u32,
    is_match: bool,
...
impl Dfa {
    ...
    /// For each query position currently active in `parent.state`,
    /// follow the edge for that query char (creating/caching the
    /// resulting DFA node); then always add the wildcard fallback edge.
...
/// One frame in the trie traversal stack — per-node iterator state.
struct TrieFrame<'tm, Data> {
    node: &'tm Node<Data>,
    /// Label decoded as UTF-8 once at frame push.
    label: Vec<char>,
    /// Current char position within `label`.
    label_offset: usize,
...
impl<'tm, Data> IterateDfaIter<'tm, Data> {
    ...
    /// One DFA filter step: push the next DFA state onto the stacks with
    /// the running-min distance, and report whether the new position is
    /// accepting.
    fn filter(
        dfa: &Dfa,
        prefix_mode: bool,
