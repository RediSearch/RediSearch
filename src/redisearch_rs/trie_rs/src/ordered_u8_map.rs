...
impl<T: FirstCharGetter> OrderedU8Map<T> {
    ...
    pub(crate) fn get(&self, key: u8) -> Option<&T> {
        ...
        let index = m.binary_search_by_key(&key, |e| e.get_first_char()).ok()?;
        m.get(index as u32)
    }
    ...
    pub(crate) fn get_or_create<F: FnOnce() -> T>(&mut self, key: u8, create: F) -> &mut T {
        ...
        let index = match m.binary_search_by_key(&key, |e| e.get_first_char()) {
            Ok(index) => index,
            Err(index) => {
                let val = create();
                m.insert(index as u32, val);
                index
            }
        };
        &mut m[index]
    ...
    pub(crate) fn insert(&mut self, key: u8, val: T) -> bool {
        let m = self.map.get_or_insert(Box::new(LowMemoryVec::new()));
        match m.binary_search_by_key(&key, |e| e.get_first_char()) {
            Ok(_index) => false,
            Err(index) => {
                m.insert(index as u32, val);
                true
