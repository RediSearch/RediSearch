...
/// One pending visit. `parent_key_len` is what the iterator's `key` length
/// should be just before this node is processed — popping a frame truncates
/// `key` to that length before extending it with `node.label()`. This replaces
/// the explicit "revisit" frames a tree DFS would otherwise need.
struct Frame<'tm, Data, S> {
    node: &'tm Node<Data>,
    state: S,
    parent_key_len: u32,
}
...
impl<'tm, Data, A: Automaton> AutomatonIter<'tm, Data, A> {
    ...
    pub(crate) fn advance(&mut self) -> Option<&'tm Data> {
        loop {
            ...
            // Restore the key to this node's parent depth, then extend with
            // its label. The truncate handles backtracking inline — we don't
            // push a separate revisit frame for the way back up the stack.
            ...
            match class {
                StateClass::Permanent => {
                    // Every descendant matches without further automaton
                    // work — push them all with the same state, no stepping.
                ...
                StateClass::Terminal => {
                    // Sink: no descendant can match. Skip pushing children.
                }
                StateClass::Live | StateClass::LiveAccepting => {
                    // Standard path: step each child's label, push survivors.
                    for child in node.children().iter().rev() {
                        if let Some(child_state) = self.automaton.step_all(&state, child.label()) {
                            self.stack.push(Frame {
                                node: child,
                                state: child_state,
                                parent_key_len: key_len_after_node,
