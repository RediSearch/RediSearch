...
unsafe extern "C" fn TrieMap_FindPrefixes(
    ...
    str: *const c_char,
    len: tm_len_t,
    results: *mut MaybeUninit<TrieMapResultBuf>,
) -> c_int {
    debug_assert!(!t.is_null(), "t cannot be NULL");
    if len > 0 {
        debug_assert!(!str.is_null(), "str cannot be NULL if len > 0");
...
/// - `min` can be NULL only if `minlen == 0`. It is not necessarily NULL-terminated.
/// - `minlen` can be 0. If so, `min` is regarded as an empty string.
/// - `max` can be NULL only if `maxlen == 0`. It is not necessarily NULL-terminated.
