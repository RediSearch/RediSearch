...
/// this is the null value of a RSValue, that we cannot link from the
/// C side.
const RS_NULLVAL: ffi::RSValue = ffi::RSValue {
    __bindgen_anon_1: ffi::RSValue__bindgen_ty_1 { numval: 0.0 },
    _bitfield_align_1: [],
    _bitfield_1: __BindgenBitfieldUnit::new([1u8]),
    refcount: 1,
};

fn rs_nullval() -> *mut ffi::RSValue {
    &RS_NULLVAL as *const ffi::RSValue as *mut ffi::RSValue
}
...
mod c_layer {
    ...
    /// 1. The pointer must be a valid pointer to an [`RSSortingVector`] created by [`NewSortingVector`].
    #[unsafe(no_mangle)]
    unsafe extern "C" fn RSSortingVector_GetLength(vec: NonNull<RSSortingVector>) -> libc::size_t {
        // Safety: Caller must ensure 1. --> Deref is safe
        unsafe { vec.as_ref() }.len() as libc::size_t
    }
