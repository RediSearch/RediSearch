...
//!
//! - `fold_libnu` exercises libnu's `nu_tofold` table (used by RediSearch's
//!   `runeFold` in `src/trie/rune_util.c`) but re-encodes the multi-
//!   codepoint output through Rust's native UTF-8 — so it does *not* pay
//!   the `nu_utf8_write` cost. Use it as a lower bound for libnu fold cost,
//!   not a faithful reproduction of the C call site.
//! - `lower_libnu` uses the same per-codepoint loop pattern as the C
//!   `unicode_tolower` hot path in `src/util/strconv.h`.
//! - ICU sides call `CaseMapper::fold_string` and
//!   `CaseMapper::lowercase_to_string` (root locale, `und`).

use std::hint::black_box;

use casemap_bench::{ALL, fold_icu, fold_libnu, lower_icu, lower_libnu};
