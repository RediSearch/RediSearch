...
use ffi::{
    DocTable_Exists, QueryError, QueryIterator, RedisSearchCtx, RedisSearchDiskIndexSpec,
...
pub unsafe extern "C" fn TagIndex2_Index(
    ...
    // SAFETY: in disk mode `ctx`/`batch` are the valid disk write context and
    // batch handle for this document write (contract), and every tag borrows
    // from the NUL-terminated C string it was built from just above, so the byte
    // past each one is a readable zero.
    let result = unsafe { tag_index.index(ctx, batch, &tags, doc_id, has_field_expiration) };
    match result {
...
pub unsafe extern "C" fn TagIndex2_Commit(
    ...
    // Tag bytes are NUL-free throughout the crate, exactly as C's own consumers
    // see them after `strlen` — the suffix trie NUL-terminates its own copies.
    let tags: Vec<&[u8]> = value_ptrs
        .iter()
        .map(|&ptr| {
            debug_assert!(!ptr.is_null(), "tag value pointers must not be null");
