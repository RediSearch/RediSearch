...
pub unsafe extern "C" fn Rust_TagIndex_OpenReader(
    ...
    match unsafe { RustTagIndex::mode(tag_index) } {
        Mode::InMemory => {
            ...
            let reader = unsafe { idx.open_reader(sctx, tag, weight, lookup) };
...
pub unsafe extern "C" fn Rust_TagIndexValueIter_Next<'trie>(
    ...
    // A `bool` return has no room for the decoding error, so a corrupt posting
    // block reads as the end of the list here — as it does at every other
    // inverted-index FFI boundary.
    iter.0.next_record(res).unwrap_or_default()
}
...
pub unsafe extern "C" fn Rust_TagIndex_GC(
    ...
    // SAFETY: contract 3 — ownership transfers here. Taking the box first is what
    // makes the early returns below discharge it too.
    let delta = *unsafe { Box::from_raw(delta) };
    // SAFETY: contract 2.
    let tag = unsafe { borrow_tag(tag, len) };
    let Some(tag) = Tag::new(tag) else {
        // A tag with an interior NUL cannot be in the index: the tries are keyed
        // by NUL-free bytes, so there is nothing for the delta to apply to.
...
/// pointers are usable as C strings as they are. Only the array itself is owned
/// by the caller.
///
/// # Safety
///
/// As [`build_ptr_array`].
unsafe fn collect_matches<'a>(matches: impl Iterator<Item = &'a CStr>) -> *mut *mut c_char {
    let elems: Vec<*const c_char> = matches.map(CStr::as_ptr).collect();
    if elems.is_empty() {
        return std::ptr::null_mut();
    }
