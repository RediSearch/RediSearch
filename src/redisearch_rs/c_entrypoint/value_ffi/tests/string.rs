...
fn new_string_creates_rm_alloc_string() {
    ...
    let str_ptr = unsafe { RSValue_String_Get(value.as_ref(), &mut out_len) };
...
fn string_get_accepts_null_lenp() {
    ...
    let str_ptr = unsafe { RSValue_String_Get(value.as_ref(), std::ptr::null_mut()) };
...
fn string_ptr_len_with_string_value() {
    ...
    let str_ptr = unsafe { RSValue_StringPtrLen(value.as_ref(), &mut out_len) };
...
fn string_ptr_len_dereferences_ref_to_string() {
    ...
    let mut out_len: size_t = 0;
    let str_ptr = unsafe { RSValue_StringPtrLen(ref_value.as_ref(), &mut out_len) };
    let bytes = unsafe { std::slice::from_raw_parts(str_ptr as *const u8, out_len) };
...
fn string_ptr_len_follows_trio_left_to_string() {
    ...
    let mut out_len: size_t = 0;
    let str_ptr = unsafe { RSValue_StringPtrLen(trio.as_ref(), &mut out_len) };
    let bytes = unsafe { std::slice::from_raw_parts(str_ptr as *const u8, out_len) };
...
fn set_string_replaces_value_with_rm_alloc_string() {
    ...
    unsafe { RSValue_SetString(value.as_ref(), str_ptr, 7) };

    let mut out_len: u32 = 0;
    let result_ptr = unsafe { RSValue_String_Get(value.as_ref(), &mut out_len) };
...
fn set_const_string_replaces_value_with_borrowed_string() {
    ...
    unsafe { RSValue_SetConstString(value.as_ref(), c"constant".as_ptr(), 8) };
