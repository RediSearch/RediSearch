...
use value::{Array, SharedValue, SharedValueRef, Value};
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn RSValue_NewArrayBuilder(len: u32) -> *mut Option<SharedValue> {
    ...
    let array = unsafe { Box::<[MaybeUninit<Option<SharedValue>>]>::assume_init(array) };

    Box::into_raw(array).cast::<Option<SharedValue>>()
}
...
pub unsafe extern "C" fn RSValue_NewArrayFromBuilder(
    values: *mut SharedValue,
    len: u32,
) -> SharedValue {
    // Safety: ensured by caller (1.)
    let array: Box<[SharedValue]> =
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn RSValue_ArrayLen(value: SharedValueRef) -> u32 {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn RSValue_ArrayItem(value: SharedValueRef, index: u32) -> SharedValueRef {
    if let Value::Array(array) = &**value {
...
mod test {
    ...
    fn test_array() {
        unsafe {
            ...
            assert_eq!(RSValue_ArrayLen(array.as_ref()), 3);

            let val = RSValue_ArrayItem(array.as_ref(), 0);
            let num = RSValue_Number_Get(val);
            assert_eq!(1.0, num);

            let val = RSValue_ArrayItem(array.as_ref(), 1);
            let num = RSValue_Number_Get(val);
            assert_eq!(2.0, num);

            let val = RSValue_ArrayItem(array.as_ref(), 2);
            let num = RSValue_Number_Get(val);
            assert_eq!(3.0, num);
