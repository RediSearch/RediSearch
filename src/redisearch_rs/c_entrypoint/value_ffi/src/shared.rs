...
use crate::RSValue;
use crate::util::{expect_value, expect_shared_value, into_rs_value, into_shared_value};
...
pub unsafe extern "C" fn RSValue_IncrRef(value: *const RSValue) -> *mut RSValue {
    ...
    into_rs_value(SharedValue::clone(&shared_value)).cast_mut()
}
...
pub unsafe extern "C" fn RSValue_Replace(dstpp: *mut *mut RSValue, src: *const RSValue) {
    ...
    unsafe {
        *dstpp = into_rs_value(clone).cast_mut();
    }
}
