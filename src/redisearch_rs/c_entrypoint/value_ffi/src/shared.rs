...
pub unsafe extern "C" fn SharedRsValue_NewStringArray(
    ...
    strs.iter()
        ...
        .map(|str| {
            ...
            let val_ptr = unsafe { SharedRsValue_NewCString(str) };
            // Safety: `val_ptr` is a valid pointer obtained through `SharedRsValue::into_raw`.
            unsafe { SharedRsValue::from_raw(val_ptr) }
        })
        .enumerate()
        // Safety: `i` does not exceed the capacity of `array`.
        .for_each(|(i, v)| unsafe { array.inner_mut().write_entry(v, i as u32) });

    let shared_value = SharedRsValue::array(array);
    shared_value.into_raw()
}
...
pub unsafe extern "C" fn SharedRsValue_NewConstStringArray(
    ...
    strs.iter()
        ...
        .map(|(str, len)| {
            // Safety: caller must ensure (3), and therefore `str` is valid to pass to
            // `SharedRsValue_NewCString`.
            let ptr = unsafe { SharedRsValue_NewConstString(str, len) };
            // Safety: `val_ptr` is a valid pointer obtained through `SharedRsValue::into_raw`.
            unsafe { SharedRsValue::from_raw(ptr) }
        })
        .enumerate()
        // Safety: `i` does not exceed the capacity of `array`.
        .for_each(|(i, v)| unsafe { array.inner_mut().write_entry(v, i as u32) });
