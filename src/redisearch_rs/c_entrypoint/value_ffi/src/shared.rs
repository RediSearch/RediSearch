...
///
/// # Safety
/// - (1) `v` must originate from a call to [`RsValue_DynRef`](crate::RsValue_DynRef)
///
...
pub extern "C" fn RsValue_NewTrio(
    left: OpaqueDynRsValueRef,
    middle: OpaqueDynRsValueRef,
    right: OpaqueDynRsValueRef,
) -> OpaqueDynRsValue {
    // Safety: caller must ensure (1), and therefore `left`, `middle`, and `right` are valid to pass to
    // `DynRsValueRef::from_opaque`.
    let left = unsafe { DynRsValueRef::from_opaque(left) };
    // Safety: see above
    let middle = unsafe { DynRsValueRef::from_opaque(middle) };
    // Safety: see above
    let right = unsafe { DynRsValueRef::from_opaque(right) };

    let left = left.to_shared();
    let middle = middle.to_shared();
    let right = right.to_shared();
...
#[unsafe(no_mangle)]
pub extern "C" fn RsValue_ToShared(v: OpaqueDynRsValueRef) -> SharedRsValue {
    // Safety: caller must ensure `v` is valid to pass to `DynRsValueRef::from_opaque`.
