...
/// - `str` must point to a valid, C string with a length of at most `u32::MAX` bytes;
/// - `str` must point to a location allocated using `rm_alloc`
...
pub unsafe extern "C" fn SharedRsValue_NewString(
    ...
    // Safety: caller must ensure that `str` is not NULL.
    let str = unsafe { debug_expect!(str) };
    // Safety: the safety requirements of this function
    // uphold those of `SharedRsValue::rm_alloc_string`
    unsafe { SharedRsValue::rm_alloc_string(str, len) }
}
...
pub unsafe extern "C" fn SharedRsValue_NewCString(str: Option<NonNull<c_char>>) -> SharedRsValue {
    ...
    // Safety: caller must ensure the length of `str` does not exceed `u32::MAX`
    let len =
        unsafe { debug_expect!(len.try_into(), "Length of str cannot be more than u32::MAX") };
...
///
/// # Safety
/// - The passed pointer must be non-null and valid for reads.
...
pub unsafe extern "C" fn SharedRsValue_NewBorrowedRedisString(
    ...
    // Safety: caller must ensure that `str` is non-null.
    let str = unsafe { debug_expect!(str) };
    // Safety: the safety requirements of this function uphold those
...
///
/// # Safety
/// - `str` must be non-null and point to a valid [`RedisModuleString`]
...
///
/// # Safety
/// - `str` must be a valid const pointer to a char sequence of `len` bytes.
///
...
pub unsafe extern "C" fn SharedRsValue_NewParsedNumber(
    ...
    // Safety: caller must ensure `str` is a valid pointer to a char sequence of `len` bytes.
...
///
/// # Safety
/// - If `sz > 0`, `str` must be a valid pointer
...
pub unsafe extern "C" fn SharedRsValue_NewStringArray(
    ...
    let strs = if sz == 0 {
        ...
        // Safety: If `sz` > 0, caller must ensure that `str` is not null.
        let strs = unsafe { debug_expect!(strs) };
        // Safety: If `sz` > 0, caller must ensure that `strs` is a valid pointer
        // that is valid for reads of `sz` bytes.
    ...
    strs.iter()
        .copied()
        // Safety: caller must ensure that `strs` is a sequences of valid, NULL
        // terminated C strings, and therefore `str` is valid to pass to
...
/// - If `sz > 0`, `str` must be a valid pointer
///   to a sequence if valid NULL-terminated C strings of length `sz`;
/// - For each of the strings `str` in `strs`, `strlen(str)` must not exceed `u32::MAX`.
///
/// @param strs Array of string pointers
/// @param sz Number of strings in the array
/// @return A pointer to a heap-allocated RsValue array
...
pub unsafe extern "C" fn SharedRsValue_NewConstStringArray(
    ...
    strs.iter()
        .copied()
        .map(|str| {
            // Safety: caller must ensure each item in `strs` points
            // to a valid, null-terminated C string.
            let len = unsafe { strlen(str) };
            // Safety: caller must ensure each string is no longer than `u32::MAX` bytes.
            let len =
                unsafe { debug_expect!(len.try_into(), "`strlen(str)` must note exceed u32::MAX") };
            (str, len)
        })
        // Safety: caller must ensure that `strs` is a sequences of valid, NULL
        // terminated C strings, and therefore `str` is valid to pass to
...
///
/// # Safety
/// - `v` must be a number value.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn SharedRsValue_Number_Get(v: &SharedRsValue) -> f64 {
    // Safety: caller must ensure `v` is a number value.
    unsafe { debug_expect!(v.get_number(), "v must be of type 'Number'") }
}
