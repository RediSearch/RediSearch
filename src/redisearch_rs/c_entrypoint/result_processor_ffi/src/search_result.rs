...
use result_processor::{SearchResultFlags, bindings::DocumentMetadata};
use std::ptr::{self, NonNull};

/// cbindgen:ignore
pub type SearchResult = result_processor::SearchResult<'static>;
...
/// [valid]: https://doc.rust-lang.org/std/ptr/index.html#safety
#[unsafe(no_mangle)]
pub unsafe extern "C" fn SearchResult_MoveToHeap(
...
pub unsafe extern "C" fn SearchResult_GetIndexResultMut(
    res: Option<NonNull<SearchResult>>,
) -> Option<NonNull<RSIndexResult<'static>>> {
    // Safety: ensured by caller (1.)
    let res = unsafe { res.unwrap().as_mut() };
...
pub unsafe extern "C" fn SearchResult_SetIndexResult(
    res: Option<NonNull<SearchResult>>,
    index_result: Option<NonNull<RSIndexResult<'static>>>,
) {
    // Safety: ensured by caller (1.)
    let res = unsafe { res.unwrap().as_mut() };
