...
macro_rules! debug_expect {
    ($opt:expr) => {{
        ...
        $crate::debug_expect!(opt, concat!(stringify!($opt), " must not be NULL"))
    }};
    ($opt:expr, $msg:expr) => {{
        ...
        #[inline(always)]
        unsafe fn do_expect<T, E: $crate::debug_expect::Expectable<T>>(opt: E, msg: &str) -> T {
