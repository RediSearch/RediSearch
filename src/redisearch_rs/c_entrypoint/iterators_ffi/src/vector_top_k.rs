...
pub unsafe extern "C" fn NewVectorTopKIterator(
    ...
    // ── Reducer (mirrors C's HybridIteratorReducer) ───────────────────────────
    ...
    if child.is_null() {
        ...
        let child_raw = child;
        // SAFETY: `child` is a valid, non-null, owning pointer with all
        // callbacks populated per caller's contract.
        #[expect(clippy::multiple_unsafe_ops_per_block)]
        let child_iter = unsafe { CRQEIterator::new(NonNull::new_unchecked(child)) };
...
pub unsafe extern "C" fn HybridIterator_GetSearchModeString(
    ...
    let mode = match mode {
        ...
        TopKMode::Batches => VecSimSearchMode_VECSIM_HYBRID_BATCHES,
