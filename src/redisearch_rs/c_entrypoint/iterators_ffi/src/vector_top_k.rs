...
pub unsafe extern "C" fn NewVectorTopKIterator(
    ...
    // Build the per-field expiration checker when caller supplies both `sctx` and
    // `filter_ctx`.
    ...
    // `TopKIterator` is root-only: `skip_to` is `unimplemented!()`. Clear
    // the vtable entry so the C highlighter falls back to sequential reads.
    // To be noted this shouldn't be necessary because that path is accessed only when the index result is not available,
    // which should not happen because it is copied when requested at that point, but as long as C code path exist,
    // and the complexity of the code is this high, it's a small price to pay to patch the vtable.
