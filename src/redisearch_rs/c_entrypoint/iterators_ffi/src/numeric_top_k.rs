...
pub unsafe extern "C" fn NewNumericTopKIterator(
    ...
    // SAFETY: precondition 4 guarantees `sctx` is valid and outlives the returned iterator, which
    ...
    let timeout = unsafe { AnyTimeoutContext::from_sctx(sctx_nn, TIMEOUT_CHECK_GRANULARITY) };
...
) -> &'static RQEIteratorWrapper<NumericTopKFfi> {
