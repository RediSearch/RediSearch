...
use rqe_iterators::{
    IteratorsConfig, NumericIteratorVariant, RQEIterator, RQESuspendedIterator,
    c2rust::CRQEIterator,
...
impl RQESuspendedIterator for NumericIteratorSuspended {
    ...
    fn resume<'a>(
        ...
        let inner_suspended_ptr = unsafe { ptr::addr_of_mut!((*active_raw).iterator) }
            as *mut <NumericIteratorVariant<'static> as RQEIterator<'static>>::Suspended;
        ...
        let inner_suspended_box: Box<
            <NumericIteratorVariant<'static> as RQEIterator<'static>>::Suspended,
        > = {
...
pub unsafe extern "C" fn NewNumericFilterIterator(
    ...
    let children: Vec<CRQEIterator> = variants
        ...
        .map(|iterator| {
            let ptr =
                RQEIteratorWrapper::boxed_new(NumericIterator::with_filter(filter_ptr, iterator));
