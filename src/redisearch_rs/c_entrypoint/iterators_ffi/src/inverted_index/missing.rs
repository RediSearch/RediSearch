...
impl<'query> RQESuspendedIterator<'query> for MissingIteratorSuspended<'query> {
        ...
        'query: 'a,
    {
        ...
        // Step 2: whole-box cast Suspended → Active (preserves the heap
        // allocation so the FFI wrapper's `header.current` stays valid).
        let raw = Box::into_raw(self);
...
impl<'query> MissingIteratorSuspended<'query> {
