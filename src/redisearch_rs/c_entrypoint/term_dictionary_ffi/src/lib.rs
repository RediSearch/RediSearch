...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn TermDictionary_RdbSave(io: *mut RedisModuleIO, t: *const TermDictionary) {
...
///   of the call.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn TermDictionary_RdbLoad(io: *mut RedisModuleIO) -> *mut TermDictionary {
    debug_assert!(!io.is_null(), "io cannot be NULL");

    // Wrapping the pointer is safe; the caller's validity guarantee on `io`
    // covers the RDB reads performed through it for the duration of the call.
    let mut rm_io = RmIo::new(io);
    match TermDictionary::rdb_load(&mut rm_io) {
