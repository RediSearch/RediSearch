...
pub unsafe extern "C" fn TermSuffixIndex_IterateWildcard(
    ...
    // A NULL predicate means "never stop"; otherwise defer the stop
    // decision (e.g. a deadline check) entirely to the caller.
    // Safety: ensured by caller (4.)
    let should_stop = || should_stop.is_some_and(|f| unsafe { f(stop_ctx) });
    let Some(matches) = index.iter_wildcard_until(pattern, should_stop) else {
        return 0;
    };
    for term in matches {
        // Safety: ensured by caller (3.)
