...
pub unsafe extern "C" fn TrieMapIterator_Next(
    ...
    if let Some(IteratorTimeoutState { deadline, counter }) = timeout {
        *counter += 1;
        // Probe the clock once every 100 entries, matching C's
        // `TIMEOUT_COUNTER_LIMIT`. The cadence is part of the observable
        // behaviour — how far past the deadline an iteration may run — so it must
        // not differ between debug and release builds.
        if *counter == 100 {
