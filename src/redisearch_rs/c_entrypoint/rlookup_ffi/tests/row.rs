...
fn rlookuprow_move() {
    ...
    let mut src = src.with_rlookup_id(lookup.id());

    #[cfg_attr(debug_assertions, allow(unused_mut))]
    let mut dst = RLookupRow::new();
    #[cfg(debug_assertions)]
    let mut dst = dst.with_rlookup_id(lookup.id());
...
extern "C" fn RSValue_IncrRef(v: Option<NonNull<ffi::RSValue>>) -> *mut ffi::RSValue {
    ...
    let v = v.unwrap();
    let refcount_ptr = unsafe { v.byte_add(offset_of!(ffi::RSValue, refcount)).cast::<u16>() };
...
/// Mock implementation of `RSValue_IncrRef` for testing purposes
#[unsafe(no_mangle)]
extern "C" fn RSValue_Decref(v: Option<NonNull<ffi::RSValue>>) {
    let v = v.unwrap();
    let refcount_ptr = unsafe { v.byte_add(offset_of!(ffi::RSValue, refcount)).cast::<u16>() };
    let refcount = unsafe { AtomicU16::from_ptr(refcount_ptr.as_ptr()) };
...
/// Mock implementation of `RSValue_IncrRef` for testing purposes
#[unsafe(no_mangle)]
extern "C" fn RS_NumVal(numval: f64) -> *mut ffi::RSValue {
    Box::into_raw(Box::new(ffi::RSValue {
        __bindgen_anon_1: ffi::RSValue__bindgen_ty_1 { numval },
