...
use numeric_range_tree::NumericRangeTree as NumericRangeTreeInner;
...
pub unsafe extern "C" fn NumericRangeTree_DebugSummary(
    ...
    let empty_tree;
    let tree: &NumericRangeTreeInner = if t.is_null() {
        empty_tree = NumericRangeTreeInner::default();
        &empty_tree
    } else {
        // SAFETY: Caller ensures `t` is a valid pointer per function safety docs.
        let NumericRangeTree(inner) = unsafe { &*t };
        inner
    };
