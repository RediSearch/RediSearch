...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn NumericRangeNode_GetRangeMut(
    node: *mut NumericRangeNode,
) -> *mut NumericRange {
    debug_assert!(!node.is_null(), "node cannot be NULL");
    // SAFETY: Caller ensures `node` is valid per function safety docs.
    let node = unsafe { &mut *node };
    match node.range_mut() {
        Some(range) => range as *mut NumericRange,
