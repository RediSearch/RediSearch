...
// ============================================================================
// NumericRange HLL accessor functions (for fork GC)
// ============================================================================
...
pub unsafe extern "C" fn NumericRange_SetHllRegisters(
    ...
    debug_assert!(!range.is_null(), "range cannot be NULL");
    debug_assert!(!registers.is_null(), "registers cannot be NULL");
    // SAFETY: Caller ensures `range` is valid per function safety docs.
    let range = unsafe { &mut *range };
    // SAFETY: Caller ensures `registers` points to valid 64-byte array.
    let regs: [u8; 64] = unsafe { std::ptr::read(registers as *const [u8; 64]) };
    range.set_hll_registers(regs);
}
...
/// - `range` must point to a valid mutable [`NumericRange`] and cannot be NULL.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn NumericRange_HllAdd(range: *mut NumericRange, value: f64) {
    debug_assert!(!range.is_null(), "range cannot be NULL");
    // SAFETY: Caller ensures `range` is valid per function safety docs.
    let range = unsafe { &mut *range };
