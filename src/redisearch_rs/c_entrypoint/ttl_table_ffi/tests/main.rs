...
#[test]
fn debug_allocated_buckets_returns_zero_for_null() {
    let buckets = unsafe { TimeToLiveTable_DebugAllocatedBuckets(ptr::null()) };
    assert_eq!(buckets, 0);
}

#[test]
fn debug_allocated_buckets_grows_after_first_add() {
    with_table(|t| {
        assert_eq!(unsafe { TimeToLiveTable_DebugAllocatedBuckets(t) }, 0);
        let v = into_ffi([fe(0, FUTURE)]);
        unsafe { TimeToLiveTable_Add(t, 0, v) };
        // First grow seeds at 64 (TTL_BUCKET_INITIAL_CAP).
        assert!(unsafe { TimeToLiveTable_DebugAllocatedBuckets(t) } >= 1);
    });
}
