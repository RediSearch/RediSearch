...
pub unsafe extern "C" fn TimeToLiveTable_Add(
    ...
    unsafe { inner.add(doc_id, field_expirations) };
}
...
///    [`TimeToLiveTable`].
#[unsafe(no_mangle)]
pub const unsafe extern "C" fn TimeToLiveTable_DebugAllocatedBuckets(
    table: *const TimeToLiveTable,
) -> usize {
    if table.is_null() {
        return 0;
...
///
/// An uninitialized `FieldExpirations` (from C side) is treated as length `0`.
///
/// # Safety
///  - `v` must be non-null.
...
pub unsafe extern "C" fn FieldExpirations_Len(v: *const FieldExpirations) -> usize {
    ...
    // SAFETY: caller guarantees `v` is non-null and the first pointer-sized
    // word at `*v` is readable.
    if unsafe { is_uninitialized(v) } {
        return 0;
    }
...
///
/// An uninitialized `FieldExpirations` (from C side) yields the empty slice,
/// encoded as `ptr == NULL` and `len == 0`.
...
///
/// Calling this on a C-zero-initialized `FieldExpirations` is a no-op.
///
...
pub unsafe extern "C" fn FieldExpirations_Free(v: *mut FieldExpirations) {
    ...
    // SAFETY: not uninitialized, so `*v` is a properly constructed
    // `FieldExpirations`, and the caller guarantees exclusive access.
    let v = unsafe { &mut *v };
...
unsafe fn build_ft_slice<'a>(ptr: *const u16, len: usize) -> &'a [u16] {
    if len == 0 {
        ...
        // SAFETY: the caller guarantees it
        unsafe { slice::from_raw_parts(ptr, len) }
    }
}

/// Returns `true` if `*v` is a C-zero-initialized [`FieldExpirations`]
/// (its inner pointer is NULL) rather than one constructed through Rust.
///
/// `Document::fieldExpirations` is born zero-bytes on the C side and only
/// gets a real [`FieldExpirations`] when the first per-field expiration is
/// encountered. The no-HFE path leaves the slot zeroed, yet `_Len`,
/// `_AsSlice` and `_Free` are still called on it; this helper lets those
/// entry points short-circuit before dispatching any [`ThinVec`] method
/// through a NULL inner pointer (which would be UB).
///
/// Distinguishing the two states is sound because [`FieldExpirations`] is
/// `#[repr(transparent)]` over [`ThinVec`], whose first word is a heap-header
/// pointer. [`ThinVec::new`] uses a non-null static empty-header sentinel,
/// so a NULL first word can only come from C zero-initialization — never
/// from a Rust constructor.
///
/// [`ThinVec`]: thin_vec::ThinVec
/// [`ThinVec::new`]: thin_vec::ThinVec::new
///
/// # Safety
///  - `v` must be non-null and the first pointer-sized word at `*v` must
///    be readable.
#[inline]
const unsafe fn is_uninitialized(v: *const FieldExpirations) -> bool {
    // SAFETY: caller guarantees `v` is non-null and the first pointer-sized
    // word at `*v` is readable. The cast is sound because `FieldExpirations`
    // is `#[repr(transparent)]` over `ThinVec`, whose layout begins with a
    // single pointer (asserted at module scope).
    unsafe { (*v.cast::<*const core::ffi::c_void>()).is_null() }
}
