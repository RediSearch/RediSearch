...
impl RSValueTrait for RSValueFFI {
    ...
    fn as_str(&self) -> Option<&str> {
        ...
        if p.t() == ffi::RSValueType_RSValue_String {
            let len: usize = 0;
            // Safety: We checked that the type is a string, so we can call RSValue_StringPtrLen safely.
            let strptr: *const u8 =
                unsafe { ffi::RSValue_StringPtrLen(self.0.as_ptr(), len as *mut usize).cast() };

            // Safety: We checked for the length above, so we can create a slice safely.
            let slice = unsafe { slice::from_raw_parts(strptr, len) };

            // Safety: We assume it is valid UTF-8
            let reval = unsafe { str::from_utf8_unchecked(slice) };
            Some(reval)
