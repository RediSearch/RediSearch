...
impl RSValueTrait for RSValueFFI {
    ...
    fn deep_deref(&self) -> &Self {
        ...
        unsafe { &*(deref_ptr as *const RSValueFFI) }
    }
    ...
    fn as_str_bytes(&self) -> Option<&[u8]> {
        if self.get_type() == ffi::RSValueType_RSValueType_String {
            ...
            let cstr: *mut c_char =
                unsafe { ffi::RSValue_String_Get(self.0.as_ptr(), std::ptr::null_mut()) };
            ...
            // Safety: We assume the C string is valid.
            Some(unsafe { slice::from_raw_parts(cstr.as_ptr().cast(), cstr.count_bytes()) })
        } else {
            None
        }
    }
