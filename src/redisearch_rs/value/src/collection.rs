...
///
/// # Invariants
/// - Can hold at most [`Self::MAX_CAPACITY`] items, which on 32-bit systems
...
impl<T> RsValueCollection<T> {
    ...
    ///
    /// # Safety
    /// - The created `RsValueCollection`'s items must all be initialized using [`RsValueCollection::write_entry`]
