...
impl RmAllocString {
    ...
    /// - (2) if `len > 0`, `str` must point to a valid, C string with a length of at most `u32::MAX` bytes;
...
impl RsValueStringData {
    ...
    unsafe fn copy_from_str_unchecked(s: &str) -> Self {
        ...
        assert!(
            !s_cpy.is_null(),
            "Failed to allocate memory for RsValueStringData"
        );
...
impl Clone for RsValueStringData {
    fn clone(&self) -> Self {
        // Safety: invariants (1) and (2)
        unsafe { Self::copy_from_str_unchecked(self.as_str()) }
    }
}
...
impl RsValueString {
    ...
    pub fn as_str(&self) -> &str {
        self.s.as_ref().map(|s| s.as_str()).unwrap_or("")
    }
}
