...
//! between Rust and C. cheadergen generates the C header (`rqe_iterator_type.h`)
//! from this crate, and the C header `iterator_api.h` includes it.
//!
//! If `IteratorType` lived inside `rqe_iterators`, its generated header
//! (`iterators_ffi.h`) would need to include `iterator_api.h` (for `QueryIterator`
//! used in function signatures), while `iterator_api.h` would need to include
//! `iterators_ffi.h` (for `IteratorType`), creating a circular include.
//!
//! By placing the enum in its own crate with its own header, we break the cycle:
//! `iterator_api.h` includes `rqe_iterator_type.h` (tiny, no other includes),
//! and `iterators_ffi.h` includes `iterator_api.h` (which already has the enum).
//!
