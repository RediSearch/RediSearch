...
#include "field.h"
// `FieldExpiration` is defined by the existing C header `src/ttl_table/ttl_table.h`
// (and read into Rust via bindgen). Forward-declare here so signatures using
// `*const FieldExpiration` / `FieldExpirations` parse without pulling in the
// full C header (which would cause a circular include).
...
// `FieldExpirations` is `#[repr(transparent)]` over `ThinVec<FieldExpiration, u64>`,
// which is itself a single non-null pointer to a heap-side `Header<u64>`.
// Passing it by value across the FFI therefore moves exactly one pointer.
// We declare it as a one-field struct here so cheadergen-emitted FFI
// signatures that take it by value compile under `[package.ttl_table] types = "skip"`.
typedef struct FieldExpirations {
    void *ptr;
} FieldExpirations;
 ...
 */
void TimeToLiveTable_VerifyInit(TimeToLiveTable * *table, size_t max_size);
 ...
 */
void TimeToLiveTable_Add(TimeToLiveTable *table, t_docId doc_id, FieldExpirations field_expirations);
 ...
 */
void TimeToLiveTable_Remove(TimeToLiveTable *table, t_docId doc_id);
 ...
 */
bool TimeToLiveTable_IsEmpty(const TimeToLiveTable *table);
 ...
 *  - `table` must point to a valid, initialized [`TimeToLiveTable`].
 */
struct FieldExpirationSlice TimeToLiveTable_GetFieldExpirations(const TimeToLiveTable *table, t_docId doc_id);
 ...
 */
bool TimeToLiveTable_VerifyDocAndField(const TimeToLiveTable *table, t_docId doc_id, uint16_t field_index, enum FieldExpirationPredicate predicate, const t_expirationTimePoint *expiration_point);
 ...
 */
bool TimeToLiveTable_VerifyDocAndFieldMask(const TimeToLiveTable *table, t_docId doc_id, uint32_t field_mask, enum FieldExpirationPredicate predicate, const t_expirationTimePoint *expiration_point, const uint16_t *ft_id_to_field_index);
 ...
 */
bool TimeToLiveTable_VerifyDocAndWideFieldMask(const TimeToLiveTable *table, t_docId doc_id, t_fieldMask field_mask, enum FieldExpirationPredicate predicate, const t_expirationTimePoint *expiration_point, const uint16_t *ft_id_to_field_index);
