...
//   Header<u64> { len: u64, cap: u64 }  (16 bytes, no padding for pointer-aligned T)
...
static inline size_t RSSortingVector_Length(const RSSortingVector *v) {
  // Header<u64> layout: { len: u64, cap: u64 }
  // len is at offset 0.
...
static inline RSValue *RSSortingVector_Get(const RSSortingVector *v, size_t idx) {
  // Data starts at offset 8 (sizeof(Header<u64>), no padding for pointer-aligned T).
