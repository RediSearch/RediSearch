 ...
 */
typedef struct ThinVec_c_void__u16 ThinVec_c_void__u16;

/**
 * Callback type for passing to [`TrieMap_IterateRange`].
 */
typedef void (*TrieMapRangeCallback)(const int8_t *, uintptr_t, void *, void *);
 ...
 * [`NewTrieMap`]: crate::NewTrieMap
 */
void TrieMap_IterateRange(struct TrieMap *trie, const char *min, int minlen, bool includeMin, const char *max, int maxlen, bool includeMax, TrieMapRangeCallback callback, void *ctx);
 ...
 * - `buf` must point to a valid TrieMapResultBuf initialized by [`TrieMap_FindPrefixes`] and cannot be NULL.
 */
void *TrieMapResultBuf_GetByIndex(TrieMapResultBuf *buf, uintptr_t index);
