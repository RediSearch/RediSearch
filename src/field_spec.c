...
void FieldSpec_Cleanup(FieldSpec* fs) {
  ...
  if (FIELD_IS(fs, INDEXFLD_T_VECTOR)) {
    ...
    if (fs->vectorOpts.vecSimIndex) {
      // diskCtx.indexName identifies a disk-backed handle. It is set after
      // fresh storage opens, or before storage binding for an unbound vector
      // index loaded from RDB, so both paths use the disk teardown API.
