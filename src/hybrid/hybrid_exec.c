...
int hybridCommandHandler(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, bool internal, bool coordinator) {
  ...
  QueryError status = {0};
  HybridRequest *hybridRequest = MakeDefaultHybridRequest();
...
static void HREQ_Execute_Callback(blockedClientHybridCtx *BCHCtx) {
  ...
  if (HybridRequest_InternalBuildPipelineAndExecute(hreq, hybridParams, outctx, sctx, &status, BCHCtx->internal, BCHCtx->coordinator)) {
