...
static inline void debugPauseStoreResultsHybrid(HybridRequest *hreq, bool before) {
  ...
  if (!RequestSyncCtx_UsesDeferredReply(hreq->syncCtx)) {
    return;
  }
 ...
 * Stores results in the request sync context so serializeStoredResults_hybrid can be called
 * from the reply_callback on the main thread.
 *
 * @param hreq The hybrid request
 * @param results Pipeline results (ownership transferred to the stored reply state)
...
void HREQ_ReplyOrStoreError(HybridRequest *hreq, RedisModuleCtx *ctx, QueryError *status) {
  if (RequestSyncCtx_UsesDeferredReply(hreq->syncCtx)) {
    ...
    QueryError_CloneFrom(status, &stored->err);
...
int HybridRequest_StartSingleCursor(HybridRequest *hreq, RedisModule_Reply *reply, bool coord) {
    HybridRequest *req = hreq;
...
static inline void replyWithCursors(RedisModuleCtx *replyCtx, arrayof(Cursor*) cursors,
    ...
    for (size_t i = 0; i < array_len(cursors); i++) {
      ...
      Cursor_Pause(cursor);
      AREQ *areq = Cursor_GetAREQ(cursor);
...
int HybridRequest_StartCursors(HybridRequest *hreq, RedisModuleCtx *replyCtx, QueryError *status, bool backgroundDepletion) {
    HybridRequest *req = hreq;
    if (req->nrequests == 0) {
...
static int HybridQueryTimeoutFailCallback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (!rsc) {
    ...
    RedisModule_Log(ctx, "warning", "HybridQueryTimeoutFailCallback: no request context");
    QueryErrorsGlobalStats_UpdateError(QUERY_ERROR_CODE_TIMED_OUT, 1, COORD_ERR_WARN);
...
static int HybridQueryCursorReplyCallback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (!rsc) {
    // Shouldn't happen, but handle gracefully
    RedisModule_Log(ctx, "warning", "HybridQueryReplyCallback: no request context");
    RedisModule_ReplyWithError(ctx, "Internal error: no request context");
    return REDISMODULE_OK;
  }

  HybridRequest *req = RequestSyncCtx_GetHybridRequest(rsc);
  ChunkReplyState *stored = RequestSyncCtx_GetReplyState(req->syncCtx);

  if (QueryError_HasError(&stored->err)) {
    QueryErrorsGlobalStats_UpdateError(QueryError_GetCode(&stored->err), 1, SHARD_ERR_WARN);
...
static int HybridRequest_BuildPipelineAndExecute(HybridRequest *hreq, HybridPipelineParams *hybridParams, RedisModuleCtx *ctx,
  ...
  if (RunInThread(ctx)) {
    ...
    return buildPipelineAndExecute(hreq, hybridParams, ctx, sctx, status, internal, false);
  }
}

static inline void DefaultCleanup(HybridRequest *hreq) {
  if (!RequestSyncCtx_HasActiveQueryCycle(hreq->syncCtx)) {
...
int hybridCommandHandler(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, bool internal,
  ...
  HybridRequest *hreq = hybridRequest;
  ...
  if (parseHybridCommand(ctx, &ac, sctx, &cmd, &status, internal, profileOptions) != REDISMODULE_OK) {
    return CleanupAndReplyStatus(ctx, hreq, cmd.hybridParams, &status, internal);
  }
  ...
  if (HybridRequest_BuildPipelineAndExecute(hreq, cmd.hybridParams, ctx, hybridRequest->sctx, &status, internal) != REDISMODULE_OK) {
