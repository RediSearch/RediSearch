...
int parseHybridCommand(RedisModuleCtx *ctx, ArgsCursor *ac,
  ...
  *parsedCmdCtx->reqConfig = RSGlobalConfig.requestConfigParams;
  ...
  // when the limit is active (search-workers == 0 && limit > 0). The hybrid
  // request shares a single reqConfig that is later copied into each
  // subquery, so capping here propagates the value to both subqueries through
