...
static int parseShardKRatioClause(ArgsCursor *ac, ParsedVectorData *pvd,
  ...
  //                                          ^
  if (AC_IsAtEnd(ac)) {
    QueryError_SetError(status, QUERY_EPARSEARGS, "Missing argument value for SHARD_K_RATIO");
    return REDISMODULE_ERR;
  }
...
static int parseKNNClause(ArgsCursor *ac, VectorQuery *vq, ParsedVectorData *pvd, QueryError *status) {
  ...
  for (int i=0; i<argumentCount; i+=2) {
    ...
    if (AC_AdvanceIfMatch(ac, "K")) {
      ...
      if (hasShardKRatio) {
        QueryError_SetError(status, QUERY_EDUPPARAM, "Duplicate SHARD_K_RATIO argument");
        return REDISMODULE_ERR;
      }
