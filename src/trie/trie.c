...
TrieNode *__trie_AddChildIdx(TrieNode *n, const rune *str, t_len offset, t_len len,
                             RSPayload *payload, float score, int idx) {
    ...
    if (n->numChildren > 1) {
        memmove(__trieNode_childKey(n, idx + 1), __trieNode_childKey(n, idx),
                (n->numChildren - idx - 1) * sizeof(rune));
        memmove(__trieNode_children(n) + idx + 1, __trieNode_children(n) + idx,
                (n->numChildren - idx - 1) * sizeof(TrieNode *));
    }
...
TrieNode *__trie_SplitNode(TrieNode *n, t_len offset) {
    ...
    memcpy(__trieNode_childKey(newChild, 0), __trieNode_childKey(n, 0),
...
TrieNode *__trieNode_MergeWithSingleChild(TrieNode *n, TrieFreeCallback freecb) {
    ...
    TrieNode *merged = __newTrieNode(nstr, 0, n->len + ch->len, NULL, 0, ch->numChildren, ch->score,
                                     __trieNode_isTerminal(ch), n->sortMode);
        ...
        if (str[offset] == *childKey) {
            ...
            *__trieNode_childKey(n, idx) = str[offset];
            ...
            if (n->sortMode == Trie_Sort_Score && n->numChildren > 1) {
                if ((idx > 0 &&
                     child->maxChildScore > __trieNode_children(n)[idx - 1]->maxChildScore) ||
                    (idx < n->numChildren - 2 &&
                     child->maxChildScore < __trieNode_children(n)[idx + 1]->maxChildScore)) {
...
TrieIterator *TrieNode_Iterate(TrieNode *n, StepFilter f, StackPopCallback pf, void *ctx) {
    ...
    it->popCallback = pf;
    it->minScore =
        INT_MIN; // terms from dictionary which are not in term trie get a valid score INT_MIN
    it->ctx = ctx;
    __ti_Push(it, n, 0);
...
static int rsbCompareExact(const void *h, const void *e) { return rsbCompareCommon(h, e, 0); }

static int rsbComparePrefix(const void *h, const void *e) { return rsbCompareCommon(h, e, 1); }
    ...
    if (n->str[localOffset] == r->origStr[globalOffset]) {
        ...
        if (globalOffset + 1 == r->lenOrigStr) {
            if (r->prefix) { // contains mode
                ...
                // printf("%s %d %d %d\n", str, array_len(r->buf), localOffset + 1, globalOffset +
                // 1);
