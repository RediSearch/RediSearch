...
void TrieNode::Print(int idx, int depth) {
  ...
  printf("%d) \"%s\" Score %f, max ChildScore %f\n", idx, +_runes.toUTF8(), _score, _maxChildScore);
...
#ifdef DEBUG_TRIE
  printf("[%.*s] (%.*s %f), state %d, string offset %d/%d, child offset %d/%d\n",
         runes.len(), +runes.toUTF8(), curr.n->_len, +curr.n->_runes.toUTF8(),
         curr.n->_score, curr.state, curr.stringOffset,
         curr.n->_len,
         curr.childOffset, curr.n->_children.size());
#endif
    ...
    case ITERSTATE_SELF:
      if (curr.stringOffset < curr.n->_len) {
        ...
        if (filter) {
          ...
          FilterCode rc = filter->Filter(b, matched, match);
...
TrieIterator::TrieIterator(TrieNode *node, DFAFilter *filter) : minScore(0), filter(filter) {
  Push(node, false);
}

//---------------------------------------------------------------------------------------------

TrieIterator::TrieIterator(DFAFilter *filter) : minScore(0), filter(filter) {
}
...
bool TrieIterator::Next(Runes &ret_runes, RSPayload &payload, float &score, int *match) {
  StepResult rc;
