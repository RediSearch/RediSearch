...
CONFIG_SETTER(setMinStemLen) {
    ...
    if (minStemLen < MIN_MIN_STEM_LENGHT) {
        QueryError_SetErrorFmt(status, MIN_MIN_STEM_LENGHT,
                               "Minimum stem length cannot be lower than %u", MIN_MIN_STEM_LENGHT);
...
CONFIG_SETTER(setMaxDocTableSize) {
    ...
    if (newsize > MAX_DOC_TABLE_SIZE) {
        QueryError_SetError(status, QUERY_ELIMIT,
                            "Value exceeds maximum possible document table size");
        return REDISMODULE_ERR;
    }
...
static inline int errorTooManyThreads(QueryError *status) {
    QueryError_SetErrorFmt(status, QUERY_ELIMIT, "Number of worker threads cannot exceed %d",
                           MAX_WORKER_THREADS);
    return REDISMODULE_ERR;
}
...
CONFIG_SETTER(trySetDeprecatedWorkersConfig) {
    RedisModule_Log(
        ...
        "MT_MODE and WORKER_THREADS are deprecated, use WORKERS and MIN_OPERATION_WORKERS instead");
...
CONFIG_SETTER(setPrivilegedThreadsNum) {
    RedisModule_Log(
        RSDummyContext, "warning",
        "PRIVILEGED_THREADS_NUM is deprecated. Setting WORKERS_PRIORITY_BIAS_THRESHOLD instead.");
...
CONFIG_GETTER(getFrisoINI) { return config->frisoIni ? sdsnew(config->frisoIni) : NULL; }
...
CONFIG_BOOLEAN_GETTER(getForkGCCleanNumericEmptyNodes,
                      gcConfigParams.forkGc.forkGCCleanNumericEmptyNodes, 0)

// _FORK_GC_CLEAN_NUMERIC_EMPTY_NODES
CONFIG_BOOLEAN_SETTER(set_ForkGCCleanNumericEmptyNodes,
                      gcConfigParams.forkGc.forkGCCleanNumericEmptyNodes)
CONFIG_BOOLEAN_GETTER(get_ForkGCCleanNumericEmptyNodes,
                      gcConfigParams.forkGc.forkGCCleanNumericEmptyNodes, 0)
...
CONFIG_SETTER(setNumericTreeMaxDepthRange) {
    ...
    if (maxDepthRange > NR_MAX_DEPTH_BALANCE) {
        QueryError_SetError(status, QUERY_EPARSEARGS,
                            "Max depth for range cannot be higher "
...
CONFIG_SETTER(setDefaultDialectVersion) {
    ...
    if (dialectVersion > MAX_DIALECT_VERSION) {
        QueryError_SetErrorFmt(status, MAX_DIALECT_VERSION,
...
CONFIG_SETTER(setGcPolicy) {
    ...
    if (!strcasecmp(policy, "DEFAULT") || !strcasecmp(policy, "FORK")) {
        ...
        QueryError_SetError(status, QUERY_EPARSEARGS,
                            "Legacy GC policy is no longer supported (since 2.6.0)");
        return REDISMODULE_ERR;
    } else {
...
CONFIG_GETTER(getGcPolicy) { return sdsnew(GCPolicy_ToString(config->gcConfigParams.gcPolicy)); }
...
CONFIG_SETTER(setUpgradeIndex) {
    ...
    if (acrc != AC_OK) {
        QueryError_SetError(status, QUERY_EPARSEARGS,
                            "Index name was not given to upgrade argument");
        return REDISMODULE_ERR;
    }
...
CONFIG_GETTER(getUpgradeIndex) { return sdsnew("Upgrade config for upgrading"); }
...
RSConfigOptions RSGlobalConfigOptions = {
    .vars = {
        ...
        {
            .name = "WORKERS",
            .helpText = "Number of worker threads to use for query processing and background "
                        "tasks. Default is 0."
#ifdef RS_COORDINATOR
                        " This configuration also affects the number of connections per shard. See "
                        "CONN_PER_SHARD."
#endif
            ,
        ...
        },
        {
            ...
            .helpText = "Number of worker threads to use for background tasks when the server is "
        ...
        {.name = "MULTI_TEXT_SLOP",
         .helpText = "Set RediSearch delta used to increase positional offsets between array slots "
                     "for multi text values."
                     "Can control the level of separation between phrases in different array slots "
                     "(related to the SLOP parameter of ft.search command)",
...
sds RSConfig_GetInfoString(const RSConfig *config) {
    ...
    ss = sdscatprintf(ss, "min word length to stem: %u, ",
                      config->iteratorsConfigParams.minStemLength);
    ss = sdscatprintf(ss, "prefix max expansions: %lld, ",
                      config->iteratorsConfigParams.maxPrefixExpansions);
    ss = sdscatprintf(ss, "query timeout (ms): %lld, ", config->requestConfigParams.queryTimeoutMS);
    ss = sdscatprintf(ss, "timeout policy: %s, ",
                      TimeoutPolicy_ToString(config->requestConfigParams.timeoutPolicy));
...
static void dumpConfigOption(const RSConfig *config, const RSConfigVar *var,
...
void DialectsGlobalStats_AddToInfo(RedisModuleInfoCtx *ctx) {
    ...
    for (int dialect = MIN_DIALECT_VERSION; dialect <= MAX_DIALECT_VERSION; ++dialect) {
        ...
        RedisModule_InfoAddFieldULongLong(ctx, field,
                                          GET_DIALECT(RSGlobalConfig.used_dialects, dialect));
    }
}
