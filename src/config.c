...
static int set_search_disk_write_buffer_size_config(const char *name, long long val, void *privdata,
                                                    RedisModuleString **err) {
  if (val != 0 &&
      (val < DISK_WRITE_BUFFER_SIZE_MIN_KB || val % DISK_WRITE_BUFFER_SIZE_ALIGNMENT_KB != 0)) {
    RS_ASSERT(err);
...
int RegisterModuleConfig_Local(RedisModuleCtx *ctx) {
  ...
  RM_TRY(RedisModule_RegisterNumericConfig(
      ...
      REDISMODULE_CONFIG_HIDDEN | REDISMODULE_CONFIG_IMMUTABLE | REDISMODULE_CONFIG_UNPREFIXED, 1,
      LLONG_MAX, get_size_t_numeric_config, set_size_t_numeric_config, NULL,
      (void *)&(RSGlobalConfig.diskWbmBudgetPerIndexMB)))

  RM_TRY(RedisModule_RegisterNumericConfig(
      ctx, "search-disk-write-buffer-size-kb", DEFAULT_DISK_WRITE_BUFFER_SIZE_KB,
      REDISMODULE_CONFIG_HIDDEN | REDISMODULE_CONFIG_IMMUTABLE | REDISMODULE_CONFIG_UNPREFIXED, 0,
      DISK_WRITE_BUFFER_SIZE_MAX_KB, get_size_t_numeric_config,
