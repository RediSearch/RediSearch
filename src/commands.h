...
#define RS_WRITE_DOC_COMMANDS(OP, ...) \
  OP(RS_ADD_CMD,            RSAddDocumentCommand, "write deny-oom",                         NULL, "write admin", true, __VA_ARGS__)           \
  OP(LEGACY_RS_SAFEADD_CMD, RSAddDocumentCommand, RS_WRITE_FLAGS_DEFAULT("write deny-oom"), NULL, "write admin", IsEnterprise(), __VA_ARGS__) \
  OP(LEGACY_RS_DEL_CMD,     DeleteCommand,        RS_WRITE_FLAGS_DEFAULT("write"),          NULL, "write admin", IsEnterprise(), __VA_ARGS__) \
  OP(RS_DEL_CMD,            DeleteCommand,        RS_WRITE_FLAGS_DEFAULT("write"),          NULL, "write admin", true, __VA_ARGS__)           \
  OP(RS_SAFEADD_CMD,        RSAddDocumentCommand, "write deny-oom",                         NULL, "write admin", true, __VA_ARGS__)
...
#define RS_OSS_CLUSTER_COMMANDS(OP) \
  OP("FT.CONFIG", NULL, "readonly", RegisterCoordConfigSubCommands, "admin", !IsEnterprise() && !isClusterEnabled, 0, 0, 0)
...
#define RS_COMMANDS(OP)                            \
  ...
  RS_WRITE_COMMANDS(OP, true, INDEX_ONLY_CMD_ARGS) \
  RS_SUG_COMMANDS(OP, true, INDEX_SUG_CMD_ARGS)    \
  RS_LOCAL_COMMANDS(OP, true, INDEX_ONLY_CMD_ARGS) \
  RS_READ_ONLY_COMMANDS(OP, true, INDEX_ONLY_CMD_ARGS)

#define RS_OSS_COMMANDS(OP)                             \
  RS_OSS_WRITE_COMMANDS(OP, !IsEnterprise(), COORD_ARGS) \
  RS_OSS_CLUSTER_COMMANDS(OP)
