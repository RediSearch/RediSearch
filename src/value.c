...
static void *_valueAlloc() { return rm_malloc(sizeof(RSValue)); }
...
inline RSValue *RS_NullVal() { return &RS_NULL; }
...
static inline int convert_to_number(const RSValue *v, RSValue *vn, QueryError *qerr) {
    ...
    if (!RSValue_ToNumber(v, &d)) {
        ...
        QueryError_SetErrorFmt(qerr, QUERY_ENOTNUMERIC, "Error converting string '%s' to number",
                               s);
        return 0;
    }
...
int RSValue_Equal(const RSValue *v1, const RSValue *v2, QueryError *qerr) {
    ...
    RSValue vn;
    if (v1->t == RSValue_Number) {
        if (!convert_to_number(v2, &vn, NULL))
            return 0;
        return cmp_numbers(v1, &vn) == 0;
    } else if (v2->t == RSValue_Number) {
        if (!convert_to_number(v1, &vn, NULL))
            return 0;
