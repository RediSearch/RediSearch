...
int HashNotificationCallback(RedisModuleCtx *ctx, int type, const char *event,
  ...
  if (isLoading) {
    // ignore key space notification if loading data from persistency
...
#define CHECK_AND_CACHE_EVENT(E) \
  ...
  else CHECK_CACHED_EVENT(hmset) else CHECK_CACHED_EVENT(del) else CHECK_CACHED_EVENT(hdel) else {
