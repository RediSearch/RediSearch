...
TotalIndexesInfo IndexesInfo_TotalInfo() {
  ...
  while ((entry = dictNext(iter))) {
    ...
    if (rc != 0) {
      RedisModule_Log(RSDummyContext, "warning", "Failed to acquire read lock on index %s: rc=%d (%s). Cannot continue getting Index info", IndexSpec_FormatName(sp, false), rc, strerror(rc));
      continue;
    }
