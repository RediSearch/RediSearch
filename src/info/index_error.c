...
size_t IndexError_ErrorCount(const IndexError *error) { return error->error_count; }

// Returns the last error message in the IndexError.
const char *IndexError_LastError(const IndexError *error) { return error->last_error; }

// Returns the key of the document that caused the error.
const RedisModuleString *IndexError_LastErrorKey(const IndexError *error) { return error->key; }

// Returns the last error time in the IndexError.
struct timespec IndexError_LastErrorTime(const IndexError *error) { return error->last_error_time; }
...
IndexError IndexError_Deserialize(MRReply *reply) {
    ...
    RedisModule_Assert(reply &&
                       (MRReply_Type(reply) == MR_REPLY_MAP ||
    ...
    RedisModule_Assert(MRReply_Type(last_error) == MR_REPLY_STRING ||
                       MRReply_Type(last_error) == MR_REPLY_STATUS);
    size_t error_len;
    ...
    // In hiredis with resp2 '+' is a status reply.
    RedisModule_Assert(MRReply_Type(key) == MR_REPLY_STRING ||
                       MRReply_Type(key) == MR_REPLY_STATUS);
    size_t key_len;
    ...
    RedisModule_Assert(MRReply_Type(last_error_time) == MR_REPLY_ARRAY &&
                       MRReply_Length(last_error_time) == 2);
