...
static void rpidxFree(ResultProcessor *iter) { rm_free(iter); }
...
IndexIterator *QITR_GetRootFilter(QueryIterator *it) {
    /* On coordinator, the root result processor will be a network result processor and we should
     * ignore it */
...
static int rpscoreNext(ResultProcessor *base, SearchResult *res) {
    ...
    do {
        ...
        res->score =
            self->scorer(&self->scorerCtx, res->indexResult, res->dmd, base->parent->minScore);
        ...
        // If we got the special score RS_SCORE_FILTEROUT - disregard the result and decrease the
        // total number of results (it's been increased by the upstream processor)
 ...
 *
 * It takes results from upstream (should be Index iterator or close; before any RP that need these
 *field), and add their additional value to the right score field before sending them downstream.
 *******************************************************************************************************************/
...
typedef struct {
    ...
    // the compare function for the heap. We use it to test if a result needs to be added to the
    // heap
...
static int rpsortNext_innerLoop(ResultProcessor *rp, SearchResult *r) {
    ...
    // get the next result from upstream. `self->pooledResult` is expected to be empty and
    // allocated.
...
static void rppagerFree(ResultProcessor *base) { rm_free(base); }
 ...
 *
 * This component should be added to the query's execution pipeline INSTEAD OF a loader, if a loader
 *is needed.
 *
...
static SearchResult *InsertResult(RPSafeLoader *self, SearchResult *resToBuffer,
...
static bool IsBufferEmpty(RPSafeLoader *self) { return self->buffer_results_count == 0; }
...
static int rpSafeLoaderNext_Accumulate(ResultProcessor *rp,
                                       SearchResult *res); // Forward declaration
...
static ResultProcessor *RPSafeLoader_New(RedisSearchCtx *sctx, RLookup *lk, const RLookupKey **keys,
...
void SetLoadersForMainThread(AREQ *r) {
    ...
    while (rp) {
        if (rp->type == RP_SAFE_LOADER) {
            // If the `Next` function is `rpSafeLoaderNext_Accumulate`, it means that the loader
            // didn't buffer any result yet (or was reset), so we can safely change it to
            // `rploaderNext`. Otherwise, we keep the `Next` function as is (rpSafeLoaderNext_Yield)
            // and set the flag `becomePlainLoader` to true, so the loader will become a plain
            // loader after the buffer is empty.
...
static char *RPTypeLookup[RP_MAX] = {
    "Index",   "Loader",        "Threadsafe-Loader", "Scorer",         "Sorter",
    "Counter", "Pager/Limiter", "Highlighter",       "Grouper",        "Projector",
    "Filter",  "Profile",       "Network",           "Metrics Applier"};
