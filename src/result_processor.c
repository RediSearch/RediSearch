...
static ResultProcessor *RPSafeLoader_New(RedisSearchCtx *sctx, RLookup *lk, const RLookupKey **keys, size_t nkeys, size_t block_size, bool forceLoad) {
...
ResultProcessor *RPLoader_New(AREQ *r, RLookup *lk, const RLookupKey **keys, size_t nkeys, bool forceLoad) {
  if (r->reqflags & QEXEC_F_RUN_IN_BACKGROUND) {
    ...
    return RPSafeLoader_New(r->sctx, lk, keys, nkeys, DEFAULT_BUFFER_BLOCK_SIZE, forceLoad);
  } else {
...
bool PipelineAddPauseRPcount(AREQ *r, size_t results_count, bool before, ResultProcessorType rp_type, QueryError *status) {
  ...
  if (!success) {
    ...
    QueryError_SetErrorFmt(status, QUERY_EGENERIC, "%s RP type not found in stream or tried to insert after last RP", RPTypeToString(rp_type));
  }
  return success;
