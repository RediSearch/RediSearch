...
SchemaRule *SchemaRule_Create(SchemaRuleArgs *args, IndexSpec *spec, QueryError *status) {
  ...
  rule->lang_field = args->lang_field ? rm_strdup(args->lang_field) : rm_strdup("__language");
  rule->score_field = args->score_field ? rm_strdup(args->score_field) : rm_strdup("__score");
  rule->payload_field = args->payload_field ? rm_strdup(args->payload_field) : rm_strdup("__payload");
