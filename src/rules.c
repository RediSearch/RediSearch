...
SchemaRule *SchemaRule_Create(SchemaRuleArgs *args, StrongRef ref, QueryError *status) {
    ...
    if (rule->filter_exp_str) {
        rule->filter_exp =
            ExprAST_Parse(rule->filter_exp_str, strlen(rule->filter_exp_str), status);
        if (!rule->filter_exp) {
...
RSLanguage SchemaRule_JsonLang(RedisModuleCtx *ctx, const SchemaRule *rule, RedisJSON jsonRoot,
                               const char *kname) {
    ...
    if (!langJson || japi->getString(langJson, &langStr, &len) != REDISMODULE_OK) {
        RedisModule_Log(NULL, "warning", "invalid field %s for key %s: not a string",
                        rule->lang_field, kname);
        goto done;
    }
...
double SchemaRule_HashScore(RedisModuleCtx *rctx, const SchemaRule *rule, RedisModuleKey *key,
    ...
    int rv =
        RedisModule_HashGet(key, REDISMODULE_HASH_CFIELDS, rule->score_field, &score_rms, NULL);
    if (rv != REDISMODULE_OK) {
        goto done;
    }
...
double SchemaRule_JsonScore(RedisModuleCtx *ctx, const SchemaRule *rule, RedisJSON jsonRoot,
                            const char *kname) {
    double score = rule->score_default;
    JSONResultsIterator jsonIter = NULL;
    if (!rule->score_field) {
...
RedisModuleString *SchemaRule_HashPayload(RedisModuleCtx *rctx, const SchemaRule *rule,
    ...
    if (rv != REDISMODULE_OK) {
        if (payload_rms != NULL)
            RedisModule_FreeString(rctx, payload_rms);
        return NULL;
    }
    return payload_rms;
}
...
bool SchemaRule_ShouldIndex(struct IndexSpec *sp, RedisModuleString *keyname, DocumentType type) {
    ...
    if (rule->filter_exp) {
        ...
        if (EvalCtx_EvalExpr(r, rule->filter_exp) != EXPR_EVAL_OK || !RSValue_BoolTest(&r->res)) {
            ret = false;
        }
...
void SchemaPrefixes_Create() { SchemaPrefixes_g = NewTrieMap(); }

static void freePrefixNode(void *ctx) { SchemaPrefixNode_Free(ctx); }

void SchemaPrefixes_Free(TrieMap *t) { TrieMap_Free(t, freePrefixNode); }
        ...
        for (int j = 0; j < array_len(node->index_specs); ++j) {
            if (StrongRef_Equals(node->index_specs[j], ref)) {
                ...
                if (array_len(node->index_specs) == 0) {
                    ...
                    TrieMap_Delete(SchemaPrefixes_g, prefixes[i], sdslen(prefixes[i]),
