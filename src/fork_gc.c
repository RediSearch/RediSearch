...
static int __attribute__((warn_unused_result)) FGC_recvBuffer(ForkGC *fgc, void **buf, size_t *len) {
...
int ForkGC::PeriodicCallback(RedisModuleCtx *ctx) {
  if (deleting) {
    return 0;
  }
  ...
  cpid = FGC_fork(this, ctx);  // duplicate the current process
