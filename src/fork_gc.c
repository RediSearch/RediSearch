...
static void FGC_updateStats(ForkGC *gc, RedisSearchCtx *sctx, size_t recordsRemoved,
                            size_t bytesCollected, size_t bytesAdded) {
...
static int __attribute__((warn_unused_result)) FGC_recvFixed(ForkGC *fgc, void *buf, size_t len) {
    while (len) {
        ...
        if (nrecvd > 0) {
            ...
            RedisModule_Log(fgc->ctx, "verbose", "ForkGC - got error while reading from pipe (%s)",
                            strerror(errno));
 ...
 * This function sends the main process an info message with general info on the inverted index
 * garbage collection. In addition, for each fixed block it sends a repair message. For deleted
 * blocks it send delete message. If the index size (number of blocks) wasn't modified (no deleted
 * blocks) we don't send a new block list. In this case, the main process will get the modifications
 * from the fix messages, that contains also a copy of the repaired block. RepairCallback and its
 * argument are passed directly to IndexBlock_Repair; see that function for more details.
 */
    ...
    for (size_t i = 0; i < idx->size; ++i) {
        ...
        if (blk->numEntries == 0) {
            ...
            *delmsg = (MSG_DeletedBlock){.ptr = bufptr, .oldix = i};
            ...
            fixmsg->oldix = i;
            fixmsg->blk = *blk; // TODO: consider sending the blocklist even if there weren't any
                                // deleted blocks instead of this redundant copy.
        ...
        if (i == idx->size - 1) {
            ...
            // this value to the number of entries exist in the main process, to conclude if any new
            // entries were added during the fork process was running. If there were, the main
            // process will discard the last block fixes. We rely on the assumption that a block is
            // small enough and it will be either handled in the next iteration, or it will get to
            // its maximum capacity and will no longer be the last block.
...
static void FGC_childCollectNumeric(ForkGC *gc, RedisSearchCtx *sctx) {
    RedisModuleKey *idxKey = NULL;
    arrayof(FieldSpec *) numericFields =
        getFieldsByType(sctx->spec, INDEXFLD_T_NUMERIC | INDEXFLD_T_GEO);
    ...
    for (int i = 0; i < array_len(numericFields); ++i) {
        ...
        tagNumHeader header = {
            .type = RSFLDTYPE_NUMERIC, .field = numericFields[i]->name, .uniqueId = rt->uniqueId};
        ...
        while ((currNode = NumericRangeTreeIterator_Next(gcIterator))) {
            ...
            bool repaired =
                FGC_childRepairInvidx(gc, sctx, idx, sendNumericTagHeader, &header, &params);
...
static void FGC_childCollectTags(ForkGC *gc, RedisSearchCtx *sctx) {
    ...
    if (array_len(tagFields) != 0) {
        for (int i = 0; i < array_len(tagFields); ++i) {
            ...
            tagNumHeader header = {
                .type = RSFLDTYPE_TAG, .field = tagFields[i]->name, .uniqueId = tagIdx->uniqueId};
...
static void FGC_childScanIndexes(ForkGC *gc, IndexSpec *spec) {
    ...
    RedisModule_Log(sctx.redisCtx, "debug", "ForkGC in index %s - child scanning indexes start",
                    sctx.spec->name);
    ...
    RedisModule_Log(sctx.redisCtx, "debug", "ForkGC in index %s - child scanning indexes end",
                    sctx.spec->name);
}
...
FGC_recvInvIdx(ForkGC *gc, InvIdxBuffers *bufs, MSG_IndexInfo *info) {
    ...
    if (FGC_recvBuffer(gc, (void **)&bufs->newBlocklist, &bufs->newBlocklistSize) !=
        REDISMODULE_OK) {
        return REDISMODULE_ERR;
    }
...
static void checkLastBlock(ForkGC *gc, InvIdxBuffers *idxData, MSG_IndexInfo *info,
    ...
    // Otherwise, we added new entries to the last block while the child was running. In this case
    // we discard all the child garbage collection, assuming they will take place in the next gc
    // iteration.
    ...
    if (info->lastblkEntriesRemoved == info->lastblkNumEntries) {
        ...
        // If all the blocks were deleted, there is no newblocklist. Otherwise, we need to add it to
        // the newBlocklist.
...
static void FGC_applyInvertedIndex(ForkGC *gc, InvIdxBuffers *idxData, MSG_IndexInfo *info,
    ...
    RS_LOG_ASSERT(idx->size >= info->nblocksOrig,
                  "Current index size should be larger or equal to original index size");
    ...
    if (idxData->newBlocklist) { // the child removed some of the blocks, but not all of them
        ...
        if (!info->lastblkDocsRemoved) {
            ...
            idxData->newBlocklist[idxData->newBlocklistSize - 1] =
                idx->blocks[info->nblocksOrig - 1];
        }

        // Number of blocks added in the parent process since the last scan
        size_t newAddedLen =
            idx->size - info->nblocksOrig; // TODO: can we just decrease by numer of deleted.
        ...
        // if idxData->newBlocklist == NULL it's either because all the blocks the child has seen
        // are gone or we didn't change the size of the index (idxData->numDelBlocks == 0). So if we
        // enter here (idxData->numDelBlocks != 0) it's the first case, all blocks the child has
        // seen need to be deleted. Note that we might want to keep the last block, although deleted
        // by the child. In this case numDelBlocks will *not include* the last block.
        idx->size -= idxData->numDelBlocks;

        // There were new blocks added to the index in the main process while the child was running,
        // and/or we decided to ignore changes made to the last block, we copy the blocks data
        // starting from the first valid block we want to keep.
...
static FGCError FGC_parentHandleTerms(ForkGC *gc) {
    ...
    if (idx->numDocs == 0) {
        ...
        if (!Trie_Delete(sctx->spec->terms, term, len)) {
            RedisModule_Log(sctx->redisCtx, "warning",
                            "RedisSearch fork GC: deleting the term '%s' from"
                            " trie in index '%s' failed",
                            term, sctx->spec->name);
        }
...
static int recvCardvals(ForkGC *fgc, arrayof(CardinalityValue) * tgt, size_t *len,
    ...
    // We use array_newlen since we read the cardinality values entries directly to the memory in
    // tgt. Meaning the header of the array will not be updates, including the length. The length of
    // the array will be used to update the range cardinality. If we don't update the len, it will
    // be 0.
...
static FGCError recvNumIdx(ForkGC *gc, NumGcInfo *ninfo) {
    ...
    if (recvCardvals(gc, &ninfo->cardValsArr, &ninfo->numCardVals, &ninfo->uniqueSum) !=
        REDISMODULE_OK) {
        goto error;
    }
    return FGC_COLLECTED;
...
static void applyNumIdx(ForkGC *gc, RedisSearchCtx *sctx, NumGcInfo *ninfo) {
    ...
    if (currNode->range->entries->numDocs == 0) {
        //   NumericRangeTree_DeleteNode(rt, (currNode->range->minVal + currNode->range->maxVal) /
        //   2);
...
static int periodicCb(void *privdata) {
    ...
    // This check must be done first, because some values (like `deletedDocsFromLastRun`) that are
    // used for early termination might never change after index deletion and will cause periodicCb
    // to always return 1, which will cause the GC to never stop rescheduling itself. If the index
    // was deleted, we don't want to reschedule the GC, so we return 0. If the index is still valid,
    // we MUST hold the strong reference to it until after the fork, to make sure the child process
    // has a valid reference to the index. If we were to try and revalidate the index after the
    // fork, it might already be droped and the child will exit before sending any data, and might
    // left the parent waiting for data that will never arrive. Attempting to revalidate the index
    // after the fork is also problematic because the parent and child are not synchronized, and the
    // parent might see the index alive while the child sees it as deleted.
    ...
    if (rc == -1) {
        RedisModule_Log(ctx, "warning", "Couldn't create pipe - got errno %d, aborting fork GC",
                        errno);
        StrongRef_Release(early_check);
        return 1;
    }
    ...
    if (cpid == 0) {
        ...
        gc->cleanNumericEmptyNodes =
            RSGlobalConfig.gcConfigParams.forkGc.forkGCCleanNumericEmptyNodes;
...
static void statsForInfoCb(RedisModuleInfoCtx *ctx, void *gcCtx) {
    ...
    RedisModule_InfoAddFieldDouble(ctx, "average_cycle_time_ms",
                                   (double)gc->stats.totalMSRun / gc->stats.numCycles);
    RedisModule_InfoAddFieldDouble(ctx, "last_run_time_ms", (double)gc->stats.lastRunTimeMs);
    RedisModule_InfoAddFieldDouble(ctx, "gc_numeric_trees_missed",
                                   (double)gc->stats.gcNumericNodesMissed);
