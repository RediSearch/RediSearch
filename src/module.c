...
int AddDocument(RedisSearchCtx *ctx, Document doc, const char **errorString, int nosave) {
    ...
    for (int i = 0; i < doc.numFields; i++) {
        ...
        switch (fs->type) {
            case F_FULLTEXT:
                totalTokens +=
                    tokenize(c, fs->weight, fs->id, idx, forwardIndexTokenFunc, idx->stemmer);
                break;
            case F_NUMERIC: {
                double score;
...
## Parameters:
    ...
    - docId: The document's id that will be returned from searches. Note that the same docId cannot
be
    ...
    - FIELDS: Following the FIELDS specifier, we are looking for pairs of <field> <text> to be
indexed.
    ...
    - LANGUAGE lang: If set, we use a stemmer for the supplied langauge during indexing. Defaults to
English.
...
u_int32_t _getHitScore(void *ctx) { return ctx ? (u_int32_t)((IndexHit *)ctx)->totalFreq : 0; }

/*
## FT.SEARCH <index> <query> [NOCONTENT] [LIMIT offset num] [INFIELDS num>field ...] [LANGUAGE lang]
[VERBATIM]
...
int RedisModule_OnLoad(RedisModuleCtx *ctx) {
    ...
    if (RedisModule_CreateCommand(ctx, "ft.search", SearchCommand, "readonly deny-oom no-cluster",
    ...
    if (RedisModule_CreateCommand(ctx, "ft.optimize", OptimizeIndexCommand, "write no-cluster", 1,
                                  1, 1) == REDISMODULE_ERR)
        return REDISMODULE_ERR;
