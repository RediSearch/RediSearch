    ...
    if ((distanceArgPos = RMUtil_ArgExists("DISTANCE", argv, argc, 0))) {
        ...
        if (RedisModule_StringToLongLong(argv[distanceArgPos + 1], &distance) != REDISMODULE_OK ||
            ...
            RedisModule_ReplyWithError(
                ctx,
                "bad distance given, distance must be a natural number between 1 to " STRINGIFY(
                    MAX_LEV_DISTANCE));
            goto end;
        }
    } // LCOV_EXCL_LINE
...
static int aliasAddCommon(RedisModuleCtx *ctx, RedisModuleString **argv, int argc,
    ...
    ArgsCursor ac = {0};
    ArgsCursor_InitRString(&ac, argv + 1, argc - 1);
    IndexLoadOptions loadOpts = {.nameR = argv[2],
                                 .flags = INDEXSPEC_LOAD_NOALIAS | INDEXSPEC_LOAD_KEY_RSTRING};
    StrongRef ref = IndexSpec_LoadUnsafeEx(ctx, &loadOpts);
    IndexSpec *sp = StrongRef_Get(ref);
    if (!sp) {
        QueryError_SetError(error, QUERY_ENOINDEX,
                            "Unknown index name (or name is an alias itself)");
...
static int AliasDelCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    IndexLoadOptions lOpts = {.nameR = argv[1], .flags = INDEXSPEC_LOAD_KEY_RSTRING};
    ...
    QueryError status = {0};
    if (IndexAlias_Del(RedisModule_StringPtrLen(argv[1], NULL), ref, 0, &status) !=
        REDISMODULE_OK) {
        return QueryError_ReplyAndClear(ctx, &status);
    } else {
...
static int AliasUpdateCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (spOrig) {
        if (IndexAlias_Del(RedisModule_StringPtrLen(argv[1], NULL), Orig_ref, 0, &status) !=
            REDISMODULE_OK) {
            return QueryError_ReplyAndClear(ctx, &status);
        }
    }
...
void GetFormattedRedisVersion(char *buf, size_t len) {
    snprintf(buf, len, "%d.%d.%d - %s", redisVersion.majorVersion, redisVersion.minorVersion,
             redisVersion.patchVersion,
             IsEnterprise() ? (isCrdt ? "enterprise-crdt" : "enterprise") : "oss");
}

void GetFormattedRedisEnterpriseVersion(char *buf, size_t len) {
    snprintf(buf, len, "%d.%d.%d-%d", rlecVersion.majorVersion, rlecVersion.minorVersion,
             rlecVersion.patchVersion, rlecVersion.buildVersion);
}
...
int IsEnterprise() { return rlecVersion.majorVersion != -1; }
...
#define INDEX_DOC_CMD_ARGS  2, 2, 1
    ...
    RM_TRY(RedisModule_CreateCommand, ctx, RS_DROP_IF_X_CMD, DropIfExistsIndexCommand, "write",
           INDEX_ONLY_CMD_ARGS);
    RM_TRY(RedisModule_CreateCommand, ctx, RS_DROP_INDEX_IF_X_CMD, DropIfExistsIndexCommand,
    ...
    RM_TRY(RedisModule_CreateCommand, ctx, RS_SUGADD_CMD, RSSuggestAddCommand, "write deny-oom", 1,
           1, 1);
