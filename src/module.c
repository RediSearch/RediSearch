...
static int rscParseProfile(searchRequestCtx *req, RedisModuleString **argv) {
  ...
  if (RMUtil_ArgIndex("FT.PROFILE", argv, 1) != -1) {
    req->profileArgs += 2;
    req->profileClock = profile_clock_now_ns();
...
searchRequestCtx *rscParseRequest(RedisModuleString **argv, int argc, QueryError* status) {
  ...
  req->initClock = profile_clock_now_ns();
...
static void profileSearchReplyCoordinator(RedisModule_Reply *reply, void *ctx) {
  ...
  RedisModule_Reply_Map(reply);
  RedisModule_ReplyKV_Double(reply, "Total Coordinator time", (double)(profile_clock_now_ns() - pCtx->totalTime) / CLOCKS_PER_MILLISEC);
  RedisModule_ReplyKV_Double(reply, "Post Processing time", (double)(profile_clock_now_ns() - pCtx->postProcessTime) / CLOCKS_PER_MILLISEC);
  RedisModule_Reply_MapEnd(reply);
}
...
static bool should_return_timeout_error(searchRequestCtx *req) {
  return RSGlobalConfig.requestConfigParams.timeoutPolicy == TimeoutPolicy_Fail
         && req->timeout != 0
         && ((double)(profile_clock_now_ns() - req->initClock) / CLOCKS_PER_MILLISEC) > req->timeout;
}
...
static int searchResultReducer(struct MRCtx *mc, int count, MRReply **replies) {
  ...
  if (!profile) {
    ...
    profileSearchReply(reply, &rCtx, count, replies, req->profileClock, profile_clock_now_ns());
  }

  TotalGlobalStats_CountQuery(QEXEC_F_IS_SEARCH, profile_clock_now_ns() - req->initClock);
