...
typedef struct {
  ...
  // One slot per usable payload: that shard's node id, or "" when it has none
  // yet. Slots, not ids, are the key, so an unnamed shard still counts.
  arrayof(const char *) shardIds;
  size_t nRejected;
  MRReply *firstError;
  // Every distinct gate pair the reporting shards sent. More than one means they
  // disagree on the recipe, so some fingerprints are incomparable. Grouping rather
  // than holding the shards against one reference matters: with a reference, which
  // shards got compared depended on which reply arrived first, so a divergence the
  // replies proved could be dropped when the odd shard happened to answer first.
  arrayof(ClusterStateGates) gateGroups;
} ClusterStateReports;

// Folds one shard payload into the picture. An error or malformed reply gets no
// slot, so it surfaces as non-reporting rather than as divergence.
static void ClusterStateReports_AddShard(ClusterStateReports *reports, MRReply *r) {
  ...
  MRReply *recipe = MRReply_ArrayElement(r, 1);
  MRReply *encVer = MRReply_ArrayElement(r, 2);
  MRReply *entries = MRReply_ArrayElement(r, 3);
  if (MRReply_Type(recipe) != MR_REPLY_INTEGER || MRReply_Type(encVer) != MR_REPLY_INTEGER ||
      MRReply_Type(entries) != MR_REPLY_ARRAY) {
    return;
  }
  // Validate every entry before taking a slot. Skipping a malformed entry instead
  // would leave the shard counted as reporting while an index it listed went
  // unrecorded, and that index would read as missing from a shard that has it -
  // which is how a future payload with a wider entry would look to this reducer.
  const size_t nEntries = MRReply_Length(entries);
  for (size_t j = 0; j < nEntries; j++) {
    MRReply *pair = MRReply_ArrayElement(entries, j);
  ...
  for (size_t j = 0; j < nEntries; j++) {
    ...
    // The payload's strlen-based reply already cut names at their first NUL, and
    // tm_len_t narrows again; either loss can merge two indexes into one entry.
    ...
    // MRReply_Integer does not check the type and hiredis callocs replies, so a
    // non-integer would read as 0 and make every such index compare equal.
    MRReply *fp = MRReply_ArrayElement(pair, 1);
...
// The most distinct schemas any one group of gate-agreeing shards holds for this index.
// Counting per group is what makes the answer independent of reply order: fingerprints
// from shards that used different recipes are not comparable, so they are never counted
// against each other, while a divergence inside any group still surfaces. Which shards
// are "wrong" is not decidable - there may be no majority - so this is a count only.
...
static void replyShardIds(RedisModule_Reply *reply, const char *key,
                          arrayof(const char *) ids) {
...
static void replyClusterStateWarning(RedisModule_Reply *reply, const ClusterStateVerdict *v) {
  ...
  if (v->nMissing > 0 || v->nSchemas > 1) {
    msg = sdscat(msg, ": ");
    if (v->nMissing > 0) {
      msg = sdscatprintf(msg, "index is missing from %zu of %zu reporting shards",
                     v->nMissing, v->reportingShards);
    }
    if (v->nMissing > 0 && v->nSchemas > 1) {
      msg = sdscat(msg, ", and ");
    }
    if (v->nSchemas > 1) {
      msg = sdscatprintf(msg, "the shards that have it hold %u different schemas",
                     v->nSchemas);
    }
  ...
  if (v->nSilent > 0 || v->nRejected > 0 || v->versionSkew || v->noFingerprint > 0) {
    msg = sdscat(msg,
                   sdslen(msg) > markerLen ? " The rest of the picture cannot be determined: "
    ...
    if (v->nRejected > 0) {
      msg = sdscatprintf(msg, "%s%zu of %zu shards rejected the request", sep,
                     v->nRejected, v->expectedShards);
      if (v->errorText) {
        // Safe unescaped because the warning is replied as a bulk string; the length
        // cap is there to bound the reply, not to sanitise it.
    ...
    if (v->noFingerprint > 0) {
      msg = sdscatprintf(msg, "%s%zu of the reporting shards could not compute a schema fingerprint", sep,
                     v->noFingerprint);
    }
    msg = sdscat(msg, ".");
  }
...
// One { index, status } entry of the public FT._LIST WITHCLUSTERSTATE reply, and the
// only producer of it - the single-shard path and the reducer both come through here,
// so the two cannot drift. A NULL verdict is the known-consistent case.
...
// The name is taken in the same lossy C-string form the shard payload sends, so a
// single-shard reply renders an index exactly as a multi-shard one does.
...
static int IndexListClusterStateReducer(struct MRCtx *mc, int count, MRReply **replies) {
  ...
  // An empty array would read as "this cluster has no indexes", so reply an error
  // instead: the shards' own when one arrived, a fanout failure otherwise.
  ...
  // The shards the fanout asked, not the current topology: one that joined since would
  // otherwise be reported as not having replied.
  ...
  // Shards the fanout asked that no usable payload came back from. A rejecting shard
  // is in here too: it was asked, and its error reply took no slot, so its id went
  // unmatched. That is what keeps nSilent below from underflowing - the count can never
  // be lower than the rejection tally, since each reply crosses off at most one id.
  const size_t nNotReporting = array_len(unreachableIds);
  // A shard that rejected the command is alive, so it is reported as a rejection
  // rather than counted among those that never answered.
  const size_t nSilent = nNotReporting - reports.nRejected;
  // More than one gate group means the reporting shards disagree on the recipe, so some
  // of their fingerprints could not be compared with each other at all.
  ...
  while (TrieMapIterator_Next(it, &name, &nameLen, &ptr)) {
    ClusterStateIndexInfo *info = ptr;
    size_t nMissing = 0;
