...
static IndexReader *NewIndexReaderGeneric(const IndexSpec *sp, InvertedIndex *idx,
                                          ...
                                          int skipMulti, RSIndexResult *record);
...
size_t indexBlock_Free(IndexBlock *blk) { return Buffer_Free(&blk->buf); }
...
// 3. Frequencies only
ENCODER(encodeFreqsOnly) { return qint_encode2(bw, (uint32_t)delta, (uint32_t)res->freq); }

// 4. Field mask only
ENCODER(encodeFieldsOnly) { return qint_encode2(bw, (uint32_t)delta, (uint32_t)res->fieldMask); }
...
ENCODER(encodeDocIdsOnly) { return WriteVarint(delta, bw); }

// 9. Encode only the doc ids
ENCODER(encodeRawDocIdsOnly) { return Buffer_Write(bw, &delta, 4); }
...
typedef struct {
    // Common fields
    uint8_t
        deltaEncoding : 3; // representing a zero-based number of bytes that stores the docId delta
                           // (delta from the previous docId) (zero delta is required to store
                           // multiple values in the same doc) Max delta size is 7 bytes (values
                           // between 0 to 7), allowing for max delta value of 2^((2^3-1)*8)-1
...
// This header is written/read to/from Inverted Index entries, followed by the actual bytes
// representing the delta (if not zero), followed by the actual bytes representing the numeric value
// (if not tiny) (see encoder `encodeNumeric` and decoder `readNumeric`) EncodingHeader internal
// structs must all be of the same size, beginning with common "base" fields, followed by specific
// fields per "derived" struct. The specific types are:
...
ENCODER(encodeNumeric) {
    ...
    size_t pos = BufferWriter_Offset(
        bw); // save the current position to the buffer. here we will store the header
    size_t sz = Buffer_Write(
        bw, "\0", sizeof(EncodingHeader)); // promote the buffer by the header size (1 byte)
    ...
    if ((double)tinyNum == realVal) {
        ...
        if (absVal == f32Num ||
            (RSGlobalConfig.numericCompress == true && fabs(absVal - f32Num) < 0.01)) {
...
size_t InvertedIndex_WriteEntryGeneric(InvertedIndex *idx, IndexEncoder encoder, t_docId docId,
    ...
    //
    // For numeric encoder the maximal delta is practically not a limit (see structs
    // `EncodingHeader` and `NumEncodingCommon`)
...
int IR_Read(void *ctx, RSIndexResult **e) {
    ...
    do {
        ...
        if (ir->skipMulti) {
            ...
            //
            // Currently the only relevant predicate for multi-value is `any`, therefore only the
            // first match in each doc is needed. More advanced predicates, such as `at least <N>`
            // or `exactly <N>`, will require adding more logic.
...
int IR_SkipTo(void *ctx, t_docId docId, RSIndexResult **hit) {
    ...
    if (ir->decoders.seeker) {
        // // if needed - skip to the next block (skipping empty blocks that may appear here due to
        // GC)
        while (BufferReader_AtEnd(&ir->br)) {
            // We're at the end of the last block...
            if (ir->currentBlock + 1 == ir->idx->size) {
...
int IndexBlock_Repair(IndexBlock *blk, DocTable *dt, IndexFlags flags, IndexRepairParams *params) {
    ...
    while (!BufferReader_AtEnd(&br)) {
        ...
        // if it's the first entry: res->docId == 0, else res->docId != 0, but it's not the first
        // entry.
        ...
        if (!docExists) {
            ...
            if (frags) {
                ...
                // In this case we are already closing holes, so we need to write back the record at
                // the writer's position. We also calculate the delta again
                if (!blk->lastId) { // This is the first entry in this block, initialize the
                                    // blk->lastId
                    blk->lastId = res->docId;
                }
                if (encoder != encodeRawDocIdsOnly) {
                    if (isLastValid) {
                        // if the last was valid, the order of the entries didn't change. We can
                        // just copy the entry, as it already contains the correct delta.
