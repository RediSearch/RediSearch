...
static void CursorList_Lock(CursorList *cl) { pthread_mutex_lock(&cl->lock); }

static void CursorList_Unlock(CursorList *cl) { pthread_mutex_unlock(&cl->lock); }
...
static uint64_t CursorList_GenerateId(CursorList *curlist) {
    uint64_t id = (curlist->is_coord ? rand_even48() : rand_odd48()) +
                  1; // 0 should never be returned as cursor id
    ...
    while ((kh_get(cursors, curlist->lookup, id) != kh_end(curlist->lookup))) {
        id = (curlist->is_coord ? rand_even48() : rand_odd48()) +
             1; // 0 should never be returned as cursor id
    }
    return id;
}
...
int Cursor_Free(Cursor *cur) { return Cursors_Purge(getCursorList(cur->is_coord), cur->id); }
...
void Cursors_RenderStats(CursorList *cl, CursorList *cl_coord, IndexSpec *spec,
    ...
    RedisModule_ReplyKV_LongLong(reply, "global_idle",
                                 ARRAY_GETSIZE_AS(&cl->idle, Cursor **) +
                                     ARRAY_GETSIZE_AS(&cl_coord->idle, Cursor **));
    RedisModule_ReplyKV_LongLong(reply, "global_total",
                                 kh_size(cl->lookup) + kh_size(cl_coord->lookup));
