...
static RLookupKey *overrideKey(RLookup *lk, RLookupKey *old, uint32_t flags) {
    ...
    new->path =
        new->name; // keeping the initial default of path = name. Path resolution will happen later.
    ...
    old->name_len = -1;
    old->flags |=
        RLOOKUP_F_HIDDEN; // Mark the old key as hidden so it won't be attempted to be returned
...
// Gets a key from the schema if the field is sortable (so its data is available), unless an RP
// upstream has promised to load the entire document.
static RLookupKey *genKeyFromSpec(RLookup *lookup, const char *name, size_t name_len,
                                  uint32_t flags) {
    const FieldSpec *fs = findFieldInSpecCache(lookup, name);
    // FIXME: LOAD ALL loads the key properties by their name, and we won't find their value by the
    // field name
...
static RLookupKey *RLookup_GetKey_common(RLookup *lookup, const char *name, size_t name_len,
                                         const char *field_name, RLookupMode mode, uint32_t flags) {
    ...
    // (unless override was requested)
    // 2. create a new key with the name of the field, and mark it as doc-source.
    // 3. if the key is in the schema, mark it as schema-source and apply all the relevant flags
    // according to the field spec.
    // 4. if the key is "loaded" at this point (in schema, sortable and un-normalized), create the
    // key but return NULL
    //    (no need to load it from the document).
    case RLOOKUP_M_LOAD:
        // NOTICE: you should not call GetKey for loading if it's illegal to load the key at the
        // given state. The responsibility of checking this is on the caller.
        if (!key) {
            ...
            // 1. The key has the origin data available (from the sorting vector, UNF) and the
            // caller didn't
            //    request to override or forced loading.
            // 2. The key is already loaded (from the document) and the caller didn't request to
            // override.
            // 3. The key was created by the query (upstream) and the caller didn't request to
            // override.

            // If the caller wanted to mark this key as explicit return, mark it as such even if we
            // don't return it.
        ...
        if (fs) {
            ...
            if (key->flags & RLOOKUP_F_VAL_AVAILABLE && !(flags & RLOOKUP_F_FORCE_LOAD)) {
                // If the key is marked as "value available", it means that it is sortable and
                // un-normalized. so we can use the sorting vector as the source, and we don't need
                // to load it from the document.
                return NULL;
            }
        } else {
            ...
            if (!(key->flags & RLOOKUP_F_NAMEALLOC)) {
              ...
              // it was already set to the same allocation for the name, so we don't need to do
              // anything.
        }
    ...
    //    2. if we are in create mode, overwrite the key (remove schema related data, mark with new
    //    flags)
    ...
    case RLOOKUP_M_READ:
        ...
        // If we didn't find the key in the schema (there is no schema) and unresolved is OK, create
        // an unresolved key.
...
void RLookup_WriteKeyByName(RLookup *lookup, const char *name, size_t len, RLookupRow *dst,
                            RSValue *v) {
    // Get the key first
...
static inline bool isValueAvailable(const RLookupKey *kk, const RLookupRow *dst,
    ...
    return (!options->forceLoad &&
            (
                ...
                // There is no value in the sorting vector, and we don't need to load it from the
                // document.
...
static int getKeyCommonHash(const RLookupKey *kk, RLookupRow *dst, RLookupLoadOptions *options,
    ...
    if (!*keyobj) {
        ...
        *keyobj =
            RedisModule_OpenKey(ctx, keyName, REDISMODULE_READ | REDISMODULE_OPEN_KEY_NOEFFECTS);
    ...
    if (val != NULL) {
        ...
        // This function might retain it, but it's thread-safe to free it afterwards without any
        // locks as it will hold the only reference to it after the next line.
...
static int getKeyCommonJSON(const RLookupKey *kk, RLookupRow *dst, RLookupLoadOptions *options,
    ...
    if (!japi) {
        ...
        RedisModule_Log(RSDummyContext, "warning",
                        "cannot operate on a JSON index as RedisJSON is not loaded");
        return REDISMODULE_ERR;
    }
...
static int loadIndividualKeys(RLookup *it, RLookupRow *dst, RLookupLoadOptions *options) {
    ...
    GetKeyFunc getKey =
        (type == DocumentType_Hash) ? (GetKeyFunc)getKeyCommonHash : (GetKeyFunc)getKeyCommonJSON;
...
static void RLookup_HGETALL_scan_callback(RedisModuleKey *key, RedisModuleString *field,
    ...
    if (!rlk) {
        ...
        rlk = RLookup_GetKey_LoadEx(pd->it, fieldCStr, fieldCStrLen, fieldCStr,
                                    RLOOKUP_F_FORCE_LOAD | RLOOKUP_F_NAMEALLOC);
...
static int RLookup_HGETALL(RLookup *it, RLookupRow *dst, RLookupLoadOptions *options) {
    ...
    if (!isFeatureSupported(RM_SCAN_KEY_API_FIX) || isCrdt) {
        ...
        for (size_t i = 0; i < len; i += 2) {
            ...
            if (!rlk) {
                // First returned document, create the key.
                rlk = RLookup_GetKey_LoadEx(it, kstr, klen, kstr,
                                            RLOOKUP_F_NAMEALLOC | RLOOKUP_F_FORCE_LOAD);
            } else if ((rlk->flags & RLOOKUP_F_QUERYSRC)
                 /* || (rlk->flags & RLOOKUP_F_ISLOADED) TODO: skip loaded keys, EXCLUDING keys that were opened by this function*/) {
        ...
        RedisModuleKey *key =
            RedisModule_OpenKey(ctx, krstr, REDISMODULE_READ | REDISMODULE_OPEN_KEY_NOEFFECTS);
...
int RLookup_LoadRuleFields(RedisModuleCtx *ctx, RLookup *it, RLookupRow *dst, IndexSpec *spec,
                           const char *keyptr) {
