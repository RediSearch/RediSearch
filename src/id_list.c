...
static inline void setEof(IdListIterator *it, int value) { it->base.isValid = !value; }

static inline int isEof(const IdListIterator *it) { return !it->base.isValid; }
...
void IL_Abort(void *ctx) { ((IdListIterator *)ctx)->base.isValid = 0; }
...
/* the last docId read */
t_docId IL_LastDocId(void *ctx) { return ((IdListIterator *)ctx)->lastDocId; }
 ...
 * on the top iterator */
size_t IL_Len(void *ctx) { return (size_t)((IdListIterator *)ctx)->size; }
...
IndexIterator *NewIdListIterator(t_docId *ids, t_offset num, double weight) {
    ...
    if (num > 0)
        memcpy(it->docIds, ids, num * sizeof(t_docId));
    setEof(it, 0);
    it->lastDocId = 0;
