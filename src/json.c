...
int FieldSpec_CheckJsonType(FieldType fieldType, JSONType type, QueryError *status) {
    ...
    case JSONType_String: {
        ...
        if (fieldType &
            (INDEXFLD_T_FULLTEXT | INDEXFLD_T_TAG | INDEXFLD_T_GEO | INDEXFLD_T_GEOMETRY)) {
            rv = REDISMODULE_OK;
        } else {
            QueryError_SetError(status, QUERY_EPARSEARGS,
                                "Invalid JSON type: String type can represent only TEXT, TAG, GEO "
                                "or GEOMETRY field");
        }
        break;
    }
...
#define BIT_CAST(type, value)                                                                      \
    ({                                                                                             \
        ...
        static_assert(sizeof(type) == sizeof(value_var),                                           \
                      "type and value must have the same size");                                   \
...
static inline uint16_t floatToFP16bits(float input) {
    ...
    if (f16 > f16infty)
        f16 = f16infty; // Clamp to signed infinity if overflowed
...
int JSON_StoreVectorAt(RedisJSON arr, size_t len, getJSONElementFunc getElement, char *target,
                       unsigned char step, QueryError *status) {
    for (int i = 0; i < len; ++i) {
...
getJSONElementFunc VecSimGetJSONCallback(VecSimType type) {
    // The right function will put a value of the right type in the address given, or return
    // REDISMODULE_ERR
    switch (type) {
    default:
...
int JSON_StoreSingleVectorInDocField(FieldSpec *fs, RedisJSON arr, struct DocumentField *df,
    ...
    if (JSON_StoreVectorAt(arr, arrLen, getElement, df->strval, VecSimType_sizeof(type), status) !=
        REDISMODULE_OK) {
        rm_free(df->strval);
        return REDISMODULE_ERR;
    }
...
int JSON_StoreMultiVectorInDocField(FieldSpec *fs, JSONIterable *itr, size_t len,
                                    struct DocumentField *df, QueryError *status) {
    VecSimType type;
    size_t dim;
    bool multi;
    getJSONElementFunc getElement;
    ...
    while ((element = JSONIterable_Next(itr))) {
        ...
        if (REDISMODULE_OK != JSON_StoreVectorAt(element, cur_dim, getElement,
                                                 df->blobArr + df->blobSize * count, step,
                                                 status)) {
            goto cleanup;
        }
        count++; // counts only the valid non-null vectors, so we store only valid vectors
                 // continuously.
    }
    df->blobArrLen = count;
...
int JSON_StoreMultiVectorInDocFieldFromIter(FieldSpec *fs, JSONResultsIterator jsonIter, size_t len,
                                            struct DocumentField *df, QueryError *status) {
    JSONIterable iter = (JSONIterable){.type = ITERABLE_ITER, .iter = jsonIter};
    return JSON_StoreMultiVectorInDocField(fs, &iter, len, df, status);
}

int JSON_StoreMultiVectorInDocFieldFromArr(FieldSpec *fs, RedisJSON arr, size_t len,
                                           struct DocumentField *df, QueryError *status) {
    JSONIterable iter = (JSONIterable){.type = ITERABLE_ARRAY, .array.arr = arr, .array.index = 0};
    return JSON_StoreMultiVectorInDocField(fs, &iter, len, df, status);
}
    ...
    while ((json = JSONIterable_Next(iterable))) {
        ...
        if (jsonType == JSONType_String) {
            ...
            QueryError_SetError(status, QUERY_EBADVAL,
                                "TEXT/TAG fields can only contain strings or nulls");
            goto error;
        }
    }
    RS_LOG_ASSERT((i + nulls) == len, "TEXT/TAG iterator count and len must be equal");
    // Remain with surplus unused array entries from skipped null values until `Document_Clear` is
    // called
    df->arrayLen = i;
...
int JSON_StoreTextInDocFieldFromIter(size_t len, JSONResultsIterator jsonIter,
                                     struct DocumentField *df, QueryError *status) {
    JSONIterable iter = (JSONIterable){.type = ITERABLE_ITER, .iter = jsonIter};
...
int JSON_StoreNumericInDocField(size_t len, JSONIterable *iterable, struct DocumentField *df,
    ...
    while ((json = JSONIterable_Next(iterable))) {
        ...
        if (jsonType == JSONType_Double || jsonType == JSONType_Int) {
            ...
            ++nulls; // Skip Nulls (TODO: consider also failing or converting to a specific value,
                     // e.g., zero)
        } else {
            // Numeric fields can handle only numeric or Nulls
            QueryError_SetError(status, QUERY_EBADVAL,
                                "NUMERIC fields can only contain numeric or nulls");
            goto error;
        }
    }
...
int JSON_StoreNumericInDocFieldFromIter(size_t len, JSONResultsIterator jsonIter,
                                        struct DocumentField *df, QueryError *status) {
    JSONIterable iter = (JSONIterable){.type = ITERABLE_ITER, .iter = jsonIter};
...
int JSON_LoadDocumentField(JSONResultsIterator jsonIter, size_t len, FieldSpec *fs,
    ...
    // we check whether a multi value is needed to be calculated for SORTABLE (avoiding re-opening
    // the key and re-parsing the path) (requires some API V2 functions to be available)
    if (rv == REDISMODULE_OK && FieldSpec_IsSortable(fs) && df->unionType == FLD_VAR_T_ARRAY &&
        ...
        if (jsonIterToValue(ctx, jsonIter, APIVERSION_RETURN_MULTI_CMP_FIRST, &rsv) ==
            REDISMODULE_OK) {
            df->multisv = rsv;
        } else {
            rv = REDISMODULE_ERR;
