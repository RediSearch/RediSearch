...
static int MR_Read(void *ctx, RSIndexResult **hit) {
    ...
    *hit = mr->base.current;
    ...
    // Advance the current index in the doc ids array, so it will point to the next id to be
    // returned. If we reached the total number of results, iterator is depleted.
    mr->curIndex++;
