...
void RS_moduleInfoFunc(RedisModuleInfoCtx *ctx, int for_crash_report) {
    ...
    RedisModule_InfoAddFieldDouble(ctx, "used_memory_indexes_human",
                                   total_info.total_mem / (float)0x100000);
    RedisModule_InfoAddFieldDouble(ctx, "total_indexing_time",
                                   total_info.indexing_time / (float)CLOCKS_PER_MILLISEC);
...
#define DO_LOG(...)                                                                                \
    ...
    DO_LOG("verbose",
           "threadpool has %lu high-priority bias that always prefer running queries "
           "when possible",
           RSGlobalConfig.highPriorityBiasNum);
#else
