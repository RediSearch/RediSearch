...
#define MIN_NOT_0(a, b) (((a) && (b)) ? MIN((a), (b)) : MAX((a), (b)))
    ...
    size_t max_process_mem =
        RedisModule_ServerInfoGetFieldUnsigned(info, "max_process_mem", NULL); // Enterprise limit
    maxmemory = MIN_NOT_0(maxmemory, max_process_mem);

    size_t total_system_memory =
        RedisModule_ServerInfoGetFieldUnsigned(info, "total_system_memory", NULL);
...
t_fieldMask IndexSpec_GetFieldBit(IndexSpec *spec, const char *name, size_t len) {
    ...
    if (!fs || !FIELD_IS(fs, INDEXFLD_T_FULLTEXT) || !FieldSpec_IsIndexable(fs))
        return 0;
        ...
        if (fm & ((t_fieldMask)1 << ii)) {
            ...
            if (FIELD_IS(fs, INDEXFLD_T_FULLTEXT) && (FieldSpec_IsUndefinedOrder(fs))) {
                QueryError_SetErrorFmt(
                    status, QUERY_EBADORDEROPTION,
                    "slop/inorder are not supported for field `%s` since it has undefined ordering",
                    fs->name);
                return 0;
            }
        }
...
static void IndexSpec_TimedOutProc(RedisModuleCtx *ctx, WeakRef w_ref) {
    ...
    RedisModule_Log(RSDummyContext, REDISMODULE_LOGLEVEL_VERBOSE, "Freeing index %s by timer",
                    sp->name);
    ...
    if (RS_IsMock) {
        ...
        RedisModuleCallReply *rep =
            RedisModule_Call(RSDummyContext, RS_DROP_INDEX_CMD, "cc!", sp->name, "DD");
        if (rep) {
            RedisModule_FreeCallReply(rep);
        }
    }

    RedisModule_Log(RSDummyContext, REDISMODULE_LOGLEVEL_VERBOSE, "Freeing index %s by timer: done",
                    sp->name);
    StrongRef_Release(spec_ref);
}

// Assuming the GIL is held.
// This can be done without locking the spec for write, since the timer is not modified or read by
// any other thread.
static void IndexSpec_SetTimeoutTimer(IndexSpec *sp, WeakRef spec_ref) {
    if (sp->isTimerSet) {
        WeakRef old_timer_ref;
        if (RedisModule_StopTimer(RSDummyContext, sp->timerId, (void **)&old_timer_ref) ==
            REDISMODULE_OK) {
            WeakRef_Release(old_timer_ref);
        }
    }
    sp->timerId = RedisModule_CreateTimer(
        RSDummyContext, sp->timeout, (RedisModuleTimerProc)IndexSpec_TimedOutProc, spec_ref.rm);
    sp->isTimerSet = true;
}
    ...
    while ((entry = dictNext(iter))) {
        ...
        if (sp->flags & Index_Temporary) {
            ...
            case TimerOp_Add:
                ...
                break;
            case TimerOp_Del:
                IndexSpec_ResetTimeoutTimer(sp);
                break;
            }
        }
    }
...
#define BLOCK_MEMORY_LIMIT                                                                         \
    ((RSGlobalConfig.vssMaxResize) ? RSGlobalConfig.vssMaxResize : memoryLimit / 10)
...
static int parseVectorField_validate_hnsw(VecSimParams *params, QueryError *status) {
    ...
    if (params->algoParams.hnswParams.blockSize ==
        0) { // indicates that block size was not set by the user
        params->algoParams.hnswParams.blockSize = MIN(DEFAULT_BLOCK_SIZE, maxBlockSize);
    }
    if (params->algoParams.hnswParams.initialCapacity ==
        SIZE_MAX) { // indicates that initial capacity was not set by the user
    ...
    if (params->algoParams.hnswParams.initialCapacity > maxBlockSize) {
        QueryError_SetErrorFmt(status, QUERY_ELIMIT,
                               "Vector index initial capacity %zu exceeded server limit (%zu with "
                               "the given parameters)",
                               params->algoParams.hnswParams.initialCapacity, maxBlockSize);
        return 0;
    }
    if (params->algoParams.hnswParams.blockSize > maxBlockSize) {
        // TODO: uncomment when BLOCK_SIZE is added to FT.CREATE on HNSW
        // QueryError_SetErrorFmt(status, QUERY_ELIMIT, "Vector index block size %zu exceeded server
        // limit (%zu with the given parameters)", fs->vectorOpts.vecSimParams.bfParams.blockSize,
        // maxBlockSize); return 0;
    }
    RedisModule_Log(RSDummyContext, "warning",
                    "creating vector index. Server memory limit: %zuB, required memory: %zuB, "
                    "available memory: %zuB",
                    memoryLimit, index_size_estimation, free_memory);
    return 1;
}
...
static int parseVectorField_validate_flat(VecSimParams *params, QueryError *status) {
    ...
    // if Block size was not set by user, sets the default to min(maxBlockSize, DEFAULT_BLOCK_SIZE)
    if (params->algoParams.bfParams.blockSize ==
        0) { // indicates that block size was not set by the user
        params->algoParams.bfParams.blockSize = MIN(DEFAULT_BLOCK_SIZE, maxBlockSize);
    }
    if (params->algoParams.bfParams.initialCapacity ==
        SIZE_MAX) { // indicates that initial capacity was not set by the user
    ...
    if (params->algoParams.bfParams.initialCapacity > maxBlockSize) {
        QueryError_SetErrorFmt(status, QUERY_ELIMIT,
                               "Vector index initial capacity %zu exceeded server limit (%zu with "
                               "the given parameters)",
                               params->algoParams.bfParams.initialCapacity, maxBlockSize);
        return 0;
    }
    if (params->algoParams.bfParams.blockSize > maxBlockSize) {
        QueryError_SetErrorFmt(
            status, QUERY_ELIMIT,
            "Vector index block size %zu exceeded server limit (%zu with the given parameters)",
            params->algoParams.bfParams.blockSize, maxBlockSize);
        return 0;
    }
    RedisModule_Log(RSDummyContext, "warning",
                    "creating vector index. Server memory limit: %zuB, required memory: %zuB, "
...
static int parseVectorField_hnsw(FieldSpec *fs, VecSimParams *params, ArgsCursor *ac,
                                 QueryError *status) {
    int rc;
    ...
    if ((rc = AC_GetSize(ac, &expNumParam, 0)) != AC_OK) {
        ...
        QERR_MKBADARGS_FMT(
            status,
            "Bad number of arguments for vector similarity index: got %d but expected even number "
            "(as algorithm parameters should be submitted as named arguments)",
            expNumParam);
        return 0;
    } else {
        expNumParam /= 2;
    ...
    while (expNumParam > numParam && !AC_IsAtEnd(ac)) {
        if (AC_AdvanceIfMatch(ac, VECSIM_TYPE)) {
            ...
            if ((rc = parseVectorField_GetMetric(ac, &params->algoParams.hnswParams.metric)) !=
                AC_OK) {
    ...
    if (expNumParam > numParam) {
        QERR_MKBADARGS_FMT(status, "Expected %d parameters but got %d", expNumParam * 2,
                           numParam * 2);
        return 0;
    }
    if (!mandtype) {
    ...
    fs->vectorOpts.expBlobSize =
        params->algoParams.hnswParams.dim * VecSimType_sizeof(params->algoParams.hnswParams.type);

    return parseVectorField_validate_hnsw(params, status);
}

static int parseVectorField_flat(FieldSpec *fs, VecSimParams *params, ArgsCursor *ac,
                                 QueryError *status) {
    int rc;
    ...
    while (expNumParam > numParam && !AC_IsAtEnd(ac)) {
        if (AC_AdvanceIfMatch(ac, VECSIM_TYPE)) {
            ...
            if ((rc = parseVectorField_GetMetric(ac, &params->algoParams.bfParams.metric)) !=
                AC_OK) {
            ...
            QERR_MKBADARGS_FMT(status, "Bad arguments for algorithm %s: %s", VECSIM_ALGORITHM_BF,
                               AC_GetStringNC(ac, NULL));
            return 0;
        }
        numParam++;
    ...
    // Calculating expected blob size of a vector in bytes.
    fs->vectorOpts.expBlobSize =
        params->algoParams.bfParams.dim * VecSimType_sizeof(params->algoParams.bfParams.type);
        ...
        if (AC_AdvanceIfMatch(ac, SPEC_NOSTEM_STR)) {
            ...
            if (AC_IsAtEnd(ac)) {
                QueryError_SetError(status, QUERY_EPARSEARGS,
                                    SPEC_PHONETIC_STR " requires an argument");
                return 0;
            }
...
static int parseTagField(FieldSpec *fs, ArgsCursor *ac, QueryError *status) {
    ...
    while (!AC_IsAtEnd(ac)) {
        if (AC_AdvanceIfMatch(ac, SPEC_TAG_SEPARATOR_STR)) {
            if (AC_IsAtEnd(ac)) {
                QueryError_SetError(status, QUERY_EPARSEARGS,
                                    SPEC_TAG_SEPARATOR_STR " requires an argument");
                rc = 0;
                break;
            }
...
static int parseVectorField(IndexSpec *sp, StrongRef sp_ref, FieldSpec *fs, ArgsCursor *ac,
                            QueryError *status) {
    ...
    if (STR_EQCASE(algStr, len, VECSIM_ALGORITHM_BF)) {
        ...
        fs->vectorOpts.vecSimParams.algoParams.tieredParams.specificParams.tieredHnswParams
            .swapJobThreshold = 0; // Will be set to default value.
        ...
        // Point to the same logCtx as the external wrapping VecSimParams object, which is the
        // owner.
...
static int parseFieldSpec(ArgsCursor *ac, IndexSpec *sp, StrongRef sp_ref, FieldSpec *fs,
                          QueryError *status) {
    if (AC_IsAtEnd(ac)) {
        QueryError_SetErrorFmt(status, QUERY_EPARSEARGS, "Field `%s` does not have a type",
                               fs->name);
        return 0;
    }
    ...
    if (AC_AdvanceIfMatch(ac, SPEC_TEXT_STR)) { // text field
        ...
        if (!parseGeometryField(sp, fs, ac, status))
            goto error;
        ...
        QueryError_SetErrorFmt(status, QUERY_EPARSEARGS, "Invalid field type for field `%s`",
                               fs->name);
        goto error;
    }
    ...
    while (!AC_IsAtEnd(ac)) {
        if (AC_AdvanceIfMatch(ac, SPEC_SORTABLE_STR)) {
            ...
            if (AC_AdvanceIfMatch(ac, SPEC_UNF_STR) || // Explicitly requested UNF
                FIELD_IS(fs,
                         INDEXFLD_T_NUMERIC) || // We don't normalize numeric fields. Implicit UNF
                TAG_FIELD_IS(fs, TagField_CaseSensitive)) { // We don't normalize case sensitive
                                                            // tags. Implicit UNF
    ...
    if (!FieldSpec_IsIndexable(fs) && FieldSpec_IndexesMissing(fs)) {
        QueryError_SetErrorFmt(
            ...
            "'Field `%s` cannot be defined with both `NOINDEX` and `INDEXMISSING`'", fs->name);
        goto error;
    }
    return 1;
...
static int IndexSpec_AddFieldsInternal(IndexSpec *sp, StrongRef spec_ref, ArgsCursor *ac,
    ...
    while (!AC_IsAtEnd(ac)) {
        ...
        if (IndexSpec_GetField(sp, fieldName, namelen)) {
            QueryError_SetErrorFmt(status, QUERY_EINVAL, "Duplicate field in schema - %s",
                                   fieldName);
            goto reset;
        }
                ...
                if ((sp->flags & Index_HasFieldAlias) && (sp->flags & Index_StoreTermOffsets)) {
                    ...
                    if (jsonPath) {
                      ...
                      RedisModule_Log(RSDummyContext, "notice",
                                      "missing RedisJSON API to parse JSONPath '%s' in attribute
                    '%s' in index '%s', assuming undefined ordering", fs->path, fs->name, sp->name);
                    } */
                }
            }
        }
        ...
        if (FieldSpec_IsSortable(fs)) {
            ...
            if (fs->sortIdx == -1) {
                QueryError_SetErrorFmt(status, QUERY_ELIMIT,
                                       "Schema is limited to %d Sortable fields", SPEC_MAX_FIELDS);
                goto reset;
            }
        } else {
            fs->sortIdx = -1;
...
// The value of the refcount can get to 0 only if the index spec itself does not point to it
// anymore, and at this point the refcount only gets decremented so there is no wory of some thread
// increasing the refcount while we are freeing the cache.
...
void CleanPool_ThreadPoolStart() {
    if (!cleanPool) {
        cleanPool = redisearch_thpool_create(1, DEFAULT_HIGH_PRIORITY_BIAS_THRESHOLD, LogCallback,
                                             "cleanPool");
    }
}
...
void addPendingIndexDrop() { __atomic_add_fetch(&pendingIndexDropCount_g, 1, __ATOMIC_RELAXED); }

void removePendingIndexDrop() { __atomic_sub_fetch(&pendingIndexDropCount_g, 1, __ATOMIC_RELAXED); }

size_t CleanInProgressOrPending() { return getPendingIndexDrop(); }
...
void IndexSpec_Free(IndexSpec *spec) {
    ...
    // For temporary index
    // This function might be called from any thread, and we cannot deal with timers without the
    // GIL. At this point we should have already stopped the timer.
    ...
    if (RSGlobalConfig.freeResourcesThread == false) {
        ...
        redisearch_thpool_add_work(cleanPool, (redisearch_thpool_proc)IndexSpec_FreeUnlinkedData,
...
void IndexSpec_RemoveFromGlobals(StrongRef spec_ref) {
    ...
    // For temporary index
    // We are dropping the index from the mainthread, but the freeing process might happen later
    // from another thread. We cannot deal with timers from other threads, so we need to stop the
    // timer now. We don't need it anymore anyway.
    if (spec->isTimerSet) {
        WeakRef old_timer_ref;
        if (RedisModule_StopTimer(RSDummyContext, spec->timerId, (void **)&old_timer_ref) ==
            REDISMODULE_OK) {
            WeakRef_Release(old_timer_ref);
        }
...
StrongRef IndexSpec_LoadUnsafeEx(RedisModuleCtx *ctx, IndexLoadOptions *options) {
    ...
    if (!RS_IsMock && (sp->flags & Index_Temporary) &&
        !(options->flags & INDEXSPEC_LOAD_NOTIMERUPDATE)) {
        IndexSpec_SetTimeoutTimer(sp, StrongRef_Demote(spec_ref));
    }
...
StrongRef IndexSpec_GetStrongRefUnsafe(const IndexSpec *spec) { return spec->own_ref; }
...
FieldSpec *IndexSpec_CreateField(IndexSpec *sp, const char *name, const char *path) {
    ...
    if (!(sp->flags & Index_FromLLAPI)) {
        ...
        case DocumentType_Hash:
            fs->tagOpts.tagSep = TAG_FIELD_DEFAULT_HASH_SEP;
            break;
...
static int FieldSpec_RdbLoad(RedisModuleIO *rdb, FieldSpec *f, StrongRef sp_ref, int encver) {
    ...
    if (encver >= INDEX_VECSIM_VERSION && FIELD_IS(f, INDEXFLD_T_VECTOR)) {
        ...
        if (encver >= INDEX_VECSIM_TIERED_VERSION) {
            if (VecSim_RdbLoad_v3(rdb, &f->vectorOpts.vecSimParams, sp_ref, f->name) !=
                REDISMODULE_OK) {
                goto fail;
            }
        } else {
            ...
            // If we're loading an old (< 2.8) rdb, we need to convert an HNSW index to a tiered
            // index
            VecSimLogCtx *logCtx = rm_new(VecSimLogCtx);
            ...
            if (f->vectorOpts.vecSimParams.algo == VecSimAlgo_HNSWLIB) {
                ...
                VecSim_TieredParams_Init(&f->vectorOpts.vecSimParams.algoParams.tieredParams,
                                         sp_ref);
                f->vectorOpts.vecSimParams.algoParams.tieredParams.specificParams.tieredHnswParams
                    .swapJobThreshold = 0;
                memcpy(f->vectorOpts.vecSimParams.algoParams.tieredParams.primaryIndexParams,
                       &hnswParams, sizeof(VecSimParams));
            }
        }
        ...
        if (encver < INDEX_VECSIM_2_VERSION) {
            ...
            case VecSimAlgo_HNSWLIB:
                f->vectorOpts.expBlobSize =
                    f->vectorOpts.vecSimParams.algoParams.hnswParams.dim *
                    VecSimType_sizeof(f->vectorOpts.vecSimParams.algoParams.hnswParams.type);
                break;
            case VecSimAlgo_BF:
                f->vectorOpts.expBlobSize =
                    f->vectorOpts.vecSimParams.algoParams.bfParams.dim *
                    VecSimType_sizeof(f->vectorOpts.vecSimParams.algoParams.bfParams.type);
                break;
            case VecSimAlgo_TIERED:
                f->vectorOpts.expBlobSize =
                    f->vectorOpts.vecSimParams.algoParams.tieredParams.primaryIndexParams
                        ->algoParams.hnswParams.dim *
                    VecSimType_sizeof(f->vectorOpts.vecSimParams.algoParams.tieredParams
                                          .primaryIndexParams->algoParams.hnswParams.type);
                break;
            }
        }
    }
    ...
    if (FIELD_IS(f, INDEXFLD_T_GEOMETRY) || (f->options & FieldSpec_Dynamic)) {
        if (encver >= INDEX_GEOMETRY_VERSION) {
            ...
            // In RedisSearch RC (2.8.1 - 2.8.3) we supported default coordinate system which was
            // not written to RDB
...
static IndexesScanner *IndexesScanner_New(StrongRef global_ref) {
    ...
    if (spec->scanner) {
        ...
        RedisModule_Log(RSDummyContext, "notice",
                        "Scanning index %s in background: cancelled and restarted", spec->name);
    }
...
void IndexesScanner_Free(IndexesScanner *scanner) {
    ...
    if (scanner->spec_name)
        rm_free(scanner->spec_name);
    rm_free(scanner);
}

void IndexesScanner_Cancel(IndexesScanner *scanner) { scanner->cancelled = true; }
...
static void Indexes_ScanAndReindexTask(IndexesScanner *scanner) {
    ...
    while (RedisModule_Scan(ctx, cursor, (RedisModuleScanCB)Indexes_ScanProc, scanner)) {
        ...
        if (counter % RSGlobalConfig.numBGIndexingIterationsBeforeSleep == 0) {
            // Sleep for one microsecond to allow redis server to acquire the GIL while we release
            // it. We do that periodically every X iterations (100 as default), otherwise we call
            // 'sched_yield()'. That is since 'sched_yield()' doesn't give up the processor for
            // enough time to ensure that other threads that are waiting for the GIL will actually
        ...
        if (scanner->cancelled) {
            RedisModule_Log(ctx, "notice",
                            "Scanning indexes in background: cancelled (scanned=%ld)",
                            scanner->scannedKeys);
            goto end;
        }
    }
...
static void IndexSpec_ScanAndReindexAsync(StrongRef spec_ref) {
    if (!reindexPool) {
        reindexPool = redisearch_thpool_create(1, DEFAULT_HIGH_PRIORITY_BIAS_THRESHOLD, LogCallback,
                                               "reindex");
    }
#ifdef _DEBUG
    RedisModule_Log(NULL, "notice", "Register index %s for async scan",
                    ((IndexSpec *)StrongRef_Get(spec_ref))->name);
#endif
    IndexesScanner *scanner = IndexesScanner_New(spec_ref);
    redisearch_thpool_add_work(reindexPool, (redisearch_thpool_proc)Indexes_ScanAndReindexTask,
                               scanner, THPOOL_PRIORITY_HIGH);
}
...
void IndexSpec_AddToInfo(RedisModuleInfoCtx *ctx, IndexSpec *sp) {
    ...
    // Index flags
    if (sp->flags & ~(Index_StoreFreqs | Index_StoreFieldFlags | Index_StoreTermOffsets |
                      Index_StoreByteOffsets) ||
        sp->flags & Index_WideSchema) {
    ...
    if (rule->lang_default)
        RedisModule_InfoAddFieldCString(ctx, "default_language",
                                        (char *)RSLanguage_ToString(rule->lang_default));
    if (rule->lang_field)
    ...
    if (num_prefixes && rule->prefixes[0][0] != '\0') {
        ...
        for (int i = 0; i < num_prefixes; ++i) {
            ...
            prefixes =
                array_ensure_append_n(prefixes, rule->prefixes[i], strlen(rule->prefixes[i]));
    ...
    for (int i = 0; i < sp->numFields; i++) {
        ...
        if (fs->options & FieldSpec_Dynamic)
            ...
            RedisModule_InfoAddFieldCString(
                ctx, "type", (char *)FieldSpec_GetTypeNames(INDEXTYPE_TO_POS(fs->types)));
    ...
    RedisModule_InfoAddFieldDouble(ctx, "vector_index_size",
                                   IndexSpec_VectorIndexSize(sp) / (float)0x100000);
    RedisModule_InfoAddFieldDouble(ctx, "offset_vectors_size",
                                   sp->stats.offsetVecsSize / (float)0x100000);
    RedisModule_InfoAddFieldDouble(ctx, "doc_table_size", sp->docs.memsize / (float)0x100000);
    RedisModule_InfoAddFieldDouble(ctx, "sortable_values_size",
                                   sp->docs.sortablesSize / (float)0x100000);
    RedisModule_InfoAddFieldDouble(ctx, "key_table_size",
                                   TrieMap_MemUsage(sp->docs.dim.tm) / (float)0x100000);
    RedisModule_InfoAddFieldDouble("tag_overhead_size_mb",
                                   IndexSpec_collect_tags_overhead(sp) / (float)0x100000);
    RedisModule_InfoAddFieldDouble("text_overhead_size_mb",
                                   IndexSpec_collect_text_overhead(sp) / (float)0x100000);
    RedisModule_InfoAddFieldDouble("total_index_memory_sz_mb",
                                   IndexSpec_TotalMemUsage(sp) / (float)0x100000);
    RedisModule_InfoEndDictField(ctx);
    ...
    RedisModule_InfoAddFieldDouble(ctx, "records_per_doc_avg",
                                   (float)sp->stats.numRecords / (float)sp->stats.numDocuments);
    RedisModule_InfoAddFieldDouble(ctx, "bytes_per_record_avg",
                                   (float)sp->stats.invertedSize / (float)sp->stats.numRecords);
    RedisModule_InfoAddFieldDouble(ctx, "offsets_per_term_avg",
                                   (float)sp->stats.offsetVecRecords / (float)sp->stats.numRecords);
    RedisModule_InfoAddFieldDouble(ctx, "offset_bits_per_record_avg",
                                   8.0F * (float)sp->stats.offsetVecsSize /
                                       (float)sp->stats.offsetVecRecords);
    RedisModule_InfoEndDictField(ctx);
    ...
    RedisModule_InfoAddFieldLongLong(ctx, "indexing",
                                     !!global_spec_scanner || sp->scan_in_progress);
    ...
    for (size_t i = 0; i < ctx.spec->numFields; i++) {
        ...
        if (FIELD_IS(fs, INDEXFLD_T_NUMERIC)) {
            Redis_DeleteKey(ctx.redisCtx,
                            IndexSpec_GetFormattedKey(ctx.spec, fs, INDEXFLD_T_NUMERIC));
        }
        if (FIELD_IS(fs, INDEXFLD_T_TAG)) {
...
int IndexSpec_CreateFromRdb(RedisModuleCtx *ctx, RedisModuleIO *rdb, int encver,
    ...
    if (oldSpec) {
        ...
        RedisModule_Log(RSDummyContext, "notice",
                        "Loading an already existing index, will just ignore.");
...
void *IndexSpec_LegacyRdbLoad(RedisModuleIO *rdb, int encver) {
    ...
    if (!sp->rule) {
        RedisModule_LogIOError(rdb, "warning",
                               "Failed creating rule for legacy index '%s', error='%s'", sp->name,
...
static void LoadingProgressCallback(RedisModuleCtx *ctx, RedisModuleEvent eid, uint64_t subevent,
    ...
    RedisModule_Log(RSDummyContext, "debug",
                    "Waiting for background jobs to be executed while"
    ...
    if (spec->flags & Index_HasVecSim) {
        for (int i = 0; i < spec->numFields; ++i) {
            if (spec->fields[i].types == INDEXFLD_T_VECTOR) {
                RedisModuleString *rmskey = RedisModule_CreateString(ctx, spec->fields[i].name,
                                                                     strlen(spec->fields[i].name));
...
#if defined(_DEBUG) && 0
    RLookupKey *k = RLookup_GetKey_LoadEx(&r->lk, UNDERSCORE_KEY, strlen(UNDERSCORE_KEY),
                                          UNDERSCORE_KEY, RLOOKUP_F_NOFLAGS);
