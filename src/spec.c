...
//---------------------------------------------------------------------------------------------
// Background scan / reindex (spec side).
//
// Drives the generic scan engine (index_scan.{c,h}) for index specs: supplies the
// per-key indexing callbacks, owns the scanner<->spec linkage and the global
// (scan-all) scanner. The engine itself stays free of any spec dependency; this is
