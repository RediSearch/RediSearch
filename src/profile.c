...
void printReadIt(RedisModule_Reply *reply, IndexIterator *root, size_t counter, double cpuTime,
    ...
    if (ir->idx->flags == Index_DocIdsOnly) {
        ...
        if (!flt || flt->geoFilter == NULL) {
            ...
            RedisModule_Reply_SimpleStringf(reply, "%g - %g", ir->decoderCtx.rangeMin,
                                            ir->decoderCtx.rangeMax);
        } else {
...
static double _recursiveProfilePrint(RedisModule_Reply *reply, ResultProcessor *rp,
...
void Profile_Print(RedisModule_Reply *reply, void *ctx) {
    ...
    if (root) {
        ...
        printIteratorProfile(reply, root, 0, 0, 2, req->reqflags & QEXEC_F_PROFILE_LIMITED,
                             &config);
    }
