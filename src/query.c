...
static QueryIterator *Query_EvalTagNode(QueryEvalCtx *q, QueryNode *qn) {
  ...
  // parse time: under WORKERS>0, evaluation can run on a worker thread well after parsing,
  // and a concurrent FT.ALTER may have since reallocated IndexSpec.fields, so a raw
  // FieldSpec* captured back then couldn't be trusted (MOD-18356).
