...
static void QueryTagNode_Free(QueryTagNode *tag) { rm_free((char *)tag->fieldName); }
...
static void QueryMissingNode_Free(QueryMissingNode *missn) { rm_free((char *)missn->fieldName); }
...
void RangeNumber_Free(RangeNumber *r) { rm_free(r); }
...
static enum QueryType parseGeometryPredicate(const char *predicate, size_t len) {
    ...
    // letter is insufficient. CONTAINS and INTERSECTS both end with S, DISJOINT and NEAREST both
    // end with T.
...
static void setFilterNode(QueryAST *q, QueryNode *n) {
    ...
    if (q->root->type == QN_PHRASE) {
        ...
        if (QueryNode_NumChildren(q->root) == 0) {
            ...
            // otherwise, add a new phrase node as the parent of the current child of the hybrid
            // vector node, and set its children to be the previous child and the new filter node.
        } else {
...
static inline void addTerm(char *str, size_t tok_len, QueryEvalCtx *q, QueryNodeOptions *opts,
                           IndexIterator ***its, size_t *itsSz, size_t *itsCap) {
...
static IndexIterator *Query_EvalPrefixNode(QueryEvalCtx *q, QueryNode *qn) {
    ...
    if (spec->suffix && qn->pfx.suffix) {
        ...
        TrieNode_IterateContains(t->root, str, nstr, qn->pfx.prefix, qn->pfx.suffix, runeIterCb,
                                 &ctx, &q->sctx->timeout);
    }
    ...
    if (!ctx.its || ctx.nits == 0) {
        ...
        return NewUnionIterator(ctx.its, ctx.nits, 1, qn->opts.weight, QN_PREFIX, qn->pfx.tok.str,
                                q->config);
    }
}
...
static IndexIterator *Query_EvalWildcardQueryNode(QueryEvalCtx *q, QueryNode *qn) {
    ...
    if (spec->suffix) {
        ...
        if (qn->opts.fieldMask == RS_FIELDMASK_ALL ||
            ...
            SuffixCtx sufCtx = {
                ...
                .callback =
                    charIterCb, // the difference is weather the function receives char or rune
                .cbCtx = &ctx,
    ...
    if (!ctx.its || ctx.nits == 0) {
        ...
        return NewUnionIterator(ctx.its, ctx.nits, 1, qn->opts.weight, QN_WILDCARD_QUERY,
...
static void rangeIterCbStrs(const char *r, size_t n, void *p, void *invidx) {
    ...
    IndexReader *ir =
        NewTermIndexReader(invidx, q->sctx->spec, RS_FIELDMASK_ALL, term, ctx->weight);
    if (!ir) {
        Term_Free(term);
        return;
    }
...
static IndexIterator *Query_EvalLexRangeNode(QueryEvalCtx *q, QueryNode *lx) {
    ...
    if (!ctx.its || ctx.nits == 0) {
        ...
        return NewUnionIterator(ctx.its, ctx.nits, 1, lx->opts.weight, QN_LEXRANGE, NULL,
                                q->config);
    }
}
...
static IndexIterator *Query_EvalFuzzyNode(QueryEvalCtx *q, QueryNode *qn) {
    ...
    return iterateExpandedTerms(q, terms, qn->pfx.tok.str, qn->pfx.tok.len, qn->fz.maxDist, 0,
                                &qn->opts);
}
...
static IndexIterator *Query_EvalOptionalNode(QueryEvalCtx *q, QueryNode *qn) {
    ...
    return NewOptionalIterator(QueryNode_NumChildren(qn) ? Query_EvalNode(q, qn->children[0])
                                                         : NULL,
...
static IndexIterator *Query_EvalGeofilterNode(QueryEvalCtx *q, QueryNode *node, double weight) {
...
static IndexIterator *Query_EvalGeometryNode(QueryEvalCtx *q, QueryNode *node) {
    ...
    IndexIterator *ret =
        api->query(index, gq->query_type, gq->format, gq->str, gq->str_len, &errMsg);
    if (ret == NULL) {
...
static IndexIterator *Query_EvalVectorNode(QueryEvalCtx *q, QueryNode *qn) {
    ...
    if (qn->opts.distField) {
        if (qn->vn.vq->scoreField) {
            // Since the KNN syntax allows specifying the distance field in two ways (...=>[KNN ...
            // AS <dist_field>] and
            // ...=>[KNN ...]=>{$YIELD_DISTANCE_AS:<dist_field>), we validate that we got it only
            // once.
...
static IndexIterator *Query_EvalTagLexRangeNode(QueryEvalCtx *q, TagIndex *idx, QueryNode *qn,
    ...
    if (ctx.nits == 0) {
        ...
        return NewUnionIterator(ctx.its, ctx.nits, 1, qn->opts.weight, QN_LEXRANGE, NULL,
                                q->config);
    }
}
...
static IndexIterator *Query_EvalTagPrefixNode(QueryEvalCtx *q, TagIndex *idx, QueryNode *qn,
    ...
    if (!qn->pfx.suffix || !withSuffixTrie) { // prefix query or no suffix triemap, use bruteforce
        ...
        while (nextFunc(it, &s, &sl, &ptr) && (itsSz < q->config->maxPrefixExpansions)) {
        ...
        for (int i = 0; i < array_len(arr); ++i) {
            ...
            for (int j = 0; j < array_len(arr[i]); ++j) {
                ...
                IndexIterator *ret =
                    TagIndex_OpenReader(idx, q->sctx->spec, arr[i][j], strlen(arr[i][j]), 1);
                if (!ret)
                    continue;
...
static IndexIterator *Query_EvalTagWildcardNode(QueryEvalCtx *q, TagIndex *idx, QueryNode *qn,
    ...
    if (idx->suffix) {
        ...
        arrayof(char *) arr = GetList_SuffixTrieMap_Wildcard(
            idx->suffix, tok->str, tok->len, q->sctx->timeout, q->config->maxPrefixExpansions);
        if (!arr) {
            // No matching terms
            rm_free(its);
            return NULL;
            ...
            for (int i = 0; i < array_len(arr); ++i) {
                ...
                IndexIterator *ret =
                    TagIndex_OpenReader(idx, q->sctx->spec, arr[i], strlen(arr[i]), 1);
                if (!ret)
                    continue;
...
static IndexIterator *Query_EvalTagNode(QueryEvalCtx *q, QueryNode *qn) {
    ...
    const FieldSpec *fs =
        IndexSpec_GetField(q->sctx->spec, node->fieldName, strlen(node->fieldName));
    if (!fs) {
        return NULL;
    }
...
static inline bool QueryNode_ValidateToken(QueryNode *n, IndexSpec *spec, RSSearchOptions *opts,
    ...
    if (n->tn.len == 0 && n->tn.str && !strcmp(n->tn.str, "") &&
        ...
        QueryError_SetErrorFmt(
            status, QUERY_ESYNTAX,
            "Use `%s` in field creation in order to index and query for empty strings",
            SPEC_INDEXEMPTY_STR);
        return false;
    }
    return true;
...
void QueryNode_AddChild(QueryNode *n, QueryNode *ch) { QueryNode_AddChildren(n, &ch, 1); }
...
static int QueryVectorNode_ApplyAttribute(VectorQuery *vq, QueryAttribute *attr) {
    if (STR_EQCASE(attr->name, attr->namelen, VECSIM_EFRUNTIME) ||
        ...
        // Move ownership on the value string, so it won't get freed when releasing the
        // QueryAttribute. The name string was not copied by the parser (unlike the value) - so we
        // copy and save it.
...
#define MK_INVALID_VALUE()                                                                         \
    ...
    if (STR_EQCASE(attr->name, attr->namelen, SLOP_ATTR)) {
        ...
        if (b) {
            qn->opts.phonetic =
                PHONETIC_ENABLED; // means we specifically asked for phonetic matching
        } else {
            qn->opts.phonetic =
    ...
    if (!res) {
        QueryError_SetErrorFmt(status, QUERY_ENOOPTION, "Invalid attribute %.*s",
