...
static QueryIterator *Query_EvalPrefixNode(QueryEvalCtx *q, QueryNode *qn) {
  ...
  if (!str) {
    QueryError_SetWithoutUserDataFmt(q->status, QUERY_ERROR_CODE_LIMIT, "SEARCH_TRIE_STRING_TOO_LONG: %s " TRIE_STR_TOO_LONG_MSG, PrefixNode_GetTypeString(&qn->pfx));
    return NULL;
  }
  ...
  if (spec->suffix && qn->pfx.suffix) {
    ...
    if (qn->opts.fieldMask == RS_FIELDMASK_ALL ||
      ...
      QueryError_SetError(q->status, QUERY_ERROR_CODE_GENERIC, "SEARCH_SUFFIX_TRIE_UNSUPPORTED: Contains query on fields without WITHSUFFIXTRIE support");
    }
  } else {
...
static QueryIterator *Query_EvalWildcardQueryNode(QueryEvalCtx *q, QueryNode *qn) {
  ...
  if (!str) {
    QueryError_SetError(q->status, QUERY_ERROR_CODE_LIMIT, "SEARCH_WILDCARD_STRING_TOO_LONG: Wildcard " TRIE_STR_TOO_LONG_MSG);
    return NULL;
  }
...
static QueryIterator *Query_EvalGeometryNode(QueryEvalCtx *q, QueryNode *node) {
  ...
  if (ret == NULL) {
    QueryError_SetWithUserDataFmt(q->status, QUERY_ERROR_CODE_BAD_VAL, "SEARCH_GEOSHAPE_QUERY_ERROR: Error querying geoshape index", ": %s",
    ...
    if (qn->vn.vq->scoreField) {
      ...
      if (strcasecmp(qn->vn.vq->scoreField, default_score_field) != 0) {
        QueryError_SetWithUserDataFmt(q->status, QUERY_ERROR_CODE_DUP_FIELD,
                               "SEARCH_VECTOR_DISTANCE_FIELD_DUPLICATE: Distance field was specified twice for vector query", ": %s and %s",
...
static inline bool QueryNode_ValidateToken(QueryNode *n, IndexSpec *spec, RSSearchOptions *opts, QueryError *status) {
  if (n->tn.len == 0 && n->tn.str && !strcmp(n->tn.str, "") && !QueryNode_DoesIndexEmpty(n, spec, opts)) {
    QueryError_SetError(status, QUERY_ERROR_CODE_SYNTAX, "SEARCH_EMPTY_STRING_INDEXING_REQUIRED: Use `" SPEC_INDEXEMPTY_STR "` in field creation in order to index and query for empty strings");
    return false;
  }
  return true;
}
...
static int QueryNode_CheckIsValid(QueryNode *n, IndexSpec *spec, RSSearchOptions *opts,
  ...
  if ((effectiveFlags & QAST_NO_WEIGHT) && n->opts.explicitWeight) {
    QueryError_SetError(status, QUERY_ERROR_CODE_WEIGHT_NOT_ALLOWED, "SEARCH_WEIGHT_NOT_ALLOWED: Weight attribute not allowed in this context");
    return REDISMODULE_ERR;
  }
  ...
  switch(n->type) {
    ...
    case QN_NUMERIC: {
        if (n->nn.nf->min > n->nn.nf->max) {
          QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_SYNTAX, "SEARCH_NUMERIC_RANGE_INVALID: Invalid numeric range (min > max)", ": @%s:[%f %f]",
    ...
    case QN_VECTOR:
      if (effectiveFlags & QAST_NO_VECTOR) {
        QueryError_SetError(status, QUERY_ERROR_CODE_VECTOR_NOT_ALLOWED, "SEARCH_VECTOR_NOT_ALLOWED: Vector query not allowed in this context");
        res = REDISMODULE_ERR;
      }
      break;
    case QN_NOT:
...
static int ValidateShardKRatio(const char *value, double *ratio, QueryError *status) {
  if (!ParseDouble(value, ratio, 1)) {
    QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_INVAL,
      "SEARCH_SHARD_K_RATIO_INVALID: Invalid shard k ratio value", " '%s'", value);
    return 0;
  }
  ...
  if (*ratio <= MIN_SHARD_WINDOW_RATIO || *ratio > MAX_SHARD_WINDOW_RATIO) {
    QueryError_SetWithoutUserDataFmt(status, QUERY_ERROR_CODE_INVAL,
      "SEARCH_SHARD_K_RATIO_OUT_OF_RANGE: Invalid shard k ratio value: Shard k ratio must be greater than %g and at most %g (got %g)",
...
#define MK_INVALID_VALUE()                                                             \
  QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_SYNTAX, "SEARCH_ATTRIBUTE_VALUE_INVALID: Invalid value", " (%.*s) for `%.*s`", \
  ...
  if (!res) {
    QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_NO_OPTION, "SEARCH_ATTRIBUTE_INVALID: Invalid attribute", " %.*s", (int)attr->namelen,
                           attr->name);
  }
  return res;
}
