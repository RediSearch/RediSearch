...
#define VECSIM_ERR_MANDATORY(status, algorithm, arg)                                               \
    QERR_MKBADARGS_FMT(                                                                            \
        status,                                                                                    \
        "Missing mandatory parameter: cannot create %s index without specifying %s argument",      \
        algorithm, arg)

typedef enum { VECSIM_QT_KNN, VECSIM_QT_RANGE } VectorQueryType;

// This struct holds VecSimRawParam array and bool array.
// the arrays should have the same length, for testing if the param in some index needs to be
// evaluated. `params` params will always hold parameter name and value as allocated string. First,
// the parser creates the param, holds the key-name and value as they appear in the query, and marks
// if the value is the literal value or an attribute name. Second, in the parameters evaluation
// step, if a param was marked as an attribute, we try to resolve it, and free its old value and
// replace it with the actual value if we succeed. It is the VecSim library job to resolve this
// strings-key-value params (array) into a VecSimQueryParams struct.
typedef struct {
...
typedef enum {
    ...
    VECSIM_HYBRID_BATCHES,  // Get the top vector results in batches upon demand, and keep the
                            // results that
    ...
    VECSIM_RANGE_QUERY, // Run range query, to return all vectors that are within a given range from
                        // the
                        //  query vector.
    VECSIM_LAST_SEARCHMODE, // Last value of this enum. Can be used to check if a given value
                            // resides within
