  ...
  switch (opt->type) {
    ...
    case Q_OPT_UNDECIDED: {
      if (!opt->field) {
        ...
        opt->type = Q_OPT_NONE;
        // Read `fieldIndex` directly, not `fieldSpec->index`: under WORKERS>0 this runs
        // on a worker thread well after parsing, and a concurrent FT.ALTER may have since
        // reallocated the spec's field array, leaving `fieldSpec` dangling (MOD-18361).
