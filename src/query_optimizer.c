...
QOptimizer *QOptimizer_New() { return rm_calloc(1, sizeof(QOptimizer)); }
...
void QOptimizer_QueryNodes(QueryNode *root, QOptimizer *opt) {
    ...
    if ((!isSortby && opt->scorerReq) ||
        (root->type == QN_VECTOR && root->vn.vq->type == VECSIM_QT_KNN)) {
        opt->type = Q_OPT_NONE;
        return;
    }
    ...
    case Q_OPT_UNDECIDED: {
        if (!opt->field) {
            ...
            IndexIterator *numericIter =
                NewNumericFilterIterator(req->sctx, opt->sortbyNode->nn.nf, &req->conc,
                                         INDEXFLD_T_NUMERIC, &req->ast.config);
...
void QOptimizer_UpdateTotalResults(AREQ *req) {
    ...
    req->qiter.totalResults =
        req->qiter.totalResults > reqOffset ? req->qiter.totalResults - reqOffset : 0;
