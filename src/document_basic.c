...
void Document_MakeRefOwner(Document *doc) { doc->flags |= DOCUMENT_F_OWNREFS; }
...
int Document_LoadSchemaFieldHash(Document *doc, RedisSearchCtx *sctx, QueryError *status) {
    ...
    if (!k || RedisModule_KeyType(k) != REDISMODULE_KEYTYPE_HASH) {
        QueryError_SetErrorFmt(status, QUERY_EINVAL, "Key %s does not exist or is not a hash",
    ...
    for (size_t ii = 0; ii < spec->numFields; ++ii) {
        ...
        doc->fields[oix].name =
            (field->name == field->path) ? doc->fields[oix].path : rm_strdup(field->name);
...
int Document_LoadSchemaFieldJson(Document *doc, RedisSearchCtx *sctx, QueryError *status) {
    ...
    for (; ii < spec->numFields; ++ii) {
        ...
        if (JSON_LoadDocumentField(jsonIter, len, field, &doc->fields[oix], ctx, status) !=
            REDISMODULE_OK) {
...
int Redis_SaveDocument(RedisSearchCtx *ctx, const AddDocumentOptions *opts, QueryError *status) {
    ...
    if (opts->languageStr) {
        ...
        if (ctx->spec->rule->lang_field == NULL) {
            ctx->spec->rule->lang_field =
                rm_strndup(UNDERSCORE_LANGUAGE, strlen(UNDERSCORE_LANGUAGE));
        }
        RedisSearchCtx_UnlockSpec(ctx);
    }
    ...
    if (opts->payload) {
        ...
        if (ctx->spec->rule->payload_field == NULL) {
            ctx->spec->rule->payload_field =
                rm_strndup(UNDERSCORE_PAYLOAD, strlen(UNDERSCORE_PAYLOAD));
        }
        RedisSearchCtx_UnlockSpec(ctx);
    }
