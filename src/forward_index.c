...
static uint32_t khtHash(const KHTableEntry *entBase) { return ((khIdxEntry *)entBase)->ent.hash; }
...
static uint32_t hashKey(const void *s, size_t n) { return rs_fnv_32a_buf((void *)s, n, 0); }
...
#define SYNONYM_BUFF_LEN 100
    ...
    if (tokCtx->idx->smap) {
        TermData *t_data =
            SynonymMap_GetIdsBySynonym(tokCtx->idx->smap, tokInfo->tok, tokInfo->tokLen);
        if (t_data) {
            char synonym_buff[SYNONYM_BUFF_LEN];
            size_t synonym_len;
            for (int i = 0; i < array_len(t_data->groupIds); ++i) {
                ForwardIndex_HandleToken(tokCtx->idx, t_data->groupIds[i],
                                         strlen(t_data->groupIds[i]), tokInfo->pos,
    ...
    if (tokInfo->phoneticsPrimary) {
        ForwardIndex_HandleToken(tokCtx->idx, tokInfo->phoneticsPrimary,
                                 strlen(tokInfo->phoneticsPrimary), tokInfo->pos,
                                 tokCtx->fieldScore, tokCtx->fieldId, TOKOPT_F_COPYSTR);
    }
