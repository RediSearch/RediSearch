...
/**
 * Most of the spec interaction is done through the RefManager, which is wrapped by a strong or weak
 * reference struct. In the LLAPI we return a pointer to an RSIndex. In order to not break the API,
 * we typedef the RSIndex to be RefManager and we return it instead of the strong reference that
 * should wrap it. we can assume that every time we get a RefManager, we can cast it to (wrap it
 * with) a strong reference and use it as such.
 */

int RediSearch_GetCApiVersion() { return REDISEARCH_CAPI_VERSION; }
...
RefManager *RediSearch_CreateIndex(const char *name, const RSIndexOptions *options) {
    ...
    if (options->stopwordsLen != -1) {
        ...
        spec->stopwords =
            NewStopWordListCStr((const char **)options->stopwords, options->stopwordsLen);
    }
    return ref.rm;
}
...
RSDoc *RediSearch_CreateDocument2(const void *docKey, size_t len, RefManager *rm, double score,
                                  const char *lang) {
    IndexSpec *sp = __RefManager_Get_Object(rm);
    RedisModuleString *docKeyStr = RedisModule_CreateString(NULL, docKey, len);

    RSLanguage language = lang               ? RSLanguage_Find(lang, 0)
                          : (sp && sp->rule) ? sp->rule->lang_default
                                             : DEFAULT_LANGUAGE;
    double docScore = !isnan(score)      ? score
                      : (sp && sp->rule) ? sp->rule->score_default
                                         : DEFAULT_SCORE;
...
void RediSearch_DocumentAddFieldNumber(Document *d, const char *fieldname, double val,
                                       unsigned as) {
...
int RediSearch_DocumentAddFieldGeo(Document *d, const char *fieldname, double lat, double lon,
                                   unsigned as) {
...
QueryNode *RediSearch_CreateContainsNode(RefManager *rm, const char *fieldName, const char *s) {
    return RediSearch_CreateAffixNode(__RefManager_Get_Object(rm), fieldName, s,
...
QueryNode *RediSearch_CreateLexRangeNode(RefManager *rm, const char *fieldName, const char *begin,
    ...
    if (fieldName) {
        ret->opts.fieldMask =
            IndexSpec_GetFieldBit(__RefManager_Get_Object(rm), fieldName, strlen(fieldName));
    }
    return ret;
}

QueryNode *RediSearch_CreateTagLexRangeNode(RefManager *rm, const char *begin, const char *end,
                                            int includeBegin, int includeEnd) {
    QueryNode *ret = NewQueryNode(QN_LEXRANGE);
    if (begin) {
        ret->lxrng.begin = begin ? rm_strdup(begin) : NULL;
        ret->lxrng.includeBegin = includeBegin;
...
QueryNode *RediSearch_CreateUnionNode(RefManager *rm) { return NewQueryNode(QN_UNION); }

QueryNode *RediSearch_CreateEmptyNode(RefManager *rm) { return NewQueryNode(QN_NULL); }

QueryNode *RediSearch_CreateNotNode(RefManager *rm) { return NewQueryNode(QN_NOT); }

int RediSearch_QueryNodeGetFieldMask(QueryNode *qn) { return qn->opts.fieldMask; }

void RediSearch_QueryNodeAddChild(QueryNode *parent, QueryNode *child) {
    QueryNode_AddChild(parent, child);
}

void RediSearch_QueryNodeClearChildren(QueryNode *qn) { QueryNode_ClearChildren(qn, 1); }

QueryNode *RediSearch_QueryNodeGetChild(const QueryNode *qn, size_t ix) {
    return QueryNode_GetChild(qn, ix);
}

size_t RediSearch_QueryNodeNumChildren(const QueryNode *qn) { return QueryNode_NumChildren(qn); }
...
static RS_ApiIter *handleIterCommon(IndexSpec *sp, QueryInput *input, char **error) {
    ...
    if (input->qtype == QUERY_INPUT_STRING) {
        if (QAST_Parse(&it->qast, &sctx, &options, input->u.s.qs, input->u.s.n, input->u.s.dialect,
    ...
    ExtScoringFunctionCtx *scoreCtx =
        Extensions_GetScoringFunction(&it->scargs, DEFAULT_SCORER_NAME);
...
RS_ApiIter *RediSearch_IterateQueryWithDialect(RefManager *rm, const char *s, size_t n,
                                               unsigned int dialect, char **error) {
    QueryInput input = {.qtype = QUERY_INPUT_STRING,
                        .u = {.s = {.qs = s, .n = n, .dialect = dialect}}};
    return handleIterCommon(__RefManager_Get_Object(rm), &input, error);
}
...
void RediSearch_QueryNodeFree(QueryNode *qn) { QueryNode_Free(qn); }

int RediSearch_QueryNodeType(QueryNode *qn) { return qn->type; }
...
void RediSearch_IndexOptionsSetStopwords(RSIndexOptions *opts, const char **stopwords,
                                         int stopwordsLen) {
