...
static ResultProcessor *buildGroupRP(PLN_GroupStep *gstp, RLookup *srclookup,
  ...
  for (size_t ii = 0; ii < nproperties; ++ii) {
    ...
    if (!srckeys[ii]) {
      ...
      if (!loadKeys || !(srckeys[ii]->flags & RLOOKUP_F_SCHEMASRC)) {
        QueryError_SetWithUserDataFmt(err, QUERY_ERROR_CODE_NO_PROP_KEY, "SEARCH_PROP_NOT_FOUND: No such property", " `%s`", fldname);
        return NULL;
      }
    }
    dstkeys[ii] = RLookup_GetKey_WriteEx(&gstp->lookup, fldname, fldname_len, RLOOKUP_F_NOFLAGS);
    if (!dstkeys[ii]) {
      QueryError_SetWithUserDataFmt(err, QUERY_ERROR_CODE_DUP_FIELD, "SEARCH_PROPERTY_DUPLICATE: Property", " `%s` specified more than once", fldname);
      return NULL;
    }
  }
  ...
  for (size_t ii = 0; ii < nreducers; ++ii) {
    ...
    if (!ff) {
      // No such reducer!
      Grouper_Free(grp);
      QueryError_SetWithUserDataFmt(err, QUERY_ERROR_CODE_NO_REDUCER, "SEARCH_REDUCER_NOT_FOUND: No such reducer", ": %s", pr->name);
      return NULL;
    }
    ...
    if (!dstkey) {
      Grouper_Free(grp);
      QueryError_SetWithUserDataFmt(err, QUERY_ERROR_CODE_DUP_FIELD, "SEARCH_PROPERTY_DUPLICATE: Property", " `%s` specified more than once", pr->alias);
      return NULL;
    }
  }
...
static ResultProcessor *getAdditionalMetricsRP(RedisSearchCtx* sctx, const QueryAST* ast, RLookup *rl, QueryError *status) {
  ...
  for (size_t i = 0; i < array_len(requests); i++) {
    ...
    if (IndexSpec_GetFieldWithLength(sctx->spec, name, name_len)) {
      QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_INDEX_EXISTS, "SEARCH_PROPERTY_EXISTS: Property", " `%s` already exists in schema", name);
      return NULL;
    }
    ...
    if (!key) {
      QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_DUP_FIELD, "SEARCH_PROPERTY_DUPLICATE: Property", " `%s` specified more than once", name);
      return NULL;
    }
      ...
      for (size_t ii = 0; ii < nkeys; ++ii) {
        ...
        if (!sortkey) {
          ...
          if (!(sortkey->flags & RLOOKUP_F_SCHEMASRC)) {
            QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_NO_PROP_KEY, "SEARCH_PROPERTY_NOT_LOADED: Property", " `%s` not loaded nor in schema", keystr);
            goto end;
          }
...
static int processLoadStepArgs(PLN_LoadStep *loadStep, RLookup *lookup, uint32_t loadFlags,
  ...
  while (!AC_IsAtEnd(ac)) {
    ...
    if (AC_AdvanceIfMatch(ac, SPEC_AS_STR)) {
      ...
      if (rv != AC_OK) {
        if (status) {
          QueryError_SetError(status, QUERY_ERROR_CODE_PARSE_ARGS, "SEARCH_LOAD_AS_NAME_MISSING: LOAD path AS name - must be accompanied with NAME");
        }
        return REDISMODULE_ERR;
      } else if (!strcasecmp(name, SPEC_AS_STR)) {
        if (status) {
          QueryError_SetError(status, QUERY_ERROR_CODE_PARSE_ARGS, "SEARCH_LOAD_ALIAS_INVALID: Alias for LOAD cannot be `AS`");
        }
        return REDISMODULE_ERR;
      }
    } else {
  ...
  if (scoresExplicitlyRequested || (isSearchReturningRows && scoringNeeded)) {
    ...
    if (HasScoreInPipeline(&params->common)) {
      if (params->common.scoreAlias) {
        ...
        if (!scoreKey) {
          QueryError_SetWithUserDataFmt(pipeline->qctx.err, QUERY_ERROR_CODE_DUP_FIELD, "SEARCH_SCORE_ALIAS_EXISTS: Could not create score alias, name already exists in query", "%s", params->common.scoreAlias);
          return;
        }
      } else {
...
int buildOutputPipeline(Pipeline *pipeline, const AggregationPipelineParams* params, uint32_t loadFlags, QueryError *status, bool forceLoad, uint32_t *outStateFlags) {
  ...
  if (params->common.reqflags & QEXEC_F_SEND_HIGHLIGHT) {
    ...
    for (size_t ii = 0; ii < params->outFields->numFields; ++ii) {
      ...
      if (!kk) {
        QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_NO_PROP_KEY, "SEARCH_PROP_NOT_FOUND: No such property", " `%s`", ff->name);
        goto error;
      } else if (!(kk->flags & RLOOKUP_F_SCHEMASRC)) {
        QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_INVAL, "SEARCH_PROPERTY_NOT_IN_SCHEMA: Property", " `%s` is not in schema", ff->name);
        goto error;
      }
      ff->lookupKey = kk;
    }
    ...
    switch (stp->type) {
      ...
      case PLN_T_FILTER: {
        ...
        if (stp->type == PLN_T_APPLY) {
          ...
          if (!dstkey) {
            ...
            QueryError_SetWithUserDataFmt(status, QUERY_ERROR_CODE_DUP_FIELD, "SEARCH_PROPERTY_DUPLICATE: Property", " `%s` specified more than once", stp->alias);
            goto error;
          }
      ...
      case PLN_T_LOAD: {
        ...
        if (curLookup != rootLookup) {
          QueryError_SetError(status, QUERY_ERROR_CODE_INVAL,
                              "SEARCH_LOAD_ORDER_INVALID: LOAD cannot be applied after projectors or reducers");
          goto error;
        }
      ...
      case PLN_T_VECTOR_NORMALIZER: {
        ...
        if (!vectorField || !FIELD_IS(vectorField, INDEXFLD_T_VECTOR)) {
          QueryError_SetError(status, QUERY_ERROR_CODE_SYNTAX, "SEARCH_VECTOR_FIELD_INVALID: Invalid vector field for normalization");
          goto error;
        }
