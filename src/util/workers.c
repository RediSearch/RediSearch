...
static void yieldCallback(void *yieldCtx) {
    ...
    if (yield_counter % 10 == 0 || yield_counter == 1) {
        RedisModule_Log(RSDummyContext, "verbose",
                        "Yield every 100 ms to allow redis server run while"
                        " waiting for workers to finish: call number %zu",
                        yield_counter);
    }
...
int workersThreadPool_CreatePool(size_t worker_count) {
    ...
    _workers_thpool = redisearch_thpool_create(worker_count, RSGlobalConfig.highPriorityBiasNum,
                                               LogCallback, "workers");
    if (_workers_thpool == NULL)
 ...
 * New workers number should be `in_event ? MAX(numWorkerThreads, minOperationWorkers) :
 * numWorkerThreads`. This function also handles the cases where the thread pool is turned on/off.
...
void workersThreadPool_SetNumWorkers() {
    ...
    if (worker_count == 0 && curr_workers > 0) {
        ...
        new_num_threads =
            redisearch_thpool_add_threads(_workers_thpool, worker_count - curr_workers);
        if (!curr_workers)
            workersThreadPool_OnActivation(worker_count);
    } else if (worker_count < curr_workers) {
        new_num_threads =
            redisearch_thpool_remove_threads(_workers_thpool, curr_workers - worker_count);
    }

    RS_LOG_ASSERT_FMT(new_num_threads == worker_count,
                      "Attempt to change the workers thpool size to %lu "
                      "resulted unexpectedly in %lu threads.",
                      worker_count, new_num_threads);
}
...
void workersThreadPool_Drain(RedisModuleCtx *ctx, size_t threshold) {
    ...
    if (RedisModule_Yield) {
        // Wait until all the threads in the pool run the jobs until there are no more than
        // <threshold> jobs in the queue. Periodically return and call RedisModule_Yield, so redis
        // can answer PINGs (and other stuff) so that the node-watch dog won't kill redis, for
        // example.
...
void workersThreadPool_Terminate(void) { redisearch_thpool_terminate_threads(_workers_thpool); }

void workersThreadPool_Destroy(void) { redisearch_thpool_destroy(_workers_thpool); }
...
int workerThreadPool_isPaused() { return redisearch_thpool_paused(_workers_thpool); }
