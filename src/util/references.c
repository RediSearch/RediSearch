...
inline void *__RefManager_Get_Object(RefManager *rm) { return rm ? rm->obj : NULL; }
...
static void RefManager_ReturnStrongReference(RefManager *rm) {
    if (__atomic_sub_fetch(&rm->strong_refcount, 1, __ATOMIC_SEQ_CST) == 0) {
        rm->freeCB(rm->obj);
        RedisModule_Log(RSDummyContext, REDISMODULE_LOGLEVEL_DEBUG, "RefManager's object freed: %p",
                        rm);
    }
}
...
StrongRef WeakRef_Promote(WeakRef w_ref) { return _Ref_GetStrong(w_ref.rm); }

WeakRef WeakRef_Clone(WeakRef ref) { return _Ref_GetWeak(ref.rm); }

void WeakRef_Release(WeakRef w_ref) { RefManager_ReturnWeakReference(w_ref.rm); }

WeakRef StrongRef_Demote(StrongRef s_ref) { return _Ref_GetWeak(s_ref.rm); }

StrongRef StrongRef_Clone(StrongRef ref) { return _Ref_GetStrong(ref.rm); }

void StrongRef_Invalidate(StrongRef s_ref) { RefManager_InvalidateObject(s_ref.rm); }

void StrongRef_Release(StrongRef s_ref) { RefManager_ReturnReferences(s_ref.rm); }

void *StrongRef_Get(StrongRef s_ref) { return __RefManager_Get_Object(s_ref.rm); }
