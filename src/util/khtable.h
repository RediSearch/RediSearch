...
struct KHTable {
  ...
  KHTable(void *allocator, size_t estSize);
  virtual ~KHTable();
  //void FreeEx(void *arg, void (*Free)(KHTableEntry *entry, void *ctx, void *arg));
  ...
  //
  // Should return 0 if the key matches, and nonzero otherwise
  virtual int Compare(const KHTableEntry *item, const void *s, size_t n, uint32_t h) = 0;

  // Retrieve the hash from the entry. This should extract the key and key length
  // from the entry and return the hash. Note that you may also cache the hash
  // value in the entry if you so desire.
  virtual uint32_t Hash(const KHTableEntry *ent) = 0;
  ...
  // The pointer passed is the `ctx` argument to KHTable_Init().
  // Note that there is no API to free an individual item.
  virtual KHTableEntry *Alloc(void *) = 0;

  // Print a textual representation of the entry to the given file. This is
  // used when printing the hash table.
  virtual void Print(const KHTableEntry *ent, FILE *fp) {}
};
...
struct KHTableIterator {
  KHTable *ht;
  size_t curBucket;
  KHTableEntry *cur;

  KHTableIterator(KHTable *ht);
  KHTableEntry *Next();
};
