...
// For now, we assume that all the jobs that are submitted are low priority jobs (not blocking any
// client). We can add the priority to the `spec_ctx` (and rename it) if needed.
int ThreadPoolAPI_SubmitIndexJobs(void *pool, void *spec_ctx, void **ext_jobs,
                                  ThreadPoolAPI_CB *cbs, size_t n_jobs) {
