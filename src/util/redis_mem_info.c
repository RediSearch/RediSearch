...
RedisMemoryFlexRatios RedisMemory_GetFlexRatios(RedisModuleCtx *ctx) {
  ...
  // big_max_ram (reported as max_ram) does not fold in max_process_mem, but getMaxRAM() does, so
  // apply it here too for parity with the server's own RAM accounting.
  ...
  return (RedisMemoryFlexRatios){
      .total = total_limit ? used_memory / (float)total_limit : 0,
      .ram = ram_limit ? used_ram / (float)ram_limit : 0,
  };
}
