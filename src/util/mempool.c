...
void mempool_test_set_global(mempool_t **global_p, const mempool_options *options) {
    ...
    if (__atomic_compare_exchange_n(global_p, &uninitialized, new_pool, 0, __ATOMIC_SEQ_CST,
                                    __ATOMIC_SEQ_CST)) {
        ...
        // Otherwise, the global pool was initialized while we created the pool, so we can destroy
        // ours.
