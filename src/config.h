...
typedef struct {
    ...
    // Can allow to control the seperation between phrases in different array slots (related to the
    // SLOP parameter in ft.search command) Default value is 100. 0 will not increment (as if all
    // text is a continus phrase).
...
#define MIN_DIALECT_VERSION                                                                        \
    1 // MIN_DIALECT_VERSION is expected to change over time as dialects become deprecated.
#define MAX_DIALECT_VERSION 4 // MAX_DIALECT_VERSION may not exceed MIN_DIALECT_VERSION + 7.
#define DIALECT_OFFSET(d)                                                                          \
    (1ULL << (d - MIN_DIALECT_VERSION)) // offset of the d'th bit. begins at MIN_DIALECT_VERSION
                                        // (bit 0) up to MAX_DIALECT_VERSION.
#define GET_DIALECT(barr, d)                                                                       \
    (!!(barr &                                                                                     \
        DIALECT_OFFSET(d))) // return the truth value of the d'th dialect in the dialect bitarray.
#define SET_DIALECT(barr, d)                                                                       \
    (barr |= DIALECT_OFFSET(d)) // set the d'th dialect in the dialect bitarray to true.
...
#define MT_BUILD_CONFIG                                                                            \
    .numWorkerThreads = 0, .minOperationWorkers = MIN_OPERATION_WORKERS,                           \
...
#define RS_DEFAULT_CONFIG                                                                          \
    {                                                                                              \
        .extLoad = NULL, .gcConfigParams.enableGC = 1, .iteratorsConfigParams.minTermPrefix = 2,   \
        ...
        .requestConfigParams.timeoutPolicy = TimeoutPolicy_Return, .cursorReadSize = 1000,         \
        .cursorMaxIdle = 300000, .maxDocTableSize = DEFAULT_DOC_TABLE_SIZE,                        \
        MT_BUILD_CONFIG.gcConfigParams.gcScanSize = GC_SCANSIZE,                                   \
        ...
        .gcConfigParams.forkGc.forkGcCleanThreshold = 100, .noMemPool = 0, .filterCommands = 0,    \
        .maxSearchResults = SEARCH_REQUEST_RESULTS_MAX, .maxAggregateResults = -1,                 \
        .iteratorsConfigParams.minUnionIterHeap = 20, .numericCompress = false,                    \
        .numericTreeMaxDepthRange = 0, .requestConfigParams.printProfileClock = 1,                 \
        .invertedIndexRawDocidEncoding = false,                                                    \
        .gcConfigParams.forkGc.forkGCCleanNumericEmptyNodes = true, .freeResourcesThread = true,   \
        .requestConfigParams.dialectVersion = 1, .vssMaxResize = 0, .multiTextOffsetDelta = 100,   \
        .used_dialects = 0, .numBGIndexingIterationsBeforeSleep = 100,                             \
