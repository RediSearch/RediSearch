...
void NumericRange_Dump(NumericRange *r, int indent) {
    ...
    ++indent;
    PRINT_INDENT(indent);
    printf("minVal %f, maxVal %f, unique_sum %f, invertedIndexSize %zu, card %hu, cardCheck %hu, "
           "splitCard %u\n",
           r->minVal, r->maxVal, r->unique_sum, r->invertedIndexSize, r->card, r->cardCheck,
           r->splitCard);
...
NRN_AddRv NumericRangeNode_Add(NumericRangeNode *n, t_docId docId, double value) {
    ...
    if (!NumericRangeNode_IsLeaf(n)) {
        // if this node has already split but retains a range, just add to the range without
        // checking anything
        size_t s = 0;
        size_t nRecords = 0;
        if (n->range) {
...
void __recursiveAddRange(Vector *v, NumericRangeNode *n, const NumericFilter *nf, size_t *total) {
    if (!n || (nf->limit && (*total >= nf->offset + nf->limit)))
        return;
    double min = nf->min;
    double max = nf->max;
    if (n->range) {
        // if the range is completely contained in the search, we can just add it and not inspect
        // any downwards
...
NRN_AddRv NumericRangeTree_Add(NumericRangeTree *t, t_docId docId, double value, int isMulti) {
    ...
    if (docId <= t->lastDocId && !isMulti) {
        // When not handling multi values - do not allow duplicate entries. This might happen due to
        // indexer bugs and we need to protect from it
...
IndexIterator *NewNumericRangeIterator(const IndexSpec *sp, NumericRange *nr,
    ...
    if (NumericFilter_IsNumeric(f) && NumericFilter_Match(f, nr->minVal) &&
        NumericFilter_Match(f, nr->maxVal)) {
...
void NumericRangeIterator_OnReopen(void *privdata) {
    ...
    RedisModuleString *numField =
        IndexSpec_GetFormattedKeyByName(sp, nu->fieldName, INDEXFLD_T_NUMERIC);
