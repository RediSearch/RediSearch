...
static double tfidfRecursive(const RSIndexResult *r, const RSDocumentMetadata *dmd,
    ...
    if (r->type == RSResultType_Term) {
        ...
        EXPLAIN(scrExp, "(TFIDF %.2f = Weight %.2f * TF %d * IDF %.2f)", res, r->weight, r->freq,
                idf);
        return res;
    }
...
static double bm25Recursive(const ScoringFunctionArgs *ctx, const RSIndexResult *r,
    ...
    if (r->type == RSResultType_Term) {
        ...
        EXPLAIN(scrExp,
                "(%.2f = Weight %.2f * F %d / (F %d + k1 1.2 * (1 - b 0.5 + b 0.5 * Average Len "
                "%.2f)))",
...
static double inline CalculateBM25Std(float b, float k1, double idf, double f, int doc_len,
                                      double avg_doc_len, RSScoreExplain *scrExp,
                                      const char *term) {
...
static double bm25StdRecursive(const ScoringFunctionArgs *ctx, const RSIndexResult *r,
    ...
    if (r->type == RSResultType_Term) {
        ...
        double idf = 1.0;
        double bm25 =
            CalculateBM25Std(b, k1, idf, 1, dmd->len, ctx->indexStats.avgDocLen, scrExp, "*");
        ret = r->weight * bm25;
    } else {
...
static double BM25StdScorer(const ScoringFunctionArgs *ctx, const RSIndexResult *r,
    ...
    EXPLAIN(scrExp, "Final BM25 : words BM25 %.2f * document score %.2f", bm25res, dmd->score);
 ...
 *
 ******************************************************************************************/
// Assumes that the spec (ctx->handle->spec) is properly guarded for reading by the caller (read
// lock or redis lock)
int DefaultExpander(RSQueryExpanderCtx *ctx, RSToken *token) {
    ...
    if (phonetic == PHONETIC_DEFAULT) {
        ...
        if (IndexSpec_CheckPhoneticEnabled(ctx->handle->spec,
                                           (*ctx->currentNode)->opts.fieldMask)) {
...
void DefaultExpanderFree(void *p) { StemmerExpanderFree(p); }
...
int DefaultExtensionInit(RSExtensionCtx *ctx) {
    ...
    if (ctx->RegisterScoringFunction(BM25_STD_SCORER_NAME, BM25StdScorer, NULL, NULL) ==
        REDISEARCH_ERR) {
        return REDISEARCH_ERR;
    }
    ...
    if (ctx->RegisterScoringFunction(TFIDF_DOCNORM_SCORER_NAME, TFIDFNormDocLenScorer, NULL,
                                     NULL) == REDISEARCH_ERR) {
        return REDISEARCH_ERR;
    }

    /* Register DOCSCORE scorer */
    if (ctx->RegisterScoringFunction(DOCSCORE_SCORER, DocScoreScorer, NULL, NULL) ==
        REDISEARCH_ERR) {
        return REDISEARCH_ERR;
    }
