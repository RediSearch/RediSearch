...
static void insertResultToHeap_Metric(HybridIterator *hr, RSIndexResult *child_res,
...
static void insertResultToHeap_Aggregate(HybridIterator *hr, RSIndexResult *res,
                                         RSIndexResult *child_res, RSIndexResult *vec_res,
                                         double *upper_bound) {
...
static void alternatingIterate(HybridIterator *hr, VecSimQueryReply_Iterator *vecsim_iter,
    ...
    while (IITER_HAS_NEXT(hr->child)) {
        if (cur_vec_res->docId == cur_child_res->docId) {
            ...
            if (ret_child != INDEXREAD_OK || ret_vec != INDEXREAD_OK)
                break;
        }
        // Otherwise, advance the iterator pointing to the lower id.
            ...
            // It may be the case where we skipped to an invalid result (in NOT iterator for
            // example), so we read to get the next valid result. If we passed cur_vec_res->docId,
            // we found the next valid id of the child.
...
static VecSimQueryReply_Code computeDistances(HybridIterator *hr) {
    ...
    while (hr->child->Read(hr->child->ctx, &cur_child_res) != INDEXREAD_EOF) {
        ...
        double metric =
            VecSimIndex_GetDistanceFrom_Unsafe(hr->index, cur_child_res->docId, qvector);
...
static bool reviewHybridSearchPolicy(HybridIterator *hr, size_t n_res_left,
                                     size_t child_upper_bound, size_t *child_num_estimated) {
...
static VecSimQueryReply_Code prepareResults(HybridIterator *hr) {
    if (hr->searchMode == VECSIM_STANDARD_KNN) {
        hr->reply = VecSimIndex_TopKQuery(hr->index, hr->query.vector, hr->query.k,
                                          &(hr->runtimeParams), hr->query.order);
    ...
    VecSimBatchIterator *batch_it =
        VecSimBatchIterator_New(hr->index, hr->query.vector, &hr->runtimeParams);
    double upper_bound = INFINITY;
...
static size_t HR_Len(void *ctx) { return HR_NumEstimated(ctx); }
...
IndexIterator *NewHybridVectorIterator(HybridIteratorParams hParams, QueryError *status) {
    ...
    if (hParams.qParams.searchMode < 0 || hParams.qParams.searchMode >= VECSIM_LAST_SEARCHMODE) {
        QueryError_SetErrorFmt(status, QUERY_EGENERIC,
                               "Creating new hybrid vector iterator has failed");
    }
    ...
    if (hParams.childIt == NULL || hParams.query.k == 0) {
        ...
        // IITER_INVALID_NUM_ESTIMATED_RESULTS is the default (invalid) value for indicating invalid
    ...
    ri->SkipTo =
        NULL; // As long as we return results by score (unsorted by id), this has no meaning.
