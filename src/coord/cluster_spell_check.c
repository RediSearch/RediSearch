...
static bool spellCheckReplySanity_resp2(MRReply *reply, uint64_t *totalDocNum, QueryError *qerr) {
  ...
  if (type != MR_REPLY_ARRAY) {
    QueryError_SetUserDataAgnosticErrorFmt(qerr, QUERY_EGENERIC, "wrong reply type. Expected array. Got %d",
                               MRReply_Type(reply));
    return false;
  }
  ...
  if (MRReply_Type(ndocs) != MR_REPLY_INTEGER) {
    QueryError_SetUserDataAgnosticErrorFmt(qerr, QUERY_EGENERIC, "Expected first reply as integer. Have %d",
                               MRReply_Type(ndocs));
    return false;
  }
...
static bool spellCheckReplySanity_resp3(MRReply *reply, uint64_t *totalDocNum, QueryError *qerr) {
  ...
  if (type != MR_REPLY_MAP) {
    QueryError_SetUserDataAgnosticErrorFmt(qerr, QUERY_EGENERIC, "wrong reply type. Expected map. Got %d",
                               MRReply_Type(reply));
    return false;
  }
  ...
  if (MRReply_Type(ndocs) != MR_REPLY_INTEGER) {
    QueryError_SetUserDataAgnosticErrorFmt(qerr, QUERY_EGENERIC, "Expected total_docs as integer. Have %d",
                               MRReply_Type(ndocs));
    return false;
  }
