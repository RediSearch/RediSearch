      ...
      case PLN_T_FILTER:
        ...
        if (!hadArrange) {
          ...
          if (filter_keys.keys.rowlen) {
            ...
            const char **argv = (const char**)rm_malloc(sizeof(*argv) * filter_keys.keys.rowlen);
            size_t argc = 0;
            for (RLookupKey *kk = filter_keys.keys.head; kk != NULL; kk = kk->next) {
...
int AREQ_BuildDistributedPipeline(AREQ *r, AREQDIST_UpstreamInfo *us, QueryError *status) {
  ...
  for (RLookupKey *kk = dstp->lk.keys.head; kk != NULL; kk = kk->next) {
