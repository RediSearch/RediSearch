...
// Callback for cursor deletion - just free the reply
static void cursorDeleteCb(struct redisAsyncContext *c, void *r, void *privdata) {
  (void)c; (void)privdata;
  if (r) MRReply_Free(r);
}

// Context for scheduling cursor delete work onto the IO thread.
// Modeled after MRIterator structure for consistency with iterManualNextCb.
typedef struct {
  IORuntimeCtx *ioRuntime;
  MRCommand *cmds;  // Array of commands to send
  size_t len;       // Number of commands
} CursorDeleteCtx;

// Runs on the IO thread to send cursor delete commands.
// Modeled after iterManualNextCb for consistency with MRIterator_Release flow.
static void cursorDeleteOnIOThread(void *p) {
  CursorDeleteCtx *ctx = p;
  IORuntimeCtx *io_runtime_ctx = ctx->ioRuntime;

  for (size_t i = 0; i < ctx->len; i++) {
    // Fire-and-forget: ignore REDIS_ERR since we can't do anything about it
    // and the cursor will eventually timeout on the shard.
    MRCluster_SendCommand(io_runtime_ctx, &ctx->cmds[i], cursorDeleteCb, NULL);
...
void MR_CursorMappingsDelete(CursorMappings *mappings, const char *indexName) {
  ...
  // Count valid cursors first
  size_t count = 0;
  for (size_t i = 0; i < array_len(mappings->mappings); i++) {
    if (mappings->mappings[i].cursorId != 0) count++;
  }
  if (count == 0) return;

  // Allocate context and commands array (similar to MRIterator's cbxs)
  CursorDeleteCtx *ctx = rm_new(CursorDeleteCtx);
  ctx->ioRuntime = MRCluster_GetIORuntimeCtx(cluster_g, MRCluster_AssignRoundRobinIORuntimeIdx(cluster_g));
  ctx->cmds = rm_malloc(sizeof(MRCommand) * count);
  ctx->len = 0;
  ...
  for (size_t i = 0; i < array_len(mappings->mappings); i++) {
    ...
    ctx->cmds[ctx->len++] = cmd;
  }

  IORuntimeCtx_Schedule(ctx->ioRuntime, cursorDeleteOnIOThread, ctx);
}
