...
static void uvFanoutRequest(void *p) {
  MRCtx *mrctx = p;
  IORuntimeCtx *ioRuntime = IORuntimePool_GetCtx(cluster_g, mrctx->ioRuntimeIdx);

  mrctx->numExpected =
      MRCluster_FanoutCommand(cluster_g->topo, ioRuntime, mrctx->mastersOnly, &mrctx->cmd, fanoutCallback, mrctx);
...
static void uvMapRequest(void *p) {
  ...
  int rc = MRCluster_SendCommand(cluster_g->topo, ioRuntime, mrctx->mastersOnly, &mrctx->cmd, fanoutCallback, mrctx);
...
int MR_Fanout(struct MRCtx *mrctx, MRReduceFunc reducer, MRCommand cmd, bool block) {
  ...
  IORuntimeCtx_Schedule(IORuntimePool_GetCtx(cluster_g, mrctx->ioRuntimeIdx), cluster_g->topo, uvFanoutRequest, mrctx);
  return REDIS_OK;
}
...
int MR_MapSingle(struct MRCtx *ctx, MRReduceFunc reducer, MRCommand cmd) {
  ...
  IORuntimeCtx_Schedule(IORuntimePool_GetCtx(cluster_g, ctx->ioRuntimeIdx), cluster_g->topo, uvMapRequest, ctx);
  return REDIS_OK;
}
...
void MR_UpdateTopology(MRClusterTopology *newTopo) {
  ...
  IORuntimeCtx_Schedule(cluster_g->control_plane_io_runtime, cluster_g->topo, uvUpdateTopologyRequest, controlPlaneCtx);
  for (size_t i = 0; i < cluster_g->io_runtimes_pool_size; i++) {
    ...
    IORuntimeCtx_Schedule(ioRuntime, cluster_g->topo, uvUpdateTopologyRequest, ctx);
  }
}
...
void MR_GetConnectionPoolState(RedisModuleCtx *ctx) {
  ...
  IORuntimeCtx_Schedule(cluster_g->control_plane_io_runtime, cluster_g->topo, uvGetConnectionPoolState, bc);
}
...
void MR_uvReplyClusterInfo(RedisModuleCtx *ctx) {
  RedisModuleBlockedClient *bc = RedisModule_BlockClient(ctx, NULL, NULL, NULL, 0);
  RedisModule_BlockedClientMeasureTimeStart(bc);
  IORuntimeCtx_Schedule(cluster_g->control_plane_io_runtime, cluster_g->topo, uvReplyClusterInfo, bc);
}
...
int MRIteratorCallback_ResendCommand(MRIteratorCallbackCtx *ctx) {
  IORuntimeCtx *io_runtime_ctx = IORuntimePool_GetCtx(cluster_g, ctx->it->ctx.ioRuntimeIdx);
  return MRCluster_SendCommand(cluster_g->topo, io_runtime_ctx, true, &ctx->cmd, mrIteratorRedisCB, ctx);
}
...
void iterStartCb(void *p) {
  MRIterator *it = p;
  IORuntimeCtx *io_runtime_ctx = IORuntimePool_GetCtx(cluster_g, it->ctx.ioRuntimeIdx);
  ...
  for (size_t i = 0; i < it->len; i++) {
    if (MRCluster_SendCommand(cluster_g->topo, io_runtime_ctx, true, &it->cbxs[i].cmd,
...
bool MR_ManuallyTriggerNextIfNeeded(MRIterator *it, size_t channelThreshold) {
  ...
  if (it->ctx.pending) {
    ...
    IORuntimeCtx_Schedule(IORuntimePool_GetCtx(cluster_g, it->ctx.ioRuntimeIdx), cluster_g->topo, iterManualNextCb, it);
...
MRIterator *MR_Iterate(const MRCommand *cmd, MRIteratorCallback cb) {
  ...
  *ret->cbxs = (MRIteratorCallbackCtx){
  ...
  IORuntimeCtx_Schedule(IORuntimePool_GetCtx(cluster_g, ret->ctx.ioRuntimeIdx), cluster_g->topo, iterStartCb, ret);
  return ret;
}
