...
static t_docId *geoRangeLoad(const GeoIndex *gi, const GeoFilter *gf, size_t *num) {
    *num = 0;
    ...
    for (size_t i = 0; i < sz; i++) {
        const char *s =
            RedisModule_CallReplyStringPtr(RedisModule_CallReplyArrayElement(rep, i), NULL);
        if (!s)
            continue;
...
IndexIterator *NewGeoRangeIterator(RedisSearchCtx *ctx, const GeoFilter *gf,
    ...
    if (gf->radius <= 0 || gf->lon > GEO_LONG_MAX || gf->lon < GEO_LONG_MIN ||
    ...
    for (size_t ii = 0; ii < GEO_RANGE_COUNT; ++ii) {
        if (ranges[ii].min != ranges[ii].max) {
            ...
            filt->geoFilter = gf;
            struct indexIterator *numIter =
                NewNumericFilterIterator(ctx, filt, csx, INDEXFLD_T_GEO, config);
            if (numIter != NULL) {
