...
int cmpAsc(const void *v1, const void *v2, const void *udata) {
    ...
    if (res1->num.value > res2->num.value)
        return 1;
    if (res1->num.value < res2->num.value)
        return -1;
...
static size_t OPT_Len(void *ctx) { return OPT_NumEstimated(ctx); }
...
static void OPT_Rewind(void *ctx) {
    ...
    if (successRatio < 0.01 || optIt->numIterations == 3) {
        ...
        size_t limitEstimate =
            QOptimizer_EstimateLimit(optIt->numDocs, optIt->childEstimate, resultsMissing);
        optIt->lastLimitEstimate = nf->limit = limitEstimate * successRatio;
    }

    // create new numeric filter
    optIt->numericIter = NewNumericFilterIterator(qOpt->sctx, qOpt->nf, qOpt->conc,
                                                  INDEXFLD_T_NUMERIC, optIt->config);
        ...
        while (1) {
            ...
            if (childRes->docId == numericRes->docId) {
                ...
                const RSDocumentMetadata *dmd =
                    DocTable_Borrow(&opt->sctx->spec->docs, childRes->docId);
                if (!dmd) {
                    continue;
                }
...
IndexIterator *NewOptimizerIterator(QOptimizer *qOpt, IndexIterator *root,
    ...
    oi->numericIter =
        NewNumericFilterIterator(qOpt->sctx, qOpt->nf, qOpt->conc, INDEXFLD_T_NUMERIC, config);
    if (!oi->numericIter) {
        oi->base.ctx = oi;
