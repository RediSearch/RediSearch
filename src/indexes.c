...
int Indexes_RdbLoad(RedisModuleIO *rdb, int encver, int when) {
  ...
  for (size_t i = 0; i < nIndexes; ++i) {
    ...
    IndexesStoreSpecResult storeResult =
        Indexes_StoreSpecAfterRdbLoad(RedisModule_GetContextFromIO(rdb), sp);
    if (storeResult != INDEXES_STORE_SPEC_OK) {
      if (storeResult == INDEXES_STORE_SPEC_LIMIT) {
        RedisModule_LogIOError(rdb, "warning",
                               "RDB Load: Max number of indexes reached for Flex indexes: %zu",
                               Indexes_Count());
      } else {
        RedisModule_LogIOError(
            rdb, "warning", "RDB Load: %s",
...
IndexesStoreSpecResult Indexes_StoreSpecAfterRdbLoad(RedisModuleCtx *ctx, IndexSpec *sp) {
  if (!sp) {
    addPendingIndexDrop();
    return INDEXES_STORE_SPEC_ERROR;
  }
  ...
  if (sp->isDuplicate) {
    ...
    IndexSpec_UnlinkLoaded(spec_ref);
    return INDEXES_STORE_SPEC_OK;
  }
  ...
  if (!SearchDisk_CheckLimitNumberOfIndexes(Indexes_Count() + 1)) {
    ...
    IndexSpec_UnlinkLoaded(spec_ref);
    return INDEXES_STORE_SPEC_LIMIT;
  }
