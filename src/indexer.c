...
static void writeCurEntries(DocumentIndexer *indexer, RSAddDocumentCtx *aCtx, RedisSearchCtx *ctx) {
    ...
    while (entry != NULL) {
        ...
        InvertedIndex *invidx =
            Redis_OpenInvertedIndexEx(ctx, entry->term, entry->len, 1, &isNew, &idxKey);
        ...
        if (spec->suffixMask & entry->fieldMask && entry->term[0] != STEM_PREFIX &&
            entry->term[0] != PHONETIC_PREFIX && entry->term[0] != SYNONYM_PREFIX_CHAR &&
            strlen(entry->term) != 0) {
...
static RSDocumentMetadata *makeDocumentId(RedisModuleCtx *ctx, RSAddDocumentCtx *aCtx,
                                          IndexSpec *spec, int replace, QueryError *status) {
            ...
            if (spec->flags & Index_HasVecSim) {
                for (int i = 0; i < spec->numFields; ++i) {
                    if (spec->fields[i].types == INDEXFLD_T_VECTOR) {
                        RedisModuleString *rmstr = RedisModule_CreateString(
                            RSDummyContext, spec->fields[i].name, strlen(spec->fields[i].name));
                        ...
                        // TODO: use VecSimReplace instead and if successful, do not insert and
                        // remove from doc
                    }
                }
            }
    ...
    RSDocumentMetadata *dmd = DocTable_Put(table, s, n, doc->score, aCtx->docFlags, doc->payload,
                                           doc->payloadSize, doc->type);
    if (dmd) {
        doc->docId = dmd->id;
        ++spec->stats.numDocuments;
    }
    ...
    for (RSAddDocumentCtx *cur = aCtx; cur && cur->doc->docId; cur = cur->next) {
        ...
        for (size_t ii = 0; ii < doc->numFields; ++ii) {
            ...
            if (IndexerBulkAdd(bulk, cur, sctx, doc->fields + ii, fs, fdata, &cur->status) != 0) {
                ...
                IndexError_AddError(&cur->spec->fields[fs->index].indexError, cur->status.detail,
                                    doc->docKey);
...
static void writeMissingFieldDocs(RSAddDocumentCtx *aCtx, RedisSearchCtx *sctx) {
    ...
    for (size_t i = 0; i < spec->numFields; i++) {
        ...
        if (!found_df) {
            ...
            RSIndexResult rec = {
                .type = RSResultType_Virtual, .docId = docId, .offsetsSz = 0, .freq = 0};
