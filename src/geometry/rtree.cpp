...
template <typename cs>
RTree<cs>::RTree()
    : allocated_{sizeof *this}, rtree_{{}, {}, {}, doc_alloc{allocated_}},
...
auto RTree<cs>::apply_intersection_of_predicates(Predicate predicate, Filter filter) const
    -> query_results {
...
template <typename cs>
template <typename Predicate, typename Filter>
auto RTree<cs>::apply_union_of_predicates(Predicate predicate, Filter filter) const
    -> query_results {
    return rtree_.qbegin(bgi::satisfies(
        [this, p = std::move(predicate), f = std::move(filter)](doc_type const &doc) -> bool {
...
auto RTree<cs>::query_begin(QueryType query_type, geom_type const &query_geom) const
    ...
    case QueryType::CONTAINS: // contains(g1, g2) == within(g2, g1)
        return apply_intersection_of_predicates(
            bgi::contains(query_mbr), [query_geom](auto const &geom) -> bool {
                return std::visit(within_filter<cs>, query_geom, geom);
            });
    case QueryType::WITHIN:
        return apply_intersection_of_predicates(
            bgi::within(query_mbr), [query_geom](auto const &geom) -> bool {
                return std::visit(within_filter<cs>, geom, query_geom);
            });
    case QueryType::DISJOINT: // disjoint(g1, g2) == !intersects(g1, g2)
        return apply_union_of_predicates(
            [query_mbr](auto const &mbr) -> bool { return !intersects_filter<cs>(mbr, query_mbr); },
            [query_geom](auto const &geom) -> bool {
                return std::visit(std::not_fn(intersects_filter<cs>), geom, query_geom);
            });
    case QueryType::INTERSECTS:
        return apply_intersection_of_predicates(
            bgi::intersects(query_mbr), [query_geom](auto const &geom) -> bool {
