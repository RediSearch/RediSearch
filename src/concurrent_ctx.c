...
int ConcurrentSearch_CreatePool(int numThreads) {
    if (!threadpools_g) {
        threadpools_g =
            array_new(redisearch_thpool_t *, 1); // Only used by the coordinator, so 1 is enough
    }
    int poolId = array_len(threadpools_g);
    array_append(threadpools_g,
                 redisearch_thpool_create(numThreads, DEFAULT_HIGH_PRIORITY_BIAS_THRESHOLD,
                                          LogCallback, "coord"));
...
void ConcurrentCmdCtx_KeepRedisCtx(ConcurrentCmdCtx *cctx) { cctx->options |= CMDCTX_KEEP_RCTX; }
...
void ConcurrentSearch_AddKey(ConcurrentSearchCtx *ctx, ConcurrentReopenCallback cb, void *privdata,
    ...
    ctx->openKeys[ctx->numOpenKeys - 1] =
        (ConcurrentKeyCtx){.cb = cb, .privdata = privdata, .freePrivData = freePrivDataCallback};
}
