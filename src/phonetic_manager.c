...
static void PhoneticManager_AddPrefix(char** phoneticTerm, alloc_context *actx) {
  if (!phoneticTerm || !(*phoneticTerm)) {
    return;
  }
  size_t len = strlen(*phoneticTerm) + 1;
  *phoneticTerm = rm_realloc(actx, *phoneticTerm, sizeof(char*) * (len + 1));
...
void PhoneticManager_ExpandPhonetics(PhoneticManagerCtx* ctx, const char* term, size_t len,
  ...
  DoubleMetaphone(bufTmp, primary, secondary, actx);
  PhoneticManager_AddPrefix(primary, actx);
  PhoneticManager_AddPrefix(secondary, actx);
}
