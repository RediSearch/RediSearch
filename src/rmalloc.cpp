...
struct Manager {
  ...
  // ~Manager() {
  //   for (auto const [pi, src] : collection) {
  //     auto ptr = reinterpret_cast<const void*>(pi);
...
void rm_log_impl(void *p, const char *file, const char *fn, std::size_t line) {
  manager().insert(p, file, fn, line);
}
