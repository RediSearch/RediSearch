...
// #define VECTOR_SCORE(p) (p->data.tag == RSResultData_Metric ? IndexResult_NumValue(p) :
// IndexResult_NumValue(AggregateResult_GetUnchecked(IndexResult_AggregateRefUnchecked(p), 0)))
...
// static void insertResultToHeap_Metric(HybridIterator *hr, RSIndexResult *child_res, RSIndexResult
// **vec_res, double *upper_bound) {

//   RSYieldableMetric_Concat(&(*vec_res)->metrics, &child_res->metrics); // Pass child metrics, if
//   there are any ResultMetrics_Add(*vec_res, hr->ownKey, IndexResult_NumValue(*vec_res));
...
//   res->data.hybridmetric.tag = RSAggregateResult_Owned; // Mark as copy, so when we free it, it
//   will also free its children. ResultMetrics_Add(res, hr->ownKey, IndexResult_NumValue(vec_res));
...
//       if (hr->topResults->count < hr->query.k || IndexResult_NumValue(cur_vec_res) <
//       *upper_bound) {
...
// static inline void computeDistances_Disk_Cleanup(VecSimAdhocBfCtx *ctx, RSIndexResult
// *cur_vec_res) {
...
// static bool reviewHybridSearchPolicy(HybridIterator *hr, size_t n_res_left, size_t
// child_upper_bound,
...
//   hr->runtimeParams.batchSize) {
//     return false;
//   }
//   // Re-evaluate the child num estimated results and the hybrid policy based on the current
//   batch. size_t new_results_cur_batch = (hr->topResults->count) - (hr->query.k - n_res_left);
...
//     hr->reply = VecSimIndex_TopKQuery(hr->index, hr->query.vector, hr->query.k,
//     &(hr->runtimeParams), hr->query.order); hr->iter = VecSimQueryReply_GetIterator(hr->reply);
...
//   &hr->runtimeParams); double upper_bound = INFINITY; VecSimQueryReply_Code code =
//   VecSim_QueryReply_OK; size_t child_num_estimated = hr->child->NumEstimated(hr->child);
...
//                                                  hr->filterCtx.predicate,
//                                                  &hr->sctx->time.current)) {
...
// // If we already have the results prepared, we are OK, and if not, we didn't execute the query
// yet so we are also OK.
...
//   RS_ASSERT(hParams.qParams.searchMode >= 0 && hParams.qParams.searchMode <
//   VECSIM_LAST_SEARCHMODE); QueryIterator* ri = HybridIteratorReducer(&hParams); if (ri) {
//     return ri;
//   }
...
//   hi->timeoutCtx = (TimeoutCtx){ .timeout = hParams.timeout, .counter =
//   hParams.sctx->time.skipTimeoutChecks ? REDISEARCH_UNINITIALIZED : 0 };
...
//     // If there is no child iterator, or the query is going to return 0 results, we can use
//     simple KNN. hi->searchMode = VECSIM_STANDARD_KNN;
//   } else {
...
//       }
//     }
//     hi->topResults = mmh_init_with_size(hParams.query.k, cmpVecSimResByScore, NULL,
//     (mmh_free_func)IndexResult_Free);
//   }
...
//   } else {
//     // Hybrid query - save the RSIndexResult subtree which is not the vector distance only if
//     required. ri->Read = HR_ReadHybridUnsorted;
//   }
//   return ri;
// }
