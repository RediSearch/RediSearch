...
static IteratorStatus HR_ReadHybridUnsortedSingle(HybridIterator *hr) {
  ...
  if (hr->checkFieldExpiration
      && !DocTable_CheckFieldExpirationPredicate(&hr->sctx->spec->docs, hr->base.current->docId,
                                                 hr->filterCtx.field.value.index,
...
static ValidateStatus HR_Revalidate(QueryIterator *ctx) {
  ...
  hr->checkFieldExpiration = hr->sctx && hr->filterCtx.field.value.index != RS_INVALID_FIELD_INDEX &&
...
QueryIterator *NewHybridVectorIterator(HybridIteratorParams hParams, QueryError *status) {
  ...
  hi->checkFieldExpiration =
      hParams.sctx && hParams.filterCtx->field.value.index != RS_INVALID_FIELD_INDEX &&
