...
/* Free it */
void _ovi_free(void *ctx) { mempool_release(pthread_getspecific(__offsetIters), ctx); }

void *newOffsetIterator() { return rm_malloc(sizeof(_RSOffsetVectorIterator)); }
...
RSOffsetIterator RSOffsetVector_Iterate(const RSOffsetVector *v, RSQueryTerm *t) {
    ...
    if (!pool) {
        mempool_options options = {.initialCap = 8, .alloc = newOffsetIterator, .free = rm_free};
    ...
    return (RSOffsetIterator){
        .Next = _ovi_Next, .Rewind = _ovi_Rewind, .Free = _ovi_free, .ctx = it};
}
...
static RSOffsetIterator _aggregateResult_iterate(const RSAggregateResult *agg) {
    ...
    if (!pool) {
        mempool_options opts = {.initialCap = 8, .alloc = aggiterNew, .free = aggiterFree};
    ...
    return (RSOffsetIterator){
        .Next = _aoi_Next, .Rewind = _aoi_Rewind, .Free = _aoi_Free, .ctx = it};
}
uint32_t _empty_Next(void *ctx, RSQueryTerm **t) { return RS_OFFSETVECTOR_EOF; }
...
void _ovi_Free(void *ctx) { rm_free(ctx); }
