...
static void taskCallback(void *data) {
    ...
    if (ret) { // The common case
        ...
        RedisModule_Log(RSDummyContext, REDISMODULE_LOGLEVEL_VERBOSE,
                        "GC %p: Self-Terminating. Index was freed.", gc);
...
void GCContext_ForceBGInvoke(GCContext *gc) { GCContext_CommonForceInvoke(gc, NULL); }
...
void GC_ThreadPoolStart() {
    if (gcThreadpool_g == NULL) {
        gcThreadpool_g = redisearch_thpool_create(
            GC_THREAD_POOL_SIZE, DEFAULT_HIGH_PRIORITY_BIAS_THRESHOLD, LogCallback, "gc");
    }
}
