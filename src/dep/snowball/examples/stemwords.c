  ...
  while (1) {
    ...
    }
    {
      ...
      while (1) {
        ...
        if (i == lim) {
          sb_symbol *newb;
          newb = (sb_symbol *)rm_realloc(b, (lim + INC) * sizeof(sb_symbol));
        ...
        if (stemmed == NULL) {
          ...
          if (pretty == 1) {
            ...
            if (sb_stemmer_length(stemmer) > 0) {
              int j;
              if (inlen < 30) {
                for (j = 30 - inlen; j > 0; j--) fputs(" ", f_out);
              } else {
                fputs("\n", f_out);
                for (j = 30; j > 0; j--) fputs(" ", f_out);
              }
            }
          }
...
static void usage(int n) {
  printf(
      "usage: %s [-l <language>] [-i <input file>] [-o <output file>] [-c <character encoding>] "
      "[-p[2]] [-h]\n"
      "\n"
