...
void FieldsGlobalStats_UpdateStats(FieldSpec *fs, int toAdd) {
    if (fs->types & INDEXFLD_T_FULLTEXT) { // text field
        ...
        if (fs->vectorOpts.vecSimParams.algo == VecSimAlgo_BF)
            ...
            if (fs->vectorOpts.vecSimParams.algoParams.tieredParams.primaryIndexParams->algo ==
                VecSimAlgo_HNSWLIB)
    ...
    if (fs->options & FieldSpec_Sortable) {
        if (fs->types & INDEXFLD_T_FULLTEXT)
            RSGlobalConfig.fieldsStats.numTextFieldsSortable += toAdd;
        else if (fs->types & INDEXFLD_T_NUMERIC)
            RSGlobalConfig.fieldsStats.numNumericFieldsSortable += toAdd;
        else if (fs->types & INDEXFLD_T_GEO)
            RSGlobalConfig.fieldsStats.numGeoFieldsSortable += toAdd;
        else if (fs->types & INDEXFLD_T_TAG)
            RSGlobalConfig.fieldsStats.numTagFieldsSortable += toAdd;
        else if (fs->types & INDEXFLD_T_GEOMETRY)
            RSGlobalConfig.fieldsStats.numGeometryFieldsSortable += toAdd;
    }
    if (fs->options & FieldSpec_NotIndexable) {
        if (fs->types & INDEXFLD_T_FULLTEXT)
            RSGlobalConfig.fieldsStats.numTextFieldsNoIndex += toAdd;
        else if (fs->types & INDEXFLD_T_NUMERIC)
            RSGlobalConfig.fieldsStats.numNumericFieldsNoIndex += toAdd;
        else if (fs->types & INDEXFLD_T_GEO)
            RSGlobalConfig.fieldsStats.numGeoFieldsNoIndex += toAdd;
        else if (fs->types & INDEXFLD_T_TAG)
            RSGlobalConfig.fieldsStats.numTagFieldsNoIndex += toAdd;
        else if (fs->types & INDEXFLD_T_GEOMETRY)
            RSGlobalConfig.fieldsStats.numGeometryFieldsNoIndex += toAdd;
    }
}
...
void FieldsGlobalStats_AddToInfo(RedisModuleInfoCtx *ctx) {
    ...
    if (RSGlobalConfig.fieldsStats.numGeoFields > 0) {
        ...
        if (RSGlobalConfig.fieldsStats.numGeoFieldsSortable > 0)
            RedisModule_InfoAddFieldLongLong(ctx, "Sortable",
                                             RSGlobalConfig.fieldsStats.numGeoFieldsSortable);
        if (RSGlobalConfig.fieldsStats.numGeoFieldsNoIndex > 0)
            RedisModule_InfoAddFieldLongLong(ctx, "NoIndex",
                                             RSGlobalConfig.fieldsStats.numGeoFieldsNoIndex);
        RedisModule_InfoEndDictField(ctx);
    }
    ...
    if (RSGlobalConfig.fieldsStats.numVectorFields > 0) {
        ...
        if (RSGlobalConfig.fieldsStats.numVectorFieldsFlat > 0)
            RedisModule_InfoAddFieldLongLong(ctx, "Flat",
                                             RSGlobalConfig.fieldsStats.numVectorFieldsFlat);
        if (RSGlobalConfig.fieldsStats.numVectorFieldsHNSW > 0)
            RedisModule_InfoAddFieldLongLong(ctx, "HNSW",
                                             RSGlobalConfig.fieldsStats.numVectorFieldsHNSW);
        RedisModule_InfoEndDictField(ctx);
    }

    if (RSGlobalConfig.fieldsStats.numGeometryFields > 0) {
        RedisModule_InfoBeginDictField(ctx, "fields_geoshape");
        RedisModule_InfoAddFieldLongLong(ctx, "Geoshape",
                                         RSGlobalConfig.fieldsStats.numGeometryFields);
        if (RSGlobalConfig.fieldsStats.numGeometryFieldsSortable > 0)
            RedisModule_InfoAddFieldLongLong(ctx, "Sortable",
                                             RSGlobalConfig.fieldsStats.numGeometryFieldsSortable);
        if (RSGlobalConfig.fieldsStats.numGeometryFieldsNoIndex > 0)
            RedisModule_InfoAddFieldLongLong(ctx, "NoIndex",
                                             RSGlobalConfig.fieldsStats.numGeometryFieldsNoIndex);
        RedisModule_InfoEndDictField(ctx);
    }
}
