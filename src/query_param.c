...
bool QueryParam_SetParam(QueryParseCtx *q, Param *target_param, void *target_value,
    ...
    case QT_TERM:
        ...
        *(char **)target_value = rm_strdupcase(source->s, source->len);
        if (target_len)
            *target_len = strlen(target_value);
        return false; // done
...
int QueryParam_Resolve(Param *param, dict *params, QueryError *status) {
    ...
    case PARAM_GEO_COORD:
        if (!ParseDouble(val, (double *)param->target, param->sign)) {
            QueryError_SetErrorFmt(status, QUERY_ESYNTAX,
                                   "Invalid numeric value (%s) for parameter `%s`", val,
                                   param->name);
            return -1;
        }
        return 1;
    ...
    case PARAM_NUMERIC_MAX_RANGE: {
        ...
        if (parseDoubleRange(val, &inclusive, (double *)param->target, isMin, param->sign,
...
int parseParams(dict **destParams, ArgsCursor *ac, QueryError *status) {
    ...
    if (*destParams) {
        QueryError_SetError(status, QUERY_EADDARGS,
                            "Multiple PARAMS are not allowed. Parameters can be defined only once");
        return REDISMODULE_ERR;
    }
    if (paramsArgs.argc == 0 || paramsArgs.argc % 2) {
        QueryError_SetError(status, QUERY_EADDARGS,
                            "Parameters must be specified in PARAM VALUE pairs");
        return REDISMODULE_ERR;
    }
