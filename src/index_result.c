...
void Term_Offset_Data_Free(RSTermRecord *tr) {
  RSOffsetVector_FreeData(&tr->owned.offsets);
}
  ...
  *ret = *src;
  ...
  switch (src->data.tag) {
    ...
    case RSResultData_Term:
      ...
      RSOffsetVector_CopyData(&IndexResult_TermRefMut(ret)->owned.offsets, &IndexResult_TermRef(src)->borrowed.offsets);
...
int RSIndexResult_HasOffsets(const RSIndexResult *res) {
  switch (res->data.tag) {
    case RSResultData_Term:
      return RSOffsetVector_Len(&IndexResult_TermRef(res)->borrowed.offsets) > 0;
    case RSResultData_Intersection:
    case RSResultData_Union:
    {
...
void IndexResult_Free(RSIndexResult *r) {
  ...
  if (r->data.tag == RSResultData_Intersection || r->data.tag == RSResultData_Union || r->data.tag == RSResultData_HybridMetric) {
    ...
    if (r->data.term.tag == RSTermRecord_Owned) {
      RSOffsetVector_FreeData(&IndexResult_TermRefMut(r)->owned.offsets);
...
void result_GetMatchedTerms(RSIndexResult *r, RSQueryTerm *arr[], size_t cap, size_t *len) {
  ...
  switch (r->data.tag) {
    ...
    case RSResultData_Term:
    {
      ...
      if (term->borrowed.term) {
        const char *s = term->borrowed.term->str;
        // make sure we have a term string and it's not an expansion
        if (s) {
          arr[(*len)++] = term->borrowed.term;
        }
      }
    }
    default:
