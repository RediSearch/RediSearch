 ...
 * Stores results in the request sync context so serializeAndReplyResults can be called
...
static void AREQ_StoreResults(AREQ *req, SearchResult **results, int rc, cachedVars cv, size_t limit) {
  ...
  QueryError_CloneFrom(qctx->err, &stored->err);
...
// Drain any queued partial results into the request sync context on the main
...
static int QueryReplyCallback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (!stored->hasStoredResults) {
    ...
    if (QueryError_HasError(&stored->err)) {
      QueryErrorsGlobalStats_UpdateError(QueryError_GetCode(&stored->err), 1, !IsInternal(req));
...
int AREQ_StartCursor(AREQ *r, RedisModule_Reply *reply, StrongRef spec_ref, QueryError *err, bool coord) {
  ...
  RS_ASSERT(!Cursor_IsHybrid(cursor)); // assuming hybrid cursors don't reach here
...
static QueryProcessingCtx *prepareForCursorRead(Cursor *cursor, bool *hasLoader, bool *initClock, QEFlags *reqFlags, QueryError *status) {
  ...
  if (req) {
    ...
    *initClock = IsProfile(req) || !IsInternal(req);
    ...
    // HybridRequest_StartSingleCursor (no AREQ, hybrid query set),
    ...
    // always carry an AREQ and take the if branch above.
    HybridRequest *hreq = Cursor_GetHybridRequest(cursor);
...
static void cursorRead(RedisModuleCtx *ctx, Cursor *cursor, size_t count, bool bg) {
  ...
  AREQ *req = Cursor_GetAREQ(cursor);
  RS_LOG_ASSERT(req, "cursorRead reached without an AREQ");
...
static inline int coordCursorReadReturnStrict(RedisModuleCtx *ctx, RequestSyncCtx *reqCtx,
  ...
  if (!cursor) {
    ...
    RequestSyncCtx_ReplyOrStoreError(reqCtx, ctx, &err);
    return REDISMODULE_OK;
  }
  AREQ *req = Cursor_GetAREQ(cursor);
  AREQ_ResetForCursorReadReturnStrict(req);
  RequestSyncCtx_BindAREQ(reqCtx, req);
  cursor->query = reqCtx;
  RequestSyncCtx_UnlockSetRequest(reqCtx);
...
int RSCursorReadCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  // Coord+FAIL is the only path that attaches a RequestSyncCtx as privdata
  ...
  if (reqCtx && RequestSyncCtx_IsCursorReadReturnStrict(reqCtx)) {
  ...
  if (RunInThread(ctx) && !upstreamBC) {
    ...
    //   (1) Coord+FAIL: upstreamBC != NULL, privdata is a RequestSyncCtx.
    ...
    if (reqCtx) {
      ...
      RequestSyncCtx_BindAREQ(reqCtx, Cursor_GetAREQ(cursor));
      cursor->query = reqCtx;
      RequestSyncCtx_UnlockSetRequest(reqCtx);
    }
    cursorRead(ctx, cursor, count, false);
