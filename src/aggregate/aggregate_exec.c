...
int RSCursorReadCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (!cursorMatchesSelectedDb(ctx, cursor)) {
    ...
    // untouched and report it as not found, before any dispatch decision. A
    // cross-DB mismatch can only happen on the standalone path (cluster
    // cursors are all on DB 0).
    Cursor_Pause(cursor);
    return RedisModule_ReplyWithErrorFormat(ctx, "Cursor not found, id: %lld", cid);
  }
