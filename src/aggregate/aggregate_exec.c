...
static size_t serializeResult(AREQ *req, RedisModule_Reply *reply, const SearchResult *r,
    ...
    if (options & QEXEC_F_REQUIRED_FIELDS) {
        ...
        // Sortkey is the first key to reply on the required fields, if we already replied it,
        ...
        for (; currentField < requiredFieldsCount; currentField++) {
            const RLookupKey *rlk = RLookup_GetKey(cv->lastLk, req->requiredFields[currentField],
                                                   RLOOKUP_M_READ, RLOOKUP_F_NOFLAGS);
            ...
            RSValue rsv;
            if (rlk && (rlk->flags & RLOOKUP_T_NUMERIC) && v && v->t != RSVALTYPE_DOUBLE &&
                !RSValue_IsNull(v)) {
                double d;
    ...
    if (!(options & QEXEC_F_SEND_NOFIELDS)) {
        ...
        if (r->flags & Result_ExpiredDoc) {
            ...
            size_t nfields = RLookup_GetLength(lk, &r->rowdata, skipFieldIndex, requiredFlags,
                                               excludeFlags, rule);
...
static bool ShouldReplyWithTimeoutError(int rc, AREQ *req) {
    return rc == RS_RESULT_TIMEDOUT && req->reqConfig.timeoutPolicy == TimeoutPolicy_Fail &&
           !IsProfile(req);
}
...
void startPipeline(AREQ *req, ResultProcessor *rp, SearchResult ***results, SearchResult *r,
                   int *rc) {
...
long calc_results_len(AREQ *req, size_t limit) {
    ...
    size_t expected_res = reqLimit + reqOffset <= req->maxSearchResults
                              ? req->qiter.totalResults
                              : MIN(req->maxSearchResults, req->qiter.totalResults);
...
static bool hasTimeoutError(QueryError *err) { return QueryError_GetCode(err) == QUERY_ETIMEDOUT; }
...
done_2:
    ...
    cursor_done = (rc != RS_RESULT_OK && !(rc == RS_RESULT_TIMEDOUT &&
...
void sendChunk(AREQ *req, RedisModule_Reply *reply, size_t limit) {
    ...
    cachedVars cv = {.lastLk = AGPLN_GetLookup(&req->ap, NULL, AGPLN_GETLOOKUP_LAST),
...
static blockedClientReqCtx *
blockedClientReqCtx_New(AREQ *req, RedisModuleBlockedClient *blockedClient, StrongRef spec) {
...
void AREQ_Execute_Callback(blockedClientReqCtx *BCRctx) {
    ...
    if (!StrongRef_Get(execution_ref)) {
        ...
        QueryError_SetError(&status, QUERY_ENOINDEX,
    ...
    // Either way, we don't want to free `req` here. we set it to NULL so that it won't be freed
    // with the context.
...
cleanup:
    // No need to unlock spec as it was unlocked by `AREQ_Execute` or will be unlocked by
    // `blockedClientReqCtx_destroy`
    RedisModule_FreeThreadSafeContext(outctx);
...
static int parseProfile(AREQ *r, int withProfile, RedisModuleString **argv, int argc,
                        QueryError *status) {
    if (withProfile != NO_PROFILE) {

// WithCursor is disabled on the shards for external use but is available internally to the
// coordinator
...
static int execCommandCommon(RedisModuleCtx *ctx, RedisModuleString **argv, int argc,
    ...
    if (RunInThread()) {
        ...
        // Mark the request as thread safe, so that the pipeline will be built in a thread safe
        // manner
    ...
    if (strcasecmp(cmd, "READ") == 0) {
        ...
        if (argc > 5) {
            ...
            if (RedisModule_StringToLongLong(argv[5], &count) != REDISMODULE_OK) {
                RedisModule_ReplyWithErrorFormat(ctx, "Bad value for COUNT: `%s`",
                                                 RedisModule_StringPtrLen(argv[5], NULL));
                RedisModule_EndReply(reply);
                return REDISMODULE_OK;
            }
        }
