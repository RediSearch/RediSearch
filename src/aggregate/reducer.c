...
int ReducerOpts_GetKey(const ReducerOptions *options, const RLookupKey **out) {
  ...
  *out = RLookup_GetKey_Read(options->srclookup, keyName, RLOOKUPKEYFLAG_HIDDEN);
  if (!*out) {
    if (options->loadKeys) {
      *out = RLookup_GetKey_Load(options->srclookup, keyName, keyName, RLOOKUPKEYFLAG_HIDDEN);
    ...
    if (!options->loadKeys || !((*out)->flags & RLOOKUPKEYFLAG_SCHEMASRC)) {
