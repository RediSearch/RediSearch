...
typedef struct AREQ {
  ...
  */
  ...
  // Selected per dispatch for shard/standalone FAIL workers. The worker encodes
  // the reply itself, and free-data cleanup finalizes the pending cursor.
...
/**
 * Free-data callback for the query blocked client (main thread only).
 * Pauses or frees any cursor pending in storedReplyState, then releases the
 * blocked client's AREQ reference.
 */
