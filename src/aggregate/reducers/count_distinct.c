...
static void *distinctNewInstance(Reducer *r) {
    BlkAlloc *ba = &r->alloc;
    distinctCounter *ctr = BlkAlloc_Alloc(
        ba, sizeof(*ctr), INSTANCE_BLOCK_NUM * sizeof(*ctr)); // malloc(sizeof(*ctr));
    ctr->count = 0;
...
Reducer *RDCRHLL_New(const ReducerOptions *options) { return newHllCommon(options, 1); }
