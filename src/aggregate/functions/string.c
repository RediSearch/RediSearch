...
int func_to_number(RSFunctionEvalCtx *ctx, RSValue *result, RSValue *argv, int argc,
                   char **err) {
  ...
  if (!RSValue_ToNumber(&argv[0], &n)) {
    ...
    FMT_ERR(err, "to_number: cannot convert string '%s'", p);
    return EXPR_EVAL_ERR;
  }
...
int func_to_str(RSFunctionEvalCtx *ctx, RSValue *result, RSValue *argv, int argc,
                char **err) {
...
int func_is_null(RSFunctionEvalCtx *ctx, RSValue *result, RSValue *argv, int argc,
                            char **err) {
  VALIDATE_ARGS("isnull", 1, 1, err);

  result->t = RSValue_Number;
  result->numval = !RSValue_BoolTest(argv);
  return EXPR_EVAL_OK;
}
