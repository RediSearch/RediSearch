...
static int handleCommonArgs(AREQ *req, ArgsCursor *ac, QueryError *status, int allowLegacy) {
  ...
  if (AC_AdvanceIfMatch(ac, "LIMIT")) {
    ...
    AREQ_AddRequestFlags(req, QEXEC_F_HAS_SORTBY);
    ...
    if (array_len(arng->sortKeys) == 0) {
      // No need to sort
      AREQ_RemoveRequestFlags(req, QEXEC_F_HAS_SORTBY);
      if (!existingSort) {
        if (arng->limit > 0) {
...
int AREQ_Compile(AREQ *req, RedisModuleString **argv, int argc, QueryError *status) {
  ...
  while (!AC_IsAtEnd(&ac)) {
    ...
    if (AC_AdvanceIfMatch(&ac, "GROUPBY")) {
      ...
      AREQ_AddRequestFlags(req, QEXEC_F_HAS_GROUPBY);
...
int AREQ_BuildPipelineWithGroupByLimits(AREQ *req, GroupByLimits groupByLimits,
  ...
  if (!hasArrange &&
      (IsSearch(req) || (IsAggregate(req) && HasDepleter(req)))) {
