...
static int ensureExtendedMode(AREQ *areq, const char *name, QueryError *status) {
    if (areq->reqflags & QEXEC_F_IS_SEARCH) {
        QueryError_SetErrorFmt(
            status, QUERY_EINVAL,
            "option `%s` is mutually exclusive with simple (i.e. search) options", name);
        return 0;
    }
...
static int parseRequiredFields(AREQ *req, ArgsCursor *ac, QueryError *status) {
    ...
    // This array contains shallow copy of the required fields names. Those copies are to use only
    // for lookup. If we need to use them in reply we should make a copy of those strings.
...
int SetValueFormat(bool is_resp3, bool is_json, uint32_t *flags, QueryError *status) {
    ...
    if (*flags & QEXEC_FORMAT_EXPAND) {
        if (!is_resp3) {
            QueryError_SetError(status, QUERY_EBADVAL,
                                "EXPAND format is only supported with RESP3");
            return REDISMODULE_ERR;
        }
        if (!is_json) {
            QueryError_SetErrorFmt(status, QUERY_EBADVAL,
                                   "EXPAND format is only supported with JSON");
            return REDISMODULE_ERR;
        }
        if (japi_ver < 4) {
            QueryError_SetError(
                status, QUERY_EBADVAL,
                "EXPAND format requires a newer RedisJSON (with API version RedisJSON_V4)");
            return REDISMODULE_ERR;
        }
    }
    return REDISMODULE_OK;
...
static int handleCommonArgs(AREQ *req, ArgsCursor *ac, QueryError *status, int allowLegacy) {
    ...
    if (AC_AdvanceIfMatch(ac, "LIMIT")) {
        ...
        if (arng->isLimited && arng->limit == 0) {
            ...
            //(SEARCH / AGGREGATE)
        } else if ((arng->limit > req->maxSearchResults) && (req->reqflags & QEXEC_F_IS_SEARCH)) {
    ...
    if (dialect_specified && req->reqConfig.dialectVersion < APIVERSION_RETURN_MULTI_CMP_FIRST &&
        req->reqflags & QEXEC_FORMAT_EXPAND) {
        QueryError_SetErrorFmt(status, QUERY_ELIMIT, "EXPAND format requires dialect %u or greater",
                               APIVERSION_RETURN_MULTI_CMP_FIRST);
        return ARG_ERROR;
    }
        ...
        while (!AC_IsAtEnd(&subArgs)) {
            ...
            if (*s == '@') {
                if (array_len(keys) >= SORTASCMAP_MAXFIELDS) {
                    QERR_MKBADARGS_FMT(status, "Cannot sort by more than %lu fields",
                                       SORTASCMAP_MAXFIELDS);
                    goto err;
                }
                s++;
                array_append(keys, s);
...
static int parseQueryArgs(ArgsCursor *ac, AREQ *req, RSSearchOptions *searchOpts,
    ...
    if (!optimization_specified && req->reqConfig.dialectVersion >= 4) {
        // If optimize was not enabled/disabled explicitly, enable it by default starting with
        // dialect 4
        req->reqflags |= QEXEC_OPTIMIZE;
    }
    ...
    if (AC_IsInitialized(&returnFields)) {
        ...
        while (!AC_IsAtEnd(&returnFields)) {
            ...
            if (AC_AdvanceIfMatch(&returnFields, SPEC_AS_STR)) {
                ...
                if (rv != AC_OK) {
                    QERR_MKBADARGS_FMT(status,
                                       "RETURN path AS name - must be accompanied with NAME");
                    return REDISMODULE_ERR;
...
static RLookup *groupStepGetLookup(PLN_BaseStep *bstp) { return &((PLN_GroupStep *)bstp)->lookup; }
...
PLN_Reducer *PLNGroupStep_FindReducer(PLN_GroupStep *gstp, const char *name, ArgsCursor *ac) {
    ...
    for (size_t ii = 0; ii < nreducers; ++ii) {
        ...
        if (nvars == AC_NumArgs(&gr->args) && !strcasecmp(gr->name, name) &&
            AC_Equals(ac, &gr->args)) {
            return gr;
        }
    }
    return NULL;
...
static void genericStepFree(PLN_BaseStep *p) { rm_free(p); }
...
static int parseGroupby(AREQ *req, ArgsCursor *ac, QueryError *status) {
    ...
    for (size_t ii = 0; ii < groupArgs.argc; ++ii) {
        if (*(char *)groupArgs.objs[ii] != '@') {
            QERR_MKBADARGS_FMT(
                status, "Bad arguments for GROUPBY: Unknown property `%s`. Did you mean `@%s`?",
...
static void applyGlobalFilters(RSSearchOptions *opts, QueryAST *ast, const RedisSearchCtx *sctx) {
    ...
    if (opts->legacy.filters) {
        ...
        array_clear(opts->legacy.filters); // so AREQ_Free() doesn't free the filters themselves,
                                           // which are now owned by the query object
    }
    if (opts->legacy.gf) {
    ...
    if (opts->inkeys) {
        ...
        for (size_t ii = 0; ii < opts->ninkeys; ++ii) {
            t_docId did =
                DocTable_GetId(&sctx->spec->docs, opts->inkeys[ii], strlen(opts->inkeys[ii]));
            if (did) {
...
int AREQ_ApplyContext(AREQ *req, RedisSearchCtx *sctx, QueryError *status) {
    ...
    int rv = QAST_Parse(ast, sctx, opts, req->query, strlen(req->query),
                        req->reqConfig.dialectVersion, status);
...
static ResultProcessor *buildGroupRP(PLN_GroupStep *gstp, RLookup *srclookup,
    ...
    for (size_t ii = 0; ii < gstp->nproperties; ++ii) {
        ...
        srckeys[ii] =
            RLookup_GetKeyEx(srclookup, fldname, fldname_len, RLOOKUP_M_READ, RLOOKUP_F_NOFLAGS);
        if (!srckeys[ii]) {
            if (loadKeys) {
                // We faild to get the key for reading, so we know getting it for loading will
                // succeed.
                srckeys[ii] = RLookup_GetKey_LoadEx(srclookup, fldname, fldname_len, fldname,
                                                    RLOOKUP_F_NOFLAGS);
        ...
        dstkeys[ii] = RLookup_GetKeyEx(&gstp->lookup, fldname, fldname_len, RLOOKUP_M_WRITE,
                                       RLOOKUP_F_NOFLAGS);
        if (!dstkeys[ii]) {
            QueryError_SetErrorFmt(err, QUERY_EDUPFIELD, "Property `%s` specified more than once",
                                   fldname);
            return NULL;
        }
    }
    ...
    for (size_t ii = 0; ii < nreducers; ++ii) {
        ...
        // Adding the reducer before validating the key, so we free the reducer if the key is
        // invalid
...
static ResultProcessor *getGroupRP(AREQ *req, PLN_GroupStep *gstp, ResultProcessor *rpUpstream,
    ...
    RLookup *firstLk = AGPLN_GetLookup(
        pln, &gstp->base, AGPLN_GETLOOKUP_FIRST); // first lookup can load fields from redis
    const RLookupKey **loadKeys = NULL;
    ResultProcessor *groupRP = buildGroupRP(
        gstp, lookup, (firstLk == lookup && firstLk->spcache) ? &loadKeys : NULL, status);
    ...
    if (loadKeys) {
        ResultProcessor *rpLoader =
            RPLoader_New(req, firstLk, loadKeys, array_len(loadKeys), forceLoad);
        array_free(loadKeys);
...
static ResultProcessor *getAdditionalMetricsRP(AREQ *req, RLookup *rl, QueryError *status) {
    ...
    for (size_t i = 0; i < array_len(requests); i++) {
        ...
        if (IndexSpec_GetField(req->sctx->spec, name, name_len)) {
            QueryError_SetErrorFmt(status, QUERY_EINDEXEXISTS,
                                   "Property `%s` already exists in schema", name);
            return NULL;
        }
        RLookupKey *key = RLookup_GetKeyEx(rl, name, name_len, RLOOKUP_M_WRITE, RLOOKUP_F_NOFLAGS);
        if (!key) {
            QueryError_SetErrorFmt(status, QUERY_EDUPFIELD,
                                   "Property `%s` specified more than once", name);
            return NULL;
        }

        // In some cases the iterator that requested the additional field can be NULL (if some other
        // iterator knows early that it has no results), but we still want the rest of the pipline
        // to know about the additional field name, because there is no syntax error and the sorter
        // should be able to "sort" by this field. If there is a pointer to the node's RLookupKey,
        // write the address.
    ...
    if (req->optimizer->type != Q_OPT_NO_SORTER) {
        if (astp->sortKeys) {
                ...
                if (!sortkey) {
                    ...
                    if (!(sortkey->flags & RLOOKUP_F_SCHEMASRC)) {
                        QueryError_SetErrorFmt(status, QUERY_ENOPROPKEY,
                                               "Property `%s` not loaded nor in schema", keystr);
                        goto end;
                    }
            ...
            if (loadKeys) {
                ...
                ResultProcessor *rpLoader =
                    RPLoader_New(req, lk, loadKeys, array_len(loadKeys), forceLoad);
...
static void buildImplicitPipeline(AREQ *req, QueryError *Status) {
    ...
    if (IsScorerNeeded(req) || (IsSearch(req) && !IsCount(req) &&
    ...
    if (req->reqflags & QEXEC_F_SEND_HIGHLIGHT) {
        ...
        for (size_t ii = 0; ii < req->outFields.numFields; ++ii) {
            ...
            if (!kk) {
                ...
                QueryError_SetErrorFmt(status, QUERY_EINVAL, "Property `%s` is not in schema",
                                       ff->name);
                goto error;
            }
            ff->lookupKey = kk;
...
int AREQ_BuildPipeline(AREQ *req, QueryError *status) {
    ...
    // If we have a JSON spec, and an "old" API version (DIALECT < 3), we don't store all the data
    // of a multi-value field in the SV as we want to return it, so we need to load and override all
    // requested return fields that are SV source.
    bool forceLoad = req->sctx && isSpecJson(req->sctx->spec) &&
                     (req->sctx->apiVersion < APIVERSION_RETURN_MULTI_CMP_FIRST);
    ...
    for (const DLLIST_node *nn = pln->steps.next; nn != &pln->steps; nn = nn->next) {
        ...
        case PLN_T_FILTER: {
            ...
            if (stp->type == PLN_T_APPLY) {
                ...
                if (!dstkey) {
                    ...
                    QueryError_SetErrorFmt(status, QUERY_EDUPFIELD,
                                           "Property `%s` specified more than once", stp->alias);
                    goto error;
                }
        ...
        case PLN_T_LOAD: {
            ...
            if (lstp->nkeys || lstp->base.flags & PLN_F_LOAD_ALL) {
                ...
                if (isSpecJson(req->sctx->spec)) {
                    // On JSON, load all gets the serialized value of the doc, and doesn't make the
                    // fields available.
