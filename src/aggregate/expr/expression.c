...
static void setReferenceValue(RSValue *dst, RSValue *src) { RSValue_MakeReference(dst, src); }

extern int func_exists(ExprEval *ctx, RSValue *result, RSValue **argv, size_t argc,
                       QueryError *err);
...
static int evalProperty(ExprEval *eval, const RSLookupExpr *e, RSValue *res) {
    ...
    if (!value) {
        if (eval->err) {
            QueryError_SetErrorFmt(eval->err, QUERY_ENOPROPVAL,
                                   "%s: has no value, consider using EXISTS if applicable",
                                   e->lookupObj->name);
        }
        res->t = RSValue_Null;
...
#define RECURSE(v)                                                                                 \
    ...
    switch (expr->t) {
    case RSExpr_Property:
        expr->property.lookupObj =
            RLookup_GetKey(lookup, expr->property.key, RLOOKUP_M_READ, RLOOKUP_F_NOFLAGS);
        if (!expr->property.lookupObj) {
            QueryError_SetErrorFmt(err, QUERY_ENOPROPKEY,
                                   "Property `%s` not loaded nor in pipeline", expr->property.key);
            return EXPR_EVAL_ERR;
        }
        break;
    case RSExpr_Function:
...
int EvalCtx_Eval(EvalCtx *r) {
    ...
    r->ee.root = r->_expr;
    if (ExprAST_GetLookupKeys((RSExpr *)r->ee.root, (RLookup *)r->ee.lookup, r->ee.err) !=
        EXPR_EVAL_OK) {
        return REDISMODULE_ERR;
    }
...
void RPEvaluator_Reply(RedisModule_Reply *reply, const char *title, const ResultProcessor *rp) {
    ...
    case RSExpr_Function:
        ...
        break;
    case RSExpr_Predicate:
        RedisModule_Reply_SimpleStringf(reply, "%s - Predicate %s", typeStr,
                                        getRSConditionStrings(expr->pred.cond));
        break;
    case RSExpr_Inverted:
