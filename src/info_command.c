...
int IndexInfoCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    REPLY_KVNUM("total_index_memory_sz_mb",
                IndexSpec_TotalMemUsage(sp, dt_tm_size, tags_overhead, text_overhead) /
                    (float)0x100000);
    REPLY_KVNUM("geoshapes_sz_mb", geom_idx_sz / (float)0x100000);
    REPLY_KVNUM("records_per_doc_avg", (float)sp->stats.numRecords / (float)sp->stats.numDocuments);
    ...
    REPLY_KVNUM("total_indexing_time",
                (float)(sp->stats.totalIndexTime / (float)CLOCKS_PER_MILLISEC));
    ...
    bool with_times =
        (argc > 2 && !strcmp(RedisModule_StringPtrLen(argv[2], NULL), WITH_INDEX_ERROR_TIME));
