...
"""Validated backport publication and deterministic PR/summary templates.

The unified workflow calls apply_target() with a trusted target allow-list and
an agent manifest. It owns the combined summary and error recovery in unified.py.
The standalone CLI remains useful with APPLY_DRY_RUN=1 for local inspection.
Every git/gh invocation uses argument arrays and PrivilegedGit sanitizes the
agent's clone before installing the publication credential.
"""
...
def apply_target(ctx: dict, git: common.PrivilegedGit, entry: dict) -> dict:
    ...
    labels = list(dict.fromkeys(ctx.get("labels", []) + ["auto-backport"] +
                              (["auto-backport-conflicts"] if status == "conflicts" else [])))
    cl = entry.get("conflict_log")
...
def summary_cell(value: str) -> str:
    """Keep agent-authored explanations inside their table cell."""
...
def summary_comment(pr: int, sha: str, rows: list[dict]) -> str:
    ...
    for row in rows:
        ...
        if re.fullmatch(r"https://github.com/[^/]+/[^/]+/pull/[0-9]+", detail):
            cell = f"#{detail.rsplit('/', 1)[-1]}"
        else:
            cell = summary_cell(detail)
        lines.append(f"| `{summary_cell(row['target'])}` | {summary_cell(row['status'])} | {cell} |")
