...
int MRReply_StringEquals(MRReply *r, const char *s, int caseSensitive) {
    if (!r || MRReply_Type(r) != MR_REPLY_STRING && MRReply_Type(r) != MR_REPLY_STATUS)
        return 0;
    size_t len;
...
inline void MRReply_Free(MRReply *reply) { freeReplyObject(reply); }

inline int MRReply_Type(const MRReply *reply) { return reply->type; }

inline long long MRReply_Integer(const MRReply *reply) { return reply->integer; }

inline double MRReply_Double(const MRReply *reply) { return reply->dval; }

inline size_t MRReply_Length(const MRReply *reply) { return reply ? reply->elements : 0; }
