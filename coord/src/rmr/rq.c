...
static char loop_th_ready =
    false; /* set to true when the event loop thread is ready to process
            * requests.   This is set to false when a new topology is
            * applied,   and set to true   when the topology check is done. */
...
static inline bool loopThreadUninitialized() {
    return __builtin_expect((__atomic_test_and_set(&loop_th_started, __ATOMIC_ACQUIRE) == false),
                            false);
}
...
static void topologyFailureCB(uv_timer_t *timer) {
    RedisModule_Log(RSDummyContext, "warning",
                    "Topology validation failed: not all nodes connected");
...
static void topologyAsyncCB(uv_async_t *async) {
    ...
    if (topo) {
        ...
        // Mark the event loop thread as not ready. This will ensure that the next event on the
        // event loop will be the topology check. If the topology hasn't changed, the topology check
        // will quickly mark the event loop thread as ready again.
        loop_th_ready = false;
        topo->cb(topo->privdata);
        rm_free(topo);
        // Finish this round of topology checks to give the topology connections a chance to
        // connect. Schedule connectivity check immediately with a 1ms repeat interval
        uv_timer_start(&topologyValidationTimer, topologyTimerCB, 0, 1);
        if (clusterConfig.topologyValidationTimeoutMS) {
            // Schedule a timer to fail the topology validation if we don't connect to all nodes in
            // time
            uv_timer_start(&topologyFailureTimer, topologyFailureCB,
                           clusterConfig.topologyValidationTimeoutMS, 0);
        }
    }
}
