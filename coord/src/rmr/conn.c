...
void MRConnManager_Free(MRConnManager *mgr) { dictRelease(mgr->map); }
...
void MRConnManager_ReplyState(MRConnManager *mgr, RedisModuleCtx *ctx) {
    ...
    while ((entry = dictNext(it))) {
        ...
        RedisModuleString *key = RedisModule_CreateStringPrintf(
            ctx, "%s:%d", pool->conns[0]->ep.host, pool->conns[0]->ep.port);
...
int MRConn_SendCommand(MRConn *c, MRCommand *cmd, redisCallbackFn *fn, void *privdata) {
    ...
    if (!cmd->cmd) {
        if (redisFormatSdsCommandArgv(&cmd->cmd, cmd->num, (const char **)cmd->strs, cmd->lens) ==
            REDIS_ERR) {
            return REDIS_ERR;
        }
    }
...
static SSL_CTX *MRConn_CreateSSLContext(const char *cacert_filename, const char *cert_filename,
                                        const char *private_key_filename,
                                        const char *private_key_pass, redisSSLContextError *error) {
...
static void MRConn_ConnectCallback(const redisAsyncContext *c, int status) {
    ...
    if (checkTLS(&client_key, &client_cert, &ca_cert, &key_file_pass)) {
        ...
        SSL_CTX *ssl_context =
            MRConn_CreateSSLContext(ca_cert, client_cert, client_key, key_file_pass, &ssl_error);
        rm_free(client_key);
        rm_free(client_cert);
        rm_free(ca_cert);
        if (key_file_pass)
        ...
        if (ssl_context == NULL || ssl_error != 0) {
            CONN_LOG(conn, "Error on ssl context creation: %s",
        ...
        if (redisInitiateSSL((redisContext *)(&c->c), ssl) != REDIS_OK) {
            ...
            // This is a temporary fix to the bug describe on
            // https://github.com/redis/hiredis/issues/1233. In case of SSL initialization failure.
            // We need to reset the callbacks value, as the `redisInitiateSSL` function will not do
            // it for us.
