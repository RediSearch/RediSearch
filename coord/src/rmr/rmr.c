...
void MR_SetCoordinationStrategy(MRCtx *ctx, bool mastersOnly) { ctx->mastersOnly = mastersOnly; }
...
int MRCtx_GetNumReplied(struct MRCtx *ctx) { return ctx->numReplied; }

MRReply **MRCtx_GetReplies(struct MRCtx *ctx) { return ctx->replies; }

RedisModuleCtx *MRCtx_GetRedisCtx(struct MRCtx *ctx) { return ctx->redisCtx; }

RedisModuleBlockedClient *MRCtx_GetBlockedClient(struct MRCtx *ctx) { return ctx->bc; }

void MRCtx_SetReduceFunction(struct MRCtx *ctx, MRReduceFunc fn) { ctx->fn = fn; }
...
bool MR_CurrentTopologyExists() { return cluster_g->topo != NULL; }
...
static void uvMapRequest(void *p) {
    ...
    int rc =
        MRCluster_SendCommand(cluster_g, mrctx->mastersOnly, &mrctx->cmd, fanoutCallback, mrctx);
...
void MR_requestCompleted() { RQ_Done(rq_g); }
...
int MR_Fanout(struct MRCtx *mrctx, MRReduceFunc reducer, MRCommand cmd, bool block) {
    if (block) {
        ...
        mrctx->bc = RedisModule_BlockClient(mrctx->redisCtx, unblockHandler, timeoutHandler,
                                            freePrivDataCB, 0); // timeout_g);
...
int MR_MapSingle(struct MRCtx *ctx, MRReduceFunc reducer, MRCommand cmd) {
    ...
    ctx->cmd = cmd;
    RedisModule_Assert(!ctx->bc);
    ctx->bc = RedisModule_BlockClient(ctx->redisCtx, unblockHandler, timeoutHandler, freePrivDataCB,
                                      0); // timeout_g);
...
static void uvUpdateTopologyRequest(void *p) { MRCLuster_UpdateTopology(cluster_g, p); }
...
void MR_UpdateConnPerShard(size_t connPerShard) {
    if (!rq_g)
        return; // not initialized yet, we have nothing to update yet.
    ...
    if (NumShards == 1) {
        ...
        // This call should only update the connection pool size for when the connection pool is
        // initialized.
...
void MR_ReplyClusterInfo(RedisModuleCtx *ctx, MRClusterTopology *topo) {
    ...
    const char *cluster_type_str =
        clusterConfig.type == ClusterType_RedisOSS ? CLUSTER_TYPE_OSS : CLUSTER_TYPE_RLABS;
        ...
        if (!topo) {
            ...
            for (int i = 0; i < topo->numShards; i++) {
                ...
                for (int j = 0; j < sh->numNodes; j++) {
                    ...
                    RedisModule_ReplyKV_SimpleStringf(
                        reply, "role", "%s%s", // TODO: move the space to "self"
            ...
            for (int i = 0; i < topo->numShards; i++) {
                ...
                for (int j = 0; j < sh->numNodes; j++) {
                    ...
                    RedisModule_Reply_SimpleStringf(
                        reply, "%s%s", // TODO: move the space to "self"
                        node->flags & MRNode_Master ? "master " : "slave ", // "master" : "slave",
                        node->flags & MRNode_Self ? "self" : "");           // " self" : ""
...
// Use before obtaining `pending` (or any other variable of the iterator) to make sure it's
...
MRCommand *MRIteratorCallback_GetCommand(MRIteratorCallbackCtx *ctx) { return &ctx->cmd; }

MRIteratorCtx *MRIteratorCallback_GetCtx(MRIteratorCallbackCtx *ctx) { return ctx->ic; }
...
void iterStartCb(void *p) {
    ...
    for (size_t i = 0; i < it->len; i++) {
        if (MRCluster_SendCommand(cluster_g, true, &it->cbxs[i].cmd, mrIteratorRedisCB,
                                  &it->cbxs[i]) == REDIS_ERR) {
...
bool MR_ManuallyTriggerNextIfNeeded(MRIterator *it, size_t channelThreshold) {
    ...
    // Since we push the triggering job to a single-threaded queue (currently), we can modify the
    // logic here to trigger the next batch when we have no commands in process and no more than
    // channelThreshold replies to process.
    ...
    // We have <= channelThreshold replies to process, so if there are pending commands we want to
    // trigger them.
    if (it->ctx.pending) {
...
MRIteratorCtx *MRIterator_GetCtx(MRIterator *it) { return &it->ctx; }
