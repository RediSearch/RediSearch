...
void _MRCluster_UpdateNodes(MRCluster *cl) {
    ...
    // if we didn't remove the node from the original nodes map copy, it means it's not in the new
    // topology, we need to disconnect the node's connections
...
int MRCluster_ConnectAll(MRCluster *cl) { return MRConnManager_ConnectAll(&cl->mgr); }
