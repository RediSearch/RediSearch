...
static spellCheckReducerTerm *
spellcheckReducerCtx_GetOrCreateTermSuggestions(spellcheckReducerCtx *ctx, const char *term) {
...
static void spellcheckReducerCtx_AddTermSuggestion(spellcheckReducerCtx *ctx, const char *term,
    ...
    spellCheckReducerTerm *reducer_term =
...
static bool spellCheckAnalyzeResult_resp3(spellcheckReducerCtx *ctx, MRReply *termReply,
                                          MRReply *suggestions) {
    const char *term = MRReply_String(termReply, NULL);
...
int spellCheckReducer_resp3(struct MRCtx *mc, int count, MRReply **replies) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        for (int j = 0; j < MRReply_Length(termMap); j += 2) {
            ...
            int sug_type = MRReply_Type(
                suggestions); // either an array of ERR(SPELL_CHECK_FOUND_TERM_IN_INDEX)
