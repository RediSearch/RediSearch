...
static struct InfoFieldTypeAndValue findInfoTypeAndValue(InfoValue *values, InfoFieldSpec *specs,
                                                         size_t numFields, const char *name) {
...
static void recomputeAverageCycleTimeMs(InfoValue *gcValues, InfoFieldSpec *gcSpecs,
                                        size_t numFields) {
    struct InfoFieldTypeAndValue avg_cycle_time_ms =
        findInfoTypeAndValue(gcValues, gcSpecs, numFields, "average_cycle_time_ms");
    if (!avg_cycle_time_ms.value) {
        return;
    }
    avg_cycle_time_ms.value->isSet = 0;

    struct InfoFieldTypeAndValue total_cycles =
        findInfoTypeAndValue(gcValues, gcSpecs, numFields, "total_cycles");
    struct InfoFieldTypeAndValue total_ms =
        findInfoTypeAndValue(gcValues, gcSpecs, numFields, "total_ms_run");
...
static void handleSpecialField(InfoFields *fields, const char *name, MRReply *value) {
    if (!strcmp(name, "index_name")) {
        ...
        processKvArray(fields, value, fields->dialectValues, dialectSpecs, NUM_DIALECT_FIELDS_SPEC,
                       1);
...
static void processKvArray(InfoFields *fields, MRReply *array, InfoValue *dsts,
                           InfoFieldSpec *specs, size_t numFields, int onlyScalarValues) {
...
static void generateFieldsReply(InfoFields *fields, RedisModule_Reply *reply) {
    ...
    if (fields->indexName) {
        RedisModule_ReplyKV_StringBuffer(reply, "index_name", fields->indexName,
                                         fields->indexNameLen);
    }
...
int InfoReplyReducer(struct MRCtx *mc, int count, MRReply **replies) {
    ...
    for (size_t ii = 0; ii < count; ++ii) {
        ...
        processKvArray(&fields, replies[ii], fields.toplevelValues, toplevelSpecs_g,
                       NUM_FIELDS_SPEC, 0);
    }
