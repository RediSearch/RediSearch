...
static bool getCursorCommand(MRReply *res, MRCommand *cmd, MRIteratorCtx *ctx) {
    ...
    if (cursorId == 0) {
        // Cursor was set to 0, end of reply chain. cmd->depleted will be set in
        // `MRIteratorCallback_Done`.
        return false;
    }
...
static int netCursorCallback(MRIteratorCallbackCtx *ctx, MRReply *rep) {
    ...
    if (cmd->rootCommand == C_DEL) {
        if (MRReply_Type(rep) == MR_REPLY_ERROR) {
            RedisModule_Log(RSDummyContext, "warning",
                            "Error returned for CURSOR.DEL command from shard");
        }
    ...
    if (!bail_out) {
        ...
        if (cmd->protocol == 3) {
            ...
            if (bail_out) {
                RedisModule_Log(RSDummyContext, "warning", "Expected reply of length 2, got %ld",
                                len);
            }
        } else {
            ...
            if (bail_out) {
                RedisModule_Log(RSDummyContext, "warning",
                                "Expected reply of length 2 or 3, got %ld", len);
            }
        }
    }
    ...
    if (cmd->protocol == 3) // RESP3
    {
        ...
        if (map && MRReply_Type(map) == MR_REPLY_MAP) {
            ...
            if (cmd->forProfiling)
                results = MRReply_MapElement(results, "results"); // profile has an extra level
...
static int getNextReply(RPNet *nc) {
    if (nc->cmd.forCursor) {
        // if there are no more than `clusterConfig.cursorReplyThreshold` replies, trigger READs at
        // the shards.
    ...
    assert(!nc->current.rows || MRReply_Type(nc->current.rows) == MR_REPLY_ARRAY ||
...
static int rpnetNext(ResultProcessor *self, SearchResult *r) {
        ...
        if (nc->curIdx == len) {
            ...
            if (cursorId == 0 && nc->shardsProfile) {
                ...
                if (resp3 && MRReply_Length(warning) > 0) {
                    ...
                    if (!strcmp(MRReply_String(warning, NULL),
    ...
    while (!root || !rows || MRReply_Length(rows) == 0) {
        ...
        if (nc->current.root && MRReply_Type(nc->current.root) == MR_REPLY_ERROR) {
            ...
            if (!strErr || strcmp(strErr, "Timeout limit was reached") ||
...
static void buildMRCommand(RedisModuleString **argv, int argc, int profileArgs,
    ...
    if (argOffset != -1 && argOffset + 3 + 1 + profileArgs < argc) {
        ...
        array_append(tmparr, RedisModule_StringPtrLen(argv[argOffset + 3 + 1 + profileArgs],
                                                      NULL)); // the dialect
    }
...
void RSExecDistAggregate(RedisModuleCtx *ctx, RedisModuleString **argv, int argc,
    ...
    if (dialect >= 2) {
        // Check if we have KNN in the query string, and if so, parse the query string to see if it
        // is a KNN section in the query. IN that case, we treat this as a SORTBY+LIMIT step.
    ...
    // (notice with cursor, we rely on the existing mechanism of AREQ to free the ctx object when
