...
static inline bool SearchCluster_Ready() { return NumShards != 0; }
...
void setKNNSpecialCase(searchRequestCtx *req, specialCaseCtx *knn_ctx) {
    ...
    // When first sorting by different field, the topk vectors should be passed to the coordinator
    // heap
    ...
    // In this case the top 5 results relevant for this sort might be the in the last 5 results of
    // the TOPK
    ...
    if (array_len(req->specialCases) > 1) {
        ...
        if (optionalSortCtx->specialCaseType == SPECIAL_CASE_SORTBY) {
            if (strcmp(optionalSortCtx->sortby.sortKey, knn_ctx->knn.fieldName) == 0) {
                // If SORTBY is done by the vector score field, the coordinator will do it and no
                // special operation is needed.
...
specialCaseCtx *prepareOptionalTopKCase(const char *query_string, RedisModuleString **argv,
    ...
    if (queryNode->type == QN_VECTOR) {
        ...
        ctx->knn.k = k;
        ctx->knn.fieldName =
            queryNode->opts.distField ? queryNode->opts.distField : queryVectorNode.vq->scoreField;
        ctx->knn.pq = NULL;
...
searchRequestCtx *rscParseRequest(RedisModuleString **argv, int argc, QueryError *status) {
    ...
    RMUtil_ParseArgsAfter("LIMIT", argv + argvOffset, argc - argvOffset, "ll", &req->offset,
                          &req->limit);
    ...
    if (dialect >= 2) {
        // Note: currently there is only one single case. For extending those cases we should use a
        // trie here.
    ...
    if (req->withSortby) {
        ...
        if ((r1->sortKey || r2->sortKey)) {
            ...
            if (r1->sortKeyNum != HUGE_VAL && r2->sortKeyNum != HUGE_VAL) {
                ...
                // r2->sortKeyLen,
                //        (int)r2->sortKeyLen, r2->sortKey, r1->sortKeyLen, (int)r1->sortKeyLen,
                //        r1->sortKey, cmp);
            } else {
                // If at least one of these has no sort key, it gets high value regardless of
                // asc/desc
...
searchResult *newResult_resp2(searchResult *cached, MRReply *arr, int j,
                              searchReplyOffsets *offsets, int explainScores) {
    ...
    if (sortKeyOffset > 0) {
        res->sortKey =
            MRReply_String(MRReply_ArrayElement(arr, j + sortKeyOffset), &res->sortKeyLen);
    }
    if (res->sortKey) {
        if (res->sortKey[0] == '#') {
            char *endptr;
...
searchResult *newResult_resp3(searchResult *cached, MRReply *results, int j,
                              searchReplyOffsets *offsets, bool explainScores,
                              specialCaseCtx *reduceSpecialCaseCtxSortBy) {
    searchResult *res = cached ? cached : rm_malloc(sizeof *res);
    res->sortKey = NULL;
    res->sortKeyNum = HUGE_VAL;
    ...
    if (offsets->sortKey > 0) {
        ...
        if (reduceSpecialCaseCtxSortBy) {
            ...
            if (require_fields) {
                sortkey =
                    MRReply_MapElement(require_fields, reduceSpecialCaseCtxSortBy->sortby.sortKey);
            }
        }
        if (!sortkey) {
...
static void getReplyOffsets(const searchRequestCtx *ctx, searchReplyOffsets *offsets) {
     ...
     * Payload
     * Sort field    ---|
     * ...              | special cases - SORTBY, TOPK. Sort key is always first for backwards
     * comptability.
     * ...           ---|
     * First field
     *
    ...
    if (ctx->specialCases) {
        ...
        for (size_t i = 0; i < nSpecialCases; i++) {
            ...
            case SPECIAL_CASE_KNN: {
                ...
                specialCasesMaxOffset =
                    MAX(specialCasesMaxOffset, ctx->specialCases[i]->knn.offset);
                break;
            }
            case SPECIAL_CASE_SORTBY: {
                ...
                specialCasesMaxOffset =
                    MAX(specialCasesMaxOffset, ctx->specialCases[i]->sortby.offset);
                break;
            }
...
static void ProcessKNNSearchResult(searchResult *res, searchReducerCtx *rCtx, double score,
                                   knnContext *knnCtx) {
...
static void ProcessKNNSearchReply(MRReply *arr, searchReducerCtx *rCtx, RedisModuleCtx *ctx) {
    ...
    if (resp3) {
        ...
        for (int j = 0; j < len; ++j) {
            res = newResult_resp3(rCtx->cachedResult, results, j, &rCtx->offsets,
                                  rCtx->searchCtx->withExplainScores, reduceSpecialCaseCtxSortBy);
            if (res && res->id) {
            ...
            MRReply *require_fields =
                MRReply_MapElement(MRReply_ArrayElement(results, j), "required_fields");
            if (!require_fields) {
                RedisModule_Log(ctx, "warning",
            ...
            MRReply *score_value =
                MRReply_MapElement(require_fields, reduceSpecialCaseCtxKnn->knn.fieldName);
            if (!score_value) {
                RedisModule_Log(ctx, "warning",
        ...
        for (int j = 1; j < len; j += step) {
            if (j + step > len) {
                RedisModule_Log(ctx, "warning",
                                "got a bad reply from redisearch, reply contains less parameters "
                                "then expected");
                rCtx->errorOccurred = true;
                break;
            }
            res = newResult_resp2(rCtx->cachedResult, arr, j, &rCtx->offsets,
                                  rCtx->searchCtx->withExplainScores);
            if (res && res->id) {
                rCtx->cachedResult = NULL;
            } else {
                RedisModule_Log(ctx, "warning",
            ...
            double d;
            GET_NUMERIC_SCORE(d, res,
                              MRReply_String(MRReply_ArrayElement(arr, j + scoreOffset), NULL));
            ProcessKNNSearchResult(res, rCtx, d, &reduceSpecialCaseCtxKnn->knn);
        }
    }
    return;
...
static void processSearchReply(MRReply *arr, searchReducerCtx *rCtx, RedisModuleCtx *ctx) {
    ...
    if (resp3) // RESP3
    {
        ...
        for (int i = 0; i < len; ++i) {
            searchResult *res = newResult_resp3(rCtx->cachedResult, results, i, &rCtx->offsets,
                                                rCtx->searchCtx->withExplainScores,
                                                rCtx->reduceSpecialCaseCtxSortby);
    ...
    } else // RESP2
    {
        ...
        for (int j = 1; j < len; j += step) {
            ...
            searchResult *res = newResult_resp2(rCtx->cachedResult, arr, j, &rCtx->offsets,
                                                rCtx->searchCtx->withExplainScores);
            processSearchReplyResult(res, rCtx, ctx);
        }
    }
...
static void noOpPostProcess(searchReducerCtx *rCtx) { return; }
...
static void sendSearchResults(RedisModule_Reply *reply, searchReducerCtx *rCtx) {
    ...
    if (reply->resp3) // RESP3
    {
        ...
        for (int i = 0; i < qlen && i < num; ++i) {
            ...
            if (!req->noContent) {
                RedisModule_ReplyKV_MRReply(reply, "extra_attributes",
                                            res->fields); // >> extra_attributes
            }
 ...
 */
static void PrintShardProfile_resp2(RedisModule_Reply *reply, int count, MRReply **replies,
                                    bool isSearch) {
...
static void PrintShardProfile_resp3(RedisModule_Reply *reply, int count, MRReply **replies,
                                    bool isSearch) {
    for (int i = 0; i < count; ++i) {
        MRReply *profile;
        if (!isSearch) {
            // On aggregate commands, take the results from the response (second component is the
            // cursor-id)
...
static void profileSearchReplyCoordinator(RedisModule_Reply *reply, void *ctx) {
    ...
    RedisModule_Reply_Map(reply);
    RedisModule_ReplyKV_Double(reply, "Total Coordinator time",
                               (double)(clock() - pCtx->totalTime) / CLOCKS_PER_MILLISEC);
    RedisModule_ReplyKV_Double(reply, "Post Processing time",
                               (double)(clock() - pCtx->postProcessTime) / CLOCKS_PER_MILLISEC);
    RedisModule_Reply_MapEnd(reply);
}

static void profileSearchReply(RedisModule_Reply *reply, searchReducerCtx *rCtx, int count,
                               MRReply **replies, clock_t totalTime, clock_t postProcessTime) {
    ...
    Profile_PrintInFormat(reply, PrintShardProfile, &shardsCtx, profileSearchReplyCoordinator,
                          &coordCtx);
...
static bool should_return_error(MRReply *reply) {
    ...
    return (!errStr || RSGlobalConfig.requestConfigParams.timeoutPolicy == TimeoutPolicy_Fail ||
...
static inline int ReplyBlockDeny(RedisModuleCtx *ctx, const RedisModuleString *cmd) {
    return RMUtil_ReplyWithErrorFmt(ctx, "Cannot perform `%s`: Cannot block",
                                    RedisModule_StringPtrLen(cmd, NULL));
}
...
static void CursorCommandInternal(RedisModuleCtx *ctx, RedisModuleString **argv, int argc,
                                  struct ConcurrentCmdCtx *cmdCtx) {
...
int FlatSearchCommandHandler(RedisModuleBlockedClient *bc, int protocol, RedisModuleString **argv,
                             int argc) {
    QueryError status = {0};
    ...
    // adding the WITHSCORES option only if there is no SORTBY (hence the score is the default sort
    // key)
    if (!req->withSortby) {
...
static int DistSearchCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    RedisModuleBlockedClient *bc =
        RedisModule_BlockClient(ctx, DistSearchUnblockClient, NULL, NULL, 0);
...
int ProfileCommandHandler(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (NumShards == 1) {
        // There is only one shard in the cluster. We can handle the command locally.
        // We must first check that we don't have a cursor, as the local command handler allows
        // cursors for multi-shard clusters support.
...
static int initSearchCluster(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (clusterConfig.type == ClusterType_RedisOSS) {
        ...
        size_t len;
        const char *isCluster =
            RedisModule_CallReplyStringPtr(RedisModule_CallReplyArrayElement(rep, 1), &len);
...
void Coordinator_ShutdownEvent(RedisModuleCtx *ctx, RedisModuleEvent eid, uint64_t subevent,
                               void *data) {
...
RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (clusterConfig.type == ClusterType_RedisLabs) {
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.AGGREGATE", SafeCmd(DistAggregateCommand),
                                         "readonly", 0, 1, -2));
    } else {
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.AGGREGATE", SafeCmd(DistAggregateCommand),
                                         "readonly", 0, 0, -1));
    }
    RM_TRY(RedisModule_CreateCommand(ctx, "FT.INFO", SafeCmd(InfoCommandHandler), "readonly", 0, 0,
                                     -1));
    RM_TRY(RedisModule_CreateCommand(ctx, "FT.SEARCH", SafeCmd(DistSearchCommand), "readonly", 0, 0,
                                     -1));
    RM_TRY(RedisModule_CreateCommand(ctx, "FT.PROFILE", SafeCmd(ProfileCommandHandler), "readonly",
                                     0, 0, -1));
    if (clusterConfig.type == ClusterType_RedisLabs) {
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.CURSOR", SafeCmd(CursorCommand), "readonly", 3, 1,
                                         -3));
    } else {
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.CURSOR", SafeCmd(CursorCommand), "readonly", 0, 0,
                                         -1));
    }
    ...
    if (RSBuildType_g == RSBuildType_OSS) {
        ...
        // suggestion commands
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.SUGADD", SafeCmd(SingleShardCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.SUGGET", SafeCmd(SingleShardCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.SUGDEL", SafeCmd(SingleShardCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.SUGLEN", SafeCmd(SingleShardCommandHandler),
                                         "readonly", 0, 0, -1));
        ...
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.CREATE", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(
            ctx, "FT._CREATEIFNX", SafeCmd(MastersFanoutCommandHandler), "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.ALTER", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT._ALTERIFNX", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.DROPINDEX", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(
            ctx, "FT._DROPINDEXIFX", SafeCmd(MastersFanoutCommandHandler), "readonly", 0, 0, -1));
        ...
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.ALIASADD", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(
            ctx, "FT._ALIASADDIFNX", SafeCmd(MastersFanoutCommandHandler), "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.ALIASDEL", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(
            ctx, "FT._ALIASDELIFX", SafeCmd(MastersFanoutCommandHandler), "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(
            ctx, "FT.ALIASUPDATE", SafeCmd(MastersFanoutCommandHandler), "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.SYNUPDATE", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(
            ctx, "FT.SYNFORCEUPDATE", SafeCmd(MastersFanoutCommandHandler), "readonly", 0, 0, -1));
    }

    // cluster set commands
    RM_TRY(RedisModule_CreateCommand(ctx, REDISEARCH_MODULE_NAME ".CLUSTERSET",
                                     SafeCmd(SetClusterCommand),
                                     "readonly allow-loading deny-script", 0, 0, -1));
    RM_TRY(RedisModule_CreateCommand(ctx, REDISEARCH_MODULE_NAME ".CLUSTERREFRESH",
                                     SafeCmd(RefreshClusterCommand), "readonly deny-script", 0, 0,
                                     -1));
    RM_TRY(RedisModule_CreateCommand(ctx, REDISEARCH_MODULE_NAME ".CLUSTERINFO",
                                     SafeCmd(ClusterInfoCommand),
                                     "readonly allow-loading deny-script", 0, 0, -1));

    // Deprecated commands. Grouped here for easy tracking
    RM_TRY(RedisModule_CreateCommand(ctx, "FT.MGET", SafeCmd(MGetCommandHandler), "readonly", 0, 0,
                                     -1));
    RM_TRY(RedisModule_CreateCommand(ctx, "FT.TAGVALS", SafeCmd(TagValsCommandHandler), "readonly",
                                     0, 0, -1));
    if (RSBuildType_g == RSBuildType_OSS) {
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.GET", SafeCmd(SingleShardCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.ADD", SafeCmd(SingleShardCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.DEL", SafeCmd(SingleShardCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT.DROP", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
        RM_TRY(RedisModule_CreateCommand(ctx, "FT._DROPIFX", SafeCmd(MastersFanoutCommandHandler),
                                         "readonly", 0, 0, -1));
    }
