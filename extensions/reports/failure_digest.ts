...
> = {
  "@gdesmott/redisearch-build": (summary, base, failed) => {
    ...
    // A build can succeed and still produce nothing usable, and the assert that
    // catches it is not a model step — so it never reaches this report, and a
    // digest built only from step failures would say nothing was wrong while
    // `verify` stopped on a missing artifact. The summary knows; read it.
    // Only when the summary carries the fields at all. A summary without them
    // is one this report cannot say anything about — an older build model, or a
    // shape that never had them — and treating absence as a missing module
    // would invent a finding for every one of those.
    const reportsArtifact = "modulePath" in summary;
    const modulePath = str(summary, "modulePath");
    const stale = summary.moduleStale === true;
    if (reportsArtifact && (modulePath === null || stale)) {
      return [
        base({
          kind: "build-artifact",
          what: "build",
          detail: stale
            ? `the build produced no module this run; ${modulePath} is left ` +
              "over from an earlier one"
            : "the build reported success without producing a module",
        }),
      ];
    }
