...
Deno.test("a build that produced no module is reported, not passed over", async () => {
  // The step succeeded; the assert that catches this is not a model step, so it
  // never reaches this report. A digest built only from failed steps would say
  // there was nothing to fix while `verify` had stopped on a missing artifact.
  ...
  assertEquals(result.json.findingCount, 1);
  const findings = result.json.findings as Array<{ kind: string }>;
  assertEquals(findings[0].kind, "build-artifact");
});
