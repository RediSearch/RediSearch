  ...
  timeout: z
    ...
    .describe(
      `Timeout in milliseconds (default ${DEFAULT_TIMEOUT_MS}).` +
        " The timeout signals the build script itself; a compiler or server it spawned is left to the operating system, so the tail of the log can be missing when one is still writing as the run ends.",
    ),
  ...
  methods: {
    build: {
      ...
      execute: async (
        ...
        // the child, and the drain below that would otherwise outlive it.
        ...
        /**
         * Decode a stream, splitting it into lines, and give up on it once the
         * run has been aborted.
         *
         * The abort reaches the build script and nothing else: make, the
         * compilers and any test server it started inherited this pipe and were
         * signalled by no one. Draining until the pipe closes therefore waits
         * for whichever of them lives longest, so a run with a hung child
         * outlives the timeout that was meant to end it — by minutes, or until
         * that child is killed by hand. Cancelling costs whatever those
         * processes had not written yet, which is a fair price for a run that
         * is over either way.
         */
        const pump = async (
          ...
          if (abort.aborted) {
            ...
            abort.addEventListener("abort", stopDraining, { once: true });
          }
