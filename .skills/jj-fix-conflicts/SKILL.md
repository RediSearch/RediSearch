...
**Autogenerated headers** (`src/redisearch_rs/headers/`): These files are generated by `cheadergen` (which runs `cargo doc` internally) and must never be manually merged. Resolve all other conflicted files in the change **first**, then fix the headers:

```bash
jj restore --from 'latest(ancestors(<conflicted-change>) ~ <conflicted-change> ~ conflicts())' <conflicted-header-file>
make generate-rust-headers
```

This must come last because `cargo doc` will fail if Rust source files still have conflict markers.
