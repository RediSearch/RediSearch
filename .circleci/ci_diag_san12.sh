...
if [[ $ASAN == 1 ]]; then
    ...
    python2 $READIES/bin/getredis --force -v 6.2 --no-tls --no-run --suffix asan --clang-asan --clang-san-blacklist /build/redis.blacklist
elif [[ $MSAN == 1 ]]; then
    mode=msan
	JSON_SAN_MODE=memory
    extra_flags="-DUSE_MSAN=ON -DMSAN_PREFIX=${SAN_PREFIX}"
    python2 $READIES/bin/getredis --force -v 6.2 --no-tls --no-run --suffix msan --clang-msan --llvm-dir /opt/llvm-project/build-msan --clang-san-blacklist /build/redis.blacklist
else
