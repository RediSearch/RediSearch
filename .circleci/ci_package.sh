...
outname_base=$(python2 -m RAMP.ramp pack -m ${RAMP_YML} -o $outname_base --print-filename-only ${RAMP_ARGS} $MODULE_SO)
echo "Output is $outname_base"
python2 -m RAMP.ramp pack -m ${RAMP_YML} -o $outname_base.zip $MODULE_SO
return 0
}
