...
interface Context {
  ...
  dataRepository: {
    ...
    findByName?: (
      ...
      {
        version?: number;
        ownerDefinition?: { workflowRunId?: string };
      } | null
    >;
  };
}
    ...
    if (str(summary, "stage") === "format") {
      return strings(summary, "unformatted").map((file) =>
        base({
          ...
          what: "formatting",
          where: file,
          detail: "needs reformatting; run `deno fmt` in swamp/extensions/",
        })
      );
    }
    const failures = list(summary, "failures").map((failure) =>
...
/**
 * The version of a step's summary and who wrote it, or null when nothing
 * answered.
 *
 ...
 *
 * The version comes back with it so the caller can read the content at that
 * exact version. Asked as two independent "latest" questions, the two answers
 * can straddle a write from another run — the per-model lock serialises runs of
 * one model but nothing stops a second workflow driving it between these two
 * calls — and the digest then pairs one version's content with another
 * version's ownership: this run's summary discarded as stale, or a stale one
 * accepted as this run's.
 */
async function metadataOf(
  context: Context,
  step: StepExecution,
): Promise<{ version?: number; runId?: string } | null> {
...
async function readSummary(
  ...
  // A pinned read is already this run's by construction and names its version.
  // Only the fallback has anything to establish, and it asks the metadata
  // first: the version that answer carries is then what the content is read at,
  // so the two cannot describe different versions.
  let version = handle?.version;
  let ofThisRun: boolean | null = pinned ? true : null;
  if (!pinned) {
    const metadata = await metadataOf(context, step);
    if (metadata) {
      ofThisRun = metadata.runId === undefined
        ? null
        : metadata.runId === context.workflowRunId;
      // Undefined leaves the read on the model's latest, which is where it was
      // before any of this: a context whose metadata carries no version is no
      // worse off, only no better.
