...
function makeContext(
  ...
  const findByName = owners === undefined ? {} : {
    findByName: (_type: string, modelId: string) => {
      ...
      // The version is optional here as it is in the real metadata: without one
      // the report is left reading the model's latest, which is what it did
      // before it started asking.
      const version = versions?.[modelId];
      return Promise.resolve(
        runId === undefined || runId === null
          ...
          ? { version, ownerDefinition: {} }
          : { version, ownerDefinition: { workflowRunId: runId } },
      );
    },
  };
...
Deno.test("reads an unpinned summary at the version the metadata named", async () => {
  // The two questions used to be asked independently, each for "latest", and a
  // write from another run landing between them left the digest pairing one
  // version's content with another version's ownership — this run's summary
  // discarded as stale, or a stale one accepted as this run's. Taking the
  // version out of the ownership answer and reading the content at it is what
  // makes the pair describe one version.
  const { context, requested } = makeContext(
    loneFailure(),
    YESTERDAYS_LINT,
    "failed",
  ...
  assertEquals(requested, [{ modelId: "clippy", version: 7 }]);
});
