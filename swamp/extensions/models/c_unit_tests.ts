...
const OWNED_ENV = [
  "SKIP_BUILD",
  "SAN",
  // `sbin/unit-tests` defaults this only when it is empty, then writes it into
  // `ASAN_OPTIONS` as `detect_leaks=…`. An inherited `DETECT_LEAKS=0` therefore
  // turns LeakSanitizer off for a sanitizer run and the suite passes with the
  // leak still in it — the one failure `verify-asan` exists to reproduce.
  "DETECT_LEAKS",
  // The runner's own controls, which decide that no test runs at all. `HELP=1`
  // makes it print usage and `exit 0` before the first test — recorded here as
  // a passing run with nothing parsed — and `GDB=1` puts the suite under an
  // interactive debugger, which for a headless phase is a run that never ends.
  // Neither is a thing a caller of this model can have meant.
  "HELP",
  "GDB",
  "COV",
  "TEST",
 ...
 * is appended to the argument vector, but the summary — the flavor, the bin
 * directory, and the `testFilter` a gate reads to decide whether the suite ran
 * in full — is derived from the typed fields alone. `extraArgs: ["TEST=x"]`
 * with `test` unset runs one test and records the run as unfiltered;
 * `["DEBUG=1"]` with `debug` unset runs the debug build and names the release
 * one. Anything build.sh forwards to CMake, and any switch the summary does not
 * speak for, is what `extraArgs` is for and is left alone.
 */
 ...
 * as a whole run.
 */
const MAX_RETAINED_LINES = 200_000;
 ...
 */
function retain(lines: string[], line: string): void {
