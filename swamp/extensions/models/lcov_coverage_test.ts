...
Deno.test("two spellings of one file merge rather than replace", () => {
  // The state this exists for: a machine where a prefix is a symlink, so the
  // trace names one physical file two ways. Totalling before the two are
  // brought together is what loses coverage — a line hit under the first
  // spelling and instrumented but unhit under the second was reported as a gap.
  const hits = parseHits([
    "SF:/home/dev/repo/src/a.c",
    "DA:1,3",
    "DA:2,0",
    "end_of_record",
    "SF:/var/home/dev/repo/src/a.c",
    "DA:2,4",
    "DA:3,0",
    "end_of_record",
  ].join("\n"));

  // Kept apart while they are still two spellings: only the caller can say
  // that they resolve to one file.
  assertEquals(hits.size, 2);

  // Merged the way the trace's own repeated records are merged — hits summed
  // per line, coverage being the union of everything that measured the file.
  const merged = new Map<number, number>(hits.get("/home/dev/repo/src/a.c"));
  for (const [line, count] of hits.get("/var/home/dev/repo/src/a.c")!) {
    merged.set(line, (merged.get(line) ?? 0) + count);
  }

  assertEquals(totalise(merged), {
    coveredLines: 2,
    totalLines: 3,
    uncovered: [3],
  });
