...
Deno.test("times out even when a child outlives the build script", async () => {
  await withTempRepo(async (dir) => {
    // The background sleep inherits the pipes and is not signalled when the
    // script is, which is what a compiler or a test server does in a real
    // build. Waiting for the pipes to close would mean waiting for it.
...
Deno.test("refuses a modelled switch passed through extraArgs", async () => {
  await withTempRepo(async (dir) => {
    await succeedingRepo(dir);
    await placeModule(dir, "release", 60_000);
    const { context, recorded } = makeContext(dir, {
      repoRoot: ".",
      buildScript: "./build.sh",
      coord: "oss",
    ...
    // build.sh would build debug while the summary named release and pointed at
    // the release directory — where the module placed above, from an earlier
    // build, would pass as this run's output and be handed to the suites.
    const error = await captureError(() =>
      build.execute({ extraArgs: ["DEBUG=1"], quiet: true }, context)
    );

    assertEquals(error?.message.includes("extraArgs sets DEBUG"), true);
    assertEquals(error?.message.includes("`debug`"), true);

    // The same switch in the spelling build.sh also accepts: it folds every
    // argument to upper case before matching, so a case-sensitive check here
    // would read as a rule that holds while letting this straight through.
    const lowercased = await captureError(() =>
      build.execute({ extraArgs: ["debug=1"], quiet: true }, context)
    );
    assertEquals(lowercased?.message.includes("extraArgs sets DEBUG"), true);

    // Rejected before anything ran, so there is no summary claiming a flavor.
    assertEquals(recorded.resources.length, 0);
    assertEquals(recorded.files.length, 0);
  });
});

Deno.test("forwards an argument the model does not speak for", async () => {
  await withTempRepo(async (dir) => {
    await succeedingRepo(dir);
    const { context, recorded } = makeContext(dir, {
      repoRoot: ".",
    ...
    await build.execute(
      { extraArgs: ["BUILD_INTEL_SVS_OPT=0"], quiet: true },
      context,
    );

    const log = recorded.files[0].lines.join("\n");
    assertEquals(log.includes("ARGS: COORD=oss BUILD_INTEL_SVS_OPT=0"), true);
  });
});
