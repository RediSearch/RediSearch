...
Deno.test("an optional check failing does not make it unready", () => {
  // UNSTABLE *is* a failing rollup with only non-required checks red, so
  // requiring a passing rollup as well made the state unreachable and put this
  // function at odds with its own mergeable-states rule. What it cost was a
  // sweep: a pull request anybody could merge read as not ready, and the CI
  // phase triaged the optional failure once per firing until its budget ran
  // out.
  assertEquals(
    isReady(
      mergeable({ mergeStateStatus: "UNSTABLE", checksState: "failing" }),
    ),
    true,
  );
  // A check still running says nothing yet about whether it is required.
  assertEquals(
    isReady(
      mergeable({ mergeStateStatus: "UNSTABLE", checksState: "pending" }),
    ),
    false,
  );
  // And a failing rollup anywhere GitHub has not said the failures are
  // optional is still a failing rollup.
  assertEquals(
    isReady(mergeable({ mergeStateStatus: "CLEAN", checksState: "failing" })),
    false,
  );
});
...
Deno.test("a pull request URL names the repository it belongs to", () => {
  ...
  assertEquals(
    ...
    "github.com/someone/RediSearch",
  );
  // A different host is a different repository, whatever it is called there:
  // an Enterprise mirror of this one answers to the same `owner/name` and has
  // the same commits, so dropping the host made the two indistinguishable.
  assertEquals(
...
Deno.test("a pull request URL yields the three things a mutation needs", () => {
  assertEquals(
    pullRequestRef("https://github.com/RediSearch/RediSearch/pull/10702"),
    { owner: "RediSearch", name: "RediSearch", number: 10702 },
  );
  // Not a pull request URL, so there is no set of threads it could authorise.
  assertEquals(
    pullRequestRef("https://github.com/RediSearch/RediSearch"),
    null,
  );
  assertEquals(pullRequestRef("10702"), null);
});

Deno.test("an id that names no thread here is refused, not sent", () => {
  // The whole reason this happens in the model: the ids come from an agent that
  // matched threads to its own fixes, and one it invented or took from another
  // pull request would otherwise reach the mutation. Resolving live feedback is
  // not undoable by validating the agent's answer afterwards.
  ...
  const sorted = sortThreads(["open-2", "settled", "invented"], threads);

  assertEquals(sorted.toResolve, ["open-2"]);
  // Already resolved is not a failure: a re-run of the same pass asks again.
  assertEquals(sorted.alreadyResolved, ["settled"]);
  assertEquals(sorted.rejected, ["invented"]);
});

Deno.test("a repository identity carries its host", () => {
  // `owner/name` is not an identity: an Enterprise mirror answers to the same
  // one and a fork shares every commit id, so a pull request there would pass
  // both the repository comparison and the head comparison while belonging
  ...
  assertEquals(
    repositoryOf("https://ghe.example.com/RediSearch/RediSearch/pull/10702"),
    "ghe.example.com/RediSearch/RediSearch",
  );
  // Both sides are read the same way, one from the pull request's URL and one
  // from the checkout's.
  assertEquals(
    identityOf("https://github.com/RediSearch/RediSearch\n"),
    "github.com/RediSearch/RediSearch",
  );
  assertEquals(
    identityOf("https://ghe.example.com/RediSearch/RediSearch.git"),
    "ghe.example.com/RediSearch/RediSearch",
  );
  assertEquals(identityOf("not a url"), "");
});

Deno.test("feedback answered by a later approval is not outstanding", () => {
  // The shape this exists for: a reviewer comments on the head and then
  // approves that same head. Counting the comment anyway sends the CI fixer at
  // feedback already answered — it finds nothing, pushes nothing, and spends an
  // attempt on a pull request that was only waiting for another reviewer.
  const reviews = [
    {
      author: { login: "alice" },
      state: "COMMENTED",
...
Deno.test("an approval followed by findings is still findings", () => {
  // The other order, and the one where being wrong loses live feedback rather
  // than inventing it.
  const reviews = [
    {
      author: { login: "alice" },
  ...
  assertEquals(reviewedHead(reviews, "head"), ["alice"]);
});
