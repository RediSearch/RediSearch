      ...
      files: [
        {
          ...
          segments: [
            // An uncalled block and its close, then two consecutive lines with
            // one of their own — enough shape for the ranges to be worth
            // asserting.
            [134, 9, 0, true, true, false],
            [136, 9, 0, false, false, false],
            [157, 13, 0, true, true, false],
            [158, 13, 0, true, true, false],
            [159, 13, 0, false, false, false],
          ],
        },
        {
...
Deno.test("fails a clean run that exported nothing", async () => {
  await withTempRepo(async (dir) => {
    // Reachable through `extraArgs`: cargo-llvm-cov's `--output-path` writes the
    // export to a file and still exits 0. Exit code alone would take that for a
    // completed measurement and record `parsed: false` beside a success.
    await Deno.mkdir(`${dir}/ws`, { recursive: true });
    await Deno.writeTextFile(`${dir}/ws/Cargo.toml`, "[workspace]\n");
    await Deno.writeTextFile(
      `${dir}/build.sh`,
    ...
    const cargo = await writeScript(
      dir,
      "fake-cargo",
      'echo "   Compiling qint v0.1.0" >&2; exit 0',
    );
    const { context, recorded } = makeContext(dir, globals(cargo));
      ...
      run.execute(
        {
          ...
          onlyIncomplete: true,
          quiet: true,
          extraArgs: ["--output-path", "cov.json"],
        },
        context,
      )
    );
    ...
    assertEquals(
      error?.message.includes("without writing a coverage export"),
      true,
    );
    const summary = summaryOf(recorded);
    assertEquals(summary.parsed, false);
    assertEquals(summary.overall, null);
    // The exit code was clean, so nothing here is a test failure — which is
    // exactly why the exit code cannot be what decides.
...
Deno.test("times out even when a child outlives cargo", async () => {
  await withTempRepo(async (dir) => {
    // The background sleep inherits both pipes and is not signalled when cargo
    // is, which is what rustc and the test binaries do in a real run. Waiting
    // for either pipe to close would mean waiting for it, and the timeout is
    // only looked at after both readers return.
...
Deno.test("measures with BINDIR unset when no binDir is given", async () => {
  await withTempRepo(async (dir) => {
...
Deno.test("does not let an inherited BINDIR decide what is measured", async () => {
  await withTempRepo(async (dir) => {
    const cargo = await workspace(
      dir,
      exportJson(),
    ...
    // otherwise measure that build's archives while the summary reports an
    // ordinary run.
    assertEquals(
      recorded.files[0].lines.join("\n").includes("ENV BINDIR=[unset]"),
      true,
