 ...
 * refuse to answer from a stale trace: pass `notOlderThan` with a bound the trace
 * has to beat and a trace left over from an earlier run is an error rather than a
 * set of made-up gaps.
 *
 * @module
 */
  ...
  repoRoot: z
    ...
    .describe(
      "Repository root the trace's paths are relative to. Relative paths " +
        "resolve against the swamp repository directory, so a checkout whose " +
        "swamp files live in a subdirectory wants `..`.",
    ),
  binDir: z
    .string()
    .min(1)
  ...
  notOlderThan: z
    ...
    .describe(
      "Fail unless the trace was written at or after this ISO 8601 timestamp, " +
        "so that gaps from an earlier run are an error rather than an answer. " +
        "Bound a fresh measurement with the *build's* `executedAt`: a model " +
        "stamps that when it finishes recording, which is after the suite wrote " +
        "the trace, so the suite's own timestamp — or this report's — rejects " +
        "every legitimate trace by the seconds between the two. To re-report " +
        "other files from a trace already measured, pass the `capturedAt` of " +
        "the summary that measured it, which is that trace's own timestamp.",
    ),
  requireAllFound: z
    .boolean()
    .default(true)
...
export function parseTrace(trace: string): Map<string, FileCoverage> {
  const hits = parseHits(trace);
  const files = new Map<string, FileCoverage>();
  for (const [path, lineHits] of hits) files.set(path, totalise(lineHits));
  return files;
}

/**
 * The per-line hit counts an lcov trace records, keyed by the path it used.
 *
 * Separate from {@linkcode parseTrace} because two paths in one trace can name
 * the same physical file — `/home/…/src/a.c` and `/var/home/…/src/a.c`, where
 * one prefix is a symlink — and only the caller knows that, since deciding it
 * means resolving the paths on disk. Totalling before that merge would lose
 * exactly what the merge is for: a line hit under one spelling and instrumented
 * but unhit under the other would count as a gap, and one instrumented under
 * only one spelling could vanish altogether.
 *
 * Exported for unit testing; not part of the model's public surface.
 */
export function parseHits(trace: string): Map<string, Map<number, number>> {
...
/**
 * Total one file's per-line hits into the coverage a caller reads.
 *
 * Exported for unit testing; not part of the model's public surface.
 */
export function totalise(hits: Map<number, number>): FileCoverage {
  ...
  return { coveredLines, totalLines: hits.size, uncovered };
}
...
/**
 * Resolve a path against a root, leaving absolute paths alone and collapsing
 * `.` and `..` segments so that a `repoRoot` pointing outside the swamp
 * repository does not leak `..` into every reported path.
 */
  ...
  methods: {
    report: {
      ...
      execute: async (
        ...
        // by the spelling either side happened to use — and *merge* what
        // collides there rather than letting the last one win. Two spellings of
        // one file is an ordinary state on a machine where a prefix is a
        // symlink, and overwriting meant a line hit under the first spelling
        // was reported as a gap, or dropped from the totals entirely.
        //
        // Merged by summing hits per line, which is the same rule the trace's
        // own repeated records are merged by: coverage is the union across
        // everything that measured the file.
        const hits = parseHits(await Deno.readTextFile(infoFile));
        const hitsByCanonical = new Map<string, Map<number, number>>();
        for (const [path, lineHits] of hits) {
          const key = await canonical(path);
          const merged = hitsByCanonical.get(key);
          if (!merged) {
            hitsByCanonical.set(key, lineHits);
            continue;
          }
          for (const [lineNo, count] of lineHits) {
            merged.set(lineNo, (merged.get(lineNo) ?? 0) + count);
          }
        }
        const byCanonical = new Map<string, FileCoverage>();
        for (const [key, lineHits] of hitsByCanonical) {
          byCanonical.set(key, totalise(lineHits));
        }
        ...
        const handle = await context.writeResource("summary", "summary", {
          ...
          // Distinct files, not distinct spellings: two paths that resolve to
          // one file are one file, and that is what the coverage was read from.
          filesInTrace: byCanonical.size,
