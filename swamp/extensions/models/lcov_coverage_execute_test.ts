...
Deno.test("re-reports from the same trace when bounded by its capturedAt", async () => {
  await withTempRepo(async (dir) => {
    await sources(dir, "src/spec.c");
    const path = await trace(dir, "flow_standalone.info", specRecords(dir));
    await Deno.utime(path, new Date("2026-07-30"), new Date("2026-07-30"));
    ...
    // Two runs, so the resources are read positionally: summaryOf insists on
    // exactly one.
    const first = recorded.resources[0].data;

    // Looking at a second file from a trace already measured is the reuse this
    // bound exists for, and the timestamp to hand back is the trace's own.
    // `executedAt` is stamped after the report finishes recording, so passing
    // that instead would reject the very trace being reused — which is why the
    // documentation names capturedAt and this test holds it to that.
    await report.execute({
      files: ["src/spec.c"],
      suite: "flow",
      deployment: "standalone",
      notOlderThan: first.capturedAt as string,
      requireAllFound: true,
    }, context);

    const second = recorded.resources[1].data;
    assertEquals(second.stale, false);
    assertEquals(second.capturedAt, first.capturedAt);
    assertEquals(
      (first.executedAt as string) > (first.capturedAt as string),
      true,
    );
  });
});
