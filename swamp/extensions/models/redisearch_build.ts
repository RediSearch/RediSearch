  ...
  extraArgs: z
    ...
    .describe(
      "Additional arguments appended to the build.sh invocation. Anything " +
        "build.sh does not recognise is forwarded to CMake as `-D<arg>`. A " +
        "switch that has a field of its own here is rejected rather than " +
        "forwarded: the summary is derived from the fields, so setting one " +
        "this way would build something other than what it reports.",
    ),
  timeout: z
    .number()
    .int()
...
const SummarySchema = z.object({
  ...
  startedAt: z
    ...
    .datetime()
    .describe(
      "Timestamp when the build was launched. Distinct from " +
        "`executedAt`, which is when it finished: a gate asking whether a run " +
        "began after some event cannot ask that of a completion time, and a " +
        "build is long enough for the difference to decide the answer.",
    ),
 ...
 *
 * `extraArgs` is appended to the argument vector, but the summary — the flavor,
 * the variant, the bin directory and the module path it reports, and whether the
 * module on disk is this build's — is derived from the typed fields alone. So a
 * switch passed here instead of through its field silently splits the two:
 * `extraArgs: ["DEBUG=1"]` with `debug` unset builds debug while the summary
 * names `release` and points at the release directory, where a module left by an
 * earlier release build passes as this run's output and the suites then run
 * against it.
 *
 * `SKIP_BUILD` is on the list with no field to redirect to: this model always
 * builds, and a run that skipped the build would report one it never ran.
 * Anything build.sh forwards to CMake, and any switch the summary does not
 * speak for, is exactly what `extraArgs` is for and is left alone.
 */
const MODELLED_SWITCHES: Record<string, string> = {
  ...
  VERBOSE: "`verbose`",
  TESTS: "`buildTests`",
  REDISEARCH_GENERATE_HEADERS: "`generateHeaders`",
  RUST_PROFILE: "`rustProfile`",
  RUST_DENY_WARNS: "`rustDenyWarns`",
  SKIP_BUILD: "nothing — this model exists to build",
};

/**
 * The switch an `extraArgs` entry sets, i.e. `DEBUG` from `DEBUG=1`.
 *
    ...
    if (owned) {
      throw new Error(
        `extraArgs sets ${switchName(arg)}, which this model derives its ` +
          ...
          `change what build.sh builds without changing the flavor, bin ` +
          `directory and module path recorded for it.`,
      );
    }
  }
}
