...
Deno.test("clears the build.sh controls it owns", () => {
  ...
  // DETECT_LEAKS is the one whose absence is silent: `sbin/unit-tests` defaults
  // it only when empty and writes it into ASAN_OPTIONS, so an inherited 0
  // turns LeakSanitizer off and the sanitizer suite passes with the leak in it.
  assertEquals(buildEnv(args).DETECT_LEAKS, "");
  // And the two that decide no test runs at all: HELP=1 prints usage and exits
  // 0 before the first test, which this model would record as a passing run
  // with nothing parsed; GDB=1 puts the suite under an interactive debugger.
  assertEquals(buildEnv(args).HELP, "");
  assertEquals(buildEnv(args).GDB, "");
});
