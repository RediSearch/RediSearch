...
Deno.test("clears the build.sh controls it owns", () => {
  ...
  // uses regardless of the REDIS_STANDALONE this model passes. RLEC is the
  // furthest along of them: at 1 runtests.sh runs the suite against an external
  // enterprise database and clears REDIS_STANDALONE itself, so an inherited one
  // changes both the topology and the module under test while the summary
  // reports the ones this run asked for. The REJSON family decides whether the
  // suite covers JSON at all: at 0 the module is not loaded and the `no_json`
  // tests are skipped rather than failed, so the run passes having exercised
  // none of the behaviour a gate reading it would be vouching for. NO_LIBEXT
  // drops one test the same way, through a skip rather than a failure. TESTFILE, FAILEDFILE, LIST and
  // RLTEST_ARGS and RLTEST_TEST_ARGS sit beside TEST in runtests.sh's own
  // test-selection block and narrow the suite the same way, none of them
  // visible in the summary. The last is the one that block appends TEST to.
  // The RLTEST_*_ARGS family and CONFIG_FILE are the same channel one level
  // along: each is read into the RLTest configuration file, and anything RLTest
  // accepts can arrive through any of them. ENV_ONLY, HELP and NOP stop the
  // suite running at all, which fails loudly rather than quietly — but still on
  // the state of somebody's shell rather than on the code.
...
Deno.test("retained output is bounded, and says so when it was cut", () => {
  // A test or server that spins printing until the three-hour timeout would
  // otherwise be held in memory in full, and the process dies before the
  // timeout can record a diagnostic — a failed run that leaves nothing to read.
  const lines: string[] = [];
  for (let i = 0; i < MAX_RETAINED_LINES + 50; i++) retain(lines, `line ${i}`);

  // Bounded, which is the requirement; the cap is a ceiling and lines are
  // dropped in blocks, so the exact length in between is not one.
  assertEquals(lines.length <= MAX_RETAINED_LINES, true);
  assertEquals(lines.length > MAX_RETAINED_LINES / 2, true);
  // The tail is what is kept, because that is where a run's totals are.
  assertEquals(lines[lines.length - 1], `line ${MAX_RETAINED_LINES + 49}`);
  // And it says it is a tail, once, so nothing reads it as the whole run.
  assertEquals(lines[0], TRUNCATION_MARKER);
  assertEquals(lines.filter((l) => l === TRUNCATION_MARKER).length, 1);
});
