 ...
 *
 * The same split the suite and lint models guard against. `extraArgs` reaches
 * `cargo llvm-cov test`, but `scope` — the field a reader takes as what was
 * measured — is derived from the manifest path, `crate`, or the workspace
 * default alone. `extraArgs: ["--lib"]` therefore measures a package's library
 * and nothing else while the summary still says `workspace`, and coverage that
 * was never attempted reads as coverage that is missing.
 *
 * `--output-path` is here for a different reason and the model already says
 * why: it redirects the export that the measurement is read from.
 */
const REFUSED_SWITCHES: Record<string, string> = {
  "-p": "`crate`, which is what `scope` is recorded from",
  "--package": "`crate`, which is what `scope` is recorded from",
  "--manifest-path": "`manifestPath`, which is what `scope` is recorded from",
  "--workspace": "nothing — the model passes it when no crate is named",
  "--exclude": "nothing — the exclusions come from build.sh",
  "--lib": "nothing — the summary speaks for everything in scope",
  "--bin": "nothing — the summary speaks for everything in scope",
  "--bins": "nothing — the summary speaks for everything in scope",
  "--test": "nothing — the summary speaks for everything in scope",
  "--tests": "nothing — the summary speaks for everything in scope",
  "--example": "nothing — the summary speaks for everything in scope",
  "--examples": "nothing — the summary speaks for everything in scope",
  "--doctests": "`doctests`",
  "--ignore-run-fail": "`ignoreTestFailure`",
};
...
/**
 * Refuse `extraArgs` that would change what was measured without changing what
 * the summary says was measured.
 *
 * Exported for unit testing; not part of the model's public surface.
 */
    ...
    if (refused) {
      throw new Error(
        `extraArgs sets ${switchName(arg)}, which this model's summary does ` +
          ...
          `would narrow what was measured while \`scope\` still names the ` +
          `whole of it, and coverage never attempted reads as coverage that ` +
          `is missing.`,
      );
    }
    // A value written directly after its own switch is a value; anything else
    // bare is a selector or a path, and cargo reads it as one.
    ...
    if (
      ...
      throw new Error(
        `extraArgs contains the positional entry ${JSON.stringify(arg)}, ` +
          "which cargo reads as a test-name filter or a path: it narrows what " +
          "was measured while `scope` still names the whole of it. Attach a " +
          'flag\'s value with "=" so that a value is never mistaken for one.',
      );
    }
  }
}
