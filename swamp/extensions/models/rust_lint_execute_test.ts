...
Deno.test("lints when the headers were regenerated but not yet committed", async () => {
  await withTempRepo(async (dir) => {
    // The ordinary state for the pre-commit `verify` this model serves: the
    // author changed an exported Rust type, regenerated the headers, and has
    // not committed either yet. Regeneration introduces nothing further, so
    // there is nothing here for a lint to refuse — telling the author to run
    // the command they have just run is not a finding, and the headers
    // belonging in the same revision as the change is what
    // /commit-guidelines is for.
    const cargo = await repo(dir, {
      regen:
        `echo "// regenerated" > "${dir}/src/redisearch_rs/headers/generated.h"`,
    });
    await gitRepoWithHeaders(dir);
    // Already regenerated, before the model runs.
    await Deno.writeTextFile(
      `${dir}/src/redisearch_rs/headers/generated.h`,
      "// regenerated\n",
    );
    const { context } = makeContext(dir, globals(cargo));
    ...
    // Regeneration first, then both profiles linted: nothing refused.
    const ran = await order(dir);
    assertEquals(ran[0], "regen");
    assertEquals(ran.length, 3);
  });
...
Deno.test("times out even when a child outlives cargo", async () => {
  await withTempRepo(async (dir) => {
    ...
    const cargo = await repo(dir, { cargo: "sleep 30 &\nexec sleep 30" });
    const { context } = makeContext(
      dir,
      globals(cargo, { profiles: ["debug"] }),
    );
...
Deno.test("lints with BINDIR unset when no binDir is given", async () => {
  await withTempRepo(async (dir) => {
...
Deno.test("does not let an inherited RUSTFLAGS defeat the lint gate", async () => {
  await withTempRepo(async (dir) => {
    const cargo = await repo(dir, {
      cargo: 'echo "ENV RUSTFLAGS=[${RUSTFLAGS-unset}]"',
    });
    const { context, recorded } = makeContext(
      dir,
      globals(cargo, { regenerateHeaders: false, profiles: ["debug"] }),
    );
    // rustc's lint cap outranks the command line, so this turns the `-D
    // warnings` the model appends into nothing: every profile comes back clean
    // and the local gate certifies what CI's clean environment rejects.
    Deno.env.set("RUSTFLAGS", "--cap-lints allow");
    ...
    assertEquals(
      recorded.files[0].lines.join("\n").includes("ENV RUSTFLAGS=[unset]"),
      true,
    );
  });
});

Deno.test("does not let an inherited BINDIR decide what is linted", async () => {
  await withTempRepo(async (dir) => {
    const cargo = await repo(dir, {
      cargo: 'echo "ENV BINDIR=[${BINDIR-unset}]"',
    });
    ...
    // otherwise lint against that build's archives while the summary reports an
    // ordinary run.
    assertEquals(
      recorded.files[0].lines.join("\n").includes("ENV BINDIR=[unset]"),
      true,
