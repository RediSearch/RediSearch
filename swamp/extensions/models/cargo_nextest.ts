...
function childEnv(
  ...
  // `CARGO_ENCODED_RUSTFLAGS` outranks `RUSTFLAGS` when both are set, so
  // assigning the latter is not enough to decide what the compiler is told: an
  // inherited `--cap-lints allow` in the encoded form caps every warning this
  // model exists to deny, and the tests compile clean here and are rejected by
  // CI. Deno can override a variable but not remove one, so dropping it means
  // handing over the whole environment without it — done only when there is
  // something to drop.
  const dropped = ["BINDIR", "CARGO_ENCODED_RUSTFLAGS"].filter((name) =>
    (name !== "BINDIR" || !args.binDir) && Deno.env.get(name) !== undefined
  );
  const dropInherited = dropped.length > 0;
    ...
    run: {
      ...
      execute: async (
        ...
        for (const [i, arg] of extra.entries()) {
          ...
          // The switch list cannot see the form that needs no switch: nextest
          // reads a bare entry as a test-name filter, so `extraArgs: ["ids"]`
          // runs a handful of tests and records `testFilter: null` — the shape
          // of a full suite, on a positive test count. `test` is the field for
          // a filter.
          //
          // An entry directly after a switch written without `=` is taken as
          // that switch's value, because `["--retries", "2"]` is how a caller
          // ordinarily passes one and there is no list here of which switches
          // take values. What that leaves uncaught is a filter written after a
          // boolean flag; writing values as `--retries=2` closes even that,
          // which is why the message asks for it.
          const previous = i === 0 ? "" : extra[i - 1];
          const isValueOfPrevious = previous.startsWith("-") &&
            !previous.includes("=");
          if (!arg.startsWith("-") && !isValueOfPrevious) {
            throw new Error(
              `extraArgs contains the positional entry ${
                JSON.stringify(arg)
              }: nextest reads it as a test-name filter, which narrows the run ` +
                `while the summary still records the filter as unset — a ` +
                `partial run then reads as the whole suite. Pass a filter as ` +
                `\`test\`, and attach a flag's value with "=" (--jobs=4).`,
            );
          }
        }
