...
export function reviewedHead(
  ...
  for (const review of reviews) {
    ...
    if (!FEEDBACK_STATES.has((review.state ?? "").toUpperCase())) continue;
    const login = review.author?.login ?? "";
...
export function isReady(status: {
  ...
  return status.state === "OPEN" && !status.isDraft &&
    status.checksState === "passing" && status.approved &&
    MERGEABLE_STATES.has(status.mergeStateStatus.toUpperCase());
}
