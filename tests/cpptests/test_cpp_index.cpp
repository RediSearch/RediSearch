...
TEST_F(IndexTest, testDocTable) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    // Verify we can borrow the document by its id
    t_docId xid = dmd->id;
    ASSERT_EQ((int)xid, i + 1);

    ASSERT_TRUE(DocTable_Pop(&dt, dmd->id) != NULL);
    DMD_Return(dmd);
  ...
  // Verify we can borrow the document by its id
  const RSDocumentMetadata *borrowed = DocTable_Borrow(&dt, dmd->id);
  ASSERT_TRUE(borrowed);
  ASSERT_EQ(dmd->id, borrowed->id);
  DMD_Return(borrowed);
  DMD_Return(dmd);
  DocTable_Free(&dt);
}
