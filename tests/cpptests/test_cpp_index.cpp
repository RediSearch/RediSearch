...
TEST_F(IndexTest, testMetric_VectorRange) {
  ...
  while (vecIt->Read(vecIt) != ITERATOR_EOF) {
    ...
    ASSERT_NEAR(IndexResult_NumValue(h), exp_dist, 1e-6);
    ASSERT_NEAR(RSValue_Number_Get(h->metrics[0].value), exp_dist, 1e-6);
    count++;
  }
  ...
  ASSERT_NEAR(IndexResult_NumValue(vecIt->current), exp_dist, 1e-6);
  ASSERT_NEAR(RSValue_Number_Get(vecIt->current->metrics[0].value), exp_dist, 1e-6);
