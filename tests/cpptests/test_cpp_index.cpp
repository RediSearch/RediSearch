...
TEST_F(IndexTest, testUnion) {
  ...
  for (int cfg = 0; cfg < 2; ++cfg) {
    ...
    irs[1] = NewInvIndIterator_TermQuery(w2, &mockQctx.sctx, f, makeTestQueryTerm(), 1);
...
TEST_F(IndexTest, testWeight) {
  ...
  QueryIterator **irs = (QueryIterator **)rm_calloc(2, sizeof(QueryIterator *));
  MockQueryEvalCtx mockQctx(10, 10);
  irs[0] = NewInvIndIterator_TermQuery(w, &mockQctx.sctx, fieldMaskOrIndex, makeTestQueryTerm(), 0.5);
  FieldMaskOrIndex f = {.mask_tag = FieldMaskOrIndex_Mask, .mask = RS_FIELDMASK_ALL};
  irs[1] = NewInvIndIterator_TermQuery(w2, &mockQctx.sctx, f, makeTestQueryTerm(), 1);
  IteratorsConfig config{};
  iteratorsConfig_init(&config);
...
TEST_F(IndexTest, testHybridVector) {
  ...
  // Test in hybrid mode.
  FieldMaskOrIndex f = {.mask_tag = FieldMaskOrIndex_Mask, .mask = RS_FIELDMASK_ALL};
  QueryIterator *ir = NewInvIndIterator_TermQuery(w, &mockQctx.sctx, f, makeTestQueryTerm(), 1);
  hParams.childIt = ir;
  ...
  ir = NewInvIndIterator_TermQuery(w, &mockQctx.sctx, f, makeTestQueryTerm(), 1);
...
TEST_F(IndexTest, testDocTablePayloadSlotLifecycle) {
  ...
  // This reference must remain valid even when the payload first acquires storage.
  const RSDocumentMetadata *borrowed = DocTable_Borrow(&dt, id);
  ASSERT_EQ(borrowed, dmd);
  ...
  for (const auto &payload : payloads) {
    ...
    EXPECT_TRUE(hasPayload(borrowed->flags));
    ASSERT_NE(borrowed->payload, nullptr);
    EXPECT_EQ(std::string(borrowed->payload->data, borrowed->payload->len), payload);
    EXPECT_EQ(borrowed->payload->data[payload.size()], '\0');
  }

  DocTable_ClearPayload(&dt, dmd);
  EXPECT_EQ(dt.memsize, baseSize);
  EXPECT_FALSE(hasPayload(borrowed->flags));
  EXPECT_TRUE(borrowed->flags & Document_HasPayloadSlot);
  EXPECT_EQ(borrowed->payload, nullptr);
  DocTable_ClearPayload(&dt, dmd);
  EXPECT_EQ(dt.memsize, baseSize);
  ...
  EXPECT_EQ(borrowed->id, id);
  EXPECT_EQ(dt.maxDocId, id);
  EXPECT_EQ(dt.size, tableSize);
  EXPECT_FLOAT_EQ(borrowed->score, 0.5);
  EXPECT_EQ(borrowed->docLen, 7u);
  EXPECT_EQ(borrowed->maxTermFreq, 3u);
  EXPECT_EQ(borrowed->expirationTimeNs, 123456789);
  EXPECT_STREQ(borrowed->keyPtr, "doc");
  EXPECT_EQ(RSSortingVector_Length(&borrowed->sortVector), 0u);
...
TEST_F(IndexTest, testDocTableEnsureExclusivePreservesRetainedReaders) {
  DocTable dt = NewDocTable(1, 1);

  RSDocumentMetadata *lean =
      DocTable_Put(&dt, "lean", 4, 1.0, Document_DefaultFlags, nullptr, 0, DocumentType_Hash);
  const RSDocumentMetadata *leanReader = DocTable_Borrow(&dt, lean->id);
  ASSERT_EQ(leanReader, lean);
  const size_t leanMemsize = dt.memsize;
  RSDocumentMetadata *currentLean = DocTable_EnsureExclusive(&dt, lean);
  ASSERT_NE(currentLean, leanReader);
  EXPECT_EQ(currentLean->id, leanReader->id);
  EXPECT_NE(currentLean->keyPtr, leanReader->keyPtr);
  EXPECT_STREQ(currentLean->keyPtr, leanReader->keyPtr);
  EXPECT_EQ(dt.memsize, leanMemsize);

  RSDocumentMetadata *full =
      DocTable_Put(&dt, "full", 4, 0.5, Document_HasPayloadSlot, "payload", 7, DocumentType_Hash);
  full->docLen = 8;
  full->maxTermFreq = 3;
  full->expirationTimeNs = 123456789;
  RSSortingVector sortVector = RSSortingVector_New(2);
  RSSortingVector_PutStr(&sortVector, 0, "sortable");
  RSSortingVector_PutNum(&sortVector, 1, 2.5);
  ASSERT_EQ(DocTable_SetSortingVector(&dt, full, sortVector), 1);

  RSByteOffsets *byteOffsets = NewByteOffsets();
  RSByteOffsets_ReserveFields(byteOffsets, 1);
  RSByteOffsetField *field = RSByteOffsets_AddField(byteOffsets, 7, 1);
  field->lastTokPos = 2;
  ByteOffsetWriter writer;
  ByteOffsetWriter_Init(&writer);
  ByteOffsetWriter_Write(&writer, 4);
  ByteOffsetWriter_Write(&writer, 9);
  ByteOffsetWriter_Move(&writer, byteOffsets);
  ByteOffsetWriter_Cleanup(&writer);
  DocTable_SetByteOffsets(full, byteOffsets);

  const RSDocumentMetadata *fullReader = DocTable_Borrow(&dt, full->id);
  ASSERT_EQ(fullReader, full);
  ASSERT_EQ(fullReader->nextInChain, currentLean);
  const auto fullFlags = fullReader->flags;
  const size_t fullMemsize = dt.memsize;
  const size_t sortablesSize = dt.sortablesSize;
  const size_t tableSize = dt.size;
  const t_docId maxDocId = dt.maxDocId;
  uint32_t oldOffsetsLen = 0;
  const char *oldOffsets =
      RSOffsetVector_GetData(&fullReader->byteOffsets->offsets, &oldOffsetsLen);

  RSDocumentMetadata *currentFull = DocTable_EnsureExclusive(&dt, full);
  ASSERT_NE(currentFull, fullReader);
  EXPECT_EQ(currentFull->id, fullReader->id);
  EXPECT_EQ(currentFull->nextInChain, currentLean);
  EXPECT_EQ(fullReader->nextInChain, currentLean);
  EXPECT_EQ(dt.size, tableSize);
  EXPECT_EQ(dt.maxDocId, maxDocId);
  EXPECT_EQ(dt.memsize, fullMemsize);
  EXPECT_EQ(dt.sortablesSize, sortablesSize);
  EXPECT_EQ(currentFull->flags, fullFlags);
  EXPECT_FLOAT_EQ(currentFull->score, 0.5);
  EXPECT_EQ(currentFull->docLen, 8u);
  EXPECT_EQ(currentFull->maxTermFreq, 3u);
  EXPECT_EQ(currentFull->expirationTimeNs, 123456789);
  EXPECT_NE(currentFull->keyPtr, fullReader->keyPtr);
  EXPECT_STREQ(currentFull->keyPtr, fullReader->keyPtr);
  ASSERT_NE(currentFull->payload, fullReader->payload);
  EXPECT_NE(currentFull->payload->data, fullReader->payload->data);
  EXPECT_EQ(std::string(currentFull->payload->data, currentFull->payload->len), "payload");
  EXPECT_NE(currentFull->sortVector.ptr, fullReader->sortVector.ptr);
  EXPECT_EQ(RSSortingVector_Get(&currentFull->sortVector, 0),
            RSSortingVector_Get(&fullReader->sortVector, 0));
  EXPECT_EQ(RSSortingVector_Get(&currentFull->sortVector, 1),
            RSSortingVector_Get(&fullReader->sortVector, 1));
  ASSERT_NE(currentFull->byteOffsets, fullReader->byteOffsets);
  EXPECT_NE(currentFull->byteOffsets->fields, fullReader->byteOffsets->fields);
  EXPECT_EQ(currentFull->byteOffsets->numFields, 1);
  EXPECT_EQ(currentFull->byteOffsets->fields[0].fieldId, 7);
  EXPECT_EQ(currentFull->byteOffsets->fields[0].firstTokPos, 1u);
  EXPECT_EQ(currentFull->byteOffsets->fields[0].lastTokPos, 2u);
  uint32_t currentOffsetsLen = 0;
  const char *currentOffsets =
      RSOffsetVector_GetData(&currentFull->byteOffsets->offsets, &currentOffsetsLen);
  EXPECT_EQ(currentOffsetsLen, oldOffsetsLen);
  EXPECT_NE(currentOffsets, oldOffsets);
  EXPECT_EQ(std::string(currentOffsets, currentOffsetsLen), std::string(oldOffsets, oldOffsetsLen));

  RSDocumentMetadata *previousLean = currentLean;
  const RSDocumentMetadata *leanCollisionReader = DocTable_Borrow(&dt, previousLean->id);
  ASSERT_EQ(leanCollisionReader, previousLean);
  currentLean = DocTable_EnsureExclusive(&dt, currentLean);
  ASSERT_NE(currentLean, leanCollisionReader);
  EXPECT_EQ(currentLean->id, leanCollisionReader->id);
  EXPECT_EQ(currentFull->nextInChain, currentLean);
  EXPECT_EQ(fullReader->nextInChain, leanCollisionReader);
  EXPECT_EQ(dt.size, tableSize);
  EXPECT_EQ(dt.memsize, fullMemsize);
  EXPECT_EQ(dt.sortablesSize, sortablesSize);
  const RSDocumentMetadata *borrowedCollisionLean = DocTable_Borrow(&dt, currentLean->id);
  ASSERT_EQ(borrowedCollisionLean, currentLean);
  DMD_Return(borrowedCollisionLean);

  currentFull->keyPtr[0] = 'F';
  EXPECT_STREQ(fullReader->keyPtr, "full");
  ASSERT_EQ(DocTable_SetPayload(&dt, currentFull, "replacement", 11), 1);
  EXPECT_EQ(dt.memsize, fullMemsize + 4);
  EXPECT_EQ(std::string(fullReader->payload->data, fullReader->payload->len), "payload");
  RSSortingVector_PutNum(&currentFull->sortVector, 1, 9.0);
  EXPECT_DOUBLE_EQ(RSValue_Number_Get(RSSortingVector_Get(&fullReader->sortVector, 1)), 2.5);
  currentFull->byteOffsets->fields[0].fieldId = 9;
  EXPECT_EQ(fullReader->byteOffsets->fields[0].fieldId, 7);

  const RSDocumentMetadata *borrowedLean = DocTable_Borrow(&dt, currentLean->id);
  ASSERT_EQ(borrowedLean, currentLean);
  DMD_Return(borrowedLean);
  const RSDocumentMetadata *borrowedFull = DocTable_Borrow(&dt, currentFull->id);
  ASSERT_EQ(borrowedFull, currentFull);
  DMD_Return(borrowedFull);

  currentFull = DocTable_EnsureExclusive(&dt, currentFull);
  EXPECT_EQ(DocTable_EnsureExclusive(&dt, currentLean), currentLean);
  DMD_Return(currentFull);
  DMD_Return(currentLean);
  DocTable_Free(&dt);

  EXPECT_EQ(leanReader->flags, Document_DefaultFlags);
  EXPECT_STREQ(leanReader->keyPtr, "lean");
  EXPECT_EQ(fullReader->flags, fullFlags);
  EXPECT_STREQ(fullReader->keyPtr, "full");
  EXPECT_EQ(std::string(fullReader->payload->data, fullReader->payload->len), "payload");
  size_t retainedStringLen = 0;
  const char *retainedString =
      RSValue_StringPtrLen(RSSortingVector_Get(&fullReader->sortVector, 0), &retainedStringLen);
  EXPECT_EQ(std::string(retainedString, retainedStringLen), "sortable");
  EXPECT_DOUBLE_EQ(RSValue_Number_Get(RSSortingVector_Get(&fullReader->sortVector, 1)), 2.5);
  uint32_t retainedOffsetsLen = 0;
  const char *retainedOffsets =
      RSOffsetVector_GetData(&fullReader->byteOffsets->offsets, &retainedOffsetsLen);
  EXPECT_EQ(retainedOffsetsLen, oldOffsetsLen);
  EXPECT_EQ(std::string(retainedOffsets, retainedOffsetsLen),
            std::string(oldOffsets, oldOffsetsLen));
  RSByteOffsetIterator iterator;
  ASSERT_EQ(RSByteOffset_Iterate(fullReader->byteOffsets, 7, &iterator), REDISMODULE_OK);
  EXPECT_EQ(RSByteOffsetIterator_Next(&iterator), 4u);
  EXPECT_EQ(RSByteOffsetIterator_Next(&iterator), 9u);
  EXPECT_EQ(RSByteOffsetIterator_Next(&iterator), RSBYTEOFFSET_EOF);
  EXPECT_EQ(fullReader->nextInChain, leanCollisionReader);
  EXPECT_EQ(leanCollisionReader->flags, Document_DefaultFlags);
  EXPECT_STREQ(leanCollisionReader->keyPtr, "lean");
  DMD_Return(leanReader);
  DMD_Return(fullReader);
  DMD_Return(leanCollisionReader);
}
...
TEST_F(IndexTest, testDocTable) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    ASSERT_EQ((int)dmd->flags, (int)(Document_HasPayloadSlot | Document_HasPayload));
...
TEST_F(IndexTest, testRawDocId) {
  ...
  QueryIterator *ir = NewInvIndIterator_TermQuery(idx, &mockQctx.sctx, f, makeTestQueryTerm(), 1);
  for (t_docId id = 1; id < 100; id += 2) {
