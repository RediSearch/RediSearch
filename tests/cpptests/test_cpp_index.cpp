...
TEST_P(IndexFlagsTest, testRWFlags) {
    ...
    size_t expectedIndexSize =
        exp_idx_no_block_memsize + exp_block_memsize + INDEX_BLOCK_INITIAL_CAP;
...
TEST_F(IndexTest, testNumericEncoding) { testNumericEncodingHelper(0); }

TEST_F(IndexTest, testNumericEncodingMulti) { testNumericEncodingHelper(1); }
...
TEST_F(IndexTest, testHybridVector) {
    ...
    // Expect to get top 10 results in the right order of the distance that passes the filter: 400,
    // 396, ..., 364.
    count = 0;
    ...
    while (hybridIt->Read(hybridIt->ctx, &h) != INDEXREAD_EOF) {
        ASSERT_EQ(h->type, RSResultType_Metric);
        // since larger ids has lower distance, in every we get higher id (where max id is the final
        // result).
        size_t expected_id = max_id - step * (count++);
        ASSERT_EQ(h->docId, expected_id);
    }
...
TEST_F(IndexTest, testMetric_VectorRange) {
    ...
    VecSimQueryReply *results = VecSimIndex_RangeQuery(
...
TEST_F(IndexTest, testIndexSpec) {
    ...
    ASSERT_EQ(f->options,
              FieldSpec_Sortable | FieldSpec_UNF); // UNF is set implicitly for sortable numerics
    ASSERT_TRUE(f->sortIdx == 1);
...
TEST_F(IndexTest, testDocTable) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        RSDocumentMetadata *dmd = DocTable_Put(&dt, buf, nkey, (double)i, Document_DefaultFlags,
                                               buf, strlen(buf), DocumentType_Hash);
        t_docId nd = dmd->id;
        DMD_Return(dmd);
        ASSERT_EQ(did + 1, nd);
        did = nd;
    ...
    RSDocumentMetadata *dmd =
        DocTable_Put(&dt, "Hello", 5, 1.0, Document_DefaultFlags, NULL, 0, DocumentType_Hash);
