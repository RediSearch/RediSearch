...
//       IndexDecoderCtx decoderCtx = {.fieldmask_tag = IndexDecoderCtx_FieldMask,
...
//       res->fieldMask = RS_FIELDMASK_ALL;
  ...
  //   FieldMaskOrIndex fieldMaskOrIndex = {.index_tag = FieldMaskOrIndex_Index, .index =
  //   RS_INVALID_FIELD_INDEX}; QueryIterator **irs = (QueryIterator **)rm_calloc(2,
  //   sizeof(QueryIterator *)); MockQueryEvalCtx mockQctx(10, 10); irs[0] =
  //   NewInvIndIterator_TermQuery(w, &mockQctx.sctx, fieldMaskOrIndex, makeTestQueryTerm(), 0.5);
  //   FieldMaskOrIndex f = {.mask_tag = FieldMaskOrIndex_Mask, .mask = RS_FIELDMASK_ALL};
  //   irs[1] = NewInvIndIterator_TermQuery(w2, &mockQctx.sctx, f, makeTestQueryTerm(), 1);
  //   IteratorsConfig config{};
  //   iteratorsConfig_init(&config);
  ...
  //   irs[1] = NewNotIterator(NewInvIndIterator_TermQuery(w2, &mockQctx2.sctx, f,
  //   makeTestQueryTerm(), 1), InvertedIndex_LastId(w2), 1, {0}, &ctx->qctx);
...
//    irs[1] = NewNotIterator(NewInvIndIterator_TermQuery(w2, &mockQctx2.sctx, f, makeTestQueryTerm(), 1), InvertedIndex_LastId(w2), 1, {0}, NULL, &ctx->qctx);
  ...
  //   FieldMaskOrIndex f = {.mask_tag = FieldMaskOrIndex_Mask, .mask = RS_FIELDMASK_ALL};
  //   MockQueryEvalCtx mockQctx(10, 10);
  //   QueryIterator *ir = NewNotIterator(NewInvIndIterator_TermQuery(w, &mockQctx.sctx, f,
  //   makeTestQueryTerm(), 1), InvertedIndex_LastId(w) + 5, 1, {0}, &ctx->qctx);
...
//   QueryIterator *ir = NewNotIterator(NewInvIndIterator_TermQuery(w, &mockQctx.sctx, f, makeTestQueryTerm(), 1), InvertedIndex_LastId(w) + 5, 1, {0}, NULL, &ctx->qctx);
  ...
  //   FieldMaskOrIndex fieldMaskOrIndex = {.index_tag = FieldMaskOrIndex_Index, .index =
  //   RS_INVALID_FIELD_INDEX}; FieldFilterContext filterCtx = {.field = fieldMaskOrIndex,
  ...
  //   // Expect to get top 10 results in reverse order of the distance that passes the filter: 364,
  //   368, ..., 400. while (vecIt->Read(vecIt) != ITERATOR_EOF) {
  ...
  //   // Test in hybrid mode.
  //   FieldMaskOrIndex f = {.mask_tag = FieldMaskOrIndex_Mask, .mask = RS_FIELDMASK_ALL};
  //   QueryIterator *ir = NewInvIndIterator_TermQuery(w, &mockQctx.sctx, f, makeTestQueryTerm(),
  //   1); hParams.childIt = ir; QueryIterator *hybridIt = NewHybridVectorIterator(hParams, &err);
  //   ASSERT_FALSE(QueryError_HasError(&err)) << QueryError_GetUserError(&err);

  //   HybridIterator *hr = (HybridIterator *)hybridIt;
  //   hr->searchMode = VECSIM_HYBRID_BATCHES;

  //   // Expect to get top 10 results in the right order of the distance that passes the filter:
  //   400, 396, ..., 364. count = 0; while (hybridIt->Read(hybridIt) != ITERATOR_EOF) {
  //     ASSERT_EQ(hybridIt->current->data.tag, RSResultData_Metric);
  //     // since larger ids has lower distance, in every we get lower id (where max id is the final
  //     result). size_t expected_id = max_id - step*(count++); ASSERT_EQ(hybridIt->lastDocId,
  //     expected_id);
  //   }
  //   ASSERT_EQ(count, k);
  ...
  //     ASSERT_EQ(hybridIt->current->data.tag, RSResultData_Metric);
  //     // since larger ids has lower distance, in every we get higher id (where max id is the
  //     final result). size_t expected_id = max_id - step*(count++); ASSERT_EQ(hybridIt->lastDocId,
  //     expected_id);
  //   }
  ...
  //   // This time, result is a tree with 2 children: vector score and subtree of terms (for
  //   scoring). count = 0; while (hybridIt->Read(hybridIt) != ITERATOR_EOF) {
  ...
  //     // since larger ids has lower distance, in every we get higher id (where max id is the
  //     final result). size_t expected_id = max_id - step*(count++); ASSERT_EQ(h->docId,
  //     expected_id);
  //   }
  //   ASSERT_EQ(count, k);
  //   ASSERT_TRUE(hybridIt->atEOF);
  ...
  //       VecSimIndex_RangeQuery(index, range_query.vector, range_query.radius, &queryParams,
  //       range_query.order);
  ...
  //   QueryIterator *metric_it = NewMetricIteratorSortedById(ids_arr, metrics_arr, results_num,
  //   VECTOR_DISTANCE);

  //   // Copy the behaviour of INV_IDX_ITERATOR in terms of SkipTo. That is, the iterator will
  //   return the
  ...
  //   StrongRef ref = IndexSpec_ParseC(NULL, spec_name, args, sizeof(args) / sizeof(const char *),
  //   &err); IndexSpec *s = (IndexSpec *)StrongRef_Get(ref);
  //   ASSERT_FALSE(QueryError_HasError(&err)) << QueryError_GetUserError(&err);
  //   ASSERT_TRUE(s);
  ...
  //   ASSERT_EQ(f->options, FieldSpec_Sortable | FieldSpec_UNF); // UNF is set implicitly for
  //   sortable numerics ASSERT_TRUE(f->sortIdx == 1); ASSERT_TRUE(IndexSpec_GetFieldWithLength(s,
  //   "fooz", 4) == NULL);
  ...
  //   };
  //   QueryError_ClearError(&err);
  //   ref = IndexSpec_ParseC(NULL, "idx", args_invalid, sizeof(args_invalid) /
  //   sizeof(args_invalid[0]), &err); ASSERT_TRUE(QueryError_HasError(&err)); ASSERT_EQ(nullptr,
  //   StrongRef_Get(ref)); ASSERT_NE(nullptr, strstr(QueryError_GetUserError(&err), "MAXTEXTFIELDS
  //   cannot be used with NOFIELDS"));
  ...
  //   ASSERT_STREQ("SEARCH_LIMIT_OVER Schema is limited to 128 TEXT fields",
  //   QueryError_GetUserError(&err));
  // #else
  //   ASSERT_STREQ("SEARCH_LIMIT_OVER Schema is limited to 64 TEXT fields",
  //   QueryError_GetUserError(&err));
  // #endif
  //   freeSchemaArgs(args);
  //   QueryError_ClearError(&err);
  ...
  //     RSDocumentMetadata *dmd = DocTable_Put(&dt, buf, nkey, (double)i, Document_DefaultFlags,
  //     buf, strlen(buf), DocumentType_Hash); t_docId nd = dmd->id; DMD_Return(dmd); ASSERT_EQ(did
  //     + 1, nd); did = nd;
  //   }
  ...
  //   DocumentType_Hash); t_docId strDocId = dmd->id; ASSERT_TRUE(0 != strDocId); ASSERT_EQ(71 +
  //   doc_table_size, (int)dt.memsize);
  ...
  //   DocumentType_Hash); ASSERT_TRUE(dmd); ASSERT_EQ(148 + doc_table_size, (int)dt.memsize);
  ...
  //   InvertedIndex *idx = NewInvertedIndex((IndexFlags)(INDEX_DEFAULT_FLAGS | Index_WideSchema),
  //   &index_memsize); ForwardIndexEntry ent = {0}; ent.docId = 1; ent.fieldMask =
  //   RS_FIELDMASK_ALL;
...
//   QueryIterator *ir = NewInvIndIterator_TermQuery(idx, &mockQctx.sctx, f, makeTestQueryTerm(),
//   1); for (t_docId id = 1; id < 100; id += 2) {
...
//       ASSERT_EQ(id + 1, cur->docId) << "Expected to skip to " << id + 1 << " but got " <<
//       cur->docId;
//     } else {
...
//   VecSimQueryParams queryParams = {0};
//   KNNVectorQuery top_k_query = {.vector = NULL, .vecLen = d, .k = k, .order = BY_SCORE};
//   FieldFilterContext filterCtx = {.field = {.index_tag = FieldMaskOrIndex_Index, .index =
//   RS_INVALID_FIELD_INDEX}, .predicate = FIELD_EXPIRATION_PREDICATE_DEFAULT};
