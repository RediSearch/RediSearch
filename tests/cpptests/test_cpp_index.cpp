...
TEST_F(IndexTest, testMetric_VectorRange) {
  ...
  while (vecIt->Read(vecIt) != ITERATOR_EOF) {
    ...
    ASSERT_EQ(RSValue_Number_Get(MetricsVec_AsSlice(&h->metrics).data[0].value), exp_dist);
    count++;
  }
  ...
  ASSERT_EQ(RSValue_Number_Get(MetricsVec_AsSlice(&vecIt->current->metrics).data[0].value), exp_dist);
