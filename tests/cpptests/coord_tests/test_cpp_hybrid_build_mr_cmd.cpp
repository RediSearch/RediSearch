...
protected:
    ...
    void testShardKRatioTransformation(const std::vector<const char*>& inputArgs,
        ...
        // Build MR command.
        MRCommand xcmd;
        HybridRequest_buildMRCommand(args, args.size(), &shardWireParams,
                                     &xcmd, nullptr, testIndexSpec,
  ...
  void testCommandTransformationWithIndexSpecAndParams(
      ...
      // ... WITHCURSOR WITHSCORES _NUM_SSTRING _SLOTS <blob> _INDEX_PREFIXES 2 prefix1 prefix2
      EXPECT_STREQ(xcmd.strs[xcmd.num - 9], "WITHCURSOR") << "WITHCURSOR should be 9th to last";
      EXPECT_STREQ(xcmd.strs[xcmd.num - 8], "WITHSCORES") << "WITHSCORES should be 8th to last";
      EXPECT_STREQ(xcmd.strs[xcmd.num - 7], "_NUM_SSTRING") << "_NUM_SSTRING should be 7th to last";
      EXPECT_STREQ(xcmd.strs[xcmd.num - 6], SLOTS_STR) << SLOTS_STR << " should be 6th to last";
      // slots blob is 5th to last (xcmd.num - 5)
