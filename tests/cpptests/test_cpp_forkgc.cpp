 ...
 * We always prefer the parent process last block. If it was simultaneously modified by both the
 * child and the parent, we take the parent's version.
 *
...
static InvertedIndex *getTagInvidx(RedisSearchCtx *sctx, const char *field, const char *value) {
...
TEST_F(FGCTest, testRemoveEntryFromLastBlock) {
    ...
    ASSERT_EQ(invertedSizeBeforeApply - fgc->stats.totalCollected,
...
TEST_F(FGCTest, testRepairLastBlockWhileRemovingMiddle) {
    ...
    // Since we added entries to the last block after the fork, we ignore the fork updates in the
    // last block
    ASSERT_EQ(1, fgc->stats.gcBlocksDenied);
...
TEST_F(FGCTest, testRepairLastBlock) {
    ...
    RS::addDocument(ctx, ism, buf, "f1", "hello");
    FGC_Apply(fgc);
    // since the block size in the main process doesn't equal to its original size as seen by the
    // child, we ignore the fork collection - the last block changes should be discarded.
    ASSERT_EQ(1, fgc->stats.gcBlocksDenied);
    ASSERT_EQ(2, iv->size);
}
