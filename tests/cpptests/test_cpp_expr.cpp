...
public:
    static void SetUpTestCase() { RegisterAllFunctions(); }
};
...
struct TEvalCtx : ExprEval {
    ...
    int eval() { return ExprEval_Eval(this, &res_s); }

    TEvalCtx operator=(TEvalCtx &) = delete;
    TEvalCtx(const TEvalCtx &) = delete;

    RSValue &result() { return res_s; }

    const char *error() const { return QueryError_GetError(&status_s); }

    operator bool() const { return root && !QueryError_HasError(&status_s); }
...
struct EvalResult {
    ...
    static EvalResult ok(double rv) { return {rv, true}; }
};
