...
TEST_F(ExprTest, testAndExpressionWithMissingFields) {
  ...
  RLookupRow row = {0};
  RLookup_WriteOwnKey(kd2, &row, RS_NumVal(1));
  ...
  // Expected: false (0) because d2==0 is false
  {
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(0, res->numval) << "@d1==0 && @d2==0 should be false when d1 is missing and d2=1";
  }
  ...
  // Expected: false (0) - same result as Test 1, regardless of order
  {
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(0, res->numval) << "@d2==0 && @d1==0 should be false when d2=1 and d1 is missing";
  }
  ...
  // Expected: false (0) because d1 is missing (treated as false)
  {
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(0, res->numval) << "@d1==1 && @d2==1 should be false when d1 is missing";
  }
  ...
  // Expected: false (0) - same result as Test 3, regardless of order
  {
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(0, res->numval) << "@d2==1 && @d1==1 should be false when d1 is missing";
  }
  ...
  // Expected: false (0)
  {
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(0, res->numval) << "@d1==0 && @d2==0 should be false when both fields are missing";
  ...
  // Expected: true (1)
  {
    ...
    RLookup_WriteOwnKey(kd1_full, &row_full, RS_NumVal(5));
    RLookup_WriteOwnKey(kd2_full, &row_full, RS_NumVal(5));
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(1, res->numval) << "@d1==5 && @d2==5 should be true when both fields are 5";

    RLookupRow_Cleanup(&row_full);
...
TEST_F(ExprTest, testOrExpressionWithMissingFields) {
  ...
  // Expected: true (1) because d2==1 is true
  {
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(1, res->numval) << "@d1==1 || @d2==1 should be true when d1 is missing and d2=1";
  }
  ...
  // Expected: true (1) - same result as Test 1, regardless of order
  {
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(1, res->numval) << "@d2==1 || @d1==1 should be true when d2=1 and d1 is missing";
  }
  ...
  // Expected: false (0)
  {
    ...
    auto res = RSValue_Dereference(&ctx.result());
    ASSERT_EQ(RSValue_Number, res->t);
    EXPECT_EQ(0, res->numval) << "@d1==1 || @d2==1 should be false when both fields are missing";
