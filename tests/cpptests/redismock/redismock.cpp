...
void RMCK_RetainString(RedisModuleCtx *ctx, RedisModuleString *s) { s->incref(); }
...
void RMCK_TrimStringAllocation(RedisModuleString *s) { s->trim(); }
...
int RMCK_CreateSubcommand(RedisModuleCommand *parent, const char *s, RedisModuleCmdFunc handler,
...
/** Allocators */
void *RMCK_Alloc(size_t n) { return malloc(n); }

void RMCK_Free(void *p) { free(p); }

void *RMCK_Calloc(size_t nmemb, size_t size) { return calloc(nmemb, size); }

void *RMCK_Realloc(void *p, size_t n) { return realloc(p, n); }

char *RMCK_Strdup(const char *s) { return strdup(s); }

#define REPLY_FUNC(basename, ...)                                                                  \
    int RMCK_Reply##basename(RedisModuleCtx *, __VA_ARGS__) { return REDISMODULE_OK; }
...
int RMCK_ReplyWithNull(RedisModuleCtx *) { return REDISMODULE_OK; }

int RMCK_ReplySetArrayLength(RedisModuleCtx *, size_t) { return REDISMODULE_OK; }
...
void RMCK_FreeThreadSafeContext(RedisModuleCtx *ctx) { delete ctx; }

void RMCK_AutoMemory(RedisModuleCtx *ctx) { ctx->automemory = true; }

void RMCK_ThreadSafeContextLock(RedisModuleCtx *) { RMCK_GlobalLock.lock(); }

void RMCK_ThreadSafeContextUnlock(RedisModuleCtx *) { RMCK_GlobalLock.unlock(); }
...
int RMCK_CallReplyType(RedisModuleCallReply *r) { return r->type; }

void RMCK_FreeCallReply(RedisModuleCallReply *r) { delete r; }
...
/** Fork */
static int RMCK_Fork(RedisModuleForkDoneHandler cb, void *user_data) { return fork(); }

static int RMCK_ExitFromChild(int retcode) { _exit(retcode); }

static int RMCK_KillForkChild(int child_pid) { return waitpid(child_pid, NULL, 0); }
...
static void *RMCK_GetSharedAPI(RedisModuleCtx *, const char *name) { return fnregistry[name]; }
