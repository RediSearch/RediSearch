...
TEST_F(LLApiTest, testAddDocumentGeoField) {
    ...
    const char *id;
    // radius < 0
    RSQNode *qn = RediSearch_CreateGeoNode(index, GEO_FIELD_NAME, 20.6543222, 0.123455, -1,
                                           RS_GEO_DISTANCE_M);
    RSResultsIterator *iter = RediSearch_GetResultsIterator(qn, index);
    ASSERT_FALSE(iter);
    ...
    // searching on the index
    qn = RediSearch_CreateGeoNode(index, GEO_FIELD_NAME, 20.6543222, 0.123455, 10,
                                  RS_GEO_DISTANCE_M);
    iter = RediSearch_GetResultsIterator(qn, index);
    ASSERT_TRUE(iter);
    ...
    qn = RediSearch_CreateGeoNode(index, GEO_FIELD_NAME, 20.6543000, 0.123000, 10,
                                  RS_GEO_DISTANCE_M);
    iter = RediSearch_GetResultsIterator(qn, index);
    ASSERT_TRUE(iter != NULL);
...
const char *words[] = {"he",    "her",         "hell",  "help",   "helper",
                       "hello", "hello world", "towel", "dealer", "bell"};
...
TEST_F(LLApiTest, duplicateFieldAdd) {
    ...
    RediSearch_DocumentAddFieldCString(d, FIELD_NAME_1, "some test to same field",
                                       RSFLDTYPE_DEFAULT);
...
TEST_F(LLApiTest, testScorer) {
    ...
    RediSearch_DocumentAddFieldCString(d1, FIELD_NAME_1, "hello world hello world",
                                       RSFLDTYPE_DEFAULT);
...
TEST_F(LLApiTest, testInfo) {
    ...
    RediSearch_CreateField(index, "dynamic1",
                           RSFLDTYPE_FULLTEXT | RSFLDTYPE_NUMERIC | RSFLDTYPE_TAG | RSFLDTYPE_GEO,
                           RSFLDOPT_NONE);
