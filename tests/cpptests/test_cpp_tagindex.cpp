...
TEST_F(TagIndexTest, testCreate) {
  ...
  // Buffer grows up to 1024 bytes trying to store 1000 bytes - it doubles each time
  size_t buffer_cap = 1024;
  size_t num_blocks = N / 1000;

  // The size of the inverted index structure is 48 bytes
  ...
  // The entry requires a capacity of 56 bytes
  size_t last_block_size = 48 + 56;
