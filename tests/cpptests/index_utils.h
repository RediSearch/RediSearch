...
/**
 * Iterates the inverted indices in the numeric tree and calculates the memory used by them.
 * This memory includes memory allocated for data and blocks metadata.
 * NOTE: the returned memory doesn't include the memory used by the tree itself.
 *
 * If @param rt is NULL, the function will return 0.
 *
 * @param failed_range is kept for API compatibility but will never be set.
 * With the Rust port, stats consistency is maintained internally by Rust.
 */
