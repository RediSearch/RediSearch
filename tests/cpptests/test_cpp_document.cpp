...
TEST_F(DocumentTest, testLoadAll) {
  ...
  ASSERT_FALSE(f == NULL);
  ASSERT_STREQ("ni2", RediSearch_HiddenStringGet(f->docFieldName, false));
  ASSERT_TRUE(0 == RedisModule_StringCompare(f->text, RMCK::RString("foo2")));
  f = Document_GetField(&d, "ni1");
  ASSERT_FALSE(f == NULL);
  ASSERT_STREQ("ni1", RediSearch_HiddenStringGet(f->docFieldName, false));
...
TEST_F(DocumentTest, testLoadSchema) {
  ...
  ASSERT_FALSE(f == NULL);
  ASSERT_STREQ("t1", RediSearch_HiddenStringGet(f->docFieldName, false));
  ASSERT_EQ(0, RedisModule_StringCompare(RMCK::RString("Hello World"), f->text));

  f = Document_GetField(&d, "t2");
  ASSERT_FALSE(f == NULL);
  ASSERT_STREQ("t2", RediSearch_HiddenStringGet(f->docFieldName, false));
