...
TEST_F(DocIdMetaTest, TestKeyCopyWithMetadata) {

  // Set up some document IDs on the source key
  uint64_t docId1 = 100;
  uint64_t docId2 = 200;
  uint64_t docId3 = 300;
  ...
  // Verify the source key has the expected values
  uint64_t retrievedDocId;
  EXPECT_EQ(DocIdMeta_GetDocIdForIndex(testKey, 0, &retrievedDocId), REDISMODULE_OK);
  EXPECT_EQ(retrievedDocId, docId1);
  EXPECT_EQ(DocIdMeta_GetDocIdForIndex(testKey, 1, &retrievedDocId), REDISMODULE_OK);
  EXPECT_EQ(retrievedDocId, docId2);
  EXPECT_EQ(DocIdMeta_GetDocIdForIndex(testKey, 5, &retrievedDocId), REDISMODULE_OK);
  EXPECT_EQ(retrievedDocId, docId3);

  // Create source and destination key strings
  RedisModuleString *srcKeyStr = RedisModule_CreateString(ctx, "testkey", 7);
  RedisModuleString *dstKeyStr = RedisModule_CreateString(ctx, "copiedkey", 9);

  // Copy the key (this should trigger the copy callback for metadata)
  EXPECT_EQ(RMCK_CopyKey(ctx, srcKeyStr, dstKeyStr), REDISMODULE_OK);

  // Open the destination key and verify the metadata was copied
  RedisModuleKey *copiedKey = RedisModule_OpenKey(ctx, dstKeyStr, REDISMODULE_READ);
  ASSERT_NE(copiedKey, nullptr);

  // Verify that the copied key has the same document IDs
  EXPECT_EQ(DocIdMeta_GetDocIdForIndex(copiedKey, 0, &retrievedDocId), REDISMODULE_OK);
  EXPECT_EQ(retrievedDocId, docId1);
  EXPECT_EQ(DocIdMeta_GetDocIdForIndex(copiedKey, 1, &retrievedDocId), REDISMODULE_OK);
  EXPECT_EQ(retrievedDocId, docId2);
  EXPECT_EQ(DocIdMeta_GetDocIdForIndex(copiedKey, 5, &retrievedDocId), REDISMODULE_OK);
  EXPECT_EQ(retrievedDocId, docId3);

  // Verify that modifying the original doesn't affect the copy (deep copy test)
  EXPECT_EQ(DocIdMeta_SetDocIdForIndex(testKey, 0, 999), REDISMODULE_OK);

  // The copied key should still have the original value
  EXPECT_EQ(DocIdMeta_GetDocIdForIndex(copiedKey, 0, &retrievedDocId), REDISMODULE_OK);
  EXPECT_EQ(retrievedDocId, docId1); // Should still be 100, not 999

  // Clean up
  RedisModule_CloseKey(copiedKey);
  RedisModule_FreeString(ctx, srcKeyStr);
  RedisModule_FreeString(ctx, dstKeyStr);
}
