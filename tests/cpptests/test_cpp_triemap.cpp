...
int testTMNumResults(TrieMap *t, const char *str, tm_iter_mode mode) {
  ...
  TrieMapIterator *it = TrieMap_Iterate(t, str, strlen(str), mode);
  affixFunc *f = TrieMapIterator_Next;
...
TEST_F(TrieMapTest, testLexOrder) {
  ...
  TrieMapIterator *iter = TrieMap_Iterate(t, "", 0, TM_ITER_MODE_DEFAULT);
  checkNext(iter, "bell");
  ...
  iter = TrieMap_Iterate(t, "", 0, TM_ITER_MODE_DEFAULT);
  checkNext(iter, "bell");
  checkNext(iter, "he");
  checkNext(iter, "hell");
  checkNext(iter, "hello");
