...
public:
    QASTCXX() { memset(static_cast<QueryAST *>(this), 0, sizeof(QueryAST)); }
    QASTCXX(RedisSearchCtx &sctx) : QASTCXX() { setContext(&sctx); }
    void setContext(RedisSearchCtx *sctx) { this->sctx = sctx; }

    bool parse(const char *s) { return parse(s, 1); }
    ...
    void print() const { QAST_Print(this, sctx->spec); }

    const char *getError() const { return QueryError_GetError(&m_status); }
...
TEST_F(QueryTest, testParser_v1) {
    ...
    assertValidQuery(
        "@title:((hello world)|((hello world)|(hallo world|werld) | hello world werld))", ctx);
...
TEST_F(QueryTest, testParser_v2) {
    ...
    assertValidQuery("*=>[KNN $KNN @KNN $KNN KNN $KNN AS $AS]",
                     ctx); // using reserved word as an attribute or field
    ...
    assertValidQuery("*=>[knn $K @vec_field $BLOB]=>{$yield_distance_as: as;}",
                     ctx); // using stop-word as the attribute value
    assertValidQuery("*=>[KNN $KNN @KNN $KNN KNN $KNN]=>{$yield_distance_as: VECTOR_RANGE;}",
                     ctx); // using reserved word as an attribute or field
    assertValidQuery(
        "*=>[KNN $K @vec_field $BLOB] =>{$yield_distance_as: vec_dist; $ef_runtime: 100;}", ctx);
    assertValidQuery("*=>[KNN $K @vec_field $BLOB] =>{$weight: 2.0; $ef_runtime: 100;}",
                     ctx); // weight is valid, but ignored
    ...
    assertInvalidQuery("*=>[KNN $K @vec_field BLOB]",
                       ctx); // pass vector as value (must be an attribute)
    assertInvalidQuery("*=>[KNN $K vec_field $BLOB]", ctx); // wrong field value (must be @field)
    assertInvalidQuery("*=>[KNN K @vec_field $BLOB]",
                       ctx); // wrong k value (can be an attribute or integer)
    assertInvalidQuery("*=>[KNN 3.14 @vec_field $BLOB]",
                       ctx); // wrong k value (can be an attribute or integer)
    assertInvalidQuery("*=>[KNN -42 @vec_field $BLOB]",
                       ctx); // wrong k value (can be an attribute or integer)
    assertInvalidQuery("*=>[KNN $K @vec_field $BLOB $EF ef foo bar x 5 AS score]",
                       ctx); // parameter as attribute
    assertInvalidQuery("*=>[KNN $K @vec_field $BLOB EF ef foo bar x 5 AS ]",
                       ctx); // not specifying score field name
    assertInvalidQuery("*=>[KNN $K @vec_field $BLOB EF ef foo bar x]",
                       ctx); // missing parameter value (passing only key)
    assertInvalidQuery("*=>[KNN $K @vec_field $BLOB => {$yield:dist}]",
                       ctx); // invalid attributes syntax
    assertInvalidQuery("*=>[KNN $K @vec_field $BLOB EF_RUNTIME 100 => {$yield_distance_as:dist;}]",
                       ctx); // invalid combined syntax
    assertInvalidQuery("*=>[KNN $K @vec_field $BLOB EF_RUNTIME 100] => {$bad_attr:dist;}",
                       ctx); // invalid vector attribute
    ...
    assertValidQuery("@title:hello=>[KNN 10 @vec_field $BLOB]", ctx);
    assertValidQuery("@title:hello=>[KNN 10 @vec_field $BLOB EF_RUNTIME 100 HYBRID_POLICY BATCHES]",
                     ctx);
    ...
    assertValidQuery("hello=>[KNN 10 @vec_field $BLOB] => {$yield_distance_as:score; "
                     "$hybrid_policy:batches; $BATCH_SIZE:100}",
                     ctx);
    ...
    assertValidQuery(
        "(@tit_le|bo_dy:barack @body|title|url|something_else:obama)=>[KNN 10 @vec_field $BLOB]",
        ctx);
    ...
    assertInvalidQuery(
        "hello world => [KNN 10 @vec_field $BLOB] OR other => [KNN 10 @vec_field $BLOB]", ctx);
    ...
    assertValidQuery("@v:[VECTOR_RANGE 0.01 $BLOB]=>{$epsilon: 0.01; $yield_distance_as: V_SCORE;}",
                     ctx);
    assertValidQuery("@v:[VECTOR_RANGE $r $BLOB]=>{$epsilon: 0.01; $yield_distance_as: V_SCORE;}",
                     ctx);
    ...
    assertValidQuery("@v:[VECTOR_RANGE 0.01 $BLOB] @text:foo OR bar @v:[VECTOR_RANGE 0.04 $BLOB2]",
                     ctx);
    ...
    assertValidQuery("@v:[VECTOR_RANGE 0.01 $BLOB]=>{$yield_distance_as: score1;} => [KNN 5 @v2 "
                     "$BLOB2 AS second_score]",
                     ctx);
    assertValidQuery("@v:[VECTOR_RANGE 0.01 $BLOB]=>{$yield_distance_as: score1;} => [KNN 5 @v2 "
                     "$BLOB2] => {$yield_distance_as:second_score;}",
                     ctx);
    assertValidQuery("@v:[VECTOR_RANGE 0.01 $BLOB] VECTOR_RANGE",
                     ctx); // Fallback VECTOR_RANGE into a term.
...
TEST_F(QueryTest, testVectorHybridQuery) {
    ...
    const char *vqt[] = {"(hello world)=>[KNN 10 @vec $BLOB]",
                         "@title:(hello|world)=>[KNN 10 @vec $BLOB]",
                         "@title:hello=>[KNN 10 @vec $BLOB]", NULL};
...
TEST_F(QueryTest, testPureNegative) {
    const char *qs[] = {"-@title:hello", "-hello", "@title:-hello", "-(foo)", "-foo",
                        "(-foo)",        NULL};
...
TEST_F(QueryTest, testAttributes) {
    ...
    const char *qt = "(@title:(foo bar) => {$weight: 0.5} @body:lol => {$weight: 0.2}) => "
