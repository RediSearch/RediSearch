...
void run_hybrid_benchmark(VecSimIndex *index, size_t max_id, size_t d, std::mt19937 rng,
    ...
    for (size_t k = 10; k <= 100; k *= 10) {
        for (size_t percent = 100; percent <= 500; percent += 100) {
            ...
            // Create a union iterator - the number of results that the iterator should return is
            // determined based on the current <percent>. Every child iterator of the union contains
            // ids: [i, step+i, 2*step+i , ...]
            ...
            VecSimQueryParams queryParams = {.hnswRuntimeParams =
                                                 HNSWRuntimeParams{.efRuntime = 0}};
            ...
            for (size_t it = 0; it < NUM_ITERATIONS; it++) {
                for (size_t i = 0; i < k; i++) {
                    ...
                    if (!found) {
                        // std::cout << "iter: "<< it <<" id wasn't found: " << hnsw_ids[it][i] <<
                        // std::endl;
                    }
                }
            }
 ...
 * - BATCHES - get a batch of the next top vectors in the vector index, and then filter, until we
 * reach k results
 * - AD-HOC brute force - compute distance for every vector whose id passes the filter, then take
 * the top k To reproduce and/or run the benchmark for different configurations:
