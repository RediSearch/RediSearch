...
TEST_P(InvIndIteratorRevalidateTest, RevalidateAfterIndexDisappears) {
    ...
    if (IsQueryIterator()) {
        ...
        if (IsNumericIterator()) {
            ...
            // Simulate the range tree being modified by adding a document
            // This will increment the revision ID when the tree structure changes
            // (e.g., node split). Use a large doc ID to ensure we add to a different
            // location than the iterator is currently at.
            NumericRangeTree_AddCompat(numericRangeTree, 999999, 0.0, false);

            // Verify that the revision ID has indeed changed
            uint32_t newRevisionId = NumericRangeTree_GetRevisionId(numericRangeTree);
            ASSERT_NE(originalRevisionId, newRevisionId)
                << "Revision ID should have changed after adding a document";
