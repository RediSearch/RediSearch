...
// Uses an unordered comparison (containment + numDocs per entry) because the
// recovered trie is loaded in `Trie_Sort_Score` mode while the original is
// `Trie_Sort_Lex`, so iteration order differs from the original.
