...
TEST_F(RangeTest, testRangeIterator) {
  ...
  for (size_t i = 0; i < 5; i++) {
    ...
    NumericFilter *flt = NewNumericFilter(std::min(min, max), std::max(min, max), 1, 1, 0, 0, true);
  ...
  for (size_t i = 0; i < 6; i++) {
    for (int j = 0; j < 2; ++j) {   
      ...
      NumericFilter *flt = NewNumericFilter(rangeArray[i][0], rangeArray[i][1], 1, 1, 0, 0, j);
      IndexIterator *it = createNumericIterator(NULL, t, flt);
      size_t numEstimated = it->NumEstimated(it->ctx);
      NumericFilter *fltLimited = NewNumericFilter(rangeArray[i][0], rangeArray[i][1], 1, 1, 0, 50, j);
