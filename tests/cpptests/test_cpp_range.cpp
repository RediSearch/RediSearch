...
void testRangeIteratorHelper(bool isMulti) {
    ...
    double rangeArray[6][2] = {{0, 1000},      {0, 3000},      {1000, 3000},
                               {15000, 20000}, {19500, 20000}, {-1000, 21000}};
    ...
    for (size_t i = 0; i < 6; i++) {
        for (int j = 0; j < 2; ++j) {
            ...
            NumericFilter *fltLimited =
                NewNumericFilter(rangeArray[i][0], rangeArray[i][1], 1, 1, j);
            fltLimited->limit = 50;
            ...
            // printf("%f %f %ld %ld\n", rangeArray[i][0], rangeArray[i][1], numEstimated,
            // numEstimatedLimited);
...
TEST_F(RangeTest, testRangeIterator) { testRangeIteratorHelper(false); }

TEST_F(RangeTest, testRangeIteratorMulti) { testRangeIteratorHelper(true); }
