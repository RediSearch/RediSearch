...
void fill_first() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int i = n; i < n * 2; i++)
            name[i - n] =
                alphanum[rand_idx[i] % sizeof(alphanum)]; // fill the array with alphanum chars
...
int main(int argc, char **argv) {
    ...
    TIME_SAMPLE_RUN(add_delete(
        "asdfg")); // now add 1000000 entries of a variant string and remove those entries
