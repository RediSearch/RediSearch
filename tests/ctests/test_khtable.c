...
static uint32_t myHash(const KHTableEntry *e) { return ((const MyEntry *)e)->hash; }

static KHTableEntry *myAlloc() { return calloc(1, sizeof(MyEntry)); }

static uint32_t calcHash(const char *s) { return rs_fnv_32a_buf((char *)s, strlen(s), 0); }
