...
int __trie_add(TrieNode **n, char *str, char *payloadStr, float sc, TrieAddOp op) {
  ...
  int rc = n->Add(runes, rlen, &payload, sc, op);
  free(runes);
  return rc;
}
...
int testPayload() {
  ...
  TrieNode *root(rootRunes, 0, 0, NULL, 0, 0, 1, 0);
  free(rootRunes);
...
int testTrie() {
  ...
  rune *rstr;
  t_len l;
  TrieNode *rn = root->RandomWalk(10, &rstr, &l);
  ASSERT(rn != NULL);
  ASSERT(rstr != NULL);
  ASSERT(l > 0);
  // ASSERT(rn->score > 0);
...
int testUnicode() {
  ...
  TrieNode *root(rn, 0, 0, NULL, 0, 0, 1, 0);
  free(rn);
  ASSERT(root != NULL)
...
int testDFAFilter() {
  ...
  TrieNode root(runes, 0, rlen, NULL, 0, 0, 0, 0);
  free(runes);
  int i = 0;
  while ((read = getline(&line, &len, fp)) != -1) {
    ...
    *sep = 0;
    ...
    int rc = root.Add(runes, rlen, NULL, (float)score, ADD_REPLACE);
    ASSERT(rc == 1);
    free(runes);
