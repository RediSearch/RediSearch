...
static long long TimeSampler_DurationMS(TimeSample *ts) { return ts->durationNS / 1000000; }
...
static double TimeSampler_IterationMS(TimeSample *ts) {
    ...
    return ((double)ts->durationNS / 1000000.0) / (double)(ts->num ? ts->num : 1.0);
}
...
#define TIME_SAMPLE_RUN_LOOP(N, blk)                                                               \
    {                                                                                              \
        ...
        TimeSampler_End(&ts);                                                                      \
        printf("Execution time for %d iterations of " #blk ": %f msec/iteration\n", ts.num,        \
               TimeSampler_IterationMS(&ts));                                                      \
    }
