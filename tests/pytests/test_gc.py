...
def testBasicGC(env):
    ...
    env.assertEqual(
        env.cmd(debug_cmd(), "DUMP_INVIDX", "idx", "world"),
        [int(i) for i in range(1, 102)],
    )
    env.assertEqual(
        env.cmd(debug_cmd(), "DUMP_NUMIDX", "idx", "id"),
        [[int(i) for i in range(1, 102)]],
    )
    env.assertEqual(
        env.cmd(debug_cmd(), "DUMP_TAGIDX", "idx", "t"),
        [["tag1", [int(i) for i in range(1, 102)]]],
    )
    ...
    env.assertEqual(
        env.cmd(debug_cmd(), "DUMP_TAGIDX", "idx", "t"),
        [["tag1", [int(i) for i in range(2, 102)]]],
    )
...
def testBasicGCWithEmptyInvIdx(env):
    ...
    # check that the gc collected the deleted docs
    env.expect(debug_cmd(), "DUMP_INVIDX", "idx", "world").error().contains(
...
def testNumericMergesTrees(env):
    ...
    env.assertGreaterEqual(
        initial_bucket_count, 2, message="Expected at least 2 buckets initially"
    )
    ...
    env.assertGreater(
        non_empty_final,
        0,
        message="Should have at least one non-empty bucket after merge",
    )
    env.assertLessEqual(
        final_bucket_count,
...
def testGCIntegrationWithRedisFork(env):
    ...
    env.expect(config_cmd(), "set", "FORK_GC_CLEAN_THRESHOLD", 0).ok()
    env.expect(
        "FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "title", "TEXT", "SORTABLE"
    ).ok()
    waitForIndex(env, "idx")
...
def testGCCleanupWithReplace(env):
    ...
    for i in range(1000):
        env.expect(
            "FT.ADD", "idx", "doc%d" % i, "1.0", "REPLACE", "FIELDS", "title", "foo1"
        ).ok()
    ...
    env.expect(debug_cmd(), "DUMP_INVIDX", "idx", "foo1").error().contains(
        "Can not find the inverted index"
    )
...
def testAutoMemory_MOD_3951():
    ...
    conn.execute_command(
        "FT.CREATE", "idx", "FILTER", '@t == "5"', "SCHEMA", "t", "TEXT"
    )
    # add docs
    conn = getConnectionByEnv(env)
    for i in range(100):
...
def testConcurrentFTInfoDuringIndexDeletion(env):
    ...
    env.expect(config_cmd(), "set", "FORK_GC_CLEAN_THRESHOLD", 0).equal("OK")
    env.expect(config_cmd(), "set", "FORK_GC_RUN_INTERVAL", 100).equal(
        "OK"
            ...
            while not stop_threads.is_set():
                try:
                    ...
                    if "index not found" in error_msg:
                        ...
                        env.assertTrue(
                            False,
                            message=f"Unexpected error in FT.INFO for {idx_name}: {e}",
                        )
                        break
                    time.sleep(0.01)
    ...
    for idx_name in index_names:
        try:
            ...
            env.assertTrue(
                False, message=f"Unexpected error dropping index {idx_name}: {e}"
            )
