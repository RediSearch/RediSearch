...
from common import skip, toSortedFlatList, waitForIndex
...
def testSummarization(env):
    ...
    env.assertEqual(
        [1, "gen1", ["txt", " The First Book of Moses, called Genesis {1:1} In the"]],
        res,
    )
...
def testPrefixExpansion(env):
    ...
    possibilities = [
        ...
        [
            ...
            [
                "txt",
                "49:3} Reuben, thou [art] my firstborn, my might, and the <b>beginning of</b> my strength, the excellency of dignity, and the excellency... ",
            ],
        ],
    ]
...
def testSummarizationDisabled(env):
    ...
    with env.assertResponseError():
        res = env.cmd("FT.SEARCH", "idx", "hello", "SUMMARIZE", "FIELDS", 1, "body")

    env.cmd("FT.CREATE", "idx2", "ON", "HASH", "NOHL", "SCHEMA", "body", "TEXT")
    waitForIndex(env, "idx")
    env.cmd("FT.ADD", "idx2", "doc2", 1.0, "FIELDS", "body", "hello world")
    with env.assertResponseError():
        res = env.cmd("FT.SEARCH", "idx2", "hello", "SUMMARIZE", "FIELDS", 1, "body")
...
def testSummarizationNoSave(env):
    ...
    waitForIndex(env, "idx")
    env.cmd("FT.ADD", "idx", "doc", 1.0, "NOSAVE", "fields", "body", "hello world")
    res = env.cmd("FT.SEARCH", "idx", "hello", "SUMMARIZE", "RETURN", 1, "body")
    # print res
...
def testSummarizationMeta(env):
    ...
    env.assertEqual(
        [1, "doc1", ["foo", "pill... ", "bar", "pillow... ", "baz", "piller... "]], res
    )
...
def testFailedHighlight(env):
    ...
    env.assertEqual(
        toSortedFlatList(
            [1, "doc1", ["f1", "foo foo foo", "f2", "bar bar bar", "f3", "baz baz baz"]]
        ),
    ...
    env.assertEqual(
        toSortedFlatList(
            [1, "doc1", ["f2", "bar bar bar", "f1", "foo foo foo", "f3", "baz baz baz"]]
        ),
        toSortedFlatList(env.cmd("ft.search idx foo highlight fields 1 f2")),
    )
    env.assertEqual(
        toSortedFlatList(
            [1, "doc1", ["f3", "baz baz baz", "f1", "foo foo foo", "f2", "bar bar bar"]]
        ),
    ...
    env.assertEqual(
        toSortedFlatList(
            [1, "doc2", ["f2", "", "f1", "foo foo foo", "f3", "baz baz baz"]]
        ),
        toSortedFlatList(env.cmd("ft.search idx2 foo highlight fields 1 f2")),
    )
    env.assertEqual(
        toSortedFlatList(
            [1, "doc2", ["f3", "baz baz baz", "f1", "foo foo foo", "f2", ""]]
        ),
    ...
    env.assertEqual(
        toSortedFlatList(
            [1, "doc3", ["f2", "not a", "f1", "foo foo foo", "f3", "baz baz baz"]]
        ),
        toSortedFlatList(env.cmd("ft.search idx3 foo highlight fields 1 f2")),
    )
    env.assertEqual(
        toSortedFlatList(
            [1, "doc3", ["f3", "baz baz baz", "f1", "foo foo foo", "f2", "not a"]]
        ),
...
def testHighlightAlias(env):
    ...
    env.expect("ft.search idx foo highlight fields 1 f1_alias").equal(
        [1, "doc", ["f1", "foo foo foo", "f2", "baz baz baz"]]
    )
    env.expect("ft.search idx foo highlight fields 1 f1").error().contains(
        "No such property `f1`"
    )  # OK
    ...
    env.assertEqual(
        toSortedFlatList(
            env.cmd("ft.search idx foo highlight fields 1 f1_alias RETURN 1 f1_alias")
        ),
