...
def testSyntax1(env):
    ...
    env.expect("ft.create", "idx", "ONfoo*", "SCHEMA", "foo", "text").equal(
        "Unknown argument `ONfoo*`"
    )

    env.expect("ft.create", "idx2", "LANGUAGE", "eng" "SCHEMA", "foo", "text").equal(
        "Invalid language"
    )

    env.expect("ft.create", "idx2", "SCORE", "1.0" "SCHEMA", "foo", "text").equal(
        "Unknown argument `foo`"
    )

    env.expect(
        "ft.create", "idx2", "PAYLOAD_FIELD", "awfw" "SCHEMA", "foo", "text"
    ).equal("Unknown argument `foo`")

    env.expect("ft.create", "idx2", "FILTER", "a" "SCHEMA", "foo", "text").equal(
        "Unknown symbol 'aSCHEMA'"
    )
...
def testPrefix0a(env):
    conn = getConnectionByEnv(env)
    env.cmd(
        "ft.create", "things", "ON", "HASH", "PREFIX", "1", "", "SCHEMA", "name", "text"
    )
...
def testFlushallManyPrefixes(env):
    ...
    for i in range(num_indices):
        env.cmd(
            "ft.create", i, "ON", "HASH", "PREFIX", "1", i, "SCHEMA", "name", "text"
        )
...
def testIdxField(env):
    ...
    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx1", "*")),
        toSortedFlatList([1, "doc1", ["name", "foo", "indexName", "idx1"]]),
    )
    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx2", "*")),
        toSortedFlatList([1, "doc2", ["name", "bar", "indexName", "idx2"]]),
    )
...
def testRename(env):
    ...
    env.expect("ft.search things foo").equal([0])
    env.expect("ft.search otherthings foo").equal(
        [1, "otherthing:foo", ["name", "foo"]]
    )
...
def testPayload(env):
    ...
    for _ in env.reloadingIterator():
        ...
        res = env.cmd("ft.search", "things", "foo")
        env.assertEqual(
            toSortedFlatList(res), toSortedFlatList([1, "thing:foo", ["name", "foo"]])
        )

        res = env.cmd("ft.search", "things", "foo", "withpayloads")
        env.assertEqual(
            toSortedFlatList(res),
            toSortedFlatList([1, "thing:foo", "stuff", ["name", "foo"]]),
        )
...
def testBinaryPayload(env):
    ...
    for _ in env.reloadingIterator():
        ...
        res = env.cmd(
            "ft.search", "things", "foo", "withpayloads", **{NEVER_DECODE: []}
        )
...
def testDuplicateFields(env):
    ...
    env.cmd(
        "FT.ADD", "idx", "doc", 1.0, "FIELDS", "txt", "foo", "txt", "bar", "txt", "baz"
    )
...
def testReplace(env):
    ...
    for _ in env.reloadingIterator():
        ...
        env.expect("ft.search", "idx", "goodbye universe", "nocontent").equal(
            [1, "doc1"]
        )
...
def testMissingArgs(env):
    env.expect(
        "FT.CREATE", "idx", "ON", "SCHEMA", "txt", "TEXT", "num", "NUMERIC"
    ).error()
    env.expect(
        "FT.CREATE",
        "idx",
...
def testWrongArgs(env):
    env.expect(
        "FT.CREATE", "idx", "SCORE", "SCHEMA", "txt", "TEXT", "num", "NUMERIC"
    ).error().contains("Invalid score")
    env.expect(
        "FT.CREATE", "idx", "SCORE", 10, "SCHEMA", "txt", "TEXT", "num", "NUMERIC"
    ).error().contains("Invalid score")
    env.expect(
        "FT.CREATE", "idx", "LANGUAGE", "SCHEMA", "txt", "TEXT", "num", "NUMERIC"
    ).error().contains("Invalid language")
    env.expect(
        "FT.CREATE",
        "idx",
        "LANGUAGE",
...
def testLanguageDefaultAndField(env):
    ...
    conn.execute_command("HSET", "doc1", "lang", "hindi", "body", "अँगरेजी अँगरेजों अँगरेज़")

    for _ in env.reloadingIterator():
        waitForIndex(env, "idxTest1")
        waitForIndex(env, "idxTest2")
        # test for language field
        res = env.cmd("FT.SEARCH", "idxTest1", "अँगरेज़")
        res1 = {res[2][i]: res[2][i + 1] for i in range(0, len(res[2]), 2)}
        env.assertEqual("अँगरेजी अँगरेजों अँगरेज़", res1["body"])
        # test for default langauge
        res = env.cmd("FT.SEARCH", "idxTest2", "अँगरेज़")
        res1 = {res[2][i]: res[2][i + 1] for i in range(0, len(res[2]), 2)}
        env.assertEqual("अँगरेजी अँगरेजों अँगरेज़", res1["body"])
...
def testScoreDecimal(env):
    ...
    env.expect(
        "FT.CREATE", "idx2", "SCORE_FIELD", "score", "schema", "title", "text"
    ).ok()
...
def testMultiFilters1(env):
    ...
    conn.execute_command(
        "HSET", "student:yes1", "first", "yes1", "last", "yes1", "age", "17"
    )
    conn.execute_command(
        "HSET", "student:yes2", "first", "yes2", "last", "yes2", "age", "15"
    )
    conn.execute_command(
        "HSET", "pupil:no1", "first", "no1", "last", "no1", "age", "17"
    )
    conn.execute_command(
        "HSET", "pupil:no2", "first", "no2", "last", "no2", "age", "15"
    )
    res1 = [
        2,
        "student:yes2",
        ["first", "yes2", "last", "yes2", "age", "15"],
        "student:yes1",
        ["first", "yes1", "last", "yes1", "age", "17"],
    ]
...
def testMultiFilters2(env):
    ...
    conn.execute_command(
        "HSET", "student:no1", "first", "no1", "last", "no1", "age", "15"
    )
    conn.execute_command(
        "HSET", "pupil:yes2", "first", "yes2", "last", "yes2", "age", "17"
    )
    conn.execute_command(
        "HSET", "pupil:no2", "first", "no2", "last", "no2", "age", "15"
    )
...
def testInfo(env):
    ...
    res_actual = env.cmd("FT.INFO test")
    res_expected = ["key_type", "HASH", "prefixes", [""], "default_score", "1"]
    env.assertEqual(res_actual[5], res_expected)
...
def testPartial(env):
    ...
    env.expect("FT.SEARCH idx bar").equal(
        [1, "doc1", ["test", "bar", "testtest", "foo"]]
    )
    ...
    env.expect("FT.SEARCH idx baz").equal(
        [1, "doc2", ["test", "baz", "testtest", "foo"]]
    )
    ...
    env.expect(debug_cmd() + " docidtoid idx doc3").equal(5)
    env.expect("FT.SEARCH idx foo").equal(
        [1, "doc3", ["test", "foo", "testtest", "foo"]]
    )
    ...
    env.expect("HINCRBY doc4 test 5.5").error().contains(
    ...
    env.assertEqual(
        ...
        [
            5,
            "doc1",
            ["test", "bar", "testtest", "foo"],
            "doc2",
            ["test", "baz", "testtest", "foo"],
            "doc3",
            ["test", "foo", "testtest", "foo"],
            "doc4",
            ["test", 11, "testtest", "5"],
            "doc5",
            ["test", 17.1, "testtest", "5.5"],
        ],
    )
...
def testSkipInitialScan(env):
    ...
    waitForIndex(env, "idx")
    env.assertEqual(
        toSortedFlatList(env.cmd("FT.SEARCH idx hello")),
        toSortedFlatList([1, "a", ["test", "hello", "text", "world"]]),
    )
    # SkipInitialIndex
    ...
    env.assertEqual(
        toSortedFlatList(env.cmd("FT.SEARCH temp_idx hello")),
        toSortedFlatList([1, "a", ["test", "hello", "text", "world"]]),
    )
    # Temporary & NoInitialIndex
    env.expect(
        "FT.CREATE temp_idx_no_scan SKIPINITIALSCAN TEMPORARY 10 SCHEMA test TEXT"
    ).equal("OK")
...
def testWrongFieldType(env):
    ...
    env.assertEqual(
        toSortedFlatList(env.cmd("FT.SEARCH idx hello")),
        toSortedFlatList([1, "a", ["t", "hello", "n", "42"]]),
    )

    res_actual = env.cmd("FT.INFO idx")
    res_actual = {
        res_actual[i]: res_actual[i + 1] for i in range(0, len(res_actual), 2)
    }
...
def testCountry(env):
    ...
    env.assertEqual(
        toSortedFlatList(res),
        toSortedFlatList([1, "address:1", ["business", "foo", "country", "usa"]]),
    )


def testIssue1571(env):
    conn = getConnectionByEnv(env)
    env.cmd("ft.create", "idx", "FILTER", '@index=="yes"', "SCHEMA", "t", "TEXT")

    conn.execute_command("hset", "doc1", "t", "foo1", "index", "yes")

    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx", "foo*")),
        toSortedFlatList([1, "doc1", ["t", "foo1", "index", "yes"]]),
    )
    ...
    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx", "foo*")),
        toSortedFlatList([1, "doc1", ["t", "foo2", "index", "yes"]]),
    )
...
def testIssue1571WithRename(env):
    ...
    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx1", "foo*")),
        toSortedFlatList([1, "idx1:{doc}1", ["t", "foo1", "index", "yes"]]),
    )
    ...
    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx2", "foo*")),
        toSortedFlatList([1, "idx2:{doc}1", ["t", "foo1", "index", "yes"]]),
    )
    env.expect("ft.search", "idx1", "foo*").equal([0])
...
def testIdxFieldJson(env):
    ...
    conn.execute_command(
        "JSON.SET", "doc:1", "$", r'{"name":"foo", "indexName":"idx1"}'
    )
    conn.execute_command(
        "JSON.SET", "doc:2", "$", r'{"name":"bar", "indexName":"idx2"}'
    )

    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx1", "*")),
        toSortedFlatList([1, "$", "doc:1", '{"name":"foo","indexName":"idx1"}']),
    )
    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx2", "*")),
        toSortedFlatList([1, "$", "doc:2", '{"name":"bar","indexName":"idx2"}']),
    )
...
def testFilterStartWith(env):
    ...
    conn.execute_command(
        "JSON.SET", "thing:bar", "$", r'{"name":"foo", "indexName":"idx1"}'
    )

    env.expect("ft.search", "things", "foo").equal(
        [1, "thing:bar", ["$", '{"name":"foo","indexName":"idx1"}']]
    )
