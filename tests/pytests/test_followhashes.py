...
def testPartial(env):
    ...
    res[2][1] = float(res[2][1])
    res[4][1] = float(res[4][1])
    env.expect('FT.SEARCH idx *').equal([5, 'doc5', ['test', '17.1', 'testtest', '5.5'],
...
def testExpiredDuringSearch(env):
    ...
    env.assertEqual(toSortedFlatList(res[1:]),
                    toSortedFlatList(['bar', ['txt1', 'hello', 'n', '20'], 
                                      'foo', ['txt1', 'hello', 'n', '0']]))
