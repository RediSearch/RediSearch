...
def dotestSanity(env, dialect):
    ...
    for index in index_list:
        ...
        # all
        env.expect("ft.search", index, r"@t:(w'*')", "LIMIT", 0, 0).equal(
            [4 * item_qty]
        )
    ...
    env.expect("ft.search", index_list[0], "w'foo*'", "LIMIT", 0, 0).error().contains(
...
def dotestSanityTag(env, dialect):
    ...
    env.expect(config_cmd(), "set", "ON_TIMEOUT", "RETURN").ok()
    env.expect(
        "ft.search", index_list[0], "@t:{w'foo*'}", "LIMIT", 0, 0
    ).error().contains("Timeout limit was reached")
    env.expect(
        "ft.search", index_list[1], "@t:{w'foo*'}", "LIMIT", 0, 0
    ).error().contains("Timeout limit was reached")
...
def testBenchmark(env):
    ...
    for i in range(item_qty):
        ...
        pl.execute_command(
            "HSET", "doc%d" % (i + item_qty * 2), "t", "foooo312%dbar312" % i
        )
        pl.execute_command(
            "HSET", "doc%d" % (i + item_qty * 3), "t", "foofo31%dbar312" % i
        )
        pl.execute()
    ...
    for i in range(1):
        ...
        start_time = time.time()
        env.expect("ft.search", index_list[i], "w'*o**o3*'", "LIMIT", 0, 0).equal(
            [2222]
        )
        print(time.time() - start_time)
        start_time = time.time()
...
def testEscape(env):
    ...
    env.expect("FT.SEARCH", "idx", "w'?????'").equal(
        [2, "doc1", ["t", "hello"], "doc2", ["t", "jello"]]
    )
    env.expect("FT.SEARCH", "idx", "w'?ello'").equal(
        [2, "doc1", ["t", "hello"], "doc2", ["t", "jello"]]
    )
    env.expect("FT.SEARCH", "idx", "w'?ello?'").equal(
        [
    ...
    env.expect("FT.SEARCH", "idx", "w'*o\\'w*'").equal(
        [1, "doc7", ["t", "hello\\'world"]]
    )  # *o'w*
    ...
    env.expect("FT.SEARCH", "idx", "w'*o\\\\w*'").equal(
        [1, "doc8", ["t", "hello\\\\world"]]
    )  # *o\w*

    # test with PARAMS
    # escape \'
    env.expect("FT.SEARCH", "idx", "w'$wcq'", "PARAMS", "2", "wcq", "*\\'*").equal(
        [
            3,
            "doc7",
            ["t", "hello\\'world"],  # *'*
    ...
    env.expect("FT.SEARCH", "idx", "w'$wcq'", "PARAMS", "2", "wcq", "*o\\'w*").equal(
        [1, "doc7", ["t", "hello\\'world"]]
    )  # *o'w*
...
def testLowerUpperCase(env):
    ...
    env.expect("FT.SEARCH", "idx", "w'*el*'", "NOCONTENT").equal(
...
def testBasic():
    ...
    env.expect("FT.SEARCH", "idx", "w'*ll*'", "NOCONTENT").equal(
        [4, "doc1", "doc2", "doc7", "doc8"]
    )
    env.expect("FT.SEARCH", "idx", "w'*llo'", "NOCONTENT").equal([2, "doc1", "doc8"])
    env.expect("FT.SEARCH", "idx", "w'he*'", "NOCONTENT").equal(
        [5, "doc1", "doc2", "doc3", "doc4", "doc6"]
    )

    env.expect(
        "FT.AGGREGATE", "idx", "w'*el*'", "LOAD", 1, "@t", "SORTBY", 1, "@t"
    ).equal([4, ["t", "helen"], ["t", "hell"], ["t", "hello"], ["t", "help"]])

    env.expect(
        "FT.AGGREGATE", "idx", "w'*ll*'", "LOAD", 1, "@t", "SORTBY", 1, "@t"
    ).equal([4, ["t", "hall"], ["t", "hallo"], ["t", "hell"], ["t", "hello"]])

    env.expect(
        "FT.AGGREGATE", "idx", "w'*llo'", "LOAD", 1, "@t", "SORTBY", 1, "@t"
    ).equal([2, ["t", "hallo"], ["t", "hello"]])

    env.expect(
        "FT.AGGREGATE", "idx", "w'he*'", "LOAD", 1, "@t", "SORTBY", 1, "@t"
    ).equal(
        [5, ["t", "heal"], ["t", "helen"], ["t", "hell"], ["t", "hello"], ["t", "help"]]
    )
...
def testSuffixCleanup(env):
    ...
    conn.execute_command(
        "FT.CREATE", "idx", "SCHEMA", "t1", "TEXT", "WITHSUFFIXTRIE", "t2", "TEXT"
    )
