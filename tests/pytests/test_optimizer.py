...
def testOptimizer(env):
    ...
    env.expect(
        "ft.search", "idx", "foo @n:[10 20]", "SORTBY", "n", "limit", 0, 2, *params
    ).equal([2, "10", "110"])
    env.expect(
        "ft.search",
        "idx",
    ...
    res = env.cmd(
        "ft.profile", "idx", "search", "query", "foo @n:[10 15]", "SORTBY", "n", *params
    )
    env.assertEqual(
        res[0],
    ...
    env.assertEqual(
        actual_profiler["Result processors profile"],
        profiler["Result processors profile"],
    )

    ### (2) range and filter w/o sort ###
    env.expect("ft.search", "idx", "foo @n:[10 20]", "limit", 0, 2, *params).equal(
        [2, "10", "12"]
    )
    env.expect("ft.search", "idx", "foo @n:[10 20]", "limit", 0, 3, *params).equal(
        [3, "10", "12", "14"]
    )
    env.expect(
        "ft.search", "idx_sortable", "foo @n:[10 20]", "limit", 0, 2, *params
    ).equal([2, "10", "12"])
    env.expect(
        "ft.search", "idx_sortable", "foo @n:[10 20]", "limit", 0, 3, *params
    ).equal([3, "10", "12", "14"])
    ...
    env.assertEqual(
        res[0], [10, "10", "12", "14", "16", "18", "20", "110", "112", "114", "116"]
    )
    actual_profiler = to_dict(res[1][1][0])
    env.assertEqual(actual_profiler["Iterators profile"], profiler["Iterators profile"])
    env.assertEqual(
    ...
    env.expect(
        "ft.search", "idx", "@tag:{foo} @n:[10 20]", "limit", 0, 2, *params
    ).equal([1, "10", "12"])
    env.expect(
        "ft.search", "idx", "@tag:{foo} @n:[10 20]", "limit", 0, 3, *params
    ).equal([1, "10", "12", "14"])
    env.expect(
        "ft.search", "idx_sortable", "@tag:{foo} @n:[10 20]", "limit", 0, 2, *params
    ).equal([1, "10", "12"])
    env.expect(
        "ft.search", "idx_sortable", "@tag:{foo} @n:[10 20]", "limit", 0, 3, *params
    ).equal([1, "10", "12", "14"])
    ...
    res = env.cmd(
        "ft.profile", "idx", "search", "query", "@tag:{foo} @n:[10 15]", *params
    )
    env.assertEqual(
        res[0][1:], ["10", "12", "14", "110", "112", "114", "210", "212", "214", "310"]
    )
    actual_profiler = to_dict(res[1][1][0])
    env.assertEqual(actual_profiler["Iterators profile"], profiler["Iterators profile"])
    env.assertEqual(
    ...
    # Search only minimal number of ranges
    env.expect(
        "ft.search", "idx", "@n:[10 20]", "SORTBY", "n", "limit", 0, 2, *params
    ).equal([2, "10", "11"])
    env.expect(
        "ft.search", "idx", "@n:[10 20]", "SORTBY", "n", "ASC", "limit", 0, 2, *params
    ).equal([2, "10", "11"])
    env.expect(
        "ft.search", "idx", "@n:[10 20]", "SORTBY", "n", "DESC", "limit", 0, 2, *params
    ).equal([2, "19921", "19920"])
    env.expect(
        "ft.search", "idx_sortable", "@n:[10 20]", "SORTBY", "n", "limit", 0, 2, *params
    ).equal([2, "10", "11"])
    env.expect(
        "ft.search",
        "idx_sortable",
    ...
    res = env.cmd(
        "ft.profile", "idx", "search", "query", "@n:[10 15]", "SORTBY", "n", *params
    )
    env.assertEqual(
        res[0], [10, "10", "11", "110", "111", "210", "211", "310", "311", "410", "411"]
    )
    actual_profiler = to_dict(res[1][1][0])
    env.assertEqual(actual_profiler["Iterators profile"], profiler["Iterators profile"])
    env.assertEqual(
    ...
    ### (6) only range ###
    # stop after enough results were collected
    env.expect("ft.search", "idx", "@n:[10 20]", "limit", 0, 2, *params).equal(
        [1, "10", "11"]
    )
    env.expect("ft.search", "idx", "@n:[10 20]", "limit", 0, 3, *params).equal(
        [1, "10", "11", "12"]
    )
    env.expect("ft.search", "idx_sortable", "@n:[10 20]", "limit", 0, 2, *params).equal(
        [1, "10", "11"]
    )
    env.expect("ft.search", "idx_sortable", "@n:[10 20]", "limit", 0, 3, *params).equal(
        [1, "10", "11", "12"]
    )
    ...
    env.assertEqual(
        res[0], [1, "10", "11", "12", "13", "14", "15", "110", "111", "112", "113"]
    )
    actual_profiler = to_dict(res[1][1][0])
    env.assertEqual(actual_profiler["Iterators profile"], profiler["Iterators profile"])
    env.assertEqual(
    ...
    ### (7) filter with sort ###
    # Search only minimal number of ranges
    env.expect("ft.search", "idx", "foo", "SORTBY", "n", "limit", 0, 2, *params).equal(
        [2, "0", "100"]
    )
    env.expect(
        "ft.search", "idx", "foo", "SORTBY", "n", "ASC", "limit", 0, 2, *params
    ).equal([2, "0", "100"])
    env.expect(
        "ft.search", "idx", "foo", "SORTBY", "n", "DESC", "limit", 0, 2, *params
    ).equal([2, "198", "98"])
    env.expect(
        "ft.search", "idx_sortable", "foo", "SORTBY", "n", "limit", 0, 2, *params
    ).equal([2, "0", "100"])
    env.expect(
        "ft.search", "idx_sortable", "foo", "SORTBY", "n", "ASC", "limit", 0, 2, *params
    ).equal([2, "0", "100"])
    env.expect(
        "ft.search",
        "idx_sortable",
    ...
    env.assertEqual(
        res[0], [10, "0", "100", "200", "300", "400", "500", "600", "700", "800", "900"]
    )
    actual_profiler = to_dict(res[1][1][0])
    env.assertEqual(actual_profiler["Iterators profile"], profiler["Iterators profile"])
    env.assertEqual(
    ...
    result = env.cmd(
        "ft.search", "idx", "foo", "SORTBY", "n", "limit", 0, 1500, *params
    )
    ...
    env.expect("ft.search", "idx", "foo", "limit", 0, 3, *params).equal(
        [3, "0", "2", "4"]
    )
    env.expect("ft.search", "idx_sortable", "foo", "limit", 0, 2, *params).equal(
        [2, "0", "2"]
    )
    env.expect("ft.search", "idx_sortable", "foo", "limit", 0, 3, *params).equal(
        [3, "0", "2", "4"]
    )
    ...
    # Search only minimal number of ranges
    env.expect(
        "ft.search", "idx", "@tag:{foo}", "SORTBY", "n", "limit", 0, 2, *params
    ).equal([2, "0", "100"])
    env.expect(
        "ft.search", "idx", "@tag:{foo}", "SORTBY", "n", "ASC", "limit", 0, 2, *params
    ).equal([2, "0", "100"])
    env.expect(
        "ft.search", "idx", "@tag:{foo}", "SORTBY", "n", "DESC", "limit", 0, 2, *params
    ).equal([2, "198", "98"])
    env.expect(
        "ft.search", "idx_sortable", "@tag:{foo}", "SORTBY", "n", "limit", 0, 2, *params
    ).equal([2, "0", "100"])
    env.expect(
        "ft.search",
        "idx_sortable",
    ...
    res = env.cmd(
        "ft.profile", "idx", "search", "query", "@tag:{foo}", "SORTBY", "n", *params
    )
    env.assertEqual(
        res[0], [10, "0", "100", "200", "300", "400", "500", "600", "700", "800", "900"]
    )
    ...
    # stop after enough results were collected
    env.expect("ft.search", "idx", "@tag:{foo}", "limit", 0, 2, *params).equal(
        [1, "0", "2"]
    )
    env.expect("ft.search", "idx", "@tag:{foo}", "limit", 0, 3, *params).equal(
        [1, "0", "2", "4"]
    )
    env.expect("ft.search", "idx_sortable", "@tag:{foo}", "limit", 0, 2, *params).equal(
        [1, "0", "2"]
    )
    env.expect("ft.search", "idx_sortable", "@tag:{foo}", "limit", 0, 3, *params).equal(
        [1, "0", "2", "4"]
    )
    ...
    # Search only minimal number of ranges
    env.expect("ft.search", "idx", "*", "SORTBY", "n", "limit", 0, 2, *params).equal(
        [2, "0", "1"]
    )
    env.expect(
        "ft.search", "idx", "*", "SORTBY", "n", "ASC", "limit", 0, 2, *params
    ).equal([2, "0", "1"])
    env.expect(
        "ft.search", "idx", "*", "SORTBY", "n", "DESC", "limit", 0, 2, *params
    ).equal([2, "99", "98"])
    env.expect(
        "ft.search", "idx_sortable", "*", "SORTBY", "n", "limit", 0, 2, *params
    ).equal([2, "0", "1"])
    env.expect(
        "ft.search", "idx_sortable", "*", "SORTBY", "n", "ASC", "limit", 0, 2, *params
    ).equal([2, "0", "1"])
    env.expect(
        "ft.search", "idx_sortable", "*", "SORTBY", "n", "DESC", "limit", 0, 2, *params
    ).equal([2, "99", "98"])
    ...
    env.assertEqual(
        res[0], [10, "0", "1", "100", "101", "200", "201", "300", "301", "400", "401"]
    )
    actual_profiler = to_dict(res[1][1][0])
    env.assertEqual(actual_profiler["Iterators profile"], profiler["Iterators profile"])
    env.assertEqual(
    ...
    env.expect("ft.search", "idx", "*", "limit", 0, 3, *params).equal(
        [1, "0", "1", "2"]
    )
    env.expect("ft.search", "idx_sortable", "*", "limit", 0, 2, *params).equal(
        [1, "0", "1"]
    )
    env.expect("ft.search", "idx_sortable", "*", "limit", 0, 3, *params).equal(
        [1, "0", "1", "2"]
    )
    ...
    result = env.cmd(
        "ft.search", "idx", "@tag:{foo}", "SORTBY", "n", "limit", 0, 1500, *params
    )
    env.assertEqual(result[0], 1500)

    result = env.cmd(
        "ft.search", "idx", "foo @n:[10 20]", "SORTBY", "n", "limit", 0, 1500, *params
    )
...
def testWOLimit(env):
    ...
    for i in range(0, repeat, 5):
        for j in range(5):
            conn.execute_command(
                "hset", i + j, "t", words[j], "tag", words[j], "n", i + j
            )
    ...
    ### (1) range and filter with sort ###
    # Search only minimal number of ranges
    env.expect("ft.search", "idx", "foo @n:[10 70]", "SORTBY", "n", *params).equal(
        [10] + res10
    )
    env.expect("ft.search", "idx", "foo @n:[10 40]", "SORTBY", "n", *params).equal(
        [6] + res6
    )
    ...
    # Search only minimal number of ranges
    env.expect(
        "ft.search", "idx", "@tag:{foo} @n:[10 70]", "SORTBY", "n", *params
    ).equal([10] + res10)
    env.expect(
        "ft.search", "idx", "@tag:{foo} @n:[10 40]", "SORTBY", "n", *params
    ).equal([6] + res6)
    ...
    env.expect("ft.search", "idx", "@n:[10 50]", "SORTBY", "n", *params).equal(
        [10] + res10
    )
    env.expect(
        "ft.search",
    ...
    env.expect(
        "ft.search", "idx", "@n:[10 12]", "WITHOUTCOUNT", "RETURN", 1, "n"
    ).equal([1] + res3)
    ...
    ### (9) no sort, no score, with sortby ###
    # Search only minimal number of ranges
    env.expect("ft.search", "idx", "@tag:{foo}", "SORTBY", "n", *params).equal(
        [10] + res10
    )
...
def testSearch(env):
    ...
    for i in range(0, repeat, len(words)):
        for j in range(len(words)):
            conn.execute_command(
                "hset", i + j, "t", words[j], "tag", words[j], "n", i % 100 + j
            )
    ...
    for _ in env.reloadingIterator():
        for i in range(len(limits)):
            ...
            for j in range(len(ranges)):
                ...
                compare_optimized_to_not(
                    env,
                    ["ft.search", "idx", "@tag:{foo} " + numRange],
                    params,
                    "case 4 " + numRange,
                )
                ...
                ### (6) only range ###
                compare_optimized_to_not(
                    env, ["ft.search", "idx", numRange], params, "case 6"
                )
            ...
            # stop after enough results were collected
            compare_optimized_to_not(
                env, ["ft.search", "idx", "@tag:{foo}"], params, "case 10"
            )
    ...
    for _ in env.reloadingIterator():
        for i in range(len(limits)):
            ...
            for j in range(len(ranges)):
                ...
                compare_optimized_to_not(
                    env,
                    ["ft.aggregate", "idx", "@tag:{foo} " + numRange],
                    params,
                    "case 4 " + numRange,
                )
                ...
                compare_optimized_to_not(
                    env,
                    ["ft.aggregate", "idx", numRange, "SORTBY", 2, "@n", "ASC"],
                    params,
                    "case 5 " + numRange,
                )
                ...
                ### (6) only range ###
                compare_optimized_to_not(
                    env, ["ft.aggregate", "idx", numRange], params, "case 6 " + numRange
                )
            ...
            compare_optimized_to_not(
                env,
                ["ft.aggregate", "idx", "foo", "SORTBY", 2, "@n", "ASC"],
                params,
                "case 7",
            )
            ...
            compare_optimized_to_not(
                env, ["ft.aggregate", "idx", "foo"], params, "case 8"
            )
            ...
            compare_optimized_to_not(
                env,
                ["ft.aggregate", "idx", "@tag:{foo}", "SORTBY", 2, "@n", "ASC"],
                params,
                "case 9",
            )
            ...
            # stop after enough results were collected
            compare_optimized_to_not(
                env, ["ft.aggregate", "idx", "@tag:{foo}"], params, "case 10"
            )
            ...
            # stop after enough results were collected
            compare_optimized_to_not(
                env, ["ft.aggregate", "idx", "*"], params, "case 12"
            )
        # input('stop')
...
def testVector():
    ...
    env.expect(
        "FT.CREATE idx SCHEMA v VECTOR FLAT 6 TYPE FLOAT32 DIM 2 DISTANCE_METRIC L2 t TEXT n NUMERIC"
    ).ok()
    ...
    for idx, query in enumerate(queries):
        ...
        env.assertEqual(
            conn.execute_command(*search, *query),
            conn.execute_command(*search, *query, "WITHOUTCOUNT"),
            message=str(idx),
        )
        if not env.isCluster():
            ...
            env.assertEqual(
                conn.execute_command(*profile, *query),
                conn.execute_command(*profile, *query, "WITHOUTCOUNT"),
                message=str(idx),
            )
...
def testOptimizeArgs(env):
    ...
    env.assertEqual(
        conn.execute_command(*query, "DIALECT", 4),
        conn.execute_command(*query, "WITHOUTCOUNT"),
    )
    env.assertNotEqual(
        conn.execute_command(*query, "DIALECT", 4),
        conn.execute_command(*query, "WITHCOUNT"),
    )
    # DIALECT 4 and WITHCOUNT explicitly specified ==> WITHCOUNT
    env.assertEqual(
        conn.execute_command(*query, "WITHCOUNT", "DIALECT", 4),
        conn.execute_command(*query, "WITHCOUNT"),
    )
    env.assertNotEqual(
        conn.execute_command(*query, "WITHCOUNT", "DIALECT", 4),
    ...
    env.assertEqual(
        conn.execute_command(*query, "DIALECT", 3),
        conn.execute_command(*query, "WITHCOUNT"),
    )
    env.assertNotEqual(
        conn.execute_command(*query, "DIALECT", 3),
    ...
    env.assertEqual(
        conn.execute_command(*query, "WITHOUTCOUNT", "DIALECT", 3),
        conn.execute_command(*query, "WITHOUTCOUNT"),
    )
    env.assertNotEqual(
        conn.execute_command(*query, "WITHOUTCOUNT", "DIALECT", 3),
...
def testOptimizeArgsDefault():
    ...
    env.assertEqual(
        conn.execute_command(*query), conn.execute_command(*query, "WITHOUTCOUNT")
    )
    env.assertNotEqual(
        conn.execute_command(*query), conn.execute_command(*query, "WITHCOUNT")
    )
