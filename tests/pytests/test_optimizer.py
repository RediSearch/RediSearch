    ...
    profiler =  {'Iterators profile':
                    ['Type', 'INTERSECT', 'Number of reading operations', 1200, 'Child iterators', [
                        ...
                        ['Type', 'UNION', 'Query type', 'NUMERIC', 'Number of reading operations', 1200, 'Child iterators', [
                            ['Type', 'NUMERIC', 'Term', '0 - 14', 'Number of reading operations', 600, 'Estimated number of matches', 3200],
                            ['Type', 'NUMERIC', 'Term', '16 - 54', 'Number of reading operations', 600, 'Estimated number of matches', 8000]]]]],
                 'Result processors profile': [
...
def testWOLimit(env):
    ...
    ### (4) TAG and range w/o sort ###
    # stop after enough results were collected
    env.expect('ft.search', 'idx', '@tag:{foo} @n:[10 70]', *params).equal([10] + res10)
    env.expect('ft.search', 'idx', '@tag:{foo} @n:[10 40]', *params).equal([6] + res6)
    ...
    ### (6) only range ###
    # stop after enough results were collected
    env.expect('ft.search', 'idx', '@n:[10 50]', *params).equal([10] + res10)
    env.expect('ft.search', 'idx', '@n:[10 12]', 'WITHOUTCOUNT', 'RETURN', 1, 'n').equal([3] + res3)
    ...
    ### (10) no sort, no score, no sortby ###
    # stop after enough results were collected
    env.expect('ft.search', 'idx', '@tag:{foo}', *params).equal([10] + res10)
    ...
    ### (12) wildcard w/o sort ###
    # stop after enough results were collected
    env.expect('ft.search', 'idx', '*', *params).equal([10] + res10)
...
def testOptimizeArgs(env):
    ...
    conn = getConnectionByEnv(env)
    env.cmd('FT.CREATE', 'idx', 'SCHEMA', 'n', 'NUMERIC')
    # Keep more than LIMIT rows on every shard so WITHOUTCOUNT remains observably
    # different from WITHCOUNT after shard replies report their returned row count.
    for i in range(0, 100 * env.shardsCount):
        conn.execute_command('HSET', i, 'n', i)
