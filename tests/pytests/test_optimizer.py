...
def testOptimizeArgs(env):
    ...
    conn = getConnectionByEnv(env)
    env.cmd('FT.CREATE', 'idx', 'SCHEMA', 'n', 'NUMERIC')
    # More docs than shards x LIMIT: WITHOUTCOUNT reports only the rows actually pulled
    # (per shard, up to LIMIT), which must stay distinguishable from WITHCOUNT's full total.
