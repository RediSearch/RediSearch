...
def test_default_scorer_startup_validation():
    ...
    try:
        env = Env(moduleArgs=f'EXTLOAD {ext_path} DEFAULT_DIALECT 2 ENABLE_UNSTABLE_FEATURES true DEFAULT_SCORER example_scorer2', startupGraceSecs=1)
        assert not env.isUp()
    ...
    try:
        env = Env(moduleArgs=f'ENABLE_UNSTABLE_FEATURES true DEFAULT_SCORER example_scorer', startupGraceSecs=1)
        assert not env.isUp()
    except Exception as e:
        # It sometimes captures the error of it not being up (PID dead and sometimes not). We cannot have a false positive that env.isUp but we still pass the test
        assert not isinstance(e, AssertionError)
