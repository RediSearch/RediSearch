...
def test_default_scorer_behavior():
    ...
    search_BM25_in_query = env.cmd('FT.SEARCH', 'idx', 'hello', 'SCORER', 'BM25', 'WITHSCORES', 'NOCONTENT')
    env.assertNotEqual(search_default_tfidf[2], search_BM25_in_query[2])  # First document score should differ
    ...
    agg_BM25_in_query = env.cmd('FT.AGGREGATE', 'idx', 'hello', 'SCORER', 'BM25', 'WITHSCORES')
    # First document score should differ between TFIDF and BM25
    # Format: [count, score1, fields1, score2, fields2, ...]
    env.assertNotEqual(float(agg_default_tfidf[1]), float(agg_BM25_in_query[1]))  # Compare first document scores

    # Change default scorer to BM25
    env.cmd('FT.CONFIG', 'SET', 'DEFAULT_SCORER', 'BM25')
    new_default = env.cmd('FT.CONFIG', 'GET', 'DEFAULT_SCORER')
    env.assertEqual(new_default, [['DEFAULT_SCORER', 'BM25']])

    search_default_BM25 = env.cmd('FT.SEARCH', 'idx', 'hello', 'WITHSCORES', 'NOCONTENT')
    env.assertEqual(search_default_BM25, search_BM25_in_query)
    ...
    agg_default_BM25 = env.cmd('FT.AGGREGATE', 'idx', 'hello', 'WITHSCORES')
    env.assertEqual(agg_default_BM25, agg_BM25_in_query)
