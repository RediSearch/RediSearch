...
def checkSlaveSynced(
    env, slaveConn, command, expected_result, time_out=5, mapping=lambda x: x
):
    try:
        ...
        env.assertTrue(
            False, message="Failed waiting for command to be executed on slave"
        )
...
def testDelReplicate():
    ...
    for i in range(10):
        master.execute_command(
            "ft.add", "idx", "doc%d" % i, 1.0, "fields", "f", "hello world"
        )
    time.sleep(0.01)
    checkSlaveSynced(
        env, slave, ("ft.get", "idx", "doc9"), ["f", "hello world"], time_out=20
    )

    for i in range(10):
        # checking for insertion
        env.assertEqual(
            ["f", "hello world"], master.execute_command("ft.get", "idx", "doc%d" % i)
        )
        env.assertEqual(
            ["f", "hello world"], slave.execute_command("ft.get", "idx", "doc%d" % i)
        )

        # deleting
        env.assertEqual(1, master.execute_command("ft.del", "idx", "doc%d" % i))

    checkSlaveSynced(env, slave, ("ft.get", "idx", "doc9"), None, time_out=20)

    for i in range(10):
        # checking for deletion
        env.assertEqual(None, master.execute_command("ft.get", "idx", "doc%d" % i))
        env.assertEqual(None, slave.execute_command("ft.get", "idx", "doc%d" % i))
...
def testDropReplicate():
  ...
  """
    ...
    def load_master():
        ...
        env.assertEqual(
            master.execute_command("WAIT", "1", "10000"), 1
...
def testDropTempReplicate():
  ...
  """
    ...
    master.execute_command(
        "FT.CREATE", "idx", "TEMPORARY", "3600", "SCHEMA", "t", "TEXT"
    )
...
def testExpireDocsSortable():
    ...
    expireDocs(
        ...
        [2, "doc1", None, "doc2", ["t", "foo"]],
    )  # With sortby - ordered by the original value, bar > foo
...
def expireDocs(isSortable, iter1_expected_without_sortby, iter1_expected_with_sortby):
    ...
    for i in range(2):
        ...
        # First iteration
        expected_res = (
            iter1_expected_without_sortby if i == 0 else iter1_expected_with_sortby
        )
        ...
        res = master.execute_command("FT.SEARCH", "idx", "*", *sortby_cmd)
        env.assertEqual(res, [1, "doc2", ["t", "foo"]], message=(msg + " master"))
        res = slave.execute_command("FT.SEARCH", "idx", "*", *sortby_cmd)
        env.assertEqual(res, [1, "doc2", ["t", "foo"]], message=(msg + " slave"))
