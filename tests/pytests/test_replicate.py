...
def testDelReplicate():
    ...
    env.assertOk(master.execute_command('ft.create', 'idx', 'ON', 'HASH',
                 'FILTER', 'startswith(@__key, "")', 'schema', 'f', 'text'))
    ...
    for i in range(10):
        master.execute_command('ft.add', 'idx', 'doc%d' % i, 1.0, 'fields',
                               'f', 'hello world')
    time.sleep(0.01)
    checkSlaveSynced(env, slave, ('ft.get', 'idx', 'doc9'),
                     ['f', 'hello world'], time_out=20)
...
def testDropReplicate():
  ...
  '''
    for j in range(100):
        ...
        master.execute_command('HSET', 'doc%d' % j, 't', 'hello%d' %
                               j, 'tg', 'world%d' % j, 'n', j, 'g', geo)

    # test for FT.DROPINDEX
    # wait for master and slave to be in sync
    env.expect('WAIT', '1', '10000').equal(1)
    master.execute_command('FT.CREATE', 'idx', 'SCHEMA',
                           't', 'TEXT', 'n', 'NUMERIC', 'tg', 'TAG', 'g', 'GEO')
...
def testDropTempReplicate():
  ...
  '''
    ...
    master.execute_command('FT.CREATE', 'idx', 'TEMPORARY',
                           '1', 'SCHEMA', 't', 'TEXT')
