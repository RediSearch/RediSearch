...
def testUniqueSum(env):
    ...
    values = [
        ("int", str(3)),
        ("negative double", str(-0.4)),
        ("positive double", str(4.67)),
    ]
    ...
    for title, value in values:
        ...
        env.assertEqual(
            (to_dict(numeric_index_tree["range"]))["unique_sum"],
            value,
            message=f"{title} before gc",
        )
...
def testSplit(env):
    ...
    def checkRange(range, expected_val, expected_numEntries, msg):
        ...
        env.assertEqual(
            inverted_index["numEntries"],
            expected_numEntries,
            message=msg + " numEntries",
        )
...
def testOverrides(env):
    ...
    for i in range(loops):
        ...
        env.assertEqual(
            hashes_number,
            int(info["num_records"]),
            message="expected ft.info:num_records",
        )

        # size shouldn't vary more than 5% from the expected size.
        delta_size = (
            abs(expected_inverted_sz_mb - round(float(info["inverted_sz_mb"]), 4))
            / expected_inverted_sz_mb
        )
        ...
        env.assertEqual(
            hashes_number, numeric_tree["numEntries"], message="expected numEntries"
        )
        env.assertEqual(0, numeric_tree["emptyLeaves"], message="expected emptyLeaves")
        env.assertEqual(
            expected_root_max_depth,
            numeric_tree["RootMaxDepth"],
            message="expected RootMaxDepth",
        )
...
def testCompression(env):
    ...
    for i in range(repeat):
        value = accuracy * i
        env.expect("ft.search", "idx", ("@n:[%s %s]" % (value, value))).equal(
            [1, str(i), ["n", str(value)]]
        )
...
def testSanity(env):
    ...
    env.expect("ft.search", "idx", ("@n:[0 %d]" % (repeat)), "limit", 0, 0).equal(
        [repeat]
    )
...
def testCompressionConfig(env):
    ...
    for i in range(100):
        num = str(1 + i / 100.0)
        env.expect("ft.search", "idx", "@n:[%s %s]" % (num, num)).equal(
            [1, str(i), ["n", num]]
        )
...
def testNumericOperators():
    ...
    res2 = env.cmd("FT.SEARCH", "idx", "@n>= +12 @n<=+14", "NOCONTENT", "WITHCOUNT")
    ...
    env.assertEqual(res2, res1)
    res1 = env.cmd("FT.SEARCH", "idx", "@n>=3.14 @n<11.5", "NOCONTENT", "WITHCOUNT")
    ...
    # Test >
    res1 = env.cmd("FT.SEARCH", "idx", "@n>12", "NOCONTENT", "SORTBY", "n", "ASC")
    ...
    env.assertEqual(res1, [0])
    res2 = env.cmd("FT.SEARCH", "idx", "@n>+$p", "NOCONTENT", "PARAMS", 2, "p", "+inf")
    env.assertEqual(res2, res1)
    res2 = env.cmd("FT.SEARCH", "idx", "@n>+$p", "NOCONTENT", "PARAMS", 2, "p", "inf")
    env.assertEqual(res2, res1)
    ...
    # Test > -inf
    res1 = env.cmd(
        "FT.SEARCH", "idx", "@n>-inf", "NOCONTENT", "LIMIT", 0, 20, "SORTBY", "n", "ASC"
    )
    env.assertEqual(
        res1,
        [
    ...
    env.assertEqual(res2, res1)
    res2 = env.cmd(
        "FT.SEARCH", "idx", "@n>=+$p", "NOCONTENT", "WITHCOUNT", "PARAMS", 2, "p", "inf"
    )
    env.assertEqual(res2, res1)
    ...
    # Test <
    res1 = env.cmd("FT.SEARCH", "idx", "@n<15", "NOCONTENT", "SORTBY", "n", "ASC")
    env.assertEqual(
        res1,
        [9, "key12", "key7", "key6", "key9", "key8", "key1", "key2", "key3", "key4"],
    )
    res2 = env.cmd(
        "FT.SEARCH",
        "idx",
    ...
    # Test < +inf
    res1 = env.cmd("FT.SEARCH", "idx", "@n<inf", "NOCONTENT", "SORTBY", "n", "ASC")
    env.assertEqual(
        res1,
        [
            10,
    ...
    # Test <= +inf
    res1 = env.cmd(
        "FT.SEARCH", "idx", "@n<=inf", "NOCONTENT", "LIMIT", 0, 12, "SORTBY", "n", "ASC"
    )
    env.assertEqual(
        res1,
        [
    ...
    res2 = env.cmd("FT.SEARCH", "idx", "@n==$n", "NOCONTENT", "PARAMS", 2, "n", "15")
    env.assertEqual(res2, res1)
    res1 = env.cmd("FT.SEARCH", "idx", "@n==+$n", "NOCONTENT", "PARAMS", 2, "n", "15")
    env.assertEqual(res2, res1)
    res1 = env.cmd(
        "FT.SEARCH", "idx", "@n==+$n   @n==15", "NOCONTENT", "PARAMS", 2, "n", "15"
    )
    env.assertEqual(res2, res1)
    ...
    res1 = env.cmd("FT.SEARCH", "idx", "@n==-11 | @n==-10", "NOCONTENT", "WITHCOUNT")
    ...
    env.assertEqual(res2, res1)
    res2 = env.cmd("FT.SEARCH", "idx", "@n==+$n", "NOCONTENT", "PARAMS", 2, "n", "3.14")
    env.assertEqual(res2, res1)
    ...
    env.assertEqual(res2, res1)
    res2 = env.cmd(
        "FT.SEARCH", "idx", "@n==+$n", "NOCONTENT", "PARAMS", 2, "n", "-3.14"
    )
    env.assertEqual(res2, res1)
    ...
    env.assertEqual(res2, res1)
    res2 = env.cmd(
        "FT.SEARCH", "idx", "@n==$n", "NOCONTENT", "WITHCOUNT", "PARAMS", 2, "n", "inf"
    )
    env.assertEqual(res2, res1)
    res2 = env.cmd(
        "FT.SEARCH", "idx", "@n==$n", "NOCONTENT", "WITHCOUNT", "PARAMS", 2, "n", "+inf"
    ...
    env.assertEqual(res2, res1)
    res2 = env.cmd("FT.SEARCH", "idx", "@n==$n", "NOCONTENT", "PARAMS", 2, "n", "-inf")
    env.assertEqual(res2, res1)

    # Test !=
    res1 = env.cmd(
        "FT.SEARCH", "idx", "@n!=12 @n!= -10", "NOCONTENT", "SORTBY", "n", "ASC"
    )
    env.assertEqual(
        res1,
        [
    ...
    res2 = env.cmd("FT.SEARCH", "idx", "@n!=+inf", "NOCONTENT", "SORTBY", "n", "ASC")
    env.assertEqual(res2, res1)
    res2 = env.cmd(
        "FT.SEARCH",
        "idx",
    ...
    res1 = env.cmd(
        "FT.SEARCH", "idx", "@n==3.14 | @n:[10 13]", "NOCONTENT", "WITHCOUNT"
    )
    env.assertEqual(res1, [4, "key1", "key2", "key3", "key8"])
    res2 = env.cmd(
        "FT.SEARCH", "idx", "@n:[3.14 3.14] | @n>=10 @n<=13", "NOCONTENT", "WITHCOUNT"
    )
    env.assertEqual(res2, res1)

    res1 = env.cmd("FT.SEARCH", "idx", "@n==11 | @n:[10 13]", "NOCONTENT", "WITHCOUNT")
    env.assertEqual(res1, [3, "key1", "key2", "key3"])
    res2 = env.cmd(
        "FT.SEARCH", "idx", "@n:[11 11] | @n>=10 @n<=13", "NOCONTENT", "WITHCOUNT"
    )
    env.assertEqual(res2, res1)
    ...
    for operator in ["==", "!=", ">", ">=", "<", "<="]:
        ...
        env.assertEqual(res1, res2)
        res2 = env.cmd(
            "FT.SEARCH", "idx", "@n " + operator + " $p", "PARAMS", 2, "p", "0"
        )
        env.assertEqual(res1, res2)
    ...
    for operator in ["==", "!=", ">", ">=", "<", "<="]:
        ...
        env.expect("FT.SEARCH", "idx", "@n" + operator + "+(inf").error()
        env.expect(
            "FT.SEARCH", "idx", "@n" + operator + "($param", "PARAMS", 2, "param", 100
        ).error()
        env.expect(
            "FT.SEARCH", "idx", "@n" + operator + "+($param", "PARAMS", 2, "param", 100
        ).error()
        ...
        env.expect("FT.SEARCH", "idx", "@n" + operator + "w").error().contains(
            "Syntax error"
        )
        env.expect(
            "FT.SEARCH", "idx", "@n" + operator + "$p", "PARAMS", 2, "p", "w"
