...
def testNumericTree(env:Env):
    ...
    def cause_split(val):
        ...
        start_tree = env.cmd('FT.DEBUG', 'DUMP_NUMIDXTREE', idx, field)
        epsilon = 0.
        while equal_structure(start_tree, env.cmd('FT.DEBUG', 'DUMP_NUMIDXTREE', idx, field)):
            add_val(val + epsilon)
            epsilon += 0.0001

    # Recursively validate the maxDepth field in the tree
    def validate_tree(tree):
        maxDepthRange = int(env.cmd('FT.CONFIG', 'GET', '_NUMERIC_RANGES_PARENTS')[0][1])
    ...
    validate_tree(env.cmd('FT.DEBUG', 'DUMP_NUMIDXTREE', idx, field))

    env.flush()
    env.expect('FT.CONFIG', 'SET', '_NUMERIC_RANGES_PARENTS', '2').ok()
    env.expect('FT.CREATE', idx, 'SCHEMA', field, 'NUMERIC').ok()
