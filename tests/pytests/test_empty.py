...
def TestEmptyNonIndexed():
    ...
    for dialect in range(2, MAX_DIALECT + 1):
        ...
        for query in ['@tag:{""}', "@tag:{''}", '@text:""', "@text:''"]:
            env.expect("FT.SEARCH", "idx", query).error().contains(
        ...
        for query in ['@tag:""', "@tag:''", '@tag:("")', "@tag:('')"]:
            env.expect("FT.SEARCH", "idx", query).equal(EMPTY_RESULT)

    env.flush()

    env.expect("FT.CREATE", "idx", "SCHEMA", "text1", "TEXT", "text2", "TEXT").ok()
    env.expect(
        "FT.CREATE", "idx2", "SCHEMA", "text1", "TEXT", "text2", "TEXT", "INDEXEMPTY"
    ).ok()
    ...
    for dialect in range(2, MAX_DIALECT + 1):
        ...
        # values
        env.expect("FT.SEARCH", "idx", '@text1|text2:""').error().contains(
            "Use `INDEXEMPTY` in field creation in order to index and query for empty strings"
        )

        # A result should be returned in case at least on of the fields indexes
        # empty values
        env.expect("FT.SEARCH", "idx2", '@text1|text2:""').equal(EMPTY_RESULT)
        conn.execute_command("HSET", "h1", "text1", "", "text2", "")
        env.expect("FT.SEARCH", "idx2", '@text1|text2:""').equal(
            [1, "h1", ["text1", "", "text2", ""]]
        )
...
def testEmptyTag():
    ...
    def testEmptyTagHash(env, conn, idx, dialect):
        ...
        res = conn.execute_command(
            "FT.SEARCH", idx, '@t:{""} | @t:{foo}', "SORTBY", "t", "ASC", "WITHCOUNT"
        )
        expected = [2, "h1", ["t", ""], "h2", ["t", "bar,"]]
        env.assertEqual(res, expected)
        ...
        res = conn.execute_command(
            "FT.SEARCH", idx, 'love | @t:{""}', "SORTBY", "text", "ASC"
        )
        expected = [
            2,
            "h1",
        ...
        expected = [ANY, ["t", "", "count", "2"], ["t", "movie", "count", "2"]]
        cmd_assert(env, cmd, expected)
    ...
    for dialect in range(2, MAX_DIALECT + 1):
        ...
        env.expect(
            "FT.CREATE", "idx", "SCHEMA", "t", "TAG", "INDEXEMPTY", "text", "TEXT"
        ).ok()
...
def testEmptyTagJSON():
    ...
    for dialect in range(2, MAX_DIALECT + 1):
        ...
        res = conn.execute_command(
            "FT.SEARCH", "jidx", '@arr:{""}', "RETURN", "1", "arr"
        )
        expected = EMPTY_RESULT
        env.assertEqual(res, expected)
...
def testEmptyText():
    ...
    for dialect in range(2, MAX_DIALECT + 1):
        ...
        # Create an index with a SORTABLE TAG field, that also indexes empty strings
        env.expect(
            "FT.CREATE", "idx_sortable", "SCHEMA", "t", "TEXT", "INDEXEMPTY", "SORTABLE"
        ).ok()
...
def testEmptyExplainCli():
    ...
    for dialect in range(2, MAX_DIALECT + 1):
        ...
        res = env.cmd("FT.EXPLAINCLI", "idx", '@text:("" "")')
        expected = ["@text:INTERSECT {", '  @text:""', '  @text:""', "}", ""]
        env.assertEqual(res, expected)
...
def testInvalidUseOfEmptyString():
    ...
    for dialect in range(2, MAX_DIALECT + 1):
        ...
        res = conn.execute_command(
            "FT.SEARCH", "idx", "@text:(%$p%)", "PARAMS", 2, "p", '""'
        )
        env.assertEqual(res, EMPTY_RESULT)
        ...
        env.expect("FT.SEARCH", "idx", '@location:[1.23 4.56 10 ""]').error().contains(
            expected_error
        )
        env.expect(
            "FT.SEARCH", "idx", "@location:[1.23 4.56 10 $p]", "PARAMS", 2, "p", ""
        ).error().contains(expected_error)

        expected_error = "Syntax error"
        env.expect("FT.SEARCH", "idx", "@location:[" " 4.56 10 km]").error().contains(
            expected_error
        )
        env.expect("FT.SEARCH", "idx", "@location:[1.23 " " 10 km]").error().contains(
            expected_error
        )
        env.expect("FT.SEARCH", "idx", "@location:[1.23 4.56 " " km]").error().contains(
            expected_error
        )
        ...
        res = env.execute_command(
            "FT.SEARCH", "idx", "@location:[$long 4.56 10 km]", "PARAMS", 2, "long", ""
        )
        env.assertEqual(res, EMPTY_RESULT)
        res = env.execute_command(
            "FT.SEARCH", "idx", "@location:[1.23 $lat 10 km]", "PARAMS", 2, "lat", ""
        )
        env.assertEqual(res, EMPTY_RESULT)
        env.expect(
            "FT.SEARCH",
            "idx",
            "@location:[1.23 4.56 $radius km]",
            "PARAMS",
            2,
            "radius",
            "",
        ).error().contains("Invalid GeoFilter radius")

        # Invalid use of empty string as $weight value
        expected_error = "Invalid value () for `weight`"
        env.expect("FT.SEARCH", "idx", '@t:{abc}=>{$weight:""}').error().contains(
            expected_error
        )
        env.expect(
            "FT.SEARCH", "idx", "@t:{abc}=>{$weight:$p}", "PARAMS", 2, "p", ""
        ).error().contains(expected_error)

        # Invalid use of empty string as $inorder value
        expected_error = "Invalid value () for `inorder`"
        env.expect("FT.SEARCH", "idx", '@t:{abc}=>{$inorder:""}').error().contains(
            expected_error
        )
        env.expect(
            "FT.SEARCH", "idx", "@t:{abc}=>{$inorder:$p}", "PARAMS", 2, "p", ""
        ).error().contains(expected_error)

        # Invalid use of empty string as $slop value
        expected_error = "Invalid value () for `slop`"
        env.expect("FT.SEARCH", "idx", '@t:{abc}=>{$slop:""}').error().contains(
            expected_error
        )
        env.expect(
            "FT.SEARCH", "idx", "@t:{abc}=>{$slop:$p}", "PARAMS", 2, "p", ""
        ).error().contains(expected_error)

        # Invalid use of empty string as $phonetic value
        expected_error = "Invalid value () for `phonetic`"
        env.expect(
            "FT.SEARCH", "idx", '@x:(hello=>{$phonetic:""} world)'
        ).error().contains(expected_error)
        env.expect(
            "FT.SEARCH", "idx", "@x:(hello=>{$phonetic:$p} world)", "PARAMS", 2, "p", ""
        ).error().contains(expected_error)
        ...
        env.expect(
            "FT.AGGREGATE",
            "idx",
            '*=>[KNN 3 @v $blob]=>{$yield_distance_as:""}',
            "PARAMS",
            "2",
            "blob",
        ...
        env.expect(
            "FT.AGGREGATE",
            "idx",
            "*=>[KNN 3 @v $blob]=>{$yield_distance_as:$p}",
            "PARAMS",
            4,
            "blob",
            create_np_array_typed([0] * dim).tobytes(),
        ...
        env.expect("FT.SEARCH", "idx", '@text|"":(abc)').error().contains(
            "Syntax error"
        )
        ...
        env.expect("FT.SEARCH", "idx", '@text|text|"":(abc)').error().contains(
            "Syntax error"
        )

        env.expect("FT.SEARCH", "idx", '@t|"":{abc}').error().contains("Syntax error")

        env.expect("FT.SEARCH", "idx", '@""|t:{abc}').error().contains("Syntax error")

        env.expect("FT.SEARCH", "idx", '@t|t|"":{abc}').error().contains("Syntax error")

        # Invalid use of an empty string in a vector query
        env.expect("FT.SEARCH", "idx", '*=>["" 4 @v $blob AS dist]').error().contains(
            "Expecting Vector Similarity command"
        )
        env.expect("FT.SEARCH", "idx", '*=>[KNN "" @v $blob AS dist]').error().contains(
            "Syntax error"
        )
...
def testEmptyParam():
    ...
    for dialect in range(2, MAX_DIALECT + 1):
        ...
        res = (
            env.expect(
                "FT.SEARCH", "idx", "@text1:($p) | @text2:($p)", "PARAMS", 2, "p", ""
            )
            .error()
            .contains(
                "Use `INDEXEMPTY` in field creation in order to index and query for empty strings"
        ...
        # Same with result
        res = env.cmd("FT.SEARCH", "idx", "@text1:($p)", "PARAMS", 2, "p", "")
        expected = [1, "h3", ["text1", ""]]
        env.assertEqual(res, expected)
...
def testemptyfuzzy():
    ...
    env.expect(
        "FT.CREATE", "idx", "SCHEMA", "t", "TEXT", "INDEXEMPTY", "t2", "TEXT"
    ).ok()
