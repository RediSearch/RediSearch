...
def test_drop_index_memory():
    ...
    env.expect('FT.INFO', DEFAULT_INDEX_NAME).error().contains(f"Index not found")
    ...
    env.expect('FT.SEARCH', 'idx', f'*=>[KNN 1 @{DEFAULT_FIELD_NAME} $vec_param]', 'PARAMS', 2, 'vec_param',
               query.tobytes(), 'NOCONTENT').error().contains(f"Index not found")
...
def test_drop_index_during_query():
    ...
    env.expect('FT.INFO', DEFAULT_INDEX_NAME).error().contains(f"Index not found")
    env.expect(*query_cmd).error().contains(f"Index not found")
    # Resume the query
