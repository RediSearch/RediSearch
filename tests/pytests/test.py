...
def testAddErrors(env):
    ...
    env.expect("ft.add fake_idx doc1 1.0 fields foo bar").error().contains(
        "Unknown index name"
    )
...
def testConditionalUpdate(env):
    ...
    env.assertOk(
        env.cmd("ft.add", "idx", "1", "1", "fields", "foo", "hello", "bar", "123")
    )
    env.assertOk(
        env.cmd(
            "ft.add",
    ...
    env.assertOk(
        env.cmd("FT.ADD", "idx", "666", "1", "IF", "@bar > 42", "FIELDS", "bar", "15")
    )
    ...
    with env.assertResponseError():
        env.assertOk(
            env.cmd(
                "FT.ADD", "idx", "666", "1", "IF", "@bar > 42", "FIELDS", "bar", "15"
            )
        )
...
def testUnionIdList(env):
    ...
    N = 100
    env.expect(
        "ft.create", "test", "ON", "HASH", "SCHEMA", "tags", "TAG", "waypoint", "GEO"
    ).ok()
    env.expect(
        "ft.add",
        "test",
    ...
    res = env.cmd(
        "ft.search",
        "test",
        "@waypoint:[-113.52 53.52 20 mi]|@tags:{ontario}",
        "nocontent",
    )
...
def testAttributes(env):
    ...
    env.assertOk(
        env.cmd(
            "ft.add", "idx", "doc1", 1.0, "fields", "title", "t1 t2", "body", "t3 t4 t5"
        )
    )
    env.assertOk(
        env.cmd(
            "ft.add", "idx", "doc2", 1.0, "fields", "body", "t1 t2", "title", "t3 t5"
        )
    )
    ...
    res = env.cmd(
        "ft.search",
        "idx",
        "(@title:(t1 t2) => {$weight: 0.2}) |(@body:(t1 t2) => {$weight: 0.5})",
        "nocontent",
    )
    ...
    res = env.cmd(
        "ft.search",
        "idx",
        "(@title:(t1 t2) => {$weight: 2.5}) |(@body:(t1 t2) => {$weight: 0.5})",
        "nocontent",
    )
    env.assertEqual([2, "doc1", "doc2"], res)

    res = env.cmd("ft.search", "idx", "(t3 t5) => {$slop: 4}", "nocontent")
    env.assertEqual([2, "doc2", "doc1"], res)
    res = env.cmd("ft.search", "idx", "(t5 t3) => {$slop: 0}", "nocontent")
...
def testUnion(env):
    ...
    for i in range(N):
        ...
        env.expect(
            ...
            "fields",
            "f",
            "hello world" if i % 2 == 0 else "hallo werld",
        ).ok()
...
def testSearch(env):
    ...
    for _ in env.reloadingIterator():
        ...
        expected = [
            2,
            "doc2",
            ["title", "hello another world", "body", "lorem ist ipsum lorem lorem"],
            "doc1",
            ["title", "hello world", "body", "lorem ist ipsum"],
        ]
...
def testGet(env):
    ...
    for i in range(100):
        ...
        env.assertIsNone(env.cmd("ft.get", "idx", "doc%dsdfsd" % i))
    env.expect("ft.get", "no_idx", "doc0").error().contains("Unknown Index name")

    rr = env.cmd("ft.mget", "idx", *("doc%d" % i for i in range(100)))
    ...
    env.expect(
        "ft.add idx doc 0.1 language arabic payload redislabs fields foo foo"
    ).ok()
...
def testDelete(env):
    ...
    for i in range(100):
        ...
        env.assertEqual(
            1, env.cmd("ft.del", "idx", "doc%d" % i, "DD" if i % 2 == 0 else "")
        )
...
def testReplace(env):
    ...
    with env.assertResponseError():
        ...
        res = env.cmd("ft.add", "idx", "doc1", 1.0, "fields", "f", "hello world")

    # now replace doc1 with a different content
    env.expect(
        "ft.add", "idx", "doc1", 1.0, "replace", "fields", "f", "goodbye universe"
    ).ok()
    ...
    for _ in env.reloadingIterator():
        ...
        res = env.cmd("ft.search", "idx", "goodbye universe", "nocontent")
        env.assertEqual(1, res[0])
...
def testDrop(env):
    ...
    env.expect("FT.DROP", "idx", "KEEPDOCS", "666").error().contains(
        "wrong number of arguments"
    )
...
def testDelete(env):
    ...
    env.expect("FT.DROPINDEX", "idx", "dd", "666").error().contains(
        "wrong number of arguments"
    )
...
def testCustomStopwords(env):
    ...
    env.expect(
        "ft.create", "idx3", "ON", "HASH", "stopwords", 0, "schema", "foo", "text"
    ).ok()
    assertInfoField(env, "idx3", "stopwords_list", [])

    # 2nd Index with NO stopwords - check global is used and freed
    env.expect(
        "ft.create", "idx4", "ON", "HASH", "stopwords", 0, "schema", "foo", "text"
    ).ok()
    ...
    env.expect(
        "ft.search", "idx5", "@foo:title=>{$inorder:true}", "DIALECT", "2"
    ).equal([0])
    ...
    for _ in env.reloadingIterator():
        ...
        env.assertEqual(
            [1, "doc1"], env.cmd("ft.search", "idx", "hello world", "nocontent")
        )
        env.assertEqual([0], env.cmd("ft.search", "idx", "to be or not", "nocontent"))

        # Custom SW index should return results just for 'to be or not'
        env.assertEqual([0], env.cmd("ft.search", "idx2", "hello world", "nocontent"))
        env.assertEqual(
            [1, "doc2"], env.cmd("ft.search", "idx2", "to be or not", "nocontent")
        )

        # No SW index should return results for both
        env.assertEqual(
            [1, "doc1"], env.cmd("ft.search", "idx3", "hello world", "nocontent")
        )
        env.assertEqual(
            [1, "doc2"], env.cmd("ft.search", "idx3", "to be or not", "nocontent")
        )
...
def testNoStopwords(env):
    ...
    for i in range(100):
        env.cmd(
            ...
            "fields",
            "title",
            "hello world" if i % 2 == 0 else "hello worlds",
        )

    res = env.cmd("ft.search", "idx", "hello a world", "NOCONTENT")
    env.assertEqual(100, res[0])

    res = env.cmd("ft.search", "idx", "hello a world", "VERBATIM", "NOCONTENT")
    env.assertEqual(50, res[0])
...
def testOptional(env):
    env.expect("ft.create", "idx", "ON", "HASH", "schema", "foo", "text").ok()
    env.expect("ft.add", "idx", "doc1", 1.0, "fields", "foo", "hello wat woot").ok()
    env.expect("ft.add", "idx", "doc2", 1.0, "fields", "foo", "hello world woot").ok()
    env.expect("ft.add", "idx", "doc3", 1.0, "fields", "foo", "hello world werld").ok()

    expected = [3, "doc1", "doc2", "doc3"]
    res = env.cmd("ft.search", "idx", "hello", "nocontent")
    env.assertEqual(res, expected)
    res = env.cmd("ft.search", "idx", "hello world", "nocontent", "scorer", "DISMAX")
...
def testExplain(env: Env):
    ...
    q = [
        ...
        "10",
        "B",
        b"\xa4\x21\xf5\x42\x18\x07\x00\xc7",
    ]
    res = env.cmd("ft.explain", "idx", *q)
    ...
    # range query
    q = [
        "@v:[VECTOR_RANGE $r $B]=>{$epsilon: 1.2; $yield_distance_as:dist}",
        "DIALECT",
        2,
        "PARAMS",
        "4",
    ...
    q = [
        "(@t:hello world) => [KNN $k @v $B EF_RUNTIME 100]",
        "DIALECT",
        2,
        "PARAMS",
        "4",
    ...
    env.assertEqual(
        run_command_on_all_shards(env, config_cmd(), "SET", "DEFAULT_DIALECT", 2),
        ["OK"] * env.shardsCount,
    )
    ...
    _testExplain(
        env,
        "idx",
        ["@t:(w'*')=>{$weight: 4; $slop:100; $inorder:true;}"],
    ...
    _testExplain(
        env,
        "idx",
        ["@g:[120.53232 12.112233 30.5 ft]"],
        "GEO g:{120.532320,12.112233 --> 30.500000 ft}\n",
    )

    # test numeric ranges
    _testExplain(
        env, "idx", ["@bar:[10 100]"], "NUMERIC {10.000000 <= @bar <= 100.000000}\n"
    )

    _testExplain(
        env, "idx", ["@bar:[-INF 100]"], "NUMERIC {-inf <= @bar <= 100.000000}\n"
    )

    _testExplain(env, "idx", ["@bar:[10 Inf]"], "NUMERIC {10.000000 <= @bar <= inf}\n")

    _testExplain(env, "idx", ["@bar:[-inf (inf]"], "NUMERIC {-inf <= @bar < inf}\n")

    _testExplain(
        env,
        "idx",
        ["@bar:[(-1 $n]", "PARAMS", "2", "n", "10"],
    ...
    _testExplain(
        env,
        "idx",
        ["@bar:[(-22 (+$n]", "PARAMS", "2", "n", "50"],
        "NUMERIC {-22.000000 < @bar < 50.000000}\n",
    )

    # test numeric operators
    _testExplain(env, "idx", ["@bar>1"], "NUMERIC {1.000000 < @bar <= inf}\n")

    _testExplain(env, "idx", ["@bar>=2"], "NUMERIC {2.000000 <= @bar <= inf}\n")

    _testExplain(env, "idx", ["@bar<-1"], "NUMERIC {-inf <= @bar < -1.000000}\n")

    _testExplain(env, "idx", ["@bar<=-3.14"], "NUMERIC {-inf <= @bar <= -3.140000}\n")

    _testExplain(env, "idx", ["@bar==5.7"], "NUMERIC {5.700000 <= @bar <= 5.700000}\n")
    ...
    _testExplain(env, "idx_im", ["@tag:{bar} | @tag:{go} ismissing(@t)"], expected)
    ...
    _testExplain(env, "idx_im", ["-@tag:{bar} | @tag:{foo} ismissing(@tag)"], expected)
    ...
    _testExplain(env, "idx_im", ["@t:(bar) | -@tag:{foo} -ismissing(@t)"], expected)
    expected = (
        "UNION {\n"
        "  INTERSECT {\n"
        "    NOT{\n"
    ...
    _testExplain(env, "idx_im", ["-ismissing(@t) ismissing(@t) | @tag:{bar}"], expected)
    ...
    _testExplain(
        env, "idx_im", ["ismissing(@t) | -@tag:{bar} -ismissing(@t)"], expected
    )
    ...
    _testExplain(env, "idx_im", ["-ismissing(@t) | -@t:{bar} ismissing(@t)"], expected)
...
def testNoIndex(env):
    ...
    res = env.cmd("ft.search", "idx", "lorem ipsum", "nocontent")
    env.assertEqual([0], res)
    res = env.cmd("ft.search", "idx", "@extra:hello", "nocontent")
    env.assertEqual([0], res)
    res = env.cmd("ft.search", "idx", "@num:[1 1]", "nocontent")
    env.assertEqual([0], res)
...
def testPartial(env):
    ...
    env.expect(
        "ft.add", "idx", "doc12", "0.1", "replace", "partial", "fields", "num1", "redis"
    ).equal("OK")
    ...
    assertResultsEqual(
        ...
        [
            2,
            "doc1",
            ["foo", "hello world", "num", "3", "extra", "jorem gipsum"],
            "doc2",
    ...
    env.expect(
        "ft.add", "idx", "doc1", "0.1", "replace", "partial", "fields", "foo", "wat wet"
    ).ok()
    res = env.cmd("ft.search", "idx", "hello world", "nocontent")
    env.assertEqual([1, "doc2"], res)
    ...
    res = env.cmd("ft.search", "idx", "wat", "nocontent", "withscores")
    ...
    res = env.cmd("ft.search", "idx", "wat", "nocontent", "withpayloads")
...
def testPaging(env):
    ...
    for i in range(N):
        env.expect(
            "ft.add", "idx", "%d" % i, 1, "fields", "foo", "hello", "bar", i
        ).ok()
    ...
    with env.assertResponseError():
        env.cmd("ft.search", "idx", "hello", "nocontent", "limit", 0, -1)
    with env.assertResponseError():
        env.cmd("ft.search", "idx", "hello", "nocontent", "limit", -1, 10)
    with env.assertResponseError():
        env.cmd("ft.search", "idx", "hello", "nocontent", "limit", 0, 2000000)
    ...
    for i in range(N):
        env.assertEqual(
            ...
            conn.execute_command(
                "hset", "doc%d" % i, "foo", "constant term%d" % (random.randrange(0, 5))
            ),
        )
    for _ in env.reloadingIterator():
        waitForIndex(env, "idx")
...
def testPrefixNodeCaseSensitive(env):
    ...
    queries_expectations = {
        ...
        "TAG_CASESENSITIVE": {
            "lowercase": {"query": ["@t:{*el*}"], "expectation": [2, "doc1", "doc3"]},
            "lowercase_params": {
                "query": ["@t:{*$p*}", "PARAMS", "2", "p", "el", "DIALECT", "2"],
                "expectation": [2, "doc1", "doc3"],
            },
            "uppercase": {"query": ["@t:{*EL*}"], "expectation": [2, "doc2", "doc4"]},
            "uppercase_params": {
                "query": ["@t:{*$p*}", "PARAMS", "2", "p", "EL", "DIALECT", "2"],
...
def testSortBy(env):
    ...
    for _ in env.reloadingIterator():
        waitForIndex(env, "idx")
        res = env.cmd("ft.search", "idx", "world", "nocontent", "sortby", "foo")
        env.assertEqual(
            [
                100,
                "doc0",
        ...
        res = env.cmd("ft.search", "idx", "world", "nocontent", "sortby", "foo", "desc")
        env.assertEqual(
            [
                100,
                "doc99",
        ...
        res = env.cmd("ft.search", "idx", "world", "nocontent", "sortby", "bar", "desc")
        env.assertEqual(
            [
                100,
                "doc0",
        ...
        res = env.cmd("ft.search", "idx", "world", "nocontent", "sortby", "bar", "asc")
        env.assertEqual(
            [
                100,
                "doc99",
...
def testNot(env):
    ...
    env.assertEqual(
        env.cmd("ft.search", "idx", "constant -dasdfasdf", "nocontent")[0], N
    )
    # not on env term
    ...
    env.expect(
        "ft.search",
        "idx",
        "constant -(term0|term1|term2|term3|term4|nothing)",
        "nocontent",
    ).equal([0])
...
def testNestedIntersection(env):
    ...
    res = [
        env.cmd("ft.search", "idx", "foo bar baz gaz", "nocontent"),
        env.cmd("ft.search", "idx", "@a:foo @b:bar @c:baz @d:gaz", "nocontent"),
        env.cmd("ft.search", "idx", "@b:bar @a:foo @c:baz @d:gaz", "nocontent"),
        env.cmd("ft.search", "idx", "@c:baz @b:bar @a:foo @d:gaz", "nocontent"),
        env.cmd("ft.search", "idx", "@d:gaz @c:baz @b:bar @a:foo", "nocontent"),
        env.cmd("ft.search", "idx", "@a:foo (@b:bar (@c:baz @d:gaz))", "nocontent"),
        env.cmd(
            "ft.search", "idx", "@c:baz (@a:foo (@b:bar (@c:baz @d:gaz)))", "nocontent"
        ),
        env.cmd("ft.search", "idx", "@b:bar (@a:foo (@c:baz @d:gaz))", "nocontent"),
        env.cmd("ft.search", "idx", "@d:gaz (@a:foo (@c:baz @b:bar))", "nocontent"),
        env.cmd("ft.search", "idx", "foo (bar baz gaz)", "nocontent"),
        env.cmd("ft.search", "idx", "foo (bar (baz gaz))", "nocontent"),
        env.cmd("ft.search", "idx", "foo (bar (foo bar) (foo bar))", "nocontent"),
        env.cmd("ft.search", "idx", "foo (foo (bar baz (gaz)))", "nocontent"),
        env.cmd(
            "ft.search",
            "idx",
            "foo (foo (bar (baz (gaz (foo bar (gaz))))))",
            "nocontent",
        ),
    ]
...
def testInKeys(env):
    ...
    for i in range(200):
        env.expect(
            "ft.add", "idx", "doc%d" % i, 1.0, "fields", "foo", "hello world"
        ).ok()
...
def testSlopInOrder(env):
    ...
    res = env.cmd("ft.search", "idx", "t2 t1", "slop", "0", "nocontent")
    env.assertEqual(1, res[0])
    env.assertEqual("doc1", res[1])
    env.assertEqual(0, env.cmd("ft.search", "idx", "t2 t1", "slop", "0", "inorder")[0])
    env.assertEqual(1, env.cmd("ft.search", "idx", "t1 t2", "slop", "0", "inorder")[0])

    env.assertEqual(2, env.cmd("ft.search", "idx", "t1 t2", "slop", "1", "inorder")[0])
    env.assertEqual(3, env.cmd("ft.search", "idx", "t1 t2", "slop", "2", "inorder")[0])
    env.assertEqual(4, env.cmd("ft.search", "idx", "t1 t2", "slop", "3", "inorder")[0])
    env.assertEqual(4, env.cmd("ft.search", "idx", "t1 t2", "inorder")[0])
    env.assertEqual(0, env.cmd("ft.search", "idx", "t t1", "inorder")[0])
    env.assertEqual(2, env.cmd("ft.search", "idx", "t1 t2 t3 t4")[0])
    env.assertEqual(0, env.cmd("ft.search", "idx", "t1 t2 t3 t4", "inorder")[0])
...
def testSlopInOrderIssue1986(env):
    ...
    env.assertEqual(
        [1, "doc2", ["title", "t2 t1"]],
        env.cmd("ft.search", "idx", "t2 t1", "slop", "0", "inorder"),
    )
    env.assertEqual(
        [1, "doc1", ["title", "t1 t2"]],
        env.cmd("ft.search", "idx", "t1 t2", "slop", "0", "inorder"),
    )
...
def testExact(env):
    ...
    env.cmd("HSET", "{doc}:1", "title", "hello world", "body", "lorem ist ipsum")
    env.cmd(
        "HSET",
        "{doc}:2",
        "title",
    ...
    for dialect in range(1, MAX_DIALECT + 1):
        res = env.cmd(
            "ft.search", "idx", '"hello world"', "verbatim", "DIALECT", dialect
        )
        ...
        res = env.cmd(
            "ft.search", "idx", 'hello "another world"', "verbatim", "DIALECT", dialect
        )
        env.assertEqual(3, len(res))
        env.assertEqual(1, res[0])
...
def testGeoErrors(env):
    ...
    env.expect("ft.add idx hotel 1.0 fields name hill location -0.1757,51.5156").equal(
        "OK"
    )
    env.expect("ft.search idx hilton geofilter location -0.1757 51.5156 1 km").equal(
        [0]
    )
    ...
    # Query errors
    env.expect(
        "ft.search idx hilton geofilter location lon 51.5156 1 km"
    ).error().contains(
        "Bad arguments for <lon>: Could not convert argument to expected type"
    )
    env.expect(
        "ft.search idx hilton geofilter location 51.5156 lat 1 km"
    ).error().contains(
        "Bad arguments for <lat>: Could not convert argument to expected type"
    )
    env.expect(
        "ft.search idx hilton geofilter location -0.1757 51.5156 radius km"
    ).error().contains(
        "Bad arguments for <radius>: Could not convert argument to expected type"
    )
    env.expect(
        "ft.search idx hilton geofilter location -0.1757 51.5156 1 fake"
    ).error().contains("Unknown distance unit fake")
    env.expect(
        "ft.search idx hilton geofilter location -0.1757 51.5156 1"
    ).error().contains("GEOFILTER requires 5 arguments")
...
def testGeo(env):
    ...
    gsearch_inline = lambda query, lon, lat, dist, unit="km": env.cmd(
        "ft.search",
        "idx",
        "{} @location:[{} {} {} {}]".format(query, lon, lat, dist, unit),
        "LIMIT",
        0,
        20,
    )
    ...
    env.expect(
        "ft.create", "idx", "ON", "HASH", "schema", "name", "text", "location", "geo"
    ).ok()
...
def testTagErrors(env):
    env.expect("ft.create", "test", "ON", "HASH", "SCHEMA", "tags", "TAG").equal("OK")
    env.expect("ft.add", "test", "1", "1", "FIELDS", "tags", "alberta").equal("OK")
    env.expect("ft.add", "test", "2", "1", "FIELDS", "tags", "ontario. alberta").equal(
        "OK"
    )
...
def testGeoDeletion(env):
    ...
    env.cmd(
        "ft.add", "idx", "doc3", 1.0, "fields", "g1", "-0.1757,51.5156", "t1", "hello"
    )
    ...
    env.cmd("ft.add", "idx", "doc2", 1.0, "replace", "fields", "t1", "just text here")
    for _ in range(100):
        forceInvokeGC(env, "idx")
...
def testScorerSelection(env):
    ...
    with env.assertResponseError():
        res = env.cmd("ft.search", "idx", "foo", "scorer", "NOSUCHSCORER")
...
def testFieldSelectors(env):
    ...
    res = env.cmd("ft.search", "idx", "@TiTle:hello world", "nocontent")
    env.assertEqual(res, [1, "doc1"])
    res = env.cmd("ft.search", "idx", "@BoDy:hello world", "nocontent")
    env.assertEqual(res, [1, "doc2"])

    res = env.cmd("ft.search", "idx", "@BoDy:hello @TiTle:world", "nocontent")
    env.assertEqual(res, [0])

    res = env.cmd("ft.search", "idx", "@BoDy:hello world @TiTle:world", "nocontent")
    env.assertEqual(res, [0])
    res = env.cmd(
    ...
    res = env.cmd("ft.search", "idx", "@BoDy:(hello|foo world|bar)", "nocontent")
    env.assertEqual(py2sorted(res), py2sorted([2, "doc1", "doc2"]))

    res = env.cmd("ft.search", "idx", "@BoDy|TiTle:(hello world)", "nocontent")
    env.assertEqual(py2sorted(res), py2sorted([2, "doc1", "doc2"]))

    res = env.cmd("ft.search", "idx", "@יוניקוד:(unicode)", "nocontent")
    env.assertEqual(py2sorted(res), py2sorted([2, "doc1", "doc2"]))

    res = env.cmd("ft.search", "idx", "@field\\.with\\,punct:(punt)", "nocontent")
    env.assertEqual(py2sorted(res), py2sorted([2, "doc1", "doc2"]))


def testStemming(env):
    env.expect("ft.create", "idx", "ON", "HASH", "schema", "title", "text").ok()
    env.expect("ft.add", "idx", "doc1", 0.5, "fields", "title", "hello kitty").ok()
    env.expect("ft.add", "idx", "doc2", 1.0, "fields", "title", "hello kitties").ok()

    res = env.cmd("ft.search", "idx", "hello kitty", "nocontent")
    env.assertEqual(3, len(res))
    env.assertEqual(2, res[0])

    res = env.cmd("ft.search", "idx", "hello kitty", "nocontent", "verbatim")
    env.assertEqual(2, len(res))
    env.assertEqual(1, res[0])
...
def testExpander(env):
    env.expect("ft.create", "idx", "ON", "HASH", "schema", "title", "text").ok()
    env.expect("ft.add", "idx", "doc1", 0.5, "fields", "title", "hello kitty").ok()
    res = env.cmd("ft.search", "idx", "kitties", "nocontent", "expander", "SBSTEM")
    env.assertEqual(2, len(res))
    env.assertEqual(1, res[0])

    res = env.cmd("ft.search", "idx", "kitties", "nocontent", "expander", "noexpander")
    env.assertEqual(1, len(res))
    env.assertEqual(0, res[0])

    res = env.cmd("ft.search", "idx", "kitti", "nocontent")
    env.assertEqual(2, len(res))
    env.assertEqual(1, res[0])

    res = env.cmd("ft.search", "idx", "kitti", "nocontent", "verbatim")
    env.assertEqual(1, len(res))
    env.assertEqual(0, res[0])
...
def testNumericRange(env):
    ...
    env.expect(
        "ft.search", "idx", "hello kitty", "filter", "score", 5
    ).error().contains("FILTER requires 3 arguments")
    env.expect(
        "ft.search", "idx", "hello kitty", "filter", "score", 5, "-inf"
    ).error().contains("Bad upper range: -inf")
    env.expect(
        "ft.search", "idx", "hello kitty", "filter", "score", 5, "(-inf"
    ).error().contains("Bad upper range: -inf")
    env.expect(
        "ft.search", "idx", "hello kitty", "filter", "score", "inf", 5
    ).error().contains("Bad lower range: inf")
    env.expect(
        "ft.search", "idx", "hello kitty", "filter", "score", "(inf", 5
    ).error().contains("Bad lower range: inf")
    env.expect(
    ...
    env.expect(
        ...
        "n",
        "10",
    ).error().contains("Bad upper range: $n")
    ...
    for _ in env.reloadingIterator():
        waitForIndex(env, "idx")
        res = env.cmd(
            "ft.search", "idx", "hello kitty", "nocontent", "filter", "score", 0, 100
        )
        ...
        res = env.cmd(
            "ft.search", "idx", "hello kitty", "nocontent", "filter", "score", 0, 50
        )
        ...
        env.assertEqual(49, res[0])
        res = env.cmd("ft.search", "idx", "@score:[(0 (50]", "verbatim", "nocontent")
        env.assertEqual(49, res[0])
        res = env.cmd(
            "ft.search",
            "idx",
            "hello kitty -@score:[(0 (50]",
            "verbatim",
            "nocontent",
            "limit",
            0,
...
def testNotIter(env):
    ...
    # middle shunk
    res = env.cmd("ft.search", "idx", "-@score:[2 4]", "verbatim", "nocontent")
    env.assertEqual(5, res[0])
    ...
    # start chunk
    res = env.cmd("ft.search", "idx", "-@score:[0 2]", "verbatim", "nocontent")
    env.assertEqual(5, res[0])
    env.debugPrint(", ".join(toSortedFlatList(res[1:])), force=TEST_DEBUG)
    ...
    # end chunk
    res = env.cmd("ft.search", "idx", "-@score:[5 7]", "verbatim", "nocontent")
    env.assertEqual(5, res[0])
    env.debugPrint(", ".join(toSortedFlatList(res[1:])), force=TEST_DEBUG)
    ...
    # whole chunk
    res = env.cmd("ft.search", "idx", "-@score:[0 7]", "verbatim", "nocontent")
    env.assertEqual(0, res[0])
...
def testGarbageCollector(env):
    ...
    for i in range(N):
        env.expect(
            ...
            "fields",
            "foo",
            " ".join(("term%d" % random.randrange(0, 10) for i in range(10))),
        ).ok()
    ...
    def get_stats(r):
        ...
        gc_stats = {
            d["gc_stats"][x]: float(d["gc_stats"][x + 1])
            for x in range(0, len(d["gc_stats"]), 2)
        }
        d["gc_stats"] = gc_stats
        return d
    ...
    if not env.isCluster():
        ...
        env.assertGreater(stats["gc_stats"]["bytes_collected"], currentIndexSize)
...
def testReturning(env):
    ...
    # also in RETURN
    res = env.cmd("ft.search", "idx", "val*", "return", 1, "f1", "sortby", "n1", "ASC")
    row = res[2]
...
def _test_create_options_real(env, *options):
    ...
    # Frequencies:
    env.assertCmdOk("ft.add", "idx", "doc100", 1.0, "fields", "f1", "foo bar")
    env.assertCmdOk(
        "ft.add", "idx", "doc200", 1.0, "fields", "f1", ("foo " * 10) + " bar"
    )
    ...
    env.assertCmdOk("ft.add", "idx", "doc300", 1.0, "fields", "f1", "Hello")
...
def testInfoCommandImplied(env):
    ...
    env.assertCmdOk(
        "ft.create", "idx", "ON", "HASH", "NOOFFSETS", "schema", "f1", "text"
    )
    res = env.cmd("ft.info", "idx")
    d = {res[i]: res[i + 1] for i in range(0, len(res), 2)}
...
def testNoStem(env):
    ...
    for _ in env.reloadingIterator():
        ...
        # Insert a document
        env.assertCmdOk(
            "ft.add", "idx", "doc", 1.0, "fields", "body", "located", "name", "located"
        )
...
def testSortbyMissingField(env):
    ...
    env.expect("ft.search", "ix", "foo", "sortby", "num").equal(
        [1, "doc1", ["txt", "foo", "noexist", "3.14"]]
    )
    env.expect("ft.search", "ix", "foo", "sortby", "noexist").error().contains(
        "Property `noexist` not loaded nor in schema"
    )
...
def testDoubleAdd(env):
    ...
    with env.assertResponseError():
        env.cmd("ft.add", "idx", "doc1", 1.0, "fields", "txt", "goodbye world")
    ...
    # Now with replace
    env.cmd("ft.add", "idx", "doc1", 1.0, "replace", "fields", "txt", "goodbye world")
...
def testConcurrentErrors(env):
    ...
    def thrfn():
        ...
        with cli.pipeline(transaction=False) as pl:
            for x in myIds:
                pl.execute_command(
                    "ft.add", "idx", x, 1.0, "fields", "txt", " hello world " * 50
                )
            try:
                pl.execute()
            except Exception as e:
...
def testLanguageField(env):
    ...
    env.cmd("FT.ADD", "idx", "doc1", 1.0, "FIELDS", "language", "gibberish")
...
def testUninitSortvector(env):
    ...
    for x in range(2000):
        env.cmd("FT.ADD", "idx", "doc{}".format(x), 1.0, "FIELDS", "f1", "HELLO")
...
def testIssue446(env):
    env.cmd("ft.create", "myIdx", "ON", "HASH", "schema", "title", "TEXT", "SORTABLE")
    env.cmd(
        "ft.add",
        "myIdx",
        "doc1",
...
def testTimeout(env):
    ...
    for i in range(num_range):
        env.expect(
            ...
            "aa" + str(i),
            "geo",
            str(i / 10000) + "," + str(i / 1000),
        )

    env.expect("ft.search", "myIdx", "aa*|aa*|aa*|aa* aa*", "limit", "0", "0").noEqual(
        [num_range]
    )

    env.expect(config_cmd(), "set", "on_timeout", "fail").ok()
    env.expect("ft.search", "myIdx", "aa*|aa*|aa*|aa* aa*", "limit", "0", "0").contains(
    ...
    env.expect(
        "ft.search", "myIdx", "aa*|aa*|aa*|aa* aa*", "TIMEOUT", "1"
    ...
    env.expect(
        ...
        "geo",
        "APPLY",
        'geodistance(@geo, "0.1,-0.1")',
        "AS",
        "geodistance1",
        "APPLY",
        'geodistance(@geo, "0.11,-0.11")',
        "AS",
        "geodistance2",
        "APPLY",
        'geodistance(@geo, "0.1,-0.1")',
...
def testAlias(env):
    conn = getConnectionByEnv(env)
    env.cmd(
        "ft.create", "idx", "ON", "HASH", "PREFIX", 1, "doc1", "schema", "t1", "text"
    )
    env.cmd(
        "ft.create", "idx2", "ON", "HASH", "PREFIX", 1, "doc2", "schema", "t1", "text"
    )

    env.expect("ft.aliasAdd", "myIndex").error()
    env.expect(
        "ft.aliasupdate", "fake_alias", "imaginary_alias", "Too_many_args"
    ).error()
    ...
    env.cmd(
        "ft.create", "idx3", "ON", "HASH", "PREFIX", 1, "doc3", "schema", "t1", "text"
    )
...
def testNoCreate(env):
    ...
    env.expect(
        "ft.add", "idx", "doc1", 1, "replace", "nocreate", "fields", "f1", "hello"
    ).error()
    env.expect("ft.add", "idx", "doc1", 1, "replace", "fields", "f1", "hello").noError()
    env.expect(
        "ft.add", "idx", "doc1", 1, "replace", "nocreate", "fields", "f1", "world"
    ).noError()
...
def testIssue484(env):
    ...
    res = env.cmd(
        ...
        "@categoryid",
        "APPLY",
        'split(format("%s-%s",@color,@categoryid),"-")',
        "as",
        "value",
        "GROUPBY",
        "1",
    ...
    expected = [
        6,
        ["value", "white", "value_count", "2"],
        ["value", "cars", "value_count", "2"],
        ["value", "small cars", "value_count", "1"],
        ["value", "blue", "value_count", "2"],
        ["value", "Big cars", "value_count", "2"],
        ["value", "green", "value_count", "1"],
    ]
...
def testIssue589(env):
    env.cmd("FT.CREATE", "incidents", "ON", "HASH", "SCHEMA", "report", "TEXT")
    env.cmd("FT.ADD", "incidents", "doc1", 1.0, "FIELDS", "report", "report content")
    env.expect("FT.SPELLCHECK", "incidents", "report :").error().contains(
...
def testIssue621(env):
    env.expect(
        "ft.create", "test", "ON", "HASH", "SCHEMA", "uuid", "TAG", "title", "TEXT"
    ).equal("OK")
    env.expect(
        "ft.add",
        "test",
    ...
    env.expect(
        "ft.add", "test", "a", "1", "REPLACE", "PARTIAL", "FIELDS", "title", "bar"
    ).equal("OK")
    res = env.cmd("ft.search", "test", "@uuid:{foo}")
    env.assertEqual(
        toSortedFlatList(res),
        toSortedFlatList([1, "a", ["uuid", "foo", "title", "bar"]]),
    )
...
def testPrefixDeletedExpansions(env):
    ...
    env.cmd(
        "ft.add", "idx", "doc_XXX", 1, "fields", "txt1", "termZZZ", "tag1", "tagZZZ"
    )
    ...
    r = env.cmd("ft.search", "idx", "@txt1:term* @tag1:{tag*}")
    env.assertEqual(
        toSortedFlatList([1, "doc_XXX", ["txt1", "termZZZ", "tag1", "tagZZZ"]]),
        toSortedFlatList(r),
    )
...
def testIssue_884(env):
    ...
    env.expect(
        ...
        "FIELDS",
        "title",
        "Fareham Civilization Church - Sermons and conversations mohsin conversation the",
    ).equal("OK")
    env.expect(
        "FT.ADD",
        "idx",
    ...
    expected = [
        ...
        "doc4",
        ["title", "mohsin conversation the conversation tahir"],
    ]
    res = env.cmd(
        "FT.SEARCH",
        "idx",
        "@title:(conversation) (@title:(conversation the conversation))=>{$inorder: true;$slop: 0}",
    )
...
def testIssue_848(env):
    env.expect(
        "FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "test1", "TEXT", "SORTABLE"
    ).equal("OK")
    env.expect("FT.ADD", "idx", "doc1", "1.0", "FIELDS", "test1", "foo").equal("OK")
    env.expect("FT.ALTER", "idx", "SCHEMA", "ADD", "test2", "TEXT", "SORTABLE").equal(
        "OK"
    )
    env.expect(
        "FT.ADD", "idx", "doc2", "1.0", "FIELDS", "test1", "foo", "test2", "bar"
    ).equal("OK")
    env.expect("FT.SEARCH", "idx", "foo", "SORTBY", "test2", "ASC").equal(
        [2, "doc2", ["test2", "bar", "test1", "foo"], "doc1", ["test1", "foo"]]
    )
...
def testMod_309(env):
    ...
    l = (
        len(res) - 1
    )  # do not count the number of results (the first element in the results)
    while cursor != 0:
...
def testIssue_865(env):
    env.expect(
        "FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "1", "TEXT", "SORTABLE"
    ).equal("OK")
    env.expect("ft.add", "idx", "doc1", "1.0", "FIELDS", "1", "foo1").equal("OK")
    env.expect("ft.add", "idx", "doc2", "1.0", "FIELDS", "1", "foo2").equal("OK")
    env.expect("ft.search", "idx", "foo*", "SORTBY", "1", "ASC").equal(
        [2, "doc1", ["1", "foo1"], "doc2", ["1", "foo2"]]
    )
    env.expect("ft.search", "idx", "foo*", "SORTBY", "1", "DESC").equal(
        [2, "doc2", ["1", "foo2"], "doc1", ["1", "foo1"]]
    )
...
def testIssue_779(env):
    ...
    env.assertEqual(
        toSortedFlatList(res), toSortedFlatList(["newf", "CAT", "ot1", "4001"])
    )

    # NOADD is expected since 4001 is not < 4000, and no updates to the doc2 is expected as a result
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1<4000 FIELDS newf DOG ot1 4000",
        "NOADD",
    )
    res = env.cmd("FT.GET idx2 doc2")
    env.assertEqual(
    ...
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1<4002 FIELDS newf DOG ot1 4002"
    ).equal("OK")
    res = env.cmd("FT.GET idx2 doc2")
    env.assertEqual(
        toSortedFlatList(res), toSortedFlatList(["newf", "DOG", "ot1", "4002"])
    )
    ...
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1<4002 FIELDS newf FISH ot1 4002"
    ).equal("NOADD")
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if to_number(@ot1)<4002 FIELDS newf FISH ot1 4002"
    ).equal("NOADD")
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1<to_str(4002) FIELDS newf FISH ot1 4002"
    ).equal("NOADD")
    res = env.cmd("FT.GET idx2 doc2")
    env.assertEqual(
        toSortedFlatList(res), toSortedFlatList(["newf", "DOG", "ot1", "4002"])
    ...
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1<4003 FIELDS newf HORSE ot1 4003"
    ).equal("OK")
    res = env.cmd("FT.GET idx2 doc2")
    env.assertEqual(
        toSortedFlatList(res), toSortedFlatList(["newf", "HORSE", "ot1", "4003"])
    )

    # Expect NOADD since 4003 is not > 4003
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1>4003 FIELDS newf COW ot1 4003"
    ).equal("NOADD")
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if 4003<@ot1 FIELDS newf COW ot1 4003"
    ).equal("NOADD")

    # Expect OK and doc2 updated since 4003 > 4002
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1>4002 FIELDS newf PIG ot1 4002"
    ).equal("OK")
    res = env.cmd("FT.GET idx2 doc2")
    env.assertEqual(
        toSortedFlatList(res), toSortedFlatList(["newf", "PIG", "ot1", "4002"])
    )

    # Syntax errors
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1<4-002 FIELDS newf DOG ot1 4002"
    ).contains("Syntax error")
    env.expect(
        "FT.ADD idx2 doc2 1.0 REPLACE PARTIAL if @ot1<to_number(4-002) FIELDS newf DOG ot1 4002"
    ).contains("Syntax error")


def testUnknownSymbolErrorOnConditionalAdd(env):
    env.expect(
        "FT.CREATE idx ON HASH SCHEMA f1 TAG f2 NUMERIC NOINDEX f3 TAG NOINDEX"
    ).ok()
    env.expect(
        "ft.add idx doc1 1.0 REPLACE PARTIAL IF @f1<awfwaf FIELDS f1 foo f2 1 f3 boo"
    ).ok()
    env.expect(
        "ft.add idx doc1 1.0 REPLACE PARTIAL IF @f1<awfwaf FIELDS f1 foo f2 1 f3 boo"
    ).error()
...
def testWrongResultsReturnedBySkipOptimization(env):
    env.expect(
        "FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "f1", "TEXT", "f2", "TEXT"
    ).equal("OK")
    env.expect(
        "ft.add", "idx", "doc1", "1.0", "FIELDS", "f1", "foo", "f2", "bar"
    ).equal("OK")
    env.expect(
        "ft.add", "idx", "doc2", "1.0", "FIELDS", "f1", "moo", "f2", "foo"
    ).equal("OK")
...
def testSummerizeWithAggregateRaiseError(env):
    ...
    env.expect(
        ...
        "COUNT",
        "0",
    ).error().contains("SUMMARIZE is not supported on FT.AGGREGATE")
...
def testLimitBadArgument(env):
    ...
    env.expect("FT.SEARCH", "idx", "*", "LIMIT", "1", "0").error().equal(
...
def testNumericFilterError(env):
    env.expect(
        "FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "test", "NUMERIC", "SORTABLE"
    ).equal("OK")
...
def testGeoFilterError(env):
    ...
    env.expect(
        "ft.search", "idx", "*", "GEOFILTER", "test", "bad", "2", "3", "km"
    ).error()
    env.expect(
        "ft.search", "idx", "*", "GEOFILTER", "test", "1", "bad", "3", "km"
    ).error()
    env.expect(
        "ft.search", "idx", "*", "GEOFILTER", "test", "1", "2", "bad", "km"
    ).error()
    env.expect(
        "ft.search", "idx", "*", "GEOFILTER", "test", "1", "2", "3", "bad"
    ).error()
...
def testReducerError(env):
    ...
    env.expect("ft.add", "idx", "doc1", "1.0", "FIELDS", "test", "1").equal("OK")
    env.expect(
        "ft.aggregate", "idx", "*", "GROUPBY", "1", "@test", "REDUCE", "COUNT", "bad"
    ).error()
    env.expect(
        "ft.aggregate",
        "idx",
...
def testGroupbyError(env):
    ...
    env.expect(
        "ft.aggregate", "idx", "*", "GROUPBY", "1", "@test", "REDUCE", "bad", "0"
    ).error()
    if not env.isCluster():  # todo: remove once fix on coordinator
        env.expect(
            "ft.aggregate",
...
def testGroupbyWithSort(env):
    ...
    env.expect(
        ...
        "as",
        "count",
    ).equal([2, ["test", "2", "count", "1"], ["test", "1", "count", "2"]])
...
def testHighlightOnAggregate(env):
    ...
    env.expect("ft.add", "idx", "doc1", "1.0", "FIELDS", "test", "foo").equal("OK")
    env.expect(
        "ft.aggregate", "idx", "*", "HIGHLIGHT", "FIELDS", "1", "test"
    ).error().contains("HIGHLIGHT is not supported on FT.AGGREGATE")
...
def testBadFilterExpression(env):
    ...
    if not env.isCluster():  # todo: remove once fix on coordinator
        env.expect(
            "ft.aggregate", "idx", "*", "LOAD", "1", "@test", "FILTER", "blabla"
        ).error()
        env.expect(
            "ft.aggregate", "idx", "*", "LOAD", "1", "@test", "FILTER", "@test1 > 1"
        ).error()
...
def testWithSortKeysOnNoneSortableValue(env):
    env.expect("FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "test", "TEXT").equal("OK")
    env.expect("ft.add", "idx", "doc1", "1.0", "FIELDS", "test", "foo").equal("OK")
    env.expect("ft.search", "idx", "*", "WITHSORTKEYS", "SORTBY", "test").equal(
        [1, "doc1", "$foo", ["test", "foo"]]
    )
...
def testWithWithRawIds(env):
    ...
    waitForIndex(env, "idx")
    env.expect("ft.add", "idx", "doc1", "1.0", "FIELDS", "test", "foo").equal("OK")
    env.expect("ft.search", "idx", "*", "WITHRAWIDS").equal(
        [1, "doc1", 1, ["test", "foo"]]
    )
...
def testSubStrErrors(env):
    ...
    env.expect(
        ...
        "as",
        "a",
    ).error().contains("Invalid arguments for function 'substr'")
    env.expect(
        "ft.aggregate",
        "idx",
        "*",
    ...
    env.expect(
        ...
        "@test2",
        "APPLY",
        'substr("test", "test", "test")',
        "as",
        "a",
    ).error().contains("Invalid type (3) for argument 1 in function 'substr'")
    env.expect(
        "ft.aggregate",
        "idx",
        "*",
...
def testToUpperLower(env):
    ...
    env.expect(
        "ft.aggregate", "idx", "*", "LOAD", "1", "@test", "APPLY", "upper(1)", "as", "a"
    ).equal([1, ["test", "foo", "a", None]])
    env.expect(
        "ft.aggregate", "idx", "*", "LOAD", "1", "@test", "APPLY", "lower(1)", "as", "a"
    ).equal([1, ["test", "foo", "a", None]])
...
def testStrFormatError(env):
    ...
    env.expect(
        ...
        "as",
        "a",
    ).equal([1, ["test", "foo", "b", None, "a", "(null)"]])

    # working example
    env.expect(
        "ft.aggregate", "idx", "foo", "APPLY", 'format("%%s-test", "test")', "as", "a"
    ).equal([1, ["a", "%s-test"]])
    env.expect(
        "ft.aggregate", "idx", "foo", "APPLY", 'format("%s-test", "test")', "as", "a"
    ).equal([1, ["a", "test-test"]])
...
def testTimeFormatError(env):
    ...
    env.expect(
        ...
        "@test",
        "APPLY",
        "timefmt(235325153152356426246246246254)",
        "as",
        "a",
    ).equal([1, ["test", "12234556", "a", None]])
...
def testParseTime(env):
    ...
    env.expect(
        ...
        "Invalid type (1) for argument 0 in function 'parsetime'. VALIDATE_ARG__STRING(v, 0) was false."
    )
...
def testMathFunctions(env):
    ...
    env.expect(
        ...
        "as",
        "a",
    ).equal([1, ["test", "12234556", "a", "12234556"]])
...
def testErrorOnOpperation(env):
    ...
    if not env.isCluster():
        env.expect(
            "ft.aggregate", "idx", "@test:[0..inf]", "APPLY", "!@test", "as", "a"
...
def testIssue1085(env):
    ...
    for i in range(1, 10):
        env.cmd(
            "FT.ADD issue1085 document_%d 1 REPLACE FIELDS foo foo%d bar %d" % (i, i, i)
        )
    res = env.cmd("FT.SEARCH", "issue1085", "@bar:[8 8]")
    env.assertEqual(
        toSortedFlatList(res),
        toSortedFlatList([1, "document_8", ["foo", "foo8", "bar", "8"]]),
    )
...
def testHindiStemmer(env):
    env.cmd(
        "FT.CREATE", "idxTest", "LANGUAGE_FIELD", "__language", "SCHEMA", "body", "TEXT"
    )
    env.cmd(
        "FT.ADD",
        "idxTest",
        ...
        "FIELDS",
        "body",
        "अँगरेजी अँगरेजों अँगरेज़",
    )
    res = env.cmd("FT.SEARCH", "idxTest", "अँगरेज़")
...
def testUnseportedSortableTypeErrorOnTags(env):
    env.expect(
        "FT.CREATE idx ON HASH SCHEMA f1 TEXT SORTABLE f2 NUMERIC SORTABLE NOINDEX f3 TAG SORTABLE NOINDEX f4 TEXT SORTABLE NOINDEX"
    ).ok()
    ...
    env.assertEqual(
        toSortedFlatList(res),
        toSortedFlatList(
            ["f1", "foo1", "f2", "2", "f3", "foo2", "f4", "foo2", "__score", "1.0"]
        ),
    )
    res = env.cmd("FT.SEARCH idx *")
    env.assertEqual(
        toSortedFlatList(res),
        toSortedFlatList(
            [1, "doc1", ["f1", "foo1", "f2", "2", "f3", "foo2", "f4", "foo2"]]
        ),
    )
...
def testIssue1158(env):
    ...
    env.assertEqual(
        toSortedFlatList(res), toSortedFlatList(["txt1", "10", "txt2", "num1"])
    )

    # only 1st checked (2nd returns an error)
    env.expect(
        "FT.ADD idx doc1 1.0 REPLACE PARTIAL if @txt1||to_number(@txt2)<5 FIELDS txt1 5"
    ).equal("OK")
    env.expect(
        "FT.ADD idx doc1 1.0 REPLACE PARTIAL if @txt3&&to_number(@txt2)<5 FIELDS txt1 5"
    ).equal("NOADD")

    # both are checked
    env.expect(
        "FT.ADD idx doc1 1.0 REPLACE PARTIAL if to_number(@txt1)>11||to_number(@txt1)>42 FIELDS txt2 num2"
    ).equal("NOADD")
    env.expect(
        "FT.ADD idx doc1 1.0 REPLACE PARTIAL if to_number(@txt1)>11||to_number(@txt1)<42 FIELDS txt2 num2"
    ).equal("OK")
    ...
    res = env.cmd("FT.GET idx doc1")
    env.assertEqual(
        toSortedFlatList(res), toSortedFlatList(["txt1", "5", "txt2", "num2"])
    )
...
def testIssue1169(env):
    ...
    env.expect(
        "FT.AGGREGATE idx foo GROUPBY 1 @txt1 REDUCE FIRST_VALUE 1 @txt2 as test"
    ).equal([1, ["txt1", "foo", "test", None]])
...
def testIssue1208(env):
    ...
    env.expect("FT.SEARCH", "idx", "@n:[1.1432E3 inf]").equal(
        [1, "doc1", ["n", "1.0321e5"]]
    )
    env.expect("FT.SEARCH", "idx", "@n:[-1.12E-3 1.12E-1]").equal(
        [1, "doc3", ["n", "0.0011"]]
    )
    res = [
        3,
    ...
    env.expect("FT.ADD idx doc3 1 REPLACE PARTIAL IF @n>42e3 FIELDS n 100").equal(
        "NOADD"
    )
...
def testFieldsCaseSensetive(env):
    ...
    env.expect("ft.search", "idx", "@n:[0 2]", "SORTBY", "n").equal(
        [2, "doc4", ["n", "1.0"], "doc7", ["n", "1.1"]]
    )
    env.expect("ft.search", "idx", "@n:[0 2]", "SORTBY", "N").error().contains(
        "not loaded nor in schema"
    )

    # make sure aggregation load are case sensitive
    env.expect("ft.aggregate", "idx", "@n:[0 2]", "LOAD", "1", "@n").equal(
        [1, ["n", "1"], ["n", "1.1"]]
    )
    ...
    env.expect(
        "ft.aggregate", "idx", "@n:[0 2]", "LOAD", "1", "@n", "apply", "@n", "as", "r"
    ).equal([1, ["n", "1", "r", "1"], ["n", "1.1", "r", "1.1"]])
    env.expect(
        "ft.aggregate", "idx", "@n:[0 2]", "LOAD", "1", "@n", "apply", "@N", "as", "r"
    ).error().contains("not loaded nor in pipeline")
    ...
    env.expect(
        "ft.aggregate", "idx", "@n:[0 2]", "LOAD", "1", "@n", "filter", "@n==1.0"
    ).equal([1, ["n", "1"]])
    env.expect(
        "ft.aggregate", "idx", "@n:[0 2]", "LOAD", "1", "@n", "filter", "@N==1.0"
    ).error().contains("not loaded nor in pipeline")
    ...
    env.expect(
        ...
        "as",
        "count",
    ).equal([2, ["n", "1", "count", "1"], ["n", "1.1", "count", "1"]])
    env.expect(
        "ft.aggregate",
        "idx",
        "@n:[0 2]",
        ...
        "as",
        "count",
    ).error().contains("No such property")

    # make sure aggregation sortby are case sensitive
    env.expect(
        "ft.aggregate", "idx", "@n:[0 2]", "LOAD", "1", "@n", "sortby", "1", "@n"
    ).equal([2, ["n", "1"], ["n", "1.1"]])
    env.expect(
        "ft.aggregate", "idx", "@n:[0 2]", "LOAD", "1", "@n", "sortby", "1", "@N"
...
def testSortedFieldsCaseSensetive(env):
    ...
    conn = getConnectionByEnv(env)
    env.cmd(
        "FT.CREATE idx ON HASH SCHEMA n NUMERIC SORTABLE f TEXT SORTABLE t TAG SORTABLE g GEO SORTABLE"
    )
    ...
    # make sure aggregation apply are case sensitive
    env.expect("ft.aggregate", "idx", "@n:[0 2]", "apply", "@n", "as", "r").equal(
        [1, ["n", "1", "r", "1"], ["n", "1.1", "r", "1.1"]]
    )
    env.expect(
        "ft.aggregate", "idx", "@n:[0 2]", "apply", "@N", "as", "r"
    ).error().contains("not loaded nor in pipeline")

    # make sure aggregation filter are case sensitive
    env.expect("ft.aggregate", "idx", "@n:[0 2]", "filter", "@n==1.0").equal(
        [1, ["n", "1"]]
    )
    env.expect("ft.aggregate", "idx", "@n:[0 2]", "filter", "@N==1.0").error().contains(
    ...
    # make sure aggregation sortby are case sensitive
    env.expect("ft.aggregate", "idx", "@n:[0 2]", "sortby", "1", "@n").equal(
        [2, ["n", "1"], ["n", "1.1"]]
    )
    env.expect("ft.aggregate", "idx", "@n:[0 2]", "sortby", "1", "@N").error().contains(
        "not loaded"
    )
    ...
    env.assertEqual(
        ...
        toSortedFlatList(
            [
                1,
                "doc1",
                ["n", "1.0", "__Language", "eng", "__Score", "1", "__Payload", "10"],
            ]
        ),
    )
...
def testReturnSameFieldDifferentCase(env):
    ...
    env.expect("ft.search", "idx", "@n:[0 2]", "RETURN", "2", "n", "N").equal(
        [1, "doc1", ["n", "1", "N", "2"]]
    )
...
def testCreateIfNX(env):
    env.expect(
        "FT._CREATEIFNX idx ON HASH SCHEMA n NUMERIC SORTABLE N NUMERIC SORTABLE"
    ).ok()
    env.expect(
        "FT._CREATEIFNX idx ON HASH SCHEMA n NUMERIC SORTABLE N NUMERIC SORTABLE"
    ).ok()
...
def testAlterIfNX(env):
    ...
    env.assertEqual(
        res,
        [
            ["identifier", "n", "attribute", "n", "type", "NUMERIC"],
...
def testInvertedIndexWasEntirelyDeletedDuringCursor():
    ...
    env.expect(debug_cmd() + " DUMP_INVIDX idx foo").error().contains(
        "not find the inverted index"
    )
...
def testNotOnly(env):
    ...
    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search idx !world")),
        toSortedFlatList([1, "b", ["txt1", "world", "txt2", "hello"]]),
    )
...
def testSchemaWithAs(env):
    ...
    for _ in env.reloadingIterator():
        ...
        env.expect("ft.search idx hello RETURN 3 txt AS baz").equal(
            [1, "a", ["baz", "hello"]]
        )
        env.expect("ft.search idx hello RETURN 3 foo AS baz").equal(
            [1, "a", ["baz", "hello"]]
        )
        env.expect("ft.search idx hello RETURN 6 txt AS baz txt AS bar").equal(
            [1, "a", ["baz", "hello", "bar", "hello"]]
        )
        env.expect("ft.search idx hello RETURN 6 txt AS baz txt AS baz").equal(
            [1, "a", ["baz", "hello"]]
        )
        ...
        env.expect("ft.search idx hello RETURN 3 not_in_schema AS txt2").equal(
            [1, "a", ["txt2", "42"]]
        )
        env.expect("ft.search idx hello RETURN 1 not_in_schema").equal(
            [1, "a", ["not_in_schema", "42"]]
        )
        env.expect("ft.search idx hello").equal(
            [1, "a", ["txt", "hello", "not_in_schema", "42"]]
        )
        ...
        env.expect("ft.search idx hello RETURN 3 txt as as").error().contains(
            "Alias for RETURN cannot be `AS`"
        )

        # LOAD for FT.AGGREGATE
        # for path - can rename
        env.expect("ft.aggregate", "idx", "hello", "LOAD", "1", "@txt").equal(
            [1, ["txt", "hello"]]
        )
        env.expect(
            "ft.aggregate", "idx", "hello", "LOAD", "3", "@txt", "AS", "txt1"
        ).equal([1, ["txt1", "hello"]])

        # for name - cannot rename
        env.expect("ft.aggregate", "idx", "hello", "LOAD", "1", "@foo").equal(
            [1, ["foo", "hello"]]
        )
        env.expect(
            "ft.aggregate", "idx", "hello", "LOAD", "3", "@foo", "AS", "foo1"
        ).equal([1, ["foo1", "hello"]])

        # for for not in schema - can rename
        env.expect("ft.aggregate", "idx", "hello", "LOAD", "1", "@not_in_schema").equal(
            [1, ["not_in_schema", "42"]]
        )
        env.expect(
            "ft.aggregate", "idx", "hello", "LOAD", "3", "@not_in_schema", "AS", "NIS"
        ).equal([1, ["NIS", "42"]])
...
def testSchemaWithAs_Duplicates(env):
    ...
    res = (
        ...
        .error()
        .contains("Duplicate field in schema - foo")
    )
...
def testMod1407(env):
    ...
    env.expect(
        "FT.AGGREGATE", "idx", "*", "SORTBY", "3", "@limit", "@LimitationTypeID", "ASC"
    ).equal(
        [
            2,
            ["limit", "foo1", "LimitationTypeID", "boo1"],
            ["limit", "foo2", "LimitationTypeID", "boo2"],
        ]
    )
...
def test_mod1548(env):
    ...
    res = conn.execute_command(
        ...
        "prod:1",
        "$",
        '{"prod:id": "35114964", "SKU": "35114964", "name":"foo", "categories":"abcat0200000"}',
    )
    env.assertOk(res)
    res = conn.execute_command(
        "JSON.SET",
        "prod:2",
        "$",
        '{"prod:id": "35114965", "SKU": "35114965", "name":"bar", "categories":"abcat0200000"}',
    )
    env.assertOk(res)

    # Supported jsonpath
    res = env.cmd(
        "FT.SEARCH", "idx", "@categories:{abcat0200000}", "RETURN", "1", "name"
    )
    ...
    env.assertEqual(
        ...
        [
            2,
            "prod:1",
            ["prod:id_bracketnotation", "35114964"],
            "prod:2",
            ["prod:id_bracketnotation", "35114965"],
        ],
    )
    ...
    env.assertEqual(
        ...
        [
            2,
            "prod:1",
            ["prod:id_dotnotation", "35114964"],
            "prod:2",
            ["prod:id_dotnotation", "35114965"],
        ],
    )
...
def test_aggregate_return_fail(env):
    env.expect("FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "test", "TEXT").equal("OK")
    env.expect("ft.add", "idx", "doc1", "1.0", "FIELDS", "test", "foo").equal("OK")
    env.expect("ft.aggregate", "idx", "*", "RETURN", "1", "test").error().contains(
        "RETURN is not supported on FT.AGGREGATE"
    )
...
def test_emoji(env):
    conn = getConnectionByEnv(env)
    env.expect("FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "test", "TEXT").equal("OK")
    env.expect("FT.CREATE", "idx_tag", "ON", "HASH", "SCHEMA", "test", "TAG").equal(
        "OK"
    )

    conn.execute_command("HSET", "doc1", "test", "a📌")
    env.expect("ft.search", "idx", "a📌").equal([1, "doc1", ["test", "a📌"]])
    env.expect("ft.search", "idx_tag", "@test:{a📌}").equal(
        [1, "doc1", ["test", "a📌"]]
    )
...
def test_mod_4200(env):
    env.expect("FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "test", "TEXT").equal("OK")
    for i in range(1001):
        env.expect("ft.add", "idx", "doc%i" % i, "1.0", "FIELDS", "test", "foo").equal(
            "OK"
        )
    env.expect(
        "ft.search", "idx", "((~foo) foo) | ((~foo) foo)", "LIMIT", "0", "0"
    ).equal([1001])
...
def test_RED_86036(env):
    ...
    res = env.cmd(
        "FT.PROFILE", "idx", "search", "query", "*", "INKEYS", "2", "doc0", "doc999"
    )
...
def test_missing_schema(env):
    ...
    env.expect("FT.CREATE", "idx2", "TEMPORARY", 1000, "foo", "bar").error().contains(
...
def cluster_set_test(env: Env):
    def verify_address(addr):
        try:
            ...
            env.assertTrue(
                False,
                message="Failed waiting cluster set command to be updated with the new IP address %s"
                % addr,
            )
    ...
    for i in range(env.shardsCount):
        shards += [
            ...
            "16383",
            "ADDR",
            f"{password}localhost:{env.envRunner.shards[i].port}",
            "MASTER",
        ]
    env.expect(
        "SEARCH.CLUSTERSET", "MYID", "0", "RANGES", str(env.shardsCount), *shards
    ).ok()
...
def test_cluster_set_errors(env: Env):
    ...
    env.expect("SEARCH.CLUSTERSET", "RANDOM").error().contains(
        "Unexpected argument"
    ).contains("RANDOM")

    env.expect("SEARCH.CLUSTERSET", "MYID").error().contains("Missing value for MYID")
    env.expect("SEARCH.CLUSTERSET", "RANGES").error().contains(
        "Missing value for RANGES"
    )
    env.expect("SEARCH.CLUSTERSET", "HASHFUNC").error().contains(
        "Missing value for HASHFUNC"
    )
    env.expect("SEARCH.CLUSTERSET", "NUMSLOTS").error().contains(
        "Missing value for NUMSLOTS"
    )

    env.expect("SEARCH.CLUSTERSET", "HASHFUNC", "yes please").error().contains(
        "Bad value for HASHFUNC: yes please"
    )
    env.expect("SEARCH.CLUSTERSET", "RANGES", "yes please").error().contains(
        "Bad value for RANGES: yes please"
    )
    env.expect("SEARCH.CLUSTERSET", "RANGES", "-1").error().contains(
        "Bad value for RANGES: -1"
    )
    env.expect("SEARCH.CLUSTERSET", "NUMSLOTS", "yes please").error().contains(
        "Bad value for NUMSLOTS: yes please"
    )
    env.expect("SEARCH.CLUSTERSET", "NUMSLOTS", "0").error().contains(
        "Bad value for NUMSLOTS: 0"
    )
    env.expect("SEARCH.CLUSTERSET", "NUMSLOTS", "1000000").error().contains(
        "Bad value for NUMSLOTS: 1000000"
    )

    # Check shard values parsing
    env.expect(
        "SEARCH.CLUSTERSET", "MYID", "1", "RANGES", "1", "BANANA"
    ).error().contains("Expected `SHARD` but got `BANANA`")
    env.expect(
        "SEARCH.CLUSTERSET", "MYID", "1", "RANGES", "1", "SHARD"
    ).error().contains("Missing value for SHARD")
    env.expect(
        "SEARCH.CLUSTERSET", "MYID", "1", "RANGES", "1", "SHARD", "1"
    ).error().contains("Expected `SLOTRANGE` but got `(nil)`")
    env.expect(
        "SEARCH.CLUSTERSET", "MYID", "1", "RANGES", "1", "SHARD", "1", "SLOTRANGE"
    ).error().contains("Missing value for SLOTRANGE")
    env.expect(
        "SEARCH.CLUSTERSET", "MYID", "1", "RANGES", "1", "SHARD", "1", "SLOTRANGE", "0"
    ).error().contains("Missing value for SLOTRANGE")
    env.expect(
        "SEARCH.CLUSTERSET",
        "MYID",
        ...
        "0",
        "banana",
    ).error().contains("Bad value for SLOTRANGE").contains("banana")
    env.expect(
        "SEARCH.CLUSTERSET",
        "MYID",
        "1",
        ...
        "1",
        "0",
    ).error().contains("Bad values for SLOTRANGE: 1, 0")
    env.expect(
        "SEARCH.CLUSTERSET",
        "MYID",
        "1",
        ...
        "0",
        "1000000",
    ).error().contains("Bad values for SLOTRANGE: 0, 1000000")
    env.expect(
        "SEARCH.CLUSTERSET",
        "MYID",
        "1",
        ...
        "0",
        "1",
    ).error().contains("Expected `ADDR` but got `(nil)`")
    env.expect(
        "SEARCH.CLUSTERSET",
        "MYID",
        "1",
        ...
        "1",
        "ADDR",
    ).error().contains("Missing value for ADDR")
    env.expect(
        "SEARCH.CLUSTERSET",
        "MYID",
        "1",
        ...
        "1:1",
        "UNIXADDR",
    ).error().contains("Missing value for UNIXADDR")
    env.expect(
        "SEARCH.CLUSTERSET",
        "MYID",
        "1",
        ...
        "1:1",
        "UNEXPECTED",
    ).error().contains("Expected end of command but got `UNEXPECTED`")
    ...
    env.expect(
        ...
        "ADDR",
        "1:1",
    ).error().contains("Expected `SHARD` but got `(nil)`")
    ...
    invalid_addresses = ["invalid", "invalid:", "localhost:invalid", "[::1:234"]
...
def test_timeoutCoordSearch_Strict():
    ...
    env.expect("ft.search", "idx", "*", "TIMEOUT", "1").error().contains(
        "Timeout limit was reached"
    )
...
def test_notIterTimeout(env):
    ...
    * Skipped on cluster since the it would only test error propagation from the
    ...
    # Create an index
    env.cmd(
        "FT.CREATE", "idx", "SCHEMA", "tag1", "TAG", "title", "TEXT", "n", "NUMERIC"
    )
