...
def test_apply(env):
    ...
    conn.execute_command(
        "HSET", "dkey:1", "name", "Lassie", "breed", "Rough Collie", "code", "ca?33-22"
    )
    conn.execute_command(
        "HSET", "dkey:2", "name", "lessly", "breed", "Poodle", "code", "ca?33-22"
    )
    conn.execute_command(
        "HSET", "dkey:3", "name", "Perrito", "breed", "poodle", "code", "ca?33-22"
    )
    conn.execute_command(
        "HSET", "dkey:4", "name", "Lou Dog", "breed", "Dalmatian", "code", "ca:99-##"
    )
    conn.execute_command(
        "HSET", "dkey:5", "name", "dipper", "breed", "dalmatian", "code", "ca:99-##"
    )
    conn.execute_command(
        "HSET", "dkey:6", "name", "Duff", "breed", "Dalmatian", "code", "gg-*33-22"
    )
    conn.execute_command(
        "HSET",
        "dkey:7",
    ...
    conn.execute_command(
        "HSET", "dkey:8", "name", "Chuck", "breed", "Saluki", "code", "*gg-33-22"
    )
    conn.execute_command(
        "HSET", "dkey:9", "name", "Tuk", "breed", "Husky", "code", "*gg-33-22"
    )
    conn.execute_command(
        "HSET", "dkey:10", "name", "Jul", "breed", "St. Bernard", "code", "gg-33-22"
    )
    ...
    res1 = env.cmd(
        "ft.aggregate", "idx", "@breed:(Dal*|Poo*|Ru*|Mo*)", "SORTBY", "1", "@name"
    )
    res2 = env.cmd(
        "ft.aggregate",
        "idx",
