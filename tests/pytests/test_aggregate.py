    ...
    for i in range(number_of_iterations):
        ...
        for line in fp:
            ...
            cmd = [
                ...
                1,
                "FIELDS",
            ] + [str(x) if x is not None else "" for x in itertools.chain(*obj.items())]
            env.cmd(*cmd)
        fp.close()
...
class TestAggregate:
    ...
    def testTimeFunctions(self):
        ...
        def expected(date: datetime):
            return [
                ...
                [
                    "dt",
                    str(int(date.timestamp())),
                    "timefmt",
                    date.strftime("%FT%TZ"),
                    "day",
                    str(int(date.replace(hour=0, minute=0, second=0).timestamp())),
                    "hour",
                    str(int(date.replace(minute=0, second=0).timestamp())),
                    "minute",
                    str(int(date.replace(second=0).timestamp())),
                    "month",
                    str(
                        int(date.replace(day=1, hour=0, minute=0, second=0).timestamp())
                    ),
                    "dayofweek",
                    str(date.isoweekday()),
                    "dayofmonth",
                    ...
                    ),  # Python tm_yday is 1-based, while C tm_yday is 0-based
                    "year",
                    str(date.year),
                ],
            ]
    ...
    def testStringFormat(self):
        cmd = [
            ...
            "price",
            "APPLY",
            'format("%s|%s|%s|%s", @title, @brand, "Mark", @price)',
            "as",
            "titleBrand",
            "LIMIT",
            "0",
    ...
    def testSum(self):
        ...
        self.env.assertEqual(
            [
                292,
                ["brand", "", "count", "1518", "sum(price)", "44780.69"],
    ...
    def testFilterBeforeLoad(self):
        ...
        if self.env.isCluster():
            ...
            self.env.assertEqual(
                [
                    [
                        ...
                        "759.12",
                        "categories",
                        "Accessories,Controllers,PC,Steering Wheels,Video Games",
                    ],
                    ...
                    [
                        ...
                        "559.99",
                        "categories",
                        "Accessories,Gaming Keyboards,Mac,Video Games",
                    ],
            ...
            self.env.expect(*cmd).error().contains(
                "Property `price` not loaded nor in pipeline"
            )
            ...
            self.env.assertEqual(
                [
                    [
                        ...
                        "Sony",
                        "categories",
                        "Accessories,Cables,Cables & Adapters,PlayStation 3,Video Games",
                        "price",
                        "5.88",
                    ],
                    [
                    ...
                    [
                        ...
                        "Sony",
                        "categories",
                        "Accessories,Headsets,Sony PSP,Video Games",
                        "price",
                        "12.99",
                    ],
                    [
                        ...
                        "Sony",
                        "categories",
                        "Movies & TV,Sony PSP,TV,Video Games",
                        "price",
                        "25.99",
                    ],
                ],
    ...
    def testSortBy(self):
        ...
        expected_res = [
            ...
            [
                "title",
                "Logitech MOMO Racing - Wheel and pedals set - 6 button(s) - PC, MAC - black",
                "price",
                "759.12",
            ],
        ...
        expected_res = [
            2265,
            ["description", "world of warcraft:the burning crusade-expansion set"],
            [
                "description",
                "wired playstation 3 controller, third party product with high quality.",
            ],
        ]
        res = self.env.cmd(
            "ft.aggregate",
    ...
    def testLoadWithDocId(self):
        ...
        exp = [
            3,
            ["brand", "", "price", "759.12", "__key", "B00006JJIC"],
    ...
    def testReducerGeneratedAliasing(self):
        ...
        self.env.assertEqual(
            "__generated_aliasfirst_valuetitle,by,price,desc", rv[1][2]
        )
    ...
    def testIssue1125(self):
        ...
        # SEARCH should fail
        self.env.expect(
            "ft.search", "games", "*", "limit", 0, 2000000
        ).error().contains("LIMIT exceeds maximum of 1000000")
        ...
        self.env.expect(
            "ft.aggregate", "games", "*", "limit", 0, 2000000
        ).error().contains("LIMIT exceeds maximum of 1000000")
    ...
    def testCountError(self):
        ...
        res = (
            ...
            .error()
            .contains(
                "Bad arguments for COUNT: Expected an argument, but none provided"
            )
        )
    ...
    def testModulo(self):
        ...
        # With MIN_INF % -1
        res = self.env.cmd(
            "ft.aggregate", "games", "*", "APPLY", "-9223372036854775808 % -1"
        )
        self.env.assertEqual(res[1][1], "0")

        # With Integers
        res = self.env.cmd("ft.aggregate", "games", "*", "APPLY", "439974354 % 5")
        self.env.assertEqual(res[1][1], "4")

        # With Negative
        res = self.env.cmd("ft.aggregate", "games", "*", "APPLY", "-54775808 % -5")
        self.env.assertEqual(res[1][1], "-3")

        res = self.env.cmd("ft.aggregate", "games", "*", "APPLY", "-14275897 % 5")
        self.env.assertEqual(res[1][1], "-2")

        # With Floats
        res = self.env.cmd("ft.aggregate", "games", "*", "APPLY", "547758.3 % 5.1")
        self.env.assertEqual(res[1][1], "3")
...
class TestAggregateSecondUseCases:
    ...
    def testSimpleAggregateWithCursor(self):
        _, cursor = self.env.cmd(
            "ft.aggregate", "games", "*", "WITHCURSOR", "COUNT", 1000
        )
...
def test_groupby_array(env: Env):
    env.expect(
        "FT.CREATE", "idx", "SCHEMA", "t1", "TEXT", "SORTABLE", "t2", "TEXT", "SORTABLE"
    ).ok()
...
def testContains(env):
    ...
    res = env.cmd(
        "ft.aggregate", "idx", "*", "load", 1, "t", "filter", 'contains(@t, "bb")'
    )
    env.assertEqual(
        toSortedFlatList(res)[1:],
        toSortedFlatList([["t", "bba"], ["t", "abb"], ["t", "abba"], ["t", "abbabb"]]),
    )
    ...
    res = env.cmd(
        "ft.aggregate", "idx", "*", "load", 1, "t", "filter", 'contains(@t, "")'
    )
    env.assertEqual(
        toSortedFlatList(res)[1:],
        toSortedFlatList(
...
def testLoadAll(env):
    ...
    conn.execute_command(
        "HSET", "doc3", "t", "hello world", "n", 17.8, "notIndexed", "aaa"
    )
    # without LOAD
    ...
    res = [
        3,
        ["__key", "doc1", "t", "hello", "n", "42", "notIndexed", "ccc"],
    ...
    if not env.isCluster():  # TODO: fix error message in cluster
        env.expect(
            "FT.AGGREGATE", "idx", "*", "LOAD", "*", "SORTBY", 1, "@notIndexed"
        ).error().contains(
            "not loaded nor in schema"
        )  # can be enabled in the future
        env.expect(
            "FT.AGGREGATE", "idx", "*", "SORTBY", 1, "@notIndexed"
        ).error().contains(
            "not loaded nor in schema"
        )  # without LOAD it's an error (unless we enable implicit LOAD of any field for SORTBY)
        env.expect(
            "FT.AGGREGATE", "idx", "*", "LOAD", "*", "SORTBY", 1, "@notExists"
        ).error().contains(
            "not loaded nor in schema"
        )  # can be enabled in the future - should pass even if notExists doesn't exist
        env.expect(
            "FT.AGGREGATE", "idx", "*", "SORTBY", 1, "@notExists"
        ).error().contains(
            "not loaded nor in schema"
        )  # without LOAD it's an error (unless we enable implicit LOAD of any field for SORTBY)
...
def testMaxAggResults(env):
    ...
    env.expect("ft.aggregate", "idx", "*", "LIMIT", "0", "10000").error().contains(
...
def testMaxAggInf(env):
    ...
    env.expect(config_cmd(), "get", "MAXAGGREGATERESULTS").equal(
        [["MAXAGGREGATERESULTS", "unlimited"]]
    )
...
def testLoadPosition(env):
    ...
    # LOAD then SORTBY
    env.expect(
        "ft.aggregate", "idx", "*", "LOAD", "1", "t1", "SORTBY", "2", "@t1", "ASC"
    ).equal([1, ["t1", "hello"]])

    # SORTBY then LOAD
    env.expect(
        "ft.aggregate", "idx", "*", "SORTBY", "2", "@t1", "ASC", "LOAD", "1", "t1"
    ).equal([1, ["t1", "hello"]])

    # two LOADs
    env.expect("ft.aggregate", "idx", "*", "LOAD", "1", "t1", "LOAD", "1", "t2").equal(
        [1, ["t1", "hello", "t2", "world"]]
    )
    ...
    if not env.isCluster():
        env.expect(
            ...
            "1",
            "t2",
        ).error().contains("not loaded nor in pipeline")
...
def testResultCounter(env):
    ...
    env.expect("FT.AGGREGATE", "idx", "*", "FILTER", '@t1 == "hello"').equal(
        [1, ["t1", "hello"], ["t1", "hello"]]
    )
    # env.expect('FT.AGGREGATE', 'idx', '*', 'FILTER', '@t1 == "hello"').equal([2, ['t1', 'hello'], ['t1', 'hello']])

    # 3rd document is a match
    env.expect("FT.AGGREGATE", "idx", "*", "FILTER", '@t1 == "world"').equal(
        [3, ["t1", "world"]]
    )
...
def testGroupProperties(env):
    ...
    env.expect(
        "FT.AGGREGATE", "idx", "*", "GROUPBY", "3", "t", "n", "tt"
    ).error().contains(
    ...
    env.expect("FT.AGGREGATE", "idx", "*", "GROUPBY", "1", "tt").error().contains(
        "Bad arguments for GROUPBY: Unknown property `tt`. Did you mean `@tt`?"
    )
    env.expect("FT.AGGREGATE", "idx", "*", "GROUPBY", "1", "@tt").equal(
        [1, ["tt", "foo"]]
    )

    # Verify we fail on grouping by the same property twice
    env.expect("FT.AGGREGATE", "idx", "*", "GROUPBY", "2", "@t", "@t").error().contains(
    ...
    env.expect(
        ...
        "AS",
        "t",
    ).error().contains("Property `t` specified more than once")
    ...
    env.expect(
        ...
        "AS",
        "my_count",
    ).error().contains("Property `my_count` specified more than once")
    ...
    env.expect(
        ...
        "COUNT",
        "0",
    ).error().contains("specified more than once")
    ...
    env.expect(
        ...
        "AS",
        "my_output",
    ).error().contains("Property `my_output` specified more than once")
...
def testWithKNN(env):
    ...
    conn.execute_command(
        "HSET", "doc1{1}", "v", create_np_array_typed([1] * dim).tobytes(), "n", "3"
    )
    conn.execute_command(
        "HSET", "doc5{1}", "v", create_np_array_typed([5] * dim).tobytes(), "n", "2"
    )
    conn.execute_command(
        "HSET", "doc6{1}", "v", create_np_array_typed([6] * dim).tobytes(), "n", "1"
    )

    conn.execute_command(
        "HSET", "doc2{3}", "v", create_np_array_typed([2] * dim).tobytes(), "n", "5"
    )
    conn.execute_command(
        "HSET", "doc3{3}", "v", create_np_array_typed([3] * dim).tobytes(), "n", "4"
    )
    conn.execute_command(
        "HSET", "doc4{3}", "v", create_np_array_typed([4] * dim).tobytes(), "n", "6"
    )
    ...
    res = conn.execute_command(
        "FT.AGGREGATE",
        "idx",
        "*=>[KNN 3 @v $blob]=>{$yield_distance_as: dist}",
        "SORTBY",
        "1",
        "@n",
        "MAX",
        ...
        "2",
        "blob",
        create_np_array_typed([0] * dim).tobytes(),
        "DIALECT",
        "2",
    )
    ...
    res = conn.execute_command(
        "FT.AGGREGATE",
        "idx",
        "*=>[KNN 3 @v $blob]=>{$yield_distance_as: square_dist}",
        "APPLY",
        "sqrt(@square_dist)",
        "AS",
        "L2_dist",
    ...
    expected_res = [
        {"L2_dist": "2", "square_dist": "4", "n": "3"},
        {"L2_dist": "6", "square_dist": "36", "n": "4"},
    ]
    ...
    conn.execute_command(
        "HSET", "doc1{1}", "v", create_np_array_typed([1] * dim).tobytes(), "n", "100"
    )
    conn.execute_command(
        "HSET", "doc2{1}", "v", create_np_array_typed([2] * dim).tobytes(), "n", "100"
    )
    conn.execute_command(
        "HSET", "doc3{1}", "v", create_np_array_typed([3] * dim).tobytes(), "n", "100"
    )

    conn.execute_command(
        "HSET", "doc4{3}", "v", create_np_array_typed([1] * dim).tobytes(), "n", "200"
    )
    conn.execute_command(
        "HSET", "doc5{3}", "v", create_np_array_typed([2] * dim).tobytes(), "n", "200"
    )
    conn.execute_command(
        "HSET", "doc6{3}", "v", create_np_array_typed([3] * dim).tobytes(), "n", "200"
    )
    ...
    res = conn.execute_command(
        "FT.AGGREGATE",
        "idx",
        "*=>[KNN 2 @v $blob]=>{$yield_distance_as: dist}",
        "GROUPBY",
        "1",
        "@n",
        "REDUCE",
...
def setup_missing_values_index(index_missing):
    ...
    schema = [
        "tag",
        "TAG",
        "INDEXMISSING" if index_missing else None,
        "num1",
        "NUMERIC",
        "num2",
        "NUMERIC",
...
def test_aggregate_group_by_on_missing_values():
    env = setup_missing_values_index(False)
    # Search for the documents with the indexed fields (sanity)
    env.expect("FT.AGGREGATE", "idx", "@tag:{val}", "GROUPBY", "1", "@num1").equal(
        [2, ["num1", "3"], ["num1", None]]
    )
    env.flush()
...
def test_aggregate_group_by_on_missing_indexed_values():
    ...
    # Search for the documents with the indexed fields (sanity)
    env.expect(
        "FT.AGGREGATE", "idx", "ismissing(@tag) | @tag:{val}", "GROUPBY", "1", "@tag"
    ).apply(group_by_result_to_dict).equal({None: "tag", "val": "tag"})
    env.flush()
...
def test_aggregate_apply_on_missing_values():
    ...
    env.expect(
        ...
        "APPLY",
        "(@num1+@num2)/2",
    ).error().contains("has no value, consider using EXISTS if applicable")
    env.flush()
...
def test_aggregate_apply_on_missing_indexed_values():
    ...
    env.expect(
        ...
        "AS",
        "T",
    ).error().contains("tag: has no value, consider using EXISTS if applicable")
    env.flush()
