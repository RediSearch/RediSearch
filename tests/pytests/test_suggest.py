...
def testSuggestions(env):
    ...
    for _ in env.reloadingIterator():
        ...
        env.expect("ft.SUGGET", "ac", "hello", "FUZZY").equal(
            ["hello world", "hello werld", "yellow world", "hallo world"]
        )

        # search fuzzy with limit of 1
        env.expect("ft.SUGGET", "ac", "hello", "FUZZY", "MAX", "1").equal(
            ["hello world"]
        )
...
def testSuggestErrors(env):
    ...
    env.expect("ft.SUGADD ac olah 1 PAYLOAD").error().contains(
        "Invalid payload: Expected an argument, but none provided"
    )
    env.expect("ft.SUGADD ac olah 1 REDIS PAYLOAD payload").error().contains(
...
def testSuggestPayload(env):
    skipOnCrdtEnv(env)
    env.assertEqual(1, env.cmd("ft.SUGADD", "ac", "hello world", 1, "PAYLOAD", "foo"))
    env.assertEqual(2, env.cmd("ft.SUGADD", "ac", "hello werld", 1, "PAYLOAD", "bar"))
    env.assertEqual(3, env.cmd("ft.SUGADD", "ac", "hello nopayload", 1, "PAYLOAD", ""))
    env.assertEqual(4, env.cmd("ft.SUGADD", "ac", "hello nopayload2", 1))
    ...
    res = env.cmd("FT.SUGGET", "ac", "hello")
    env.assertEqual(
        ["hello world", "hello werld", "hello nopayload", "hello nopayload2"], res
    )
    res = env.cmd("FT.SUGGET", "ac", "hello", "WITHPAYLOADS", "WITHSCORES")
...
def testIssue_490(env):
    ...
    env.expect(
        ...
        "1",
        "PAYLOAD",
        "RediSearch, an awesome search engine",
    ).equal(1)
    env.expect("ft.sugget", "sug", "Redis", "WITHPAYLOADS").equal(
        ["RediSearch", "RediSearch, an awesome search engine"]
    )
    ...
    env.expect(
        ...
        "INCR",
        "PAYLOAD",
        "RediSearch 2.0, next gen search engine",
    ).equal(1)
    env.expect("ft.sugget", "sug", "Redis", "WITHPAYLOADS").equal(
        ["RediSearch", "RediSearch 2.0, next gen search engine"]
    )
