...
def downloadFiles(rdbs=None):
    ...
    for f in rdbs:
        ...
        if not os.path.exists(path):
            subprocess.run(
                ["wget", "--no-check-certificate", BASE_RDBS_URL + f, "-O", path, "-q"]
            )
        if not os.path.exists(path):
            return False
    return True
...
def testRDBCompatibility(env):
    ...
    env = Env(
        moduleArgs="UPGRADE_INDEX idx; PREFIX 1 tt; LANGUAGE french; LANGUAGE_FIELD MyLang; SCORE 0.5; SCORE_FIELD MyScore; PAYLOAD_FIELD MyPayload; UPGRADE_INDEX idx1"
    )
...
def testRDBCompatibility_vecsim():
    ...
    rdbs = ["redisearch_2.4.14_with_vecsim.rdb", "redisearch_2.6.9_with_vecsim.rdb"]
    ...
    for fileName in rdbs:
        ...
        for vec_field, algo in zip(vec_fields, algorithms):
            infos[algo] = to_dict(
                env.cmd(debug_cmd() + " VECSIM_INFO idx " + vec_field)
            )
