...
def testHashMinStemLen(env):
    ...
    env.assertEqual(res, [2, "{doc}:2", ["t", "fries"], "{doc}:3", ["t", "frying"]])
...
def testJsonMinStemLen(env):
    ...
    ########################################################
    # Create the index with default MIN_STEMMING_LEN (4)
    env.cmd("FT.CREATE", "idx_min4", "ON", "JSON", "SCHEMA", "$.t", "AS", "t", "TEXT")
    ...
    res = env.cmd("FT.SEARCH", "idx_min4", "fried", "SORTBY", "t", "ASC", "NOCONTENT")
    ...
    env.flush()
    env.cmd(config_cmd(), "SET", "MINSTEMLEN", 3)
    env.cmd("FT.CREATE", "idx_min3", "ON", "JSON", "SCHEMA", "$.t", "AS", "t", "TEXT")
    env.cmd("JSON.SET", "{doc}:1", "$", r'{"t":"fry"}')
    ...
    env.cmd("JSON.SET", "{doc}:2", "$", r'{"t":"fries"}')
    env.cmd("JSON.SET", "{doc}:3", "$", r'{"t":"frying"}')
    res = env.cmd("FT.SEARCH", "idx_min3", "fried", "SORTBY", "t", "ASC", "NOCONTENT")
