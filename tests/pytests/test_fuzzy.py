...
def testBasicFuzzy(env):
    ...
    env.assertEqual(
        set(res[2]), set(["title", "hello world", "body", "this is a test"])
    )
...
def testLdLimit(env):
    ...
    env.assertEqual(
        [1, "doc1", ["title", "hello world"]], env.cmd("ft.search", "idx", "%word%")
    )  # should be ok
    env.assertEqual(
        [0], env.cmd("ft.search", "idx", r"%sword%")
    )  # should return nothing
    env.assertEqual(
        [1, "doc1", ["title", "hello world"]], env.cmd("ft.search", "idx", r"%%sword%%")
    )
...
def testFuzzyWithNumbersOnly(env):
    env.expect(
        "ft.create", "idx", "ON", "HASH", "schema", "test", "TEXT", "SORTABLE"
    ).equal("OK")
    env.expect("ft.add", "idx", "doc1", "1.0", "FIELDS", "test", "12345").equal("OK")

    MAX_DIALECT = set_max_dialect(env)
    for dialect in range(2, MAX_DIALECT + 1):
        env.expect("ft.search", "idx", "%%21345%%", "DIALECT", dialect).equal(
            [1, "doc1", ["test", "12345"]]
        )
...
def testTagFuzzy(env):
    ...
    env.cmd("HSET", "doc", "t", "hello world")
    env.expect("FT.SEARCH", "idx1", "@t:{(%worl%)}").equal(
        [1, "doc", ["t", "hello world"]]
    )
