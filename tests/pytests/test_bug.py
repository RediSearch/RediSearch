...
def load_vectors_to_redis(conn, n_vec, query_vec_index, vec_size, data_type='FLOAT32'):
    np.random.seed(10)
...
def load_docs(client, vector_field_name, d, n):
    avg = 0
    for i in range(1, n+1):
        ...
        avg = 0.9 * avg + 0.1 * ingest
        print(f'{i}/{n} in {avg:.07} sec         ', end='\r')
    print(n, n, sep='/', end='\n')
    return np.random.rand(1, d).astype(np.float32)

def test_async_vecsim():
    env = Env(moduleArgs='DEFAULT_DIALECT 2 WORKER_THREADS 8 ENABLE_THREADS TRUE')
    ...
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'vector', 'VECTOR', algo, '8', 'TYPE', 'FLOAT32', 'DISTANCE_METRIC', 'COSINE',
               'DIM', dim, 'M', M).ok()

    np.random.seed(42)
    start = time.time()
    # query_vec = load_vectors_to_redis(conn, n, 0, dim)
    # query_vec = load_vectors_with_texts_into_redis(conn, 'vector', dim, n)[0][1]
    query_vec = load_docs(conn, 'vector', dim, n)
    total = time.time() - start
    print('Total load time: ', total)

    conn.execute_command('FT.SEARCH', 'idx', '*=>[KNN 10 @vector $blob AS dist]', 'PARAMS', '2', 'blob', query_vec.tobytes(), 'SORTBY', 'dist', 'RETURN', '1', 'dist')
...
#     # test start
#     conn = getConnectionByEnv(env)
#     conn.execute_command('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', '2','DISTANCE_METRIC', 'L2')
...
#         conn.execute_command('FT.CREATE', 'idx1', 'SCHEMA', 'v_HNSW', 'VECTOR', 'ASYNC-HNSW', '14', 'TYPE', data_type,
...
#             env.assertEqual(info_data_HNSW[:-5], ['ALGORITHM', 'ASYNC-HNSW', 'TYPE', data_type, 'DIMENSION', 1024, 'METRIC', 'COSINE', 'IS_MULTI_VALUE', 0, 'INDEX_SIZE', 0, 'INDEX_LABEL_COUNT', 0, 'M', 16, 'EF_CONSTRUCTION', 200, 'EF_RUNTIME', 10, 'MAX_LEVEL', -1, 'ENTRYPOINT', -1, 'MEMORY'])
...
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', dim, 'DISTANCE_METRIC', 'COSINE',
...
#     env.assertEqual(info_data['ALGORITHM'], 'ASYNC-HNSW')
...
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW').error().contains('Bad arguments for vector similarity number of parameters')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6').error().contains('Expected 6 parameters but got 0')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '1').error().contains('Bad number of arguments for vector similarity index: got 1 but expected even number')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '2', 'SIZE').error().contains('Bad arguments for algorithm HNSW: SIZE')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '2', 'TYPE').error().contains('Bad arguments for vector similarity HNSW index type')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '4', 'TYPE', 'FLOAT32', 'DIM').error().contains('Bad arguments for vector similarity HNSW index dim')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '4', 'DIM', '1024', 'DISTANCE_METRIC', 'IP').error().contains('Missing mandatory parameter: cannot create HNSW index without specifying TYPE argument')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '4', 'TYPE', 'FLOAT32', 'DISTANCE_METRIC', 'IP').error().contains('Missing mandatory parameter: cannot create HNSW index without specifying DIM argument')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '4', 'TYPE', 'FLOAT32', 'DIM', '1024').error().contains('Missing mandatory parameter: cannot create HNSW index without specifying DISTANCE_METRIC argument')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC').error().contains('Bad arguments for vector similarity HNSW index metric')

#     # invalid init args
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'DOUBLE', 'DIM', '1024', 'DISTANCE_METRIC', 'IP').error().contains('Bad arguments for vector similarity HNSW index type')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', 'str', 'DISTANCE_METRIC', 'IP').error().contains('Bad arguments for vector similarity HNSW index dim')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'REDIS').error().contains('Bad arguments for vector similarity HNSW index metric')
...
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'INITIAL_CAP', 'str', 'M', '16', 'EF_CONSTRUCTION', '200') \
#         .error().contains('Bad arguments for vector similarity HNSW index initial cap')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'INITIAL_CAP', '100', 'M', 'str', 'EF_CONSTRUCTION', '200') \
#         .error().contains('Bad arguments for vector similarity HNSW index m')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'INITIAL_CAP', '100', 'M', '16', 'EF_CONSTRUCTION', 'str') \
#         .error().contains('Bad arguments for vector similarity HNSW index efConstruction')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'INITIAL_CAP', '100', 'M', '16', 'EF_RUNTIME', 'str') \
#         .error().contains('Bad arguments for vector similarity HNSW index efRuntime')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'INITIAL_CAP', '100', 'M', '16', 'EF_RUNTIME', '14.3') \
#         .error().contains('Bad arguments for vector similarity HNSW index efRuntime')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'INITIAL_CAP', '100', 'M', '16', 'EF_RUNTIME', '-10') \
#         .error().contains('Bad arguments for vector similarity HNSW index efRuntime')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'INITIAL_CAP', '100', 'M', '16', 'EPSILON', 'str') \
#         .error().contains('Bad arguments for vector similarity HNSW index epsilon')
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'INITIAL_CAP', '100', 'M', '16', 'EPSILON', '-1') \
#         .error().contains('Bad arguments for vector similarity HNSW index epsilon')
...
#                         'v', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', VECSIM_DATA_TYPES[0], 'DIM', '2', 'DISTANCE_METRIC', 'COSINE',
...
#     conn.execute_command('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', dimension, 'DISTANCE_METRIC', 'L2', 't', 'TEXT')
...
#     for data_type in VECSIM_DATA_TYPES:
#         env.expect('FT.CREATE', f'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', data_type,
...
#     for data_type in VECSIM_DATA_TYPES:
#         conn.execute_command('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '8', 'TYPE', data_type,
...
#     for data_type in VECSIM_DATA_TYPES:
#         env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '8', 'TYPE', data_type,
...
#     dimension = 128
#     qty = 100
#     conn.execute_command('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32',
...
#     conn = getConnectionByEnv(env)
#     dimension = 128
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', dimension, 'DISTANCE_METRIC', 'L2').ok()
...
#         env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', data_type, 'DIM', dimension, 'DISTANCE_METRIC', 'L2').ok()
#         waitForIndex(env, 'idx')
...
#         env.assertOk(conn.execute_command('FT.CREATE', currIdx, 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '8', 'TYPE', data_type,
#                                           'DIM', dim, 'DISTANCE_METRIC', 'L2', 'INITIAL_CAP', 10000))
#         currIdx+=1

#         # Index initial size exceeded limits
#         env.expect('FT.CREATE', currIdx, 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '8', 'TYPE', data_type,
...
#         # env.expect('FT.CREATE', currIdx, 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '10', 'TYPE', 'FLOAT32',
...
#         currIdx+=1
#         # TODO: uncomment when BLOCK_SIZE is added to FT.CREATE on HNSW
#         # env.expect('FT.CREATE', currIdx, 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '10', 'TYPE', data_type,
#         #            'DIM', '16', 'DISTANCE_METRIC', 'L2', 'INITIAL_CAP', 100, 'BLOCK_SIZE', block_size).error().contains(
#         #            f'Vector index block size {block_size} exceeded server limit')
#         # currIdx+=1
...
#         env.expect('FT.CREATE', 'idx-hnsw', 'SCHEMA', 'v', 'VECTOR', 'ASYNC-HNSW', '8', 'TYPE', data_type,
...
#     env = Env(moduleArgs='DEFAULT_DIALECT 2 WORKER_THREADS 8 ENABLE_THREADS TRUE ON_TIMEOUT FAIL')
#     conn = getConnectionByEnv(env)
#     nshards = env.shardsCount
#     timeout_expected = 0 if env.isCluster() else 'Timeout limit was reached'

#     vecsim_algorithms_and_sizes = [('FLAT', 80000 * nshards), ('ASYNC-HNSW', 10000 * nshards)]
...
#             '$.vecs[*]', 'AS', 'hnsw', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', data_t, 'DIM', dim, 'DISTANCE_METRIC', 'L2',
...
#     env.expect('FT.CREATE', 'idx', 'ON', 'JSON', 'SCHEMA',
#                '$.vecs', 'AS', 'vecs', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', dim, 'DISTANCE_METRIC', 'L2').ok()
...
#     env.expect('FT.CREATE', 'idx', 'SCHEMA', 'vector', 'VECTOR', 'ASYNC-HNSW', '12', 'TYPE', 'FLOAT32', 'DIM',
...
#                    't', 'TEXT', 'num', 'NUMERIC',
#                    'v_hnsw', 'VECTOR', 'ASYNC-HNSW', '6', 'TYPE', data_type,
