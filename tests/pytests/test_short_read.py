...
ExpectedIndex = collections.namedtuple(
    "ExpectedIndex", ["count", "pattern", "search_result_count"]
)

RDBS_SHORT_READS = {
    "short-reads/redisearch_2.2.0.rdb.zip": ExpectedIndex(
        2, "shortread_idxSearch_[1-9]", [20, 55]
    ),
    "short-reads/rejson_2.0.0.rdb.zip": ExpectedIndex(
        2, "shortread_idxJson_[1-9]", [20, 55]
    ),
    "short-reads/redisearch_2.2.0_rejson_2.0.0.rdb.zip": ExpectedIndex(
        2, "shortread_idxSearchJson_[1-9]", [10, 35]
    ),
    "short-reads/redisearch_2.8.0.rdb.zip": ExpectedIndex(
        2, "shortread_idxSearch_with_geom_[1-9]", [20, 60]
    ),
    "short-reads/redisearch_2.8.4.rdb.zip": ExpectedIndex(
        2, "shortread_idxSearch_with_geom_[1-9]", [20, 60]
    ),
    "short-reads/redisearch_2.10.3.rdb.zip": ExpectedIndex(
        2, "shortread_idxSearch_[1-9]", [10, 35]
    ),
    "short-reads/redisearch_2.10.3_missing.rdb.zip": ExpectedIndex(
        2, "shortread_idxSearch_[1-9]", [20, 55]
    ),
}
RDBS_COMPATIBILITY = {
...
def rand_name(k):
    ...
    return "".join(
        [
            ...
            for _ in range(0, random.randint(2, max(2, k)))
        ]
    )
...
def rand_num(k):
    ...
    return chr(random.randint(ord("1"), ord("9"))) + "".join(
        [
            chr(random.randint(ord("0"), ord("9")))
            for _ in range(0, random.randint(1, max(1, k - 1)))
        ]
    )
...
def get_polygon(x, y, i):
    return "POLYGON({} {}, {} {}, {} {}, {} {}, {} {})".format(
        x, y, x + 15 * i, y, x + 15 * i, y + 17.2 * i, x, y + 17.2 * i, x, y
    )


def add_index(
    env, isHash, index_name, key_suffix, num_prefs, num_keys, num_geometry_keys=0
):
    ...
    cmd_create.extend(
        [
            ...
            "dm:es",
            "sortable",
            get_identifier("field2", isHash),
            "as",
            "f2",
            "numeric",
            "sortable",
            get_identifier("field3", isHash),
            "as",
            "f3",
            "geo",
            get_identifier("field4", isHash),
            "as",
            "f4",
            "tag",
            "separator",
            ";",
            get_identifier("field6", isHash),
            "as",
            "f6",
            "tag",
            "INDEXEMPTY",
            get_identifier("field7", isHash),
            "as",
            "f7",
            "tag",
            "INDEXEMPTY",
            "SORTABLE",
            get_identifier("field8", isHash),
            "as",
            "f8",
            "TEXT",
            "INDEXEMPTY",
            get_identifier("field9", isHash),
            "as",
            "f9",
            "TEXT",
            "INDEXEMPTY",
            "SORTABLE",
            get_identifier("field10", isHash),
            "as",
            "f10",
            "tag",
            "INDEXEMPTY",
            "SORTABLE",
            get_identifier("field15", isHash),
            "as",
            "f15",
            "tag",
            "INDEXMISSING",
            get_identifier("field16", isHash),
            "as",
            "f16",
            "tag",
            "INDEXMISSING",
            "SORTABLE",
            get_identifier("field17", isHash),
            "as",
            "f17",
            "text",
            "INDEXMISSING",
            get_identifier("field18", isHash),
            "as",
            "f18",
            "text",
            "INDEXMISSING",
            "SORTABLE",
            get_identifier("field11", isHash),
            "text",
            "nostem",
            get_identifier("field12", isHash),
            "numeric",
            "noindex",
            get_identifier("field13", isHash),
            "geo",
            get_identifier("field14", isHash),
            "tag",
            "noindex",
            get_identifier("myLang", isHash),
            "text",
            get_identifier("myScore", isHash),
            "numeric",
        ]
    )
    ...
    for i in range(1, num_keys + 1):
        if isHash:
            cmd = [
                "hset",
                "pref" + str(i) + ":k" + str(i) + "_" + rand_num(5) + key_suffix,
                "a" + rand_name(5),
                rand_num(2),
                "b" + rand_name(5),
                rand_num(3),
    ...
    for i in range(num_keys + 1, num_keys + num_geometry_keys + 1):
        ...
        if isHash:
            ...
            cmd = [
                ...
                "pref" + str(i) + ":k" + str(i) + "_" + rand_num(5) + key_suffix,
                "$",
                r'{"field5":"' + geom_wkt + r'", "field15":"' + geom_wkt + r'"}',
            ]
            env.assertOk(conn.execute_command(*cmd))
...
def _testCreateIndexRdbFilesWithJSON(env):
    ...
    create_indices(
        env, "redisearch_2.8.12_rejson_2.0.0.rdb", "idxSearchJson", True, True
    )
...
class ShardMock:
    ...
    def StartListening(self, port, attempts=1):
        ...
        for i in range(1, attempts + 1):
            self.stream_server = gevent.server.StreamServer(
                ("127.0.0.1", port), self._handle_conn
            )
            try:
            ...
            self.env.assertNotEqual(
                ShardMock.server_port,
                None,
                message="%s: StartListening(%d/%d) %d -> %d"
                % (self.__class__.__name__, i, attempts, port, ShardMock.server_port),
            )
            break
        else:
            raise Exception(
                "%s StartListening failed: %s"
...
class Debug:
    ...
    def print_bytes_incremental(self, env, data, total_len, name):
        ...
        self.dbg_str = "{} {:=0{}n}:{:<2}({:<3})".format(
            self.dbg_str, self.dbg_ndx, byte_count_width, printable_ch, ch
        )
    ...
    with ShardMock(env) as shardMock:
        ...
        if server_version_less_than(env, "7.0.0"):
            ...
            if is_shortread:
                ...
                for ind, expected_result_count in zip(
                    expected_indices, expected_index.search_result_count
                ):
