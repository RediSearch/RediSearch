...
def runTestWithSeed(env, s=None):
    ...
    for i in range(count):
        ...
        env.expect("FT.SEARCH", "idx", "@n:[%d,%d]" % (x, x), "LIMIT", 0, 0).equal(
            [count / num_values]
        )
...
def testMemoryAfterDrop(env):
    ...
    for i in range(idx_count):
        env.expect(
            "FT.CREATE", "idx%d" % i, "PREFIX", 1, "%ddoc" % i, "SCHEMA", "n", "NUMERIC"
        ).ok()
...
def testMemoryAfterDrop_geo(env):
    ...
    for i in range(idx_count):
        env.expect(
            "FT.CREATE", "idx%d" % i, "PREFIX", 1, "%ddoc" % i, "SCHEMA", "g", "GEO"
        ).ok()
...
def testMemoryAfterDrop_text(env):
    ...
    for i in range(idx_count):
        env.expect(
            "FT.CREATE", "idx%d" % i, "PREFIX", 1, "%ddoc" % i, "SCHEMA", "t", "TEXT"
        ).ok()
...
def testMemoryAfterDrop_tag(env):
    ...
    for i in range(idx_count):
        env.expect(
            "FT.CREATE", "idx%d" % i, "PREFIX", 1, "%ddoc" % i, "SCHEMA", "tg", "TAG"
        ).ok()
