...
class TestDebugCommands(object):
    ...
    def __init__(self):
        ...
        self.env.expect("HSET", "doc1", "name", "meir", "age", "34", "t", "test").equal(
            3
        )
        self.env.cmd("SET", "foo", "bar")

    def testDebugWrongArity(self):
        self.env.expect(debug_cmd(), "dump_invidx").error().contains(
        ...
        self.env.assertEqual(
            [
                ...
                [
                    ...
                    ["index", 2, "field", "t AS t", "value", "test"],
                ],
            ],
            rv,
        )
    ...
    def testInfoTagIndex(self):
        ...
        self.env.expect(debug_cmd(), "INFO_TAGIDX", "idx", "t").equal(["num_values", 1])
        self.env.expect(
            debug_cmd(), "INFO_TAGIDX", "idx", "t", "dump_id_entries"
        ).equal(["num_values", 1, "values", []])
        self.env.expect(
            debug_cmd(), "INFO_TAGIDX", "idx", "t", "count_value_entries"
        ).equal(["num_values", 1, "values", []])
        self.env.expect(
            debug_cmd(), "INFO_TAGIDX", "idx", "t", "dump_id_entries", "limit", "1"
        ).equal(
            [
                "num_values",
                1,
                "values",
                [["value", "test", "num_entries", 1, "num_blocks", 1, "entries", [1]]],
            ]
        )
        self.env.expect(
            debug_cmd(), "INFO_TAGIDX", "idx", "t", "count_value_entries", "limit", "1"
        ).equal(
            [
                "num_values",
                1,
                "values",
                [["value", "test", "num_entries", 1, "num_blocks", 1]],
            ]
        )
        self.env.expect(
            debug_cmd(),
    ...
    def testIdToDocIdOnUnexistingId(self):
        self.env.expect(debug_cmd(), "idtodocid", "idx", "2").error().equal(
            "document was removed"
        )
        self.env.expect(debug_cmd(), "idtodocid", "idx", "docId").error().equal(
            "bad id given"
        )

    def testDumpPhoneticHash(self):
        self.env.expect(debug_cmd(), "dump_phonetic_hash", "test").equal(
            ["<TST", "<TST"]
        )
        self.env.expect(debug_cmd(), "DUMP_PHONETIC_HASH", "test").equal(
            ["<TST", "<TST"]
    ...
    def testInvertedIndexSummary(self):
        self.env.expect(debug_cmd(), "invidx_summary", "idx", "meir").equal(
            [
                "numDocs",
                1,
                "numEntries",
    ...
    def testGCStopAndContinueSchedule(self):
        self.env.expect(
            debug_cmd(), "GC_STOP_SCHEDULE", "non-existing"
        ).error().contains("Unknown index name")
        self.env.expect(
            debug_cmd(), "GC_CONTINUE_SCHEDULE", "non-existing"
        ).error().contains("Unknown index name")
        self.env.expect(debug_cmd(), "GC_CONTINUE_SCHEDULE", "idx").error().contains(
            "GC is already running periodically"
        )
    ...
    def testTTLcommands(self):
        ...
        self.env.expect(debug_cmd(), "TTL", "non-existing").error().contains(
            "Unknown index name"
        )
        self.env.expect(debug_cmd(), "TTL_PAUSE", "non-existing").error().contains(
            "Unknown index name"
        )
        self.env.expect(debug_cmd(), "TTL_EXPIRE", "non-existing").error().contains(
            "Unknown index name"
        )
        self.env.expect(debug_cmd(), "TTL", "idx").error().contains(
            "Index is not temporary"
        )
        self.env.expect(debug_cmd(), "TTL_PAUSE", "idx").error().contains(
            "Index is not temporary"
        )
        self.env.expect(debug_cmd(), "TTL_EXPIRE", "idx").error().contains(
            "Index is not temporary"
        )
        ...
        self.env.assertGreater(
            self.env.cmd(debug_cmd(), "TTL", "idx_temp"), 3000
        )  # It should be close to 3600.
        self.env.expect(debug_cmd(), "TTL_PAUSE", "idx_temp").ok()
        self.env.expect(debug_cmd(), "TTL_PAUSE", "idx_temp").error().contains(
            "Index does not have a timer"
        )
    ...
    def testStopAndResumeWorkersPool(self):
        ...
        self.env.expect(debug_cmd(), "WORKERS", "pause").error().contains(
            "Operation failed: workers thread pool doesn't exists or is not running"
        )
        self.env.expect(debug_cmd(), "WORKERS", "resume").ok()
        self.env.expect(debug_cmd(), "WORKERS", "resume").error().contains(
            "Operation failed: workers thread pool doesn't exists or is already running"
        )
    ...
    def testWorkersPoolDrain(self):
        ...
        self.env.expect(debug_cmd(), "WORKERS", "pause").ok()
        self.env.expect(debug_cmd(), "WORKERS", "drain").error().contains(
            "Operation failed: workers thread pool is not running"
        )
        self.env.expect(
            "HSET",
            "doc1",
            ...
            "test",
            "v",
            create_np_array_typed([1, 2]).tobytes(),
        ).equal(1)
        ...
        self.env.assertEqual(
            stats,
            {
                "totalJobsDone": orig_stats["totalJobsDone"],
        ...
        self.env.assertEqual(
            stats,
            {
                "totalJobsDone": orig_stats["totalJobsDone"] + 1,
                "totalPendingJobs": orig_stats["totalPendingJobs"] - 1,
                "highPriorityPendingJobs": orig_stats["highPriorityPendingJobs"],
                "lowPriorityPendingJobs": orig_stats["lowPriorityPendingJobs"] - 1,
                "numThreadsAlive": self.workers_count,
...
def testDumpHNSW(env):
    ...
    env.cmd(
        "FT.CREATE temp-idx ON JSON prefix 1 _ SCHEMA "
    ...
    env.cmd(
        *[
            ...
            "_doc1",
            "$",
            '{"v_HNSW":[1, 1], "v_HNSW_multi":[[1, 1], [2, 2]], "v_flat":[1, 1]}',
        ]
    )
    env.cmd(
        *[
            ...
            "_doc2",
            "$",
            '{"v_HNSW":[3, 3], "v_HNSW_multi":[[3, 3], [4, 4]], "v_flat":[3, 3]}',
        ]
    )
    env.cmd(
        *[
            ...
            "_doc3",
            "$",
            '{"v_HNSW_multi":[[5, 5], [6, 6]], "v_flat":[5, 5]}',
        ]
    )
    env.expect(index_info(env, "temp-idx")["num_docs"], 3)

    # Test error handling
    env.expect(debug_cmd(), "DUMP_HNSW", "temp-idx").error().contains(
        f"wrong number of arguments for '{debug_cmd()}|DUMP_HNSW' command"
    )
    env.expect(debug_cmd(), "DUMP_HNSW", "bad_idx", "v").error().contains(
        "Can not create a search ctx"
    )
    env.expect(debug_cmd(), "DUMP_HNSW", "temp-idx", "bad_vec_field").error().contains(
        "Vector index not found"
    )
    env.expect(debug_cmd(), "DUMP_HNSW", "temp-idx", "v_flat").error().contains(
        "Vector index is not an HNSW index"
    )
    env.expect(debug_cmd(), "DUMP_HNSW", "temp-idx", "v_HNSW_multi").error().contains(
        "Command not supported for HNSW multi-value index"
    )
    env.expect(
        debug_cmd(), "DUMP_HNSW", "temp-idx", "v_HNSW", "_bad_doc_name"
    ).error().contains("The given key does not exist in index")

    # Test valid scenarios - with and without specifying a specific document (dump for all if doc is not provided).
    env.expect(debug_cmd(), "DUMP_HNSW", "temp-idx", "v_HNSW", "_doc1").equal(
        ["Doc id", 1, ["Neighbors in level 0", 2]]
    )

    env.expect(debug_cmd(), "DUMP_HNSW", "temp-idx", "v_HNSW").equal(
        [
            ["Doc id", 1, ["Neighbors in level 0", 2]],
            ["Doc id", 2, ["Neighbors in level 0", 1]],
