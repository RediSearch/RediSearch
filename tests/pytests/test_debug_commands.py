...
class TestDebugCommands(object):
    ...
    def testInfoTagIndex(self):
        self.env.expect('FT.DEBUG', 'info_tagidx', 'idx', 't') \
            .equal(['num_values', 1, 'memory_usage', 29])
        self.env.expect('FT.DEBUG', 'INFO_TAGIDX', 'idx', 't') \
            .equal(['num_values', 1, 'memory_usage', 29])
        self.env.expect('FT.DEBUG', 'INFO_TAGIDX', 'idx', 't', 'dump_id_entries') \
            .equal(['num_values', 1, 'memory_usage', 29, 'values', []])
        self.env.expect('FT.DEBUG', 'INFO_TAGIDX', 'idx', 't', 'count_value_entries') \
            .equal(['num_values', 1, 'memory_usage', 29, 'values', []])
