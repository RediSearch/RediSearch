...
def testSpecIndexesInfo(env: Env):
    ...
    # adding the document will create a new index block (48 bytes) with 8 bytes of buffer capacity
    expected_reply["inverted_indexes_memory"] = getInvertedIndexInitialSize(env, ['NUMERIC']) + 56
    debug_output = env.cmd(debug_cmd(), 'SPEC_INVIDXES_INFO', 'idx')
    env.assertEqual(to_dict(debug_output), expected_reply)
