    ...
    try:
        for policy in ('return', 'fail'):
            ...
            for args, point in queries:
                ...
                # Standalone HYBRID and coordinator AGGREGATE already serialize
                # on workers and retain their clock-based timeouts on this branch.
                clock_only = 'FT.AGGREGATE' if env.isCluster() else 'FT.HYBRID'
                callback_timeout = policy == 'fail' and args[0] != clock_only
                actions = ('success', 'disconnect', 'timeout') if callback_timeout else ('success', 'disconnect')
