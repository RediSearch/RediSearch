...
doc1_content = [
    {
        ...
        "seq": [1, 5],
        "nested1": [
            {"name": "top1_1_1", "seq": [2, 3, 40]},
            {"name": "top1_1_2", "seq": [5, 4, 2, 2, 2]},
            {"name": "top1_1_3", "seq": [10]},
        ],
        "nested2": [
            {"name": "top1_2_1", "seq": [1.5, 1.6, 2]},
            {"name": "top1_2_2", "seq": [0]},
            {"name": "top1_2_3", "seq": [11]},
        ],
    },
    {
        "name": "top2",
        ...
        "nested1": [
            {"name": "top2_1_1", "seq": []},
            {"name": "top2_1_2", "seq": [1]},
            {"name": "top2_1_3", "seq": [10, 20, 30, 40, 50, 60]},
        ],
        "nested2": [
            {"name": "top2_2_1", "seq": [-1.5, -1.6, -2]},
            {"name": "top2_2_2", "seq": [3.1415]},
            {"name": "top2_2_3", "seq": [42, 64, -1, 10e20, -10.0e-5]},
        ],
    },
]
...
def testBasic(env):
    ...
    env.expect(
        "FT.CREATE", "idx1", "ON", "JSON", "SCHEMA", "$..seq[*]", "AS", "seq", "NUMERIC"
    ).ok()
    env.expect(
        "FT.CREATE",
        "idx2",
    ...
    env.expect("FT.SEARCH", "idx3", "@seq:[-0.0002 -0.0001]", "NOCONTENT").equal(
        [1, "doc:1"]
    )
...
def testRange(env):
    ...
    for doc in range(0, doc_num):
        ...
        conn.execute_command(
            "JSON.SET", "doc:{}".format(doc + 1), "$", json.dumps({"top": top})
        )
    ...
    env.expect(
        "FT.SEARCH", "idx:all", "@val:[-inf (-{}]".format(max_val), "NOCONTENT"
    ).equal([0])
    env.expect(
        "FT.SEARCH", "idx:all", "@val:[({} +inf]".format(max_val), "NOCONTENT"
    ).equal([0])
    ...
    for dialect in [1, 2, 3]:
        for doc in range(doc_num, 0, -1):
            ...
            if dialect > 1:
                ...
                env.assertEqual(
                    toSortedFlatList(res), [1, lastdoc], message="[{}]".format(lastdoc)
                )
...
def testDebugDump(env):
    ...
    conn = getConnectionByEnv(env)
    env.expect(
        "FT.CREATE", "idx:top", "ON", "JSON", "SCHEMA", "$[*]", "AS", "val", "NUMERIC"
    ).ok()
...
def testInvertedIndexMultipleBlocks(env):
    ...
    for doc in range(doc_num, 0, -1):
        conn.execute_command(
            ...
            "doc:{}".format(doc),
            "$",
            json.dumps({"arr": [doc, doc + doc_num - overlap], "arr2": [doc]}),
        )
    expected_ids = range(1, doc_num + 1)
    res = conn.execute_command(debug_cmd(), "DUMP_NUMIDX", "idx", "arr")
    env.assertEqual(
        set(toSortedFlatList(res)), set(expected_ids), message="DUMP_NUMIDX"
    )
    ...
    res = conn.execute_command(
        "FT.SEARCH",
        "idx",
        "@arr:[{} {}]".format(doc_num - overlap + 1, doc_num),
        "NOCONTENT",
        "LIMIT",
        "0",
        overlap * 2,
    ...
    expected_docs = [
        "doc:{}".format(i)
        for i in chain(range(1, overlap + 1), range(doc_num - overlap + 1, doc_num + 1))
    ]
    env.assertEqual(
        toSortedFlatList(res[1:]), toSortedFlatList(expected_docs), message="FT.SEARCH"
    )
...
def checkInfoAndGC(env, idx, doc_num, create, delete):
    ...
    env.assertLessEqual(
        int(info["total_inverted_index_blocks"]), 1
    )  # 1 block might already be there
...
def testInfoAndGC(env):
    ...
    def create_json_docs_multi(env, doc_num):
        for doc in range(1, doc_num + 1):
            ...
            conn.execute_command(
                "JSON.SET", "doc:{}".format(doc), "$", json.dumps({"top": val_list})
            )
    ...
    def create_json_docs_single(env, doc_num):
        for doc in range(1, doc_num + 1):
            conn.execute_command(
                ...
                "doc:{}".format(doc),
                "$",
                json.dumps({"top": random.uniform(1, 100000)}),
            )
    ...
    def create_hash_docs(env, doc_num):
        for doc in range(1, doc_num + 1):
            conn.execute_command(
                "HSET", "doc:{}".format(doc), "top", random.uniform(1, 100000)
            )
    ...
    checkInfoAndGC(
        env, "idx_json_mult", doc_num, create_json_docs_multi, delete_json_docs
    )
    ...
    checkInfoAndGC(
        env, "idx_json_single", doc_num, create_json_docs_single, delete_json_docs
    )
...
def prepareSortBy(env, is_flat_arr, default_dialect):
    ...
    env.expect(
        "FT.CREATE", "idx", "ON", "JSON", "SCHEMA", jsonpath, "AS", "val", "NUMERIC"
    ).ok()
    ...
    for doc in range(1, doc_num + 1):
        ...
        if val_count:
            ...
            val_list.insert(0, doc)
        conn.execute_command(
            "JSON.SET", "{}".format(doc), "$", json.dumps({"top": val_list})
        )
    ...
    query = [
        "FT.SEARCH",
        "idx",
        "@val:[3000 8000] | @val:[{} {}] | @val:[{} {}]".format(
            int(doc_num / 2), int(doc_num / 2), doc_num, doc_num
        ),
        "NOCONTENT",
        "LIMIT",
        0,
...
def checkSortByBWC(env, is_flat_arr):
    ...
    env.assertEqual(
        1, 1, message="flat {}, default dialect {}".format(is_flat_arr, default_dialect)
    )
...
def testInfoStats(env):
    ...
    while doc_created < doc_num:
        ...
        conn.execute_command(
            "JSON.SET",
            "doc:multi:{{{}}}".format(doc_created),
            "$",
            json.dumps({"top": val_list}),
        )
        # Multi docs with single value
        for i in range(val_count):
            conn.execute_command(
                "JSON.SET",
                "doc:single:{{{}}}".format(doc_created + i),
                "$",
                json.dumps({"top": val_list[i]}),
            )
...
def testInfoStatsAndSearchAsSingle(env):
    ...
    max_attr_num = 5
    schema_list = [
        ["$.val{}".format(i), "AS", "val{}".format(i), "NUMERIC"]
        for i in range(1, max_attr_num + 1)
    ]
    create_idx_single = [
        "FT.CREATE",
        "idx:single",
    ...
    for doc in range(1, doc_num + 1):
        ...
        conn.execute_command(
            "JSON.SET",
            "doc:multi:{{{}}}".format(doc),
            "$",
            json.dumps({"val1": val_list}),
        )
        ...
        json_val = {
            k: v
            for (k, v) in zip(
                ["val{}".format(i + 1) for i in range(val_count)], val_list
            )
        }
        conn.execute_command(
            "JSON.SET", "doc:single:{{{}}}".format(doc), "$", json.dumps(json_val)
        )
    ...
    for _ in range(1000):
        ...
        expression_for_single = "|".join(
            [
                "@val{}:[{} {}]".format(i, val_from, val_to)
                for i in range(1, max_attr_num + 1)
            ]
        )
        res_single = conn.execute_command(
            "FT.SEARCH", "idx:single", expression_for_single, "NOCONTENT"
        )
        res_single = list(
            map(
                lambda v: v.replace(":single:", "::") if isinstance(v, str) else v,
                res_single,
            )
        )
        res_multi = conn.execute_command(
            "FT.SEARCH",
            "idx:multi",
            "@val1:[{} {}]".format(val_from, val_to),
            "NOCONTENT",
        )
        res_multi = list(
            map(
                lambda v: v.replace(":multi:", "::") if isinstance(v, str) else v,
                res_multi,
            )
        )
        env.assertEqual(
            res_single, res_multi, message="[{} {}]".format(val_from, val_to)
        )
...
def testConsecutiveValues(env):
    ...
    i = -5000
    env.expect(
        "FT.CREATE", "idx", "ON", "JSON", "SCHEMA", "$.val", "AS", "val", "NUMERIC"
    ).ok()
    for doc in range(1, num_docs + 1):
        conn.execute_command(
            "JSON.SET", "doc:{}".format(doc), "$", json.dumps({"val": [i, i + 1]})
        )
        i = i + 1

    env.expect("FT.SEARCH", "idx", "@val:[-5000 -4999]", "NOCONTENT").equal(
        [2, "doc:1", "doc:2"]
    )
    env.expect("FT.SEARCH", "idx", "@val:[5 6]", "NOCONTENT").equal(
        [3, "doc:5005", "doc:5006", "doc:5007"]
    )
    env.expect("FT.SEARCH", "idx", "@val:[4999 5000]", "NOCONTENT").equal(
        [2, "doc:9999", "doc:10000"]
    )
    ...
    env.expect("FT.SEARCH", "idx", "@val:[-6 -5]", "NOCONTENT").equal(
        [3, "doc:5005", "doc:5006", "doc:5007"]
    )
    env.expect("FT.SEARCH", "idx", "@val:[-5000 -4999]", "NOCONTENT").equal(
        [2, "doc:9999", "doc:10000"]
...
def testDebugRangeTree(env):
    ...
    env.expect(debug_cmd(), "DUMP_NUMIDXTREE", "idx", "val").equal(
        [
            "numRanges",
            1,
            "numEntries",
            ...
            [
                ...
                [
                    ...
                    [
                        ...
                        "size",
                        1,
                        "blocks_efficiency (numEntries/size)",
                        str(9),
                        "values",
                        [
                            "value",
            ...
            "Tree stats:",
            ["Average memory efficiency (numEntries/size)/numRanges", str(9)],
        ]
    )
...
def checkMultiNumericReturn(env, expected, default_dialect, is_sortable):
    ...
    message = "dialect {}, sortable {}".format(
        "default" if default_dialect else 3, is_sortable
    )
    ...
    env.assertEqual(
        conn.execute_command(
            "FT.SEARCH", "idx_flat", "@val:[2 3]", "RETURN", "1", "val", *dialect_param
        ),
        expected[1],
        message=message,
    )
    ...
    env.assertEqual(
        conn.execute_command(
            "FT.SEARCH", "idx_arr", "@val:[2 3]", "RETURN", "1", "val", *dialect_param
        ),
        expected[2],
        message=message,
    )
    ...
    res = conn.execute_command(
        "FT.AGGREGATE", "idx_arr", "@val:[2 3]", "GROUPBY", "1", "@val", *dialect_param
    )
    ...
    env.assertEqual(
        conn.execute_command(
            "FT.AGGREGATE", "idx_arr", "@val:[2 3]", "LOAD", "1", "@val", *dialect_param
        ),
        [1, ["val", expected[2][2][1]]],
        message=message,
    )
