...
def testCn(env):
    ...
    env.cmd(
        "ft.create", "idx", "ON", "HASH", "LANGUAGE", "CHINESE", "schema", "txt", "text"
    )
    waitForIndex(env, "idx")
    env.cmd("ft.add", "idx", "doc1", 1.0, "FIELDS", "txt", text)
    res = env.cmd(
        "ft.search", "idx", "之旅", "SUMMARIZE", "HIGHLIGHT", "LANGUAGE", "chinese"
    )
    ...
    env.cmd(
        "ft.search",
        "idx",
        "简介Redisson 是一个高级的分布式协调Redis客户端",
        "language",
        "chinese",
    )
...
def testTradSimp(env):
    ...
    waitForIndex(env, "idx")
    env.cmd(
        "ft.add", "idx", "genS", 1.0, "language", "chinese", "fields", "txt", GEN_CN_S
    )
    env.cmd(
        "ft.add", "idx", "genT", 1.0, "language", "chinese", "fields", "txt", GEN_CN_T
    )
...
def testMixedEscapes(env):
    ...
    env.cmd(
        ...
        "fields",
        "txt",
        "one \\:\\:hello two 器上同步 \\-hello world\\- two 器上同步",
    )
