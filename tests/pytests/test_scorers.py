...
from common import getConnectionByEnv, server_version_at_least, skip, waitForIndex
...
def testHammingScorer(env):
    ...
    for i in range(16):
        res = conn.execute_command(
            "hset", "doc%d" % i, "PAYLOAD", ("%x" % i) * 8, "title", "hello world"
        )
    ...
    for i in range(16):
        ...
        res = env.cmd(
            "ft.search", "idx", "*", "SCORER", "HAMMING", "WITHSCORES", "WITHPAYLOADS"
        )
        env.assertEqual(res[2], "0")
...
def testScoreIndex(env):
    ...
    for _ in env.reloadingIterator():
        ...
        for i, scorer in enumerate(scorers):
            ...
            res = [
                round(float(x), 2) if j > 0 and (j - 1) % 2 == 1 else x
...
def testTFIDFScorerExplanation(env):
    ...
    env.assertEqual(
        res[2][1],
        [
            "Final TFIDF : words TFIDF 20.00 * document score 0.50 / norm 10 / slop 1",
            [
                [
    ...
    env.assertEqual(
        res[5][1],
        [
            "Final TFIDF : words TFIDF 20.00 * document score 1.00 / norm 10 / slop 2",
            [
                [
                    "(Weight 1.00 * total children TFIDF 20.00)",
                    [
    ...
    env.assertEqual(
        res[8][1],
        [
            "Final TFIDF : words TFIDF 20.00 * document score 0.10 / norm 10 / slop 3",
            [
                [
                    "(Weight 1.00 * total children TFIDF 20.00)",
                    [
    ...
    env.assertEqual(
        res[2][1],
        [
            "Final TFIDF : words TFIDF 30.00 * document score 0.50 / norm 10 / slop 1",
            [
                [
    ...
    res1 = [
        "Final TFIDF : words TFIDF 40.00 * document score 1.00 / norm 10 / slop 1",
        [
            [
...
def testBM25ScorerExplanation(env):
    ...
    if env.isCluster():
        ...
        env.assertEqual(
            res[2][1],
            [
                "Final BM25 : words BM25 0.70 * document score 0.50 / slop 1",
                [
                    [
        ...
        env.assertEqual(
            res[5][1],
            [
                "Final BM25 : words BM25 0.70 * document score 1.00 / slop 2",
                [
                    [
                        "(Weight 1.00 * children BM25 0.70)",
                        [
        ...
        env.assertEqual(
            res[8][1],
            [
                "Final BM25 : words BM25 0.70 * document score 0.10 / slop 3",
                [
                    [
                        "(Weight 1.00 * children BM25 0.70)",
                        [
...
def testBM25STDScorerExplanation(env):
    ...
    waitForIndex(env, "idx")
    conn.execute_command(
        "HSET", "doc1", "title", "hello world", "body", "lorem ist ipsum"
    )
    conn.execute_command(
        "HSET",
        "doc2",
    ...
    if env.isCluster():
        env.assertEqual(
            res[2][1],
            [
                "Final BM25 : words BM25 1.13 * document score 1.00",
                [
                    [
        ...
        env.assertEqual(
            res[5][1],
            [
                "Final BM25 : words BM25 0.72 * document score 1.00",
                [
                    [
        ...
        env.assertEqual(
            res[8][1],
            [
                "Final BM25 : words BM25 0.71 * document score 1.00",
                [
                    [
        ...
        env.assertEqual(
            res[2][1],
            [
                "Final BM25 : words BM25 0.53 * document score 1.00",
                [
                    [
        ...
        env.assertEqual(
            res[5][1],
            [
                "Final BM25 : words BM25 0.52 * document score 1.00",
                [
                    [
...
def testDisMaxScorerExplanation(env):
    ...
    env.assertEqual(
        res[2][1],
        [
            "20.00 = Weight 1.00 * children DISMAX 20.00",
            [
                "DISMAX 10.00 = Weight 1.00 * Frequency 10",
                "DISMAX 10.00 = Weight 1.00 * Frequency 10",
            ],
        ],
    )
...
def testScoreReplace(env):
    ...
    if not env.isCluster:
        ...
        env.expect("FT.SEARCH idx redisearch withscores nocontent").equal(
            [1, "doc1", "1"]
        )
...
def testExposeScore(env):
    ...
    env.expect("FT.SEARCH", "idx", "~hello", "ADDSCORES").error().equal(
        "ADDSCORES is not supported on FT.SEARCH"
    )
