...
class TestShardTimeout:
    ...
    def test_shard_timeout_fail_paused_before_reply(self):
        """Test shard timeout with FAIL policy when query is paused before reply.

        Uses SET_PAUSE_BEFORE_REPLY to pause before acquiring the replying lock.
        Verifies that when timeout is triggered, the timeout callback waits for reply to finish.
        """
        env = self.env
        ...
        wait_for_condition(
            lambda: (getIsReplyPaused(env) == 1, {'paused': getIsReplyPaused(env)}),
            'Timeout while waiting for reply to pause'
        )
