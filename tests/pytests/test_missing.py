...
ALL_DOCS = [
    5,
    DOC_WITH_ONLY_FIELD1,
    DOC_WITH_ONLY_FIELD2,
    DOC_WITH_BOTH,
    DOC_WITH_NONE,
    DOC_WITH_BOTH_AND_TEXT,
]
...
def testMissingValidations():
    ...
    # empty values
    env.expect(
        "FT.CREATE", "idx", "SCHEMA", "tag", "TAG", "text", "TEXT", "numeric", "NUMERIC"
    ).ok()
    ...
    env.expect(
        "FT.CREATE", "idx3", "SCHEMA", "f1", "TAG", "INDEXMISSING", "NOINDEX"
    ).error().contains(
...
def testMissingBasic():
    ...
    res = env.cmd(
        "FT.SEARCH", "idx", "ismissing(@ta)", "NOCONTENT", "SORTBY", "te", "ASC"
    )
    ...
    res = env.cmd(
        "FT.SEARCH", "idx", "ismissing(@te)", "NOCONTENT", "SORTBY", "ta", "ASC"
    )
    env.assertEqual(res[0], 2)
...
def MissingTestIndex(
    env, conn, idx, ftype, field1, field2, val1, val2, field1Opt, isjson=False
):
    ...
    env.assertEqual(
        res, [3, DOC_WITH_ONLY_FIELD1, DOC_WITH_BOTH, DOC_WITH_BOTH_AND_TEXT]
    )
    ...
    res = conn.execute_command(
        "FT.SEARCH",
        idx,
        f"ismissing(@{field1}) @text:(dummy)",
        "NOCONTENT",
        "WITHCOUNT",
    )
    env.assertEqual(res, expected)
    res = conn.execute_command(
        "FT.SEARCH", idx, f"@text:(dummy) ismissing(@{field1})", "NOCONTENT"
    )
    env.assertEqual(res, expected)
    ...
    expected = [1, DOC_WITH_ONLY_FIELD2]
    res = conn.execute_command(
        "FT.SEARCH", idx, f"ismissing(@{field1}) -@text:(dummy)", "NOCONTENT"
    )
    env.assertEqual(res, expected)
    res = conn.execute_command(
        "FT.SEARCH", idx, f"-@text:(dummy) ismissing(@{field1})", "NOCONTENT"
    ...
    if not isjson:
        conn.execute_command(
            "HSET", DOC_WITH_ONLY_FIELD2, field1, val1, field2, val2, "id", 2
        )
    else:
        conn.execute_command(
            "JSON.SET",
            DOC_WITH_ONLY_FIELD2,
            "$",
            json.dumps({field1: val1, field2: val2, "id": 2}),
        )
    res = conn.execute_command("FT.SEARCH", idx, f"ismissing(@{field1})", "NOCONTENT")
    env.assertEqual(res, [1, DOC_WITH_NONE])
    ...
    if not isjson:
        ...
        conn.execute_command(
            "JSON.SET", DOC_WITH_ONLY_FIELD2, "$", json.dumps({field2: val2, "id": 2})
        )
    ...
    if not isjson:
        ...
        conn.execute_command(
            "JSON.SET", DOC_WITH_ONLY_FIELD1, "$", json.dumps({"id": 1})
        )
    ...
    if not isjson:
        ...
        conn.execute_command(
            "JSON.SET", DOC_WITH_ONLY_FIELD1, "$", json.dumps({field1: val1, "id": 1})
        )
...
def MissingTestTwoMissingFields(
    env, conn, idx, ftype, field1, field2, val1, val2, isjson=False
):
    ...
    expected = [1, DOC_WITH_NONE]
    res = conn.execute_command(
        "FT.SEARCH", idx, f"ismissing(@{field1}) ismissing(@{field2})", "NOCONTENT"
    )
    env.assertEqual(res, expected)
    ...
    if raw_encoding == "false":
        ...
        res = conn.execute_command(
            "FT.SEARCH",
            idx,
            f"ismissing(@{field1}) -ismissing(@{field2})",
            "NOCONTENT",
            "WITHCOUNT",
            "SORTBY",
            "id",
        ...
        res = conn.execute_command(
            "FT.SEARCH",
            idx,
            f"-ismissing(@{field2}) ismissing(@{field1})",
            "NOCONTENT",
            "WITHCOUNT",
            "SORTBY",
            "id",
...
def JSONMissingTest(env, conn):
    ...
    def _populateJSONDB(conn, ftype, field1, field2, val1, val2):
        j_with_only_field1 = {field1: val1, "id": 1}
        conn.execute_command(
            "JSON.SET", DOC_WITH_ONLY_FIELD1, "$", json.dumps(j_with_only_field1)
        )
        j_with_only_field2 = {field2: val2, "id": 2}
        conn.execute_command(
            "JSON.SET", DOC_WITH_ONLY_FIELD2, "$", json.dumps(j_with_only_field2)
        )
        j_with_both = {field1: val1, field2: val2, "id": 3}
        ...
        j_with_both_and_text = {field1: val1, field2: val2, "text": "dummy", "id": 5}
        conn.execute_command(
            "JSON.SET", DOC_WITH_BOTH_AND_TEXT, "$", json.dumps(j_with_both_and_text)
        )
    ...
    for field, ftype, opt, val1, val2, field1Opt in fields_and_values:
        ...
        MissingTestIndex(
            env, conn, jidx, ftype, field1, field2, val1, val2, field1Opt, True
        )
        env.flush()
        ...
        MissingTestTwoMissingFields(
            env, conn, idx, ftype, field1, field2, val1, val2, True
        )
        env.flush()
...
def HashMissingTest(env, conn):
    ...
    def _populateHashDB(conn, ftype, field1, field2, val1, val2):
        ...
        # Populate db
        conn.execute_command("HSET", DOC_WITH_ONLY_FIELD1, field1, val1, "id", 1)
        conn.execute_command("HSET", DOC_WITH_ONLY_FIELD2, field2, val2, "id", 2)
        conn.execute_command("HSET", DOC_WITH_BOTH, field1, val1, field2, val2, "id", 3)
        conn.execute_command("HSET", DOC_WITH_NONE, "text", "dummy", "id", 4)
        conn.execute_command(
            "HSET",
            DOC_WITH_BOTH_AND_TEXT,
            field1,
    ...
    for field, ftype, opt, val1, val2, field1Opt in fields_and_values:
        ...
        # Perform the tests using two missing fields of the same type
        MissingTestTwoMissingFields(
            env, conn, idx, ftype, field1, field2, val1, val2, False
        )
        env.flush()
...
def testMissingWithExists():
    ...
    exists = env.cmd(
        "FT.AGGREGATE", "idx", "*", "LOAD", "2", "foo", "goo", "FILTER", "!EXISTS(@foo)"
    )
...
def testFilterOnMissingValues():
    ...
    # Search for the documents with the indexed fields (sanity)
    env.expect("FT.SEARCH", "idx", "@foo:{val}", "FILTER", "goo", "0", "10").equal(
        [1, "doc2", ["foo", "val", "goo", "3"]]
    )
