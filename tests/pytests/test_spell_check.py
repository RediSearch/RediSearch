...
def test_spell_check_with_params(env:Env):
    """Test FT.SPELLCHECK with parameter placeholders (MOD-10596).
    Verifies that using $placeholder syntax in dialect 2/3 returns an error
    instead of crashing the server."""
    env.cmd('ft.create', 'idx', 'ON', 'HASH', 'SCHEMA', 'name', 'TEXT', 'body', 'TEXT')
    with env.getClusterConnectionIfNeeded() as r:
        r.execute_command('hset', 'doc1', 'name', 'name1', 'body', 'body1')

    # Dialect 2: $placeholder should error, not crash
    env.expect('ft.spellcheck', 'idx', '$a', 'DIALECT', '2').error().contains('Parameter not found `a`')

    # Dialect 3: the exact scenario from MOD-10596 that caused the crash
    env.expect('ft.spellcheck', 'idx', '$a', 'DIALECT', '3').error().contains('Parameter not found `a`')

    # Fuzzy with $placeholder should also error
    env.expect('ft.spellcheck', 'idx', '%$tok%', 'DIALECT', '2').error().contains('Parameter not found `tok`')
