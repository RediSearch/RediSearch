...
def testBasicSpellCheckWithNoResult(env):
    ...
    with env.getClusterConnectionIfNeeded() as r:
        r.execute_command(
            "ft.add", "idx", "doc1", 1.0, "FIELDS", "name", "name1", "body", "body1"
        )
        r.execute_command(
            "ft.add", "idx", "doc2", 1.0, "FIELDS", "name", "name2", "body", "body2"
        )
        r.execute_command(
            "ft.add", "idx", "doc3", 1.0, "FIELDS", "name", "name2", "body", "name2"
        )
    waitForIndex(env, "idx")
    env.expect("ft.spellcheck", "idx", "somenotexiststext").equal(
        [["TERM", "somenotexiststext", []]]
    )
...
def testSpellCheckResultsOrder(env):
    ...
    with env.getClusterConnectionIfNeeded() as r:
        r.execute_command(
            "ft.add", "idx", "doc1", 1.0, "FIELDS", "name", "Elior", "body", "body1"
        )
        r.execute_command(
            "ft.add", "idx", "doc2", 1.0, "FIELDS", "name", "Hila", "body", "body2"
        )
    waitForIndex(env, "idx")
    exp = [["TERM", "elioh", [["0.5", "elior"]]], ["TERM", "hilh", [["0.5", "hila"]]]]
