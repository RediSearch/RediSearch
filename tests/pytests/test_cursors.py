...
def loadDocs(env, count=100, idx="idx", text="hello world"):
    env.expect(
        "FT.CREATE", idx, "ON", "HASH", "prefix", 1, idx, "SCHEMA", "f1", "TEXT"
    ).ok()
...
def testCursorDifferentConnections(env: Env):
    ...
    _, cursor = con2.execute_command(
        "FT.AGGREGATE", "idx", "*", "WITHCURSOR", "COUNT", 3
    )
...
def testIndexDropWhileIdle(env: Env):
    ...
    while not np.all(
        [
            env.getConnection(i).execute_command("DBSIZE") > 1
    ...
    if env.isCluster():
        res, cursor = env.cmd(
            f"FT.CURSOR READ idx {cursor} COUNT 1"
        )  # read the last result
        env.assertEqual(res[1:], [[]], message=f"res == {res}")
    else:
        env.expect(f"FT.CURSOR READ idx {cursor}").error().contains(
...
def exceedCursorCapacity(env):
    ...
    env.expect("FT.AGGREGATE", "idx", "*", "WITHCURSOR", "COUNT", 1).error().contains(
        "Too many cursors allocated for index"
    )
...
def CursorOnCoordinator(env: Env):
    ...
    res, cursor = env.cmd(
        "FT.AGGREGATE", "idx", "*", "LOAD", "*", "WITHCURSOR", "COUNT", 1
    )
    env.assertEqual(res, [1, ["n", "0"]])
    env.expect(f"FT.CURSOR READ idx {cursor}").equal(
        [[0], 0]
    )  # empty reply from shard - 0 results and depleted cursor
    ...
    for threshold in configs:
        ...
        _, cursor = env.cmd(
            "FT.AGGREGATE", "idx", "*", "LOAD", "*", "WITHCURSOR", "COUNT", count
        )
        ...
        with env.getConnection().monitor() as monitor:
            ...
            for _ in range((env.shardsCount - threshold) * 10):
                cmd = next_command()
                env.assertTrue(
                    cmd.startswith(exp), message=f"expected `{exp}` but got `{cmd}`"
                )
            ...
            env.assertTrue(
                found,
                message=f"`_FT.CURSOR READ` was not observed within {expected_reads + 1} commands",
            )
            if found:
                env.debugPrint(
                    f"Found `_FT.CURSOR READ` in the {number_to_ordinal(i)} try"
                )
...
def testCursorDepletionNonStrictTimeoutPolicy(env):
    ...
    # it until depleted
    res, cursor = env.cmd(
        "FT.AGGREGATE", "idx", "*", "WITHCURSOR", "COUNT", "10000", "TIMEOUT", "1"
    )
...
def test_mod_6597(env):
    ...
    env.expect("FT.CREATE", "idx", "ON", "HASH", "SCHEMA", "test", "NUMERIC").equal(
        "OK"
    )
...
def testCountArgValidation(env):
    ...
    env.expect(
        ...
        "Bad arguments for COUNT: Could not convert argument to expected type"
    )
    ...
    env.expect("FT.CURSOR", "READ", "idx", str(cid), "LOVE", "3").error().contains(
        "Unknown argument `LOVE`"
    )

    # Query the cursor with bad subcommand
    env.expect("FT.CURSOR", "READS", "idx", str(cid)).error().contains(
        "Unknown subcommand"
    )
    env.expect("FT.CURSOR", "DELS", "idx", str(cid)).error().contains(
        "Unknown subcommand"
    )
    env.expect("FT.CURSOR", "GCS", "idx", str(cid)).error().contains(
        "Unknown subcommand"
    )

    # Query the cursor with a bad value for the `COUNT` argument
    env.expect("FT.CURSOR", "READ", "idx", str(cid), "COUNT", "2.3").error().contains(
        "Bad value for COUNT: `2.3`"
    )
