    ...
    for _ in env.reloadingIterator():
        ...
        for n in range(N - 1):
            ...
            res = env.cmd(
                "ft.search",
                "idx",
                "hello world @tags:{tag\\ %d|tag %d}" % (n, n + 1),
                "nocontent",
            )
...
def testSeparator(env):
    ...
    env.expect(
        ...
        "hello world",
        "tags",
        "x:hello world: fooz bar:foo,bar:BOO FAR",
    ).ok()
    for _ in env.reloadingIterator():
        waitForIndex(env, "idx")
        for q in (
...
def testTagPrefix(env):
    ...
    env.expect(
        ...
        "hello world",
        "tags",
        "hello world,hello-world,hell,jell",
    ).ok()
    env.expect(debug_cmd(), "dump_tagidx", "idx", "tags").equal(
        [["hell", [1]], ["hello world", [1]], ["hello-world", [1]], ["jell", [1]]]
    )
...
def testTagFieldCase(env):
    ...
    for _ in env.reloadingIterator():
        waitForIndex(env, "idx")
        env.assertEqual([0], env.cmd("FT.SEARCH", "idx", "@tags:{HELLO WORLD}"))
        env.assertEqual(
            [1, "doc1"], env.cmd("FT.SEARCH", "idx", "@TAgs:{HELLO WORLD}", "NOCONTENT")
        )
        env.assertEqual(
            [1, "doc1"], env.cmd("FT.SEARCH", "idx", "@TAgs:{foo bar}", "NOCONTENT")
        )
        env.assertEqual(
            [0], env.cmd("FT.SEARCH", "idx", "@TAGS:{foo bar}", "NOCONTENT")
        )
...
def testTagCaseSensitive(env):
    ...
    if not env.isCluster():
        ...
        env.expect(debug_cmd(), "dump_tagidx", "idx2", "t").equal(
            [["FOO", [1, 2]], ["foo", [1, 3]]]
        )
        env.expect(debug_cmd(), "dump_tagidx", "idx3", "t").equal(
            [["foo", [2, 3]], ["foo,foo", [1]]]
        )
        env.expect(debug_cmd(), "dump_tagidx", "idx4", "t").equal(
            [["FOO", [2]], ["foo", [3]], ["foo,FOO", [1]]]
        )
        env.expect(debug_cmd(), "dump_tagidx", "idx5", "t").equal(
            [["FOO", [2]], ["foo", [3]], ["foo,FOO", [1]]]
        )

    env.expect("FT.SEARCH", "idx1", "@t:{FOO}").equal(
        [3, "doc1", ["t", "foo,FOO"], "doc2", ["t", "FOO"], "doc3", ["t", "foo"]]
    )
    env.expect("FT.SEARCH", "idx1", "@t:{foo}").equal(
        [3, "doc1", ["t", "foo,FOO"], "doc2", ["t", "FOO"], "doc3", ["t", "foo"]]
    )

    env.expect("FT.SEARCH", "idx2", "@t:{FOO}").equal(
        [2, "doc1", ["t", "foo,FOO"], "doc2", ["t", "FOO"]]
    )
    env.expect("FT.SEARCH", "idx2", "@t:{foo}").equal(
        [2, "doc1", ["t", "foo,FOO"], "doc3", ["t", "foo"]]
    )
    ...
    # not casesensitive
    env.expect("FT.SEARCH", "idx1", "@t:{F\\ O}").equal(
        [3, "doc1", ["t", "f o,F O"], "doc2", ["t", "F O"], "doc3", ["t", "f o"]]
    )
    env.expect("FT.SEARCH", "idx1", "@t:{f\\ o}").equal(
        [3, "doc1", ["t", "f o,F O"], "doc2", ["t", "F O"], "doc3", ["t", "f o"]]
    )
    ...
    # casesensitive
    env.expect("FT.SEARCH", "idx2", "@t:{F\\ O}").equal(
        [2, "doc1", ["t", "f o,F O"], "doc2", ["t", "F O"]]
    )
    env.expect("FT.SEARCH", "idx2", "@t:{f\\ o}").equal(
        [2, "doc1", ["t", "f o,F O"], "doc3", ["t", "f o"]]
    )

    # not casesensitive
    env.expect("FT.SEARCH", "idx3", "@t:{f\\ o\\,f\\ o}").equal(
        [1, "doc1", ["t", "f o,F O"]]
    )
    env.expect("FT.SEARCH", "idx3", "@t:{f\\ o\\,F\\ O}").equal(
        [1, "doc1", ["t", "f o,F O"]]
    ...
    env.expect("FT.SEARCH", "idx3", "@t:{F\\ O}").equal(
        [2, "doc2", ["t", "F O"], "doc3", ["t", "f o"]]
    )
    env.expect("FT.SEARCH", "idx3", "@t:{f\\ o}").equal(
        [2, "doc2", ["t", "F O"], "doc3", ["t", "f o"]]
    )
    ...
    # casesensitive
    env.expect("FT.SEARCH", "idx4", "@t:{f\\ o\\,f\\ o}").equal([0])
    env.expect("FT.SEARCH", "idx4", "@t:{f\\ o\\,F\\ O}").equal(
        [1, "doc1", ["t", "f o,F O"]]
    )
    env.expect("FT.SEARCH", "idx4", "@t:{F\\ O\\,F\\ O}").equal([0])
    env.expect("FT.SEARCH", "idx4", "@t:{F\\ O}").equal([1, "doc2", ["t", "F O"]])
    env.expect("FT.SEARCH", "idx4", "@t:{f\\ o}").equal([1, "doc3", ["t", "f o"]])
...
def testTagGCClearEmpty(env):
    ...
    env.expect("FT.SEARCH", "idx", "@t:{foo}").equal(
        [2, "doc4", ["t", "foo"], "doc5", ["t", "foo"]]
    )
...
def test_empty_suffix_withsuffixtrie(env):
    ...
    conn = getConnectionByEnv(env)
    env.expect(
        "FT.CREATE", "idx_suffixtrie", "SCHEMA", "t", "TAG", "WITHSUFFIXTRIE"
    ).ok()
...
def testDialect2TagExact():
    ...
    # Test exact match
    res = env.cmd(
        "FT.SEARCH", "idx", '@tag:{"abc:1"}', "NOCONTENT", "SORTBY", "id", "ASC"
    )
    ...
    res = env.cmd(
        "FT.SEARCH",
        "idx",
        '@tag:{"abc:1-xyz:2"} | @tag:{"joe@mail.com"}',
        "NOCONTENT",
        "SORTBY",
        "id",
        "ASC",
    ...
    res = env.cmd(
        "FT.SEARCH", "idx", '@tag:{"xyz:2" | hello}', "NOCONTENT", "SORTBY", "id", "ASC"
    )
    env.assertEqual(res, expected)
    res = env.cmd(
        "FT.SEARCH", "idx", '@tag:{hello | "xyz:2"}', "NOCONTENT", "SORTBY", "id", "ASC"
    )
    env.assertEqual(res, expected)
    ...
    res = env.cmd("FT.SEARCH", "idx", '@tag:{*"@mail."*}=>{$weight:3.4}', "NOCONTENT")
    ...
    res = env.cmd(
        "FT.EXPLAIN", "idx", "@tag1:{w'$param'}", "PARAMS", "2", "param", "hello world"
    )
    ...
    res = env.cmd("FT.SEARCH", "idx", "@tag:{w'?'}", "SORTBY", "id", "ASC", "NOCONTENT")
    ...
    res = env.cmd(
        "FT.SEARCH", "idx", "@tag:{$param}", "PARAMS", "2", "param", "with: space"
    )
    env.assertEqual(res, expected_result)
    ...
    res = env.cmd(
        "FT.SEARCH", "idx", "@tag:{$param}", "PARAMS", "2", "param", "leading:space"
    )
    env.assertEqual(res, expected_result)
...
def testDialect2SpecialChars():
    ...
    for c in punct:
        conn.execute_command(
            "HSET", f"doc{ord(chr(c))}", "text", f"single\\{chr(c)}term"
        )
    ...
    for c in punct:
        ...
        res = env.execute_command(
            "FT.SEARCH",
            "idx",
            f"@text:(single\\{chr(c)}term single* *term)",
            "NOCONTENT",
        )
        env.assertEqual(res, expected)
        ...
        res = conn.execute_command(
            "FT.SEARCH",
            "idx",
            f"@text:(single\\{chr(c)}term | hanoi)",
            "NOCONTENT",
            "SORTBY",
            "text",
            "ASC",
    ...
    for c in punct:
        ...
        res = env.execute_command("FT.SEARCH", "idx", f"@text:(two{char}words)")
        env.assertEqual(res, expected)

        res = env.execute_command("FT.EXPLAINCLI", "idx", f"@text:(two{char}words)")
        env.assertEqual(res, expected_explain)

    # Test control characters
    for cntrl in range(1, 32):
        res = env.execute_command("FT.SEARCH", "idx", f"@text:(two{chr(cntrl)}words)")
        env.assertEqual(res, expected)
...
def testTagUNF():
    ...
    res = env.cmd(
        "FT.AGGREGATE", "idx", "*", "GROUPBY", "1", "@tag", "REDUCE", "COUNT", "0"
    )
    env.assertEqual(res[0], 1)
    ...
    res = env.cmd(
        "FT.AGGREGATE", "idx_unf", "*", "GROUPBY", "1", "@tag", "REDUCE", "COUNT", "0"
    )
