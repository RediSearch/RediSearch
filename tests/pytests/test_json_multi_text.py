...
def testMultiText(env):
    ...
    conn.execute_command(
        "JSON.SET", "doc:1", "$", json.dumps(json.loads(doc1_content)["category"])
    )
    conn.execute_command(
        "JSON.SET", "doc:2", "$", json.dumps(json.loads(doc2_content)["category"])
    )
    conn.execute_command(
        "JSON.SET", "doc:3", "$", json.dumps(json.loads(doc3_content)["category"])
    )
    conn.execute_command(
        "JSON.SET", "doc:4", "$", json.dumps(json.loads(doc4_content)["category"])
    )
    ...
    env.cmd(
        ...
        "AS",
        "author",
        "TEXT",  # testing root path, so reuse the single top-level value
        "$",
        "AS",
        "category",
        "TEXT",
...
def testMultiTextNested(env):
    ...
    env.assertEqual(
        ...
        toSortedFlatList([2, "doc:3", "doc:1"]),
        message="idx_book",
    )
...
def searchMultiTextCategory(env):
    ...
    for idx in ["idx_category_arr", "idx_category_arr_author_flat"]:
        ...
        cond.call(
            ...
            "SLOP",
            "98",
        ).expect_when(True, expect_0).expect_when(False, expect_undef_order)
        cond.call(
            "FT.SEARCH",
            idx,
            "@category:(database programming)",
            ...
            "SLOP",
            "99",
        ).expect_when(True, expect_1).expect_when(False, expect_undef_order)
        cond.call(
            "FT.SEARCH",
            idx,
            "@category:(database programming)",
        ...
        cond.call(
            "FT.SEARCH",
            idx,
            "@category:(database programming)=>{$slop:99}",
            "NOCONTENT",
            "SLOP",
            "1",
        ).expect_when(True, expect_1).expect_when(False, expect_undef_order)
        cond.call(
            "FT.SEARCH",
            idx,
            "@category:(database programming)=>{$slop:100; $inorder:true}",
            "NOCONTENT",
        ).expect_when(True, expect_0).expect_when(False, expect_undef_order)
        cond.call(
            "FT.SEARCH",
            idx,
            "@category:(database programming)=>{$slop:100; $inorder:false}",
            "NOCONTENT",
        ).expect_when(True, expect_1).expect_when(False, expect_undef_order)
        ...
        env.assertEqual(
            toSortedFlatList(res),
            toSortedFlatList([2, "doc:2", "doc:3"]),
            message="B " + idx,
        )

        env.expect("FT.SEARCH", idx, "@category:(high performance)", "NOCONTENT").equal(
            [1, "doc:2"]
        )
        env.expect("FT.SEARCH", idx, "@category:(cloud)", "NOCONTENT").equal(
            [1, "doc:3"]
        )

    # Multi-value attributes which have no definite ordering cannot use slop or inorder
    env.expect(
        "FT.SEARCH", "idx_category_flat", "@category:(programming science)=>{$slop:200}"
...
def searchMultiTextAuthor(env):
    ...
    for idx in ["idx_author_flat"]:
        env.expect("FT.SEARCH", idx, "@author:(Richard)", "NOCONTENT").equal(
            [1, "doc:1"]
        )
        ...
        env.assertEqual(
            toSortedFlatList(res), toSortedFlatList([3, "doc:1", "doc:2", "doc:3"])
        )

    # None-exact phrase using multi-value attributes which have no definite ordering cannot use slop or inorder
    env.expect(
        "FT.SEARCH", "idx_author_flat", "@author:(Redis Ltd.)=>{$slop:200}"
    ).error().contains("has undefined ordering")
    env.expect(
        "FT.SEARCH", "idx_author_flat", "@author:(Redis Ltd.)=>{$inorder:true}"
    ).error().contains("has undefined ordering")

    env.expect(
        "FT.SEARCH", "idx_author_flat", "@category|author:(Redis Ltd.)=>{$slop:200}"
    ).error().contains("has undefined ordering")
    env.expect(
        "FT.SEARCH", "idx_author_flat", "@category|author:(Redis Ltd.)=>{$inorder:true}"
    ).error().contains("has undefined ordering")

    env.expect(
        "FT.SEARCH",
        "idx_author_flat",
        '@category|author:("Redis Ltd.")=>{$inorder:true}',
    ).error().contains("has undefined ordering")

    env.expect(
        "FT.SEARCH", "idx_category_arr_author_flat", "@author:(Redis Ltd.)=>{$slop:200}"
    ).error().contains("has undefined ordering")
    env.expect(
        "FT.SEARCH",
        "idx_category_arr_author_flat",
        "@author:(Redis Ltd.)=>{$inorder:true}",
    ).error().contains("has undefined ordering")
...
def testUndefinedOrderingWithSlopAndInorder(env):
    ...
    cond.call(
        "FT.SEARCH",
        "idx_category_arr_author_flat",
        "@category:(does not matter)=>{$slop:200}",
    ).expect_when(True, lambda q: q.equal([0])).expect_when(False, expect_undef_order)
    cond.call(
        "FT.SEARCH",
        "idx_category_arr_author_flat",
        "@category:(does not matter)=>{$inorder:false}",
    ).expect_when(True, lambda q: q.equal([0])).expect_when(False, expect_undef_order)
    cond.call(
        "FT.SEARCH",
        "idx_category_arr_author_flat",
        "@category:(does not matter)=>{$inorder:true}",
    ).expect_when(True, lambda q: q.equal([0])).expect_when(False, expect_undef_order)

    env.expect(
        "FT.SEARCH",
        "idx_category_arr_author_flat",
        "@author:(does not matter)=>{$slop:200}",
    ).error().contains("has undefined ordering")
    env.expect(
        "FT.SEARCH",
        "idx_category_arr_author_flat",
        "@author:(does not matter)=>{$inorder:false}",
    ).error().contains("has undefined ordering")
    env.expect(
        "FT.SEARCH",
        "idx_category_arr_author_flat",
        "@author:(does not matter)=>{$inorder:true}",
    ).error().contains("has undefined ordering")
    ...
    env.expect(
        "FT.SEARCH", "idx_category_arr_author_flat", "(does not matter)", "SLOP", "200"
    ).error().contains("has undefined ordering")
    env.expect(
        "FT.SEARCH", "idx_category_arr_author_flat", "(does not matter)", "INORDER"
    ).error().contains("has undefined ordering")
    ...
    env.cmd(
        "FT.CREATE",
        "idx_category_arr_author_flat_2",
        "ON",
        "JSON",
        "NOOFFSETS",
        "SCHEMA",
...
def testMultiNonText(env):
    ...
    for i, v in enumerate(non_text_dict.values()):
        ...
        env.assertEqual(
            int(index_info(env, idx)["hash_indexing_failures"]),
            res_failures,
            message=str(i),
        )
...
def testMultiNonTextNested(env):
    ...
    for i, v in enumerate(non_text_dict.values()):
        res_failures = 0 if i + 1 <= 5 else 1
        env.assertEqual(
            int(index_info(env, "idx{}".format(i + 1))["hash_indexing_failures"]),
            res_failures,
        )
...
def testMultiSortRoot(env):
    ...
    for i, gag in enumerate(gag_arr):
        conn.execute_command(
            "JSON.SET", "multi:doc:{}".format(i + 1), "$", json.dumps(gag)
        )

    # docs with a single string
    for i, gag in enumerate(gag_arr):
        conn.execute_command(
            "JSON.SET", "single:doc:{}".format(i + 1), "$", json.dumps(gag[0])
        )
...
def testMultiSortNested(env):
    ...
    for i, gag in enumerate(gag_arr):
        conn.execute_command(
            "JSON.SET",
            "multi:doc:{}".format(i + 1),
            "$",
            json.dumps({"chalkboard": gag}),
        )
    ...
    for i, gag in enumerate(gag_arr):
        conn.execute_command(
            "JSON.SET",
            "single:doc:{}".format(i + 1),
            "$",
            json.dumps({"chalkboard": gag[0]}),
        )
...
def sortMulti(env, text_cmd_args, tag_cmd_args):
    ...
    for i, (text_arg, tag_arg) in enumerate(zip(text_cmd_args, tag_cmd_args)):
        ...
        env.assertEqual(
            trim_in_list("multi:", env.cmd("FT.SEARCH", "idx1_multi_text", *text_arg)),
            trim_in_list(
                "single:", env.cmd("FT.SEARCH", "idx1_single_text", *text_arg)
            ),
            message="{} with arg `{}`".format("multi TEXT with single TEXT", text_arg),
        )
        # Multi TAG with single TAG
        env.assertEqual(
            trim_in_list("multi:", env.cmd("FT.SEARCH", "idx2_multi_tag", *tag_arg)),
        ...
        env.assertEqual(
            env.cmd("FT.SEARCH", "idx1_multi_text", *text_arg),
            env.cmd("FT.SEARCH", "idx2_multi_tag", *tag_arg),
            message="{} text arg `{}` tag arg `{}`".format(
                "multi TEXT with multi TAG", text_arg, tag_arg
            ),
        )
    ...
    if not env.isCluster():
        ...
        for i, text_arg in enumerate(text_cmd_args):
            ...
            env.assertEqual(
                trim_in_list(
                    "multi:", env.cmd("FT.SEARCH", "idx1_multi_text", *text_arg)
                ),
                trim_in_list(
                    "single:", env.cmd("FT.SEARCH", "idx1_single_text", *text_arg)
                ),
...
def testMultiEmptyBlankOrNone(env):
    ...
    for i, val in enumerate(values):
        conn.execute_command(
            "JSON.SET", "doc:{}".format(i + 1), "$", json.dumps({"val": val})
        )
    conn.execute_command("JSON.SET", "doc", "$", json.dumps({"val": ["haha"]}))
    env.expect(
        "FT.SEARCH", "idx", "@val:(haha)", "NOCONTENT", "SORTBY", "val", "ASC"
    ).equal([1, "doc"])
...
def testconfigMultiTextOffsetDelta(env):
    ...
    env.expect(config_cmd(), "SET", "MULTI_TEXT_SLOP", "101").error().contains(
    ...
    env.expect(
        "FT.SEARCH", "idx_category_arr", "@category:(mathematics database)", "NOCONTENT"
    ).equal([1, "doc:1"])
...
def checkMultiTextReturn(env, expected, default_dialect, is_sortable, is_sortable_unf):
    ...
    sortable_param = (
        ["SORTABLE", "UNF"]
        if is_sortable_unf
        else (["SORTABLE"] if is_sortable else [])
    )
    message = "dialect {}, sortable {}, unf {}".format(
        "default" if default_dialect else 3, is_sortable, is_sortable_unf
    )
    ...
    doc1_content = {
        "Name": "Product1",
        "Sellers": [
            {"Name": "Seller1", "Locations": ["FL", "AL"]},
            {"Name": "Seller2", "Locations": ["MS", "GA"]},
        ],
    }
    ...
    def expect_case(val):
        return (
            val.lower()
            if (is_sortable and not is_sortable_unf) and isinstance(val, str)
            else val
        )
    ...
    env.assertEqual(
        conn.execute_command(
            ...
            "RETURN",
            "3",
            "$.Sellers[0].Locations[1]",
            "AS",
            "arr_1",
            *dialect_param
        ),
        expect_case(expected[0]),
        message=message,
    )
    env.assertEqual(
        conn.execute_command(
    ...
    env.assertEqual(
        ...
        expect_case(expected[2]),
        message=message,
    )

    # Currently not considering `UNF` with multi value (MOD-4345)
    res = conn.execute_command(
        "FT.AGGREGATE", "idx_flat", expr, "LOAD", "1", "@val", *dialect_param
    )
    env.assertEqual(res[1][1].lower(), expected[3][2][1].lower(), message=message)

    res = conn.execute_command(
        "FT.AGGREGATE", "idx_flat", expr, "GROUPBY", "1", "@val", *dialect_param
    )
    env.assertEqual(res[1][1].lower(), expected[3][2][1].lower(), message=message)
    ...
    env.assertEqual(
        conn.execute_command(
            "FT.SEARCH", "idx_arr", expr, "RETURN", "1", "val", *dialect_param
        ),
        expect_case(expected[2]),
        message=message,
    )
    ...
    res = conn.execute_command(
        "FT.AGGREGATE", "idx_arr", expr, "GROUPBY", "1", "@val", *dialect_param
    )
    ...
    res = conn.execute_command(
        "FT.AGGREGATE", "idx_arr", expr, "LOAD", "1", "@val", *dialect_param
    )
    env.assertEqual(res[1][1].lower(), expected[2][2][1].lower())
