...
def test_worker_threads_sanity():
    ...
    for it in range(2):
        ...
        env.assertEqual(
            getWorkersThpoolStats(env)["totalPendingJobs"],
            n_vectors,
            message=f"iteration {it+1}",
        )
        env.assertEqual(
            getWorkersThpoolStats(env)["totalJobsDone"],
            0 if it == 0 else 2 * n_vectors,
            message=f"iteration {it+1}",
        )
        ...
        for i in env.reloadingIterator():
            ...
            env.assertEqual(
                ...
                (
                    (0 if it == 0 else 2 * n_vectors)
                    if i == 1
                    else (2 * n_vectors if it == 0 else 5 * n_vectors)
                ),
                message=f"iteration {it+1} {'before' if i==1 else 'after'} loading",
            )
            ...
            if i == 2:
                env.assertEqual(
                    debug_info["BACKGROUND_INDEXING"],
                    0,
                    message=f"iteration {it+1} after reloading",
                )
...
def do_burst_threads_sanity(algo, data_type, test_name):
    ...
    additional_params = (
        ["EF_CONSTRUCTION", n_vectors, "EF_RUNTIME", n_vectors]
        if algo == "HNSW"
        else []
    )
    ...
    for i in env.reloadingIterator():
        ...
        if algo == "HNSW":
            ...
            if (
                i == 2
            ):  # after reloading in HNSW, we expect to run insert job for each vector
        ...
        if algo == "HNSW":
            ...
            env.assertEqual(
                getWorkersThpoolStats(env)["totalJobsDone"], expected_total_jobs
            )
...
def test_workers_priority_queue():
    ...
    while debug_info["BACKGROUND_INDEXING"] == 1:
        ...
        res = env.cmd(
            "FT.SEARCH",
            "idx",
            f"*=>[KNN $K @vector $vec_param EF_RUNTIME {n_vectors}]",
            "SORTBY",
            "__vector_score",
            "RETURN",
            1,
...
def test_buffer_limit():
    buffer_limit = 100
    env = initEnv(
        moduleArgs=f"WORKERS 2 DEFAULT_DIALECT 2 TIERED_HNSW_BUFFER_LIMIT {buffer_limit}"
    )
    n_shards = env.shardsCount
    dim = 4
    ...
    env.assertEqual(
        to_dict(debug_info["BACKEND_INDEX"])["INDEX_SIZE"],
        n_local_vectors - buffer_limit,
    )
...
def test_async_updates_sanity():
    env = initEnv(
        moduleArgs="WORKERS 2 DEFAULT_DIALECT 2 TIERED_HNSW_BUFFER_LIMIT 10000"
    )
    ...
    n_local_vectors_before_update = get_vecsim_debug_dict(env, "idx", "vector")[
        "INDEX_LABEL_COUNT"
    ]
    assertInfoField(env, "idx", "num_docs", n_vectors)
    res = env.cmd(
    ...
    env.assertEqual(
        sum([to_dict(shard_stat)["totalPendingJobs"] for shard_stat in stats]), 0
    )  # 0 in each shard
    total_jobs_done = sum(
        [to_dict(shard_stat)["totalJobsDone"] for shard_stat in stats]
    )

    env.assertEqual(
        total_jobs_done, n_vectors + n_shards
    )  # job per vector + one job for the query.
    debug_info = get_vecsim_debug_dict(env, "idx", "vector")
    env.assertEqual(
        to_dict(debug_info["BACKEND_INDEX"])["INDEX_SIZE"],
        n_local_vectors_before_update,
    )
    ...
    query_vec = load_vectors_to_redis(
        env, n_vectors, 0, dim, ids_offset=0, seed=11
    )  # new seed to generate new vectors
    assertInfoField(env, "idx", "num_docs", n_vectors)
    debug_info = get_vecsim_debug_dict(env, "idx", "vector")
    local_marked_deleted_vectors = to_dict(debug_info["BACKEND_INDEX"])[
        "NUMBER_OF_MARKED_DELETED"
    ]
    ...
    n_local_vectors = get_vecsim_debug_dict(env, "idx", "vector")["INDEX_LABEL_COUNT"]
    env.assertEqual(
        to_dict(debug_info["FRONTEND_INDEX"])["INDEX_SIZE"], n_local_vectors
    )
    env.assertEqual(
        to_dict(debug_info["BACKEND_INDEX"])["INDEX_SIZE"],
        n_local_vectors_before_update,
    ...
    while local_marked_deleted_vectors > block_size:
        ...
        res = env.cmd(
            "FT.SEARCH",
            "idx",
            f"*=>[KNN $K @vector $vec_param EF_RUNTIME {n_local_vectors}]",
            "SORTBY",
            "__vector_score",
            "RETURN",
            1,
        ...
        local_marked_deleted_vectors_new = to_dict(debug_info["BACKEND_INDEX"])[
            "NUMBER_OF_MARKED_DELETED"
        ]
        env.assertLessEqual(
            local_marked_deleted_vectors_new, local_marked_deleted_vectors
        )
...
def test_multiple_loaders():
    ...
    env.expect(*cmd).noError().apply(lambda x: x[1:]).equal(
        [["n", "0"], ["n", "1"], ["n", "2"], ["n", "3"], ["n", "4"]]
    )
...
def test_switch_loader_modes():
    ...
    read_from_cursor = (
        lambda cursor: env.expect("FT.CURSOR", "READ", "idx", cursor).noError().res[1]
    )
...
def test_change_workers_number():
    ...
    def check_threads(expected_num_threads_alive, expected_n_threads):
        env.assertEqual(
            getWorkersThpoolStats(env)["numThreadsAlive"], expected_num_threads_alive
        )
