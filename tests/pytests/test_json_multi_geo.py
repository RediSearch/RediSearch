...
def testBasic(env):
    ...
    # idx1 contains 24 entries, expected size of inverted index = 464
    ...
    #     Buffer grows up to 128 bytes trying to store 11 entries 8 bytes each.

    # right range size = 232:
    #     Size of NewInvertedIndex() structure = 104
    #         sizeof InvertedIndex = 48
    #         bytes to store number of entries = 8
    #         sizeof IndexBlock = 48
    #     Buffer grows up to 128 bytes trying to store 13 entries 8 bytes each.
    expected_info['inverted_sz_mb'] = 464 / (1024 * 1024)
    compare_index_info_dict(env, 'idx1', expected_info, "idx1 after insert")

    # Expected size of inverted index for idx2 = 104 + 32 = 136
    #     Size of NewInvertedIndex() structure = 104
    #     Buffer grows up to 32 bytes trying to store 3 entries 8 bytes each = 32
    expected_info['inverted_sz_mb'] = 136 / (1024 * 1024)
    compare_index_info_dict(env, 'idx2', expected_info, "idx2 after insert")

    # Expected size of inverted index for idx2 = 104 + 64 = 168
    #     Size of NewInvertedIndex() structure = 104
    #     Buffer grows up to 64 bytes trying to store 5 entries, 8 bytes each = 64
    expected_info['inverted_sz_mb'] = 168 / (1024 * 1024)
    ...
    # equivalent to the sum of the size of idx2 and idx3 = 136 + 168 = 304
    expected_info['inverted_sz_mb'] = 304 / (1024 * 1024)
