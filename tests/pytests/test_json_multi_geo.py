...
def testBasic(env):
    ...
    # idx1 contains 24 entries spread across the numeric range tree. After the
    # Arc<ThinVec> sealed-region change every inverted index carries the Arc
    # refcount header on top of the previous 380 bytes.
    expected_info['inverted_sz_mb'] = 428 / (1024 * 1024)
    compare_index_info_dict(env, 'idx1', expected_info, "idx1 after insert")

    # idx2: one GEO field, 3 entries × 8 bytes encoded into the tail block.
    expected_info['inverted_sz_mb'] = 138 / (1024 * 1024)
    compare_index_info_dict(env, 'idx2', expected_info, "idx2 after insert")

    # idx3: one GEO field, 5 entries × 8 bytes encoded into the tail block.
    expected_info['inverted_sz_mb'] = 159 / (1024 * 1024)
    ...
    # equivalent to the sum of the size of idx2 and idx3 = 138 + 159 = 297
    expected_info['inverted_sz_mb'] = 297 / (1024 * 1024)
