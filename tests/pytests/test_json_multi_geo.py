...
doc1_content = [
    {
        ...
        "loc": ["1,2", "3,4"],
        "nested1": [
            {"name": "top1_1_1", "loc": ["2,3", "40,25"]},
            {"name": "top1_1_2", "loc": ["21.2,21.3", "21.4,21.5", "22,22"]},
            {"name": "top1_1_3", "loc": ["10,20"]},
        ],
        "nested2": [
            {"name": "top1_2_1", "loc": ["1.2,1.3", "1.4,1.5", "2,2"]},
            {"name": "top1_2_2", "loc": ["0,0"]},
            {"name": "top1_2_3", "loc": ["11,11", "12,12"]},
        ],
    },
    {
        "name": "top2",
        ...
        "nested1": [
            {"name": "top2_1_1", "loc": []},
            {"name": "top2_1_2", "loc": ["1,2"]},
            {"name": "top2_1_3", "loc": []},
        ],
        "nested2": [
            {"name": "top2_2_1", "loc": ["-1.5,-1.6", "-2,2"]},
            {"name": "top2_2_2", "loc": ["3.1415,2.7182"]},
            {
                "name": "top2_2_3",
                "loc": [
                    "42,64",
...
def testBasic(env):
    ...
    env.expect(
        "FT.CREATE", "idx1", "ON", "JSON", "SCHEMA", "$..loc[*]", "AS", "loc", "GEO"
    ).ok()
    env.expect(
        "FT.CREATE",
        "idx2",
    ...
    env.expect(
        ...
        "loc",
        "GEO",
    ).ok()  # ["42,64", "-50,-72", "-100,-20", "43.422649,11.126973", "29.497825,-82.141870"]
    ...
    # Geo range and Not
    env.expect("FT.SEARCH", "idx1", "@loc:[1.2 1.1 40 km]", "NOCONTENT").equal(
        [1, "doc:1"]
    )
    env.expect("FT.SEARCH", "idx1", "-@loc:[1.2 1.1 40 km]", "NOCONTENT").equal([0])

    env.expect("FT.SEARCH", "idx1", "@loc:[0 0 +inf km]", "NOCONTENT").equal(
        [1, "doc:1"]
    )
    env.expect("FT.SEARCH", "idx1", "-@loc:[0 0 +inf km]", "NOCONTENT").equal([0])

    env.expect("FT.SEARCH", "idx2", "@loc:[1.42 1.52 5 km]", "NOCONTENT").equal(
        [1, "doc:1"]
    )
    env.expect("FT.SEARCH", "idx2", "-@loc:[1.42 1.52 5 km]", "NOCONTENT").equal([0])

    env.expect("FT.SEARCH", "idx3", "@loc:[42 64 200 km]", "NOCONTENT").equal(
        [1, "doc:1"]
    )
    ...
    # Intersect
    env.expect(
        "FT.SEARCH", "idx4", "@loc1:[1.2 1.1 40 km] @loc2:[29.5 -82 20 km]", "NOCONTENT"
    ).equal([1, "doc:1"])
    env.expect(
        "FT.SEARCH", "idx4", "@loc1:[1.2 1.1 40 km] @loc2:[50 50 1 km]", "NOCONTENT"
    ).equal([0])
    env.expect(
        "FT.SEARCH",
        "idx4",
        "@loc1:[-10 -10 1 km]     @loc2:[29.5 -82 20 km]",
        "NOCONTENT",
    ).equal([0])
...
def testMultiNonGeo(env):
    ...
    env.expect("FT.SEARCH", "idx1", "@root:[29.72 34.96 1 km]", "NOCONTENT").equal(
        [1, "doc:1:"]
    )
    env.expect("FT.SEARCH", "idx2", "@root:[29.72 34.96 1 km]", "NOCONTENT").equal(
        [1, "doc:2:"]
    )
...
def testDebugDump(env):
    ...
    conn = getConnectionByEnv(env)
    env.expect(
        "FT.CREATE", "idx:top", "ON", "JSON", "SCHEMA", "$[*]", "AS", "val", "GEO"
    ).ok()
    env.expect(
        "JSON.SET", "doc:1", "$", json.dumps(["21.2,21.3", "21.4,21.5", "22,22"])
    ).ok()
...
def checkMultiGeoReturn(env, expected, default_dialect, is_sortable):
    ...
    env.assertEqual(
        len(expected),
        3,
        message="dialect {}, sortable {}".format(dialect_param, is_sortable),
    )
