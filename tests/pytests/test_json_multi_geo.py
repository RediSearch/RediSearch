...
def testBasic(env):
    ...
    # idx1 contains 24 entries, expected size of inverted index = 413
    ...
    # right range size = 169:
    #     Size of NewInvertedIndex() structure = 96
    #         sizeof InvertedIndex = 40
    #         bytes to store number of entries = 8
    #         sizeof IndexBlock = 48
    #     Buffer grows up to 73 bytes trying to store 13 entries 8 bytes each.
    expected_info['inverted_sz_mb'] = 365 / (1024 * 1024)
    compare_index_info_dict(env, 'idx1', expected_info, "idx1 after insert")

    # Expected size of inverted index for idx2 = 48 + 26 = 74
    #     Size of NewInvertedIndex() structure = 48
    #     Buffer grows up to 26 bytes trying to store 3 entries 8 bytes each = 26
    expected_info['inverted_sz_mb'] = 74 / (1024 * 1024)
    compare_index_info_dict(env, 'idx2', expected_info, "idx2 after insert")

    # Expected size of inverted index for idx2 = 48 + 47 = 95
    #     Size of NewInvertedIndex() structure = 48
    #     Buffer grows up to 47 bytes trying to store 5 entries, 8 bytes each = 47
    expected_info['inverted_sz_mb'] = 95 / (1024 * 1024)
    ...
    # equivalent to the sum of the size of idx2 and idx3 = 74 + 95 = 169
    expected_info['inverted_sz_mb'] = 169 / (1024 * 1024)
