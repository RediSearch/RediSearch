...
def runQueryWithPausePoint(env, query_cmd, pause_before=None, pause_after=None,
                          rp_type=None, pause_count=0, internal_only=False,
                          callback=None, timeout=10):
    """
    Runs a debug query with a pause point, executes a callback while paused, then resumes.

    Executes a query in a separate thread with a pause point at a specified result processor.
    Waits for the query to pause, executes a callback function, and then resumes the query.

    Args:
        env: The test environment
        query_cmd: The query command (e.g., ['FT.SEARCH', 'idx', '*'])
        pause_before: If True, pause before the RP (mutually exclusive with pause_after)
        pause_after: If True, pause after the RP (mutually exclusive with pause_before)
        rp_type: The result processor type to pause at (e.g., 'Index', 'Loader')
        pause_count: Number of results to process before pausing (default: 0)
        internal_only: If True, add INTERNAL_ONLY flag to debug params (default: False)
        callback: Optional callback function to execute while query is paused
        timeout: Maximum time to wait for query to pause (default: 10 seconds)

    Returns:
        The query result
    """
    if not pause_before and not pause_after:
        raise ValueError("Either pause_before or pause_after must be specified")
    if pause_before and pause_after:
        raise ValueError("Cannot specify both pause_before and pause_after")
    if not rp_type:
        raise ValueError("rp_type must be specified")

    # Build debug params
    debug_params = []
    if pause_before:
        debug_params.extend(['PAUSE_BEFORE_RP_N', rp_type, pause_count])
    else:
        debug_params.extend(['PAUSE_AFTER_RP_N', rp_type, pause_count])
    ...
    while getIsRPPaused(env) != 1:
        if time.time() - start_time > timeout:
            raise TimeoutError(f"Query did not pause within {timeout} seconds")
        time.sleep(0.05)
    ...
    t_query.join(timeout=timeout)
    if t_query.is_alive():
        raise TimeoutError(f"Query did not complete within {timeout} seconds after resume")
