...
from unittest.mock import _ANY, ANY
...
class DialectEnv(Env):
    ...
    def set_dialect(self, dialect):
        ...
        result = run_command_on_all_shards(
            self, config_cmd(), "SET", "DEFAULT_DIALECT", dialect
        )
        expected_result = ["OK"] * self.shardsCount
        self.assertEqual(
            result,
            expected_result,
            message=f"Failed to set dialect to {dialect} on all shards",
        )
...
def server_version_is_at_least(ver):
    ...
    if server_ver is None:
        ...
        v = (
            subprocess.run([Defaults.binary, "--version"], stdout=subprocess.PIPE)
            .stdout.decode()
            .split()[2]
            .split("=")[1]
        )
...
def dump_numeric_index_tree_root(env, idx, numeric_field):
    ...
    root_dump = {
        tree_root_stats[i]: tree_root_stats[i + 1]
...
COORD_BUILD = (
    any("coord-oss" in m for m in Defaults.module) or "coord-oss" in Defaults.module
)
...
def forceInvokeGC(env, idx="idx", timeout=None):
    ...
    if timeout is not None:
        if timeout == 0:
            env.debugPrint(
                "forceInvokeGC: note timeout is infinite, consider using a big timeout instead.",
                force=True,
            )
...
def no_msan(f):
    ...
    def wrapper(env, *args, **kwargs):
        if SANITIZER == "memory":
            fname = f.__name__
            env.debugPrint(
                "skipping {} due to memory sanitizer".format(fname), force=True
            )
            env.skip()
            return
...
def unstable(f):
    ...
    def wrapper(env, *args, **kwargs):
        if UNSTABLE == True:
            fname = f.__name__
            env.debugPrint(
                "skipping {} because it is unstable".format(fname), force=True
            )
            env.skip()
            return
        return f(env, *args, **kwargs)
...
def skip(
    ...
    msan=False,
    noWorkers=False,
    redis_less_than=None,
    redis_greater_equal=None,
    min_shards=None,
    arch=None,
    gc_no_fork=None,
    no_json=False,
    ...
    def decorate(f):
        def wrapper():
            ...
            if (
                gc_no_fork
                and Env().cmd(config_cmd(), "GET", "GC_POLICY")[0][1] != "fork"
            ):
                raise SkipTest()
...
def set_max_dialect(env):
    ...
    if MAX_DIALECT == 0:
        ...
        MAX_DIALECT = max(
            [int(key.replace(prefix, "")) for key in info.keys() if prefix in key]
        )
    return MAX_DIALECT
...
def has_json_api_v2(env):
    return module_ver_filter(
        env, "ReJSON", lambda ver: True if ver == 999999 or ver >= 20200 else False
    )
...
class Bfloat16Array(np.ndarray):
    ...
    def tobytes(self):
        ...
        return b"".join(
            b32[i : i + 2] for i in range(Bfloat16Array.offset, len(b32), 4)
        )
...
def load_vectors_to_redis(
    env, n_vec, query_vec_index, vec_size, data_type="FLOAT32", ids_offset=0, seed=10
):
...
def sortResultByKeyName(res, start_index=1):
    ...
    pairs = [
        (name, sortResultByKeyName(value, 0) if isinstance(value, list) else value)
        for name, value in zip(res[start_index::2], res[start_index + 1 :: 2])
    ]
...
def get_TLS_args():
    ...
    subprocess.run(
        [os.path.join(root, "sbin", "gen-test-certs"), str(1 if with_pass else 0)]
    ).check_returncode()
