...
def test_shard_connections_reach_connected(env: Env):
    """With the TCP-connect watchdog always armed in MRConn_Connect, every pool
    connection must still reach the `Connected` state (and stay there across a
    pool expansion that creates new MRConn objects at runtime)."""
    verify_shard_init(env)
    ...
    # Force pool expansion: new MRConn objects go through the same
    # MRConnManager_Expand -> MR_NewConn -> MRConn_Connect code path.
