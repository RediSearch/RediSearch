...
def test_required_fields(env):
    ...
    env.expect("ft.search", "idx", "hello", "_REQUIRED_FIELDS", "1", "t").equal(
        [1, "0", "$hello", ["t", "hello"]]
    )
    env.expect(
        "ft.search",
    ...
    env.expect(
        "ft.search", "idx", "hello", "nocontent", "_REQUIRED_FIELDS", "1", "t"
    ).equal([1, "0", None])
...
def test_curly_brackets(env):
    ...
    env.expect("ft.search", "idx", "hello").equal(
        [1, "foo{bar}", ["t", "Hello world!"]]
    )
    env.expect("ft.aggregate", "idx", "hello", "LOAD", 1, "__key").equal(
        [1, ["__key", "foo{bar}"]]
    )
...
def test_single_shard_optimization():
    ...
    # Search
    env.expect("FT.SEARCH", "idx", "hello", "SORTBY", "n", "RETURN", 0).equal(
        [100] + [str(i) for i in range(10)]
    )

    # Aggregate
    env.expect("FT.AGGREGATE", "idx", "hello", "SORTBY", "1", "@n").equal(
        [100] + [["n", str(i)] for i in range(10)]
    )
    ...
    # Profile
    env.expect("FT.PROFILE", "idx", "SEARCH", "QUERY", "hello").noError().apply(
        lambda res: to_dict(res[1])["Coordinator"]
    ).equal([])
    # A simple validation that we get a standalone error response
    env.expect("FT.PROFILE", "idx", "SEARCH", "hello", "world").error().contains(
        "The QUERY keyword is expected"
    )
    # Verify that PROFILE does not support WITHCURSOR
    env.expect(
        "FT.PROFILE", "idx", "AGGREGATE", "QUERY", "hello", "WITHCURSOR"
    ).error().contains("FT.PROFILE does not support cursor")

    # SpellCheck
    env.expect("FT.SPELLCHECK", "idx", "hell").equal(
        [["TERM", "hell", [["1", "hello"]]]]
    )
