...
def testGetConfigOptions(env):
    ...
    check_config('BG_INDEX_OOM_PAUSE_TIME')
...
def testSetConfigOptions(env):
    ...
    env.expect(config_cmd(), 'set', 'BG_INDEX_OOM_PAUSE_TIME', 1).equal('OK')
...
def testSetConfigOptionsErrors(env):
    ...
    env.expect(config_cmd(), 'set', 'BG_INDEX_OOM_PAUSE_TIME', -1).contains('Value is outside acceptable bounds')
    env.expect(config_cmd(), 'set', 'BG_INDEX_OOM_PAUSE_TIME', UINT32_MAX+1).contains('Value is outside acceptable bounds')
@skip(cluster=True)
def testAllConfig(env):
    ...
    env.assertEqual(res_dict['BG_INDEX_OOM_PAUSE_TIME'][0], '0')
...
def testInitConfig():
    ...
    _test_config_num('BG_INDEX_OOM_PAUSE_TIME', 0)
...
numericConfigs = [
    ...
    ('search-bg_index-oom-pause-time','BG_INDEX_OOM_PAUSE_TIME', 0, 0, UINT32_MAX, False, False),
    # Cluster parameters
