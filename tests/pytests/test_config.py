...
# These configs register LLONG_MAX so over-cap startup values reach the module
# setter and are clamped for compatibility with the legacy path.
CLAMPED_CONFIGS = {
...
def testConfigAPIRunTimeNumericParams():
    ...
    def _testNumericConfig(env, configName, ftConfigName, default, min, max):
        ...
        if configName in CLAMPED_CONFIGS:
            # Values above the legacy maximum are accepted and clamped.
