...
def testConfigErrors(env):
    ...
    env.expect(config_cmd(), "no_such_command", "idx").equal(
        "No such configuration action"
    )
    env.expect(config_cmd(), "idx").error().contains("wrong number of arguments")
    env.expect(config_cmd(), "set", "_NUMERIC_RANGES_PARENTS", 3).equal(
        "Max depth for range cannot be higher than max depth for balance"
    )
    env.expect(config_cmd(), "set", "MINSTEMLEN", 1).error().contains(
        "Minimum stem length cannot be lower than"
    )
    if MT_BUILD:
        env.expect(config_cmd(), "set", "WORKERS", 1_000_000).error().contains(
            "Number of worker threads cannot exceed"
        )
...
def testGetConfigOptions(env):
    def check_config(conf):
        env.expect(config_cmd(), "get", conf).noError().apply(lambda x: x[0][0]).equal(
            conf
        )
...
def testSetConfigOptions(env):
    ...
    env.expect(config_cmd(), "set", "MINPREFIX", "str").equal(
        "Could not convert argument to expected type"
    )
...
def testSetConfigOptionsErrors(env):
    ...
    env.expect(config_cmd(), "set", "MAXEXPANSIONS", "str").equal(
        "Could not convert argument to expected type"
    )
    env.expect(config_cmd(), "set", "TIMEOUT", "str").equal(
        "Could not convert argument to expected type"
    )
    env.expect(config_cmd(), "set", "FORKGC_SLEEP_BEFORE_EXIT", "str").equal(
        "Could not convert argument to expected type"
    )
    env.expect(config_cmd(), "set", "FORKGC_SLEEP_BEFORE_EXIT", "str").equal(
        "Could not convert argument to expected type"
    ...
    if MT_BUILD:
        env.expect(config_cmd(), "set", "WORKERS", 2**13 + 1).contains(
            "Number of worker threads cannot exceed"
        )
        env.expect(config_cmd(), "set", "MIN_OPERATION_WORKERS", 2**13 + 1).contains(
            "Number of worker threads cannot exceed"
        )
...
def testImmutable(env):
    ...
    if MT_BUILD:
        env.expect(config_cmd(), "set", "TIERED_HNSW_BUFFER_LIMIT").error().contains(
            not_modifiable
        )
        env.expect(config_cmd(), "set", "PRIVILEGED_THREADS_NUM").error().contains(
            not_modifiable
        )  # deprecated
        env.expect(
            config_cmd(), "set", "WORKERS_PRIORITY_BIAS_THRESHOLD"
        ).error().contains(not_modifiable)
    ...
    env.expect(config_cmd(), "set", "PARTIAL_INDEXED_DOCS").error().contains(
        not_modifiable
    )
    env.expect(config_cmd(), "set", "UPGRADE_INDEX").error().contains(not_modifiable)
    env.expect(config_cmd(), "set", "RAW_DOCID_ENCODING").error().contains(
        not_modifiable
    )
    env.expect(config_cmd(), "set", "BG_INDEX_SLEEP_GAP").error().contains(
        not_modifiable
    )
...
def testConfigErrorsCoord(env):
    env.expect(config_cmd(), "set", "SEARCH_THREADS", "banana").error().contains(
        not_modifiable
    )
    env.expect(config_cmd(), "set", "SEARCH_THREADS", "-1").error().contains(
        not_modifiable
    )
