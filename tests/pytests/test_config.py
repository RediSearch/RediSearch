...
def testGetConfigOptions(env):
    ...
    check_config('_INDEX_MEM_PERCENT')
...
def testSetConfigOptions(env):
    ...
    env.expect(config_cmd(), 'set', '_INDEX_MEM_PERCENT', 1).equal('OK')
...
def testSetConfigOptionsErrors(env):
    ...
    env.expect(config_cmd(), 'set', '_INDEX_MEM_PERCENT', -1).contains('Value is outside acceptable bounds')
    env.expect(config_cmd(), 'set', '_INDEX_MEM_PERCENT', 101).contains('Memory limit for indexing cannot be greater then 100%')
...
def testAllConfig(env):
    ...
    env.assertEqual(res_dict['_INDEX_MEM_PERCENT'][0], '80')
...
def testInitConfig():
    ...
    _test_config_num('_INDEX_MEM_PERCENT', 80)
...
numericConfigs = [
    ...
    ('search-_stop-indexing-memory-percent', '_INDEX_MEM_PERCENT', 80, 0, 100, False, False),
    # Cluster parameters
