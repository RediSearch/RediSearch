...
def _build_mixed_burst_commands():
    # Intentionally build a 3:1 FT.SEARCH-to-FT.INFO prefix so the burst
    # stays mixed while still driving the coordinator into the saturation region,
    # then continue draining the info-heavy tail.
    return ([search] * 3 + [info]) * 30 + [info] * 120
...
def _run_burst_command(env, command, exceptions, completed, lock):
    try:
        ...
        elif command[0] == 'FT.INFO':
            env.assertEqual(to_dict(res).get('index_name'), 'idx')
...
def test_search_and_aggregate_burst():
    ...
    try:
        ...
        wait_for_condition(
            ...
            'Timeout waiting for mixed FT.SEARCH/FT.INFO burst to drain',
            timeout=15,
        )
    ...
    env.assertEqual(to_dict(env.cmd(*info)).get('index_name'), 'idx')
