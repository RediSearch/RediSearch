...
def test_param_errors(env):
    ...
    env.assertOk(conn.execute_command('FT.CREATE', 'idx', 'SCHEMA', 'foo', 'TEXT', 'bar', 'TAG', 'g', 'GEO', 'num', 'NUMERIC', 'v', 'VECTOR', 'HNSW', '3', 'TYPE', 'INT32', 'DIM', '2', 'DISTANCE_METRIC', 'L2'))
    waitForIndex(env, 'idx')
...
def test_vector(env):
    ...
    env.expect('FT.CREATE idx SCHEMA v VECTOR HNSW 3 TYPE INT32 DIM 2 DISTANCE_METRIC L2').ok()
