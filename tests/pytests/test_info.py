...
def testInfo(env):
    ...
    for i in range(count):
        ...
        pl.execute_command(
            ...
            "hello%d" % i,
            "body",
            "%dhello%dworld%dhow%dare%dyou%dtoday%d" % (i, i, i, i, i, i, i),
            "n",
            i / 17.0,
            "geo",
            geo,
...
def test_numeric_info(env):
    ...
    env.assertEqual(
        res4, exp3
    )  # Numeric field is sortable, explicitly NOINDEX, and automatically UNF
    env.assertEqual(
        res5, exp3
    )  # Numeric field is sortable, explicitly NOINDEX, and explicitly UNF
