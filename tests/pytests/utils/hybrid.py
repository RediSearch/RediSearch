...
def translate_vector_query(vector_query, vector_blob, index_name, cmd_suffix=None):
    """
...
def translate_search_query(search_query, index_name, cmd_suffix=None):
    """
...
def run_test_scenario(env, index_name, scenario, vector_blob):
    ...
    search_cmd = translate_search_query(
                    scenario['search_equivalent'],
                    index_name,
                    scenario.get('search_suffix', ''))
...
def run_linear_test_scenario(env, index_name, scenario, vector_blob):
    """
    Run a test scenario using linear combination algorithm instead of RRF

    Args:
        env: Test environment
        index_name: Redis index name
        scenario: Dict with test scenario containing:
            - 'search_equivalent': Search query string
            - 'vector_equivalent': Vector query string
            - 'vector_suffix': Optional suffix for vector command
            - 'alpha': Weight coefficient for search results (default: 1)
            - 'beta': Weight coefficient for vector results (default: 1)
            - 'window': Window size for limiting results (default: 20)
        vector_blob: Vector data as bytes
    ...
    Example scenario:
        {
            'hybrid_query': "SEARCH hello VSIM @vector $BLOB",
            'search_equivalent': 'hello world',
    ...
    vector_cmd = translate_vector_query(
                    scenario['vector_equivalent'], vector_blob,
                    index_name, scenario.get('vector_suffix', ''))
    # print(f"Linear Vector command: {vector_cmd}")
    vector_results_raw = conn.execute_command(*vector_cmd)

    # Process vector results
    vector_results = _process_vector_response(vector_results_raw)

    # Get linear combination parameters from scenario
    alpha = scenario.get('alpha', 1)
    beta = scenario.get('beta', 1)
    window = scenario.get('window', 20)

    # print(f"Linear Search results: {search_results}")
    # print(f"Linear Vector results: {vector_results}")

    # Calculate expected linear combination results
    expected_linear = linear(search_results, vector_results, alpha=alpha, beta=beta, window=window)
    _sort_adjacent_same_scores(expected_linear)
    # print(f"Linear Expected results: {expected_linear}")

    # For comparison purposes, we can still execute a hybrid query if available
    # This allows testing against actual Redis hybrid results vs our linear combination
    hybrid_cmd = translate_hybrid_query(
                scenario['hybrid_query'], vector_blob, index_name)
    # print(f"Hybrid command: {hybrid_cmd}")
    hybrid_results_raw = conn.execute_command(*hybrid_cmd)

    hybrid_results, ranking_info = _process_hybrid_response(hybrid_results_raw)
    _sort_adjacent_same_scores(hybrid_results)

    # Create comparison table showing hybrid vs linear results
    comparison_table = _create_comparison_table(
        hybrid_results, expected_linear, ranking_info, search_results, vector_results)

    # Note: This compares hybrid results against linear combination
    # The test may fail if hybrid uses RRF while we're using linear combination
    # This is expected behavior - use this for analysis, not strict validation
    # print(f"\nLinear Combination Test (alpha={alpha}, beta={beta}, window={window}):")
    # print("Comparing Redis Hybrid (actual) vs Linear Combination (expected)")
    # print(comparison_table)

    _validate_results(env, hybrid_results, expected_linear, comparison_table)
    return True
