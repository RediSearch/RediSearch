...
def testBasicSynonymsUseCase(env):
    ...
    env.assertEqual(
        set(res[2]), set(["title", "he is a boy", "body", "this is a test"])
    )
...
def testSynonymGroupWithThreeSynonyms(env):
    ...
    env.assertEqual(
        env.cmd("ft.synupdate", "idx", "id1", "boy", "child", "offspring"), "OK"
    )
...
def testSynonymWithMultipleDocs(env):
    ...
    env.assertEqual(
        set(res[4]), set(["title", "she is a girl", "body", "the child sister"])
    )
...
def testSynonymUpdate(env):
    ...
    env.assertEqual(
        env.cmd(
            "ft.synupdate", "idx", "id1", "SKIPINITIALSCAN", "boy", "child", "offspring"
        ),
        "OK",
    )
    env.expect(
    ...
    env.assertEqual(
        set(res[2]), set(["title", "he is another baby", "body", "another test"])
    )
...
def testSynonymUpdateUnknownIndex(env):
    env.expect("ft.synupdate", "idx", "0", "child").error().contains(
        "Unknown index name"
    )
...
def testTwoSynonymsSearch(env):
    ...
    env.assertEqual(res[0:2], [1, "doc1"])
    env.assertEqual(
        set(res[2]), set(["title", "he is a boy child boy", "body", "another test"])
    )
...
def testSynonymsIntensiveLoad(env):
    ...
    for _ in env.reloadingIterator():
        ...
        for i in range(iterations):
            ...
            env.assertEqual(
                set(res[2]),
                set(["title", "he is a boy%d" % i, "body", "this is a test"]),
            )
...
def testSynonymsLowerCase(env):
    ...
    env.assertEqual(
        toSortedFlatList(dump),
        toSortedFlatList((["ahalan", ["id1"], "shalom", ["id1"], "hello", ["id1"]])),
    )
