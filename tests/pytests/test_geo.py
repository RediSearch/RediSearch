...
def testGeoHset(env):
    ...
    env.expect("FT.SEARCH", "idx", "@g:[1.23 4.56 1 km]").equal(
        [1, "geo2", ["g", "1.23,4.56"]]
    )
...
def testGeoSortable(env):
    ...
    env.expect(
        ...
        "*",
        "APPLY",
        "geodistance(@location,1.25,4.5)",
        "AS",
        "distance",
        "GROUPBY",
        "1",
        "@distance",
    ).equal([1, ["distance", "7032.37"]])


def testGeoFtAdd(env):
    env.expect("FT.CREATE idx SCHEMA g GEO").ok()
    env.expect(
        "FT.ADD", "idx", "geo1", "1", "FIELDS", "g", "1.23", "4.56"
    ).error().contains("Fields must be specified in FIELD VALUE pairs")
    env.expect("FT.ADD", "idx", "geo2", "1", "FIELDS", "g", "1.23,4.56").ok()
    env.expect(
        "FT.ADD", "idx", "geo3", "1", "FIELDS", "g", '"1.23,4.56"'
    ).ok()  # this is an error and won't index
    env.expect("FT.SEARCH", "idx", "@g:[1.23 4.56 1 km]").equal(
        [1, "geo2", ["g", "1.23,4.56"]]
    )
...
def testGeoLong(env):
    ...
    env.expect("FT.SEARCH", "idx", "*").equal(
        [1, "geo", ["g", "1.2345678901234567890,4.5678901234567890"]]
    )
    env.expect("FT.SEARCH", "idx", "@g:[1.23 4.56 2 km]").equal(
        [1, "geo", ["g", "1.2345678901234567890,4.5678901234567890"]]
    )
...
def testGeoDistanceSimple(env):
    env.expect(
        "ft.create", "idx", "schema", "name", "text", "location", "geo", "hq", "geo"
    ).ok()
    ...
    env.expect("FT.SEARCH", "idx", "@location:[1.23 4.56 10 km]", "nocontent").equal(
        [4, "geo1", "geo2", "geo3", "geo4"]
    )
    ...
    env.expect("FT.SEARCH", "idx", "@location:[1.23 4.56 -10 km]").error().contains(
        "Invalid GeoFilter radius"
    )
    env.expect("FT.SEARCH", "idx", "@location:[181 4.56 10 km]").error().contains(
        "Invalid GeoFilter lat/lon"
    )
...
def testGeoDistanceFile(env):
    ...
    env.expect(
        ...
        "@location",
        "APPLY",
        "geodistance(@location,-0.15036,51.50566)",
        "AS",
        "distance",
        "GROUPBY",
        "1",
...
def testGeoOnReopen(env):
    ...
    def loadDocs(num=len(hotels), offset=0):
        for i in range(num):
            ...
            conn.execute_command(
                "HSET", i + offset, "name", hotel_name, "location", f"{long},{lat}"
            )
    ...
    res, cursor = conn.execute_command(
        "FT.AGGREGATE",
        "idx",
        "@location:[-0.15036 51.50566 10000 km]",
        "LOAD",
        3,
        "@__key",
        "AS",
