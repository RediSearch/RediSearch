...
def buildExpireDocsResults(isSortable, isJson):
    ...
    results[both_docs_no_sortby] = (
        [2, "doc1", ["t", "bar"], "doc2", ["t", "arr"]]
        if not isJson
        else [2, "doc1", ["$", '{"t":"bar"}'], "doc2", ["$", '{"t":"arr"}']]
    )
    results[both_docs_sortby] = (
        [2, "doc2", ["t", "arr"], "doc1", ["t", "bar"]]
        if not isJson
        else [
            2,
            "doc2",
    ...
    results[doc2_is_null] = (
        [2, "doc1", ["t", "bar"], "doc2", None]
        if not isJson
        else [2, "doc1", ["$", '{"t":"bar"}'], "doc2", None]
    )
    results[doc2_is_null_sortby] = (
        [2, "doc1", ["t", "bar"], "doc2", None]
        if not isJson
        else [2, "doc1", ["t", "bar", "$", '{"t":"bar"}'], "doc2", None]
    )
    results[doc2_is_null_sortby_sorted] = (
        [2, "doc2", None, "doc1", ["t", "bar"]]
        if not isJson
        else [2, "doc2", None, "doc1", ["t", "bar", "$", '{"t":"bar"}']]
    )

    # on Json we also return the sortby field value.
    results[only_doc1_sortby] = (
        [1, "doc1", ["t", "bar"]]
        if not isJson
        else [1, "doc1", ["t", "bar", "$", '{"t":"bar"}']]
    )
    results[only_doc1_no_sortby] = (
        [1, "doc1", ["t", "bar"]] if not isJson else [1, "doc1", ["$", '{"t":"bar"}']]
    )
...
def expireDocs(env, isSortable, expected_results, isJson):
    ...
    for sortby in [False, True]:
        ...
        env.assertEqual(
            res, expected_results[both_docs_no_sortby], message="both docs exist"
        )
        ...
        if isSortable:
            expected_res = expected_results[
                doc2_is_null_sortby_sorted if sortby else doc2_is_null
            ]
        else:
            expected_res = expected_results[
                doc2_is_null_sortby if sortby else doc2_is_null
            ]
        ...
        env.assertEqual(
            res,
            expected_results[only_doc1_sortby if sortby else only_doc1_no_sortby],
            message=msg,
        )
        ...
        res = conn.execute_command(
            "FT.SEARCH", "idx", "*", "WITHSCORES", "EXPLAINSCORE"
        )
        ...
        res = conn.execute_command(
            "FT.SEARCH", "idx", "*", "WITHSCORES", "EXPLAINSCORE", *sortby_cmd
        )

        if isSortable:
            env.assertEqual(
                res,
                add_explain_to_results(
                    expected_results[
                        doc2_is_null_sortby_sorted if sortby else doc2_is_null
                    ]
                ),
                message=(msg + " WITHSCORES, EXPLAINSCORE"),
            )
        else:
            env.assertEqual(
                res,
                add_explain_to_results(
                    expected_results[doc2_is_null_sortby if sortby else doc2_is_null]
                ),
                message=(msg + " WITHSCORES, EXPLAINSCORE"),
            )
...
# otherwise 'f'
def commonFieldExpiration(
    env, schema, fields, expiration_interval_to_fields, document_name_to_expire
):
    ...
    def setup_field_expiration(current_documents):
        for doc_name, should_expire in document_name_to_expire.items():
            ...
            for (
                expiration_interval,
                field_list,
            ) in expiration_interval_to_fields.items():
                conn.execute_command(
                    "HPEXPIRE",
                    doc_name,
                    str(expiration_interval),
                    "FIELDS",
                    str(len(field_list)),
                    *field_list,
                )
    ...
    env.expect("FT.SEARCH", "idx", "*").apply(transform_document_list_to_dict).equal(
        expected_results
    )
    ...
    for field_name_and_value, expected_docs in expected_inverted_index.items():
        (
            env.expect(
                "FT.SEARCH",
                "idx",
                f"@{field_name_and_value}:{field_name_and_value}",
                "NOCONTENT",
            )
            .apply(sort_document_names)
            .equal([len(expected_docs), *expected_docs])
        )
    conn.execute_command("FLUSHALL")
...
def testSingleExpireField(env):
    ...
    commonFieldExpiration(
        env,
        schema,
        field_to_additional_schema_keywords.keys(),
