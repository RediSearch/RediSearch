...
def test_search():
    ...
    env.cmd(
        "FT.create", "idx1", "PREFIX", 1, "doc", "SCHEMA", "f1", "TEXT", "f2", "TEXT"
    )
    ...
    exp = {
        ...
        "results": [
            {
                "id": "doc2",
                "extra_attributes": {"f1": "3", "f2": "2", "f3": "4"},
                "values": [],
            },
...
def test_search_timeout():
    ...
    env = Env(
        protocol=3,
        moduleArgs=f"DEFAULT_DIALECT 2 MAXPREFIXEXPANSIONS {num_range} TIMEOUT 1 ON_TIMEOUT FAIL",
    )
    ...
    for i in range(num_range):
        conn.execute_command(
            "HSET", f"doc{i}", "t", f"aa{i}", "geo", f"{i/10000},{i/1000}"
        )
...
def test_profile(env):
    ...
    env.expect("FT.PROFILE", "idx1", "SEARCH", "QUERY", "*", "FORMAT", "STRING").equal(
        exp
    )
...
def test_coord_profile():
    ...
    res["Results"]["results"].sort(
        key=lambda x: (
            ""
            if x["extra_attributes"].get("f1") == None
            else x["extra_attributes"]["f1"]
        )
    )
    ...
    exp = {
        ...
        "Profile": {
            ...
            "Coordinator": {
                ...
                "Warning": "None",
                "Result processors profile": [
                    {"Type": "Network", "Time": ANY, "Counter": 2}
                ],
            },
        },
    }
...
def test_aggregate():
    ...
    res = env.cmd(
        "FT.aggregate", "idx1", "*", "LOAD", 2, "f1", "f2", "FORMAT", "STRING"
    )
    res["results"].sort(
        key=lambda x: (
            ""
            if x["extra_attributes"].get("f2") == None
            else x["extra_attributes"].get("f2")
        )
    )
    exp = {
        "attributes": [],
    ...
    res = env.cmd(
        "FT.aggregate", "idx1", "*", "LOAD", 3, "f1", "f2", "f3", "FORMAT", "STRING"
    )
    exp = {
        "attributes": [],
        "warning": [],
...
def test_cursor():
    ...
    exp = {
        ...
        "total_results": 3,
        "format": "STRING",
        "results": [{"extra_attributes": {"f1": "3", "f2": "3"}, "values": []}],
    }
    res, cursor = env.cmd(
        "FT.aggregate",
        "idx1",
    ...
    env.cmd(
        "FT.create", "idx2", "PREFIX", 1, "folder", "SCHEMA", "f1", "TEXT", "f2", "TEXT"
    )
...
def test_config():
    ...
    env.assertEqual(
        res["TIMEOUT"], "0"
    )  # FIXME: should be '501'. This is a bug in the RESP3 implementation because we have 2 configurations with the same name
    ...
    env.assertEqual(
        res, {"TIMEOUT": {"Description": "Query (search) timeout", "Value": "501"}}
    )
...
def testSpellCheckIssue437():
    ...
    env.expect(
        ...
        "INCLUDE",
        "slang",
    ).equal({"results": {"tooni": [{"toonie": 0.0}]}})
        ...
        "slots": [
            {
                "end": 5461,
                "nodes": [
                    {"host": "127.0.0.1", "id": ANY, "port": ANY, "role": "master self"}
                ],
                "start": 0,
            },
            {
                "end": 10923,
                "nodes": [
                    {"host": "127.0.0.1", "id": ANY, "port": ANY, "role": "master "}
                ],
                "start": 5462,
            },
            {
...
def test_profile_crash_mod5323():
    ...
    res = env.cmd(
        "FT.PROFILE", "idx", "SEARCH", "LIMITED", "QUERY", "%hell% hel*", "NOCONTENT"
    )
    exp = {
        "Results": {
            "warning": [],
...
def test_profile_child_itrerators_array():
    ...
    exp = {
        "Results": {
            "warning": [],
            "attributes": [],
            "results": [{"values": [], "id": "1"}, {"values": [], "id": "2"}],
            "total_results": 2,
            "format": "STRING",
        },
        "Profile": {
...
def testExpandErrorsResp3():
    ...
    env.expect("FT.SEARCH", "idx", "*", "FORMAT").error().contains(
        "Need an argument for FORMAT"
    )
    env.expect("FT.SEARCH", "idx", "*", "FORMAT", "XPAND").error().contains(
        "FORMAT XPAND is not supported"
    )
    env.expect("FT.AGGREGATE", "idx", "*", "FORMAT").error().contains(
        "Need an argument for FORMAT"
    )
    env.expect("FT.AGGREGATE", "idx", "*", "FORMAT", "XPAND").error().contains(
        "FORMAT XPAND is not supported"
    )

    env.expect(
        "FT.SEARCH", "idx", "*", "FORMAT", "EXPAND", "DIALECT", 2
    ).error().contains("requires dialect 3 or greater")

    # On HASH
    env.cmd(
        "ft.create", "idx2", "on", "hash", "SCHEMA", "$.arr", "as", "arr", "numeric"
    )
    env.expect("FT.SEARCH", "idx2", "*", "FORMAT", "EXPAND").error().contains(
...
def testExpandErrorsResp2():
    env = Env(protocol=2)
    env.cmd("ft.create", "idx", "on", "json", "SCHEMA", "$.arr", "as", "arr", "numeric")
    env.expect("FT.SEARCH", "idx", "*", "FORMAT", "EXPAND").error().contains(
        "EXPAND format is only supported with RESP3"
    )

    env.expect("FT.AGGREGATE", "idx", "*", "FORMAT", "EXPAND").error().contains(
        "EXPAND format is only supported with RESP3"
    )

    # On HASH
    env.cmd(
        "ft.create", "idx2", "on", "hash", "SCHEMA", "num", "numeric", "str", "text"
    )
    env.expect("FT.SEARCH", "idx2", "*", "FORMAT", "EXPAND").error().contains(
        "EXPAND format is only supported with RESP3"
    )
...
def testExpandJson():
    ...
    exp_string = {
        ...
        "results": [
            {
                "id": "doc1",
                "extra_attributes": {"num": "1", "$": doc1_js},
                "values": [],
            },
            {
                "id": "doc2",
                "extra_attributes": {"num": "2", "$": doc2_js},
                "values": [],
            },
        ],
    }
    ...
    res = env.cmd(
        "FT.SEARCH", "idx", "*", "LIMIT", 0, 2, "FORMAT", "EXPAND", "SORTBY", "num"
    )
    env.assertEqual(res, exp_expand)

    res = env.cmd(
        "FT.SEARCH", "idx", "*", "LIMIT", 0, 2, "FORMAT", "STRING", "SORTBY", "num"
    )
    env.assertEqual(res, exp_string)
        ...
        "results": [
            {
                ...
                "extra_attributes": {
                    "$.arr[?(@>2)]": "[2.1,3.14]",
                    "str": '["foo"]',
                    "multi": "[1.0,2.1,3.14,10,20,33.33]",
                    "arr": "[[1.0,2.1,3.14]]",
                },
                "values": [],
            },
    ...
    res = env.cmd(
        "FT.SEARCH", "idx", "*", "LIMIT", 0, 2, "RETURN", *load_args, "DIALECT", 3
    )
    env.assertEqual(res, exp_string)
...
def testExpandHash():
    ...
    exp_string = {
        ...
        "results": [
            {
                "id": "doc1",
                "extra_attributes": {"num": "1", "str": "foo", "other": "fu"},
                "values": [],
            },
            {
                "id": "doc2",
                "extra_attributes": {"num": "2", "str": "bar", "other": "bur"},
                "values": [],
            },
        ],
    }
    ...
    res["results"].sort(
        key=lambda x: (
            ""
            if x["extra_attributes"].get("num") == None
            else x["extra_attributes"].get("num")
        )
    )
    env.assertEqual(res, exp_string)
    ...
    exp_string = {
        ...
        "results": [
            {
                "id": "doc1",
                "extra_attributes": {"num": "1", "other": "fu"},
                "values": [],
            },
            {
                "id": "doc2",
                "extra_attributes": {"num": "2", "other": "bur"},
                "values": [],
            },
        ],
    }
...
def testExpandJsonVector():
    ...
    doc1_content = {"v": [1, 2, 3], "num": 1}
    # json string format
    doc1_content_js = json.dumps(doc1_content, separators=(",", ":"))

    doc2_content = {"v": [4, 2, 0], "num": 2}
    # json string format
        ...
        "results": [
            {
                "id": "doc1",
                "extra_attributes": {
                    "__vec_score": 6.7095842361450195,
                    "$": [doc1_content],
                },
                "values": [],
            },
...
def test_vecsim_1():
    ...
    with env.getClusterConnectionIfNeeded() as r:
        r.execute_command(
            ...
            "docvecsimidx0z0",
            "vector_FLAT",
            np.array([0.0, 0.0], dtype=np.float32).tobytes(),
        )
        r.execute_command(
            "HSET",
            "docvecsimidx0z1",
            "vector_FLAT",
            np.array([1.0, 1.0], dtype=np.float32).tobytes(),
        )
        r.execute_command(
            "HSET",
            "docvecsimidx0z2",
            "vector_FLAT",
            np.array([2.0, 2.0], dtype=np.float32).tobytes(),
        )
        r.execute_command(
            "HSET",
            "docvecsimidx0z3",
            "vector_FLAT",
            np.array([3.0, 3.0], dtype=np.float32).tobytes(),
        )
    exp3 = {
        "attributes": [],
        "warning": [],
    ...
    env.assertEqual(
        dict_diff(
            res,
            exp3 if env.protocol == 3 else exp2,
            show=True,
