...
def testSearchUpdatedContent(env):
    ...
    expected = [
        2,
        "doc:1",
        ["$", json.loads(plain_val_1_raw)],
        "doc:2",
        ["$", json.loads(plain_val_2_raw)],
    ]
    ...
    env.assertEqual(
        ...
        toSortedFlatList([1, ["labelT", "rex"], ["labelT", "riceratops"]]),
    )
    env.expect("ft.aggregate", "idx1", "re*", "LOAD", "1", "labelT").equal(
        [1, ["labelT", "rex"]]
    )
    ...
    # test JSON.NUMINCRBY
    env.expect("json.numincrby", "doc:1", "$.n", int_incrby_3).equal(
        "[" + plain_int_res_val_3 + "]"
    )
    ...
    expected = [
        1,
        "doc:1",
        ["$", json.loads(r'{"t":"hescelosaurus","n":' + plain_int_res_val_3 + "}")],
    ]
    ...
    env.expect("ft.search", "idx1", "riceratops", "RETURN", "1", "$.n").equal(
        [1, "doc:2", ["$.n", "9"]]
    )
    env.expect("ft.search", "idx1", "riceratops", "RETURN", "1", "$.t").equal(
        [1, "doc:2", ["$.t", "riceratops"]]
    )
...
def testHandleUnindexedTypes(env):
    ...
    env.expect("ft.search", "idx", "*", "RETURN", "2", "string", "int_arr").equal(
        [
            1,
            "doc:1",
            [
...
def testReturnAllTypes(env):
    ...
    env.expect("JSON.SET", "doc:1", "$", doc1_content).ok()
    env.cmd(
        "FT.CREATE", "idx", "ON", "JSON", "SCHEMA", "$.string", "AS", "string", "TEXT"
    )
...
def testNoContent(env):
    # Test NOCONTENT
    env.cmd(
        "FT.CREATE", "idx", "ON", "JSON", "SCHEMA", "$.t", "TEXT", "$.flt", "NUMERIC"
    )
...
def testReturnRoot(env):
    ...
    env.expect("ft.search", "idx", "foo", "RETURN", "1", "$").equal(
        [1, "doc:1", ["$", '{"t":"foo"}']]
    )
...
def testNonEnglish(env):
    ...
    env.assertEqual(
        json.loads(env.cmd("json.get", "doc:4", ".")),
        json.loads(japanese_doc_value_raw),
    )
    ...
    env.assertEqual(
        json.loads(env.cmd("json.get", "doc:5", ".")), json.loads(chinese_value_1_raw)
    )

    env.expect(
        "ft.search", "idx1", "*", "RETURN", "3", "$.t", "AS", "MyReturnLabel"
    ).equal(
        [2, "doc:4", ["MyReturnLabel", "ドラゴン"], "doc:5", ["MyReturnLabel", "踪迹"]]
    )
...
def testToggle(env):
    # JSON.TOGGLE
    env.expect(
        "FT.CREATE", "idx", "ON", "JSON", "SCHEMA", "$.boolT", "AS", "boolT", "TAG"
    ).ok()
...
def testStrappend(env):
    ...
    env.expect("ft.search", "idx", "RedisLabs").equal(
        [1, "doc:1", ["$", '{"t":"RedisLabs"}']]
    )
...
def testArrayCommands(env):
    conn = getConnectionByEnv(env)
    env.cmd("FT.CREATE", "idx", "ON", "JSON", "SCHEMA", "$.tag[*]", "AS", "tag", "TAG")

    env.assertOk(conn.execute_command("JSON.SET", "doc:1", "$", '{"tag":["foo"]}'))
    env.assertEqual(
        conn.execute_command("JSON.ARRAPPEND", "doc:1", "$.tag", '"bar"'), [2]
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:1", "$.tag[*]"), '["foo","bar"]'
    )
    ...
    # use JSON.ARRINSERT
    env.assertEqual(
        conn.execute_command("JSON.ARRINSERT", "doc:1", "$.tag", "2", '"baz"'), [3]
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:1", "$.tag[*]"), '["foo","bar","baz"]'
    )
    ...
    # use JSON.ARRPOP
    env.assertEqual(
        conn.execute_command("JSON.ARRPOP", "doc:1", "$.tag", "1"), ['"bar"']
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:1", "$.tag[*]"), '["foo","baz"]'
    )
    env.assertEqual(conn.execute_command("JSON.ARRLEN", "doc:1", "$.tag"), [2])
    ...
    # use JSON.ARRTRIM
    env.assertEqual(
        conn.execute_command("JSON.ARRINSERT", "doc:1", "$.tag", "0", '"1"'), [3]
    )
    env.assertEqual(
        conn.execute_command("JSON.ARRINSERT", "doc:1", "$.tag", "0", '"2"'), [4]
    )
    env.assertEqual(
        conn.execute_command("JSON.ARRAPPEND", "doc:1", "$.tag", '"3"', '"4"'), [6]
    )
    env.assertEqual(conn.execute_command("JSON.ARRLEN", "doc:1", "$.tag"), [6])

    env.assertEqual(
        conn.execute_command("JSON.ARRTRIM", "doc:1", "$.tag", "2", "3"), [2]
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:1", "$.tag[*]"), '["foo","baz"]'
    )
...
def testArrayCommands_withVector(env):
    ...
    for data_type in ["FLOAT32", "FLOAT64"]:
        ...
        env.assertEqual(
            conn.execute_command("JSON.ARRAPPEND", "doc:1", "$.v", "2"), [2]
        )
        ...
        # use JSON.ARRTRIM
        env.assertEqual(
            conn.execute_command("JSON.ARRINSERT", "doc:1", "$.v", "0", '"a"'), [3]
        )
        env.assertEqual(
            conn.execute_command("JSON.ARRINSERT", "doc:1", "$.v", "0", '"b"'), [4]
        )
        env.assertEqual(
            conn.execute_command("JSON.ARRAPPEND", "doc:1", "$.v", '"c"', '"d"'), [6]
        )
        ...
        env.assertEqual(
            conn.execute_command("JSON.ARRTRIM", "doc:1", "$.v", "2", "3"), [2]
        )
        env.assertEqual(conn.execute_command("JSON.GET", "doc:1", "$.v[*]"), "[1,3]")
        env.assertEqual(conn.execute_command("JSON.ARRLEN", "doc:1", "$.v"), [2])
        waitForIndex(env, "idx")
        ...
        env.assertEqual(
            conn.execute_command("JSON.NUMINCRBY", "doc:1", "$.v[0]", "1"), "[2]"
        )
        ...
        env.assertEqual(
            conn.execute_command("JSON.SET", "doc:1", "$.v[1]", "true"), "OK"
        )
...
def testMultiValueTag(env):
    ...
    #
    env.assertOk(
        conn.execute_command("JSON.SET", "doc:1", "$", '{"tag":["foo", "bar", "baz"]}')
    )
    env.assertOk(
        conn.execute_command("JSON.SET", "doc:2", "$", '{"tag":["foo, bar", "baz"]}')
    )
    env.assertOk(
        conn.execute_command("JSON.SET", "doc:3", "$", '{"tag":["foo, bar, baz"]}')
    )
    ...
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:2", "$"), '[{"tag":["foo, bar","baz"]}]'
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:2", "$.tag"), '[["foo, bar","baz"]]'
    )
    ...
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:3", "$"), '[{"tag":["foo, bar, baz"]}]'
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:3", "$.tag"), '[["foo, bar, baz"]]'
    )
...
def testMultiValueTag_Recursive_Decent(env):
    conn = getConnectionByEnv(env)
    env.cmd("FT.CREATE", "idx", "ON", "JSON", "SCHEMA", "$..name", "AS", "name", "TAG")
    conn.execute_command(
        "JSON.SET", "doc:1", "$", '{"name":"foo", "in" : {"name":"bar"}}'
    )
...
def testDemo(env):
    ...
    env.expect(
        "FT.CREATE airports ON JSON SCHEMA $.iata AS iata TAG                          \
    ...
    env.assertEqual(
        slice_at(slice_at(info, "index_definition")[0], "key_type")[0], "JSON"
    )
    env.assertEqual(
        slice_at(info, "attributes")[0],
        [
            [
                "identifier",
                "$.iata",
    ...
    env.expect("FT.SEARCH", "airports", "sfo", "RETURN", "1", "$.name").equal(
        [1, "A:SFO", ["$.name", "San Francisco International Airport"]]
    )
    ...
    expected_res = [
        ...
        [
            ...
            "SFO",
            "$",
            '{"iata":"SFO","name":"San Francisco International Airport","location":"-122.375,37.6189995"}',
        ],
    ]
    res = env.cmd(
        "FT.AGGREGATE", "airports", "sfo", "LOAD", "1", "$", "SORTBY", "1", "@iata"
    )
    env.assertEqual(toSortedFlatList(res), toSortedFlatList(expected_res))

    res = env.cmd(
        "FT.AGGREGATE", "airports", "sfo", "SORTBY", "1", "@iata", "LOAD", "1", "$"
    )
    env.assertEqual(toSortedFlatList(res), toSortedFlatList(expected_res))
...
def testIndexSeparation(env):
    ...
    env.cmd(
        ...
        "doc:1",
        "$",
        r'{"t":"riceratops","t2":"telmatosaurus","n":9072,"flt":97.2}',
    )
    env.cmd(
        "FT.CREATE",
        "idxJson2",
...
def testAsProjection(env):
    ...
    env.cmd(
        ...
        "doc:1",
        "$",
        r'{"t":"riceratops","n":"9072","flt":97.2, "sub":{"t":"rex"}}',
    )

    # Test RETURN with label from schema
    env.expect("FT.SEARCH", "idx", "*", "RETURN", "3", "$.t", "AS", "txt").equal(
        [1, "doc:1", ["txt", "riceratops"]]
    )
    # Test LOAD with label from schema
    env.expect("FT.AGGREGATE", "idx", "*", "LOAD", "3", "@$.t", "AS", "txt").equal(
        [1, ["txt", "riceratops"]]
    )

    # Test RETURN with label not from schema
    env.expect("FT.SEARCH", "idx", "*", "RETURN", "3", "$.n", "AS", "num").equal(
        [1, "doc:1", ["num", "9072"]]
    )
...
def testAsProjectionRedefinedLabel(env):
    ...
    env.expect(
        "ft.search", "idx2", "*", "RETURN", "3", "$.t", "AS", "MyOnTheFlyReturnLabel"
    ).equal([1, "doc:1", ["MyOnTheFlyReturnLabel", "riceratops"]])
    env.expect(
        "ft.aggregate", "idx2", "*", "LOAD", "3", "@$.t", "AS", "MyOnTheFlyReturnLabel"
    ).equal([1, ["MyOnTheFlyReturnLabel", "riceratops"]])
    ...
    env.expect(
        "ft.search", "idx2", "*", "RETURN", "4", "$.n", "AS", "labelT", "labelT"
    ).equal([1, "doc:1", ["labelT", "9072"]])

    # TODO: re-enable this
    if False:  # UNSTABLE_TEST
        env.expect(
            "ft.aggregate", "idx2", "*", "LOAD", "4", "@$.n", "AS", "labelT", "labelT"
        ).equal([1, ["labelT", '"9072"', "labelT", "riceratops"]])

    env.expect(
        "ft.search", "idx2", "*", "RETURN", "4", "$.n", "AS", "labelT", "labelN"
    ).equal([1, "doc:1", ["labelT", "9072", "labelN", "9072"]])
    env.expect(
        "ft.aggregate", "idx2", "*", "LOAD", "4", "@$.n", "AS", "labelT", "labelN"
    ).equal([1, ["labelT", "9072", "labelN", "9072"]])
...
def testNumeric(env):
    ...
    env.expect("FT.SEARCH", "idx", "*", "RETURN", "3", "$.n", "AS", "int").equal(
        [1, "doc:1", ["int", "9"]]
    )
    env.expect(
        "FT.SEARCH", "idx", "@n:[0 10]", "RETURN", "3", "$.n", "AS", "int"
    ).equal([1, "doc:1", ["int", "9"]])
    env.expect("FT.SEARCH", "idx", "@f:[9.5 9.9]", "RETURN", "1", "f").equal(
        [1, "doc:1", ["f", "9.72"]]
    )
    env.expect(
        "FT.SEARCH", "idx", "@f:[9.5 9.9]", "RETURN", "3", "$.f", "AS", "flt"
...
def testLanguage(env):
    ...
    env.assertOk(
        conn.execute_command(
            "JSON.SET", "doc:1", "$", r'{"t":"traduzioni", "lang":"Italian"}'
        )
    )
    env.expect("ft.search", "idx", "tradu*", "RETURN", "1", "$.t").equal(
        [1, "doc:1", ["$.t", "traduzioni"]]
    )

    env.assertOk(
        conn.execute_command("JSON.SET", "doc:2", "$", r'{"domanda":"perché"}')
    )
    env.expect("ft.search", "idx2", "per*", "RETURN", "1", "$.domanda").equal(
        [1, "doc:2", ["$.domanda", "perché"]]
    )
...
def test_WrongJsonType(env):
    ...
    env.assertOk(
        conn.execute_command(
            "JSON.SET", "doc", "$", '{"object1":{"1":"foo", "2":"bar"}}'
        )
    )
    env.assertOk(
        conn.execute_command(
            "JSON.SET", "doc", "$", '{"object2":{"1":"foo", "2":"bar"}}'
        )
    )
    env.assertOk(
        conn.execute_command(
            "JSON.SET", "doc", "$", '{"object3":{"1":"foo", "2":"bar"}}'
        )
    )
    env.assertOk(
        conn.execute_command(
            "JSON.SET", "doc", "$", '{"object4":{"1":"foo", "2":"bar"}}'
        )
    )
    env.assertOk(
        conn.execute_command(
            "JSON.SET", "doc", "$", '{"object5":{"1":"foo", "2":"bar"}}'
        )
    )

    env.assertOk(
        conn.execute_command("JSON.SET", "doc", "$", '{"array1":["foo", "bar"]}')
    )
    env.assertOk(
        conn.execute_command("JSON.SET", "doc", "$", '{"array2":["foo", "bar"]}')
    )
    env.assertOk(
        conn.execute_command("JSON.SET", "doc", "$", '{"array3":["foo", "bar"]}')
    )
...
def testTagNoSeparetor(env):
    ...
    env.assertOk(
        conn.execute_command("JSON.SET", "doc:1", "$", '{"tag1":"foo,bar,baz"}')
    )
    env.assertOk(
        conn.execute_command("JSON.SET", "doc:2", "$", '{"tag2":["foo","bar,baz"]}')
    )

    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:1", "$"), '[{"tag1":"foo,bar,baz"}]'
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:1", "$.tag1"), '["foo,bar,baz"]'
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:2", "$"), '[{"tag2":["foo","bar,baz"]}]'
    )
    env.assertEqual(
        conn.execute_command("JSON.GET", "doc:2", "$.tag2[*]"), '["foo","bar,baz"]'
    )

    env.expect("FT.SEARCH", "idx", "@tag_list:{foo\\,bar\\,baz}").equal(
        [1, "doc:1", ["$", '{"tag1":"foo,bar,baz"}']]
    )
    env.expect("FT.SEARCH", "idx", "@tag_array:{bar\\,baz}").equal(
        [1, "doc:2", ["$", '{"tag2":["foo","bar,baz"]}']]
    )
...
def testImplicitUNF(env):
    ...
    env.assertEqual(
        info_res["attributes"][0][-1], "UNF"
    )  # UNF is implicit with SORTABLE on JSON
...
def testScoreField(env):
    ...
    env.assertOk(
        conn.execute_command(
            ...
            "tst:permit1",
            "$",
            r'{"_score":0.8, "description":"Fix the facade"}',
        )
    )
    env.assertOk(
        conn.execute_command(
            ...
            "tst:permit2",
            "$",
            r'{"_score":0.7, "description":"Fix the facade"}',
        )
    )
    env.assertOk(
        conn.execute_command(
            ...
            "tst:permit3",
            "$",
            r'{"_score":0.9, "description":"Fix the facade"}',
        )
    )
    ...
    res = [
        3,
        "tst:permit3",
        ["$", '{"_score":0.9,"description":"Fix the facade"}'],
        "tst:permit1",
        ["$", '{"_score":0.8,"description":"Fix the facade"}'],
        "tst:permit2",
        ["$", '{"_score":0.7,"description":"Fix the facade"}'],
    ]
...
def testTagArrayLowerCase(env):
    ...
    env.assertOk(
        conn.execute_command(
            ...
            "json1",
            "$",
            r'{"attributes":[{"name":"Brand1","value":"Vivo"}]}',
        )
    )
    env.assertOk(
        conn.execute_command(
            ...
            "json2",
            "$",
            r'{"attributes":[{"name":"Brand2","value":"Ext,vivo"}]}',
        )
    )
    ...
    env.expect(
        "FT.SEARCH", "idx2", "@attrs:{Vivo}", "SORTBY", "name", "NOCONTENT"
    ).equal([2, "json1", "json2"])
    env.expect(
        "FT.SEARCH", "idx2", "@attrs:{vivo}", "SORTBY", "name", "NOCONTENT"
    ).equal([2, "json1", "json2"])
...
def check_index_with_null(env, idx):
    expected = [
        ...
        [
            ...
            "1",
            "$",
            '{"sort":1,"num":null,"txt":"hello","tag":"world","geo":"1.23,4.56","vec":[0,1]}',
        ],
        "doc2",
        [
            "sort",
            "2",
            "$",
            '{"sort":2,"num":0.8,"txt":null,"tag":"world","geo":"1.23,4.56","vec":[0,1]}',
        ],
        "doc3",
        [
            "sort",
            "3",
            "$",
            '{"sort":3,"num":0.8,"txt":"hello","tag":null,"geo":"1.23,4.56","vec":[0,1]}',
        ],
        "doc4",
        [
            "sort",
            "4",
            "$",
            '{"sort":4,"num":0.8,"txt":"hello","tag":"world","geo":null,"vec":[0,1]}',
        ],
        "doc5",
        [
            "sort",
            "5",
            "$",
            '{"sort":5,"num":0.8,"txt":"hello","tag":"world","geo":"1.23,4.56","vec":null}',
        ],
    ]
...
def testNullValue(env):
    ...
    conn.execute_command(
        ...
        "doc1",
        "$",
        r'{"sort":1, "num":null, "txt":"hello", "tag":"world", "geo":"1.23,4.56", "vec":[0,1]}',
    )
    conn.execute_command(
        "JSON.SET",
        "doc2",
        "$",
        r'{"sort":2, "num":0.8, "txt":null, "tag":"world", "geo":"1.23,4.56", "vec":[0,1]}',
    )
    conn.execute_command(
        "JSON.SET",
        "doc3",
        "$",
        r'{"sort":3, "num":0.8, "txt":"hello", "tag":null, "geo":"1.23,4.56", "vec":[0,1]}',
    )
    conn.execute_command(
        "JSON.SET",
        "doc4",
        "$",
        r'{"sort":4, "num":0.8, "txt":"hello", "tag":"world", "geo":null, "vec":[0,1]}',
    )
    conn.execute_command(
        "JSON.SET",
        "doc5",
        "$",
        r'{"sort":5, "num":0.8, "txt":"hello", "tag":"world", "geo":"1.23,4.56", "vec":null}',
    )
...
def testVector_correct_eval(env):
    ...
    for data_type in ["FLOAT32", "FLOAT64"]:
        ...
        env.assertOk(
            conn.execute_command("JSON.SET", "j2", "$", r'{"vec":[1,-0.189207144]}')
        )
        env.assertOk(
            conn.execute_command("JSON.SET", "j3", "$", r'{"vec":[2.772453851,1]}')
        )
        ...
        expected_res = [
            4,
            "j1",
            ["score", spatial.distance.sqeuclidean(np.array([1, 1]), query_vec)],
            "j2",
            [
                "score",
                spatial.distance.sqeuclidean(np.array([1, -0.189207144]), query_vec),
            ],
            "j3",
            [
                "score",
                spatial.distance.sqeuclidean(np.array([2.772453851, 1]), query_vec),
            ],
            "j4",
            ["score", spatial.distance.sqeuclidean(np.array([-1, 1]), query_vec)],
        ]
        ...
        for i in range(1, len(expected_res), 2):
            ...
            if data_type == "FLOAT32":
                env.assertAlmostEqual(
                    expected_res[i + 1][1], float(actual_res[i + 1][1]), 1e-6
                )
            else:  # data type is float64, expect higher precision
                env.assertAlmostEqual(
                    expected_res[i + 1][1], float(actual_res[i + 1][1]), 1e-9
                )
        conn.execute_command("FT.DROPINDEX", "idx", "DD")
...
def testUpperLower():
    ...
    env.assertOk(
        conn.execute_command("JSON.SET", "group:1", "$", r'{"tags": ["tag1"]}')
    )
    env.expect(
        "FT.AGGREGATE",
        "groupIdx",
    ...
    env.expect(
        ...
        "AS",
        "low",
    ).equal([1, ["tags", '["TAG1"]', "low", "tag1"]])

    # expect the same result for multi-value case

    # validate the `upper` case
    env.assertOk(
        conn.execute_command("JSON.SET", "group:1", "$", r'{"tags": ["tag1", "tag2"]}')
    )
    env.expect(
        "FT.AGGREGATE",
        "groupIdx",
        ...
        "AS",
        "upp",
    ).equal([1, ["tags", '["tag1","tag2"]', "upp", "TAG1"]])
    ...
    env.expect(
        ...
        "AS",
        "low",
    ).equal([1, ["tags", '["TAG1","TAG2"]', "low", "tag1"]])
...
def testTagAutoescaping(env):
    ...
    # Test prefix
    res = env.cmd(
        "FT.SEARCH", "idx", '@tag:{"abc:"*}=>{$weight:3.4}', "NOCONTENT", "WITHCOUNT"
    )
    ...
    # Test suffix
    res = env.cmd(
        "FT.SEARCH", "idx", '@tag:{*"xyz:2"}=>{$weight:3.4}', "NOCONTENT", "WITHCOUNT"
    )
