...
def testSanitySearchPointWithin(env):
    ...
    res = env.cmd(
        ...
        2,
        "poly",
        "POLYGON((0 0, 0 250, 250 250, 250 0, 0 0))",
        "NOCONTENT",
        "DIALECT",
        3,
    )
...
def testSanitySearchJsonWithin(env):
    ...
    conn.execute_command(
        ...
        "small",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 100 100, 100 1, 1 1))"}',
    )
    conn.execute_command(
        "JSON.SET",
        "large",
        "$",
        '{"geom": "POLYGON((1 1, 1 200, 200 200, 200 1, 1 1))"}',
    )
...
def testSanitySearchJsonCombined(env):
    conn = getConnectionByEnv(env)
    env.expect(
        "FT.CREATE idx ON JSON SCHEMA $.geom AS geom GEOSHAPE FLAT $.name as name TEXT"
    ).ok()
    ...
    conn.execute_command(
        ...
        "p1",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 100 100, 100 1, 1 1))", "name": "Homer"}',
    )
    conn.execute_command(
        "JSON.SET",
        "p2",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 90 90, 100 1, 1 1))", "name": "Bart"}',
    )
    env.expect(
        "FT.SEARCH",
        "idx",
...
def testWKTIngestError(env):
    ...
    conn.execute_command(
        ...
        "p1",
        "$",
        '{"geom": "POLIKON((1 1, 1 100, 100 100, 100 1, 1 1))", "name": "Homer"}',
    )
    env.assertContains(
        "Error indexing geoshape: Should start with 'POLYGON'", get_last_error()
    )
    # Missing parenthesis
    conn.execute_command(
        "JSON.SET",
        "p2",
        "$",
        '{"geom": "POLYGON(1 1, 1 100, 100 100, 100 1, 1 1))", "name": "Patty"}',
    )
    env.assertContains("Error indexing geoshape: Expected '(' at '1'", get_last_error())
    # Zero coordinates
    conn.execute_command(
        "JSON.SET", "p6", "$", '{"geom": "POLYGON(()())", "name": "Mr. Burns"}'
    )
    env.assertContains(
        "Error indexing geoshape: invalid geometry: Geometry has too few points",
        get_last_error(),
    )
    # Too few coordinates
    conn.execute_command(
        ...
        "p7",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 1 1))", "name": "Milhouse"}',
    )
    env.assertContains(
        "Error indexing geoshape: invalid geometry: Geometry has too few points",
        get_last_error(),
    ...
    conn.execute_command(
        ...
        "p8",
        "$",
        '{"geom": "POLYGON((1 1, 1 200, 1 100, 100 1, 1 1))", "name": "Marge"}',
    )
    env.assertContains(
        "Error indexing geoshape: invalid geometry: Geometry has spikes",
        get_last_error(),
    )
    # Self-intersection
    conn.execute_command(
        ...
        "p9",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 50 50, 50 -50, 1 150, 100 50, 1 1))", "name": "Lisa"}',
    )
    env.assertContains(
        "Error indexing geoshape: invalid geometry: Geometry has invalid self-intersections",
        get_last_error(),
    )
    ...
    conn.execute_command(
        ...
        "p10",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 100 100, 100 1, 1 1) (25 150, 75 150, 75 250, 25 250, 25 150))", "name": "Sideshow Bob"}',
    )
    env.assertContains(
        "Error indexing geoshape: invalid geometry: Geometry has interior rings defined outside the outer boundary",
        get_last_error(),
    )
    # Nested interiors
    conn.execute_command(
        ...
        "p11",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 100 100, 100 1, 1 1) (25 25, 75 25, 75 75, 25 75, 25 25) (49 49, 51 49, 51 51, 49 51, 49 49))", "name": "Krusty"}',
    )
    env.assertContains(
        "Error indexing geoshape: invalid geometry: Geometry has nested interior rings",
        get_last_error(),
    )
    # Invalid coordinates
    conn.execute_command(
        ...
        "p14",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 100 100, bart, 1 1))", "name": "Bart"}',
    )
    ...
    conn.execute_command(
        ...
        "p3",
        "$",
        '{"geom": "POLYGON((1 1, 1 , 100 100, 100 1, 1 1))", "name": "Moe"}',
    )
    ...
    conn.execute_command(
        ...
        "p4",
        "$",
        '{"geom": "POLYGON((1 1, 1 100 100 100, 100 1, 1 1))", "name": "Seymour"}',
    )
    ...
    conn.execute_command(
        ...
        "p5",
        "$",
        '{"geom": "POLYGON((1 1 1 100, 100 100, 100 1, 1 1))", "name": "Ned"}',
    )
    ...
    conn.execute_command(
        ...
        "p13",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 100 100, 100 1, 100 1, 1 1))", "name": "Selma"}',
    )
    # Hourglass
    conn.execute_command(
        "JSON.SET",
        "p15",
        "$",
        '{"geom": "POLYGON((1 1, 1 100, 50 50, 50 -50, 1 1))", "name": "Maggie"}',
    )
...
def testWKTQueryError(env):
    ...
    env.expect(
        "FT.SEARCH",
        "idx",
        "@name:(Ho*) @geom:[within POLYGON((0 0, 0 150, 150 150, 150 0, 0 0))]",
        "NOCONTENT",
        "DIALECT",
        3,
    ).error().contains("Syntax error")
    ...
    env.expect(
        ...
        2,
        "poly",
        "POLIGON((0 0, 0 150, 150 150, 150 0, 0 0))",
        "NOCONTENT",
        "DIALECT",
        3,
    ).error()
    ...
    env.expect(
        ...
        2,
        "poly",
        "POLIGON((0 0, a 150, 150 150, 150 0, 0 0))",
        "NOCONTENT",
        "DIALECT",
        3,
    ).error()
...
def testSimpleUpdate(env):
    ...
    conn.execute_command(
        "HSET", "k1", "geom", "POLYGON((1 1, 1 100, 100 100, 100 1, 1 1))"
    )
    conn.execute_command(
        "HSET", "k2", "geom", "POLYGON((1 1, 1 200, 200 200, 200 1, 1 1))"
    )
    conn.execute_command(
        "HSET",
        "k3",
        "geom",
        "POLYGON((1 1, 1 200, 200 200, 200 1, 1 1))",
        "geom2",
        "POLYGON((1 1, 1 140, 140 140, 140 1, 1 1))",
    )
    ...
    # Update
    conn.execute_command(
        "HSET", "k2", "geom", "POLYGON((1 1, 1 120, 120 120, 120 1, 1 1))"
    )
    # Dump geoshape index
    assert_index_num_docs(env, "idx", "geom", 3)
    assert_index_num_docs(env, "idx", "geom2", 1)
    ...
    res = env.cmd(
        ...
        2,
        "poly",
        "POLYGON((2 2, 2 150, 150 150, 150 2, 2 2))",
        "DIALECT",
        3,
    )
...
def testFieldUpdate(env):
    ...
    env.expect(
        ...
        2,
        "poly",
        "POLYGON((1 1, 1 150, 150 150, 150 1, 1 1))",
        "DIALECT",
        3,
    ).equal([1, "k1", [*field1, *field2]])
    ...
    env.expect(
        ...
        2,
        "poly",
        "POLYGON((1 1, 1 170, 170 170, 170 1, 1 1))",
        "DIALECT",
        3,
    ).equal([1, "k1", [*field1, *field2]])
...
def testFtInfo(env):
    ...
    env.expect(
        "FT.CREATE", "idx1", "SCHEMA", "geom", "GEOSHAPE", "FLAT", "txt", "TEXT"
    ).ok()
    ...
    conn.execute_command(
        "HSET", "doc1", "txt", "Not a real POLYGON((1 1, 1 100, 100 100, 100 1, 1 1))"
    )
    res = to_dict(env.cmd("FT.INFO idx1"))
    ...
    for i in range(1, doc_num + 1):
        conn.execute_command(
            ...
            f"doc{i}",
            "geom",
            f"POLYGON(({2*i} {2*i}, {2*i} {100+2*i}, {100+2*i} {100+2*i}, {100+2*i} {2*i}, {2*i} {2*i}))",
        )
