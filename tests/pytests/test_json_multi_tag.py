...
def testMultiTagReturnSimple(env):
    ...
    env.expect(
        "FT.SEARCH", "idx1", "@category:{logic}", "RETURN", "1", "category"
    ).equal(res1)
    env.expect(
        "FT.SEARCH",
        "idx1",
...
def testMultiTagBool(env):
    ...
    conn.execute_command(
        ...
        "doc:1",
        "$",
        '{"foo": {"bar": [true, true]}, "fu": {"bar": [false, true]}}',
    )
    conn.execute_command(
        "JSON.SET",
        "doc:2",
        "$",
        '{"foo": {"bar": [true, true]}, "fu": {"bar": [true, true]}}',
    )
    conn.execute_command(
        "JSON.SET",
        "doc:3",
        "$",
        '{"foo": {"bar": [false, false]}, "fu": {"bar": [false, false]}}',
    )
    env.expect(
        "FT.CREATE",
        "idx_multi",
    ...
    env.assertEqual(toSortedFlatList(res), toSortedFlatList([2, "doc:2", "doc:1"]))
    env.expect("FT.SEARCH", "idx_single", "@bar:{false}", "NOCONTENT").equal(
        [1, "doc:3"]
    )
...
def testMultiTag(env):
    ...
    env.cmd(
        ...
        "AS",
        "author",
        "TAG",  # testing root path, so reuse the single top-level value
        "$",
        "AS",
        "category",
        "TAG",
...
def searchMultiTagCategory(env):
    ...
    for idx in ["idx_category_arr", "idx_category_arr_author_flat"]:
        ...
        env.assertEqual(
            toSortedFlatList(res), toSortedFlatList([1, "doc:3"]), message="B " + idx
        )

        env.expect(
            "FT.SEARCH", idx, "@category:{high\ performance}", "NOCONTENT"
        ).equal([1, "doc:2"])
...
def searchMultiTagAuthor(env):
    ...
    for idx in ["idx_author_flat"]:
        env.expect("FT.SEARCH", idx, "@author:{Donald\ Knuth}", "NOCONTENT").equal(
            [1, "doc:1"]
        )
...
def checkMultiTagReturn(env, expected, default_dialect, is_sortable, is_sortable_unf):
    ...
    env.assertEqual(
        ...
        message="dialect {}, sortable {}, unf {}".format(
            dialect_param, is_sortable, is_sortable_unf
        ),
    )
    ...
    env.expect(
        ...
        "arr_1",
        *dialect_param
    ).equal(expect_case(expected[0]))
    env.expect(
        "FT.SEARCH", "idx_flat", expr, "RETURN", "1", "val", *dialect_param
    ).equal(expect_case(expected[3]))
    env.expect(
        "FT.SEARCH",
        "idx_flat",
        expr,
