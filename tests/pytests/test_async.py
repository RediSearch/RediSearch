...
def testAlterIndex(env):
    ...
    for i in range(N):
        res = conn.execute_command(
            "hset", "foo:%d" % i, "name", "john doe", "age", str(10 + i)
        )
...
def test_mod4745(env):
    ...
    for i in range(N):
        res = conn.execute_command(
            ...
            "foo:%d" % i,
            "name",
            f"some string with information to index in the "
...
def test_eval_node_errors_async():
    ...
    for i in range(n_docs):
        env.assertEqual(
            conn.execute_command(
                ...
                "hello",
                "v",
                create_np_array_typed([i / 1000] * dim).tobytes(),
            ),
            2,
        )
    ...
    env.expect("ft.search", "idx", "@foo:*ell*", "NOCONTENT").error().contains(
        "Contains query on fields without WITHSUFFIXTRIE support"
    )
    env.expect(
        "FT.SEARCH", "idx", "*=>[KNN 2 @v $b]", "PARAMS", "2", "b", "abcdefg"
    ).error().contains(
        "Error parsing vector similarity query: query vector blob size (7) does not match"
        f" index's expected size ({dim*4})."
    )
    env.expect(
        "FT.SEARCH",
        "idx",
        "@v:[VECTOR_RANGE 10000000 $vec_param]",
        "NOCONTENT",
        "LIMIT",
        0,
        n_docs,
        ...
        "TIMEOUT",
        1,
    ).error().equal("Timeout limit was reached")
    ...
    env.expect(
        ...
        "b",
        create_np_array_typed([0] * dim).tobytes(),
    ).error().contains(f"Property `v` already exists in schema")
