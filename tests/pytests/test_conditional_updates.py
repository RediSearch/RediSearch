...
def testConditionalUpdateOnNoneExistingNumericField(env):
    env.assertOk(
        env.cmd(
            "ft.create", "idx", "ON", "HASH", "schema", "id1", "numeric", "SORTABLE"
        )
    )
    ...
    env.assertOk(
        env.cmd("FT.ALTER", "idx", "SCHEMA", "ADD", "id2", "numeric", "SORTABLE")
    )
...
def testConditionalUpdateOnNoneExistingTextField(env):
    env.assertOk(
        env.cmd("ft.create", "idx", "ON", "HASH", "schema", "id1", "text", "SORTABLE")
    )
...
def testConditionalUpdateOnNoneExistingTagField(env):
    env.assertOk(
        env.cmd("ft.create", "idx", "ON", "HASH", "schema", "id1", "tag", "SORTABLE")
    )
    env.assertOk(env.cmd("ft.add", "idx", "doc1", 1.0, "fields", "id1", "some_text"))
