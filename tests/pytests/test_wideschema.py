...
from common import arch_int_bits, waitForIndex
...
def testWideSchema(env):
    ...
    for _ in env.reloadingIterator():
        ...
        for i in range(FIELDS):
            ...
            res = env.cmd(
                "ft.search", "idx", "@field_%d:token_%d" % (i, i), "NOCONTENT"
            )
            env.assertEqual(res[0], N)

            res = env.cmd(
                "ft.explain", "idx", "@field_%d:token_%d" % (i, i), "VERBATIM"
            ).strip()
            env.assertEqual("@field_%d:token_%d" % (i, i), res)

            res = env.cmd(
                "ft.search", "idx", "hello @field_%d:token_%d" % (i, i), "NOCONTENT"
            )
            env.assertEqual(res[0], N)
        ...
        res = env.cmd(
            "ft.search",
            "idx",
            " ".join(("@field_%d:token_%d" % (i, i) for i in range(FIELDS))),
        )
        env.assertEqual(res[0], N)

        res = env.cmd(
            "ft.search", "idx", " ".join(("token_%d" % (i) for i in range(FIELDS)))
        )
        env.assertEqual(res[0], N)
