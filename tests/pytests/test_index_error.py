...
def test_multiple_index_failures_json(env):
    ...
    for _ in env.reloadingIterator():
      for idx in ['idx1', 'idx2']:
          ...
          expected_error_dict = {
              indexing_failures_str: 1,
              last_indexing_error_str: f"SEARCH_JSON_TYPE_BAD: Invalid JSON type: String type can represent only TEXT, TAG, GEO or GEOMETRY field",
              last_indexing_error_key_str: 'doc',
              'background indexing status': 'OK',
          }
          ...
          expected_failed_field_stats = [
              ...
              ['indexing failures', 1, 'last indexing error',
              f"SEARCH_JSON_TYPE_BAD: Invalid JSON type: String type can represent only TEXT, TAG, GEO or GEOMETRY field",
              'last indexing error key', 'doc']
          ]
