...
def test_1414(env):
    ...
    env.expect("ft.search", "idx", "*", "limit", "0", "1234567").error().contains(
        "LIMIT exceeds maximum of 1000000"
    )
    env.expect(config_cmd(), "set", "MAXSEARCHRESULTS", "-1").ok()
    env.assertEqual(
        toSortedFlatList(env.cmd("ft.search", "idx", "*", "limit", "0", "1234567")),
...
def test_1601(env):
    ...
    conn.execute_command(
        "HSET", "movie:1", "title", "Star Wars: Episode I - The Phantom Menace"
    )
    conn.execute_command(
        "HSET", "movie:2", "title", "Star Wars: Episodes II - Attack of the Clones"
    )
    conn.execute_command(
        "HSET", "movie:3", "title", "Star Wars: Episode III - Revenge of the Sith"
    )
...
def testMultiSortby(env):
    ...
    env.expect("ft.search idx foo nocontent sortby t1 sortby t3").error().contains(
        "Multiple SORTBY steps are not allowed"
    )
    env.expect(
        "ft.aggregate idx foo nocontent sortby 2 @t1 asc sortby 2 @t3 desc"
    ).error().contains(
...
def test_MOD_865(env):
    ...
    env.expect(*args_list).error().contains(
        "Schema is limited to {} TEXT fields".format(arch_int_bits())
    )
    env.expect("FT.DROPINDEX", "idx")
...
def test_issue1826(env):
    ...
    env.expect("FT.SEARCH", "idx", "boy with glasses").equal(
        [1, "doc", ["t", "boy with glasses"]]
    )
    env.expect("FT.SEARCH", "idx", "boy With glasses").equal(
        [1, "doc", ["t", "boy with glasses"]]
    )
...
def test_issue1834(env):
    ...
    env.expect("FT.SEARCH", "idx", "hell|hello", "HIGHLIGHT").equal(
        [1, "doc", ["t", "<b>hell</b> <b>hello</b>"]]
    )
...
def test_issue1932(env):
    ...
    env.expect(
        ...
        "1",
        "@t",
    ).error().contains("OFFSET exceeds maximum of 1000000")
...
def test_issue1988(env):
    ...
    env.expect("FT.SEARCH", "idx", "foo", "WITHSCORES").equal(
        [1, "doc1", "1", ["t", "foo"]]
    )
    env.expect("FT.SEARCH", "idx", "foo", "SORTBY", "t").equal(
        [1, "doc1", ["t", "foo"]]
    )
    env.expect("FT.SEARCH", "idx", "foo", "WITHSCORES", "SORTBY", "t").equal(
        [1, "doc1", "1", ["t", "foo"]]
    )
...
def test_MOD1266(env):
    ...
    env.expect(
        "FT.SEARCH", "idx", "*", "sortby", "n2", "DESC", "RETURN", "1", "n2"
...
def testMemAllocated(env):
    ...
    assertInfoField(
        env, "idx1", "key_table_size_mb", "2.765655517578125e-05", delta=0.01
    )
    conn.execute_command("HSET", "doc2", "t", "hello world")
    assertInfoField(
        env, "idx1", "key_table_size_mb", "8.296966552734375e-05", delta=0.01
    )
    conn.execute_command("HSET", "d3", "t", "help")
    assertInfoField(
        env, "idx1", "key_table_size_mb", "0.00013828277587890625", delta=0.01
    )
    ...
    assertInfoField(
        env, "idx2", "key_table_size_mb", "0.027684211730957031", delta=0.01
    )
...
def testUNF(env):
    ...
    conn.execute_command(
        "HSET", "doc1", "txt", "FOO", "txt_unf", "FOO", "tag", "FOO", "tag_unf", "FOO"
    )
    ...
    env.expect(
        ...
        "@tag",
        "@tag_unf",
    ).equal([1, ["txt", "masse", "txt_unf", "Maße", "tag", "masse", "tag_unf", "Maße"]])
    ...
    env.expect(
        ...
        "@tag",
        "@tag_unf",
    ).equal([1, ["txt", "Maße", "txt_unf", "Maße", "tag", "Maße", "tag_unf", "Maße"]])
...
def test_MOD_1517(env):
    ...
    # first tag is nil
    conn.execute_command(
        "HSET", "doc2", "field2", "val2", "amount1", "1", "amount2", "1"
    )
    # second tag is nil
    conn.execute_command(
        "HSET", "doc3", "field1", "val1", "amount1", "1", "amount2", "1"
    )
    # both tags are nil
    ...
    res = [
        4,
        ["field1", None, "field2", None, "amount1Sum", "1", "amount2Sum", "1"],
...
def test_MOD1544(env):
    ...
    env.expect(
        "FT.SEARCH", "idx", "@name:(John)", "RETURN", "1", "name", "HIGHLIGHT"
    ).equal(res)
    env.expect(
        "FT.SEARCH",
        "idx",
    ...
    env.assertEqual(
        ...
        toSortedFlatList(
            [
                4,
                "doc2",
                ["t", "<span style='background-color:yellow'>world2</span>... "],
                "doc1",
                ["t", "<span style='background-color:yellow'>world1</span>... "],
                "doc0",
                ["t", "world0"],
                "doc3",
                ["t", "world3"],
...
def test_2370(env):
    ...
    # missing fields
    env.expect("FT.SEARCH", "idx", "*").equal(
        [2, "doc1", ["t1", "foo", "t2", "bar"], "doc2", ["t1", "baz"]]
    )


def test_MOD1907(env):
    # Test FT.CREATE w/o fields parameters
    env.expect("FT.CREATE", "idx", "ON", "HASH", "SCHEMA").error().contains(
        "Fields arguments are missing"
    )
    env.expect("FT.CREATE", "idx", "STOPWORDS", 0, "SCHEMA").error().contains(
        "Fields arguments are missing"
    )
...
def test_SkipFieldWithNoMatch(env):
    ...
    env.assertEqual(
        res[1][1][0][3], ["Type", "TEXT", "Term", "foo", "Counter", 1, "Size", 1]
    )
    ...
    env.assertEqual(
        res[1][1][0][3], ["Type", "TEXT", "Term", "bar", "Counter", 1, "Size", 1]
    )
...
def testOverMaxResults():
    ...
    for c in commands:
        ...
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "1", "20").equal(
            [res[0], *res[2:]]
        )
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "5", "10").equal(
            [res[0], *res[6:11]]
        )
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "10", "10").equal(
            [10]
        )
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "20", "10").equal(
            [10]
        )
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "30", "10").equal(
            "OFFSET exceeds maximum of 20"
        )
        ...
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "1", "20").equal(
            [res[0], *[str(i) for i in range(1, 20, 1)]]
        )
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "15", "10").equal(
            [20, *res[6:11]]
        )
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "20", "10").equal(
            [20]
        ...
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "1", "20").equal(
            [30, *[str(i) for i in range(1, 20, 1)]]
        )
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "10", "10").equal(
            [30, *res[1:11]]
        )
        ...
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "25", "10").equal(
            "OFFSET exceeds maximum of 20"
        )
        env.expect("FT.SEARCH", "idx", "*", "NOCONTENT", "LIMIT", "30", "10").equal(
            "OFFSET exceeds maximum of 20"
...
def test_MOD_3372(env):
    ...
    env.expect("FT.EXPLAIN", "idx", "foo").equal(
        "UNION {\n  foo\n  +foo(expanded)\n}\n"
    )
    env.expect("FT.EXPLAIN", "idx", "foo", "verbatim").equal("foo\n")
    env.expect("FT.EXPLAIN", "non-exist", "foo").error().equal(
        "non-exist: no such index"
    )
    ...
    if not env.isCluster():
        ...
        env.expect("FT.EXPLAINCLI", "idx", "foo").equal(
            ["UNION {", "  foo", "  +foo(expanded)", "}", ""]
        )
        env.expect("FT.EXPLAINCLI", "idx", "foo", "verbatim").equal(["foo", ""])
        env.expect("FT.EXPLAINCLI", "non-exist", "foo").error().equal(
            "non-exist: no such index"
        )
...
def test_MOD_3540(env):
    ...
    env.expect(
        "FT.SEARCH", "idx", "*", "SORTBY", "t", "DESC", "MAX", "1"
    ).error().contains("SORTBY MAX is not supported by FT.SEARCH")

    env.expect(
        "FT.AGGREGATE", "idx", "*", "SORTBY", "2", "@t", "DESC", "MAX", "1", "LOAD", "*"
    ).equal([10, ["t", "9"]])
...
def test_sortby_Noexist(env):
    ...
    env.expect("FT.SEARCH", "idx", "*", "SORTBY", "t", "ASC", "LIMIT", "0", "2").equal(
        [4, "doc1", ["t", "1"], "doc3", ["t", "3"]]
    )
    env.expect("FT.SEARCH", "idx", "*", "SORTBY", "t", "DESC", "LIMIT", "0", "2").equal(
        [4, "doc3", ["t", "3"], "doc1", ["t", "1"]]
    )
    ...
    if not env.isCluster():
        env.expect(
            "FT.SEARCH", "idx", "*", "SORTBY", "t", "ASC", "LIMIT", "0", "3"
        ).equal(
            [4, "doc1", ["t", "1"], "doc3", ["t", "3"], "doc2", ["somethingelse", "2"]]
        )
        env.expect(
            "FT.SEARCH", "idx", "*", "SORTBY", "t", "DESC", "LIMIT", "0", "3"
        ).equal(
            [4, "doc3", ["t", "3"], "doc1", ["t", "1"], "doc4", ["somethingelse", "4"]]
        )
...
def test_sortby_Noexist_Sortables(env):
    ...
    for count, args in enumerate(sortable_options):
        ...
        res = conn.execute_command(
            "FT.SEARCH", "idx{}".format(count), "*", "sortby", "numval", "ASC"
        )
        env.assertEqual(
            res,
            [
        ...
        res = conn.execute_command(
            "FT.SEARCH", "idx{}".format(count), "*", "sortby", "numval", "DESC"
        )
        env.assertEqual(
            res,
            [
...
def test_mod_4207(env):
    ...
    env.expect("FT.SEARCH", "idx1", "*", "NOCONTENT").equal(
        [3, "address:1", "address:2", "address:4"]
    )
    env.expect("FT.SEARCH", "idx2", "*", "NOCONTENT").equal(
        [3, "address:1", "address:2", "address:3"]
    )
...
def test_mod_4255(env):
    ...
    # get first result
    res = env.cmd(
        "FT.AGGREGATE", "idx", "*", "LOAD", "1", "@test", "WITHCURSOR", "COUNT", "1"
    )
...
def test_as_startswith_as(env):
    ...
    env.expect(
        "FT.CREATE", "idx", "ON", "JSON", "SCHEMA", "$.attr1", "AS", "asa", "TEXT"
    ).equal("OK")
    conn.execute_command("JSON.SET", "doc2", "$", '{"attr1": "foo", "attr2": "bar"}')

    env.expect("FT.SEARCH", "idx", "@asa:(foo)", "RETURN", 1, "asa").equal(
        [1, "doc2", ["asa", "foo"]]
    )
    env.expect("FT.SEARCH", "idx", "@asa:(foo)", "RETURN", 3, "asa", "AS", "asa").equal(
        [1, "doc2", ["asa", "foo"]]
    )
    env.expect(
        "FT.SEARCH", "idx", "@asa:(foo)", "RETURN", 3, "$.attr1", "AS", "asa"
    ).equal([1, "doc2", ["asa", "foo"]])
    env.expect(
        "FT.SEARCH", "idx", "@asa:(foo)", "RETURN", 3, "$.attr1", "AS", "$.attr2"
    ).equal([1, "doc2", ["$.attr2", "foo"]])
    env.expect(
        "FT.SEARCH", "idx", "@asa:(foo)", "RETURN", 3, "asa", "AS", "$.attr2"
    ).equal([1, "doc2", ["$.attr2", "foo"]])

    env.expect("FT.AGGREGATE", "idx", "@asa:(foo)", "LOAD", 1, "asa").equal(
        [1, ["asa", "foo"]]
    )
    env.expect(
        "FT.AGGREGATE", "idx", "@asa:(foo)", "LOAD", 3, "asa", "AS", "asa"
    ).equal([1, ["asa", "foo"]])
    env.expect(
        "FT.AGGREGATE", "idx", "@asa:(foo)", "LOAD", 3, "$.attr1", "AS", "asa"
    ).equal([1, ["asa", "foo"]])
    env.expect(
        "FT.AGGREGATE", "idx", "@asa:(foo)", "LOAD", 3, "$.attr1", "AS", "$.attr2"
    ).equal([1, ["$.attr2", "foo"]])
    env.expect(
        "FT.AGGREGATE", "idx", "@asa:(foo)", "LOAD", 3, "asa", "AS", "$.attr2"
    ).equal([1, ["$.attr2", "foo"]])
...
def test_mod4296_badexpr(env):
    ...
    env.expect(
        "FT.AGGREGATE", "idx", "foo", "LOAD", 1, "@t", "APPLY", "1%0", "as", "foo"
    ).equal([1, ["t", "foo", "foo", "nan"]])
    env.expect(
        "FT.AGGREGATE", "idx", "foo", "LOAD", 1, "@t", "APPLY", "1/0", "as", "foo"
    ).equal([1, ["t", "foo", "foo", "nan"]])
...
def test_mod5062(env):
    ...
    aggregate_profile = env.cmd(
        "FT.PROFILE", "idx", "AGGREGATE", "QUERY", "hello", "SORTBY", "1", "@t"
    )
...
def test_mod5252(env):
    ...
    res = env.cmd(
        "FT.AGGREGATE", "idx", "*", "LOAD", "1", "@__key", "SORTBY", "1", "@__key"
    )
...
def test_mod_6276(env):
    ...
    # Actual Test
    env.expect(
        debug_cmd(), "GC_STOP_SCHEDULE", "idx"
    ).ok()  # Stop the gc from running uncontrollably
    env.expect(debug_cmd(), "GC_WAIT_FOR_JOBS").equal(
        "DONE"
    )  # Make sure there are no running gc jobs
    env.expect("MULTI").ok()  # Start an atomic transaction:
    env.cmd(
        debug_cmd(), "GC_CONTINUE_SCHEDULE", "idx"
    )  # 1. Reschedule the gc - add a job to the queue
...
def test_mod5791(env):
    ...
    env.assertEqual(
        2, con.execute_command("HSET", "doc1", "t", "Hello world", "v", "abcdefgh")
    )
    env.assertEqual(
        2, con.execute_command("HSET", "doc2", "t", "Hello world", "v", "abcdefgi")
    )
    ...
    res = env.cmd(
        "FT.SEARCH",
        "idx",
        "(@v:[VECTOR_RANGE 0.8 $blob] @t:hello) | @v:[VECTOR_RANGE 0.8 $blob]",
        "WITHSCORES",
        "DIALECT",
        "2",
        "params",
...
def mod5778_add_new_shard_to_cluster(env: Env):
    ...
    env.assertEqual(
        conn.execute_command("search.clusterinfo")[:2],
        ["num_partitions", int(initial_shards_count)],
    )
    ...
    env.assertEqual(
        int(new_shard_conn.execute_command("cluster info")["cluster_known_nodes"]),
        initial_shards_count + 1,
    )
    env.assertEqual(
        new_shard_conn.execute_command("search.clusterinfo")[:2],
        ["num_partitions", int(initial_shards_count)],
    )
    ...
    env.assertEqual(
        new_shard_conn.execute_command(
            f"CLUSTER SETSLOT 0 IMPORTING {source_shard_id}"
        ),
        "OK",
    )
    env.assertEqual(
        conn.execute_command(f"CLUSTER SETSLOT 0 MIGRATING {new_shard_id}"), "OK"
    )
    env.assertEqual(
        new_shard_conn.execute_command(f"CLUSTER SETSLOT 0 NODE {new_shard_id}"), "OK"
    )
    env.assertEqual(
        conn.execute_command(f"CLUSTER SETSLOT 0 NODE {new_shard_id}"), "OK"
    )
    ...
    env.assertEqual(
        len(unique_shards),
        initial_shards_count + 1,
        message=f"cluster info is {cluster_info}",
    )

    # Verify that slot 0 moved to the new shard,
    shards_with_slot_0 = [shard for shard in cluster_info[9:] if shard[0] == 0]
    env.assertEqual(
        len(shards_with_slot_0), 1, message=f"cluster info is {cluster_info}"
    )
    env.assertEqual(
        shards_with_slot_0[0][2][0],
        new_shard_id,
        message=f"cluster info is {cluster_info}",
    )
...
def test_mod5910(env):
    ...
    res = env.execute_command(
        "FT.PROFILE", "idx", "search", "query", "(@n:[1 3] (@t:one | @t:two))"
    )
    ...
    env.assertEqual(
        "OK",
        con.execute_command(
            config_cmd(), "SET", "_PRIORITIZE_INTERSECT_UNION_CHILDREN", "true"
        ),
    )
    res = con.execute_command(
        "FT.PROFILE", "idx", "search", "query", "(@n:[1 3] (@t:one | @t:two))"
    )
    iterators_profile = to_dict(res[1][1][0])["Iterators profile"]
    env.assertEqual(iterators_profile[1], "INTERSECT")
...
def test_mod_4374(env):
    ...
    print(
        conn.execute_command(
            "FT.SEARCH", "idx", "val|unique", "withscores", "nocontent"
        )
    )
...
def test_mod_4375(env):
    ...
    print(
        conn.execute_command(
            "FT.SEARCH", "idx", "(-@t:even | @n:[0 5])", "nocontent", "dialect", "2"
        )
    )
...
def test_mod6510_vecsim_hybrid_adhoc_timeout(env):
    ...
    env.expect(
        f"FT.CREATE idx SCHEMA v VECTOR HNSW 10 DIM {dim} DISTANCE_METRIC L2 TYPE FLOAT32 M 2 EF_CONSTRUCTION 5"
        f" t text"
    ).equal("OK")
    ...
    env.expect(
        "FT.SEARCH",
        "idx",
        "meta=>[KNN 5 @v $vec_param HYBRID_POLICY ADHOC_BF]",
        "NOCONTENT",
        "PARAMS",
        2,
        "vec_param",
...
def test_mod_6541(env: Env):
    ...
    # Test Lua
    for cmd in cmds:
        env.expect("EVAL", f"return redis.call{cmd}", "0").error().contains(
            expect_error(cmd)
        )
...
def test_4732(env):
    ...
    env.expect("FT.SEARCH", "idx", "@txt:(hello\\\\)", "NOCONTENT").equal(
        [2, "doc1", "doc2"]
    )
    env.expect("FT.SEARCH", "idx", "@txt:(world)", "NOCONTENT").equal(
...
def test_unsafe_simpleString_values():
    ...
    env.expect(
        "FT.CREATE", unsafe_index, "PREFIX", "1", unsafe_value, "SCHEMA", "t", "TEXT"
    ).ok()
    ...
    expected = {
        "identifier": escape(unsafe_field),
        "attribute": escape(unsafe_field),
        "SEPARATOR": "\\n",
    }
    ...
    env.assertEqual(
        error_info["last indexing error key"], unsafe_value
    )  # key is not escaped
    ...
    expected = [
        {
            "id": unsafe_value,
            "extra_attributes": {unsafe_field: "tag\r1\ntag\r2"},
            "values": [],
        }
    ]
    env.expect(
