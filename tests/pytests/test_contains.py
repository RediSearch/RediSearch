...
def testWITHSUFFIXTRIEParamText(env):
    ...
    env.expect(
        "ft.create", "idx", "schema", "t", "TEXT", "SORTABLE", "WITHSUFFIXTRIE"
    ).error()
    env.expect(
        "ft.create", "idx", "schema", "t", "TEXT", "SORTABLE", "NOSTEM"
    ).error()  # sortable must be last
    ...
    # with sortable
    env.expect(
        "ft.create", "idx_sortable", "schema", "t", "TEXT", "WITHSUFFIXTRIE", "SORTABLE"
    ).ok()
    res_info = [
        [
            "identifier",
    ...
    # nostem 1st
    env.expect(
        "ft.create", "idx_nostem1", "schema", "t", "TEXT", "WITHSUFFIXTRIE", "NOSTEM"
    ).ok()
    res_info = [
        [
            "identifier",
    ...
    # nostem 2nd
    env.expect(
        "ft.create", "idx_nostem2", "schema", "t", "TEXT", "NOSTEM", "WITHSUFFIXTRIE"
    ).ok()
...
def testWITHSUFFIXTRIEParamTag(env):
    conn = getConnectionByEnv(env)
    env.expect(
        "ft.create", "idx", "schema", "t", "TAG", "SORTABLE", "WITHSUFFIXTRIE"
    ).error()
    env.expect(
        "ft.create", "idx", "schema", "t", "TAG", "SORTABLE", "CASESENSITIVE"
    ).error()  # sortable must be last
    ...
    # with sortable
    env.expect(
        "ft.create", "idx_sortable", "schema", "t", "TAG", "WITHSUFFIXTRIE", "SORTABLE"
    ).ok()
    res_info = [
        [
            "identifier",
...
def testBasicContains(env):
    ...
    conn.execute_command(
        "HSET", "doc1", "title", "hello world", "body", "this is a test"
    )
...
def testSanity(env: Env):
    ...
    env.expect("ft.search", index_list[0], "foo*", "LIMIT", 0, 0).error().contains(
        "Timeout limit was reached"
    )
    env.expect("ft.search", index_list[1], "foo*", "LIMIT", 0, 0).error().contains(
        "Timeout limit was reached"
    )
...
def testSanityTags(env):
    ...
    for i in range(len(index_list)):
        ...
        env.expect("ft.search", index_list[i], "@t:{*ooo1*}", "LIMIT", 0, 0).equal(
            [222]
        )
    ...
    env.expect(config_cmd(), "set", "TIMEOUT", 1).ok()
    env.expect(config_cmd(), "set", "ON_TIMEOUT", "RETURN").ok()
    env.expect("ft.search", index_list[0], "@t:{foo*}", "LIMIT", 0, 0).error().contains(
        "Timeout limit was reached"
    )
    env.expect("ft.search", index_list[1], "@t:{foo*}", "LIMIT", 0, 0).error().contains(
        "Timeout limit was reached"
    )
...
def testContainsGC(env):
    ...
    conn = getConnectionByEnv(env)
    conn.execute_command(
        "FT.CREATE", "idx", "SCHEMA", "t", "TEXT", "WITHSUFFIXTRIE", "SORTABLE"
    )
...
def testContainsGCTag(env):
    ...
    conn = getConnectionByEnv(env)
    conn.execute_command(
        "FT.CREATE", "idx", "SCHEMA", "t", "TAG", "WITHSUFFIXTRIE", "SORTABLE"
    )
...
def testContainsDebugCommand(env):
    ...
    env.expect(debug_cmd(), "DUMP_SUFFIX_TRIE", "idx", "field").error().contains(
        "Could not find given field in index spec"
    )

    conn.execute_command("FT.CREATE", "idx_no", "SCHEMA", "text", "TEXT")
    env.expect(debug_cmd(), "DUMP_SUFFIX_TRIE", "idx_no").error().contains(
        "Index does not have suffix trie"
    )

    conn.execute_command(
        "FT.CREATE", "idx_tag", "SCHEMA", "tag", "TAG", "WITHSUFFIXTRIE"
    )
    env.expect(debug_cmd(), "DUMP_SUFFIX_TRIE", "idx", "tag_no").error().contains(
        "Could not find given field in index spec"
    )
    env.expect(
        debug_cmd(), "DUMP_SUFFIX_TRIE", "idx", "tag_no", "tag_yes"
...
def testContainsMixedWithSuffix(env):
    ...
    env.expect("ft.search", "idx", "@t2:*ell*", "NOCONTENT").error().contains(
...
def test_params(env):
    ...
    env.expect("ft.search", "idx", "$prefix*", "PARAMS", 2, "prefix", "wor").equal(
        [1, "doc1", ["t", "world"]]
    )
    env.expect("ft.search", "idx", "*$contains*", "PARAMS", 2, "contains", "orl").equal(
        [1, "doc1", ["t", "world"]]
    )
    env.expect("ft.search", "idx", "*$suffix", "PARAMS", 2, "suffix", "rld").equal(
        [1, "doc1", ["t", "world"]]
    )
