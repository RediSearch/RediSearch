...
def testProfileSearch(env):
    ...
    env.expect("ft.profile", "profile", "idx", "*", "nocontent").error().contains(
        "No `SEARCH` or `AGGREGATE` provided"
    )
    ...
    # test EMPTY
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "query", "redis", "nocontent"
    )
    env.assertEqual(actual_res[1][1][0][3], ["Type", "EMPTY", "Counter", 0])

    # test single term
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "query", "hello", "nocontent"
    )
    env.assertEqual(
        actual_res[1][1][0][3],
    ...
    # test UNION
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "query", "hello|world", "nocontent"
    )
    expected_res = [
        "Type",
        "UNION",
    ...
    # test PREFIX
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "query", "hel*", "nocontent"
    )
    expected_res = [
        "Type",
        "UNION",
    ...
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "query", "hello", "inkeys", 1, "1"
    )
    expected_res = [
        "Type",
        "INTERSECT",
...
def testProfileSearchLimited(env):
    ...
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "limited", "query", "%hell% hel*"
    )
    expected_res = [
        "Type",
        "INTERSECT",
...
def testProfileCursor(env):
    ...
    env.cmd("ft.create", "idx", "SCHEMA", "t", "text")
    env.expect(
        "ft.profile", "idx", "search", "bad_arg1", "bad_arg2"
    ).error()  # This also should not crash nor fail on memory checks
    env.expect(
        "ft.profile", "idx", "aggregate", "query", "*", "WITHCURSOR"
    ).error().contains("FT.PROFILE does not support cursor")
...
def testProfileErrors(env):
    ...
    env.expect("ft.profile", "idx", "SEARCH").error().contains(
        "wrong number of arguments"
    )
    env.expect("ft.profile", "idx", "SEARCH", "QUERY").error().contains(
        "wrong number of arguments"
    )
    # wrong `query` type
    env.expect("ft.profile", "idx", "redis", "QUERY", "*").error().contains(
        "No `SEARCH` or `AGGREGATE` provided"
    )
    # miss `QUERY` keyword
    if not env.isCluster():
        env.expect("ft.profile", "idx", "SEARCH", "FIND", "*").error().contains(
...
def testProfileNegativeNumeric():
    ...
    values_ranges = [
        {"min_val": -docs, "title": "only negatives"},
    ...
    for values_range in values_ranges:
        ...
        actual_res = conn.execute_command(
            "ft.profile", "idx", "search", "query", "@n:[-inf +inf]", "nocontent"
        )
        ...
        def extract_child_range(child: dict):
            ...
            env.assertEqual(
                range_dict["max"],
                range_dict["min"] + child["Size"] - 1,
                message=f"{title}: range_max should equal range_min + (range_size - 1)",
            )
            return range_dict
        ...
        env.assertEqual(
            float(actual_min_val),
            min_val,
            message=f"{title}: The first child iterator should contain the tree min val",
        )
        ...
        env.assertEqual(
            max_val,
            range_last,
            message=f"{title}: The max value of the last child should equal the max val of the tree",
        )
        env.cmd("flushall")
...
def testProfileTag(env):
    ...
    # tag profile
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "query", "@t:{foo}", "nocontent"
    )
    env.assertEqual(
        actual_res[1][1][0][3], ["Type", "TAG", "Term", "foo", "Counter", 2, "Size", 2]
    )
...
def testProfileVector(env):
    ...
    env.expect(
        "FT.CREATE idx SCHEMA v VECTOR FLAT 6 TYPE FLOAT32 DIM 2 DISTANCE_METRIC L2 t TEXT"
    ).ok()
    ...
    env.assertEqual(
        actual_profile["Result processors profile"][1], expected_vecsim_rp_res
    )
    env.assertEqual(
        to_dict(env.cmd(debug_cmd(), "VECSIM_INFO", "idx", "v"))["LAST_SEARCH_MODE"],
        "STANDARD_KNN",
    )
    ...
    actual_res = conn.execute_command(
        ...
        "search",
        "query",
        "(@t:hello other)=>[KNN 2 @v $vec HYBRID_POLICY BATCHES]",
        "SORTBY",
        "__v_score",
        "PARAMS",
        "2",
    ...
    actual_res = conn.execute_command(
        ...
        "search",
        "query",
        "(@t:hello other)=>[KNN 2 @v $vec HYBRID_POLICY BATCHES BATCH_SIZE 100]",
        "SORTBY",
        "__v_score",
        "PARAMS",
        "2",
    ...
    actual_res = conn.execute_command(
        ...
        "search",
        "query",
        "(@t:hello other)=>[KNN 2 @v $vec BATCH_SIZE 100]",
        "SORTBY",
        "__v_score",
        "PARAMS",
        "2",
...
def testResultProcessorCounter(env):
    ...
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "query", "foo|bar", "limit", "0", "0"
    )
    env.assertEqual(actual_res[0], [2])
    res = [["Type", "Index", "Counter", 2], ["Type", "Counter", "Counter", 1]]
...
def testProfileMaxPrefixExpansion(env):
    ...
    actual_res = conn.execute_command(
        "ft.profile", "idx", "search", "query", "foo*", "limit", "0", "0"
    )
    env.assertEqual(
        actual_res[1][1][0][3][6:8], ["Warning", "Max prefix expansion reached"]
    )
...
def TimedOutWarningtestCoord(env):
    ...
    if env.protocol == 2:
        for shard_profile in res[1][1]:
            env.assertEqual(
                to_dict(shard_profile)["Warning"], "Timeout limit was reached"
            )
    else:
        for shard_profile in res["Profile"]["Shards"]:
