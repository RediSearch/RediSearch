...
def testProfileSearch(env):
  ...
  env.assertEqual(actual_res[1][5], ['Iterators profile', ['Type', 'WILDCARD', 'Number of reading operations', 2]])

  # test EMPTY
  actual_res = conn.execute_command('ft.profile', 'idx', 'search', 'query', 'redis', 'nocontent')
  env.assertEqual(actual_res[1][5], ['Iterators profile', ['Type', 'EMPTY', 'Number of reading operations', 0]])

  # test single term
  actual_res = conn.execute_command('ft.profile', 'idx', 'search', 'query', 'hello', 'nocontent')
  env.assertEqual(actual_res[1][5], ['Iterators profile', ['Type', 'TEXT', 'Term', 'hello', 'Number of reading operations', 1, 'Estimated number of matches', 1]])
  ...
  expected_res_d2 = ['Iterators profile', ['Type', 'INTERSECT', 'Number of reading operations', 1, 'Child iterators',
                      ...
                      ['Type', 'TEXT', 'Term', 'hello', 'Number of reading operations', 1, 'Estimated number of matches', 1],
                      ['Type', 'TEXT', 'Term', 'hello', 'Number of reading operations', 1, 'Estimated number of matches', 1]]]
  env.assertEqual(actual_res[1][5], expected_res if dialect == 1 else expected_res_d2)
...
def testProfileAggregate(env):
  ...
  env.assertEqual(actual_res[1][6], expected_res)
...
def testProfileVector(env):
  ...
  expected_iterators_res = ['Iterators profile', ['Type', 'VECTOR', 'Number of reading operations', 3, 'Vector search mode', 'STANDARD_KNN']]
  expected_vecsim_rp_res = ['Type', 'Metrics Applier', 'Results processed', 3]
  env.assertEqual(actual_res[0], [3, '4', '2', '1'])
  env.assertEqual(actual_res[1][5], expected_iterators_res)
  env.assertEqual(actual_res[1][6][2], expected_vecsim_rp_res)
  ...
  expected_iterators_res = ['Iterators profile', ['Type', 'METRIC - VECTOR DISTANCE', 'Number of reading operations', 2, 'Vector search mode', 'RANGE_QUERY']]
...
# Test with hybrid query variations
  ...
  actual_res = conn.execute_command('ft.profile', 'idx', 'search', 'query', '(@t:hello world)=>[KNN 3 @v $vec]',
                                    'SORTBY', '__v_score', 'PARAMS', '2', 'vec', 'aaaaaaaa', 'nocontent')
  expected_iterators_res = ['Iterators profile', ['Type', 'VECTOR', 'Number of reading operations', 2, 'Vector search mode', 'HYBRID_ADHOC_BF', 'Child iterator',
  ...
  expected_iterators_res = ['Iterators profile', ['Type', 'VECTOR', 'Number of reading operations', 3,
                                                  'Vector search mode', 'HYBRID_BATCHES', 'Batches number', 2,
                                                  'Largest batch size', 4, 'Largest batch iteration (zero based)', 0, 'Child iterator',
  ...
  expected_iterators_res = ['Iterators profile', ['Type', 'VECTOR', 'Number of reading operations', 0,
                                                  'Vector search mode', 'HYBRID_BATCHES_TO_ADHOC_BF', 'Batches number', 2,
                                                  'Largest batch size', 13, 'Largest batch iteration (zero based)', 1, 'Child iterator',
                                                  ['Type', 'INTERSECT', 'Number of reading operations', 2, 'Child iterators',
  ...
  actual_res = conn.execute_command('ft.profile', 'idx', 'search', 'query', '(@t:hello other)=>[KNN 2 @v $vec HYBRID_POLICY BATCHES]',
                                    'SORTBY', '__v_score', 'PARAMS', '2', 'vec', '????????', 'nocontent')
  expected_iterators_res = ['Iterators profile', ['Type', 'VECTOR', 'Number of reading operations', 0, 'Vector search mode', 'HYBRID_BATCHES',
                                                  'Batches number', 13, 'Largest batch size', 20001, 'Largest batch iteration (zero based)', 12, 'Child iterator',
  ...
  expected_iterators_res = ['Iterators profile', ['Type', 'VECTOR', 'Number of reading operations', 0,
                                                  'Vector search mode', 'HYBRID_BATCHES', 'Batches number', 200, 'Largest batch size', 100,
                                                  'Largest batch iteration (zero based)', 0, 'Child iterator',
  ...
  expected_iterators_res = ['Iterators profile', ['Type', 'VECTOR', 'Number of reading operations', 0,
                                                  'Vector search mode', 'HYBRID_BATCHES_TO_ADHOC_BF', 'Batches number', 2,
                                                  'Largest batch size', 100, 'Largest batch iteration (zero based)', 0, 'Child iterator',
                                                  ['Type', 'INTERSECT', 'Number of reading operations', 2, 'Child iterators',
                                                   ['Type', 'TEXT', 'Term', 'hello', 'Number of reading operations', 5, 'Estimated number of matches', 10000],
                                                   ['Type', 'TEXT', 'Term', 'other', 'Number of reading operations', 3, 'Estimated number of matches', 10000]]]]
  env.assertEqual(actual_res[1][5], expected_iterators_res)
...
def testResultProcessorCounter(env):
  ...
  env.assertEqual(actual_res[1][6], res)
...
def testProfileMaxPrefixExpansion(env):
  ...
  env.assertEqual(actual_res[1][5][1][6:8], ['Warning', 'Max prefix expansions limit was reached'])
...
def TimedOutWarningtestCoord(env):
  ...
  # Verify 'Internal cursor reads' is present in each shard's profile (cluster AGGREGATE only)
  for shard_profile in get_shards_profile(env, res):
    ...
    env.assertGreater(shard_profile['Internal cursor reads'], 0, message=f"full reply output: {res}")
    ...
    try:
      ...
      if env.isCluster():
        ...
        for key, shard in shards.items():
          if key == 'Coordinator':
            break
          shards_iter_profile.append(shard['Iterators profile'][0])
      else:
        shards = [res['profile']]
        shards_iter_profile = [res['profile']['Iterators profile'][0]]

      env.assertGreater(len(shards_iter_profile), 0, message=scenario_message)
      found = False
      # Check at least one shard has the expected search mode
      for iter_profile in shards_iter_profile:
