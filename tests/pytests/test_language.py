...
def testHashIndexLanguage(env):
    ...
    res = env.cmd(
        "FT.SEARCH", "idx_it", "cherry", "language", "english", "SORTBY", "word", "DESC"
    )
    ...
    res = env.cmd(
        "FT.SEARCH", "idx_it", "orange", "language", "english", "SORTBY", "word", "DESC"
    )
    ...
    env.assertEqual(
        res, [2, "{word}:4", ["word", "oranges"], "{word}:3", ["word", "orange"]]
    )
    ...
    res = env.cmd(
        "FT.SEARCH", "idx_en", "orange", "language", "english", "SORTBY", "word", "ASC"
    )
    env.assertEqual(res, expected)
    res = env.cmd(
        "FT.SEARCH", "idx_en", "oranges", "language", "english", "SORTBY", "word", "ASC"
    )
    env.assertEqual(res, expected)
...
def testJsonIndexLanguage(env):
    ...
    expected = [
        2,
        "{word}:1",
        ["word", "arancia", "$", '{"word":"arancia"}'],
        "{word}:2",
        ["word", "arance", "$", '{"word":"arance"}'],
    ]
    ...
    expected = [
        2,
        "{word}:4",
        ["word", "oranges", "$", '{"word":"oranges"}'],
        "{word}:3",
        ["word", "orange", "$", '{"word":"orange"}'],
    ]
    res = env.cmd(
        "FT.SEARCH",
        "idx_it",
    ...
    # Create index - language English per index (by default)
    env.cmd(
        "FT.CREATE", "idx_en", "ON", "JSON", "SCHEMA", "$.word", "AS", "word", "TEXT"
    )
    # Wait for index to be created
    waitForIndex(env, "idx_en")
    ...
    expected = [
        2,
        "{word}:6",
        ["word", "cherries", "$", '{"word":"cherries"}'],
        "{word}:5",
        ["word", "cherry", "$", '{"word":"cherry"}'],
    ]
    for word in ["cherry", "cherries"]:
        res = env.cmd(
            "FT.SEARCH", "idx_en", word, "language", "english", "SORTBY", "word", "ASC"
        )
        env.assertEqual(res, expected)
...
def testHashIndexLanguageField(env):
    ...
    conn.execute_command(
        "HSET", "{word}:1", "word", "arancia", "__lang", "italian"
    )  # oranges
    conn.execute_command(
        "HSET", "{word}:2", "word", "arance", "__lang", "italian"
    )  # oranges
    conn.execute_command("HSET", "{word}:3", "word", "ciliegia")  # cherry
    conn.execute_command("HSET", "{word}:4", "word", "ciliegie")  # cherries
    conn.execute_command("HSET", "{word}:5", "word", "orange", "__lang", "english")
    conn.execute_command("HSET", "{word}:6", "word", "oranges", "__lang", "english")
    conn.execute_command("HSET", "{word}:7", "word", "cherry", "__lang", "english")
    conn.execute_command("HSET", "{word}:8", "word", "cherries", "__lang", "english")
    conn.execute_command(
        "HSET", "{word}:9", "word", "xaranja", "__lang", "unexistent_language"
    )
    conn.execute_command(
        "HSET", "{word}:10", "word", "fragola", "__lang", "italian"
    )  # strawberry
    ...
    for word in ["cherry", "cherries"]:
        res = env.cmd(
            "FT.SEARCH", "idx_it", word, "SORTBY", "word", "DESC", "LANGUAGE", "english"
        )
        env.assertEqual(res, expected)
    ...
    for word in ["arancia", "arance", "ciliegia", "ciliegie"]:
        ...
        res2 = env.cmd(
            "FT.SEARCH", "idx_en", word, "SORTBY", "word", "DESC", "LANGUAGE", "english"
        )
    ...
    for idx in ["idx_lang", "idx_def_lang"]:
        ...
        res = env.cmd("FT.SEARCH", idx, "@__lang:{Italian}", "SORTBY", "word")
        env.assertEqual(
            res,
            [
                3,
        ...
        res = env.cmd(
            "FT.SEARCH", idx, "@__lang:{Italian} arance", "SORTBY", "word", "ASC"
        )
        env.assertEqual(
            res, [2, "{word}:2", ["word", "arance"], "{word}:1", ["word", "arancia"]]
        )

        # Search words in English - only English words should be returned
        res = env.cmd("FT.SEARCH", idx, "@__lang:{english}", "SORTBY", "word")
        env.assertEqual(
            res,
            [
                4,
    ...
    for word in ["arancia", "arance", "ciliegia", "ciliegie", "cherry", "cherries"]:
        res1 = env.cmd(
            "FT.SEARCH", "idx_it", word, "NOCONTENT", "SORTBY", "word", "DESC"
        )
        res2 = env.cmd(
            "FT.SEARCH",
            "idx_it_no_lang_field",
...
def testJsonIndexLanguageField(env):
    ...
    # Create sample data in Italian
    conn.execute_command(
        "JSON.SET", "{word}:1", "$", r'{"word":"arancia", "__lang": "italian"}'
    )  # orange
    conn.execute_command(
        "JSON.SET", "{word}:2", "$", r'{"word":"arance", "__lang": "italian"}'
    )  # oranges
    conn.execute_command("JSON.SET", "{word}:3", "$", r'{"word":"ciliegia"}')  # cherry
    conn.execute_command(
        "JSON.SET", "{word}:4", "$", r'{"word":"ciliegie"}'
    )  # cherries
    conn.execute_command(
        "JSON.SET", "{word}:5", "$", r'{"word":"orange", "__lang": "english"}'
    )
    conn.execute_command(
        "JSON.SET", "{word}:6", "$", r'{"word":"oranges", "__lang": "english"}'
    )
    conn.execute_command(
        "JSON.SET", "{word}:7", "$", r'{"word":"cherry", "__lang": "english"}'
    )
    conn.execute_command(
        "JSON.SET", "{word}:8", "$", r'{"word":"cherries", "__lang": "english"}'
    )
    conn.execute_command(
        "JSON.SET",
        "{word}:9",
    ...
    conn.execute_command(
        "JSON.SET", "{word}:10", "$", r'{"word":"fragola", "__lang": "italian"}'
    )  # strawberry
    ...
    for word in ["arancia", "arance"]:
        res = env.cmd(
            "FT.SEARCH", "idx_it", word, "SORTBY", "word", "DESC", "NOCONTENT"
        )
        env.assertEqual(res, expected)
    ...
    # default language of the index
    # It should return 1 result using invalid stemming in Italian
    res = env.cmd("FT.SEARCH", "idx_it", "cherry", "SORTBY", "word", "ASC", "NOCONTENT")
    expected = [1, "{word}:7"]
    env.assertEqual(res, expected)
    ...
    # But in this case, the stemming in Italian, generates matching words
    # in English
    res = env.cmd("FT.SEARCH", "idx_it", "orange", "SORTBY", "word", "ASC", "NOCONTENT")
    ...
    for word in ["ciliegia", "ciliegie"]:
        res = env.cmd("FT.SEARCH", "idx_it", word, "SORTBY", "word", "ASC", "NOCONTENT")
        env.assertEqual(res, expected)
    ...
    for word in ["cherry", "cherries"]:
        res = env.cmd(
            "FT.SEARCH", "idx_en", word, "SORTBY", "word", "DESC", "NOCONTENT"
        )
        env.assertEqual(res, expected)
    ...
    for idx in ["idx_lang", "idx_def_lang"]:
        # Search words in Italian - only Italian words should be returned
        res = env.cmd(
            "FT.SEARCH", idx, "@__lang:{Italian}", "SORTBY", "word", "NOCONTENT"
        )
        ...
        # Search words in English - only English words should be returned
        res = env.cmd(
            "FT.SEARCH", idx, "@__lang:{english}", "SORTBY", "word", "NOCONTENT"
        )
...
def testLanguageInfo(env):
    ...
    for language in languages:
        ...
        idx = {
            index_definition[i]: index_definition[i + 1]
