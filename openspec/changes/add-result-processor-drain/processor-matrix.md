...
| --- | --- | --- | --- | --- |
| `RP_INDEX` | Source over the query iterator; optional async disk metadata queue | Atomically close the source and return EOF. Do not advance/revalidate the iterator, submit I/O, poll, or wait. A result already claimed by `Next` remains on that path. | Add a terminal drain flag checked at safe points in both sync and async `Next`. Avoid touching async queues from Drain. | No new Drain error. Race before read, during iterator read, async job not started/pending/ready, and cleanup of every queued item. |
